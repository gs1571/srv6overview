...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-lsr-ospfv3-srv6-extensions-15
...
**Keywords:** OSPF, OSPFv3, Segment Routing, SRv6

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-lsr-ospfv3-srv6-extensions-15)
- [alternate](https://dx.doi.org/10.17487/rfc9513)
...
- [2. SRv6 Capabilities TLV](#capability)
- [3. Advertisement of Supported Algorithms](#algorithm)
- [4. Advertisement of Maximum SRv6 SID Depths](#SRH-limits)
  - [4.1. Maximum Segments Left MSD Type](#section-4.1)
  - [4.2. Maximum End Pop MSD Type](#section-4.2)
  - [4.3. Maximum H.Encaps MSD Type](#section-4.3)
  - [4.4. Maximum End D MSD Type](#section-4.4)
- [5. SRv6 SIDs and Reachability](#SRv6-SID)
  - [5.1. SRv6 Flexible Algorithm](#SRV6FA)
- [6. Advertisement of Anycast Property](#ANYCAST)
- [7. SRv6 Locator LSA](#LOCLSA)
  - [7.1. SRv6 Locator TLV](#LOCTLV)
  - [7.2. SRv6 Locator Sub-TLVs](#LOCSUBTLVS)
- [8. Advertisement of SRv6 End SIDs](#END)
- [9. Advertisement of SRv6 SIDs Associated with Adjacencies](#SRv6-Neighbor)
  - [9.1. SRv6 End.X SID Sub-TLV](#P2P)
  - [9.2. SRv6 LAN End.X SID Sub-TLV](#LAN)
- [10. SRv6 SID Structure Sub-TLV](#SRSTRUCTTLV)
- [11. Advertising Endpoint Behaviors](#AdvtEndBeh)
- [12. Security Considerations](#Security)
- [13. IANA Considerations](#IANA)
  - [13.1. OSPF Router Information TLVs](#IANARI)
  - [13.2. OSPFv3 LSA Function Codes](#IANALSA)
  - [13.3. OSPFv3 Prefix Options](#IANAPO)
  - [13.4. OSPFv3 SRv6 Capabilities TLV Flags](#IANASRV6CAP)
  - [13.5. OSPFv3 SRv6 End SID Sub-TLV Flags](#IANASRV6END)
  - [13.6. OSPFv3 SRv6 Adjacency SID Sub-TLV Flags](#IANASRV6ENDX)
  - [13.7. OSPFv3 Extended-LSA Sub-TLVs](#IANAELSTLVS)
  - [13.8. OSPFv3 SRv6 Locator LSA TLVs](#IANALLTLVS)
  - [13.9. OSPFv3 SRv6 Locator LSA Sub-TLVs](#IANALLSTLVS)
  - [13.10. OSPFv3 Extended-LSA Sub-TLVs](#IANAOSPFV3EXTLSA)
...
The Segment Routing (SR) architecture [RFC8402](#RFC8402)
...
SR can be instantiated on the IPv6 data plane through
      the use of the Segment Routing Header (SRH) defined in [RFC8754](#RFC8754). SR instantiation on the IPv6 data plane
      is referred to as SRv6.

The network programming paradigm for SRv6 is specified in [RFC8986](#RFC8986). It describes how any behavior can be bound to a SID
...
This document specifies OSPFv3 extensions to support SRv6
      capabilities as defined in [RFC8986](#RFC8986), [RFC8754](#RFC8754), and [RFC9259](#RFC9259). The extensions include
      ...
      Familiarity with [RFC8986](#RFC8986) is necessary to understand the
...
2. Several sub-TLVs to advertise various SRv6 Maximum SID
          Depths.
...
The SRv6 Capabilities TLV is used by an OSPFv3 router to advertise
      its support for the SR Segment Endpoint Node [RFC8754](#RFC8754)
      functionality along with its SRv6-related capabilities. This is an
      optional top-level TLV of the OSPFv3 Router Information LSA [RFC7770](#RFC7770) that **MUST** be advertised by an SRv6-enabled
      router.
...
**Length:**
  2-octet field. The total length (in octets) of the value
            portion of the TLV, including nested sub-TLVs.

**Reserved:**
  2-octet field. It **MUST** be set to 0 on transmission and
...
**Flags:**
  ...
  **O-flag:**
    If set, then the router is capable of supporting the
                  O-flag in the SRH flags, as specified in [RFC9259](#RFC9259).

  **Other flags are not defined and are reserved for future use.
              They **MUST** be set to 0 on transmission and
              **MUST** be ignored on receipt.**
...
An SRv6-enabled OSPFv3 router advertises its algorithm support using
      the SR-Algorithm TLV defined in [RFC8665](#RFC8665) and as described in
      [RFC8666](#RFC8666).
...
[RFC8476](#RFC8476) defines the means to advertise node- and link-specific values for Maximum SID Depth (MSD) types. Node MSDs are
      advertised using the Node MSD TLV in the OSPFv3 Router Information LSA
      [RFC7770](#RFC7770), while Link MSDs are advertised using the Link
      MSD sub-TLV of the Router-Link TLV [RFC8362](#RFC8362). The format
      of the MSD types for OSPFv3 is defined in [RFC8476](#RFC8476).

The MSD types for SRv6 that are defined in [RFC9352](#RFC9352) for IS-IS are also used by OSPFv3. These MSD types
...
The Maximum End Pop MSD Type signals the maximum number of SIDs in
        ...
        of the SRH" or "Ultimate Segment Pop (USP) of the SRH", which are flavors defined in
        [RFC8986](#RFC8986). If the advertised value is zero or
...
The Maximum H.Encaps MSD Type signals the maximum number of SIDs
        ...
        [RFC8986](#RFC8986). If the advertised value is zero or no value
...
The Maximum End D MSD Type specifies the maximum number of SIDs
        ...
        End.X with USD as defined in [RFC8986](#RFC8986). If the
        ...
        of the inner packet when the outer IPv6 header contains an SRH.

<a name="SRv6-SID"></a>
# SRv6 SIDs and Reachability

An SRv6 SID is 128 bits and consists of locator,
      function, and argument parts as described in [RFC8986](#RFC8986).
...
Locators **MUST** be advertised within an SRv6 Locator TLV (see [Section 7.1](#LOCTLV)) using an SRv6 Locator LSA (see [Section 7](#LOCLSA)). The SRv6 Locator LSA is introduced instead of
      reusing the respective Extended Prefix LSAs [RFC8362](#RFC8362) for
      ...
      and the SRv6 Locator reachability advertisements).
...
Forwarding entries for the locators advertised in the SRv6 Locator
      ...
      existing OSPFv3 LSA types: Intra-Area, Inter-Area, External, and Not-So-Stubby Area (NSSA).
      The advertisement and propagation of the SRv6 Locator LSAs also follow
      the OSPFv3 [RFC5340](#RFC5340) specifications for the respective
      ...
      forwarding plane follows the OSPFv3 [RFC5340](#RFC5340)
      specifications for the respective LSA types.

Locators associated with algorithms 0 and 1 (refer to [RFC8402](#RFC8402)) **SHOULD** also be advertised using Extended LSA types with extended TLVs [RFC8362](#RFC8362) so that
      routers that do not support SRv6 will install a forwarding entry for
      SRv6 traffic matching those locators. When operating in Extended LSA
      sparse-mode [RFC8362](#RFC8362), these locators **SHOULD** also be
      advertised using Legacy LSAs [RFC5340](#RFC5340).

When SRv6 Locators are also advertised as Intra-Area-Prefix-LSAs
...
In cases where a locator advertisement is received both in a prefix
      ...
      Prefix TLVs using Extended LSAs) and an SRv6 Locator TLV, the
      prefix reachability advertisement in the Legacy LSA or Extended
      LSA **MUST** be preferred over the advertisement in the SRv6 Locator TLV
      when installing entries in the forwarding plane. This prevents
...
SRv6 SIDs are advertised as sub-TLVs in the SRv6 Locator TLV except
      for SRv6 End.X SIDs and LAN End.X SIDs, which are associated with a specific
...
SRv6 SIDs received from other OSFPv3 routers are not directly
      ...
      to SRv6 SIDs depends upon the existence of a covering locator.
...
- Another prefix advertised via Legacy or Extended LSA
          advertisement is the longest prefix match.

<a name="SRV6FA"></a>
## SRv6 Flexible Algorithm

[RFC9350](#RFC9350) specifies IGP Flexible Algorithm
        mechanisms for OSPFv3. 
[RFC9350](#RFC9350)
        explains SRv6 forwarding for Flexible Algorithms, and analogous
        procedures apply for supporting SRv6 Flexible Algorithms using OSPFv3.
        When the algorithm value that is advertised in the SRv6 Locator TLV
        (refer to [Section 7.1](#LOCTLV)) represents a Flexible Algorithm,
        the procedures described in [RFC9350](#RFC9350)
        are followed for the programming of those specific SRv6 Locators.

Locators associated with Flexible Algorithms **SHOULD NOT** be
        ...
        Algorithm forwarding (i.e., algorithm 0).

The procedures for OSPFv3 Flexible Algorithm for SR-MPLS, as specified in [RFC9350](#RFC9350), also apply for SRv6; these procedures include a) ASBR reachability,
b) inter-area, external, and NSSA prefix advertisements, and c) the use of
those prefix advertisements in Flexible Algorithm route computation.
...
The AC-bit (value 0x80) in the OSPFv3 PrefixOptions field [RFC5340](#RFC5340) is defined
...
The AC-bit and the N-bit **MUST NOT** both be set. If the N-bit and
      AC-bit are both set in the prefix/SRv6 Locator advertisement, the receiving
...
All the nodes advertising the same anycast SRv6 Locator **MUST**
      ...
      Failure to do so may result in traffic being dropped or misrouted.

The PrefixOptions field is common to the prefix reachability
      advertisements (i.e., the base OSPFv3 prefix LSA types defined in [RFC5340](#RFC5340), the OSPFv3 Extended Prefix TLV types defined in
      [RFC8362](#RFC8362)), and the SRv6 Locator TLV advertisements
      specified in [Section 7.1](#LOCTLV) of this document. When a router
      ...
      the prefix reachability advertisement **MUST** be preferred.
...
The format of the TLVs within the body of the SRv6 Locator LSA is the
      same as the format used by [RFC3630](#RFC3630). The variable TLV
...
The Length field defines the length of the value portion in octets
      ...
      TLV. The padding is composed of zeros.

<a name="LOCTLV"></a>
## SRv6 Locator TLV

The SRv6 Locator TLV is a top-level TLV of the SRv6 Locator LSA
...
**Length:**
  2-octet field. The total length (in octets) of the
              value portion of the TLV, including nested sub-TLVs.

**Route Type:**
  1-octet field. The type of the locator route. The
...
**Algorithm:**
  1-octet field. The algorithm associated with the
              SRv6 Locator. Algorithm values are defined in the "IGP Algorithm
              Types" registry [RFC8665](#RFC8665).

**Locator Length:**
  1-octet field. Specifies the length of the
              locator prefix as the number of locator bits from the range
              (1-128).

**PrefixOptions:**
  1-octet field. Specifies the prefix options
              bits/flags as specified in [RFC5340](#RFC5340) and further
              extended by [RFC8362](#RFC8362) and [Section 6](#ANYCAST)
              of this document.

**Metric:**
  4-octet field. The metric value associated with the
              SRv6 Locator. The metric value of 0xFFFFFFFF **MUST** be considered as
              unreachable.

**Locator:**
  1-16 octets. This field encodes the
            advertised SRv6 Locator as an IPv6 Prefix as specified in [RFC5340](#RFC5340).

**Sub-TLVs:**
  Used to advertise sub-TLVs that provide additional
...
The following OSPFv3 Extended-LSA sub-TLVs corresponding to the
        ...
        SRv6 Locator TLV using code points as specified in [Section 13.9](#IANALLSTLVS):

- IPv6-Forwarding-Address sub-TLV [RFC8362](#RFC8362)
- Route-Tag sub-TLV [RFC8362](#RFC8362)
- Prefix Source OSPF Router-ID sub-TLV [RFC9084](#RFC9084)
- Prefix Source Router Address sub-TLV [RFC9084](#RFC9084)

<a name="END"></a>
# Advertisement of SRv6 End SIDs

The SRv6 End SID sub-TLV is a sub-TLV of the SRv6 Locator TLV in the
      SRv6 Locator LSA (defined in [Section 7](#LOCLSA)). It is used to
      ...
      advertised as described in [Section 9](#SRv6-Neighbor). Every
      SRv6-enabled OSPFv3 router **SHOULD** advertise at least one SRv6 SID
      associated with an End behavior for itself as specified in [RFC8986](#RFC8986), although it **MAY** omit doing so if that node is not
      going to be used as a Segment Endpoint (e.g., for TE or Topology Independent Loop-Free Alternate (TI-LFA)) by any
      SR Source Node.

SRv6 End SIDs inherit the algorithm from the parent locator. The SRv6
      End SID **MUST** be allocated from its associated locator. SRv6 End SIDs
      that are NOT allocated from the associated locator **MUST** be ignored.

The router **MAY** advertise multiple instances of the SRv6 End SID
...
**Flags:**
  1-octet field. Specifies the flags associated
          with the SID. No flags are currently defined, and this field
          **MUST** be set to 0 on transmission and
          **MUST** be ignored on receipt.

**Reserved:**
  1-octet field. It **MUST** be set to 0 on transmission and
            **MUST** be ignored on receipt.

**Endpoint Behavior:**
  2-octet field. The Endpoint behavior code point for this SRv6 SID as defined in [RFC8986](#RFC8986).
            Supported behavior values for this sub-TLV are defined in [Section 11](#AdvtEndBeh) of this document. Unsupported or unrecognized
            behavior values are ignored by the receiver.

**SID:**
  16-octet field. This field encodes the advertised SRv6
            SID.
...
<a name="SRv6-Neighbor"></a>
# Advertisement of SRv6 SIDs Associated with Adjacencies

The SRv6 Endpoint behaviors defined in [RFC8986](#RFC8986)
      ...
      like OSPFv3) is the End.X behavior, which is an instruction to forward to
      a specific neighbor on a specific link. These SRv6 SIDs and others that
      are defined in [RFC8986](#RFC8986), which are specific to links or
...
Therefore, SRv6 SIDs that are specific to a particular neighbor, such
      as End.X, are not advertised as a sub-TLVs of the SRv6 Locator TLV. Instead, they are advertised via two different optional sub-TLVs of the E-Router-Link TLV defined in
      [RFC8362](#RFC8362):

**SRv6 End.X SID sub-TLV:**
  Used for OSPFv3 adjacencies over
...
**SRv6 LAN End.X SID sub-TLV:**
  Used for OSPFv3 adjacencies on
...
Every SRv6-enabled OSPFv3 router **SHOULD** instantiate at least
      one unique SRv6 End.X SID corresponding to each of its neighbors,
      although it **MAY** omit doing so if features like TE or
      TI-LFA that require End.X SID
      ...
      E-Router-Link TLV for a single link.
...
All End.X and LAN End.X SIDs **MUST** be subsumed by the subnet of a
      ...
      OSPFv3 router in an SRv6 Locator TLV. End.X SIDs that do not meet this
...
**Endpoint Behavior:**
  2-octet field. The Endpoint behavior code point for this SRv6 SID as defined in [RFC8986](#RFC8986).
              Supported behavior values for this sub-TLV are defined in [Section 11](#AdvtEndBeh) of this document. Unsupported or
              unrecognized behavior values are ignored by the receiver.

**Flags:**
  1-octet field. The flags are defined as follows:

  **B-Flag:**
    Backup Flag. If set, the SID refers to a path that
  ...
  **P-Flag:**
    Persistent Flag. If set, the SID is persistently
                    ...
                    router restart and session/interface flap.

  **Other flags are not defined and are reserved for future
                use. They **MUST** be set to 0 on transmission and **MUST** be ignored
                on receipt.**


**Reserved1:**
  1-octet field. It **MUST** be set to 0 on transmission
              and **MUST** be ignored on receipt.

**Algorithm:**
  1-octet field. The algorithm associated with the
              SRv6 Locator from which the SID is allocated. Algorithm values are
              defined in the "IGP Algorithm Types" registry [RFC8665](#RFC8665).

**Weight:**
  1-octet field. Its value represents the weight of the
              End.X SID for load-balancing. The use of the weight is defined in
              [RFC8402](#RFC8402).

**Reserved2:**
  2-octet field. It **MUST** be set to 0 on transmission
              and **MUST** be ignored on receipt.
...
**Endpoint Behavior:**
  2-octet field. The code point for the
            Endpoint behavior for this SRv6 SID as defined in [RFC8986](#RFC8986).
...
**Neighbor Router-ID:**
  4-octet field. It specifies the OSPFv3
...
<a name="SRSTRUCTTLV"></a>
# SRv6 SID Structure Sub-TLV

The SRv6 SID Structure sub-TLV is used to advertise the structure of the
      SRv6 SID as defined in [RFC8986](#RFC8986). It is used as an
      optional sub-TLV of the following:

- SRv6 End SID sub-TLV (refer to [Section 8](#END))
- SRv6 End.X SID sub-TLV (refer to [Section 9.1](#P2P))
- SRv6 LAN End.X SID sub-TLV (refer to [Section 9.2](#LAN))
...
**LB Length:**
  1-octet field. SRv6 SID Locator Block length in
            bits.

**LN Length:**
  1-octet field. SRv6 SID Locator Node length in
            bits.

**Function Length:**
  1-octet field. SRv6 SID Function length in
            bits.

**Argument Length:**
  1-octet field. SRv6 SID Argument length in
            bits.
...
The SRv6 SID Structure sub-TLV **MUST NOT** appear more than once in its
      ...
      parent sub-TLV **MUST** be ignored by the receiver.
...
The SRv6 SID Structure sub-TLV is intended for informational use by
      ...
      (as defined in [RFC8754](#RFC8754)) for forwarding packets. As an
...
- Validation of SRv6 SIDs being instantiated in the network and
        ...
        [RFC9514](#RFC9514) and then monitored for
...
Endpoint behaviors are defined in [RFC8986](#RFC8986). The
      code points for the Endpoint behaviors are defined in the "SRv6 Endpoint
      Behaviors" registry of [RFC8986](#RFC8986). This section lists the
      Endpoint behaviors and their code points, which **MAY** be advertised by
      OSPFv3 and the sub-TLVs in which each type **MAY** appear.

**Table: SRv6 Endpoint Behaviors in OSPFv3**
...
This document introduces extensions to the OSPFv3 protocol and, as such,
      does not affect existing security considerations for OSPFv3 as
      documented in [RFC5340](#RFC5340). [RFC7166](#RFC7166)
...
Reception of a malformed TLV or sub-TLV **SHOULD** be counted and/or logged
      in a rate-limited manner for further analysis.

This document describes the OSPFv3 extensions required to support
      SR over an IPv6 data plane. The security considerations for
      SR are discussed in [RFC8402](#RFC8402). [RFC8986](#RFC8986) defines the SRv6 Network Programming concept and
      specifies the main SR behaviors to enable the creation of
...
| --- | --- | --- |
| 20 | SRv6 Capabilities | RFC 9513, [Section 2](#capability) |

<a name="IANALSA"></a>
## OSPFv3 LSA Function Codes

IANA has allocated the following code point in
        the "OSPFv3 LSA Function Codes" registry within the "Open Shortest Path First v3 (OSPFv3) Parameters"
        registry group:

| Value | LSA Function Code Name | Reference |
| --- | --- | --- |
| 42 | SRv6 Locator LSA | RFC 9513, [Section 7](#LOCLSA) |
...
| --- | --- | --- |
| 0x80 | AC-bit | RFC 9513, [Section 6](#ANYCAST) |

<a name="IANASRV6CAP"></a>
## OSPFv3 SRv6 Capabilities TLV Flags

IANA has created a new subregistry named "OSPFv3 SRv6
        Capabilities TLV Flags" within the "Open Shortest Path First v3 (OSPFv3) Parameters"
        ...
        The registration procedure is "Standards Action" as defined in [RFC8126](#RFC8126).

The following assignment has been made per this document:

| Bit | Description | Reference |
| --- | --- | --- |
| 1 | O-flag | RFC 9513, [Section 2](#capability) |

<a name="IANASRV6END"></a>
## OSPFv3 SRv6 End SID Sub-TLV Flags

IANA has created a new subregistry named "OSPFv3 SRv6
        End SID Sub-TLV Flags" within the "Open Shortest Path First v3 (OSPFv3) Parameters"
...
<a name="IANASRV6ENDX"></a>
## OSPFv3 SRv6 Adjacency SID Sub-TLV Flags

IANA has created a new subregistry named "OSPFv3 SRv6
        Adjacency SID Sub-TLV Flags" within the "Open Shortest Path First v3 (OSPFv3) Parameters"
        registry group to control the assignment of bits 0 to 7 in the Flags
        ...
        "Standards Action" as defined in [RFC8126](#RFC8126).

The following assignments have been made per this document:

| Bit | Description | Reference |
| --- | --- | --- |
| 0 | B-flag | RFC 9513, Sections [9.1](#P2P) and [9.2](#LAN) |
| 1 | S-flag | RFC 9513, Sections [9.1](#P2P) and [9.2](#LAN) |
| 2 | P-flag | RFC 9513, Sections [9.1](#P2P) and [9.2](#LAN) |

<a name="IANAELSTLVS"></a>
## OSPFv3 Extended-LSA Sub-TLVs

IANA has allocated the following code points
...
| --- | --- | --- | --- |
| 30 | SRv6 SID Structure | Y | RFC 9513, [Section 10](#SRSTRUCTTLV) |
| 31 | SRv6 End.X SID | Y | RFC 9513, [Section 9.1](#P2P) |
| 32 | SRv6 LAN End.X SID | Y | RFC 9513, [Section 9.2](#LAN) |

<a name="IANALLTLVS"></a>
## OSPFv3 SRv6 Locator LSA TLVs

IANA has created a new subregistry named "OSPFv3 SRv6 Locator LSA
        TLVs" within the "Open Shortest Path First v3 (OSPFv3) Parameters" registry group to define top-level TLVs for the OSPFv3 SRv6
        Locator LSA. The initial
        assignments are below:

| Value | Description | Reference |
| --- | --- | --- |
| 0 | Reserved | RFC 9513 |
| 1 | SRv6 Locator | RFC 9513, [Section 7.1](#LOCTLV) |

Types in the range 0-32767 are allocated via IETF Review or IESG
        Approval [RFC8126](#RFC8126).
...
<a name="IANALLSTLVS"></a>
## OSPFv3 SRv6 Locator LSA Sub-TLVs

IANA has created a new subregistry named "OSPFv3 SRv6 Locator LSA
        Sub-TLVs" within the "Open Shortest Path First v3 (OSPFv3) Parameters" registry    
group to define sub-TLVs at any level of nesting for
        the SRv6 Locator LSA TLV.
        The initial assignment are below:

| Value | Description | Reference |
| --- | --- | --- |
| 0 | Reserved | RFC 9513 |
| 1 | SRv6 End SID | RFC 9513, [Section 8](#END) |
| 2 | IPv6-Forwarding-Address | [RFC8362](#RFC8362); RFC 9513, [Section 7.2](#LOCSUBTLVS) |
| 3 | Route-Tag | [RFC8362](#RFC8362); RFC 9513, [Section 7.2](#LOCSUBTLVS) |
| 4 | Prefix Source OSPF Router-ID | [RFC9084](#RFC9084); RFC 9513, [Section 7.2](#LOCSUBTLVS) |
| 5 | Prefix Source Router Address | [RFC9084](#RFC9084); RFC 9513, [Section 7.2](#LOCSUBTLVS) |
| 10 | SRv6 SID Structure | RFC 9513, [Section 10](#SRSTRUCTTLV) |

Types in the range 0-32767 are allocated via IETF Review
        or IESG Approval [RFC8126](#RFC8126).
...
The following note has been added to this registry to
        ensure that any document requesting allocations in this registry
...
<a name="IANAOSPFV3EXTLSA"></a>
## OSPFv3 Extended-LSA Sub-TLVs

IANA has added the following note to
        the "OSPFv3 Extended-LSA Sub-TLVs" registry within the "Open Shortest Path First v3 (OSPFv3) Parameters" registry group. The purpose of this note is to ensure that
...
<a name="RFC7166"></a> **[RFC7166]** M. Bhatia, V. Manral, A. Lindem, "Supporting Authentication Trailer for OSPFv3", RFC 7166, DOI 10.17487/RFC7166, March 2014. <https://www.rfc-editor.org/info/rfc7166> [Local MD](rfc7166.md)
...
<a name="RFC9084"></a> **[RFC9084]** A. Wang, A. Lindem, J. Dong, P. Psenak, K. Talaulikar, "OSPF Prefix Originator Extensions", RFC 9084, DOI 10.17487/RFC9084, August 2021. <https://www.rfc-editor.org/info/rfc9084> [Local MD](rfc9084.md)
...
The authors would like to acknowledge the contributions of Dean Cheng
      in the early draft versions of this document. The authors would like to thank
      Ran Chen and Detao Zhao for their suggestions related to the extension
      of PrefixOptions for the signaling of the anycast property.

The authors would like to thank Chenzichao,
      Dirk Goethals, Baalajee S,
      Yingzhen Qu, Shraddha Hegde,
      Dhruv Dhody, Martin       Vigoureux, and Reese Enghardt for their review
      and comments on this document. The authors would like to thank Acee Lindem for his detailed shepherd review and feedback
      for improvement of this document. The authors would like to thank
      John Scudder for his AD review and suggestions to
