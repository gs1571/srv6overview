...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-pce-stateful-pce-optional-13
...
**Keywords:** PCEP, Stateful, optional processing

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-pce-stateful-pce-optional-13)
- [alternate](https://dx.doi.org/10.17487/rfc9753)
...
- [2. Overview](#section-2)
  - [2.1. Usage Example](#section-2.1)
- [3. PCEP Extension](#section-3)
  - [3.1. STATEFUL-PCE-CAPABILITY TLV](#section-3.1)
  - [3.2. Handling of the P Flag](#section-3.2)
    - [3.2.1. The PCRpt Message](#section-3.2.1)
      - [3.2.1.1. Delegation](#section-3.2.1.1)
    - [3.2.2. The PCUpd Message and the PCInitiate Message](#section-3.2.2)
  - [3.3. Handling of the I Flag](#section-3.3)
    - [3.3.1. The PCUpd Message](#section-3.3.1)
    - [3.3.2. The PCRpt Message](#section-3.3.2)
    - [3.3.3. The PCInitiate Message](#section-3.3.3)
  - [3.4. Unknown Object Handling](#section-3.4)
- [4. Security Considerations](#section-4)
- [5. IANA Considerations](#section-5)
  - [5.1. STATEFUL-PCE-CAPABILITY TLV](#section-5.1)
- [6. Manageability Considerations](#section-6)
  - [6.1. Control of Function and Policy](#section-6.1)
  - [6.2. Information and Data Models](#section-6.2)
  - [6.3. Liveness Detection and Monitoring](#section-6.3)
  - [6.4. Verify Correct Operations](#section-6.4)
  - [6.5. Requirements on Other Protocols](#section-6.5)
...
PCEP extensions for the stateful PCE model [RFC8231](#RFC8231)
      ...
      Generalized MPLS (GMPLS) tunnels. [RFC8281](#RFC8281) describes the
...
[RFC5440](#RFC5440) defined the P flag (Processing-Rule) in the
      ...
      considered by the PCE during path computation. Stateful PCE [RFC8231](#RFC8231) specifies that the P and I flags of the PCEP objects
      ...
      (PCRpt) [RFC8231](#RFC8231), the Path Computation Update Request
      (PCUpd) [RFC8231](#RFC8231), and the LSP Initiate Request
      (PCInitiate) [RFC8281](#RFC8281) messages.

This document updates [RFC8231](#RFC8231) concerning usage of
...
Setting the P flag in the PCReq message to handle unknown objects 
  is as described in [RFC5440](#RFC5440).
Further, [RFC8231](#RFC8231) defined the usage of the LSP Error Code TLV in the
...
The PCRpt message is used to report the current state of an LSP. As
        ...
        <actual-attribute-list> are encoded (see [RFC8231](#RFC8231)). For example, the <intended-attribute-list>
        could include the METRIC object to indicate a limiting constraint
        (Bound 'B' flag set) for the Path Delay Variation metric [RFC8233](#RFC8233). 
...
Thus, this document specifies how the already existing P and I
        ...
        stateful PCEP message exchange. 
 The scope of how P and I flags are applied is defined in [RFC5440](#RFC5440) and is unchanged by this document. Therefore, these flags can only be applied to an entire PCEP
...
A PCEP speaker indicates its ability to support the handling of the
        ...
        the STATEFUL-PCE-CAPABILITY TLV, as defined in [RFC8231](#RFC8231). A new flag, the R (RELAX) flag, is added to this
...
The R flag **MUST** be set by both the PCC and PCE to indicate support for
        ...
        objects with the P or I bits set, it **MUST** ignore the flags. [RFC8231](#RFC8231) states that P and I flags of the PCEP objects
        are set to 0 on transmission and
        ignored on receipt. It fails to mention the behaviour of objects
        defined outside of [RFC8231](#RFC8231), leading to ambiguity.

## Handling of the P Flag

### The PCRpt Message

The P flag in the PCRpt message [RFC8231](#RFC8231) allows a
...
Delegation is an operation to grant a PCE temporary rights to
            ...
            described in [RFC8051](#RFC8051). Note that for the delegated
...
The PCC **MUST** acknowledge this by sending the PCRpt message with
            ...
            per the processing rules of unacceptable update in [RFC8231](#RFC8231).

### The PCUpd Message and the PCInitiate Message

The P flag in the PCUpd message [RFC8231](#RFC8231) and the
          PCInitiate message [RFC8281](#RFC8281) allows a PCE to specify
...
The I flag in the PCUpd message [RFC8231](#RFC8231) allows a
...
The I flag in the PCRpt message [RFC8231](#RFC8231) allows a
...
Note that when a PCC is unable to set up a path that meets all
          ...
          LSP-ERROR-CODE TLV [RFC8231](#RFC8231) indicating the reason
          for the failure.

### The PCInitiate Message

The I flag has no meaning in the PCInitiate message [RFC8281](#RFC8281), so the I flag **MUST** set to 0 on transmission and
          ignored on receipt.
...
This document updates the handling of unknown objects in the stateful
   ...
   header in a similar way as described in [RFC5440](#RFC5440).  That is, if a 
   ...
   **MUST** be rejected, and the PCE **MUST** send a PCErr message with Error-
   Type="Unknown Object" or "Not supported object" [RFC5440](#RFC5440).
If the P flag is not set, the PCEP
        speaker can ignore the object and continue with the message
        processing as defined.

[RFC8231](#RFC8231) defined the LSP Error Code TLV to be carried
...
This document specifies how the already existing P and I flags in the
      ...
      concerns. The security considerations identified in [RFC5440](#RFC5440), [RFC8231](#RFC8231), and [RFC8281](#RFC8281) continue to apply.

As per [RFC8231](#RFC8231), it is **RECOMMENDED** that these PCEP
      ...
      using Transport Layer Security (TLS) [RFC8253](#RFC8253) as per the
      recommendations and best current practices described in [RFC9325](#RFC9325).

# IANA Considerations

## STATEFUL-PCE-CAPABILITY TLV

[RFC8231](#RFC8231) defined the STATEFUL-PCE-CAPABILITY TLV
...
An implementation supporting this document **SHOULD** allow the
        ...
        this purpose, the PCEP YANG module [PCEP-YANG](#PCEP-YANG) could be extended in the future.
...
Mechanisms defined in this document do not imply any new operation
        verification requirements in addition to those already listed in [RFC5440](#RFC5440).
...
Mechanisms defined in this document do not have any impact on
        network operations in addition to those already listed in [RFC5440](#RFC5440).
...
<a name="PCEP-YANG"></a> **[PCEP-YANG]** D. Dhody, V. P. Beeram, J. Hardwick, J. Tantsura, "A YANG Data Model for Path Computation Element Communications Protocol (PCEP)", Internet-Draft draft-ietf-pce-pcep-yang-30, January 2025. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-pce-pcep-yang-30>
...
Thanks to Jonathan Hardwick for the discussion
      and suggestions around this document.
...
- Organization: Huawei Technologies
- Address: H1, Huawei Xiliu Beipo Village, Songshan Lake, Dongguan, Guangdong, 523808, China
- Email: zhenghaomian@huawei.com
