...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-pce-wson-rwa-ext-17
...
- [prev](https://datatracker.ietf.org/doc/draft-ietf-pce-wson-rwa-ext-17)
- [alternate](https://dx.doi.org/10.17487/rfc8780)
...
Information about the current status of this document, any
            ...
            [https://www.rfc-editor.org/info/rfc8780](https://www.rfc-editor.org/info/rfc8780).
...
- [1. Introduction](#sect-3)
- [2. Terminology](#sect-1)
- [3. Requirements Language](#sect-2)
- [4. Encoding of an RWA Path Request](#sect-4)
  - [4.1. Wavelength Assignment (WA) Object](#sect-4.1)
  - [4.2. Wavelength Selection TLV](#sect-4.2)
  - [4.3. Wavelength Restriction TLV](#sect-4.3)
    - [4.3.1. Link Identifier Field](#sect-4.3.1)
    - [4.3.2. Wavelength Constraint Field](#sect-4.3.2)
  - [4.4. Signal Processing Capability Restrictions](#sect-4.4)
    - [4.4.1. Signal Processing Exclusion](#sect-4.4.1)
    - [4.4.2. Signal Processing Inclusion](#sect-4.4.2)
- [5. Encoding of an RWA Path Reply](#sect-5)
  - [5.1. Wavelength Allocation TLV](#sect-5.1)
  - [5.2. Error Indicator](#sect-5.2)
  - [5.3. NO-PATH Indicator](#sect-5.3)
- [6. Manageability Considerations](#sect-6)
  - [6.1. Control of Function and Policy](#sect-6.1)
  - [6.2. Liveness Detection and Monitoring](#sect-6.2)
  - [6.3. Verifying Correct Operation](#sect-6.3)
  - [6.4. Requirements on Other Protocols and Functional Components](#sect-6.4)
  - [6.5. Impact on Network Operation](#sect-6.5)
- [7. Security Considerations](#sect-7)
- [8. IANA Considerations](#sect-8)
  - [8.1. New PCEP Object: Wavelength Assignment Object](#sect-8.1)
  - [8.2. WA Object Flag Field](#sect-8.2)
  - [8.3. New PCEP TLV: Wavelength Selection TLV](#sect-8.3)
  - [8.4. New PCEP TLV: Wavelength Restriction TLV](#sect-8.4)
  - [8.5. Wavelength Restriction TLV Action Values](#sect-8.5)
  - [8.6. New PCEP TLV: Wavelength Allocation TLV](#sect-8.6)
  - [8.7. Wavelength Allocation TLV Flag Field](#sect-8.7)
  - [8.8. New PCEP TLV: Optical Interface Class List TLV](#sect-8.8)
  - [8.9. New PCEP TLV: Client Signal Information TLV](#sect-8.9)
  - [8.10. New Bit Flag for NO-PATH-VECTOR TLV](#sect-8.10)
  - [8.11. New Error-Types and Error-Values](#sect-8.11)
  - [8.12. New Subobjects for the Exclude Route Object](#sect-8.12)
  - [8.13. New Subobjects for the Include Route Object](#sect-8.13)
  - [8.14. Request for Updated Note for LMP TE Link Object Class Type](#sect-8.14)
- [9. References](#section-9)
  - [9.1. Normative References](#section-9.1)
  - [9.2. Informative References](#section-9.2)
- [Acknowledgments](#sect-9)
- [Contributors](#sect-11)
- [Authors' Addresses](#authors-addresses)

<a name="sect-3"></a>
# Introduction

[RFC5440](#RFC5440) specifies the Path Computation Element Communication
...
This document provides the PCEP extensions for the support of
   ...
   [RFC6163](#RFC6163) and [RFC7449](#RFC7449).
...
An LSC Label Switched Path (LSP) may span one
   ...
   wavelength-continuity constraint. [Figure 1](#fig-1) illustrates the
...
For example (see [Figure 1](#fig-1)), within a translucent WSON, an LSC
...
This document aligns with
 [RFC8779](#RFC8779) for generic properties such as label, label set, and
...
The optical modulation properties, which are also referred to as signal
   compatibility, are already considered in the signaling in [RFC7581](#RFC7581) and [RFC7688](#RFC7688). In order to improve the
...
This document, however, does not address optical impairments as part
   of RWA path computation. See [RFC6566](#RFC6566) for the framework for optical
   impairments.
...
[Figure 2](#fig-2) shows one typical PCE-based implementation, which is
...
1. Explicit Label Control [RFC3471](#RFC3471)
...
2. A Label Set where the PCE provides a range of potential
...
Per [RFC5440](#RFC5440), the format of a PCReq message after incorporating the
...
If the WA object is present in the request, it **MUST** be encoded after the
   END-POINTS object as defined in [RFC8779](#RFC8779). The WA object
...
**Flags field (16 bits):**
  ...
  **M (1 bit):**
    Wavelength Allocation Mode. The M bit is used to indicate the mode of
          ...
          Label Set Field as described in [RFC7579](#RFC7579)
          in the response.  See [Section 5](#sect-5) of this document for the encoding
...
The Wavelength Selection TLV is used to indicate the wavelength
   ...
   set in the WA object specified in [Section 4.1](#sect-4.1). This TLV **MUST NOT** be
   used when the M bit is cleared.

The encoding of this TLV is specified as the WavelengthSelection sub-TLV
   in [RFC7689](#RFC7689). IANA has
...
See [Section 4.3.1](#sect-4.3.1) for the encoding of the Link
        Identifier field.
...
**Action (8 bits):**
  ...
  **2-255:**
  ...
  If a PCE receives an unrecognized Action value, the PCE **MUST** send a
     ...
     an Error-value=3. See [Section 5.2](#sect-5.2) for details.
...
**Link Identifiers:**
  Identifies each link ID for which
  	...
  	the Count field. See [Section 4.3.1](#sect-4.3.1) for
  	encoding of the Link Identifier field.

**Wavelength Constraint:**
  See [Section 4.3.2](#sect-4.3.2) for the encoding of the
...
Various encoding errors are possible with this TLV (e.g., not
   ...
   PCErr message with Error-Type=27 and Error-value=3. See [Section 5.2](#sect-5.2) for details.

<a name="sect-4.3.1"></a>
### Link Identifier Field

The Link Identifier field can be an IPv4 [RFC3630](#RFC3630), IPv6 [RFC5329](#RFC5329), or
   unnumbered interface ID [RFC4203](#RFC4203).
...
The Type field is extensible and matches the "TE_LINK Object Class type
   ...
   Link Management Protocol (LMP) [RFC4204](#RFC4204) (see [LMP-PARAM](#LMP-PARAM)). IANA has added
   an introductory note before the aforementioned registry stating that the values
   have additional usage for the Link Identifier Type field. See [Section 8.14](#sect-8.14).
...
The Wavelength Constraint field of the Wavelength Restriction
   ...
   [RFC7579](#RFC7579) with the base label encoded as a 32-bit LSC
   label, as defined in [RFC6205](#RFC6205).  The Label Set format is repeated here
   for convenience, with the base label internal structure included.
   See [RFC6205](#RFC6205) for a description of Grid, Channel Spacing (C.S.), Identifier, and n, and see [RFC7579](#RFC7579) for the details of each action.
...
See Sections [RFC7579](#RFC7579)-[RFC7579](#RFC7579) of [RFC7579](#RFC7579) for details on additional
...
The supported signal processing capabilities considered in the RWA
   Information Model [RFC7446](#RFC7446) are:

- Optical Interface Class List
- Bit Rate
- Client Signal

The bit rate restriction is already expressed in the BANDWIDTH object in [RFC8779](#RFC8779).
...
The PCC/PCE should be able to exclude particular types of signal
   ...
   [RFC5521](#RFC5521) defines how the Exclude Route
...
```
Refer to [RFC5521](#RFC5521) for the definitions of
...
**Attribute (8 bits):**
  [RFC5521](#RFC5521) defines several Attribute values; the only
...
The encoding of the Signal Processing Inclusion subobjects is
   similar to the process in [Section 4.4.1](#sect-4.4.1) where the 'X' field is replaced with the 'L'
   field; all the other fields remain the same. The 'L' field is
   described in [RFC3209](#RFC3209).

<a name="sect-5"></a>
# Encoding of an RWA Path Reply

This section provides the encoding of an RWA Path Reply for a
   wavelength allocation request as discussed in [Section 4](#sect-4).
...
1. Explicit Label Control (ELC) where the PCE allocates
...
The type for the Wavelength Allocation TLV is 10 (see [Section 8.4](#sect-8.4)). Note
...
**Link Identifier:**
  Identifies the interface to which the
            assignment wavelength(s) is applied. See [Section 4.3.1](#sect-4.3.1) for encoding of the Link Identifier field.

**Allocated Wavelength(s):**
  Indicates the allocated wavelength(s) to be associated with the
            link identifier. See [Section 4.3.2](#sect-4.3.2)
            for encoding details.


This TLV is carried in a PCRep message as an Attribute TLV [RFC5420](#RFC5420)
   in the Hop Attribute subobjects [RFC7570](#RFC7570) in the Explicit Route Object (ERO) [RFC5440](#RFC5440).
...
To communicate the reason(s) for not being able to find RWA for the
   ...
   [RFC5440](#RFC5440).  The object may contain a NO-PATH-VECTOR TLV to provide
...
In addition to the parameters already listed in [RFC5440](#RFC5440), a PCEP implementation **SHOULD** allow configuration of the
...
The PCEP Link-State mechanism [PCEP-LS](#I-D.lee-pce-pcep-ls-optical) may be used to advertise
...
The security considerations discussed in [RFC5440](#RFC5440) are relevant for
   this document; this document does not introduce any new security
   issues. If an operator wishes to keep the information
   distributed by WSON private, PCEPS (Usage of TLS to Provide a Secure Transport for PCEP) [RFC8253](#RFC8253) **SHOULD** be used.
...
As described in [Section 4.1](#sect-4.1), a new PCEP
   object is defined to carry wavelength-assignment-related constraints. IANA
   has allocated the following in the "PCEP Objects" subregistry [PCEP-NUMBERS](#PCEP-NUMBERS):
...
As described in [Section 4.1](#sect-4.1), IANA has
   created the "WA Object Flag Field" subregistry under the "Path Computation
   Element Protocol (PCEP) Numbers" registry [PCEP-NUMBERS](#PCEP-NUMBERS) to
   manage the Flags field of the WA object. New values are to be assigned by
   Standards Action [RFC8126](#RFC8126). Each bit should
...
In [Section 4.2](#sect-4.2), a new PCEP TLV is defined to
   indicate wavelength selection constraints. IANA has made the following
   allocation in the "PCEP TLV Type Indicators" subregistry [PCEP-NUMBERS](#PCEP-NUMBERS):
...
In [Section 4.3](#sect-4.3), a new PCEP TLV is defined to indicate
   wavelength restrictions. IANA has made the following allocation in
   the "PCEP TLV Type Indicators" subregistry [PCEP-NUMBERS](#PCEP-NUMBERS):
...
As described in [Section 4.3](#sect-4.3), IANA has
...
In [Section 5.1](#sect-5.1), a new PCEP TLV
   ...
   "PCEP TLV Type Indicators" subregistry [PCEP-NUMBERS](#PCEP-NUMBERS):
...
As described in [Section 5.1](#sect-5.1), IANA has
   created a new "Wavelength Allocation TLV Flag Field" subregistry under the
   "Path Computation Element Protocol (PCEP) Numbers" registry [PCEP-NUMBERS](#PCEP-NUMBERS) to
   manage the Flags field of the Wavelength Allocation TLV. New values
   are to be assigned by Standards Action [RFC8126](#RFC8126).  Each bit should
   be tracked with the following qualities:
...
In [Section 4.4](#sect-4.4), a new PCEP TLV is defined to
...
In [Section 5.3](#sect-5.3), a new bit flag is defined to be
...
IANA has made the following allocation for this new bit flag in the
   "NO-PATH-VECTOR TLV Flag Field" subregistry [PCEP-NUMBERS](#PCEP-NUMBERS):
...
In [Section 5.2](#sect-5.2), new PCEP error
   codes are defined for WSON RWA errors. IANA has made the following allocations
   in the "PCEP-ERROR Object Error Types and Values" subregistry [PCEP-NUMBERS](#PCEP-NUMBERS):
...
The "Path Computation Element Protocol (PCEP) Numbers" registry
        contains a subregistry titled "XRO Subobjects" [PCEP-NUMBERS](#PCEP-NUMBERS). Per [Section 4.4.1](#sect-4.4.1), IANA has added the following subobjects that can
...
subregistry titled "IRO Subobjects" [PCEP-NUMBERS](#PCEP-NUMBERS).  Per [Section 4.4.2](#sect-4.4.2), IANA has added the following
...
The "TE_LINK Object Class type name space (Value 11)" registry was created
   for the Link Management Protocol (LMP) [RFC4204](#RFC4204). As discussed in [Section 4.3.1](#sect-4.3.1), IANA has added the following note at the top of the
   "TE_LINK Object Class type name space (Value 11)" registry [LMP-PARAM](#LMP-PARAM):
...
<a name="RFC7579"></a> **[RFC7579]** G. Bernstein, Y. Lee, D. Li, W. Imajuku, J. Han, "General Network Element Constraint Encoding for GMPLS-Controlled Networks", RFC 7579, DOI 10.17487/RFC7579, June 2015. <https://www.rfc-editor.org/info/rfc7579> [Local MD](rfc7579.md)

<a name="RFC7581"></a> **[RFC7581]** G. Bernstein, Y. Lee, D. Li, W. Imajuku, J. Han, "Routing and Wavelength Assignment Information Encoding for Wavelength Switched Optical Networks", RFC 7581, DOI 10.17487/RFC7581, June 2015. <https://www.rfc-editor.org/info/rfc7581> [Local MD](rfc7581.md)

<a name="RFC7688"></a> **[RFC7688]** Y. Lee, G. Bernstein, "GMPLS OSPF Enhancement for Signal and Network Element Compatibility for Wavelength Switched Optical Networks", RFC 7688, DOI 10.17487/RFC7688, November 2015. <https://www.rfc-editor.org/info/rfc7688> [Local MD](rfc7688.md)

<a name="RFC7689"></a> **[RFC7689]** G. Bernstein, S. Xu, Y. Lee, G. Martinelli, H. Harai, "Signaling Extensions for Wavelength Switched Optical Networks", RFC 7689, DOI 10.17487/RFC7689, November 2015. <https://www.rfc-editor.org/info/rfc7689> [Local MD](rfc7689.md)
...
<a name="LMP-PARAM"></a> **[LMP-PARAM]** "Link Management Protocol (LMP) Parameters", <https://www.iana.org/assignments/lmp-parameters/>

<a name="I-D.lee-pce-pcep-ls-optical"></a> **[PCEP-LS]** Y Lee, H Zheng, D Ceccarelli, W Wang, P Park, B Yoon, "PCEP Extension for Distribution of Link-State and TE information for Optical Networks", Internet-Draft draft-lee-pce-pcep-ls-optical-09, March 2020. Work in Progress <https://tools.ietf.org/html/draft-lee-pce-pcep-ls-optical-09>

<a name="PCEP-NUMBERS"></a> **[PCEP-NUMBERS]** "Path Computation Element Protocol (PCEP) Numbers", <https://www.iana.org/assignments/pcep/>
...
<a name="RFC4204"></a> **[RFC4204]** J. Lang, "Link Management Protocol (LMP)", RFC 4204, DOI 10.17487/RFC4204, October 2005. <https://www.rfc-editor.org/info/rfc4204> [Local MD](rfc4204.md)
...
<a name="RFC5420"></a> **[RFC5420]** A. Farrel, D. Papadimitriou, JP. Vasseur, A. Ayyangar, "Encoding of Attributes for MPLS LSP Establishment Using Resource Reservation Protocol Traffic Engineering (RSVP-TE)", RFC 5420, DOI 10.17487/RFC5420, February 2009. <https://www.rfc-editor.org/info/rfc5420> [Local MD](rfc5420.md)
...
<a name="RFC6566"></a> **[RFC6566]** Y. Lee, G. Bernstein, D. Li, G. Martinelli, "A Framework for the Control of Wavelength Switched Optical Networks (WSONs) with Impairments", RFC 6566, DOI 10.17487/RFC6566, March 2012. <https://www.rfc-editor.org/info/rfc6566> [Local MD](rfc6566.md)

<a name="RFC7446"></a> **[RFC7446]** Y. Lee, G. Bernstein, D. Li, W. Imajuku, "Routing and Wavelength Assignment Information Model for Wavelength Switched Optical Networks", RFC 7446, DOI 10.17487/RFC7446, February 2015. <https://www.rfc-editor.org/info/rfc7446> [Local MD](rfc7446.md)
...
The authors would like to thank Adrian Farrel, Julien Meuric, Dhruv Dhody,
   and Benjamin Kaduk for many helpful comments that greatly
...
- Address: Fremont, CA, United States of America
...
- Organization: CTTC
- Address: PMT Ed B4 Av., Castelldefels, Barcelona, 08860, Spain
- Email: ramon.casellas@cttc.es
