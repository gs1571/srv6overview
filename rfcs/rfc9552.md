...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-idr-rfc7752bis-17
...
- [prev](https://datatracker.ietf.org/doc/draft-ietf-idr-rfc7752bis-17)
- [alternate](https://dx.doi.org/10.17487/rfc9552)
...
- [2. Motivation and Applicability](#APPL)
  - [2.1. MPLS-TE with PCE](#PCE)
  - [2.2. ALTO Server Network API](#ALTO)
- [3. BGP Speaker Roles for BGP-LS](#ROLES)
- [4. Advertising IGP Information into BGP-LS](#IGPTOBGP)
- [5. Carrying Link-State Information in BGP](#BGPLS)
  - [5.1. TLV Format](#TLV-section)
  - [5.2. The Link-State NLRI](#BGPLSNLRI)
    - [5.2.1. Node Descriptors](#NODEDESC)
    - [5.2.2. Link Descriptors](#LINKDESC)
    - [5.2.3. Prefix Descriptors](#PREFIXDESC)
  - [5.3. The BGP-LS Attribute](#BGPLSATTR)
    - [5.3.1. Node Attribute TLVs](#NODEATTR)
    - [5.3.2. Link Attribute TLVs](#link_attribute)
    - [5.3.3. Prefix Attribute TLVs](#PFXATTR)
  - [5.4. Private Use](#PRIVATE)
  - [5.5. BGP Next-Hop Information](#BGPNH)
  - [5.6. Inter-AS Links](#INTERAS)
  - [5.7. OSPF Virtual Links and Sham Links](#OSPFVL)
  - [5.8. OSPFv2 Type 4 Summary-LSA & OSPFv3 Inter-Area-Router-LSA](#OSPFTYPE4)
  - [5.9. Handling of Unreachable IGP Nodes](#UNREACHNODES)
  - [5.10. Router-ID Anchoring Example: ISO Pseudonode](#ISISPN)
  - [5.11. Router-ID Anchoring Example: OSPF Pseudonode](#OSPFPN)
  - [5.12. Router-ID Anchoring Example: OSPFv2 to IS-IS Migration](#OSPF2ISIS)
- [6. Link to Path Aggregation](#LINKPATHAGGREGATION)
  - [6.1. Example: No Link Aggregation](#section-6.1)
  - [6.2. Example: ASBR to ASBR Path Aggregation](#section-6.2)
  - [6.3. Example: Multi-AS Path Aggregation](#section-6.3)
- [7. IANA Considerations](#IANA)
  - [7.1. BGP-LS Registries](#Registries)
    - [7.1.1. BGP-LS NLRI Types Registry](#NLRITYPESREG)
    - [7.1.2. BGP-LS Protocol-IDs Registry](#PROTOIDREG)
    - [7.1.3. BGP-LS Well-Known Instance-IDs Registry](#INSTIDREG)
    - [7.1.4. BGP-LS Node Flags Registry](#NFLAGSIDREG)
    - [7.1.5. BGP-LS MPLS Protocol Mask Registry](#MPLSMASKREG)
    - [7.1.6. BGP-LS IGP Prefix Flags Registry](#IGPPFLAGSREG)
    - [7.1.7. BGP-LS TLVs Registry](#TLVREG)
  - [7.2. Guidance for Designated Experts](#DE-Guidance)
- [8. Manageability Considerations](#Manageability)
  - [8.1. Operational Considerations](#Operational-Considerations)
    - [8.1.1. Operations](#Operations)
    - [8.1.2. Installation and Initial Setup](#Initial-Setup)
    - [8.1.3. Migration Path](#Migration-Path)
    - [8.1.4. Requirements for Other Protocols and Functional Components](#Other-Protocols)
    - [8.1.5. Impact on Network Operation](#Network-Operation)
    - [8.1.6. Verifying Correct Operation](#Verifying-Correct-Operation)
  - [8.2. Management Considerations](#Management-Considerations)
    - [8.2.1. Management Information](#Management-Information)
    - [8.2.2. Fault Management](#Fault-Management)
    - [8.2.3. Configuration Management](#Configuration-Management)
    - [8.2.4. Accounting Management](#Accounting-Management)
    - [8.2.5. Performance Management](#Performance-Management)
    - [8.2.6. Security Management](#Security-Management)
- [9. TLV/Sub-TLV Code Points Summary](#TLVSUMMARY)
...
- [Appendix A. Changes from RFC 7752](#CHANGES)
...
The contents of a Link-State Database (LSDB) or of an IGP's Traffic
      ...
      System (AS) to make better decisions.

The IETF has defined the Path Computation Element (PCE) [RFC4655](#RFC4655) as a mechanism for achieving the computation of
      ...
      also defined the ALTO server [RFC5693](#RFC5693) as an entity that
...
This document describes a mechanism by which link-state and TE
      information can be collected from networks and shared with external
      components using the BGP routing protocol [RFC4271](#RFC4271). This
      ...
      tunnel) links. The mechanism described is subject to policy control.
...
An illustration of the collection of link-state and TE information
      and its distribution to consumers is shown in [Figure 1](#MECHANISM-OVERVIEW) below.
...
This document obsoletes [RFC7752](#RFC7752) by completely
      replacing that document. It makes some small changes and clarifications
      to the previous specification as documented in [Appendix A](#CHANGES).
...
As described in [RFC4655](#RFC4655), a PCE can be used to
...
- Within a single area, the PCE offers enhanced computational
            ...
            computation across the whole area.
...
Previous solutions used per-domain path computation [RFC5152](#RFC5152). The source router could only compute the path for
        ...
        AS Border Routers (ASBRs) as loose-hops [RFC3209](#RFC3209) and using the IGP-computed
...
The PCE presents a computation server that may have visibility into
        ...
        for the area(s) it serves, but [RFC4655](#RFC4655) does not
...
<a name="ALTO"></a>
## ALTO Server Network API

An ALTO server [RFC5693](#RFC5693) is an entity that generates
        ...
        between PIDs listed in the Network Map. For more details, see [RFC7285](#RFC7285).
...
In [Figure 1](#MECHANISM-OVERVIEW), the
...
**BGP-LS Producer:**
  The term BGP-LS Producer refers to a BGP Speaker
...
**BGP-LS Consumer:**
  The term BGP-LS Consumer refers to a consumer
            ...
            information to a BGP Speaker for origination into BGP-LS.

**BGP-LS Propagator:**
  The term BGP-LS Propagator refers to a BGP
            ...
            Consumer after performing BGP Decision Process.


The above roles are not mutually exclusive. The same BGP
      ...
      providing this information to a BGP-LS Consumer.
...
<a name="IGPTOBGP"></a>
# Advertising IGP Information into BGP-LS
...
The details of the interface between IGPs and BGP for the
      ...
      required for the advertisement of link-state information into BGP-LS.
...
Information in the Link-State NLRIs and the BGP-LS Attribute is
        ...
        [Figure 4](#TLV-figure) and applies to both the NLRI and the
        BGP-LS Attribute encodings.
...
To compare NLRIs with unknown TLVs, all TLVs within the NLRI **MUST**
        ...
        ambiguity arising therefrom is prevented.
...
For both the NLRI and BGP-LS Attribute parts, all TLVs are
        ...
        or required in specific conditions.

The TLVs within the BGP-LS Attribute **SHOULD** be ordered in ascending
        order by TLV type. The BGP-LS Attribute with unordered TLVs **MUST NOT** be
...
The MP_REACH_NLRI and MP_UNREACH_NLRI attributes are BGP's
...
All non-VPN link, node, and prefix information **SHALL** be encoded
        using AFI 16388 / SAFI 71. VPN link, node, and prefix information
        **SHALL** be encoded using AFI 16388 / SAFI 72.
...
For two BGP Speakers to exchange Link-State NLRI, they **MUST** use BGP
        ...
        properly processing such NLRI. This is done as specified in [RFC4760](#RFC4760) by using capability code 1 (multiprotocol BGP),
...
New Link-State NLRI types may be introduced in the future. Since
        ...
        in the Multiprotocol BGP (MP-BGP) capability [RFC4760](#RFC4760),
        ...
        [RFC7606](#RFC7606) (paragraph 2) of [RFC7606](#RFC7606) for Link-State address family. An
        implementation **MUST** handle unknown Link-State NLRI types as opaque
        objects and **MUST** preserve and propagate them.

The format of the Link-State NLRI is shown in the following
        figures.
...
Route Distinguishers are defined and discussed in [RFC4364](#RFC4364).
...
The 'Direct' and 'Static configuration' protocol types **SHOULD** be
        ...
        used. For modeling virtual links, such as described in [Section 6](#LINKPATHAGGREGATION), the Protocol-ID 'Static configuration'
        **SHOULD** be used.

A router may run multiple protocol instances of OSPF or IS-IS
        ...
        the same link. See [RFC8202](#RFC8202) and [RFC6549](#RFC6549). These instances define independent IGP routing
...
To support multiple IGP instances, an implementation needs to
        ...
        Unique BGP-LS Instance-IDs **MUST** be assigned to routing protocol
        ...
        based on the BGP-LS Instance-ID.
...
Each Node Descriptor, Link Descriptor, and Prefix Descriptor
        ...
        NLRI Types for the protocols that are listed in [Table 2](#PROTOCOL-IDS). Documents extending BGP-LS specifications
...
Each link is anchored by a pair of Router-IDs that are used by
          ...
          Router-IDs [RFC5305](#RFC5305) [RFC6119](#RFC6119). When
          configured, these auxiliary TE Router-IDs (TLV 1028/1029) **MUST** be
          included in the node attribute described in [Section 5.3.1](#NODEATTR) and **MAY** be included in the link attribute
          described in [Section 5.3.2](#link_attribute). The advertisement of
...
It is desirable that the Router-ID assignments inside the Node
          ...
          described in [RFC1918](#RFC1918). BGP-LS uses the Autonomous
          System Number to disambiguate the Router-IDs, as described in
          [Section 5.2.1.1](#gbl_uniqueness).

<a name="gbl_uniqueness"></a>
#### Globally Unique Node/Link/Prefix Identifiers
...
1. The same node **MUST NOT** be represented by two
                keys (otherwise, one node will look like two nodes).
2. Two different nodes **MUST NOT** be represented
...
We define an "IGP domain" to be the set of nodes (hence, by
            ...
            it may happen that two redundant IGPs are in place.
...
The BGP-LS Instance-ID carried in the Identifier field, as
            described earlier along with a set of sub-TLVs described in [Section 5.2.1.4](#node_desc_tlvs), allows specification of a flexible key
            ...
            domain is uniquely identified even across a multi-AS network.

<a name="LOCALNODEDESC"></a>
#### Local Node Descriptors

The Local Node Descriptors TLV contains Node Descriptors for
            ...
            or more Node Descriptor sub-TLVs defined in [Section 5.2.1.4](#node_desc_tlvs).
...
```
<a name="REMOTENODEDESC"></a>
#### Remote Node Descriptors

The Remote Node Descriptors TLV contains Node Descriptors for
            ...
            defined in [Section 5.2.1.4](#node_desc_tlvs).
...
```
<a name="node_desc_tlvs"></a>
#### Node Descriptor Sub-TLVs

The Node Descriptor sub-TLV type code points and lengths are
            listed in the following table:

**Table: Node Descriptor Sub-TLVs**
...
The sub-TLV values in Node Descriptor TLVs are defined as
            follows:

**Autonomous System:**
  Opaque value (32-bit AS
...
**BGP-LS Identifier:**
  Opaque value (32-bit ID).
                  ...
                  document (refer to [Appendix A](#CHANGES) for more details).
                  It **MAY** be advertised for compatibility with [RFC7752](#RFC7752) implementations. See the final paragraph of
                  this section for further considerations and a recommended
                  default value.

**OSPF Area-ID:**
  Used to identify the 32-bit area
                  ...
                  information is not associated with a specific area.
...
At most, there **MUST** be one instance of each sub-TLV type present
...
The BGP-LS Identifier was introduced by [RFC7752](#RFC7752), and its use is being deprecated by this
            ...
            BGP-LS Producer implementations that conform to [RFC7752](#RFC7752) to ensure consistency of NLRI encoding for
...
The Link Descriptor field is a set of Type/Length/Value (TLV)
          triplets. The format of each TLV is shown in [Section 5.1](#TLV-section). The Link Descriptor TLVs uniquely identify a
...
A link between two nodes is not considered as complete (or
          ...
          check' that is performed by link-state IGPs.
...
The format and semantics of the Value fields in most Link
          ...
          fields in IS-IS Extended IS Reachability sub-TLVs, which are defined in [RFC5305](#RFC5305), [RFC5307](#RFC5307), and [RFC6119](#RFC6119). Although the encodings for Link Descriptor TLVs
          were originally defined for IS-IS, the TLVs can carry data sourced
          by either IS-IS or OSPF.

The following TLVs are defined as Link Descriptors in the Link
          NLRI:

**Table: Link Descriptor TLVs**

| TLV Code Point | Description | IS-IS TLV/Sub-TLV | Reference |
| --- | --- | --- | --- |
| 258 | Link Local/Remote Identifiers | 22/4 | [RFC5307](#RFC5307) |
| 259 | IPv4 interface address | 22/6 | [RFC5305](#RFC5305) |
| 260 | IPv4 neighbor address | 22/8 | [RFC5305](#RFC5305) |
| 261 | IPv6 interface address | 22/12 | [RFC6119](#RFC6119) |
| 262 | IPv6 neighbor address | 22/13 | [RFC6119](#RFC6119) |
| 263 | Multi-Topology Identifier | --- | [Section 5.2.2.1](#MT-ID) |
...
The information about a link present in the LSA/LSP originated by
      ...
      If interface and neighbor addresses, either IPv4 or IPv6, are
              ...
              as descriptors of a link since they are not considered unique.

      If interface and neighbor addresses are not present and the
              ...
              IPv6 link-local addressing on them.
      ...
      The Multi-Topology Identifier TLV **MUST** be included as a Link
              ...
              a non-default topology.

The TLVs/sub-TLVs corresponding to the interface addresses and/or
...
The Multi-Topology Identifier (MT-ID) TLV carries one or more IS-IS or
            OSPF Multi-Topology Identifiers for a link, node, or prefix.

The semantics of the IS-IS MT-ID are defined in Sections [RFC5120](#RFC5120) and 
            [RFC5120](#RFC5120) of [RFC5120](#RFC5120). The semantics of the OSPF
            MT-ID are defined in [RFC4915](#RFC4915). If
            ...
            0 to 127 are valid for the MT-ID.

The format of the MT-ID TLV is shown in the following
            figure.
...
The MT-ID TLV **MAY** be included as a Link Descriptor, as a Prefix
            ...
            the Bits R are reserved and **MUST** be set to 0 (as per [RFC5120](#RFC5120)) when originated and ignored on
            receipt.

In the BGP-LS Attribute of a Node NLRI, one MT-ID TLV
            ...
            IS-IS, the Bits R are set as per [RFC5120](#RFC5120).
...
**Table: Prefix Descriptor TLVs**

| TLV Code Point | Description | Length | Reference |
| --- | --- | --- | --- |
| 263 | Multi-Topology Identifier | variable | [Section 5.2.2.1](#MT-ID) |
| 264 | OSPF Route Type | 1 | [Section 5.2.3.1](#OSPFRTETYPE) |
| 265 | IP Reachability Information | variable | [Section 5.2.3.2](#IPREACHINFO) |
...
<a name="OSPFRTETYPE"></a>
#### OSPF Route Type

The OSPF Route Type TLV is an optional TLV corresponding to
            ...
            Opaque LSA [RFC7684](#RFC7684)). The route type advertised in
            the OSPFv2 Extended Prefix TLV ([RFC7684](#RFC7684)) does not make a distinction between Type 1 and
            ...
            value 0 is carried in the OSPFv2 Extended Prefix TLV.
...
```
The Type and Length fields of the TLV are defined in
            [Table 5](#table_prefix_descriptor_tlv). The Route Type
...
```
The Type and Length fields of the TLV are defined in [Table 5](#table_prefix_descriptor_tlv). The following two fields
            ...
            4 octets for prefix length 25 up to 32, etc.
...
The size of the BGP-LS Attribute may potentially grow large,
        ...
        single Link-State NLRI. The BGP specification [RFC4271](#RFC4271)
        mandates a maximum BGP message size of 4096 octets. It is **RECOMMENDED**
        that implementations support the extended message size for BGP [RFC8654](#RFC8654) to
        ...
        Link-State NLRI that crosses the maximum limit for a BGP message.

An implementation **MAY** adopt mechanisms to avoid this problem that
...
When a BGP-LS Propagator finds that it is exceeding the maximum BGP
        ...
        apply the 'Attribute Discard' error-handling approach [RFC7606](#RFC7606), and handle the propagation as described in [Section 8.2.2](#Fault-Management). When a BGP-LS Propagator needs to perform
        'Attribute Discard' for reducing the BGP UPDATE message size as
        specified in [RFC8654](#RFC8654), it **MUST** first
        discard the BGP-LS Attribute to enable the detection and diagnosis of
        this error condition as discussed in [Section 8.2.2](#Fault-Management). This brings the deployment consideration
        ...
        set of BGP Speakers that all support the contents of [RFC8654](#RFC8654).
...
| --- | --- | --- | --- |
| 263 | Multi-Topology Identifier | variable | [Section 5.2.2.1](#MT-ID) |
| 1024 | Node Flag Bits | 1 | [Section 5.3.1.1](#NODEFLAGBITS) |
| 1025 | Opaque Node Attribute | variable | [Section 5.3.1.5](#OPAQUENODE) |
| 1026 | Node Name | variable | [Section 5.3.1.3](#NODENAME) |
| 1027 | IS-IS Area Identifier | variable | [Section 5.3.1.2](#ISISAREA) |
| 1028 | IPv4 Router-ID of Local Node | 4 | [RFC5305](#RFC5305) |
| 1029 | IPv6 Router-ID of Local Node | 16 | [RFC6119](#RFC6119) |

<a name="NODEFLAGBITS"></a>
#### Node Flag Bits TLV
...
**Table: Node Flag Bits Definitions**

| Bit | Description | Reference |
| --- | --- | --- |
| 'O' | Overload Bit | [ISO10589](#ISO10589) |
| 'A' | Attached Bit | [ISO10589](#ISO10589) |
| 'E' | External Bit | [RFC2328](#RFC2328) |
| 'B' | ABR Bit | [RFC2328](#RFC2328) |
| 'R' | Router Bit | [RFC5340](#RFC5340) |
| 'V' | V6 Bit | [RFC5340](#RFC5340) |

The bits that are not defined **MUST** be set to 0 by the
            originator and **MUST** be ignored by the receiver.

<a name="ISISAREA"></a>
#### IS-IS Area Identifier TLV

An IS-IS node can be part of only a single IS-IS area. However,
...
The Node Name TLV is optional. The encoding semantics for the
            node name has been borrowed from [RFC5301](#RFC5301). The
...
The Value field is encoded in 7-bit ASCII. If a user interface
            ...
            the ToASCII and/or ToUnicode algorithm as described in [RFC5890](#RFC5890) to achieve the correct format for transmission
            or display.

[RFC5301](#RFC5301) describes an IS-IS-specific extension,
            and [RFC5642](#RFC5642) describes an OSPF extension for the
...
```
<a name="aux_routerid_node"></a>
#### Local IPv4/IPv6 Router-ID TLVs

The local IPv4/IPv6 Router-ID TLVs are used to describe
            ...
            given type, then each one is encoded as a separate TLV.
...
In the case of OSPF, this TLV **MUST NOT** be used to advertise
...
```
The Type is as specified in [Table 6](#node-attribute_tlv). The length is variable.

<a name="link_attribute"></a>
### Link Attribute TLVs

Link Attribute TLVs are TLVs that may be encoded in the BGP-LS
          Attribute with a Link NLRI. Each 'Link Attribute' is a
          Type/Length/Value (TLV) triplet formatted as defined in [Section 5.1](#TLV-section). The format and semantics of the Value fields
          ...
          which are defined in [RFC5305](#RFC5305) and [RFC5307](#RFC5307).
          ...
          the TLVs can carry data sourced by either IS-IS or OSPF.
...
| TLV Code Point | Description | IS-IS TLV/Sub-TLV | Reference |
| --- | --- | --- | --- |
| 1028 | IPv4 Router-ID of Local Node | 134/--- | [RFC5305](#RFC5305) |
| 1029 | IPv6 Router-ID of Local Node | 140/--- | [RFC6119](#RFC6119) |
| 1030 | IPv4 Router-ID of Remote Node | 134/--- | [RFC5305](#RFC5305) |
| 1031 | IPv6 Router-ID of Remote Node | 140/--- | [RFC6119](#RFC6119) |
| 1088 | Administrative group (color) | 22/3 | [RFC5305](#RFC5305) |
| 1089 | Maximum link bandwidth | 22/9 | [RFC5305](#RFC5305) |
| 1090 | Max. reservable link bandwidth | 22/10 | [RFC5305](#RFC5305) |
| 1091 | Unreserved bandwidth | 22/11 | [RFC5305](#RFC5305) |
| 1092 | TE Default Metric | 22/18 | [Section 5.3.2.3](#TEDEFAULTMETTLV) |
| 1093 | Link Protection Type | 22/20 | [RFC5307](#RFC5307) |
| 1094 | MPLS Protocol Mask | --- | [Section 5.3.2.2](#MPLSPROTOTLV) |
| 1095 | IGP Metric | --- | [Section 5.3.2.4](#IGPMETTLV) |
| 1096 | Shared Risk Link Group | --- | [Section 5.3.2.5](#SRLGTLV) |
| 1097 | Opaque Link Attribute | --- | [Section 5.3.2.6](#OPAQUELINK) |
| 1098 | Link Name | --- | [Section 5.3.2.7](#LINKNAME) |

<a name="aux_routerid_link"></a>
#### IPv4/IPv6 Router-ID TLVs
...
<a name="MPLSPROTOTLV"></a>
#### MPLS Protocol Mask TLV

The MPLS Protocol Mask TLV carries a bitmask describing which
...
Generation of the MPLS Protocol Mask TLV is only valid for and
            **SHOULD** only be used with originators that have local link insight,
            ...
            as per [Table 2](#PROTOCOL-IDS). The MPLS Protocol Mask TLV
            **MUST NOT** be included in NLRIs with the other Protocol-IDs listed
            in [Table 2](#PROTOCOL-IDS).
...
```
The following bits are defined, and the reserved bits **MUST** be
            set to zero and **SHOULD** be ignored on receipt:

**Table: MPLS Protocol Mask TLV Codes**

| Bit | Description | Reference |
| --- | --- | --- |
| 'L' | Label Distribution Protocol (LDP) | [RFC5036](#RFC5036) |
| 'R' | Extension to RSVP for LSP Tunnels (RSVP-TE) | [RFC3209](#RFC3209) |

The bits that are not defined **MUST** be set to 0 by the
            originator and **MUST** be ignored by the receiver.

<a name="TEDEFAULTMETTLV"></a>
#### TE Default Metric TLV

The TE Default Metric TLV carries the Traffic Engineering
...
The Shared Risk Link Group (SRLG) TLV carries the Shared Risk
            Link Group information (see Section ["Shared Risk Link Group
            Information"](#RFC4202) of [RFC4202](#RFC4202)). It contains a data
            structure consisting of a (variable) list of SRLG values, where
            each element in the list has 4 octets, as shown in [Figure 22](#SRLG). The length of this TLV is 4 * (number of SRLG
            values).
...
```
The SRLG TLV for OSPF-TE is defined in [RFC4203](#RFC4203). In IS-IS, the SRLG information is carried in
            two different TLVs: the GMPLS-SRLG TLV (for IPv4) (Type 138) defined in
            [RFC5307](#RFC5307) and the IPv6 SRLG TLV (Type 139) defined
            in [RFC6119](#RFC6119). Both IPv4 and IPv6 SRLG information
            is carried in a single TLV.

<a name="OPAQUELINK"></a>
#### Opaque Link Attribute TLV
...
In the case of OSPFv2, this TLV **MUST NOT** be used to advertise
            ...
            Extended Link Opaque LSA [RFC7684](#RFC7684). In the case of
            OSPFv3, this TLV **MUST NOT** be used to advertise TLVs other than
            those in the OSPFv3 E-Router-LSA or E-Link-LSA [RFC8362](#RFC8362).
...
The Link Name TLV is optional. The Value field identifies the
            ...
            use of the FQDN or a substring of it is strongly **RECOMMENDED**. The
            maximum length of the Link Name TLV is 255 octets.
...
| TLV Code Point | Description | Length | Reference |
| --- | --- | --- | --- |
| 1152 | IGP Flags | 1 | [Section 5.3.3.1](#IGPFLAGS) |
| 1153 | IGP Route Tag | 4*n | [RFC5130](#RFC5130) |
| 1154 | IGP Extended Route Tag | 8*n | [RFC5130](#RFC5130) |
| 1155 | Prefix Metric | 4 | [RFC5305](#RFC5305) |
| 1156 | OSPF Forwarding Address | 4 | [RFC2328](#RFC2328) |
| 1157 | Opaque Prefix Attribute | variable | [Section 5.3.3.6](#OPAQUEPREFIX) |

<a name="IGPFLAGS"></a>
#### IGP Flags TLV

The IGP Flags TLV contains one octet of IS-IS and OSPF flags
...
```
The Value field contains bits defined according to the table
            below:

**Table: IGP Flag Bits Definitions**

| Bit | Description | Reference |
| --- | --- | --- |
| 'D' | IS-IS Up/Down Bit | [RFC5305](#RFC5305) |
| 'N' | OSPF "no unicast" Bit | [RFC5340](#RFC5340) |
| 'L' | OSPF "local address" Bit | [RFC5340](#RFC5340) |
| 'P' | OSPF "propagate NSSA" Bit | [RFC5340](#RFC5340) |
...
The IGP Route Tag TLV carries original IGP Tags (IS-IS [RFC5130](#RFC5130) or OSPF) of the prefix and is encoded as
            follows:
...
The Value field contains one or more Route Tags as learned in
            the IGP topology.

<a name="ext_route_tag"></a>
#### IGP Extended Route Tag TLV

The IGP Extended Route Tag TLV carries IS-IS Extended Route
            Tags of the prefix [RFC5130](#RFC5130) and is encoded as
            follows:
...
The Extended Route Tag field contains one or more Extended
            Route Tags as learned in the IGP topology.

<a name="prefix_metric"></a>
#### Prefix Metric TLV

The Prefix Metric TLV is an optional attribute and may only
            appear once. If present, it carries the metric of the prefix as
            known in the IGP topology, as described in [RFC5305](#RFC5305) (and therefore represents the reachability cost
...
<a name="ospf_fwd_addr"></a>
#### OSPF Forwarding Address TLV

The OSPF Forwarding Address TLV [RFC2328](#RFC2328) [RFC5340](#RFC5340) carries the OSPF forwarding address as known in
...
<a name="OPAQUEPREFIX"></a>
#### Opaque Prefix Attribute TLV
...
In the case of OSPFv2, this TLV **MUST NOT** be used to advertise
            information carried using TLVs other than those in the OSPFv2
            Extended Prefix Opaque LSA [RFC7684](#RFC7684).
	    In the case
...
```
The Type is as specified in [Table 10](#prefix-attribute_tlv). The length is variable.

<a name="PRIVATE"></a>
## Private Use

TLVs for Vendor Private Use are supported using the code point
        range reserved as indicated in [Section 7](#IANA). For such TLV use
        in the NLRI or BGP-LS Attribute, the format described in [Section 5.1](#TLV-section) is to be used and a 4-octet field **MUST** be
        ...
        field of the Link-State NLRI format as described in [Section 5.2](#BGPLSNLRI) to carry the Enterprise Code [ENTNUM](#ENTNUM). This enables the use of vendor-specific extensions
        without conflicts.
...
BGP link-state information for both IPv4 and IPv6 networks can be
        ...
        **SHOULD** be an IPv4 address. Similarly, if an IPv6 BGP session is used,
        ...
        [RFC4760](#RFC4760). The Length field of the next-hop address
        ...
        [RFC2545](#RFC2545). For VPN Subsequent Address Family Identifier
        (SAFI), as per custom, an 8-byte Route Distinguisher set to all zero
        is prepended to the next hop.

The BGP Next-Hop is used by each BGP-LS Speaker to validate the
        ...
        any rule regarding the rewrite of the BGP Next-Hop.

<a name="INTERAS"></a>
## Inter-AS Links

The main source of TE information is the IGP, which is not active
        on inter-AS links. In some cases, the IGP may have information of
        inter-AS links [RFC5392](#RFC5392) [RFC9346](#RFC9346). In
        ...
        inter-AS links is outside the scope of this document.

<a name="OSPFVL"></a>
## OSPF Virtual Links and Sham Links

In an OSPF [RFC2328](#RFC2328) [RFC5340](#RFC5340)
...
In an OSPF network, sham links [RFC4577](#RFC4577) [RFC6565](#RFC6565) are used to provide intra-area connectivity between
...
<a name="OSPFTYPE4"></a>
## OSPFv2 Type 4 Summary-LSA & OSPFv3 Inter-Area-Router-LSA

OSPFv2 [RFC2328](#RFC2328) defines the type 4 summary-LSA and
        OSPFv3 [RFC5340](#RFC5340) defines the inter-area-router-LSA for an Area
        ...
        carried by these LSAs is outside the scope of this document.

<a name="UNREACHNODES"></a>
## Handling of Unreachable IGP Nodes

Consider an OSPF network as shown in [Figure 31](#INCORRECTTOPORR), where R2 and R3 are the BGP-LS Producers
        ...
        the a0 and a1 references respectively against the links.

A BGP-LS Consumer talks to BGP route reflector RR0, which is a
...
```
Consider a scenario where the link between R5 and R6 is
...
Now, R5 will remove the link R5-R6 from its Router LSA, and this
        ...
        from R6's stale Router LSA.

At the same time, R6 has removed the link R6-R5 from its Router
        ...
        derives from R5's stale Router LSA.
...
A BGP-LS Producer **SHOULD** withdraw all link-state objects advertised
        ...
        **MAY** continue to advertise link-state objects corresponding to
        ...
        as those described in [RFC7911](#RFC7911) when using RRs. Details of these
...
The encoding of a broadcast LAN in IS-IS provides a good example of
        how Router-IDs are encoded. Consider [Figure 32](#ISISPseudonodes).
...
```
<a name="OSPFPN"></a>
## Router-ID Anchoring Example: OSPF Pseudonode

The encoding of a broadcast LAN in OSPF provides a good example of
        how Router-IDs and local Interface IPs are encoded. Consider [Figure 33](#OSPFPseudonodes). This represents a broadcast LAN between a
...
The LAN subnet 198.51.100.0/24 is not included in the Router LSA of
        ...
        mask from the Network LSA.

<a name="OSPF2ISIS"></a>
## Router-ID Anchoring Example: OSPFv2 to IS-IS Migration
...
Consider a point-to-point link between two routers, A and B, which
	initially were OSPFv2-only routers and then had IS-IS enabled on them.
        ...
        (A, B) can be identified in both the IS-IS and OSPF protocols.

<a name="LINKPATHAGGREGATION"></a>
...
## Example: No Link Aggregation

Consider [Figure 34](#no-link-aggregation). Both AS1 and AS2
...
The brief difference between the "no-link aggregation" example and
        this example is that no specific link gets exposed. Consider [Figure 35](#asbr-link-aggregation). The only link that gets advertised
        ...
        used are hidden from the topology.
...
```
## Example: Multi-AS Path Aggregation

Service providers in control of multiple ASes may even decide to
        not expose their internal inter-AS links. Consider [Figure 36](#multi-as-aggregation). AS3 is modeled as a single node that
        connects to the border routers of the aggregated domain.
...
As this document obsoletes [RFC7752](#RFC7752) and [RFC9029](#RFC9029), IANA has updated all registration
...
IANA has assigned SAFI values 71 (BGP-LS) and 72 (BGP-LS-VPN) in the
      ...
      (SAFI) Parameters" registry group.
...
IANA has created a "Border Gateway Protocol - Link-State (BGP-LS)
      ...
      [https://www.iana.org/assignments/bgp-ls-parameters](https://www.iana.org/assignments/bgp-ls-parameters).
...
This section also incorporates all the changes to the allocation
      ...
      for designated experts introduced by [RFC9029](#RFC9029).
...
<a name="NLRITYPESREG"></a>
### BGP-LS NLRI Types Registry
...
A range is reserved for Private Use [RFC8126](#RFC8126). All
          other allocations within the registry are to be made using the
          "Expert Review" policy [RFC8126](#RFC8126), which requires
          documentation of the proposed use of the allocated value and
          approval by the designated expert assigned by the IESG.

<a name="PROTOIDREG"></a>
### BGP-LS Protocol-IDs Registry

The "BGP-LS Protocol-IDs" registry has been set up for assignment
          for the one-octet-sized code points for BGP-LS Protocol-IDs and
          populated with the values shown below:

**Table: BGP-LS Protocol-IDs**
...
The "BGP-LS Well-Known Instance-IDs" registry that was set up via
          [RFC7752](#RFC7752) is no longer required. IANA has
          marked this registry obsolete and changed its registration
          procedure to "registry closed".

<a name="NFLAGSIDREG"></a>
### BGP-LS Node Flags Registry

The "BGP-LS Node Flags" registry has been created for
          the one-octet-sized flags field of the Node Flag Bits TLV (1024) and
          populated with the initial values shown below:

**Table: BGP-LS Node Flags**
...
Allocations within the registry are to be made using the "Expert
          Review" policy [RFC8126](#RFC8126), which requires documentation
          of the proposed use of the allocated value and approval by the
          designated expert assigned by the IESG.

<a name="MPLSMASKREG"></a>
### BGP-LS MPLS Protocol Mask Registry

The "BGP-LS MPLS Protocol Mask" registry has been
...
**Table: BGP-LS MPLS Protocol Mask**
...
<a name="IGPPFLAGSREG"></a>
### BGP-LS IGP Prefix Flags Registry

The "BGP-LS IGP Prefix Flags" registry has been created
          for the one-octet-sized flags field of the IGP Flags TLV (1152) and
          populated with the initial values shown below:

**Table: BGP-LS IGP Prefix Flags**
...
<a name="TLVREG"></a>
### BGP-LS TLVs Registry

The "BGP-LS Node Descriptor, Link Descriptor, Prefix Descriptor,
          and Attribute TLVs" registry was created via [RFC7752](#RFC7752). Per this document, IANA has renamed that
...
**Table: BGP-LS TLVs Registration Process**
...
A range is reserved for Private Use [RFC8126](#RFC8126). All
          ...
          are to be made using the "Expert Review" policy [RFC8126](#RFC8126), which requires documentation of the proposed use
          of the allocated value and approval by the designated expert
          assigned by the IESG.

The registry was pre-populated with the values shown in [Table 18](#BGPLSCODEPOINTS), and the reference for each 
          allocation has been changed to this document and the respective
          section where those TLVs are specified.

<a name="DE-Guidance"></a>
...
8. In the event that the document is a working group document or
...
<a name="Operational-Considerations"></a>
...
It is **RECOMMENDED** that operators deploying BGP-LS enable two or
...
In a multi-domain IGP network, the correct provisioning of the
          ...
          to [Section 5.2](#BGPLSNLRI) for more details.

<a name="Initial-Setup"></a>
### Installation and Initial Setup

Configuration parameters defined in [Section 8.2.3](#Configuration-Management) **SHOULD** be initialized to the
          following default values:

- The Link-State NLRI capability is turned off for all
              neighbors.
- The maximum rate at which Link-State NLRIs will be
              advertised/withdrawn from neighbors is set to 200 updates per
              second.

<a name="Migration-Path"></a>
### Migration Path
...
The proposed extension is only activated between BGP peers after
          ...
          on/off on an individual peer basis (see [Section 8.2.3](#Configuration-Management)), so the extension can be
          gradually rolled out in the network.

<a name="Other-Protocols"></a>
...
<a name="Network-Operation"></a>
### Impact on Network Operation

The frequency of Link-State NLRI updates could interfere with
          ...
          NLRIs as discussed in [Section 8.1.1](#Operations).

Distribution of Link-State NLRIs **SHOULD** be limited to a single
          admin domain, which can consist of multiple areas within an AS or
          multiple ASes.

<a name="Verifying-Correct-Operation"></a>
...
<a name="Management-Information"></a>
### Management Information

The IDR Working Group has documented and continues to document
...
This section describes the fault management actions, as described
          in [RFC7606](#RFC7606), that are to be performed for the
          handling of BGP UPDATE messages for BGP-LS.
...
A Link-State NLRI **MUST NOT** be considered malformed or invalid
          ...
          fields (i.e., semantic errors), as described in Sections [5.1](#TLV-section) and [5.2](#BGPLSNLRI).

A BGP-LS Speaker **MUST** perform the following syntactic validation
...
- The sum of all TLV lengths found in a Link-State NLRI
              corresponds to the Total NLRI Length field of all its
              descriptors.
- The length of the TLVs and, when the TLV is recognized then,
              the length of its sub-TLVs in the NLRI are valid.
...
- The rule regarding the ordering of TLVs has been followed as
              described in [Section 5.1](#TLV-section).
- For NLRIs carrying either a Local or Remote Node Descriptor
              TLV, there is not more than one instance of a sub-TLV
              present.

When the error that is determined allows for the router to skip
          ...
          processing similar to what is described in [RFC7606](#RFC7606)). In other cases, where the error in the NLRI
...
A BGP-LS Attribute **MUST NOT** be considered malformed or invalid
          based on the inclusion/exclusion of TLVs or contents of the TLV
          fields (i.e., semantic errors), as described in Sections [5.1](#TLV-section) and [5.3](#BGPLSATTR).
...
- The syntactic correctness of the Attributes (including the BGP-LS
              Attribute) have been verified as per [RFC7606](#RFC7606).
- The length of each TLV and, when the TLV is recognized then,
              the length of its sub-TLVs in the BGP-LS Attribute are valid.
...
When the error that is determined allows for the router to skip
          ...
          **MUST** handle such malformed BGP-LS Attribute as 'Attribute Discard'.
          ...
          handling is the same as described above for the malformed NLRI.

Note that the 'Attribute Discard' action results in the loss of
...
When a BGP Speaker receives an UPDATE message with Link-State
          ...
          **SHOULD** preserve and propagate the Link-State NLRIs, unless denied by
          ...
          detected the fault with the BGP-LS Attribute.

An implementation **SHOULD** log a message for any errors found
          during syntax validation for further analysis.

A BGP-LS Propagator, even when it has a coexisting BGP-LS
...
- the inclusion and use of TLVs/sub-TLVs with specific
              Link-State NLRI types is valid
...
An implementation **SHOULD** allow the operator to specify neighbors
          to which Link-State NLRIs will be advertised and from which
          Link-State NLRIs will be accepted.
...
An implementation **MUST** allow the operator to configure an 8-octet
          BGP-LS Instance-ID. Refer to [Section 5.2](#BGPLSNLRI) for guidance
          to the operator for the configuration of BGP-LS Instance-ID.

An implementation **SHOULD** allow the operator to configure Autonomous System Number (ASN) and
          BGP-LS identifiers (refer to [Section 5.2.1.4](#node_desc_tlvs)).

An implementation **SHOULD** allow the operator to configure a 4096-byte size limit for a BGP-LS UPDATE message on a BGP-LS Producer or allow larger values when they know that all BGP-LS Speakers support the extended message size [RFC8654](#RFC8654).

<a name="Accounting-Management"></a>
...
- Number of Link-State NLRI updates sent/received, per
              neighbor
- Number of errored received Link-State NLRI updates, per
              neighbor
...
An operator **MUST** define an import policy to limit inbound updates
          as follows:

- Drop all updates from peers that are only serving BGP-LS
              Consumers.

An implementation **MUST** have the means to limit inbound
          updates.
...
**Table: Summary Table of TLV/Sub-TLV Code Points**

| TLV Code Point | Description | Reference Section |
| --- | --- | --- |
| 256 | Local Node Descriptors | [Section 5.2.1.2](#LOCALNODEDESC) |
| 257 | Remote Node Descriptors | [Section 5.2.1.3](#REMOTENODEDESC) |
| 258 | Link Local/Remote Identifiers | [Section 5.2.2](#LINKDESC) |
| 259 | IPv4 interface address | [Section 5.2.2](#LINKDESC) |
| 260 | IPv4 neighbor address | [Section 5.2.2](#LINKDESC) |
| 261 | IPv6 interface address | [Section 5.2.2](#LINKDESC) |
| 262 | IPv6 neighbor address | [Section 5.2.2](#LINKDESC) |
| 263 | Multi-Topology Identifier | [Section 5.2.2.1](#MT-ID) |
| 264 | OSPF Route Type | [Section 5.2.3.1](#OSPFRTETYPE) |
| 265 | IP Reachability Information | [Section 5.2.3.2](#IPREACHINFO) |
| 512 | Autonomous System | [Section 5.2.1.4](#node_desc_tlvs) |
| 513 | BGP-LS Identifier (deprecated) | [Section 5.2.1.4](#node_desc_tlvs) |
| 514 | OSPF Area-ID | [Section 5.2.1.4](#node_desc_tlvs) |
| 515 | IGP Router-ID | [Section 5.2.1.4](#node_desc_tlvs) |
| 1024 | Node Flag Bits | [Section 5.3.1.1](#NODEFLAGBITS) |
| 1025 | Opaque Node Attribute | [Section 5.3.1.5](#OPAQUENODE) |
| 1026 | Node Name | [Section 5.3.1.3](#NODENAME) |
| 1027 | IS-IS Area Identifier | [Section 5.3.1.2](#ISISAREA) |
| 1028 | IPv4 Router-ID of Local Node | Sections
              [5.3.1.4](#aux_routerid_node) and [5.3.2.1](#aux_routerid_link) |
| 1029 | IPv6 Router-ID of Local Node | Sections
              [5.3.1.4](#aux_routerid_node) and [5.3.2.1](#aux_routerid_link) |
| 1030 | IPv4 Router-ID of Remote Node | [Section 5.3.2.1](#aux_routerid_link) |
| 1031 | IPv6 Router-ID of Remote Node | [Section 5.3.2.1](#aux_routerid_link) |
| 1088 | Administrative group (color) | [Section 5.3.2](#link_attribute) |
| 1089 | Maximum link bandwidth | [Section 5.3.2](#link_attribute) |
| 1090 | Max. reservable link bandwidth | [Section 5.3.2](#link_attribute) |
| 1091 | Unreserved bandwidth | [Section 5.3.2](#link_attribute) |
| 1092 | TE Default Metric | [Section 5.3.2.3](#TEDEFAULTMETTLV) |
| 1093 | Link Protection Type | [Section 5.3.2](#link_attribute) |
| 1094 | MPLS Protocol Mask | [Section 5.3.2.2](#MPLSPROTOTLV) |
| 1095 | IGP Metric | [Section 5.3.2.4](#IGPMETTLV) |
| 1096 | Shared Risk Link Group | [Section 5.3.2.5](#SRLGTLV) |
| 1097 | Opaque Link Attribute | [Section 5.3.2.6](#OPAQUELINK) |
| 1098 | Link Name | [Section 5.3.2.7](#LINKNAME) |
| 1152 | IGP Flags | [Section 5.3.3.1](#IGPFLAGS) |
| 1153 | IGP Route Tag | [Section 5.3.3.2](#route_tag) |
| 1154 | IGP Extended Route Tag | [Section 5.3.3.3](#ext_route_tag) |
| 1155 | Prefix Metric | [Section 5.3.3.4](#prefix_metric) |
| 1156 | OSPF Forwarding Address | [Section 5.3.3.5](#ospf_fwd_addr) |
| 1157 | Opaque Prefix Attribute | [Section 5.3.3.6](#OPAQUEPREFIX) |
...
Procedures and protocol extensions defined in this document do not
      ...
      of [RFC4271](#RFC4271) for a discussion of BGP security. Also,
      refer to [RFC4272](#RFC4272) and [RFC6952](#RFC6952) for
      analysis of security issues for BGP.
...
The operator should ensure that a BGP-LS Speaker does not accept
      ...
      Consumer by using the policy configuration options discussed in Sections [8.2.3](#Configuration-Management) and [8.2.6](#Security-Management). Generally, an operator is aware of the
      ...
      represent erroneous information, feedback loops, or false input.
...
<a name="ENTNUM"></a> **[ENTNUM]** "Private Enterprise Numbers (PENs)", <https://www.iana.org/assignments/enterprise-numbers/>
...
<a name="RFC4203"></a> **[RFC4203]** K. Kompella, Y. Rekhter, "OSPF Extensions in Support of Generalized Multi-Protocol Label Switching (GMPLS)", RFC 4203, DOI 10.17487/RFC4203, October 2005. <https://www.rfc-editor.org/info/rfc4203> [Local MD](rfc4203.md)
...
<a name="RFC4577"></a> **[RFC4577]** E. Rosen, P. Psenak, P. Pillay-Esnault, "OSPF as the Provider/Customer Edge Protocol for BGP/MPLS IP Virtual Private Networks (VPNs)", RFC 4577, DOI 10.17487/RFC4577, June 2006. <https://www.rfc-editor.org/info/rfc4577> [Local MD](rfc4577.md)
...
<a name="RFC4915"></a> **[RFC4915]** P. Psenak, S. Mirtorabi, A. Roy, L. Nguyen, P. Pillay-Esnault, "Multi-Topology (MT) Routing in OSPF", RFC 4915, DOI 10.17487/RFC4915, June 2007. <https://www.rfc-editor.org/info/rfc4915> [Local MD](rfc4915.md)

<a name="RFC5036"></a> **[RFC5036]** L. Andersson, I. Minei, B. Thomas, "LDP Specification", RFC 5036, DOI 10.17487/RFC5036, October 2007. <https://www.rfc-editor.org/info/rfc5036> [Local MD](rfc5036.md)

<a name="RFC5120"></a> **[RFC5120]** T. Przygienda, N. Shen, N. Sheth, "M-ISIS: Multi Topology (MT) Routing in Intermediate System to Intermediate Systems (IS-ISs)", RFC 5120, DOI 10.17487/RFC5120, February 2008. <https://www.rfc-editor.org/info/rfc5120> [Local MD](rfc5120.md)

<a name="RFC5130"></a> **[RFC5130]** S. Previdi, M. Shand, C. Martin, "A Policy Control Mechanism in IS-IS Using Administrative Tags", RFC 5130, DOI 10.17487/RFC5130, February 2008. <https://www.rfc-editor.org/info/rfc5130> [Local MD](rfc5130.md)

<a name="RFC5301"></a> **[RFC5301]** D. McPherson, N. Shen, "Dynamic Hostname Exchange Mechanism for IS-IS", RFC 5301, DOI 10.17487/RFC5301, October 2008. <https://www.rfc-editor.org/info/rfc5301> [Local MD](rfc5301.md)
...
<a name="RFC5642"></a> **[RFC5642]** S. Venkata, S. Harwani, C. Pignataro, D. McPherson, "Dynamic Hostname Exchange Mechanism for OSPF", RFC 5642, DOI 10.17487/RFC5642, August 2009. <https://www.rfc-editor.org/info/rfc5642> [Local MD](rfc5642.md)

<a name="RFC5890"></a> **[RFC5890]** J. Klensin, "Internationalized Domain Names for Applications (IDNA): Definitions and Document Framework", RFC 5890, DOI 10.17487/RFC5890, August 2010. <https://www.rfc-editor.org/info/rfc5890> [Local MD](rfc5890.md)
...
<a name="RFC6565"></a> **[RFC6565]** P. Pillay-Esnault, P. Moyer, J. Doyle, E. Ertekin, M. Lundberg, "OSPFv3 as a Provider Edge to Customer Edge (PE-CE) Routing Protocol", RFC 6565, DOI 10.17487/RFC6565, June 2012. <https://www.rfc-editor.org/info/rfc6565> [Local MD](rfc6565.md)
...
<a name="RFC7684"></a> **[RFC7684]** P. Psenak, H. Gredler, R. Shakir, W. Henderickx, J. Tantsura, A. Lindem, "OSPFv2 Prefix/Link Attribute Advertisement", RFC 7684, DOI 10.17487/RFC7684, November 2015. <https://www.rfc-editor.org/info/rfc7684> [Local MD](rfc7684.md)

<a name="RFC7770"></a> **[RFC7770]** A. Lindem, N. Shen, JP. Vasseur, R. Aggarwal, S. Shaffer, "Extensions to OSPF for Advertising Optional Router Capabilities", RFC 7770, DOI 10.17487/RFC7770, February 2016. <https://www.rfc-editor.org/info/rfc7770> [Local MD](rfc7770.md)
...
<a name="RFC8362"></a> **[RFC8362]** A. Lindem, A. Roy, D. Goethals, V. Reddy Vallem, F. Baker, "OSPFv3 Link State Advertisement (LSA) Extensibility", RFC 8362, DOI 10.17487/RFC8362, April 2018. <https://www.rfc-editor.org/info/rfc8362> [Local MD](rfc8362.md)

<a name="RFC8654"></a> **[RFC8654]** R. Bush, K. Patel, D. Ward, "Extended Message Support for BGP", RFC 8654, DOI 10.17487/RFC8654, October 2019. <https://www.rfc-editor.org/info/rfc8654> [Local MD](rfc8654.md)
...
<a name="RFC5392"></a> **[RFC5392]** M. Chen, R. Zhang, X. Duan, "OSPF Extensions in Support of Inter-Autonomous System (AS) MPLS and GMPLS Traffic Engineering", RFC 5392, DOI 10.17487/RFC5392, January 2009. <https://www.rfc-editor.org/info/rfc5392> [Local MD](rfc5392.md)

<a name="RFC5693"></a> **[RFC5693]** J. Seedorf, E. Burger, "Application-Layer Traffic Optimization (ALTO) Problem Statement", RFC 5693, DOI 10.17487/RFC5693, October 2009. <https://www.rfc-editor.org/info/rfc5693> [Local MD](rfc5693.md)

<a name="RFC5706"></a> **[RFC5706]** D. Harrington, "Guidelines for Considering Operations and Management of New Protocols and Protocol Extensions", RFC 5706, DOI 10.17487/RFC5706, November 2009. <https://www.rfc-editor.org/info/rfc5706> [Local MD](rfc5706.md)

<a name="RFC6549"></a> **[RFC6549]** A. Lindem, A. Roy, S. Mirtorabi, "OSPFv2 Multi-Instance Extensions", RFC 6549, DOI 10.17487/RFC6549, March 2012. <https://www.rfc-editor.org/info/rfc6549> [Local MD](rfc6549.md)
...
<a name="RFC8202"></a> **[RFC8202]** L. Ginsberg, S. Previdi, W. Henderickx, "IS-IS Multi-Instance", RFC 8202, DOI 10.17487/RFC8202, June 2017. <https://www.rfc-editor.org/info/rfc8202> [Local MD](rfc8202.md)

<a name="RFC9029"></a> **[RFC9029]** A. Farrel, "Updates to the Allocation Policy for the Border Gateway Protocol - Link State (BGP-LS) Parameters Registries", RFC 9029, DOI 10.17487/RFC9029, June 2021. <https://www.rfc-editor.org/info/rfc9029> [Local MD](rfc9029.md)

<a name="RFC9346"></a> **[RFC9346]** M. Chen, L. Ginsberg, S. Previdi, D. Xiaodong, "IS-IS Extensions in Support of Inter-Autonomous System (AS) MPLS and GMPLS Traffic Engineering", RFC 9346, DOI 10.17487/RFC9346, February 2023. <https://www.rfc-editor.org/info/rfc9346> [Local MD](rfc9346.md)
...
1. Updated [Figure 1](#MECHANISM-OVERVIEW) in [Section 1](#INTRO) and added [Section 3](#ROLES) to illustrate the
...
2. Clarified aspects related to advertisement of link-state
          information from IGPs into BGP-LS in [Section 4](#IGPTOBGP).
3. In [Section 5.1](#TLV-section), clarified aspects about TLV
          ...
          unknown TLV and so, based on [RFC7606](#RFC7606) guidelines,
          might discard the unknown NLRI types. This aspect is now
          unambiguously clarified in [Section 5.2](#BGPLSNLRI). Also, the
          TLVs in the BGP-LS Attribute that are not ordered are not to be
          considered malformed.
4. Clarified aspects of mandatory and optional TLVs in both NLRI and
          BGP-LS Attribute portions all through the document.
5. In [Section 5.3](#BGPLSATTR), the handling of a large-sized BGP-LS Attribute with growth in BGP-LS
...
6. Clarified that the document describes the NLRI descriptor TLVs
          ...
          and NLRI types that they introduce.
7. In [Section 5.2](#BGPLSNLRI), clarified the use of the Identifier field in the
          ...
          on a router. The IANA registry is accordingly being removed.
8. The BGP-LS Identifier TLV in the Node Descriptors has been
          deprecated. Its use was not well specified by [RFC7752](#RFC7752), and there has been some amount of confusion
...
9. Clarified that the Area-ID TLV is mandatory in the Node
          ...
          applicable. Also clarified the IS-IS area and area addresses.
10. Moved the MT-ID TLV from the Node Descriptor section to under the
...
11. Clarified that IPv6 link-local addresses are not advertised in
          ...
          used instead for links with IPv6 link-local addresses only.
12. Updated the usage of OSPF Route Type TLV to mandate its use for
          OSPF prefixes in [Section 5.2.3.1](#OSPFRTETYPE) since this is required
          ...
          prefixes.
13. Clarified the specific OSPFv2 and OSPFv3 protocol TLV
...
15. Updated the Node Name TLV in [Section 5.3.1.3](#NODENAME) with the
          OSPF specification.
16. Clarified the size of the IS-IS Narrow Metric
...
17. Clarified the advertisement of the prefix corresponding to the
          LAN segment in an OSPF network in [Section 5.11](#OSPFPN).
18. Clarified the advertisement and support for OSPF-specific
          concepts like virtual links, sham links, and Type 4 LSAs in Sections [5.7](#OSPFVL) and [5.8](#OSPFTYPE4).
19. Introduced the Private Use TLV code point space and specified their
          encoding in [Section 5.4](#PRIVATE).
20. In [Section 5.9](#UNREACHNODES), introduced where issues related to
          the consistency of reporting IGP link-state along with their
          solutions are covered.
21. Added a recommendation for isolation of BGP-LS sessions from other
          BGP route exchanges to avoid errors and faults in BGP-LS affecting
          the normal BGP routing.
22. Updated the Fault Management section with detailed rules based on
...
23. Changed the management of BGP-LS IANA registries from
          ...
          of changes introduced via [RFC9029](#RFC9029) that are obsoleted
          by this document.
...
This document update to the BGP-LS specification [RFC7752](#RFC7752) is a result of feedback and input from the
      ...
      experience with BGP-LS.

Cengiz Alaettinoglu and Parag Amritkar brought forward the need to
      clarify the advertisement of a LAN subnet for OSPF.

We would like to thank Balaji Rajagopalan, Srihari Sangli, Shraddha Hegde,
      Andrew Stone, Jeff Tantsura, Acee Lindem, Les Ginsberg, Jie Dong,
      ...
      Jeffrey Haas for his detailed shepherd review and input for improving
      the document.
...
We would like to thank Nischal Sheth, Alia Atlas, David Ward, Derek Yeung,
      Murtuza Lightwala, John Scudder, Kaliraj Vairavakkalai, Les Ginsberg,
      Liem Nguyen, Manish Bhardwaj, Matt Miller, Mike Shand, Peter Psenak,
      Rex Fernando, Richard Woundy, Steven Luong, Tamas Mondal, Waqas Alam,
      Vipin Kumar, Naiming Shen, Carlos Pignataro, Balaji Rajagopalan,
      Yakov Rekhter, Alvaro Retana, Barry Leiba, and Ben Campbell for their
      comments on [RFC7752](#RFC7752).

<a name="Contributors"></a>
# Contributors

The following persons contributed significant text to [RFC7752](#RFC7752) and
