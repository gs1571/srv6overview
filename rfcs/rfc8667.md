...
**Workgroup:** IS-IS for IP Internets

**Keywords:** MPLS, SID, IGP, IS-IS, Label advertisement, Segment Routing

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-pce-segment-routing-16)
- [alternate](https://dx.doi.org/10.17487/rfc8667)
...
- [2. Segment Routing Identifiers](#section-2)
  - [2.1. Prefix Segment Identifier (Prefix-SID) Sub-TLV](#PREFIXSIDSUBTLV)
    - [2.1.1. Flags](#FLAGS)
    - [2.1.2. Prefix-SID Propagation](#PROPAGATION)
  - [2.2. Adjacency Segment Identifier](#section-2.2)
    - [2.2.1. Adjacency Segment Identifier (Adj-SID) Sub-TLV](#ADJSIDSUBTLV)
    - [2.2.2. Adjacency Segment Identifier (LAN-Adj-SID) Sub-TLV](#LANADJSIDSUBTLV)
  - [2.3. SID/Label Sub-TLV](#SIDLABELSUBTLV)
  - [2.4. SID/Label Binding TLV](#BINDINGTLV)
    - [2.4.1. Flags](#section-2.4.1)
    - [2.4.2. Range](#section-2.4.2)
    - [2.4.3. Prefix Length, Prefix](#section-2.4.3)
    - [2.4.4. Mapping Server Prefix-SID](#section-2.4.4)
    - [2.4.5. SID/Label Sub-TLV](#BSIDSUBTLV)
    - [2.4.6. Example Encodings](#BSIDEXAMPLE)
  - [2.5. Multi-Topology SID/Label Binding TLV](#MTBINDINGTLV)
- [3. Router Capabilities](#section-3)
  - [3.1. SR-Capabilities Sub-TLV](#SRCAPSUBTLV)
  - [3.2. SR-Algorithm Sub-TLV](#SRALGOSUBTLV)
  - [3.3. SR Local Block Sub-TLV](#SRLBSUBTLV)
  - [3.4. SRMS Preference Sub-TLV](#SRMSPREFSUBTLV)
- [4. IANA Considerations](#IANA)
  - [4.1. Sub-TLVs for Types 22, 23, 25, 141, 222, and 223](#section-4.1)
  - [4.2. Sub-TLVs for Types 135, 235, 236, and 237](#section-4.2)
  - [4.3. Sub-TLVs for Type 242](#section-4.3)
  - [4.4. New TLV Codepoint and Sub-TLV Registry](#section-4.4)
...
There are additional segment types, e.g., the Binding SID as defined in
      [RFC8402](#RFC8402). This document also defines an advertisement
      for one type of Binding SID: the Mirror Context segment.

This document describes the IS-IS extensions that need to be
      introduced for Segment Routing operating on an MPLS data plane.

The Segment Routing architecture is described in [RFC8402](#RFC8402).  Segment Routing use cases are described in [RFC7855](#RFC7855).
...
The Segment Routing architecture [RFC8402](#RFC8402) defines
      ...
      the IGP-LAN-Adjacency Segment, and the Binding Segment.

<a name="PREFIXSIDSUBTLV"></a>
## Prefix Segment Identifier (Prefix-SID) Sub-TLV

A new IS-IS sub-TLV is defined: the Prefix Segment Identifier
        (Prefix-SID) sub-TLV.

The Prefix-SID sub-TLV carries the Segment Routing IGP-Prefix-SID
        as defined in [RFC8402](#RFC8402). The 'Prefix-SID' **MUST** be
        unique within a given IGP domain (when the L-Flag is not set).

A Prefix-SID sub-TLV is associated to a prefix advertised by a node
        and **MAY** be present in any of the following TLVs:

- TLV-135 (Extended IPv4 reachability) defined in [RFC5305](#RFC5305).
- TLV-235 (Multi-topology IPv4 Reachability) defined in [RFC5120](#RFC5120).
- TLV-236 (IPv6 IP Reachability) defined in [RFC5308](#RFC5308).
- TLV-237 (Multi-topology IPv6 IP Reachability) defined in [RFC5120](#RFC5120).
- The Binding TLV and Multi-Topology Binding TLV are defined in Sections [2.4](#BINDINGTLV) and [2.5](#MTBINDINGTLV),
            respectively.
...
- Type:
      ...
      +-+-+-+-+-+-+-+-+
                where:
                R-Flag:
                  Re-advertisement Flag. If set, then the prefix to
                ...
                another protocol).
                  N-Flag:
                  Node-SID Flag. If set, then the Prefix-SID refers
                ...
                described in [RFC8402](#RFC8402).
                  P-Flag:
                  No-PHP 
(No Penultimate Hop-Popping) Flag. If set, then the penultimate hop **MUST NOT** pop the Prefix-SID before delivering the packet to the
              ...
              a Prefix-SID that has an Explicit NULL value (0 for IPv4 and 2
                for IPv6) before forwarding the packet.
                  V-Flag:
                  Value Flag. If set, then the Prefix-SID carries a
                value (instead of an index). By default, the flag is UNSET.
                  L-Flag:
                  Local Flag. If set, then the value/index carried by
                the Prefix-SID has local significance. By default, the flag is
                UNSET.
                  Other bits:
                  **MUST** be zero when originated and ignored when
                received.

- Algorithm:
              the router may use various algorithms when
            calculating reachability to other nodes or to prefixes attached to
            these nodes. Algorithm identifiers are defined in [Section 3.2](#SRALGOSUBTLV). Examples of these algorithms are metric-based
...
- At origination, the Prefix-SID Algorithm field **MUST** be set to 0
            or to any value advertised in the SR-Algorithm sub-TLV (see [Section 3.2](#SRALGOSUBTLV)).
- A router receiving a Prefix-SID from a remote node and with an
            ...
            SR-Algorithm sub-TLV (see [Section 3.2](#SRALGOSUBTLV)) **MUST** ignore
            the Prefix-SID sub-TLV.
- SID/Index/Label as defined in [Section 2.1.1.1](#VANDLFLAGS).
...
In many use cases, a 'stable transport' address is overloaded as an
        ...
        those among other Prefixes advertised from a given node.
...
The V-Flag indicates whether the SID/Index/Label field is a
            value or an index.

The L-Flag indicates whether the value/index in the
            SID/Index/Label field has local or global significance.
...
The R-Flag **MUST** be set for prefixes that are not local to the
            router and are advertised because of:
...
The N-Flag is used in order to define a Node-SID. A router **MAY**
            set the N-Flag only if all of the following conditions are
            met:

- The prefix to which the Prefix-SID is attached is local to
                the router (i.e., the prefix is configured on one of the local
                interfaces, e.g., a 'stable transport' loopback).
- The prefix to which the Prefix-SID is attached has a Prefix
                length of either /32 (IPv4) or /128 (IPv6).

The router **MUST** ignore the N-Flag on a received Prefix-SID if
...
The Prefix Attribute Flags sub-TLV [RFC7794](#RFC7794)
            ...
            of the N-Flag and R-Flag advertised in that sub-TLV **MUST** be used, and
...
- If the P-Flag is not set, then any upstream neighbor of the
                ...
                unset, the received E-Flag is ignored.
- If the P-Flag is set, then:
                If the E-Flag is not set, then any upstream neighbor of
...
The Prefix-SID sub-TLV **MUST** be included when the associated
          Prefix Reachability TLV is propagated across level boundaries.

The Level-1-2 router that propagates the Prefix-SID sub-TLV
          between levels maintains the content (flags and SID), except as noted
          in Sections [2.1.1.2](#RANDNFLAGS) and [2.1.1.3](#EANDPFLAGS).

## Adjacency Segment Identifier

A new IS-IS sub-TLV is defined: the Adjacency Segment Identifier (Adj-SID)
        sub-TLV.

The Adj-SID sub-TLV is an optional sub-TLV carrying the Segment
        Routing IGP-Adjacency-SID as defined in [RFC8402](#RFC8402) with
        flags and fields that may be used, in future extensions of Segment
        Routing, for carrying other types of SIDs.

IS-IS adjacencies are advertised using one of the IS Neighbor TLVs
        below:

- TLV-22 (Extended IS reachability) [RFC5305](#RFC5305)
- TLV-222 (MT-ISN) [RFC5120](#RFC5120)
- TLV-23 (IS Neighbor Attribute) [RFC5311](#RFC5311)
- TLV-223 (MT IS Neighbor Attribute) [RFC5311](#RFC5311)
- TLV-141 (inter-AS reachability information) [RFC5316](#RFC5316)

Multiple Adj-SID sub-TLVs **MAY** be associated with a single
        IS Neighbor.
...
- Type:
      ...
      +-+-+-+-+-+-+-+-+
                  ...
                  IPv6-encapsulated traffic to the neighbor.
                    B-Flag:
                    Backup Flag. If set, the Adj-SID is eligible for
                  ...
                  value. By default, the flag is SET.
                    L-Flag:
                    Local Flag. If set, then the value/index carried
...
- An SR-capable router **MAY** allocate more than one Adj-SID to an
              adjacency.
- An SR-capable router **MAY** allocate the same Adj-SID to
...
- Examples of Adj-SID sub-TLV use are described in [RFC8402](#RFC8402).
...
When Segment Routing is used, each router in the LAN **MAY**
...
The following new sub-TLV is defined: LAN Adjacency Segment Identifier (LAN-Adj-SID) containing the
...
- Type:
      ...
      |F|B|V|L|S|P|   | 
      +-+-+-+-+-+-+-+-+
                  where the F-Flag, B-Flag, V-Flag, L-Flag, S-Flag, and P-Flag are defined in [Section 2.2.1](#ADJSIDSUBTLV).
...
- Other bits:
              ...
              is defined in [RFC8402](#RFC8402).
                Neighbor System-ID:
                IS-IS System-ID of length "ID Length" as
              defined in [ISO10589](#ISO10589).
                SID/Index/Label:
                As defined in [Section 2.1.1.1](#VANDLFLAGS).

Multiple LAN-Adj-SID sub-TLVs **MAY** be encoded.

Note that this sub-TLV **MUST NOT** appear in TLV 141.

In case TLV-22, TLV-23, TLV-222, or TLV-223 (reporting the adjacency to the
...
Each router within the level, by receiving the DIS PN LSP as well
...
<a name="SIDLABELSUBTLV"></a>
## SID/Label Sub-TLV

The SID/Label sub-TLV may be present in the following TLVs/sub-TLVs
        defined in this document:

- SR-Capabilities sub-TLV ([Section 3.1](#SRCAPSUBTLV))
- SR Local Block sub-TLV ([Section 3.3](#SRLBSUBTLV))
- SID/Label Binding TLV ([Section 2.4](#BINDINGTLV))
- Multi-Topology SID/Label Binding TLV ([Section 2.5](#MTBINDINGTLV))

Note that the codepoint used in all of the above cases is the
        SID/Label sub-TLV codepoint specified in the new "sub-TLVs for
        TLV 149 and 150" registry created by this document.
...
The SID/Label Binding TLV may be used to advertise prefixes to
        SID/Label mappings. This functionality is called the Segment Routing
        Mapping Server (SRMS). The behavior of the SRMS is defined in [RFC8661](#RFC8661).

The SID/Label Binding TLV may also be used to advertise a Mirror
        SID indicating the ability of a node to process traffic originally destined to
        another IGP node. This behavior is defined in [RFC8402](#RFC8402).
...
- Type:
              ...
              1 octet
              RESERVED:
              1 octet (**SHOULD** be transmitted as 0 and **MUST** be
            ignored on receipt)
              Range:
              2 octets
              Prefix Length:
...
- F-Flag:
                Address-Family Flag. If unset, then the prefix
              ...
              is described in [RFC8402](#RFC8402).
                S-Flag:
                If set, the SID/Label Binding TLV **SHOULD** be flooded
              ...
              bit **MUST NOT** be altered during the TLV leaking.
                D-Flag:
                When the SID/Label Binding TLV is leaked from Level-2
              ...
              across levels.
                A-Flag:
                Attached Flag. The originator of the SID/Label
...
- An implementation may decide not to honor the S-Flag in order
...
The 'Prefix' represents the Forwarding Equivalence Class at the
          tail end of the advertised path. The 'Prefix' does not need to
          correspond to a routable prefix of the originating node.

The 'Prefix Length' field contains the length of the prefix in
...
The Prefix-SID sub-TLV is defined in [Section 2.1](#PREFIXSIDSUBTLV) and contains the SID/Index/Label value
          ...
          Prefix-SID sub-TLV **MUST NOT** be present when the M-Flag is set.

#### Prefix-SID Flags

The Prefix-SID Flags are defined in [Section 2.1](#PREFIXSIDSUBTLV). The Mapping Server **MAY** advertise a
...
The other flags defined in [Section 2.1](#PREFIXSIDSUBTLV) are
...
As the Mapping Server does not specify the originator of a
            ...
            done. The required information consists of:

- A prefix reachability advertisement for the prefix has been
                received, which includes the Prefix Attribute Flags sub-TLV
                [RFC7794](#RFC7794).
- X-Flag and R-Flag are both set to 0 in the Prefix Attribute
                Flags sub-TLV.

In the absence of a Prefix Attribute Flags sub-TLV [RFC7794](#RFC7794), the A-Flag in the Binding TLV indicates that
            the originator of a prefix reachability advertisement is directly
            connected to the prefix; thus, PHP **MUST** be done by the neighbors
...
The Algorithm field contains the identifier of the algorithm
            ...
            the Algorithm field is described in [Section 2.1](#PREFIXSIDSUBTLV).

<a name="BSIDSUBTLV"></a>
### SID/Label Sub-TLV

The SID/Label sub-TLV (Type: 1) contains the SID/Label value as
          defined in [Section 2.3](#SIDLABELSUBTLV). It **MUST** be present in
          the SID/Label Binding TLV when the M-Flag is set in the Flags field
          of the parent TLV.

<a name="BSIDEXAMPLE"></a>
### Example Encodings
...
Example 2: If the following IPv4 prefixes need to be mapped into
          the corresponding Prefix-SID indexes, then:
...
Example 3: If the following IPv6 prefixes need to be mapped into
          the corresponding Prefix-SID indexes, then:
...
It is not expected that a network operator will be able to keep
...
a router would need to advertise three instances of the
          Binding TLV.

<a name="MTBINDINGTLV"></a>
## Multi-Topology SID/Label Binding TLV

The Multi-Topology SID/Label Binding TLV allows the support of
        Multi-Topology IS-IS (M-ISIS) as defined in [RFC5120](#RFC5120). The Multi-Topology
        SID/Label Binding TLV has the same format as the SID/Label Binding TLV
        defined in [Section 2.4](#BINDINGTLV) with the difference consisting
        of a Multi-topology Identifier (MT ID) as defined here below:
...
- Type:
  ...
  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
            RESVD:
                  Reserved bits. **MUST** be reset on transmission and
                ignored on receive.
                  MT ID:
...
- The other fields and sub-TLVs are defined in [Section 2.4](#BINDINGTLV).

# Router Capabilities

This section defines sub-TLVs that are inserted into the IS-IS
      Router Capability that is defined in [RFC7981](#RFC7981).

<a name="SRCAPSUBTLV"></a>
...
The Router Capability TLV specifies flags that control its
        ...
        accordingly, i.e., the S-Flag in the Router Capability TLV [RFC7981](#RFC7981) **MUST** be unset.
...
- Type:
              ...
              Variable
              Flags:
              1 octet of flags. The following are defined:
                0 1 2 3 4 5 6 7 
   +-+-+-+-+-+-+-+-+
   |I|V|           | 
   +-+-+-+-+-+-+-+-+
...
- where:
            I-Flag:
                  MPLS IPv4 Flag. If set, then the router is capable
                of processing SR-MPLS-encapsulated IPv4 packets on all
                interfaces.
                  V-Flag:
                  MPLS IPv6 Flag. If set, then the router is capable
...
- One or more Segment Routing Global Block (SRGB) Descriptor entries, each of which have the
            following format:
            Range:
                  3 octets
                  SID/Label sub-TLV:
                  MPLS label as defined in [Section 2.3](#SIDLABELSUBTLV)
  - Range:
                  3 octets
                  SID/Label sub-TLV:
                  MPLS label as defined in [Section 2.3](#SIDLABELSUBTLV)
...
The SR-Capabilities sub-TLV **MAY** be advertised in an LSP of any
        ...
        same originator **SHOULD** select the first advertisement in the lowest-numbered
        LSP.
...
When multiple SRGB Descriptors are advertised, the entries define an
        ...
        have a disruptive effect on forwarding.

When a router adds a new SRGB Descriptor to an existing
...
When a router receives multiple overlapping ranges, it **MUST** conform
        to the procedures defined in [RFC8660](#RFC8660).
...
The router may use various algorithms when calculating reachability
        ...
        defined in [RFC8665](#RFC8665).
        The following values have been defined:

- 0:
              SPF algorithm based on link metric.
            ...
            the SPF path with a different path based on local policy.
              1:
              Strict SPF algorithm based on link
...
The SR-Algorithm sub-TLV is optional. It **MUST NOT** be advertised
...
When the originating router does not advertise the SR-Algorithm
        sub-TLV, it implies that algorithm 0 is the only algorithm supported by the routers
        that support the extensions defined in this document.

When the originating router does advertise the SR-Algorithm
...
- Type:
              ...
              Variable
              Flags:
              1 octet of flags. None are defined at this stage.
...
Within the context of IS-IS, the reporting of Local SIDs is done
        ...
        and protocols that are outside the scope of this document.
...
<a name="SRMSPREFSUBTLV"></a>
## SRMS Preference Sub-TLV

The SRMS Preference sub-TLV is
...
This document makes the following registrations in the "Sub-TLVs
...
This document makes the following registrations in the "Sub-TLVs
        for TLV 135, 235, 236, and 237" registry.
...
With the use of the extensions defined in this document, IS-IS
      ...
      [RFC3031](#RFC3031). In general, the same type of attacks that can be carried out
...
Existing security extensions as described in [RFC5304](#RFC5304)
         and [RFC5310](#RFC5310) apply to these Segment Routing extensions.
...
<a name="RFC8660"></a> **[RFC8660]** A Bashandy, C Filsfils, S Previdi, B Decraene, S Litkowski, R Shakir, "Segment Routing with the MPLS Data Plane", December 2019. <https://www.rfc-editor.org/info/rfc8660> [Local MD](rfc8660.md)
...
<a name="RFC5311"></a> **[RFC5311]** D. McPherson, L. Ginsberg, S. Previdi, M. Shand, "Simplified Extension of Link State PDU (LSP) Space for IS-IS", RFC 5311, DOI 10.17487/RFC5311, February 2009. <https://www.rfc-editor.org/info/rfc5311> [Local MD](rfc5311.md)
