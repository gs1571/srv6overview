...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-isis-l2bundles-07
...
- [prev](https://datatracker.ietf.org/doc/draft-ietf-isis-l2bundles-07)
- [alternate](https://dx.doi.org/10.17487/rfc8668)
...
Information about the current status of this document, any
            ...
            [https://www.rfc-editor.org/info/rfc8668](https://www.rfc-editor.org/info/rfc8668).
...
- [3. L2 Bundle Member Attributes TLV](#BUNDLE)
  - [3.1. Parallel L3 Adjacencies](#L3-adj)
  - [3.2. Shared Attribute Sub-TLVs](#section-3.2)
- [4. Advertising L2 Bundle Member Adj-SIDs](#section-4)
  - [4.1. L2 Bundle Member Adjacency Segment Identifier Sub-TLV](#section-4.1)
  - [4.2. L2 Bundle Member LAN Adjacency SID Sub-TLV](#section-4.2)
...
- [Appendix A. Example Encoding](#section-appendix.a)
...
There are deployments where the Layer 3 interface on which an IS-IS
      ...
      Link Aggregation Group (LAG) [IEEE802.1AX](#IEEE802.1AX). This reduces the number of
      ...
      to IS-IS such as Path Computation Elements (PCEs) [RFC4655](#RFC4655) may wish to
      ...
      document provide the means to advertise this information.
...
This document introduces a new TLV to advertise link attribute
      ...
      interface on which IS-IS operates.

[RFC8667](#RFC8667) introduces a new link attribute, adjacency segment
...
A new TLV is introduced to advertise L2 Bundle Member attributes.
      ...
      [ISO10589](#ISO10589) Decision Process.
...
described in [Section 3.1](#L3-adj) immediately follows the flags field.  If the P-Flag
is set to 0, then none of the sub-TLVs described in [Section 3.1](#L3-adj) are
present.
              Other bits:
...
- NOTE: An L2 Bundle Member Descriptor is a Link Local
              Identifier as defined in [RFC4202](#RFC4202).
  - NOTE: An L2 Bundle Member Descriptor is a Link Local
              Identifier as defined in [RFC4202](#RFC4202).
    - NOTE: An L2 Bundle Member Descriptor is a Link Local
              Identifier as defined in [RFC4202](#RFC4202).
...
When there exist multiple L3 adjacencies to the same neighbor,
        ...
        uniquely identify the L3 adjacency:

- IPv4 Interface Address (sub-TLV 6 defined in [RFC5305](#RFC5305))
- IPv6 Interface Address (sub-TLV 12 defined in [RFC6119](#RFC6119))
...
The set of L2 Bundle Member Descriptors that may be advertised
        ...
        in the shared attribute sub-TLVs.

All existing sub-TLVs defined in the IANA registry for Sub-TLVs for TLVs 22, 23,
...
[RFC8667](#RFC8667) defines sub-TLVs to advertise Adj-SIDs for L3 adjacencies.
      ...
      set of Adj-SIDs (one per L2 Bundle Member) in a single sub-TLV.
...
+-+-+-+-+-+-+-+-+
              where:
                F-Flag:
                  Address-Family Flag.  If unset, then the Adj-SID refers to
an L2 Bundle Member with outgoing IPv4 encapsulation.  If set, then the
Adj-SID refers to an L2 Bundle Member with outgoing IPv6 encapsulation.
                  V-Flag:
...
Adj-SID has local significance.  By default, the flag is SET.
                  S-Flag:
                  Set Flag.  When set, the S-Flag indicates that the Adj-SID
refers to a set of L2 Bundle Members (and therefore **MAY** be assigned to other
L2 Bundle Members as well).
                  P-Flag:
...
the L3 ADJ-SID defined in [RFC8667](#RFC8667).  * indicates a flag used in the L3
Adj-SID sub-TLV, but one that is NOT used in this sub-TLV.  These bits **SHOULD**
be sent as 0 and **MUST** be ignored on receipt.
  - 0 1 2 3 4 5 6 7
+-+-+-+-+-+-+-+-+ 
|F|*|V|L|S|P|   | 
+-+-+-+-+-+-+-+-+
...
be sent as 0 and **MUST** be ignored on receipt.
- Weight:
              1 octet.  The value represents the weight of the Adj-SID for the purpose
of load balancing.  The use of the weight is defined in [RFC8402](#RFC8402).
...
- L2 Bundle Member Adj-SID Descriptors:
              There **MUST** be one descriptor for each of the L2 Bundle Members advertised
...
- SID/Index/Label:
              ...
              A 3-octet local label where the 20 rightmost bits are used for encoding
the label value.  In this case, the V- and L-Flags **MUST** be set.
                  A 4-octet index defining the offset in the SID/Label space advertised by
this router. See [RFC8667](#RFC8667). In this case, V- and L-Flags **MUST** be unset.
  - SID/Index/Label:
                  According to the V- and L-Flags, it contains either:
  - A 3-octet local label where the 20 rightmost bits are used for encoding
the label value.  In this case, the V- and L-Flags **MUST** be set.
...
## L2 Bundle Member LAN Adjacency SID Sub-TLV
...
Adj-SID refers to an L2 Bundle Member with outgoing IPv6 encapsulation.
                  V-Flag:
                  Value Flag.  If set, then the Adj-SID carries a value.  By default, the
flag is SET.
                  L-Flag:
                  Local Flag.  If set, then the value/index carried by the Adj-SID has local
significance.  By default, the flag is SET.
                  S-Flag:
                  Set Flag.  When set, the S-Flag indicates that the Adj-SID refers to a set
of L2 Bundle Members (and therefore **MAY** be assigned to other L2 Bundle Members
as well).
                  P-Flag:
                  Persistent Flag.  When set, the P-Flag indicates that the Adj-SID is
persistently allocated, i.e., the Adj-SID value remains consistent across
router restart and/or interface flap.
                  Other bits:
                  **MUST** be zero when originated and ignored when
received.
              NOTE: The flags are deliberately kept congruent to the flags in the L3
LAN Adjacency SID defined in [RFC8667](#RFC8667).  * indicates a flag used in the L3 Adj-SID
sub-TLV, but one that is NOT used in this sub-TLV.  These bits **SHOULD** be sent
...
the label value.  In this case, the V- and L-Flags **MUST** be set.
                  A 4-octet index defining the offset in the SID/Label space advertised by
this router. See [RFC8667](#RFC8667). In this case, V- and L-Flags
**MUST** be unset.
  - SID/Index/Label:
                  According to the V- and L-Flags, it contains either:
  - A 3-octet local label where the 20 rightmost bits are used for encoding
...
The name of the IANA registry for Sub-TLVs for TLVs 22, 23, 141, 222, and 223
...
The IS-IS protocol has supported the advertisement of link attribute
      ...
      advertisements defined in [RFC4202](#RFC4202), [RFC5305](#RFC5305), [RFC8570](#RFC8570), and [RFC8667](#RFC8667), but are associated with L2 links that are part of a bundle interface
      on which the IS-IS protocol operates. There are therefore no new
      security issues introduced by the extensions in this document.
...
As always, if the protocol is used in an environment where
      ...
      authentication as defined in [RFC5304](#RFC5304) and [RFC5310](#RFC5310) is recommended to prevent such attacks.
...
<a name="IEEE802.1AX"></a> **[IEEE802.1AX]** "IEEE Standard for Local and metropolitan area networks -- Link Aggregation", IEEE 802.1AX, <https://ieeexplore.ieee.org/document/7055197>
...
<a name="RFC8667"></a> **[RFC8667]** S Previdi, L Ginsburg, C Filsfils, A Bashandy, H Gredler, B Decraene, "IS-IS Extensions for Segment Routing", RFC 8667, DOI 10.17487/RFC8667, December 2019. <https://www.rfc-editor.org/info/rfc8667> [Local MD](rfc8667.md)
