...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-pce-association-diversity-14
...
**Keywords:** Disjoint, Disjointness, Association

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-pce-association-diversity-14)
- [alternate](https://dx.doi.org/10.17487/rfc8800)
...
Information about the current status of this document, any
            ...
            [https://www.rfc-editor.org/info/rfc8800](https://www.rfc-editor.org/info/rfc8800).
...
- [3. Motivation](#mot)
- [4. Applicability](#app)
- [5. Protocol Extension](#section-5)
  - [5.1. Association Group](#encoding)
  - [5.2. Disjoint TLVs](#tlvs)
  - [5.3. Disjointness Objective Functions](#Disjointness-objective-functions)
  - [5.4. Relationship to SVEC](#svec)
    - [5.4.1. SVEC and OF](#svec-of)
  - [5.5. P Flag Considerations](#P-Flag-Consideration)
  - [5.6. Disjointness Computation Issues](#dis)
- [6. Security Considerations](#section-6)
- [7. IANA Considerations](#section-7)
  - [7.1. Association Type](#section-7.1)
  - [7.2. PCEP TLVs](#section-7.2)
  - [7.3. Objective Functions](#section-7.3)
  - [7.4. NO-PATH-VECTOR Bit Flags](#section-7.4)
  - [7.5. PCEP-ERROR Codes](#section-7.5)
- [8. Manageability Considerations](#section-8)
  ...
  - [8.4. Verification of Correct Operations](#section-8.4)
...
[RFC5440](#RFC5440) describes the Path Computation
   Element Communication 
   Protocol (PCEP), which enables the communication between a Path
   Computation Client (PCC) and a Path Control Element (PCE) or between
   two PCEs based on the PCE architecture [RFC4655](#RFC4655).

The PCEP Extensions for Stateful PCE Model [RFC8231](#RFC8231) 
...
[RFC8697](#RFC8697) introduces a generic
   ...
   to the active and passive modes of a stateful PCE [RFC8231](#RFC8231) or a stateless PCE [RFC4655](#RFC4655).
...
This document specifies a PCEP extension to signal that a set of LSPs
      ...
      the requested type of diversity.  Sections [3](#mot) and [4](#app) describe
...
**Figure 1: Disjoint Paths with Different Head Ends and
	Tail Ends**
...
[Section 5.4](#svec) describes the relationship
      between the Disjoint Association Group (DAG) and Synchronization VECtor
      (SVEC) object.

The PCEP extension for stateful PCE [RFC8231](#RFC8231) 
defined new PCEP messages -- Path Computation Report (PCRpt), Path Computation
Update (PCUpd), and Path Computation Initiate (PCInitiate) [RFC8281](#RFC8281). These messages use a PLSP-ID in the LSP
...
the active and passive modes of a stateful PCE is described in [RFC8697](#RFC8697).

Using the extension to PCEP defined in this document, the PCC uses the
extension defined in [RFC8697](#RFC8697) to indicate
...
Association ID, can overcome this limitation, as described in [RFC8697](#RFC8697). 
...
**Figure 2: Sample Use Cases for Carrying Disjoint Association Group over
	PCEP Session**
...
As per [RFC8697](#RFC8697), LSPs
     are associated with other LSPs with which they interact by adding
     them to a common association group. As described in [RFC8697](#RFC8697), the association group is uniquely
...
[RFC8697](#RFC8697) specifies the mechanism
...
This Association type is considered to be both dynamic and
	operator-configured in nature. As per [RFC8697](#RFC8697), the association group could be manually created by the
   ...
   Source. (Refer to [RFC8697](#RFC8697).)
...
There are different types of disjointness identified by the flags
	(T, S, N, and L) in the DISJOINTNESS-CONFIGURATION TLV (see [Section 5.2](#tlvs)). All LSPs in a particular Disjoint
...
- VENDOR-INFORMATION-TLV: Used to communicate arbitrary
	  vendor-specific behavioral information, described in [RFC7470](#RFC7470).
- OF-List TLV: Used to communicate the disjointness objective
	  function. See [Section 5.3](#Disjointness-objective-functions).
...
**Flags:**
  ...
  **P (Shortest Path) bit:**
    When set, this indicates that the
    	      ...
    	      examples in [Section 5.5](#P-Flag-Consideration).
  ...
  **T (Strict Disjointness) bit:**
    When set, if disjoint
                  ...
                  objective function (cf. [Section 5.3](#Disjointness-objective-functions))
                  or using the local policy if no objective function is requested
                  (e.g., using a lower disjoint type (link instead of node) or
                  fully relaxing disjointness constraint).  See [Section 5.6](#dis) for further details.
...
<a name="Disjointness-objective-functions"></a>
...
An objective function (OF) **MAY** be applied to the disjointness
      ...
      OF-List TLV (defined in [RFC5541](#RFC5541)) is
...
[RFC5440](#RFC5440) defines a mechanism for
   ...
   'Request-ID-number' in the RP (Request Parameters) object.  [RFC6007](#RFC6007) further clarifies the use of the SVEC
...
The SVEC object includes a Flags field that indicates the potential
	...
	Path Computation Reply
        ...
        per [RFC5440](#RFC5440). It should be noted that
...
This document defines three new OF-codes in [Section 5.3](#Disjointness-objective-functions) to
...
- SVEC object with domain-diverse bit=1 [RFC8685](#RFC8685); node-diverse bit=1, and
...
<a name="P-Flag-Consideration"></a>
## P Flag Considerations

As mentioned in [Section 5.2](#tlvs), the P flag
...
```
Note: In [Figure 4](#fig4), the cost of all the links is 1, unless explicitly marked otherwise.
...
```
Note: In [Figure 5](#fig5), the cost of all the links is 1, unless
...
In case of a network event leading to an impossible strict
	...
	the PCE **MAY** add the NO-PATH-VECTOR TLV [RFC5440](#RFC5440) in the LSP object.
...
When the T flag is unset, the PCE is allowed to relax disjointness
        by applying a requested objective function ([Section 5.3](#Disjointness-objective-functions)) if
...
There are some cases where the PCE may need to completely relax the
	...
	[PCE-OPTIONAL](#I-D.dhody-pce-stateful-pce-optional)
	for a proposed mechanism.

# Security Considerations

This document defines one new PCEP Association type, which by itself
      does not add any new security concerns beyond those discussed in [RFC5440](#RFC5440), 
      [RFC8231](#RFC8231), [RFC7470](#RFC7470), and [RFC8697](#RFC8697). But
...
Also, as stated in [RFC8697](#RFC8697), much of
...
Thus, securing the PCEP session using Transport Layer Security (TLS)
      [RFC8253](#RFC8253), as per the recommendations
...
This document defines a new Association type, originally described
        in [RFC8697](#RFC8697). IANA has assigned the
        following new value in the "ASSOCIATION Type Field" subregistry [RFC8697](#RFC8697) within the "Path Computation
...
IANA has created a new subregistry, named
	...
	to be assigned by Standards Action [RFC8126](#RFC8126). Each bit should be tracked with the following
	qualities:
...
**Table: DISJOINTNESS-CONFIGURATION TLV Flag Field**
...
**Table: NO-PATH-VECTOR TLV Flag Field**
...
An implementation **SHOULD** allow the operator to view the disjoint
	...
	YANG module [PCEP-YANG](#I-D.ietf-pce-pcep-yang)
...
Apart from the operation
        ...
        [RFC5440](#RFC5440), a PCEP implementation
...
Mechanisms defined in [RFC5440](#RFC5440) also apply to PCEP extensions defined in this
...
<a name="RFC7470"></a> **[RFC7470]** F. Zhang, A. Farrel, "Conveying Vendor-Specific Constraints in the Path Computation Element Communication Protocol", RFC 7470, DOI 10.17487/RFC7470, March 2015. <https://www.rfc-editor.org/info/rfc7470> [Local MD](rfc7470.md)
...
<a name="I-D.dhody-pce-stateful-pce-optional"></a> **[PCE-OPTIONAL]** C Li, H Zheng, S Litkowski, "Extension for Stateful PCE to allow Optional Processing of PCEP Objects", Internet-Draft draft-dhody-pce-stateful-pce-optional-06, July 2020. Work in Progress <https://tools.ietf.org/html/draft-dhody-pce-stateful-pce-optional-06>

<a name="I-D.ietf-pce-pcep-yang"></a> **[PCEP-YANG]** D Dhody, J Hardwick, V Beeram, J Tantsura, "A YANG Data Model for Path Computation Element Communications Protocol (PCEP)", Internet-Draft draft-ietf-pce-pcep-yang-14, July 2020. Work in Progress <https://tools.ietf.org/html/draft-ietf-pce-pcep-yang-14>
...
A special thanks to the authors of
      [RFC8697](#RFC8697); this document borrows
      some text from it. The authors would also like to thank Adrian Farrel 
      and Julien Meuric for the valuable comments.

Thanks to Emmanuel Baccelli for the RTGDIR review.

Thanks to Dale Worley for a detailed GENART review.

Thanks to Alvaro Retana, Benjamin Kaduk, Suresh Krishnan,
      Roman Danyliw, Alissa       Cooper, and Éric Vyncke for the IESG review.
...
- Address: Divyashree Techno Park, Whitefiled, Bangalore, Karnataka, 560066, India
