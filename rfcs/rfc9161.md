...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-bess-evpn-proxy-arp-nd-16
...
**Keywords:** ARP suppression, flood suppression, ARP unicast-forward, Duplicate IP Detection

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-bess-evpn-proxy-arp-nd-16)
- [alternate](https://dx.doi.org/10.17487/rfc9161)
...
Information about the current status of this document, any
            ...
            [https://www.rfc-editor.org/info/rfc9161](https://www.rfc-editor.org/info/rfc9161).
...
- [1. Introduction](#sect-2)
  - [1.1. The Data Center Use Case](#sect-2.1)
  - [1.2. The Internet Exchange Point Use Case](#sect-2.2)
- [2. Terminology](#sect-1)
- [3. Solution Description](#sect-4)
  - [3.1. Proxy ARP/ND Sub-functions](#section-3.1)
  - [3.2. Learning Sub-function](#sect-4.1)
    - [3.2.1. Proxy ND and the NA Flags](#sect-4.1.1)
  - [3.3. Reply Sub-function](#sect-4.2)
  - [3.4. Unicast-Forward Sub-function](#sect-4.3)
  - [3.5. Maintenance Sub-function](#sect-4.4)
  - [3.6. Flood (to Remote PEs) Handling](#sect-4.5)
  - [3.7. Duplicate IP Detection](#sect-4.6)
- [4. Solution Benefits](#sect-5)
- [5. Deployment Scenarios](#sect-6)
  - [5.1. All Dynamic Learning](#sect-6.1)
  - [5.2. Dynamic Learning with Proxy ARP/ND](#sect-6.2)
  - [5.3. Hybrid Dynamic Learning and Static Provisioning with Proxy ARP/ND](#sect-6.3)
  - [5.4. All Static Provisioning with Proxy ARP/ND](#sect-6.4)
  - [5.5. Example of Deployment in Internet Exchange Points](#sect-6.5)
  - [5.6. Example of Deployment in Data Centers](#sect-6.6)
- [6. Security Considerations](#sect-7)
- [7. IANA Considerations](#sect-8)
- [8. References](#section-8)
  - [8.1. Normative References](#section-8.1)
  - [8.2. Informative References](#section-8.2)
- [Acknowledgments](#sect-10)
...
As specified in [RFC7432](#RFC7432), the IP
...
EVPN and its associated Proxy ARP/ND function are extremely useful in
      ...
      routers and Customer Edges (CEs). [RFC6820](#RFC6820) describes the address
      resolution problems in large DC networks.

This document describes the Proxy ARP/ND function in [RFC7432](#RFC7432) networks, augmented by the
      capability of the ARP/ND Extended Community [RFC9047](#RFC9047). From that perspective, this document updates [RFC7432](#RFC7432).
...
As described in [RFC6820](#RFC6820), the IPv4
        and IPv6 address resolution can create a lot of issues in large
        DCs. In particular, the issues created by IPv4 Address
        Resolution Protocol procedures may be significant.

On one hand, ARP Requests use broadcast MAC addresses; therefore,
        any Tenant System in a large Broadcast Domain will see a large amount
        of ARP traffic, which is not addressed to most of the receivers.
...
On the other hand, the flooding issue becomes even worse if some
        ...
        implementations will persistently retry sending ARP Requests. As [RFC6820](#RFC6820) states, there are no clear
...
The implementation described in this document is especially useful
        in IXP networks.

A typical IXP provides access to a large Layer 2 Broadcast Domain
...
In these IXP networks, most of the CEs are typically peering
...
The issue might be better in IPv6 routers if Multicast Listener
        ...
        configured to broadcast periodic Gratuitous ARPs (GARPs) [RFC5227](#RFC5227). For IPv6, the fact that IPv6 CEs
        ...
        only grow worse as new CEs are added.
...
**DAD:**
  Duplicate Address Detection, as per [RFC4861](#RFC4861)
...
**EVPN:**
  Ethernet Virtual Private Network, as per [RFC7432](#RFC7432)
...
**NUD:**
  Neighbor Unreachability Detection, as per [RFC4861](#RFC4861)

**O Flag:**
  Override Flag in NA messages, as per [RFC4861](#RFC4861)

**PE:**
  Provider Edge router

**R Flag:**
  Router Flag in NA messages, as per [RFC4861](#RFC4861)
...
**S Flag:**
  Solicited Flag in NA messages, as per [RFC4861](#RFC4861)

**SN-multicast address:**
  Solicited-Node IPv6 multicast address used by NS messages

**TLLA:**
  Target Link Layer Address, as per [RFC4861](#RFC4861)

**VPLS:**
  Virtual Private LAN Service


This document assumes familiarity with the terminology used in [RFC7432](#RFC7432).

<a name="sect-4"></a>
# Solution Description

[Figure 1](#ure-proxy-arp-nd-network-example) illustrates an
      example EVPN network where the Proxy ARP/ND function is enabled.
...
**Dynamic entries:**
  Learned by snooping a CE's ARP and ND messages; for instance,
  	see IP4->M4 in [Figure 1](#ure-proxy-arp-nd-network-example).

**Static entries:**
  Provisioned on the PE by the management system; for instance,
  	see IP3->M3 in [Figure 1](#ure-proxy-arp-nd-network-example).
...
**EVPN-learned entries:**
  Learned from the IP/MAC information encoded in the received RT2's
  	...
  	[Figure 1](#ure-proxy-arp-nd-network-example).


As a high-level example, the operation of the EVPN Proxy ARP/ND function in
the network of [Figure 1](#ure-proxy-arp-nd-network-example) is described below. In this example, we assume IP1, IP2, and
...
2. The PEs start adding dynamic, static, and EVPN-learned entries to
  ...
  1. PE3 adds IP1->M1 and IP2->M2 based on the EVPN routes
              received from PE1 and PE2. Those entries were previously learned
              as dynamic entries in PE1 and PE2, respectively, and advertised
              in BGP EVPN.
  2. PE3 adds IP4->M4 as dynamic. This entry is learned by
              snooping the corresponding ARP messages sent by CE4.
...
3. When CE3 sends an ARP Request asking for the MAC address of IP1,
          ...
          Intercept the ARP Request and perform a Proxy ARP lookup for
              IP1.
            If the lookup is successful (as in [Figure 1](#ure-proxy-arp-nd-network-example)), PE3 will send an
  ...
  1. Intercept the ARP Request and perform a Proxy ARP lookup for
              IP1.
  2. If the lookup is successful (as in [Figure 1](#ure-proxy-arp-nd-network-example)), PE3 will send an
              ARP Reply with IP1->M1. The ARP Request will not be flooded
              to the EVPN network or any other local CEs.
  3. If the lookup is not successful, PE3 will flood the ARP
              Request in the EVPN network and the other local CEs.
...
1. PEs will start adding entries in a similar way as they would for IPv4;
  ...
  1. IP1->M1 and IP2->M2 are
          ...
          learning the dynamic entries, the R and O Flags contained in the
          snooped NA messages will be added to the Proxy ND entries too.
  2. PE1 and PE2 will advertise those entries in EVPN MAC/IP
              Advertisement routes, including the corresponding learned R and
              O Flags in the ARP/ND Extended Community.
  3. PE3 also adds IP4->M4 as dynamic after snooping an NA
              message sent by CE4.
...
As PE3 learns more and more host entries in the Proxy ARP/ND table,
      ...
      static, the ARP/ND flooding among PEs is in fact totally suppressed.
...
In a network where CEs move between PEs, the Proxy ARP/ND function
      ...
      attached to the destination PE, the mechanisms described in [Section 3.5](#sect-4.4) make sure that the Proxy ARP/ND tables are
      eventually updated.
...
A Proxy ARP/ND implementation in an EVPN BD **MUST**
...
Static entries are provisioned from the management plane. A static
        ...
        **MUST** be advertised in EVPN with an ARP/ND Extended
        ...
        per [RFC9047](#RFC9047). When the I Flag in the
        ...
        Sticky bit (S) or sequence number in the [RFC7432](#RFC7432) MAC Mobility Extended Community. Note that the
        ...
        subject to the [RFC7432](#RFC7432) mobility
        procedures.
...
The PE **MUST** create EVPN-learned entries from the received valid
        EVPN MAC/IP Advertisement routes containing a MAC and IP address.
...
**Proxy ND dynamic entries:**
  The PE **MUST** snoop the NA messages (Ethertype
  	    ...
  	    **MUST NOT** create dynamic IP->MAC entries from NS
  	    ...
  	    Proxy ND reply function.  See [Section 3.2.1](#sect-4.1.1) for more information about the R Flag.
...
In case of a PE reboot, the static and EVPN entries will be
        ...
        remote PEs and dynamic entries gradually relearned again.

<a name="sect-4.1.1"></a>
### Proxy ND and the NA Flags

[RFC4861](#RFC4861) describes the use of the R Flag in IPv6
          address resolution:
...
- In those cases where the IsRouter flag changes from TRUE to
            ...
            router, as specified in [RFC4861](#RFC4861). This is needed to detect when
...
- EVPN-learned entries **SHOULD** learn the R Flag
            ...
            Community [RFC9047](#RFC9047) received from
            ...
            follow [RFC7432](#RFC7432) but do not
...
Note that, typically, IP->MAC entries with O = 0 will not be
          ...
          ARP/ND Extended Community will be generated.
...
1. When replying to ARP Requests or NS messages:
  ...
  - For an NA message in response to an address resolution NS or
              ...
              Target Address. M1 **MUST** be used as the Target
              Link Local Address (TLLA).
2. A PE **SHOULD NOT** reply to a request/solicitation received on the
...
3. A PE **SHOULD** reply to broadcast/multicast address resolution
            ...
            Address Conflict Detection as specified in [RFC5227](#RFC5227). A PE **SHOULD NOT** reply to unicast address
            resolution requests (for instance, NUD NS messages).
4. When replying to an NS, a PE **SHOULD** set the Flags in the NA
            messages as follows:
            The R bit is set as it was learned for the IP->MAC entry
                in the NA messages that created the entry (see [Section 3.2.1](#sect-4.1.1)).
              The S Flag will be set/unset as per [RFC4861](#RFC4861).
  ...
  - The O Flag will be set in all the NA messages issued by the
              ...
              messages with as many NA responses as "anycast" entries there
              are in the Proxy ND table.
5. For Proxy ARP, a PE **MUST** only reply to ARP Requests with the
            format specified in [RFC0826](#RFC0826).
6. For Proxy ND, a PE **MUST** reply to NS messages
            with known options with the format and options specified in [RFC4861](#RFC4861) and **MAY** reply,
            ...
            The "reply" option implies that the PE ignores the unknown
              ...
              Proxy ND table.
              The "unicast-forward" option is described in [Section 3.4](#sect-4.3).
...
As discussed in [Section 3.3](#sect-4.2), in some cases, the
...
If "unicast-forward unknown-options" is enabled, upon a successful
        ...
        explained in [Section 3.3](#sect-4.2). The "unicast-forward
        unknown-options" configuration allows the support of new applications
        using ARP/ND in the BD while still reducing the flooding.

Irrespective of the enabled option, if there is no successful
        Proxy ARP/ND lookup, the unknown ARP Request / NS message will be flooded in the
        context of the BD, as per [Section 3.6](#sect-4.5).
...
**Send-refresh option:**
  The PE **MAY** send periodic refresh messages
              ...
              **RECOMMENDED** to be a third of the age-time or a half
              of the age-time in scaled networks.
  ...
  The refresh request messages **SHOULD** be sent only
...
The Proxy ARP/ND function implicitly helps reduce the flooding of
...
The Proxy ARP/ND function **MUST** support duplicate IP
        ...
        CEs will continue to carry out the DAD procedures as per [RFC4862](#RFC4862). The solution described in this
        ...
        the result of an attack. [RFC6957](#RFC6957)
...
The distributed nature of EVPN and Proxy ARP/ND allows the easy
        ...
        MAC duplication detection function supported by [RFC7432](#RFC7432) for MAC addresses.

Duplicate IP detection monitors "IP-moves" in the Proxy ARP/ND
        table in the following way:

1. When an existing active IP1->MAC1
...
2. In order to detect the duplicate IP faster, the PE
...
3. Upon detecting a duplicate IP situation:
  ...
  1. The entry in duplicate detected
            state cannot be updated with new dynamic or EVPN-learned entries
            for the same IP. The operator **MAY** override the
            entry, though, with a static IP->MAC.
  2. The PE **SHOULD** alert the operator and stop responding to
                ARP/NS for the duplicate IP until a corrective action is
                taken.
  3. Optionally, the PE **MAY** associate an
              "anti-spoofing-mac" (AS-MAC) to the duplicate IP in the
              Proxy ARP/ND table. The PE will send a GARP/unsolicited-NA
              message with IP1->AS-MAC to the local CEs as well as an RT2
              (with IP1->AS-MAC) to the remote PEs. This will update the
              ...
              indication in the RT2) that can be used directly in the BD to
              drop frames is for further study.
4. The duplicate IP situation will be cleared when a corrective
...
1. May completely suppress
      ...
      the PE BDs.
2. Significantly reduces the flooding of the ARP/ND
...
3. Provides a way to refresh periodically the CE
...
4. Provides a mechanism to detect duplicate IP addresses and avoid
...
In this scenario for minimum security and mitigation, EVPN is
        ...
        all-active multihoming in the peering network.
...
Although this option does not require any of the procedures
        ...
        concerned. The options described in Sections [5.2](#sect-6.2), [5.3](#sect-6.3), and [5.4](#sect-6.4) are only possible in
        EVPN networks in combination with their Proxy ARP/ND capabilities.
...
This scenario makes use of the Learning, Reply, and Maintenance
        ...
        uses default flooding for unknown ARP Requests / NS messages.

<a name="sect-6.3"></a>
## Hybrid Dynamic Learning and Static Provisioning with Proxy ARP/ND

Some IXPs and other operators want to protect particular hosts on
        ...
        Proxy ARP/ND is enabled as described in [Section 5.2](#sect-6.2) on
        the BD.

This scenario makes use of the same sub-functions as in [Section 5.2](#sect-6.2) but with static entries added by
        the Learning sub-function.
...
Nowadays, almost all IXPs install some security rules in order to
        ...
        described in [Section 5.4](#sect-6.4), "All Static
...
The following describes the operational procedures that are needed
...
1. A new customer is connected the first time to the IXP:
            Before the connection between the customer router
...
DCs normally have different requirements than IXPs in terms of
        Proxy ARP/ND. Some differences are listed below:

1. The required mobility in virtualized
        DCs makes the "Dynamic Learning" or "Hybrid Dynamic and Static
        Provisioning" models more appropriate than the "All Static
        Provisioning" model.
2. IPv6 "anycast" may be required in DCs, while it is typically not
          ...
          the Proxy ND function, the duplicate IP detection procedure in [Section 3.7](#sect-4.6) must be disabled.
3. DCs may require special options on ARP/ND as opposed to the
...
The security considerations of [RFC7432](#RFC7432) and [RFC9047](#RFC9047) apply to this document too. Note that EVPN does not
...
The procedures in this document reduce the amount of ARP/ND message
      ...
      [Section 3.6](#sect-4.5) can provide an additional
...
While the unicast-forward and/or flood suppression sub-functions
      ...
      the ARP/ND resolution that the CE needs.
...
The solution also provides protection against Denial-of-Service (DoS)
      ...
      detection and the use of an AS-MAC as explained in [Section 3.7](#sect-4.6) protects the BD against ARP/ND
      spoofing.
...
This document provides some examples and guidelines that can be used
      ...
      described in [EURO-IX-BCP](#EURO-IX-BCP). For example,
      ...
      and ACLs can provide an additional level of security.
...
<a name="RFC0826"></a> **[RFC0826]** D. Plummer, "An Ethernet Address Resolution Protocol: Or Converting Network Protocol Addresses to 48.bit Ethernet Address for Transmission on Ethernet Hardware", STD 37, RFC 826, DOI 10.17487/RFC0826, November 1982. <https://www.rfc-editor.org/info/rfc826> [Local MD](rfc0826.md)
...
<a name="RFC9047"></a> **[RFC9047]** J. Rabadan, S. Sathappan, K. Nagaraj, W. Lin, "Propagation of ARP/ND Flags in an Ethernet Virtual Private Network (EVPN)", RFC 9047, DOI 10.17487/RFC9047, June 2021. <https://www.rfc-editor.org/info/rfc9047> [Local MD](rfc9047.md)

## Informative References

<a name="EURO-IX-BCP"></a> **[EURO-IX-BCP]** Euro-IX, "European Internet Exchange Association", <https://www.euro-ix.net/en/forixps/set-ixp/ixp-bcops>
...
<a name="RFC6820"></a> **[RFC6820]** T. Narten, M. Karir, I. Foo, "Address Resolution Problems in Large Data Center Networks", RFC 6820, DOI 10.17487/RFC6820, January 2013. <https://www.rfc-editor.org/info/rfc6820> [Local MD](rfc6820.md)

<a name="RFC6957"></a> **[RFC6957]** F. Costa, J-M. Combes, X. Pougnard, H. Li, "Duplicate Address Detection Proxy", RFC 6957, DOI 10.17487/RFC6957, June 2013. <https://www.rfc-editor.org/info/rfc6957> [Local MD](rfc6957.md)

<a name="sect-10"></a>
# Acknowledgments

The authors want to thank Ranganathan       Boovaraghavan, Sriram Venkateswaran, Manish Krishnan, Seshagiri Venugopal,
      Tony Przygienda, Robert       Raszuk, and Iftekhar Hussain for their review
      and contributions.  Thank you to Oliver Knapp as
      well for his detailed review.
...
- Organization: Nokia
- Address: 777 Middlefield Road, Mountain View, CA, 94043, United States of America
- Email: jorge.rabadan@nokia.com

**Senthil Sathappan**

- Organization: Nokia
- Address: 701 E. Middlefield Road, Mountain View, CA, 94043, United States of America
- Email: senthil.sathappan@nokia.com
