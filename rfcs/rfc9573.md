...
**Category:** std
**Updates:** 6514, 7432, 7582
**Submission Type:** IETF
**Consensus:** true
**IPR:** pre5378Trust200902
**Doc Name:** draft-ietf-bess-mvpn-evpn-aggregation-label-14
...
**Keywords:** DCB, Tunnel Aggregation

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-bess-mvpn-evpn-aggregation-label-14)
- [alternate](https://dx.doi.org/10.17487/rfc9573)
...
- [3. Proposed Solutions](#solution)
  - [3.1. MP2MP Tunnels](#section-3.1)
  - [3.2. Segmented Tunnels](#seg)
  - [3.3. Summary of Label Allocation Methods](#methods)
- [4. Specifications](#section-4)
  - [4.1. Context-Specific Label Space ID Extended Community](#section-4.1)
  - [4.2. Procedures](#section-4.2)
...
A Multicast VPN (MVPN) can use Point-to-Multipoint (P2MP) tunnels (set up by RSVP-TE, Multipoint LDP (mLDP), or PIM) to
     ...
     "upstream-assigned MPLS label" [RFC5331](#RFC5331) for each VPN, and each packet
...
An MVPN or EVPN can also use Bit Index Explicit Replication (BIER) [RFC8279](#RFC8279) to transmit VPN multicast
     traffic [RFC8556](#RFC8556) or EVPN BUM traffic
     [BIER-EVPN](#I-D.ietf-bier-evpn).
     ...
     used, the PE transmitting a packet (the "Bit-Forwarding Ingress Router" (BFIR) [RFC8279](#RFC8279)) must
     ...
     identifies their VPN or BD. (See [RFC8556](#RFC8556) and [BIER-EVPN](#I-D.ietf-bier-evpn) for
...
When an egress PE receives a packet from an aggregate tunnel, it must
     ...
     [RFC5331](#RFC5331) that matches the default label space from which
...
Familiarity with MVPN/EVPN protocols and procedures is assumed.
       Some terms are listed below for convenience.

**VPN:**
  Virtual Private Network. In this document, "VPN" specifically refers to an IP
  	  VPN [RFC4364](#RFC4364).
...
**PMSI [RFC6513](#RFC6513):**
  Provider Multicast Service Interface. A pseudo-overlay interface
  	 ...
  	 underlay/provider tunnels. It includes Inclusive/Selective PMSIs (I/S-PMSIs) (often referred
...
**Aggregate Tunnel:**
  A tunnel that instantiates x-PMSIs of multiple MVPNs or EVPN
  	  BDs.

**IMET [RFC7432](#RFC7432):**
  Inclusive Multicast Ethernet Tag.  An EVPN-specific name
  	  for an I-PMSI Auto-Discovery (A-D) route.

**PTA [RFC6514](#RFC6514):**
  PMSI Tunnel Attribute. A BGP attribute that may be attached to
...
**RBR:**
  Regional Border Router. A border router between segmentation
...
**ESI Label [RFC7432](#RFC7432):**
  A label that identifies an ES.

**SRGB [RFC8402](#RFC8402):**
  Segment Routing (SR) Global Block. The set of global segments in
  	  the SR domain.  In SR-MPLS [RFC8660](#RFC8660), an SRGB is a local property of a node and
...
**Context-Specific Label Space [RFC5331](#RFC5331):**
...
**Upstream Assigned [RFC5331](#RFC5331):**
...
- In an EVPN, a single customer ("tenant") may have a large number of
          BDs, and the use of aggregate RSVP-TE or mLDP P2MP tunnels may
          become important, since each tunnel creates state at the
          intermediate nodes.
- The use of BIER as the transport for an MVPN/EVPN is becoming more and
...
One method of achieving this is to reserve a portion of the default label space
     ...
     on all nodes, as described in [RFC8402](#RFC8402).
...
The MVPN/EVPN signaling defined in [RFC6514](#RFC6514) and [RFC7432](#RFC7432) assumes that
...
Multipoint-to-Multipoint (MP2MP) tunnels present the same problem ([Section 2](#problem))
      that can be solved the same way ([Section 3](#solution)), with
      the following additional requirement.

Per [RFC7582](#RFC7582) ("Multicast Virtual Private Network (MVPN): Using Bidirectional P-Tunnels"), when MP2MP
    tunnels are used for an MVPN, the root of the MP2MP tunnel is required to
    allocate and advertise "PE Distinguisher Labels" ([RFC6513](#RFC6513)).  These labels are assigned
...
There are some additional issues to be considered when an MVPN or
          EVPN is using "tunnel segmentation" (see [RFC6514](#RFC6514),
          [RFC7524](#RFC7524), and Sections [RFC9572](#RFC9572) and [RFC9572](#RFC9572) of [RFC9572](#RFC9572)).

<a name="select"></a>
### Selective Tunnels

For selective tunnels that instantiate S-PMSIs (see Sections
            [RFC6513](#RFC6513) and
            [RFC6513](#RFC6513) of
            [RFC6513](#RFC6513) and [RFC9572](#RFC9572)), the procedures outlined above
...
A selective tunnel carries one or more particular sets of flows to a
     ...
     of flows is identified by an S-PMSI A-D route [RFC6514](#RFC6514).  The PTA of the S-PMSI route identifies the tunnel used to
...
### Per-PE/Region Tunnels

Similarly, for segmented per-PE (MVPN (C-*,C-*) S-PMSI or EVPN
            IMET) or per-AS/region (MVPN Inter-AS I-PMSI or EVPN per-region
            I-PMSI) tunnels [RFC6514](#RFC6514) [RFC7432](#RFC7432)
            [RFC9572](#RFC9572), labels need to be
            ...
            allocated to itself (see [Section 3.2.1](#select)).
...
### Alternative to Per-PMSI Label Allocation
...
3. A central entity assigns disjoint label
...
Option [1](#dcb-assignment) is simplest,
       ...
       Option [3](#context-block) is needed only
...
**ID-Type:**
  A 2-octet field that specifies the type of Label Space
            ID.  In this document, the ID-Type is 0, indicating that the
            ID-Value field is a label.

**ID-Value:**
  A 4-octet field that specifies the value of the Label Space ID.
         When it is a label (with ID-Type 0), the most significant 20-bit portion
         is set to the label value.


This document introduces a DCB-flag (Bit 47 as assigned by IANA) in the
       "Additional PMSI Tunnel Attribute Flags" BGP Extended Community [RFC7902](#RFC7902).

In the remainder of this document, when we say that a BGP-MVPN/EVPN A-D route
       carries a DCB-flag or has a DCB-flag attached to it, we mean the following:

- The route carries a PTA and its Flags field has
...
The protocol and procedures specified in this section **MAY** be used
       ...
       points **MUST** support the procedures. How to ensure this behavior is outside the scope of this document.
...
Via methods outside the scope of this document, each VPN/BD/ES is assigned
       ...
       and the BD label **MUST** be assigned from the same label space. If PE
       Distinguisher Labels are used [RFC7582](#RFC7582), they **MUST** be allocated
...
When a PE originates/re-advertises an x-PMSI/IMET route, the route **MUST**
       carry a DCB-flag if and only if the label in its PTA is assigned
       from the DCB.

If the VPN/BD/ES/PMSI label is assigned from one of those few context-specific label
       spaces, a Context-Specific Label Space ID EC **MUST** be attached to the
...
When a PE receives an x-PMSI/IMET route with the Context-Specific Label
       Space ID EC, it **MUST** place an entry in its default MPLS forwarding table
...
An x-PMSI/IMET route **MUST NOT** carry both the
        ...
        **MUST** be treated as withdrawn.  If neither the DCB-flag
        nor the Context-Specific Label Space ID EC is attached, the label in
        the PTA or ESI Label EC **MUST** be treated as the
        upstream-assigned label from the label space of the source PE, and
        procedures provided in [RFC6514](#RFC6514) and [RFC7432](#RFC7432)
          **MUST** be followed.

If a PE originates two x-PMSI/IMET routes with the same tunnel,
    it **MUST** ensure that one of the following scenarios applies, so that the PE receiving the routes
...
This document allows three methods ([Section 3.3](#methods)) of
      label allocation for MVPN PEs [RFC6514](#RFC6514) or EVPN PEs [RFC7432](#RFC7432) and
      specifies corresponding signaling and procedures. The first method (Option [1](#dcb-assignment)) is
      the equivalent of using common SRGBs [RFC8402](#RFC8402) from the regular
      per-platform label space. The second method (Option [2](#context-space)) is the equivalent of using
      common SRGBs from a third-party label space [RFC5331](#RFC5331). The third
      method (Option [3](#context-block)) is a variation of the second in that the third-party label
...
[RFC6514](#RFC6514), [RFC7432](#RFC7432), [RFC8402](#RFC8402), and [RFC5331](#RFC5331)
...
IANA has created the "Context-Specific Label Space ID Type"
      registry within the "Border Gateway Protocol (BGP) Extended Communities"
      group of registries. The registration procedure is First Come First Served [RFC8126](#RFC8126).
...
<a name="RFC7582"></a> **[RFC7582]** E. Rosen, IJ. Wijnands, Y. Cai, A. Boers, "Multicast Virtual Private Network (MVPN): Using Bidirectional P-Tunnels", RFC 7582, DOI 10.17487/RFC7582, July 2015. <https://www.rfc-editor.org/info/rfc7582> [Local MD](rfc7582.md)

<a name="RFC7902"></a> **[RFC7902]** E. Rosen, T. Morin, "Registry and Extensions for P-Multicast Service Interface Tunnel Attribute Flags", RFC 7902, DOI 10.17487/RFC7902, June 2016. <https://www.rfc-editor.org/info/rfc7902> [Local MD](rfc7902.md)
...
<a name="I-D.ietf-bier-evpn"></a> **[BIER-EVPN]** Z. Zhang, A. Przygienda, A. Sajassi, J. Rabadan, "EVPN BUM Using BIER", Internet-Draft draft-ietf-bier-evpn-14, January 2024. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-bier-evpn-14>
...
<a name="RFC8556"></a> **[RFC8556]** E. Rosen, M. Sivakumar, T. Przygienda, S. Aldrin, A. Dolganow, "Multicast VPN Using Bit Index Explicit Replication (BIER)", RFC 8556, DOI 10.17487/RFC8556, April 2019. <https://www.rfc-editor.org/info/rfc8556> [Local MD](rfc8556.md)
...
<a name="RFC9572"></a> **[RFC9572]** Z Zhang, W Lin, J Rabadan, K Patel, A Sajassi, "Updates to EVPN Broadcast, Unknown Unicast, or Multicast (BUM) Procedures", RFC 9572, DOI 10.17487/RFC9572, May 2024. <https://www.rfc-editor.org/info/rfc9572> [Local MD](rfc9572.md)
