...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-idr-sr-policy-safi-13
...
- [prev](https://datatracker.ietf.org/doc/draft-ietf-idr-sr-policy-safi-13)
- [alternate](https://dx.doi.org/10.17487/rfc9830)
...
- [2. SR Policy Encoding](#ENCODING)
  - [2.1. SR Policy SAFI and NLRI](#SRPOLICYSAFI)
  - [2.2. SR Policy and Tunnel Encapsulation Attribute](#ENCAPSATTR)
  - [2.3. Applicability of Tunnel Encapsulation Attribute Sub-TLVs](#ENDCOLOR)
  - [2.4. SR Policy Sub-TLVs](#SRPOLICYTLV)
    - [2.4.1. Preference Sub-TLV](#PREFTLV)
    - [2.4.2. Binding SID Sub-TLV](#BINDINGSIDTLV)
    - [2.4.3. SRv6 Binding SID Sub-TLV](#SRV6BINDINGSIDTLV)
    - [2.4.4. Segment List Sub-TLV](#SLTLV)
    - [2.4.5. Explicit NULL Label Policy Sub-TLV](#ENLPTLV)
    - [2.4.6. SR Policy Priority Sub-TLV](#POLICYPRIORITY)
    - [2.4.7. SR Policy Candidate Path Name Sub-TLV](#POLICYCPNAME)
    - [2.4.8. SR Policy Name Sub-TLV](#POLICYNAME)
- [3. Color Extended Community](#EXTCOLOR)
- [4. SR Policy Operations](#OPERATIONS)
  - [4.1. Advertisement of SR Policies](#CONFIG)
  - [4.2. Reception of an SR Policy NLRI](#RECEPT)
    - [4.2.1. Validation of an SR Policy NLRI](#ACCEPT)
    - [4.2.2. Eligibility for Local Use of an SR Policy NLRI](#USABLE)
    - [4.2.3. Propagation of an SR Policy](#PROPAGATE)
- [5. Error Handling and Fault Management](#ERROR)
- [6. IANA Considerations](#IANA)
  - [6.1. Subsequent Address Family Identifiers (SAFI) Parameters](#IANASAFI)
  - [6.2. BGP Tunnel Encapsulation Attribute Tunnel Types](#IANATUNNEL)
  - [6.3. BGP Tunnel Encapsulation Attribute Sub-TLVs](#IANATUNNSUBTLV)
  - [6.4. Color Extended Community Flags](#IANAEXTCOM)
  - [6.5. SR Policy Segment List Sub-TLVs](#IANASIDLIST)
  - [6.6. SR Policy Binding SID Flags](#IANABSIDFLAGS)
  - [6.7. SR Policy SRv6 Binding SID Flags](#IANASRV6BSIDFLAGS)
  - [6.8. SR Policy Segment Flags](#IANASIDFLAGS)
  - [6.9. Color Extended Community Color-Only Types](#IANAEXTCOMCOFIELD)
  - [6.10. SR Policy ENLP Values](#IANAENLP)
...
[RFC9256](#RFC9256) further details the concepts of SR Policy
      ...
      in [RFC8402](#RFC8402). [RFC8660](#RFC8660) describes the
      representation and processing of this ordered list of segments as an
      MPLS label stack for SR-MPLS. [RFC8754](#RFC8754) and [RFC8986](#RFC8986) describe the same for SRv6 with the use of the
      Segment Routing Header (SRH).

The functionality related to SR Policy described in [RFC9256](#RFC9256) can be conceptually viewed as being incorporated in
      an SR Policy Module (SRPM). The following is a reminder of the high-level
      functionality of SRPM:

- Learning multiple CPs for an SR Policy via
          various mechanisms (CLI, NETCONF, PCEP, or BGP).
- Selection of the best CP for an SR Policy.
- Associating a Binding SID (BSID) to the selected CP
          of an SR Policy.
- Installation of the selected CP and its BSID in the
          forwarding plane.
...
This document specifies the use of BGP to distribute one or more of
      ...
      the scope of BGP (i.e., resides within SRPM on the headend node).

This document specifies a way of representing SR Policy CPs in BGP UPDATE messages. BGP can then be used to propagate the SR
...
This document introduces a BGP Subsequent Address Family Identifier (SAFI) for
...
While, for simplicity, the text in this document states that BGP
      ...
      different distinguisher as defined in [Section 2.1](#SRPOLICYSAFI)),
      PCEP, NETCONF, or local policy configuration.

Typically, an SR Policy Controller [RFC9256](#RFC9256) defines
      ...
      Route Reflector (RR) (see [RFC4456](#RFC4456) and [Section 4.2.3](#PROPAGATE)).
...
In some situations, it is undesirable for a controller or BGP egress
      ...
      within an SR Domain (refer to [Section 7](#Security) for the
...
The SR Policy SAFI route updates utilize the Tunnel Encapsulation
      ...
      This usage differs notably from the approach described in [RFC9012](#RFC9012), where the Tunnel Encapsulation Attribute is
      ...
      existing AFI/SAFIs as defined in [RFC9012](#RFC9012). Details
      regarding the processing of the Tunnel Encapsulation Attribute for the
      SR Policy SAFI are provided in Sections [2.2](#ENCAPSATTR) and [2.3](#ENDCOLOR).

The northbound advertisement of the operational state of the SR
      Policy CPs as part of BGP - Link State (BGP-LS) [RFC9552](#RFC9552)
      topology information is specified in [BGP-LS-SR-POLICY](#I-D.ietf-idr-bgp-ls-sr-policy).

The signaling of Dynamic and Composite CPs (Sections
      [RFC9256](#RFC9256) and [RFC9256](#RFC9256), respectively, of
      [RFC9256](#RFC9256)) is outside the scope of this
      document.

The Color Extended Community (as defined in [RFC9012](#RFC9012)) is used to steer traffic into an SR Policy, as
      described in [RFC9256](#RFC9256). [Section 3](#EXTCOLOR) of this
      document updates [RFC9012](#RFC9012) with
...
<a name="SRPOLICYSAFI"></a>
## SR Policy SAFI and NLRI

The SR Policy SAFI with
        code point 73 is introduced in this document. The AFI used **MUST** be IPv4(1) or IPv6(2).
...
**NLRI Length:**
  1 octet indicating the length expressed in bits as
            defined in [RFC4760](#RFC4760). When AFI = 1,
            the value **MUST** be 96; when AFI = 2, the value
            **MUST** be 192.

**Distinguisher:**
  4-octet value uniquely identifying the SR Policy in
  ...
  - to differentiate multiple CPs
            of the same SR Policy
...
**Color:**
  4 octets that carry an unsigned non-zero integer
            value indicating the Color of the SR Policy as specified in [RFC9256](#RFC9256). The Color is
            used to match the Color of the destination prefixes to steer traffic
            into the SR Policy as specified in [RFC9256](#RFC9256).

**Endpoint:**
  a value that identifies the Endpoint of an SR Policy. The
...
The Color and Endpoint are used to automate the steering of BGP
        service routes on an SR Policy as described in [RFC9256](#RFC9256).

The NLRI containing an SR Policy CP is carried in a BGP
        UPDATE message [RFC4271](#RFC4271) using BGP multiprotocol
        extensions [RFC4760](#RFC4760) with an AFI of 1 or 2 (IPv4 or
        IPv6) and with a SAFI of 73. The fault management and error handling
        in the encoding of the NLRI are specified in [Section 5](#ERROR).

A BGP UPDATE message that carries the MP_REACH_NLRI or MP_UNREACH_NLRI
...
The next-hop network address field in SR Policy SAFI (73) updates
        ...
        [RFC4760](#RFC4760) and [RFC2545](#RFC2545).

It is important to note that at any BGP speaker receiving BGP
        ...
        selection rules. Details of the procedures are described in [Section 4](#OPERATIONS).

It has to be noted that if several CPs of the same SR
        ...
        **RECOMMENDED** that each NLRI use a different distinguisher. If BGP has
        ...
        Router-ID) as described in [RFC9256](#RFC9256).
        The ASN would be the ASN of the origin and the BGP Router-ID is
        determined in the following order:

- From the Route Origin Community [RFC4360](#RFC4360) if
            present and carrying an IP Address, or
- As the BGP ORIGINATOR_ID [RFC4456](#RFC4456) if present,
            or
- As the BGP Router-ID of the peer from which the update was
            received as a last resort.

[RFC9256](#RFC9256) specifies the selection
        of the active CP of the SR Policy by the SRPM based on the
        information provided to it by BGP.

<a name="ENCAPSATTR"></a>
## SR Policy and Tunnel Encapsulation Attribute

The content of the SR Policy CP is encoded in the
        Tunnel Encapsulation Attribute defined in [RFC9012](#RFC9012)
        ...
        malformed and handled by the "treat-as-withdraw" strategy [RFC7606](#RFC7606).
...
- The SR Policy SAFI NLRI is defined in [Section 2.1](#SRPOLICYSAFI).
- The Tunnel Encapsulation Attribute is defined in [RFC9012](#RFC9012).
...
- Preference, Binding SID, Priority, SR Policy Name, SR Policy
            ...
            sub-TLVs are defined in [Section 2.4](#SRPOLICYTLV).
...
A Tunnel Encapsulation Attribute **MUST NOT** contain more than one TLV
        ...
        handled by the "treat-as-withdraw" strategy [RFC7606](#RFC7606).

BGP does not need to perform the validation of the tunnel (i.e., SR
        Policy) itself as indicated in [RFC9012](#RFC9012).
        The validation of the SR Policy information that is advertised using
        the sub-TLVs specified in [Section 2.4](#SRPOLICYTLV) is performed by
        the SRPM.
...
The Tunnel Egress Endpoint and Color sub-TLVs of the Tunnel
        Encapsulation Attribute, as defined in [RFC9012](#RFC9012), are
        ...
        sub-TLVs are present, a BGP speaker **MUST** ignore them and **MAY** remove
        them from the Tunnel Encapsulation Attribute during propagation.

Similarly, any other sub-TLVs, including those specified in [RFC9012](#RFC9012), that do not have explicitly defined applicability
        to the SR Policy SAFI **MUST** be ignored by the BGP speaker and **MAY** be
        removed from the Tunnel Encapsulation Attribute during
        propagation.

<a name="SRPOLICYTLV"></a>
## SR Policy Sub-TLVs
...
Preference, Binding SID, SRv6 Binding SID, Segment-List, Priority,
        ...
        Encapsulation Attribute [RFC9012](#RFC9012) being defined in this
        section.

Weight and Segment are sub-TLVs of the Segment-List sub-TLV
        mentioned above.
...
The fault management and error handling in the encoding of the
        sub-TLVs defined in this section are specified in [Section 5](#ERROR). For the TLVs/sub-TLVs that are specified as single
        instance, only the first instance of that TLV/sub-TLV is used: the
        other instances **MUST** be ignored and **MUST NOT** considered to be
        malformed.
...
None of the sub-TLVs defined in the following subsections have any
        ...
        further processing as described in [RFC9256](#RFC9256).
...
The Preference sub-TLV is used to carry the Preference of an SR
          ...
          SRPM as described in [RFC9256](#RFC9256).

The Preference sub-TLV is **OPTIONAL**; it **MUST NOT** appear more
...
**Length:**
  Specifies the length of the value field (i.e., not
                including Type and Length fields) in terms of octets. The value
                **MUST** be 6.

**Flags:**
  1 octet of flags. No flags are defined in this
                document. The Flags field **MUST** be set to zero on transmission
                and **MUST** be ignored on receipt.

**RESERVED:**
  1 octet of reserved bits. This field **MUST** be set to
                zero on transmission and **MUST** be ignored on receipt.

**Preference:**
  a 4-octet value indicating the Preference of the
                SR Policy CP as described in [RFC9256](#RFC9256).


<a name="BINDINGSIDTLV"></a>
### Binding SID Sub-TLV

The Binding SID sub-TLV is used to signal the BSID-related
          information of the SR Policy CP. The contents of this
          sub-TLV are used by the SRPM as described in [RFC9256](#RFC9256).

The Binding SID sub-TLV is **OPTIONAL**; it **MUST NOT** appear more
          than once in the SR Policy encoding.

When the Binding SID sub-TLV is used to signal an SRv6 SID, the
          selection of the corresponding SRv6 Endpoint Behavior [RFC8986](#RFC8986) to be instantiated is determined by the headend
          node. It is **RECOMMENDED** that the SRv6 Binding SID sub-TLV, as
          defined in [Section 2.4.3](#SRV6BINDINGSIDTLV), be used when signaling an SRv6 BSID
...
**Length:**
  Specifies the length of the value field (i.e., not
                including Type and Length fields) in terms of octets. The value
                **MUST** be 18 when a SRv6 BSID is present, 6 when an SR-MPLS
                BSID is present, or 2 when no BSID is present.

**Flags:**
  1 octet of flags. The following flags are defined in
                the registry "SR Policy Binding SID Flags" as described in [Section 6.6](#IANABSIDFLAGS):
  ...
  **Figure 5: SR Policy Binding SID Flags**
  ...
  - The S-Flag encodes the "Specified-BSID-Only"
                  behavior. It is used by SRPM as described in [RFC9256](#RFC9256).
  - The I-Flag encodes the "Drop-Upon-Invalid" 
                  behavior. It is used by SRPM as described in [RFC9256](#RFC9256) to
...
**RESERVED:**
  1 octet of reserved bits. **MUST** be set to zero on
                transmission and **MUST** be ignored on receipt.

**Binding SID:**
  If the length is 2, then no BSID is
  ...
  **Figure 6: Binding SID Label Encoding**
  ...
  ```
  The Label field is validated by the SRPM but **MUST NOT** contain the reserved MPLS label values (0-15). If the length
                is 18, then the BSID contains a 16-octet SRv6 SID.


<a name="SRV6BINDINGSIDTLV"></a>
### SRv6 Binding SID Sub-TLV

The SRv6 Binding SID sub-TLV is used to signal the SRv6-BSID-related information of an SR Policy CP. It enables
          the specification of the SRv6 Endpoint Behavior [RFC8986](#RFC8986) to be instantiated on the
          headend node. The contents of this sub-TLV are used by the SRPM as
          described in [RFC9256](#RFC9256).

The SRv6 Binding SID sub-TLV is **OPTIONAL**. More than one SRv6
          Binding SID sub-TLV **MAY** be signaled in the same SR Policy encoding
...
**Flags:**
  ...
  **Figure 8: SR Policy SRv6 Binding SID Flags**
  ...
  - The I-Flag encodes the "Drop-Upon-Invalid"
                  ...
                  [RFC9256](#RFC9256).
  - The B-Flag, when set, indicates the presence of
                  the "SRv6 Endpoint Behavior & SID Structure" encoding
                  specified in [Section 2.4.4.2.4](#BEHAVIORSTRUCT).
  - The unassigned bits in the Flags field **MUST** be set to zero
                  upon transmission and **MUST** be ignored upon receipt.
**RESERVED:**
  1 octet of reserved bits. This field **MUST** be set to
...
**SRv6 Binding SID:**
  Contains a 16-octet SRv6 SID. The value 0
                **MAY** be used when the controller wants to indicate the desired
                SRv6 Endpoint Behavior, SID Structure, or flags without
                specifying the BSID.

**SRv6 Endpoint Behavior and SID Structure:**
  Optional, as
                defined in [Section 2.4.4.2.4](#BEHAVIORSTRUCT). The SRv6 Endpoint
                Behavior and SID Structure **MUST NOT** be included when the SRv6
                SID has not been included.


<a name="SLTLV"></a>
### Segment List Sub-TLV

The Segment List sub-TLV encodes a single explicit path towards
          the Endpoint as described in [RFC9256](#RFC9256). The Segment List sub-TLV
...
The Segment List sub-TLV contains zero or more Segment sub-TLVs
...
**Length:**
  The total length (not including the Type and Length
...
Validation of an explicit path encoded by the Segment List
...
The Weight sub-TLV is **OPTIONAL**; it **MUST NOT** appear more than
            once inside the Segment List sub-TLV.
...
**RESERVED:**
  1 octet of reserved bits. This field **MUST** be set
                  to zero on transmission and **MUST** be ignored on receipt.

**Weight:**
  4 octets carrying an unsigned integer value indicating the
                  weight associated with a segment list as described in
                  [RFC9256](#RFC9256). A weight value of zero is
                  invalid.
...
A Segment sub-TLV describes a single segment in a segment list
...
The Segment sub-TLVs are **OPTIONAL** and **MAY** appear multiple times
            in the Segment List sub-TLV.

[RFC9256](#RFC9256) defines several Segment
            Types:
...
The following subsections specify the sub-TLVs used for
            Segment Types A and B. The other segment types are specified in
            [RFC9831](#RFC9831). As specified in
            [RFC9256](#RFC9256), a mix of SR-MPLS and SRv6
            segments make the segment-list invalid.

<a name="TYPEA"></a>
##### Segment Type A

The Type A Segment sub-TLV encodes a single SR-MPLS SID. The
              format is as follows and is used to encode MPLS Label fields as
              specified in [RFC3032](#RFC3032) and [RFC5462](#RFC5462):
...
**Length:**
  Specifies the length of the value field (i.e.,
                    not including Type and Length fields) in terms of octets.
                    The value **MUST** be 6.

**Flags:**
  1 octet of flags as defined in [Section 2.4.4.2.3](#SEGMENTFLAGS).

**RESERVED:**
  1 octet of reserved bits. This field **MUST** be
...
- The S bit **MUST** be zero upon transmission and **MUST** be
                  ignored upon reception.
- If the originator wants the receiver to choose the TC
                  value, it sets the TC field to zero.
- If the originator wants the receiver to choose the TTL
                  value, it sets the TTL field to 255.
- If the originator wants to recommend a value for these
                  fields, it puts those values in the TC and/or TTL
                  fields.
- The receiver **MAY** override the originator's values for
...
The Type B Segment sub-TLV encodes a single SRv6 SID. The
...
**SRv6 Endpoint Behavior and SID Structure:**
  Optional, as
                    defined in [Section 2.4.4.2.4](#BEHAVIORSTRUCT). The SRv6
                    Endpoint Behavior and SID Structure **MUST NOT** be included
                    when the SRv6 SID has not been included.


The sub-TLV code point 2 defined for the advertisement of
              Segment Type B in the earlier draft versions of this document has been
              deprecated to avoid backward compatibility issues.

<a name="SEGMENTFLAGS"></a>
##### SR Policy Segment Flags

The Segment Type sub-TLVs described above may contain the
              following SR Policy Segment Flags in their Flags field. Also
              refer to [Section 6.8](#IANASIDFLAGS):
...
- When the V-Flag is set, it is used by SRPM for "SID
                  verification" as described in [RFC9256](#RFC9256).
- When the B-Flag is set, it indicates the presence of
                  the "SRv6 Endpoint Behavior & SID Structure" encoding
                  specified in [Section 2.4.4.2.4](#BEHAVIORSTRUCT).
- The unassigned bits in the Flags field **MUST** be set to zero
                  upon transmission and **MUST** be ignored upon receipt.
...
- B-Flag applies to Segment Type B. If B-Flag appears with
                  Segment Type A, it **MUST** be ignored.

<a name="BEHAVIORSTRUCT"></a>
##### SRv6 Endpoint Behavior and SID Structure

The Segment Type sub-TLVs described above **MAY** contain the
              SRv6 Endpoint Behavior and SID Structure [RFC8986](#RFC8986) encoding as described below:
...
**Endpoint Behavior:**
  2 octets. It carries the SRv6 Endpoint
                    Behavior code point for this SRv6 SID as defined in [RFC8986](#RFC8986). When
                    set with the value 0xFFFF (i.e., Opaque), the choice of SRv6
                    Endpoint Behavior is left to the headend.

**Reserved:**
  2 octets of reserved bits. This field **MUST** be
                    set to zero on transmission and **MUST** be ignored on
                    receipt.

**Locator Block Length:**
  1 octet. SRv6 SID Locator Block
                    length in bits.

**Locator Node Length:**
  1 octet. SRv6 SID Locator Node
                    length in bits.

**Function Length:**
  1 octet. SRv6 SID Function length in
                    bits.

**Argument Length:**
  1 octet. SRv6 SID Arguments length in
                    bits.


The total of the locator block, locator node, function, and
              argument lengths **MUST** be less than or equal to 128.

<a name="ENLPTLV"></a>
### Explicit NULL Label Policy Sub-TLV

To steer an unlabeled IP packet into an SR Policy for the MPLS
          data plane, it is necessary to push a label stack of one or more
          labels on that packet.

The Explicit NULL Label Policy (ENLP) sub-TLV is used to indicate
          whether an Explicit NULL Label [RFC3032](#RFC3032) must be
          pushed on an unlabeled IP packet before any other labels.

If an ENLP sub-TLV is not present, the decision of whether to
          push an Explicit NULL label on a given packet is a matter of local
          configuration.

The ENLP sub-TLV is **OPTIONAL**; it **MUST NOT** appear more than
...
**ENLP (Explicit NULL Label Policy):**
  ...
  **2:**
    Push an IPv6 Explicit NULL label on an unlabeled IPv6
  ...
  **4:**
  ...
  This field can have one of the values as specified in [Section 6.10](#IANAENLP). The ENLP unassigned values
                may be used for future extensions. Implementations adhering to
                this document **MUST** ignore the ENLP sub-TLV with
                unrecognized values (viz. other than 1 through 4). The behavior
                signaled in this sub-TLV **MAY** be overridden by
                local configuration by the network operator based on their
                deployment requirements. [RFC9256](#RFC9256) describes the behavior on the
                headend for the handling of the Explicit NULL label.


<a name="POLICYPRIORITY"></a>
...
The Priority sub-TLV is **OPTIONAL**; it **MUST NOT** appear more than
          once in the SR Policy encoding.
...
**Priority:**
  A 1-octet value indicating the priority as
                specified in [RFC9256](#RFC9256).
...
It is **RECOMMENDED** that the size of the symbolic name for the
          CP be limited to 255 bytes. Implementations **MAY** choose
...
An operator **MAY** set the SR Policy Name sub-TLV to associate a
          ...
          being advertised via the SR Policy NLRI.

Usage of the SR Policy Name sub-TLV is described in [RFC9256](#RFC9256).
...
The SR Policy Name sub-TLV may exceed 255 bytes in length due to a
          ...
          [RFC9012](#RFC9012), the sub-TLV
          type defines the size of the Length field. Therefore, for the SR Policy
          Name sub-TLV, a code point of 128 or higher is used.
...
The SR Policy Name sub-TLV is **OPTIONAL**; it **MUST NOT** appear more
          than once in the SR Policy encoding.
...
**SR Policy Name:**
  Symbolic name for the SR Policy without a NULL
                terminator with encoding as specified in [RFC9256](#RFC9256).


<a name="EXTCOLOR"></a>
# Color Extended Community

The Color Extended Community [RFC9012](#RFC9012) is used to
      ...
      [RFC9012](#RFC9012); hence, it is outside the scope of this document
      as well.
...
**Type 0 (bits 00):**
  Specific Endpoint Match. Request a match for the
            Endpoint that is the BGP NH.

**Type 1 (bits 01):**
  Specific or Null Endpoint Match. Request a match
            for either the Endpoint that is the BGP NH or a null Endpoint (e.g.,
            a default gateway).

**Type 2 (bits 10):**
  Specific, Null, or Any Endpoint Match. Request
...
The details of the SR Policy steering mechanisms based on these
      Color-Only types are specified in [RFC9256](#RFC9256).

One or more Color Extended Communities **MAY** be
      associated with a BGP route update. Sections [RFC9256](#RFC9256), [RFC9256](#RFC9256), and [RFC9256](#RFC9256) of [RFC9256](#RFC9256) specify the steering behaviors over SR Policies when
      multiple Color Extended Communities are associated with a BGP route.

<a name="OPERATIONS"></a>
# SR Policy Operations

As mentioned in [Section 1](#INTRO), BGP is not the actual
      ...
      are specified in [RFC9256](#RFC9256).
...
The distinguisher of each SR Policy NLRI prevents undesired BGP
        route selection among these SR Policy NLRIs and allows their
        propagation across RRs [RFC4456](#RFC4456).
...
Moreover, one or more route targets **SHOULD** be attached to the
        ...
        headends for the advertised SR Policy update.
...
If no route target is attached to the SR Policy NLRI, then it is
        ...
        NO_ADVERTISE community [RFC1997](#RFC1997) **MUST** be attached to
        the SR Policy update (see further details in [Section 4.2.3](#PROPAGATE)).

<a name="RECEPT"></a>
## Reception of an SR Policy NLRI

On reception of an SR Policy NLRI, a BGP speaker first determines
        if it is valid as described in [Section 4.2.1](#ACCEPT); then, the BGP speaker performs the decision process for
        selection of the best route ([RFC4271](#RFC4271)). The key difference from the base BGP decision
        ...
        [Section 4.2.2](#USABLE). The selected best route is
        "propagated" ([RFC4271](#RFC4271)) as described in [Section 4.2.3](#PROPAGATE), irrespective of its "usability" by the local
        router.
...
When a BGP speaker receives an SR Policy NLRI from a neighbor, it
          **MUST** first perform validation based on the following rules in
          addition to the validation described in [Section 5](#ERROR):
...
- The SR Policy NLRI **MUST** include a distinguisher, Color, and
              ...
              Endpoint).
- The SR Policy update **MUST** have either the NO_ADVERTISE
...
- The Tunnel Encapsulation Attribute **MUST** be attached to the
              BGP UPDATE message and **MUST** have a Tunnel Type TLV set to SR Policy
              (code point is 15).
...
If one or more route targets are present, then at least one route
          ...
          to be considered usable. The BGP Identifier is defined in [RFC4271](#RFC4271) as a 4-octet IPv4 address and is updated by [RFC6286](#RFC6286) as a 4-octet, unsigned, non-zero integer.
...
Once BGP on the receiving node has determined that the SR Policy
          ...
          selection process as described in [RFC9256](#RFC9256).

When an update for an SR Policy NLRI results in its becoming
          unusable, BGP **MUST** delete its corresponding SR Policy CP
          from the SRPM.

The SRPM applies the rules defined in [RFC9256](#RFC9256) to determine whether the SR Policy
...
SR Policy NLRIs that have the NO_ADVERTISE community attached to
          them **MUST NOT** be propagated.

By default, a BGP node receiving an SR Policy NLRI **MUST NOT**
          propagate it to any External BGP (EBGP) neighbor. An implementation **MAY** provide an
          ...
          domain (see [Section 7](#Security) for details).

A BGP node advertises a received SR Policy NLRI to its Internal BGP (IBGP)
          neighbors according to normal IBGP propagation rules.

By default, a BGP node receiving an SR Policy NLRI **SHOULD NOT**
...
A BGP node **MUST NOT** alter the SR Policy information carried in
...
This section describes the error-handling actions, as described in
      [RFC7606](#RFC7606), that are to be performed for the handling of
      the BGP UPDATE messages for the BGP SR Policy SAFI.
...
A BGP speaker **MUST** perform the following syntactic validation of the
      ...
      endpoint address as specified in [Section 2.1](#SRPOLICYSAFI).
...
When the error determined allows for the router to skip the malformed
      ...
      when a "AFI/SAFI disable" action is not possible.

The validation of the TLVs/sub-TLVs introduced in this document and
      defined in their respective subsections of [Section 2.4](#SRPOLICYTLV)
      ...
      TLVs/sub-TLVs specified in [RFC9012](#RFC9012) **MUST**
      be done as described in that document. In case of any error detected,
      either at the attribute or its TLV/sub-TLV level, the
      "treat-as-withdraw" strategy **MUST** be applied. This is because an SR
      Policy update without a valid Tunnel Encapsulation Attribute (comprised
      of all valid TLVs/sub-TLVs) is not usable.

An SR Policy update that is determined not to be valid (and, therefore,
      malformed) based on the rules described in [Section 4.2.1](#ACCEPT) **MUST** be
      handled by the "treat-as-withdraw" strategy.

The validation of the individual fields of the TLVs/sub-TLVs defined
      in [Section 2.4](#SRPOLICYTLV) are beyond the scope of BGP as they are
      handled by the SRPM as described in the individual TLV/sub-TLV
      subsections. A BGP implementation **MUST NOT** perform semantic
...
This document uses code point allocations from the following existing
      registries in the "Subsequent Address Family Identifiers (SAFI) Parameters" registry group:
...
This document creates the following new
      registries in the "Border Gateway Protocol (BGP) Tunnel Encapsulation" registry group:
...
This document registers a SAFI code point in the "SAFI Values" registry of the "Subsequent Address
        Family Identifiers (SAFI) Parameters" registry group as follows:

**Table: BGP SAFI Code Point**
...
This document registers a Tunnel Type code point in the "BGP Tunnel
        Encapsulation Attribute Tunnel Types" registry under the "Border Gateway Protocol (BGP) Tunnel Encapsulation" registry group.

**Table: Tunnel Type Code Point**

| Value | Description | Reference |
| --- | --- | --- |
| 15 | SR Policy | RFC 9830 |

<a name="IANATUNNSUBTLV"></a>
## BGP Tunnel Encapsulation Attribute Sub-TLVs

This document defines sub-TLVs in the "BGP Tunnel
        Encapsulation Attribute Sub-TLVs" registry under the "Border Gateway Protocol (BGP) Tunnel Encapsulation" registry group.

**Table: BGP Tunnel Encapsulation Attribute Sub-TLV Code Points**
...
**Table: SR Policy Segment List Sub-TLV Code Points**
...
**Table: SR Policy Binding SID Flags**
...
**Table: SR Policy SRv6 Binding SID Flags**
...
<a name="IANASIDFLAGS"></a>
## SR Policy Segment Flags

This document creates a new registry called "SR
        Policy Segment Flags" under the "Border Gateway Protocol (BGP) Tunnel Encapsulation" registry group.
        The registration policy of this registry is "IETF Review" (see [RFC8126](#RFC8126)).

The following flags are defined:

**Table: SR Policy Segment Flags**
...
<a name="IANAEXTCOMCOFIELD"></a>
## Color Extended Community Color-Only Types

This document creates a new registry called "Color
...
**Table: Color Extended Community Color-Only Types**
...
IANA will maintain a new registry under the
        "Segment Routing" registry group with the registration policy of
        "Standards Action" (see [RFC8126](#RFC8126)). The new registry is
        called "SR Policy ENLP Values" and contains the code points allocated
        to the ENLP field defined in [Section 2.4.5](#ENLPTLV). The registry
        contains the following code points:

**Table: SR Policy ENLP Values**
...
The security mechanisms of the base BGP security model apply to the
      ...
      Considerations section of [RFC4271](#RFC4271) for a discussion of
      BGP security. Also, refer to [RFC4272](#RFC4272) and [RFC6952](#RFC6952) for analysis of security issues for BGP.
...
The BGP SR Policy extensions specified in this document enable
      ...
      trusted SR domain [RFC8402](#RFC8402); its security
      ...
      or in error to an EBGP peering session outside the SR domain.
...
The specification of BGP models is an ongoing work based on [BGP-YANG-MODEL](#I-D.ietf-idr-bgp-model); its future extensions are expected
      ...
      can be managed using the same data models.

The YANG data model for the operation and management of SR Policies [SR-POLICY-YANG](#I-D.ietf-spring-sr-policy-yang) reports the SR Policies
...
<a name="RFC1997"></a> **[RFC1997]** R. Chandra, P. Traina, T. Li, "BGP Communities Attribute", RFC 1997, DOI 10.17487/RFC1997, August 1996. <https://www.rfc-editor.org/info/rfc1997> [Local MD](rfc1997.md)
...
<a name="RFC6286"></a> **[RFC6286]** E. Chen, J. Yuan, "Autonomous-System-Wide Unique BGP Identifier for BGP-4", RFC 6286, DOI 10.17487/RFC6286, June 2011. <https://www.rfc-editor.org/info/rfc6286> [Local MD](rfc6286.md)
...
<a name="I-D.ietf-idr-bgp-model"></a> **[BGP-YANG-MODEL]** M. Jethanandani, K. Patel, S. Hares, J. Haas, "YANG Model for Border Gateway Protocol (BGP-4)", Internet-Draft draft-ietf-idr-bgp-model-18, October 2024. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-idr-bgp-model-18>
...
<a name="RFC4456"></a> **[RFC4456]** T. Bates, E. Chen, R. Chandra, "BGP Route Reflection: An Alternative to Full Mesh Internal BGP (IBGP)", RFC 4456, DOI 10.17487/RFC4456, April 2006. <https://www.rfc-editor.org/info/rfc4456> [Local MD](rfc4456.md)
...
<a name="I-D.ietf-spring-sr-policy-yang"></a> **[SR-POLICY-YANG]** K. Raza, T. Saleh, Z. Shunwan, D. Voyer, M. Durrani, S. Matsushima, V. Beeram, "YANG Data Model for Segment Routing Policy", Internet-Draft draft-ietf-spring-sr-policy-yang-05, May 2025. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-spring-sr-policy-yang-05>

# Acknowledgments

The authors of this document would like to thank Shyam Sethuram, John Scudder,
      Przemyslaw Krol, Alex id      Bogdanov, Nandan Saha, Bruno       Decraene, Gurusiddesh Nidasesi, Kausik Majumdar, Zafar Ali, Swadesh Agarwal, Jakob Heitz,
      Viral Patel, Peng Shaofu,
      Cheng Li, Martin Vigoureux,
      John Scudder, Vincent Roca,
      Brian Haberman, Mohamed       Boucadair, Shunwan Zhuang, Andrew Alston, Jeffrey (Zhaohui)       Zhang, Nagendra Nainar, Rajesh Melarcode Venkateswaran, Nat       Kao, Boris Hassanov, Vincent       Roca, Russ Housley, and Dan       Romascanu for their comments and review of this document. The authors
      would like to thank Susan Hares for her detailed
      shepherd review that helped in improving the document.
