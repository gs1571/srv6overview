...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-idr-bgpls-srv6-ext-14
...
**Keywords:** BGP, BGP-LS, Segment Routing, SRv6

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-idr-bgpls-srv6-ext-14)
- [alternate](https://dx.doi.org/10.17487/rfc9514)
...
Information about the current status of this document, any
            ...
            [https://www.rfc-editor.org/info/rfc9514](https://www.rfc-editor.org/info/rfc9514).
...
- [1. Introduction](#INTROLSSRV6)
  - [1.1. Requirements Language](#section-1.1)
- [2. BGP-LS Extensions for SRv6](#BGP-LS-SRv6)
- [3. SRv6 Node Attributes](#SRNODEATTR)
  - [3.1. SRv6 Capabilities TLV](#SRCAPATTR)
  - [3.2. SRv6 Node MSD Types](#SRNODEMSD)
- [4. SRv6 Link Attributes](#SRLINKATTR)
  - [4.1. SRv6 End.X SID TLV](#SRENDXTLV)
  - [4.2. SRv6 LAN End.X SID TLV](#SRLANENDXTLV)
  - [4.3. SRv6 Link MSD Types](#SRLINKMSD)
- [5. SRv6 Prefix Attributes](#SRPFXATTR)
  - [5.1. SRv6 Locator TLV](#SRLOCTLV)
- [6. SRv6 SID NLRI](#SRSIDNLRI)
  - [6.1. SRv6 SID Information TLV](#SRSIDINFO)
- [7. SRv6 SID Attributes](#SRSIDATTR)
  - [7.1. SRv6 Endpoint Behavior TLV](#SRFUNCTLV)
  - [7.2. SRv6 BGP PeerNode SID TLV](#SRPEERTLV)
- [8. SRv6 SID Structure TLV](#SRSTRUCTTLV)
- [9. IANA Considerations](#IANA)
  - [9.1. BGP-LS NLRI Types](#NLRITYPES)
  - [9.2. BGP-LS NLRI and Attribute TLVs](#BGPLSTLVS)
  - [9.3. SRv6 BGP EPE SID Flags](#BGPEPEFLAGS)
- [10. Manageability Considerations](#Manageability)
...
- [Appendix A. Differences with BGP-EPE for SR-MPLS](#BGPEPE)
- [Acknowledgements](#Acknowledgements)
- [Contributors](#section-appendix.c)
- [Authors' Addresses](#authors-addresses)

<a name="INTROLSSRV6"></a>
# Introduction

SRv6 refers to Segment Routing instantiated on the IPv6 data plane
      [RFC8402](#RFC8402). An SRv6 segment is often referred to by its
      SRv6 Segment Identifier (SID).

The network programming paradigm [RFC8986](#RFC8986) is central
...
The IS-IS and OSPFv3 link-state routing protocols have been extended
      to advertise some of these SRv6 SIDs and SRv6-related information [RFC9352](#RFC9352) [RFC9513](#RFC9513). Other SRv6 SIDs may be
      instantiated on a node via other mechanisms for topological or service
      functionalities.

The advertisement of SR-related information along with the topology is specified in [RFC9085](#RFC9085)
      for the MPLS data plane instantiation (SR-MPLS) and in [RFC9086](#RFC9086) for BGP Egress Peer Engineering (EPE). On similar lines, introducing the
...
<a name="BGP-LS-SRv6"></a>
# BGP-LS Extensions for SRv6

BGP-LS [RFC7752](#RFC7752) defines the Node, Link, and Prefix
...
When a BGP-LS router advertises topology information that it sources
      ...
      corresponding SRv6 information from the SRv6 extensions for IS-IS [RFC9352](#RFC9352) or OSPFv3 [RFC9513](#RFC9513) as applicable. In
...
The SRv6 information pertaining to a node is advertised via the
      BGP-LS Node NLRI using the BGP-LS Attribute TLVs as follows:

- The SRv6 capabilities of the node are advertised via the SRv6
          Capabilities TLV ([Section 3.1](#SRCAPATTR)).
- Maximum SID Depth (MSD) types introduced for SRv6 are advertised
          ([Section 3.2](#SRNODEMSD)) using the Node MSD TLV specified in
          [RFC8814](#RFC8814).
- Algorithm support for SRv6 is advertised via the SR-Algorithm TLV
          specified in [RFC9085](#RFC9085).

The SRv6 information pertaining to a link is advertised via the
      BGP-LS Link NLRI using the BGP-LS Attribute TLVs as follows:

- The SRv6 SID of the IGP Adjacency SID or the BGP EPE Peer Adjacency
          SID [RFC8402](#RFC8402) is advertised via the SRv6 End.X SID
          TLV introduced in this document ([Section 4.1](#SRENDXTLV)).
- The SRv6 SID of the IGP Adjacency SID to a non-Designated Router (DR)
          or non-Designated Intermediate System (DIS) [RFC8402](#RFC8402)
          is advertised via the SRv6 LAN End.X SID TLV introduced in this
          document ([Section 4.2](#SRLANENDXTLV)).
- MSD types introduced for SRv6 are advertised ([Section 4.3](#SRLINKMSD)) using the Link MSD TLV specified in [RFC8814](#RFC8814).
...
- The SRv6 Locator is advertised via the SRv6 Locator TLV introduced in
          this document ([Section 5.1](#SRLOCTLV)).
- The attributes of the SRv6 Locator are advertised via the Prefix
          Attribute Flags TLV specified in [RFC9085](#RFC9085).

The SRv6 SIDs associated with the node are advertised using the
      BGP-LS SRv6 SID NLRI introduced in this document ([Section 6](#SRSIDNLRI)). 
...
If the SRv6 SIDs had been advertised
      ...
      Attribute TLVs for the SRv6 SID NLRI are introduced in this document as follows:

- The Endpoint behavior of the SRv6 SID is advertised via the SRv6
          Endpoint Behavior TLV ([Section 7.1](#SRFUNCTLV)).
- The BGP EPE Peer Node context for a PeerNode SID and the Peer
          Set context for a PeerSet SID [RFC8402](#RFC8402) are
          advertised via the SRv6 BGP PeerNode SID TLV ([Section 7.2](#SRPEERTLV)).
...
Subsequent sections of this document specify the encoding and usage
      ...
      common field definitions provided in [RFC7752](#RFC7752).
...
This BGP-LS Attribute TLV is used to announce the SRv6 capabilities
        ...
        support by the node. A single instance of this TLV **MUST**
        be included in the BGP-LS Attribute for each SRv6-capable node. The
        IS-IS SRv6 Capabilities sub-TLV [RFC9352](#RFC9352) and the OSPFv3 SRv6 Capabilities TLV [RFC9513](#RFC9513) that map to this BGP-LS TLV are
...
**Flags:**
  2-octet field. The flags are copied from the IS-IS SRv6
              Capabilities sub-TLV ([RFC9352](#RFC9352)) or from the OSPFv3
              SRv6 Capabilities TLV ([RFC9513](#RFC9513)) in the case of
              IS-IS or OSPFv3, respectively.

**Reserved:**
  2-octet field that **MUST** be set to 0 when originated and
              ignored on receipt.
...
The Node MSD TLV [RFC8814](#RFC8814) of the BGP-LS Attribute
        of the Node NLRI is also used to advertise the limits and the Segment
        Routing Header (SRH) [RFC8754](#RFC8754) operations supported by
        the SRv6-capable node. The SRv6 MSD types specified in
        [RFC9352](#RFC9352) are also used with
        ...
        MSD types for BGP-LS are identical to those specified in [RFC9352](#RFC9352).
...
Each MSD type is encoded in the BGP-LS Node MSD TLV as a one-octet
        ...
        Node MSD advertisements specified in [RFC8814](#RFC8814).

<a name="SRLINKATTR"></a>
# SRv6 Link Attributes
...
SRv6 attributes and SIDs associated with a link or adjacency are
      ...
      associated with the BGP-LS Link NLRI.
...
The SRv6 End.X SID TLV is used to advertise the SRv6 SIDs
        ...
        this TLV is derived from the IS-IS SRv6 End.X SID sub-TLV ([RFC9352](#RFC9352)) or the OSPFv3
        SRv6 End.X SID sub-TLV ([RFC9513](#RFC9513)) in the case of IS-IS
...
This TLV is also used by BGP-LS to advertise the BGP EPE Peer
        ...
        [RFC9086](#RFC9086). The SRv6 SID for the BGP Peer Adjacency
        using End.X behaviors (viz. End.X, End.X with PSP, End.X with USP, and
        End.X with PSP & USP) [RFC8986](#RFC8986) indicates the
...
**Endpoint Behavior:**
  2-octet field. The Endpoint behavior code
              point for this SRv6 SID as defined in [RFC8986](#RFC8986).

**Flags:**
  1 octet of flags. The flags are copied from the
            IS-IS SRv6 End.X SID sub-TLV ([RFC9352](#RFC9352)) or the OSPFv3 SRv6 End.X SID
            sub-TLV ([RFC9513](#RFC9513))
            in the case of IS-IS or OSPFv3, respectively. In the case of the BGP EPE
            Peer Adjacency SID, the flags are as defined in [Section 7.2](#SRPEERTLV).

**Algorithm:**
  1-octet field. Algorithm associated with the
              SID.

**Weight:**
  1-octet field. The value represents the weight of the
              SID for the purpose of load balancing. The use of the weight is
              defined in [RFC8402](#RFC8402).

**Reserved:**
  1-octet field that **MUST** be set to 0 when originated
...
**Sub-TLVs:**
  Used to advertise sub-TLVs that provide additional
              ...
              [Section 8](#SRSTRUCTTLV).


<a name="SRLANENDXTLV"></a>
...
For a LAN interface, an IGP node ordinarily announces only its
        ...
        End.X SID sub-TLV ([RFC9352](#RFC9352)) or the OSPFv3 SRv6 LAN
        End.X SID sub-TLV ([RFC9513](#RFC9513)) in the case of IS-IS
        ...
        [RFC8986](#RFC8986) are advertised using the SRv6 LAN End.X SID
        TLV.

More than one instance of this TLV (one for each SRv6 LAN End.X SID) can be included in the BGP-LS
        Attribute.

The BGP-LS IS-IS SRv6 LAN End.X SID and BGP-LS OSPFv3 SRv6 LAN
        End.X SID TLVs have the following format:
...
**Flags:**
  1 octet of flags. The flags are copied from the IS-IS
              SRv6 LAN End.X SID sub-TLV ([RFC9352](#RFC9352)) or the OSPFv3 SRv6
              LAN End.X SID sub-TLV ([RFC9513](#RFC9513)) in the case of
              IS-IS or OSPFv3, respectively.
...
**Neighbor ID:**
  6 octets of Neighbor System-ID in the IS-IS SRv6 LAN
...
The Link MSD TLV [RFC8814](#RFC8814) of the BGP-LS Attribute
        ...
        The SRv6 MSD types specified in [ ](#RFC9352) are also used with
        the BGP-LS Link MSD TLV, as these code points are shared between the IS-IS,
        OSPF, and BGP-LS protocols. The description and semantics of these new
        MSD types for BGP-LS are identical as specified in [RFC9352](#RFC9352).

Each MSD type is encoded in the BGP-LS Link MSD TLV as a one-octet
        type followed by a one-octet value as derived from the IS-IS or OSPFv3
        Link MSD advertisements specified in [RFC8814](#RFC8814).

<a name="SRPFXATTR"></a>
# SRv6 Prefix Attributes
...
As specified in [RFC8986](#RFC8986), an SRv6 SID comprises
        locator, function, and argument parts.
...
The information advertised via this TLV is derived from the IS-IS
        SRv6 Locator TLV ([RFC9352](#RFC9352)) or the OSPFv3 SRv6
        Locator TLV ([RFC9513](#RFC9513)) in the case of IS-IS
        or OSPFv3, respectively.

The IPv6 Prefix matching the locator may also be advertised as
        ...
        [RFC7752](#RFC7752) that carries the routing metric for this
...
**Flags:**
  1 octet of flags. The flags are copied from the IS-IS
              SRv6 Locator TLV ([RFC9352](#RFC9352)) or the OSPFv3 SRv6
              Locator TLV ([RFC9513](#RFC9513)) in the case of
              IS-IS or OSPFv3, respectively.
...
**Reserved:**
  2-octet field. The value **MUST** be set to 0 when
              originated and ignored on receipt.

**Metric:**
  4-octet field. The value of the metric for the locator
              copied from the IS-IS SRv6 Locator TLV ([RFC9352](#RFC9352)) or the OSPFv3 SRv6
              Locator TLV ([RFC9513](#RFC9513)) in the case of
              IS-IS or OSPFv3, respectively.
...
The Link-State NLRI defined in [RFC7752](#RFC7752) is extended
      to carry the SRv6 SID information.
...
**Protocol-ID:**
  1-octet field that specifies the information source
            protocol [RFC7752](#RFC7752).

**Identifier:**
  8-octet value as defined in [RFC7752](#RFC7752).

**Local Node Descriptors TLV:**
  Set of Node Descriptor TLVs for the
            local node as defined in [RFC7752](#RFC7752) for IGPs, the Direct Protocol-ID,                                       
        and the Static configuration Protocol-ID or as defined in [RFC9086](#RFC9086)
            for BGP.

**SRv6 SID Descriptors:**
  Set of SRv6 SID Descriptor TLVs. This field
            **MUST** contain a single SRv6 SID Information TLV ([Section 6.1](#SRSIDINFO)) and **MAY** contain the Multi-Topology Identifier
            TLV [RFC7752](#RFC7752).


New TLVs for advertisement within the BGP-LS Attribute [RFC7752](#RFC7752) are defined in [Section 7](#SRSIDATTR) to carry
      the attributes of an SRv6 SID.

<a name="SRSIDINFO"></a>
## SRv6 SID Information TLV

An SRv6 SID that is associated with the node and advertised using
        the SRv6 SID NLRI is encoded using the SRv6 SID Information TLV.

When advertising the SRv6 SIDs from the IGPs, the SID information
        is derived from the IS-IS SRv6 End SID sub-TLV ([RFC9352](#RFC9352)) or the OSPFv3 SRv6 End
        SID sub-TLV ([RFC9513](#RFC9513)) in the case of IS-IS
        or OSPFv3, respectively.

The TLV carries the SRv6 SIDs corresponding to the BGP PeerNode and
        PeerSet SIDs [RFC8402](#RFC8402) when SRv6 BGP EPE functionality
        is enabled in BGP.
...
Each SRv6 SID instantiated on an SRv6-capable node has specific
        instructions (called "behavior") bound to it. [RFC8986](#RFC8986)
...
When advertising the SRv6 SIDs from the IGPs, the Endpoint
        behavior, Flags, and Algorithm are derived from the IS-IS SRv6 End SID
        sub-TLV ([RFC9352](#RFC9352)) or the OSPFv3 SRv6 End
        SID sub-TLV ([RFC9513](#RFC9513)) in the case of IS-IS
        or OSPFv3, respectively.
...
When advertising the SRv6 SIDs corresponding to the BGP EPE
        ...
        node. Flags are currently not defined. The algorithm value **MUST** be 0
        unless an algorithm is associated locally with the SRv6 Locator from
        which the SID is allocated.
...
**Endpoint Behavior:**
  2-octet field. The Endpoint behavior code
              ...
              Endpoint Behaviors" IANA registry ([RFC8986](#RFC8986)).

**Flags:**
  1 octet of flags. The flags map to the IS-IS or
              ...
              Protocol-ID. Undefined flags **MUST** be set to 0 when originating and ignored on
              receipt.
...
<a name="SRPEERTLV"></a>
## SRv6 BGP PeerNode SID TLV

The BGP PeerNode and PeerSet SIDs for SR-MPLS are specified in
        [RFC9086](#RFC9086). Similar Peer Node and Peer Set functionality
        can be realized with SRv6 using SIDs with END.X behavior. Refer to
        [Appendix A](#BGPEPE) for some differences between the signaling of
        ...
        advertised by BGP for the EPE functionality. This TLV **MUST** be included
        along with SRv6 SIDs that are associated with the BGP PeerNode or
        PeerSet functionality.
...
**Flags:**
  ...
  **Other bits are reserved for future use and
              **MUST** be set to 0 when originated and ignored on
...
**Peer AS Number:**
  4 octets of the BGP AS number of the peer
              router.

**Peer BGP Identifier:**
  4 octets of the BGP Identifier (BGP
              Router-ID) of the peer router.
...
The SRv6 SID Structure TLV is used to advertise the length of each
      individual part of the SRv6 SID as defined in [RFC8986](#RFC8986).
      ...
      and OSPFv3 SRv6 LAN End.X SID TLVs.
...
When advertising SRv6 SIDs from the IGPs, the SRv6 SID Structure
      ...
      ([RFC9352](#RFC9352)) or the
      OSPFv3 SRv6 SID Structure sub-TLV ([RFC9513](#RFC9513)) in the case of IS-IS or
      OSPFv3, respectively.
...
IANA has assigned the following code points in the
         "BGP-LS NLRI Types" registry:

**Table: Addition to BGP-LS NLRI Types Registry**
...
**Table: Additions to BGP-LS NLRI and Attribute TLVs Registry**
...
<a name="BGPEPEFLAGS"></a>
## SRv6 BGP EPE SID Flags

Per this document, IANA has created a new registry called "SRv6
        ...
        registry is "Standards Action" according to [RFC8126](#RFC8126).

The initial contents of the registry are as follows:

**Table: New SRv6 BGP EPE SID Flags Registry**
...
The new protocol extensions introduced in this document augment the
      ...
      than as discussed in Section [Manageability Considerations](#RFC7752) of [RFC7752](#RFC7752). Specifically, the malformed attribute tests for
      syntactic checks in Section [Fault Management](#RFC7752) of [RFC7752](#RFC7752) now encompass the new BGP-LS extensions defined in
      ...
      (e.g., an application or a controller) and not BGP.
...
The manageability considerations related to BGP EPE functionality are
      discussed in [RFC9086](#RFC9086) in the context of SR-MPLS; they
      also apply to this document in the context of SRv6.
...
The extensions specified in this document do not introduce any new
      ...
      underlying SRv6 features are covered by [SRV6-YANG](#I-D.ietf-spring-srv6-yang).

<a name="Security"></a>
# Security Considerations

The new protocol extensions introduced in this document augment the
      existing IGP topology information that is distributed via [RFC7752](#RFC7752). The advertisement of the SRv6
      ...
      [RFC7752](#RFC7752).  Section [Security Considerations](#RFC7752) of [RFC7752](#RFC7752) also applies to these extensions. The
      procedures and new TLVs defined in this document, by themselves, do not
      affect the BGP-LS security model discussed in [RFC7752](#RFC7752).

The extensions introduced in this document are used to propagate
      IGP-defined information [RFC9352](#RFC9352) [RFC9513](#RFC9513). These extensions represent the
      ...
      described in [RFC9352](#RFC9352) and [RFC9513](#RFC9513)).

The security considerations related to BGP EPE functionality are
      discussed in [RFC9086](#RFC9086) in the context of SR-MPLS, and they
      also apply to this document in the context of SRv6.

BGP-LS SRv6 extensions enable traffic-engineering use cases within
      the SR domain. SR operates within a trusted domain [RFC8402](#RFC8402), and its security considerations also apply to BGP-LS
...
<a name="RFC9085"></a> **[RFC9085]** S. Previdi, K. Talaulikar, C. Filsfils, H. Gredler, M. Chen, "Border Gateway Protocol - Link State (BGP-LS) Extensions for Segment Routing", RFC 9085, DOI 10.17487/RFC9085, August 2021. <https://www.rfc-editor.org/info/rfc9085> [Local MD](rfc9085.md)
...
<a name="I-D.ietf-spring-srv6-yang"></a> **[SRV6-YANG]** S. Raza, S. Agarwal, X. Liu, Z. Hu, I. Hussain, H. C. Shah, D. Voyer, S. Matsushima, K. Horiba, J. Rajamanickam, A. Abdelsalam, "YANG Data Model for SRv6 Base and Static", Internet-Draft draft-ietf-spring-srv6-yang-02, September 2022. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-spring-srv6-yang-02>

<a name="BGPEPE"></a>
# Differences with BGP-EPE for SR-MPLS
...
The signaling of SRv6 SIDs corresponding to BGP-EPE functionality as
      ...
      SIDs as specified in [RFC9086](#RFC9086). This section provides a
      high-level overview of the same.
...
The authors would like to thank Peter Psenak,
      Arun Babu, Pablo Camarillo,
      Francois Clad, Peng Shaofu,
      Cheng Li, Dhruv Dhody,
      Tom Petch, and Dan Romascanu
      for their review of this document and their comments.  The authors would
      also like to thank Susan Hares for her shepherd
      review and Adrian Farrel for his detailed Routing Area
      Directorate review.
