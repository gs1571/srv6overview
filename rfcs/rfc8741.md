...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-pce-lsp-control-request-11
...
- [prev](https://datatracker.ietf.org/doc/draft-ietf-pce-lsp-control-request-11)
- [alternate](https://dx.doi.org/10.17487/rfc8741)
...
- [3. LSP Control Request Flag](#LSP-Gain-Flag)
- [4. Operation](#Operation)
- [5. Security Considerations](#Security)
- [6. IANA Considerations](#IANA)
- [7. Manageability Considerations](#section-7)
  - [7.1. Control of Function and Policy](#section-7.1)
  - [7.2. Information and Data Models](#section-7.2)
  - [7.3. Liveness Detection and Monitoring](#section-7.3)
  - [7.4. Verify Correct Operations](#section-7.4)
  - [7.5. Requirements on Other Protocols](#section-7.5)
  - [7.6. Impact on Network Operations](#section-7.6)
...
"Path Computation Element Communication Protocol (PCEP) Extensions
      ...
      extensions to PCEP [RFC5440](#RFC5440) to enable
      stateful control of Traffic Engineering Label Switched Paths (TE LSPs)
      between and across PCEP sessions in compliance with [RFC4657](#RFC4657). 
...
**Delegation:**
  As per [RFC8051](#RFC8051), an operation to
...
**Revocation:**
  As per [RFC8231](#RFC8231), an operation
...
For redundant stateful PCEs ([RFC8231](#RFC8231)), during a PCE failure, one of the redundant PCEs
      ...
      a newly elected primary PCE can request to take over control.
...
The PCEs could use a proprietary algorithm to decide which LSPs can
      ...
      request control of some LSPs is needed.
...
Note that [RFC8231](#RFC8231) specifies a
      mechanism for a PCC to delegate an orphaned LSP to another PCE. The
      mechanism defined in this document can be used in conjunction with [RFC8231](#RFC8231). Ultimately, it is the PCC that
      decides which PCE to delegate the orphaned LSP to.

This specification provides a simple extension that allows a PCE
     to request control of one or more LSPs from any PCC over the stateful
      ...
      the LSPs are specified in accordance with [RFC8231](#RFC8231) unless explicitly set aside in this
      document.
...
Readers of this document are expected to have some familiarity with [RFC8231](#RFC8231).
...
<a name="LSP-Gain-Flag"></a>
# LSP Control Request Flag

The Stateful PCE Request Parameters (SRP) object is defined in
      [RFC8231](#RFC8231) and includes
      a Flags field.

A new "LSP Control Request" flag (30), also called the C
...
The C flag is ignored in case the R flag [RFC8281](#RFC8281) in the SRP object
      is set.
...
During normal operation, a PCC that wishes to delegate the control of
      an LSP sets the Delegate (D) flag ([RFC8231](#RFC8231)) to 1 in all PCRpt messages pertaining to the
...
An implementation of this feature needs to make
      ...
      check for PLSP-ID (as per [RFC8231](#RFC8231)). The D flag and C flag are mutually exclusive in a
...
accordance with stateful PCEP [RFC8231](#RFC8231). If
...
It should be noted that a legacy implementation of PCC that does not
   ...
   specified in [RFC8231](#RFC8231) (i.e., a PCErr with
   ...
   0, the error condition, as specified in [RFC8231](#RFC8231), (i.e., a PCErr with Error-type=19 (Invalid Operation)
   and error-value 3 (Attempted LSP Update Request for an LSP identified by an
   unknown PLSP-ID)) would be triggered.

[RFC8281](#RFC8281) describes the setup,
   ...
   in this document in conjunction with those in [RFC8281](#RFC8281).

<a name="Security"></a>
# Security Considerations

The security considerations listed in [RFC8231](#RFC8231) and [RFC8281](#RFC8281) 
...
A PCC is the ultimate arbiter of delegation. As per [RFC8231](#RFC8231), a local policy at the PCC is used to
...
Note that a PCE may not be sure if a PCC supports this feature. A
      ...
      in turn respond with an error, as described in [Section 4](#Operation). So, a PCE would learn this fact only when it wants to
      take control over an LSP. A PCE might also be susceptible to downgrade
      attacks by falsifying the error condition.

As per [RFC8231](#RFC8231), it is **RECOMMENDED**
   ...
   [RFC8253](#RFC8253), as per the recommendations and
   best current practices in 
   BCP 195 [RFC7525](#RFC7525) (unless explicitly
   excluded in [RFC8253](#RFC8253)).
...
All manageability requirements and considerations listed in [RFC5440](#RFC5440)  
  and [RFC8231](#RFC8231) 
...
The PCEP YANG module [PCEP-YANG](#I-D.ietf-pce-pcep-yang) could be extended to include a mechanism to trigger
	the LSP control request.

## Liveness Detection and Monitoring

Mechanisms defined in this document do not imply any new liveness detection
  and monitoring requirements in addition to those already listed in [RFC5440](#RFC5440).
...
Mechanisms defined in [RFC5440](#RFC5440) 
  ...
  Further, the mechanism described in this document can help the operator to
	request control of the LSPs at a particular PCE.
...
Thanks to Benjamin Kaduk, Martin Vigoureux, Alvaro Retana, and
      Barry Leiba for IESG reviews.
...
- Organization: AT&T
- Address: 200 S Laurel Avenue, Middletown, NJ, 07748, United States of America
- Email: cy098@att.com
...
- Organization: Cisco Systems, Inc.
- Address: 125 High Street, Boston, Massachusetts, 02110, United States of America
- Email: jakarthi@cisco.com
