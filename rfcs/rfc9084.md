...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-lsr-ospf-prefix-originator-12
...
- [prev](https://datatracker.ietf.org/doc/draft-ietf-lsr-ospf-prefix-originator-12)
- [alternate](https://dx.doi.org/10.17487/rfc9084)
...
Information about the current status of this document, any
            ...
            [https://www.rfc-editor.org/info/rfc9084](https://www.rfc-editor.org/info/rfc9084).
...
- [2. Protocol Extensions](#extensions.)
  - [2.1. Prefix Source OSPF Router-ID Sub-TLV](#SrcRtrTLV)
  - [2.2. Prefix Source Router Address Sub-TLV](#POTLV)
- [3. Elements of Procedure](#procedures)
- [4. Security Considerations](#security)
- [5. Operational Considerations](#operational)
...
Prefix attributes are advertised in OSPFv2 [RFC2328](#RFC2328)
      using the Extended Prefix Opaque Link State Advertisement (LSA) [RFC7684](#RFC7684) and in OSPFv3 [RFC5340](#RFC5340) using the
      various Extended Prefix LSA types [RFC8362](#RFC8362).
...
The procedures for identification of the originating router for a
      ...
      translation of Not-So-Stubby Area (NSSA) [RFC3101](#RFC3101) LSAs to AS-external LSAs, the information associated
      with the NSSA ASBR (or the router outside the OSPF domain) is not
      propagated across the OSPF domain.
...
While typically the originator of information in OSPF is identified
      ...
      Router Address, as respectively defined in [RFC3630](#RFC3630) and [RFC5329](#RFC5329) for
...
This document proposes extensions to the OSPF protocol for the
      ...
      - Link State (BGP-LS) [RFC7752](#RFC7752) [RFC9085](#RFC9085).
...
The key words "**MUST**", "**MUST NOT**",
        ...
        are to be interpreted as described in BCP 14 [RFC2119](#RFC2119) [RFC8174](#RFC8174) when, and only when, they appear in all capitals,
        as shown here.
...
For OSPFv2, the Prefix Source OSPF Router-ID Sub-TLV is an optional
        ...
        External-Prefix TLV [RFC8362](#RFC8362) when originating either
        an IPv4 [RFC5838](#RFC5838) or an IPv6 prefix advertisement.
...
The parent TLV of a prefix advertisement **MAY** include
...
For intra-area prefix advertisements, the Prefix Source OSPF
        Router-ID Sub-TLV **MUST** be considered invalid and ignored if the OSPF
...
A received Prefix Source OSPF Router-ID Sub-TLV with the OSPF
        Router ID field set to 0 **MUST** be considered invalid and
        ignored. Additionally, reception of such sub-TLVs
        **SHOULD** be logged as an error (subject to rate
        limiting).
...
**Where:**
  ...
  **Router Address:**
    ...
    TLV [RFC3630](#RFC3630) or in the OSPFv3 Router IPv6
    Address TLV [RFC5329](#RFC5329).
...
A received Prefix Source Router Address Sub-TLV that has an invalid
        ...
        **MUST** be considered invalid and ignored. Additionally,
        reception of such sub-TLVs **SHOULD** be logged as an
...
This section describes the procedure for the advertisement of the
      ...
      along with the prefix advertisement.
...
If the originating node is advertising an OSPFv2 Router Address TLV
      [RFC3630](#RFC3630) or an OSPFv3 Router IPv6
      Address TLV [RFC5329](#RFC5329), then the same
      address **MUST** be used in the Router Address field of the
      ...
      [RFC7684](#RFC7684) or N-bit set [RFC8362](#RFC8362)) on the originating node to
...
When an ABR generates inter-area prefix advertisements into its
      ...
      **MUST** only use the prefix originator information from this
      set of nodes.  The ABR **MUST NOT** include the Prefix Source
...
Implementations may support the propagation of the originating node
      ...
      outside the OSPF domain that owns the prefix.

When translating NSSA prefix advertisements [RFC3101](#RFC3101) to AS external prefix advertisements, the NSSA ABR
...
Since this document extends the OSPFv2 Extended Prefix LSA, the
      security considerations for [RFC7684](#RFC7684) are applicable.
      Similarly, since this document extends the OSPFv3
      E-Intra-Area-Prefix-LSA, E-Inter-Area-Prefix-LSA, E-AS-External-LSA, and
      E-NSSA-LSA, the security considerations for [RFC8362](#RFC8362) are
      ...
      the security aspects of OSPF protocol operations.
...
**Table: Codepoints in OSPFv2 Extended Prefix TLV Sub-TLVs**
...
**Table: Codepoints in OSPFv3 Extended-LSA Sub-TLVs**
...
Many thanks to Les Ginsberg for his suggestions
on this document. Also, thanks to Jeff Tantsura,
Rob Shakir, Gunter Van de Velde, Goethals Dirk, Smita Selot, Shaofu Peng, John E. Drake, and Baalajee S. for their review and
valuable comments. The authors would also like to thank Alvaro Retana for his detailed review and suggestions for
...
- Address: Changping District, Beijing, 102209, China
- Email: wangaj3@chinatelecom.cn
