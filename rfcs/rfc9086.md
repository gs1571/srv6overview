...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-idr-bgpls-segment-routing-epe-19
...
**Keywords:** BGP, BGP-LS, Segment Routing

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-idr-bgpls-segment-routing-epe-19)
- [alternate](https://dx.doi.org/10.17487/rfc9086)
...
Information about the current status of this document, any
            ...
            [https://www.rfc-editor.org/info/rfc9086](https://www.rfc-editor.org/info/rfc9086).
...
- [3. BGP Peering Segments](#BGPPEERINGSEG)
- [4. BGP-LS NLRI Advertisement for BGP Protocol](#EPENLRI)
  - [4.1. BGP Router-ID and Member AS Number](#BGPIDCONFEDMEMBER)
  - [4.2. Mandatory BGP Node Descriptors](#MANDATORYNODEDESC)
  - [4.3. Optional BGP Node Descriptors](#OPTIONALNODEDESC)
- [5. BGP-LS Attributes for BGP Peering Segments](#PEERSEGMENTS)
  - [5.1. Advertisement of the PeerNode SID](#PEERNODESID)
  - [5.2. Advertisement of the PeerAdj SID](#PEERADJSID)
  - [5.3. Advertisement of the PeerSet SID](#PEERSETSID)
- [6. IANA Considerations](#IANA)
  - [6.1. New BGP-LS Protocol-ID](#IANAPROT)
  - [6.2. Node Descriptors and Link Attribute TLVs](#IANANODEATTR)
...
Segment Routing (SR) leverages source routing.
...
The SR architecture [RFC8402](#RFC8402) defines three types of
...
This document describes extensions to the BGP - Link State Network
      ...
      defined for BGP-LS [RFC7752](#RFC7752) for
      ...
      data plane. The corresponding extensions for SRv6 are specified in [BGPLS-SRV6](#I-D.ietf-idr-bgpls-srv6-ext).

[RFC9087](#RFC9087) illustrates a centralized controller-based BGP Egress
      ...
      policy can be realized using the SR Policy framework [SR-POLICY](#I-D.ietf-spring-segment-routing-policy).
...
This document introduces a new BGP-LS Protocol-ID for BGP and defines
      ...
      advertised in the same BGP-LS Link NLRI.
...
<a name="BGPPEERINGSEG"></a>
# BGP Peering Segments

As described in [RFC8402](#RFC8402), a
...
An ingress border router of an AS may compose a list of SIDs to steer
      ...
      egress PE peer or peering interface.

Each BGP session **MUST** be described by a PeerNode
      ...
      their encoding are described in detail in [Section 5](#PEERSEGMENTS).
...
Any BGP Peering SIDs instantiated on the node are advertised via
      ...
      NLRI and its corresponding BGP-LS Attribute are described in [RFC9087](#RFC9087).

<a name="EPENLRI"></a>
# BGP-LS NLRI Advertisement for BGP Protocol
...
**Table: BGP-LS Protocol Identifier for BGP**
...
The use of a new Protocol-ID allows separation and differentiation
      ...
      carrying IGP link-state information defined in [RFC7752](#RFC7752).
...
The BGP Peering information along with their Peering Segments are
      ...
      as defined in [RFC7752](#RFC7752). In order to
      correctly describe BGP nodes, new TLVs are defined in this section.

[RFC7752](#RFC7752) defines BGP-LS Link NLRI
      type as follows:
...
+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
```
  Node Descriptors and Link Descriptors are defined in [RFC7752](#RFC7752).


<a name="BGPIDCONFEDMEMBER"></a>
## BGP Router-ID and Member AS Number
...
- BGP Router Identifier (BGP Router-ID):
            Type: 516
              ...
              Value: 4-octet unsigned non-zero integer representing the
                BGP Identifier as defined in [RFC6286](#RFC6286)
...
- Member-AS Number (Member-ASN)
            Type: 517
              ...
              Value: 4-octet unsigned non-zero integer representing the
                Member-AS Number [RFC5065](#RFC5065)

<a name="MANDATORYNODEDESC"></a>
## Mandatory BGP Node Descriptors

The following Node Descriptor TLVs **MUST** be included in BGP-LS NLRI
...
- Autonomous System Number (TLV 512) [RFC7752](#RFC7752), which contains the Autonomous System Number
          (ASN) or AS Confederation Identifier (an ASN) [RFC5065](#RFC5065), if confederations are used, of the local BGP
          node.

Note that [RFC6286](#RFC6286)
...
- Autonomous System Number (TLV 512) [RFC7752](#RFC7752), which contains the ASN or the AS Confederation
          Identifier (an ASN) [RFC5065](#RFC5065), if
          confederations are used, of the peer BGP node.

<a name="OPTIONALNODEDESC"></a>
## Optional BGP Node Descriptors

The following Node Descriptor TLVs **MAY** be included in BGP-LS NLRI
        as Local Node Descriptors when distributing BGP information:
...
- Node Descriptors as defined in [RFC7752](#RFC7752).
...
<a name="PEERSEGMENTS"></a>
# BGP-LS Attributes for BGP Peering Segments
...
**Table: BGP-LS TLV Code Points for BGP-EPE**
...
```
- Type: 1101, 1102, or 1103 as listed in [Table 2](#CODEPOINTVALUES)
...
+-+-+-+-+-+-+-+-+
          V-Flag: Value Flag. If set, then the SID carries a label
            ...
            Peering SID forwarding entry is implementation specific. [RFC9087](#RFC9087) discusses some of the possible
...
- SID/Index/Label. According to the TLV length and the V- and L-Flag settings, it contains either:
          A 3-octet local label where the 20 rightmost bits are used
            ...
            Global Block (SRGB) [RFC8402](#RFC8402)
...
Additional BGP-LS Link Attribute TLVs as defined in [RFC7752](#RFC7752) **MAY** be included with the BGP-LS Link NLRI in order to
      ...
      BGP-LS Attribute along with the PeerNode SID TLV.

<a name="PEERNODESID"></a>
## Advertisement of the PeerNode SID

The PeerNode SID TLV includes a SID associated with the BGP peer
        node that is described by a BGP-LS Link NLRI as specified in [Section 4](#EPENLRI).

The PeerNode SID, at the BGP node advertising it, has the following
        semantics (as defined in [RFC8402](#RFC8402)):
...
- Link Descriptors include the addresses used by the BGP session
            encoded using TLVs as defined in [RFC7752](#RFC7752):
            IPv4 Interface Address (TLV 259) contains the BGP session
                IPv4 local address.
              IPv4 Neighbor Address (TLV 260) contains the BGP session
                IPv4 peer address.
              IPv6 Interface Address (TLV 261) contains the BGP session
                IPv6 local address.
              IPv6 Neighbor Address (TLV 262) contains the BGP session
                IPv6 peer address.
  - IPv4 Interface Address (TLV 259) contains the BGP session
                IPv4 local address.
  - IPv4 Neighbor Address (TLV 260) contains the BGP session
...
<a name="PEERADJSID"></a>
## Advertisement of the PeerAdj SID
...
The PeerAdj SID TLV includes a SID associated with the underlying
        ...
        specified in [Section 4](#EPENLRI).
...
- Link Descriptors **MUST** include the following TLV, as defined in
  ...
  - Link Local/Remote Identifiers (TLV 258) contains the
                ...
                Remote Identifier. The value 0 is used by default when the
                link remote identifier is unknown.
- Additional Link Descriptors TLVs, as defined in [RFC7752](#RFC7752), **MAY** also be included to describe the addresses
...
<a name="PEERSETSID"></a>
## Advertisement of the PeerSet SID
...
The PeerSet SID TLV includes a SID that is shared amongst BGP peer
        ...
        as specified in [Section 4](#EPENLRI).
...
The PeerSet SID TLV containing the same SID value (encoded as
        defined in [Figure 2](#PEERSID)) is included in the BGP-LS
        Attribute for all of the BGP-LS Link NLRI corresponding to the
        PeerNode or PeerAdj segments associated with the peer set.
...
<a name="IANAPROT"></a>
## New BGP-LS Protocol-ID

This document defines a new value in the registry "BGP-LS
        Protocol-IDs":

**Table: BGP-LS Protocol-ID**

| Protocol-ID | NLRI information source protocol | Reference |
| --- | --- | --- |
| 7 | BGP | RFC 9086 |

<a name="IANANODEATTR"></a>
## Node Descriptors and Link Attribute TLVs
...
**Table: BGP-LS Descriptor TLV Code Points**
...
**Table: BGP-LS Attribute TLV Code Points**
...
The new protocol extensions introduced in this document augment the
      existing IGP topology information BGP-LS distribution [RFC7752](#RFC7752) by adding support for distribution
      of BGP peering topology information. As such, [RFC7752](#RFC7752) (Manageability Considerations) applies
      to these new extensions as well.

Specifically, the malformed Link-State NLRI and BGP-LS Attribute
      tests for syntactic checks in [RFC7752](#RFC7752) (Fault Management) now apply to the TLVs defined in
...
A consumer of the BGP-LS information retrieves this information from
      a BGP Speaker, over a BGP-LS session (refer to Sections [RFC7752](#RFC7752) and [RFC7752](#RFC7752) of [RFC7752](#RFC7752)). The handling of semantic or
...
BGP Peering Segments are associated with the normal BGP routing
      ...
      session. It is expected that this isolation as described in [RFC7752](#RFC7752) is followed when advertising BGP
      peering topology information via BGP-LS.
...
[RFC7752](#RFC7752) defines BGP-LS NLRI to
      which the extensions defined in this document apply. [RFC7752](#RFC7752) also applies to these extensions. The procedures and new
...
BGP-EPE enables engineering of traffic when leaving the
      ...
      Routing operates within a trusted domain [RFC8402](#RFC8402), and
...
<a name="I-D.ietf-idr-bgpls-srv6-ext"></a> **[BGPLS-SRV6]** Gaurav Dawra, Clarence Filsfils, Ketan Talaulikar, Mach Chen, Daniel Bernier, Bruno Decraene, "BGP Link State Extensions for SRv6", Internet-Draft draft-ietf-idr-bgpls-srv6-ext-08, June 2021. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-idr-bgpls-srv6-ext-08>

<a name="RFC9087"></a> **[RFC9087]** C Filsfils, S Previdi, G Dawra, E Aries, D Afanasiev, "Segment Routing Centralized BGP Egress Peer Engineering", RFC 9087, DOI 10.17487/RFC9087, August 2021. <https://www.rfc-editor.org/info/rfc9087> [Local MD](rfc9087.md)

<a name="I-D.ietf-spring-segment-routing-policy"></a> **[SR-POLICY]** C. Filsfils, K. Talaulikar, D. Voyer, A. Bogdanov, P. Mattes, "Segment Routing Policy Architecture", Internet-Draft draft-ietf-spring-segment-routing-policy-13, May 2021. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-spring-segment-routing-policy-13>

<a name="Acknowledgements"></a>
# Acknowledgements

The authors would like to thank Jakob Heitz,
      Howard Yang, Hannes       Gredler, Peter Psenak, Arjun       Sreekantiah, and Bruno Decraene for their 
      feedback and comments. Susan Hares helped in improving the clarity of
