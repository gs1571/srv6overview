...
*(BGP Prefix-SID in Large-Scale DCs)*
...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-spring-segment-routing-msdc-11
...
- [prev](https://datatracker.ietf.org/doc/draft-ietf-spring-segment-routing-msdc-11)
- [alternate](https://dx.doi.org/10.17487/rfc8670)
...
- [2. Large-Scale Data-Center Network Design Summary](#LARGESCALEDC)
  - [2.1. Reference Design](#REFDESIGN)
- [3. Some Open Problems in Large Data-Center Networks](#OPENPROBS)
- [4. Applying Segment Routing in the DC with MPLS Data Plane](#APPLYSR)
  - [4.1. BGP Prefix Segment (BGP Prefix-SID)](#BGPREFIXSEGMENT)
  - [4.2. EBGP Labeled Unicast (RFC 8277)](#eBGP8277)
    - [4.2.1. Control Plane](#CONTROLPLANE)
    - [4.2.2. Data Plane](#DATAPLANE)
    - [4.2.3. Network Design Variation](#VARIATIONS)
    - [4.2.4. Global BGP Prefix Segment through the Fabric](#FABRIC)
    - [4.2.5. Incremental Deployments](#INCRDEP)
  - [4.3. IBGP Labeled Unicast (RFC 8277)](#iBGP3107)
- [5. Applying Segment Routing in the DC with IPv6 Data Plane](#IPV6)
- [6. Communicating Path Information to the Host](#COMMHOSTS)
- [7. Additional Benefits](#BENEFITS)
  - [7.1. MPLS Data Plane with Operational Simplicity](#MPLSIMPLE)
  - [7.2. Minimizing the FIB Table](#MINFIB)
  - [7.3. Egress Peer Engineering](#EPE)
  - [7.4. Anycast](#ANYCAST)
- [8. Preferred SRGB Allocation](#SINGLESRGB)
- [9. IANA Considerations](#IANA)
- [10. Manageability Considerations](#MANAGE)
- [11. Security Considerations](#SEC)
...
Segment Routing (SR), as described in [RFC8402](#RFC8402), leverages the source-routing
...
The use cases described in this document should be considered in the
      ...
      in [RFC7938](#RFC7938). This document extends it by applying SR
      both with IPv6 and MPLS data planes.

<a name="LARGESCALEDC"></a>
# Large-Scale Data-Center Network Design Summary

This section provides a brief summary of the Informational RFC
      [RFC7938](#RFC7938), which outlines a practical network design
      suitable for data centers of various scales:
...
The following is a schematic of a five-stage Clos topology with four
      ...
      treated similarly, with a few modifications to the logic.
...
```
In the reference topology illustrated in [Figure 1](#FIGLARGE),
        it is assumed:

- Each node is its own autonomous system (AS) (Node X has AS X). 4-byte AS numbers
            are recommended ([RFC6793](#RFC6793)).
            For simple and efficient route propagation filtering,
                Node5, Node6, Node7, and Node8 use the same AS; Node3 and Node4
                use the same AS; and Node9 and Node10 use the same AS.
              ...
              Without loss of generality, these details will be
...
- Shortest-path routing with ECMP implements an oblivious routing
          ...
          exist, which may cause a traffic hotspot.
...
A BGP Prefix Segment is a segment associated with a BGP prefix. A
        ...
        along the ECMP-aware best path to the related prefix.

The BGP Prefix Segment is defined as the BGP Prefix-SID Attribute
        in [RFC8669](#RFC8669), which contains an
...
In this document, the network design decision has been made to
        ...
        simplification as explained in [Section 8](#SINGLESRGB), but this
        is not a requirement.

For illustration purposes, when considering an MPLS data plane, it
...
Referring to [Figure 1](#FIGLARGE) and 
	[RFC7938](#RFC7938), the following design modifications are
        introduced:

- Each node peers with its neighbors via an EBGP session with
            extensions defined in [RFC8277](#RFC8277) (named "EBGP8277"
            throughout this document) and with the BGP Prefix-SID attribute
            extension as defined in [RFC8669](#RFC8669).
- The forwarding plane at Tier-2 and Tier-1 is MPLS.
- The forwarding plane at Tier-3 is either IP2MPLS (if the host
            sends IP traffic) or MPLS2MPLS (if the host sends MPLS-encapsulated traffic).

[Figure 2](#FIGSMALL) zooms into a path from ServerA to ServerZ within the topology of [Figure 1](#FIGLARGE).
...
```
   ...
   +-----+                                         +-----+
     ...
     A                    <- Servers ->             Z
```
Referring to Figures [1](#FIGLARGE) and [2](#FIGSMALL), and assuming the IP address with the AS and
...
Node11 originates 192.0.2.11/32 in BGP and allocates to it a
          BGP Prefix-SID with label-index: index11 [RFC8669](#RFC8669).
...
Node7 receives the above update. As it is SR capable, Node7 is
...
Node4 receives the above update. As it is SR capable, Node4 is
          able to interpret the BGP Prefix-SID; therefore, it allocates the local
...
Node1 receives the above update. As it is SR capable, Node1 is
          able to interpret the BGP Prefix-SID; therefore, it allocates the local
          (incoming) label 16011 to the NLRI (instead of allocating a
...
Referring to [Figure 1](#FIGLARGE), and assuming all nodes
...
A network design choice could consist of switching all the
...
A network design choice could consist of allowing the hosts to
          send MPLS-encapsulated traffic based on the Egress Peer Engineering
          (EPE) use case as defined in [SR-CENTRAL-EPE](#I-D.ietf-spring-segment-routing-central-epe). For example,
          ...
          label is an EPE peer segment ([SR-CENTRAL-EPE](#I-D.ietf-spring-segment-routing-central-epe)) at Node11
          directing the traffic to Z.

<a name="FABRIC"></a>
### Global BGP Prefix Segment through the Fabric

When the previous design is deployed, the operator enjoys global
          BGP Prefix-SID and label allocation throughout the DC fabric.
...
- Traffic-engineered path to Node11: An application on a host
              ...
              to Node11 along the path (Node5, Node9, Node11).
...
The design previously described can be deployed incrementally.
          ...
          us show how the fabric connectivity is preserved.

From a signaling viewpoint, nothing would change; even though
          Node7 does not support the BGP Prefix-SID, it does propagate the
          attribute unmodified to its neighbors.

From a label-allocation viewpoint, the only difference is that
...
As a result, all the data-plane entries across the network would
...
The key point is that the end-to-end Label Switched Path (LSP) is
          ...
          label (the incoming label) but never for the outgoing label.
...
- Each node peers with its neighbors via an internal BGP session
            (IBGP) with extensions defined in [RFC8277](#RFC8277) (named
            "IBGP8277" throughout this document).
- Each node acts as a route reflector for each of its neighbors
...
- For simple and efficient route propagation filtering and as
            illustrated in [Figure 3](#IBGPFIG):
  ...
  - Node3 and Node4 use the same Cluster ID (Cluster-2).
  - Node9 and Node10 use the same Cluster ID (Cluster-3).
- The control-plane behavior is mostly the same as described in
...
Node5 originates 2001:DB8::5/128 with the attached BGP Prefix-SID for
      IPv6 packets destined to segment 2001:DB8::5 ([RFC8402](#RFC8402)).
...
The control-plane and data-plane processing of all the other nodes in
      ...
      (Spine node) and Node11 (ToR node) respectively.
...
An application on HostA that needs to send traffic to HostZ via only
      ...
      Routing Header (SRH, [IPv6-SRH](#I-D.ietf-6man-segment-routing-header)). The destination
      address and active segment is set to 2001:DB8::5. The next and last
      segment is set to 2001:DB8::11.
...
There are two general methods for communicating path information to
      ...
      network events, while the host pulls other state on demand.
...
As required by [RFC7938](#RFC7938), no new signaling protocol
        is introduced. The BGP Prefix-SID is a lightweight extension to BGP
        Labeled Unicast [RFC8277](#RFC8277). It applies either to EBGP- or
        IBGP-based designs.
...
Provided the same SRGB is configured on all nodes, all nodes use
        the same MPLS label for a given IP prefix. This is simpler from an
        operation standpoint, as discussed in [Section 8](#SINGLESRGB).
...
It is straightforward to combine the design illustrated in this
        ...
        [SR-CENTRAL-EPE](#I-D.ietf-spring-segment-routing-central-epe).
...
For example, the controller only needs to inject a per-flow state
        on the HostA to force it to send its traffic destined to a specific
        Internet destination D via a selected border node (say Node12 in [Figure 1](#FIGLARGE) instead of another border node, Node11) and a
...
The design presented in this document preserves the availability
        and load-balancing properties of the base design presented in [RFC8402](#RFC8402).
...
Assuming a consistent unique SRGB across the fabric as described in
      ...
      use {16005 16011} and {16008 16011} respectively for FA2 and FB2.
...
Let us assume a design where the SRGB is different at every node and
      ...
      of the BGP Prefix-SID as defined in [RFC8669](#RFC8669): SRGB of Node K starts at value
...
In this case, the controller would need to collect and store all of
      ...
      same SRGB, both policies are the same {16011}).
...
Even worse, the controller would instruct A to use {1005, 5011} for
      ...
      prone, more complex, and more difficult to troubleshoot.
...
The design and deployment guidelines described in this document are
      based on the network design described in [RFC7938](#RFC7938).
...
The deployment model assumed in this document is based on a single
      ...
      be leaked outside the domain.

As recommended in [RFC8402](#RFC8402),
      the same SRGB should be allocated in all nodes in order to facilitate
      the design, deployment, and operations of the domain.

When EPE ([SR-CENTRAL-EPE](#I-D.ietf-spring-segment-routing-central-epe)) is used (as
      explained in [Section 7.3](#EPE)), the same operational model is
...
This document proposes to apply SR to a well-known
      scalability requirement expressed in [RFC7938](#RFC7938) using the
      BGP Prefix-SID as defined in [RFC8669](#RFC8669).

It has to be noted, as described in [Section 10](#MANAGE), that the
      design illustrated in [RFC7938](#RFC7938) and in this document
...
Therefore, the solution proposed in this document does not introduce
      any additional security concerns from what is expressed in [RFC7938](#RFC7938) and [RFC8669](#RFC8669). It
      ...
      point of the domain as described in [Section 10](#MANAGE).
...
<a name="RFC7938"></a> **[RFC7938]** P. Lapukhov, A. Premji, J. Mitchell, "Use of BGP for Routing in Large-Scale Data Centers", RFC 7938, DOI 10.17487/RFC7938, August 2016. <https://www.rfc-editor.org/info/rfc7938> [Local MD](rfc7938.md)
...
<a name="RFC6793"></a> **[RFC6793]** Q. Vohra, E. Chen, "BGP Support for Four-Octet Autonomous System (AS) Number Space", RFC 6793, DOI 10.17487/RFC6793, December 2012. <https://www.rfc-editor.org/info/rfc6793> [Local MD](rfc6793.md)

<a name="I-D.ietf-spring-segment-routing-central-epe"></a> **[SR-CENTRAL-EPE]** C Filsfils, S Previdi, G Dawra, E Aries, D Afanasiev, "Segment Routing Centralized BGP Egress Peer Engineering", Internet-Draft draft-ietf-spring-segment-routing-central-epe-10, December 2017. Work in Progress <https://tools.ietf.org/html/draft-ietf-spring-segment-routing-central-epe-10>
...
- Address: 2077 Gateway Place, Suite #400, San Jose, CA 95119, United States of America
- Email: exa@arrcus.com
