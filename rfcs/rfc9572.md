...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-bess-evpn-bum-procedure-updates-14
...
**Keywords:** per-region I-PMSI, selective multicast forwarding, tunnel segmentation, inter-AS segmentation, inter-region segmentation

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-bess-evpn-bum-procedure-updates-14)
- [alternate](https://dx.doi.org/10.17487/rfc9572)
...
- [2. Tunnel Segmentation](#segmentation)
  - [2.1. Reasons for Tunnel Segmentation](#motivation)
- [3. Additional Route Types of EVPN NLRI](#routes)
  - [3.1. Per-Region I-PMSI A-D Route](#per-region)
  - [3.2. S-PMSI A-D Route](#section-3.2)
  - [3.3. Leaf A-D Route](#leaf)
- [4. Selective Multicast](#section-4)
- [5. Inter-AS Segmentation](#inter-as)
  - [5.1. Differences from Section 7.2.2 of RFC 7117 when Applied to EVPNs](#interaschanges)
  - [5.2. I-PMSI Leaf Tracking](#leaf-tracking)
  - [5.3. Backward Compatibility](#compatibility)
    - [5.3.1. Designated ASBR Election](#election)
- [6. Inter-Region Segmentation](#inter-region)
  - [6.1. Area/AS vs. Region](#region)
  - [6.2. Per-Region Aggregation](#aggregation)
  - [6.3. Use of S-NH-EC](#section-6.3)
  - [6.4. Ingress PE's I-PMSI Leaf Tracking](#section-6.4)
- [7. Multihoming Support](#section-7)
...
[RFC7117](#RFC7117) specifies procedures for multicast in the Virtual Private LAN
       ...
       Multicast VPN (MVPN) procedures specified in [RFC6513](#RFC6513) and [RFC6514](#RFC6514).
       [RFC7524](#RFC7524) specifies inter-area tunnel segmentation procedures for
	   both VPLS multicast and MVPNs.

[RFC7432](#RFC7432) specifies BGP MPLS-based Ethernet VPN (EVPN) procedures,
       including those handling Broadcast, Unknown Unicast, or Multicast
       (BUM) traffic. [RFC7432](#RFC7432) refers to [RFC7117](#RFC7117) for details but leaves a few feature gaps related to selective tunnel and tunnel segmentation ([Section 2.1](#motivation)).

This document aims to fill in those gaps by covering the use of selective
       and segmented tunnels in EVPNs. 
       In the same way that [RFC7432](#RFC7432) refers to [RFC7117](#RFC7117) for details, this document only specifies differences from relevant procedures provided in [RFC7117](#RFC7117) and [RFC7524](#RFC7524), rather than repeating the text from those documents.
       Note that these differences are applicable to EVPNs only
       and are not updates to [RFC7117](#RFC7117) or [RFC7524](#RFC7524).
...
It is assumed that the reader is familiar with concepts and terminologies related to MVPN technology [RFC6513](#RFC6513) [RFC6514](#RFC6514), VPLS multicast [RFC7117](#RFC7117), and EVPN technology [RFC7432](#RFC7432). For convenience, the following terms are briefly
       explained.

**AS:**
  Autonomous System

**PMSI [RFC6513](#RFC6513):**
...
**Leaf Auto-Discovery (A-D) Route [RFC6513](#RFC6513):**
...
**IMET A-D Route [RFC7432](#RFC7432):**
...
**SMET A-D Route [RFC9251](#RFC9251):**
...
MVPN provider tunnels and EVPN/VPLS BUM provider tunnels, which are
       ...
       which are summarized in [Section 2.1](#motivation) of this document.
       [RFC6513](#RFC6513) and [RFC6514](#RFC6514) cover MVPN inter-AS segmentation,
       [RFC7117](#RFC7117) covers
       VPLS multicast inter-AS segmentation, and [RFC7524](#RFC7524) (seamless MPLS
...
There is a difference between MVPN and VPLS multicast inter-AS
       segmentation (the VPLS approach is briefly described in [Section 5.1](#interaschanges)). For simplicity, EVPNs will use the same procedures as those for
...
For inter-area segmentation, [RFC7524](#RFC7524) requires the use of the Inter-Area
       ...
       differ from [RFC7524](#RFC7524) to make the segmentation procedures transparent to
       ingress and egress PEs.

[RFC7524](#RFC7524) assumes that segmentation happens at area borders.
       However, it could be at "regional" borders, where a region could be a
       sub-area, or even an entire AS plus its external links
       ([Section 6.1](#region)); this would
...
For example, an MVPN/VPLS/EVPN may span multiple providers,
       ...
       provider A uses ingress replication [RFC7988](#RFC7988), provider B uses RSVP-TE
       P2MP [RFC4875](#RFC4875), and provider C uses Multipoint LDP (mLDP) [RFC6388](#RFC6388)). Even if they use
...
Segmentation can also be used to divide an AS/area into smaller regions,
       ...
       [RFC7524](#RFC7524), a PE only needs to replicate to local PEs and Area Border Routers (ABRs).
...
Another advantage of the smaller region is smaller Bit Index
       Explicit Replication (BIER) subdomains [RFC8279](#RFC8279).
...
[RFC7432](#RFC7432) defines the format of EVPN NLRI as follows:
...
So far, eight route types have been defined in [RFC7432](#RFC7432),
        [RFC9136](#RFC9136), and
        [RFC9251](#RFC9251):

**Table: Pre-existing Route Types**
...
The "Route Type specific" field of the Type 9 and Type 10 EVPN NLRIs
        ...
        ([Section 3.3](#leaf)), i.e., non-Leaf A-D routes
...
The per-region I-PMSI A-D route has the following format. Its usage is
       discussed in [Section 6.2](#aggregation).
...
Other than the addition of the Ethernet Tag ID and Originator's Addr
        ...
        to the S-PMSI A-D route as defined in [RFC7117](#RFC7117). The procedures specified
        in [RFC7117](#RFC7117) also apply (including wildcard functionality),
...
The general procedures for Leaf A-D routes were first specified in
       [RFC6514](#RFC6514) for MVPNs. The principles therein apply to VPLSs and EVPNs as well.
       [RFC7117](#RFC7117) provides details regarding VPLS multicast, and this document points
       out some specifics for EVPNs, e.g., in [Section 5](#inter-as).

# Selective Multicast

[RFC9251](#RFC9251) specifies
...
With the above procedures, selective forwarding is done for all flows,
       ...
       defined for selective multicast for VPLS in [RFC7117](#RFC7117) are used. Other
       than the fact that different route types and formats are specified with an EVPN SAFI
       for S-PMSI A-D and Leaf A-D routes ([Section 3](#routes)), all
       procedures specified in [RFC7117](#RFC7117) with respect to selective multicast apply to
...
An optimization to the procedures provided in [RFC7117](#RFC7117) may be applied.  Even if
...
The optional optimizations specified for MVPNs in
       [RFC8534](#RFC8534) are also applicable
...
<a name="interaschanges"></a>
## Differences from Section 7.2.2 of RFC 7117 when Applied to EVPNs

The first paragraph of [RFC7117](#RFC7117) says:
...
The above VPLS behavior requires complicated VPLS-specific procedures
       ...
       is used; the above text from [RFC7117](#RFC7117) is not applicable to EVPNs.
...
With the different approach for EVPNs/MVPNs, each ASBR will re-advertise
       ...
       in [RFC7117](#RFC7117) itself.

The following bullet in [RFC7117](#RFC7117) does not apply
	to EVPNs.

The following bullet in [RFC7117](#RFC7117):

is changed to the following when applied to EVPNs:

Accordingly, the following paragraph in [RFC7117](#RFC7117):

is changed to the following when applied to EVPNs:

<a name="leaf-tracking"></a>
...
An ingress PE does not set the L flag in its IMET A-D route's PTA,
       ...
       its AS, and [RFC7432](#RFC7432) explicitly states that the L
       flag must be set to 0.

An implementation of [RFC7432](#RFC7432) might have used the Originating
...
If this backward-compatibility problem needs to be addressed, the
	following procedure **MUST** be used (see [Section 6.2](#aggregation)
...
When an ASBR re-advertises a per-region I-PMSI A-D route into an AS
       ...
       The EC and its use are specified in [RFC8584](#RFC8584).
...
<a name="inter-region"></a>
# Inter-Region Segmentation

<a name="region"></a>
## Area/AS vs. Region

[RFC7524](#RFC7524) addresses MVPN/VPLS inter-area segmentation and does not explicitly cover
...
The inter-AS segmentation procedures specified so far ([RFC6513](#RFC6513), [RFC6514](#RFC6514),
       [RFC7117](#RFC7117), and [Section 5](#inter-as) of this document) require all
...
The MVPN Inter-AS I-PMSI A-D route can be better called a "per-AS I-PMSI
       ...
       will advertise such a route, with the same Region ID and Ethernet Tag ID ([Section 3.1](#per-region)). Similar to the per-PE I-PMSI A-D
...
[RFC7524](#RFC7524) specifies the use of the S-NH-EC because it does not allow ABRs
...
Similar to [RFC7524](#RFC7524), the upstream RBR **MUST** (auto-)configure
	...
	field set to 0. Using this technique, the mechanisms specified in [RFC4684](#RFC4684)
...
[RFC7524](#RFC7524) specifies that when an ingress PE/ASBR (re-)advertises a
       ...
       I-PMSI A-D routes, as specified in [Section 5.2](#leaf-tracking).

The same backward-compatibility issue exists, and the same solution
       as that for the inter-AS case applies, as specified in [Section 5.3](#compatibility).
...
To support multihoming with segmentation, Ethernet Segment Identifier (ESI) labels **SHOULD** be
	...
	[RFC9573](#RFC9573) for all
	tunnel types, including Ingress Replication tunnels [RFC7988](#RFC7988).
...
The procedures specified in this document for selective forwarding via S-PMSI/Leaf A-D routes
	  are based on the same procedures as those used for MVPNs [RFC6513](#RFC6513) [RFC6514](#RFC6514)
	  and VPLS multicast [RFC7117](#RFC7117). The procedures for tunnel segmentation as specified
	  in this document are based on similar procedures used for MVPN inter-AS tunnel segmentation [RFC6514](#RFC6514) and inter-area tunnel segmentation [RFC7524](#RFC7524), as well as procedures
	  for VPLS multicast inter-AS tunnel segmentation [RFC7117](#RFC7117).
    When applied to EVPNs, they do not introduce new security concerns
    beyond those discussed in [RFC6513](#RFC6513), [RFC6514](#RFC6514), [RFC7117](#RFC7117),
    and [RFC7524](#RFC7524). They also do not introduce new security concerns
    compared to [RFC7432](#RFC7432).
...
<a name="RFC7117"></a> **[RFC7117]** R. Aggarwal, Y. Kamite, L. Fang, Y. Rekhter, C. Kodeboniya, "Multicast in Virtual Private LAN Service (VPLS)", RFC 7117, DOI 10.17487/RFC7117, February 2014. <https://www.rfc-editor.org/info/rfc7117> [Local MD](rfc7117.md)
...
<a name="RFC7524"></a> **[RFC7524]** Y. Rekhter, E. Rosen, R. Aggarwal, T. Morin, I. Grosclaude, N. Leymann, S. Saad, "Inter-Area Point-to-Multipoint (P2MP) Segmented Label Switched Paths (LSPs)", RFC 7524, DOI 10.17487/RFC7524, May 2015. <https://www.rfc-editor.org/info/rfc7524> [Local MD](rfc7524.md)
...
<a name="RFC8534"></a> **[RFC8534]** A. Dolganow, J. Kotalwar, E. Rosen, Z. Zhang, "Explicit Tracking with Wildcard Routes in Multicast VPN", RFC 8534, DOI 10.17487/RFC8534, February 2019. <https://www.rfc-editor.org/info/rfc8534> [Local MD](rfc8534.md)

<a name="RFC8584"></a> **[RFC8584]** J. Rabadan, S. Mohanty, A. Sajassi, J. Drake, K. Nagaraj, S. Sathappan, "Framework for Ethernet VPN Designated Forwarder Election Extensibility", RFC 8584, DOI 10.17487/RFC8584, April 2019. <https://www.rfc-editor.org/info/rfc8584> [Local MD](rfc8584.md)
...
<a name="RFC9573"></a> **[RFC9573]** Z Zhang, E Rosen, W Lin, Z Li, IJ Wijnands, "MVPN/EVPN Tunnel Aggregation with Common Labels", RFC 9573, DOI 10.17487/RFC9573, May 2024. <https://www.rfc-editor.org/info/rfc9573> [Local MD](rfc9573.md)
...
<a name="RFC4875"></a> **[RFC4875]** R. Aggarwal, D. Papadimitriou, S. Yasukawa, "Extensions to Resource Reservation Protocol - Traffic Engineering (RSVP-TE) for Point-to-Multipoint TE Label Switched Paths (LSPs)", RFC 4875, DOI 10.17487/RFC4875, May 2007. <https://www.rfc-editor.org/info/rfc4875> [Local MD](rfc4875.md)

<a name="RFC6388"></a> **[RFC6388]** IJ. Wijnands, I. Minei, K. Kompella, B. Thomas, "Label Distribution Protocol Extensions for Point-to-Multipoint and Multipoint-to-Multipoint Label Switched Paths", RFC 6388, DOI 10.17487/RFC6388, November 2011. <https://www.rfc-editor.org/info/rfc6388> [Local MD](rfc6388.md)

<a name="RFC7988"></a> **[RFC7988]** E. Rosen, K. Subramanian, Z. Zhang, "Ingress Replication Tunnels in Multicast VPN", RFC 7988, DOI 10.17487/RFC7988, October 2016. <https://www.rfc-editor.org/info/rfc7988> [Local MD](rfc7988.md)

<a name="RFC8279"></a> **[RFC8279]** IJ. Wijnands, E. Rosen, A. Dolganow, T. Przygienda, S. Aldrin, "Multicast Using Bit Index Explicit Replication (BIER)", RFC 8279, DOI 10.17487/RFC8279, November 2017. <https://www.rfc-editor.org/info/rfc8279> [Local MD](rfc8279.md)
...
The following people also contributed to this document through their earlier
