...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-spring-segment-routing-mpls-22
...
- [prev](https://datatracker.ietf.org/doc/draft-ietf-spring-segment-routing-mpls-22)
- [alternate](https://dx.doi.org/10.17487/rfc8660)
...
- [1. Introduction](#convert-section-1)
  - [1.1. Requirements Language](#convert-section-1.1)
- [2. MPLS Instantiation of Segment Routing](#convert-section-2)
  - [2.1. Multiple Forwarding Behaviors for the Same Prefix](#convert-section-2.1)
  - [2.2. SID Representation in the MPLS Forwarding Plane](#convert-section-2.2)
  - [2.3. Segment Routing Global Block and Local Block](#convert-section-2.3)
  - [2.4. Mapping a SID Index to an MPLS Label](#convert-section-2.4)
  - [2.5. Incoming Label Collision](#convert-section-2.5)
    - [2.5.1. Tiebreaking Rules](#convert-section-2.5.1)
    - [2.5.2. Redistribution between Routing Protocol Instances](#convert-section-2.5.2)
  - [2.6. Effect of Incoming Label Collision on Outgoing Label Programming](#convert-section-2.6)
  - [2.7. PUSH, CONTINUE, and NEXT](#convert-section-2.7)
    - [2.7.1. PUSH](#convert-section-2.7.1)
    - [2.7.2. CONTINUE](#convert-section-2.7.2)
    - [2.7.3. NEXT](#convert-section-2.7.3)
  - [2.8. MPLS Label Downloaded to the FIB for Global and Local SIDs](#convert-section-2.8)
  - [2.9. Active Segment](#convert-section-2.9)
  - [2.10. Forwarding Behavior for Global SIDs](#convert-section-2.10)
    - [2.10.1. Forwarding for PUSH and CONTINUE of Global SIDs](#convert-section-2.10.1)
    - [2.10.2. Forwarding for the NEXT Operation for Global SIDs](#convert-section-2.10.2)
  - [2.11. Forwarding Behavior for Local SIDs](#convert-section-2.11)
    - [2.11.1. Forwarding for the PUSH Operation on Local SIDs](#convert-section-2.11.1)
    - [2.11.2. Forwarding for the CONTINUE Operation for Local SIDs](#convert-section-2.11.2)
    - [2.11.3. Outgoing Label for the NEXT Operation for Local SIDs](#convert-section-2.11.3)
- [3. IANA Considerations](#convert-section-3)
- [4. Manageability Considerations](#convert-section-4)
- [5. Security Considerations](#convert-section-5)
- [6. References](#section-6)
  - [6.1. Normative References](#section-6.1)
  - [6.2. Informative References](#section-6.2)
- [Appendix A. Examples](#convert-section-a)
  - [A.1. IGP Segment Examples](#convert-section-a.1)
  - [A.2. Incoming Label Collision Examples](#convert-section-a.2)
    - [A.2.1. Example 1](#convert-section-a.2.1)
    - [A.2.2. Example 2](#convert-section-a.2.2)
    - [A.2.3. Example 3](#convert-section-a.2.3)
    - [A.2.4. Example 4](#convert-section-a.2.4)
    - [A.2.5. Example 5](#convert-section-a.2.5)
    - [A.2.6. Example 6](#convert-section-a.2.6)
    - [A.2.7. Example 7](#convert-section-a.2.7)
    - [A.2.8. Example 8](#convert-section-a.2.8)
    - [A.2.9. Example 9](#convert-section-a.2.9)
    - [A.2.10. Example 10](#convert-section-a.2.10)
    - [A.2.11. Example 11](#convert-section-a.2.11)
    - [A.2.12. Example 12](#convert-section-a.2.12)
    - [A.2.13. Example 13](#convert-section-a.2.13)
    - [A.2.14. Example 14](#convert-section-a.2.14)
  - [A.3. Examples for the Effect of Incoming Label Collision on an Outgoing Label](#convert-section-a.3)
    - [A.3.1. Example 1](#convert-section-a.3.1)
    - [A.3.2. Example 2](#convert-section-a.3.2)
- [Acknowledgements](#convert-section-7)
- [Contributors](#convert-section-6)
- [Authors' Addresses](#authors-addresses)

<a name="convert-section-1"></a>
# Introduction

The Segment Routing architecture [RFC8402](#RFC8402) can be directly applied to
   ...
   behavior is specified in other documents such as [RFC8665](#RFC8665), [RFC8666](#RFC8666), and [RFC8667](#RFC8667).

The Segment Routing problem statement is described in [RFC7855](#RFC7855).

Coexistence of SR over the MPLS forwarding plane with LDP [RFC5036](#RFC5036) is
   specified in [RFC8661](#RFC8661).

Policy routing and traffic engineering using Segment Routing can be
   found in [ROUTING-POLICY](#ROUTING-POLICY).

<a name="convert-section-1.1"></a>
...
<a name="convert-section-2"></a>
# MPLS Instantiation of Segment Routing

MPLS instantiation of Segment Routing fits in the MPLS architecture
   as defined in [RFC3031](#RFC3031) from both a control-plane and forwarding-plane
   perspective:

- From a control-plane perspective, [RFC3031](#RFC3031) does not mandate a
      single signaling protocol.  Segment Routing makes use of various
      control-plane protocols such as link-state IGPs [RFC8665](#RFC8665) [RFC8666](#RFC8666) [RFC8667](#RFC8667).
...
A global SID is a label, or an index that may be mapped to an
   ...
   packet. [Section 2.4](#convert-section-2.4) specifies the procedure to map a global segment
...
- Every range in the list of ranges specifying the SRGB **MUST NOT**
      cover or overlap with a reserved label value or range [RFC7274](#RFC7274),
      respectively.
...
A local segment **MAY** be allocated from the Segment Routing Local Block
   (SRLB) [RFC8402](#RFC8402) or from any unused label as long as it does not use
   ...
   particular router [ROUTING-POLICY](#ROUTING-POLICY). The rules
...
This subsection specifies how the MPLS label value is calculated
   ...
   MCC such as IS-IS [RFC8667](#RFC8667) or OSPF
   [RFC8665](#RFC8665). This section only
...
Consider a SID represented by the index "I". Consider an SRGB as
   specified in [Section 2.3](#convert-section-2.3). The total size of the SRGB, represented by
...
An example for how a router calculates labels and forwards traffic
   ...
   [Appendix A.1](#convert-section-a.1).

<a name="convert-section-2.5"></a>
## Incoming Label Collision

The MPLS Architecture [RFC3031](#RFC3031) defines the term Forwarding
...
We define SR FEC [RFC8402](#RFC8402) as one of the following:

- (Prefix, Routing Instance, Topology, Algorithm) [RFC8402](#RFC8402), where a
...
- (number of adjacencies, list of next hops, list of outgoing
      ...
      parallel adjacencies [RFC8402](#RFC8402).
- (Endpoint, Color). This FEC represents an SR Policy [RFC8402](#RFC8402).
- (Mirror SID). The Mirror SID (see [RFC8402](#RFC8402)) is the IP
      address advertised by the advertising node to identify the Mirror SID.
      The IP address is encoded as specified in [Section 2.5.1](#convert-section-2.5.1).
...
This section covers the **RECOMMENDED** procedure for handling the scenario
   ...
   found in [Appendix A.2](#convert-section-a.2).
...
2. Within an MCC, apply tiebreaking rules to select one FEC only, and
      ...
      than the shortest path first [RFC8402](#RFC8402) are not installed in the
      FIB.
          ...
          deterministic.
  1. If the same set of FECs are attached to the same label "L1",
          then the tiebreaking rules **MUST** always select the same FEC
          irrespective of the order in which the FECs and the label "L1"
          are received. In other words, the tiebreaking rule **MUST** be
          deterministic.
3. If there is still collision between the FECs belonging to
      ...
      FEC.
4. Install the selected FEC into the IP FIB and its incoming label into
        the label FIB.
5. The remaining FECs with the default algorithm (see the
      Prefix-SID algorithm specification [RFC8402](#RFC8402)) may be installed
      ...
      [RFC8402](#RFC8402) are not installed in the FIB.

<a name="convert-section-2.5.1"></a>
### Tiebreaking Rules
...
- Dynamic SID assignment:
         ...
         defined by the implementation.
                The Binding SID [RFC8402](#RFC8402) assigned to the SR Policy always has a
...
- Each FEC is assigned a FEC type encoded in 8 bits. The type codepoints 
      ...
      of this section are as follows:
              120:
                    ...
                    140:
                    Parallel Adjacency [RFC8402](#RFC8402)
                    150:
                    SR Policy [RFC8402](#RFC8402)
                    160:
                    Mirror SID [RFC8402](#RFC8402)
...
<a name="convert-section-2.5.2"></a>
...
- Else,
         ...
         the route according to [Section 2.4](#convert-section-2.4) and install it in FIB.
  - the index is not redistributed and if the receiving instance
         decides to advertise an index with the redistributed route, it
         is the duty of the receiving instance to allocate a fresh
         index relative to its own SRGB. Note that in this case, the
...
<a name="convert-section-2.5.2.1"></a>
#### Illustration
...
<a name="convert-section-2.5.2.2"></a>
#### Illustration 2

Consider the example in the illustration described in [Section 2.5.2.1](#convert-section-2.5.2.1).
...
In the general case, the ingress node may not have the exact same
   ...
   the tiebreaking rules specified in [Section 2.5](#convert-section-2.5) **MUST NOT** be
...
Examples for the behavior specified in this section can be found in
   [Appendix A.3](#convert-section-a.3).

<a name="convert-section-2.7"></a>
...
PUSH, NEXT, and CONTINUE are operations applied by the forwarding
   ...
   [RFC8402](#RFC8402). This subsection specifies how to implement each of these
   operations in the MPLS forwarding plane.

<a name="convert-section-2.7.1"></a>
### PUSH

As described in [RFC8402](#RFC8402), PUSH corresponds to pushing one or more
   ...
   tunnel [RFC7510](#RFC7510) or the Layer 2 Tunneling Protocol version 3 (L2TPv3) [RFC4817](#RFC4817), towards a particular
   next hop.

 When pushing labels onto a packet's label stack, the Time-to-Live
   (TTL) field [RFC3032](#RFC3032) [RFC3443](#RFC3443) and the Traffic Class (TC)
   field [RFC3032](#RFC3032) [RFC5462](#RFC5462) of each label stack entry must, of
   course, be set.  This document does not specify any set of rules for
   setting these fields; that is a matter of local policy. Sections [2.10](#convert-section-2.10) and [2.11](#convert-section-2.11) specify additional details about forwarding
   behavior.

<a name="convert-section-2.7.2"></a>
### CONTINUE

As described in [RFC8402](#RFC8402), the CONTINUE operation corresponds to
   swapping the incoming label with an outgoing label. The value of the
   outgoing label is calculated as specified in Sections [2.10](#convert-section-2.10) and [2.11](#convert-section-2.11).

<a name="convert-section-2.7.3"></a>
### NEXT

As described in [RFC8402](#RFC8402), NEXT corresponds to popping the topmost
   ...
   received packet was an Adj-SID [RFC8402](#RFC8402); on receiving the
   ...
   virtual interface (e.g., the UDP tunnel [RFC7510](#RFC7510) or L2TPv3 tunnel
   [RFC4817](#RFC4817)) towards the next hop corresponding to the Adj-SID.

<a name="convert-section-2.7.3.1"></a>
#### Mirror SID

If the active SID in the received packet was a Mirror SID (see [RFC8402](#RFC8402)) allocated by the receiving router, the receiving
   ...
   label in [RFC5331](#RFC5331) because a Mirror SID does not make any
   assumptions about the packet underneath the top label.

<a name="convert-section-2.8"></a>
...
The label corresponding to the global SID "Si", which is represented by the
   ...
   label is calculated as specified in [Section 2.4](#convert-section-2.4).

For Local SIDs, the MCC is responsible for downloading the correct
   label value to the FIB. For example, an IGP with SR extensions [RFC8667](#RFC8667) [RFC8665](#RFC8665) downloads the MPLS label corresponding to an Adj-SID [RFC8402](#RFC8402).

<a name="convert-section-2.9"></a>
## Active Segment

When instantiated in the MPLS domain, the active segment on a packet
   corresponds to the topmost label and is calculated
   according to the procedure specified in Sections [2.10](#convert-section-2.10) and [2.11](#convert-section-2.11). When
   arriving at a node, the topmost label corresponding to the active SID
   matches the MPLS label downloaded to the FIB as specified in [Section 2.4](#convert-section-2.4).

<a name="convert-section-2.10"></a>
...
This document covers the calculation of the outgoing label for the
   ...
   document and may be covered in other documents such as [ROUTING-POLICY](#ROUTING-POLICY).

<a name="convert-section-2.10.1"></a>
...
Suppose an MCC on router "R0" determines that, before sending the packet towards a neighbor "N", the PUSH or CONTINUE
  ...
  UDP tunnel [RFC7510](#RFC7510) or L2TPv3 tunnel [RFC4817](#RFC4817)).
...
The method by which the MCC on router "R0" determines that the PUSH or
   ...
   "Si" for the PUSH operation is the case where IS-IS [RFC8667](#RFC8667) 
...
For the CONTINUE operation, an example of a method used to determine the SID
   "Si" is the case where IS-IS [RFC8667](#RFC8667) receives the Prefix-SID "Si" sub-TLV advertised with
...
- If neighbor "N" does not support SR or advertises an invalid
       SRGB or a SRGB that is too small for the SID "Si", then:
         ...
         [RFC3031](#RFC3031) and [RFC3032](#RFC3032), forward the packet. The method
         ...
         determined by another MCC, such as LDP [RFC5036](#RFC5036), to send the
         packet.
  ...
  - Otherwise, drop the packet.
- Else,
              Calculate the outgoing label as specified in [Section 2.4](#convert-section-2.4) using
          the SRGB of neighbor "N".
              ...
              pushing rules specified in [RFC3032](#RFC3032).
                    Else,
                      swap the incoming label with the calculated label
           according to the label-swapping rules in [RFC3031](#RFC3031).
...
<a name="convert-section-2.10.2"></a>
### Forwarding for the NEXT Operation for Global SIDs

As specified in [Section 2.7.3](#convert-section-2.7.3), the NEXT operation corresponds to popping
...
The action on the packet after popping the topmost label depends on
   ...
   popped. Examples of the NEXT operation are described in [Appendix A.1](#convert-section-a.1)

<a name="convert-section-2.11"></a>
...
<a name="convert-section-2.11.1"></a>
...
Suppose an MCC on router "R0" determines that the PUSH operation is to
   ...
   through a physical or virtual interface such as a UDP tunnel [RFC7510](#RFC7510)
   or L2TPv3 tunnel [RFC4817](#RFC4817).

An example of such a Local SID is an Adj-SID allocated and advertised
   by IS-IS [RFC8667](#RFC8667). The method by
   ...
   failure using TI-LFA [FAST-REROUTE](#FAST-REROUTE).

As mentioned in [RFC8402](#RFC8402), a Local SID is specified by an MPLS label.
   Hence, the PUSH operation for a Local SID is identical to the label push
   operation using any MPLS label [RFC3031](#RFC3031). The forwarding action after
...
<a name="convert-section-2.11.2"></a>
...
A Local SID on router "R0" corresponds to a local label.
   ...
   MPLS label [RFC3031](#RFC3031).

<a name="convert-section-2.11.3"></a>
...
The  NEXT operation for Local SIDs is identical to the NEXT operation for
   global SIDs as specified in [Section 2.10.2](#convert-section-2.10.2).

<a name="convert-section-3"></a>
# IANA Considerations

This document has no IANA actions.

<a name="convert-section-4"></a>
...
This document describes the applicability of Segment Routing over the
   ...
   [RFC8402](#RFC8402) apply to the MPLS data plane when used with Segment
   Routing. SR Operations, Administration, and Maintenance (OAM) use cases for the MPLS data plane are defined in
   [RFC8403](#RFC8403).  SR OAM procedures for the MPLS data plane are defined in
   [RFC8287](#RFC8287).

<a name="convert-section-5"></a>
# Security Considerations

This document does not introduce additional security requirements and
   mechanisms other than the ones described in [RFC8402](#RFC8402).
...
<a name="RFC3443"></a> **[RFC3443]** P. Agarwal, B. Akyol, "Time To Live (TTL) Processing in Multi-Protocol Label Switching (MPLS) Networks", RFC 3443, DOI 10.17487/RFC3443, January 2003. <https://www.rfc-editor.org/info/rfc3443> [Local MD](rfc3443.md)
...
<a name="RFC7274"></a> **[RFC7274]** K. Kompella, L. Andersson, A. Farrel, "Allocating and Retiring Special-Purpose MPLS Labels", RFC 7274, DOI 10.17487/RFC7274, June 2014. <https://www.rfc-editor.org/info/rfc7274> [Local MD](rfc7274.md)
...
<a name="FAST-REROUTE"></a> **[FAST-REROUTE]** S Litkowski, A Bashandy, C Filsfils, B Decraene, P Francois, D Voyer, F Clad, P Camarillo, "Topology Independent Fast Reroute using Segment Routing", Internet-Draft draft-ietf-rtgwg-segment-routing-ti-lfa-01, March 2019. Work in Progress <https://tools.ietf.org/html/draft-ietf-rtgwg-segment-routing-ti-lfa-01>

<a name="RFC4817"></a> **[RFC4817]** M. Townsley, C. Pignataro, S. Wainner, T. Seely, J. Young, "Encapsulation of MPLS over Layer 2 Tunneling Protocol Version 3", RFC 4817, DOI 10.17487/RFC4817, March 2007. <https://www.rfc-editor.org/info/rfc4817> [Local MD](rfc4817.md)
...
<a name="RFC5331"></a> **[RFC5331]** R. Aggarwal, Y. Rekhter, E. Rosen, "MPLS Upstream Label Assignment and Context-Specific Label Space", RFC 5331, DOI 10.17487/RFC5331, August 2008. <https://www.rfc-editor.org/info/rfc5331> [Local MD](rfc5331.md)
...
<a name="RFC8403"></a> **[RFC8403]** R. Geib, C. Filsfils, C. Pignataro, N. Kumar, "A Scalable and Topology-Aware MPLS Data-Plane Monitoring System", RFC 8403, DOI 10.17487/RFC8403, July 2018. <https://www.rfc-editor.org/info/rfc8403> [Local MD](rfc8403.md)

<a name="RFC8661"></a> **[RFC8661]** A Bashandy, C Filsfils, S Previdi, B Decraene, S Litkowski, "Segment Routing MPLS Interworking with LDP", December 2019. <https://www.rfc-editor.org/info/rfC8661> [Local MD](rfc8661.md)

<a name="RFC8665"></a> **[RFC8665]** P Psenak, S Previdi, C Filsfils, H Gredler, R Shakir, W Henderickx, J Tantsura, "OSPF Extensions for Segment Routing", December 2019. <https://www.rfc-editor.org/info/rfc8665> [Local MD](rfc8665.md)

<a name="RFC8666"></a> **[RFC8666]** P Psenak, S Previdi, "OSPFv3 Extensions for Segment Routing", December 2019. <https://www.rfc-editor.org/info/rfc8666> [Local MD](rfc8666.md)

<a name="RFC8667"></a> **[RFC8667]** S Previdi, L Ginsberg, C Filsfils, A Bashandy, H Gredler, B Decraene, "IS-IS Extensions for Segment Routing", December 2019. <https://www.rfc-editor.org/info/rfc8667> [Local MD](rfc8667.md)

<a name="ROUTING-POLICY"></a> **[ROUTING-POLICY]** C Filsfils, S Sivabalan, D Voyer, A Bogdanov, P Mattes, "Segment Routing Policy Architecture", Internet-Draft draft-ietf-spring-segment-routing-policy-05, November 2019. Work in Progress <https://tools.ietf.org/html/draft-ietf-spring-segment-routing-policy-05>

<a name="convert-section-a"></a>
# Examples

<a name="convert-section-a.1"></a>
## IGP Segment Examples

Consider the network diagram of [Figure 1](#fig1) and the IP addresses and IGP
   segment allocations of [Figure 2](#fig2). Assume that the network is running
   IS-IS with SR extensions [RFC8667](#RFC8667),
...
R2 is the next hop along the shortest path towards R8. By applying
   the steps in [Section 2.8](#convert-section-2.8), the outgoing label downloaded to R1's FIB
   corresponding to the global SID index "8" is 1008 because the SRGB of
   R2 = [1000,5000] as shown in [Figure 2](#fig2).

Because the packet is IPv4, R1 applies the PUSH operation using the
   label value 1008 as specified in [Section 2.10.1](#convert-section-2.10.1). The resulting MPLS
   header will have the "S" bit [RFC3032](#RFC3032) set because it is followed
...
The packet arrives at router R2.
 ...
 Because top label 1008
   ...
   in [Section 2.10.1](#convert-section-2.10.1).
...
<a name="convert-section-a.2.1"></a>
### Example 1
...
<a name="convert-section-a.2.2"></a>
### Example 2
...
IS-IS on Node A assigns label 1006 to the globally significant
   ...
   sub-TLV as described in [RFC8667](#RFC8667)). Hence, the incoming label corresponding
...
<a name="convert-section-a.2.3"></a>
### Example 3
...
The operator on Node A configures IS-IS on Node A to assign label
   ...
   L-Flag is clear in the Adj-SID sub-TLV as described in [RFC8667](#RFC8667)).
...
<a name="convert-section-a.2.4"></a>
### Example 4
...
From the point of view of Node A, FEC1 and FEC2 both use dynamic SID
   ...
   [Section 2.5.1](#convert-section-2.5.1), the Binding SID has a higher administrative distance
   than the Prefix-SID; hence, FEC1 wins.

<a name="convert-section-a.2.5"></a>
### Example 5
...
IS-IS on Node A assigns label 1010 to the globally significant
   Adj-SID (i.e., when advertised, the L-Flag is clear in the Adj-SID
   sub-TLV as described in [RFC8667](#RFC8667)).
...
<a name="convert-section-a.2.6"></a>
### Example 6
...
<a name="convert-section-a.2.7"></a>
### Example 7
...
<a name="convert-section-a.2.8"></a>
### Example 8
...
<a name="convert-section-a.2.9"></a>
### Example 9
...
<a name="convert-section-a.2.10"></a>
### Example 10
...
<a name="convert-section-a.2.11"></a>
### Example 11
...
<a name="convert-section-a.2.12"></a>
### Example 12
...
<a name="convert-section-a.2.13"></a>
### Example 13
...
<a name="convert-section-a.2.14"></a>
### Example 14
...
This section presents examples to illustrate the effect of incoming
   ...
   [Section 2.6](#convert-section-2.6).

<a name="convert-section-a.3.1"></a>
### Example 1
...
The rule in [Section 2.6](#convert-section-2.6) means Node A **MUST NOT** install an ingress
   MPLS forwarding entry corresponding to FEC2 (the losing FEC, which
   would be for prefix 203.0.113.222/32).

<a name="convert-section-a.3.2"></a>
### Example 2
...
The rule described in [Section 2.6](#convert-section-2.6) means that Node A **MUST NOT** install
   a VPN route with label stack = <S3,S4,V1> (corresponding to FEC2.)

<a name="convert-section-7"></a>
# Acknowledgements
