...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-bess-srv6-services-15
...
- [prev](https://datatracker.ietf.org/doc/draft-ietf-bess-srv6-services-15)
- [alternate](https://dx.doi.org/10.17487/rfc9252)
...
- [1. Introduction](#INTRO)
  - [1.1. Requirements Language](#REQ)
- [2. SRv6 Services TLVs](#SIDTLV)
- [3. SRv6 Service Sub-TLVs](#SRv6-TLV)
  - [3.1. SRv6 SID Information Sub-TLV](#SRv6-SID-INFO)
  - [3.2. SRv6 Service Data Sub-Sub-TLVs](#SID-SERVICE-DATA-TLV)
    - [3.2.1. SRv6 SID Structure Sub-Sub-TLV](#SRv6-SID-STRUCTURE)
- [4. Encoding SRv6 SID Information](#SIDENCODE)
- [5. BGP-Based L3 Service over SRv6](#L3BGP)
  - [5.1. IPv4 VPN over SRv6 Core](#L3BGPVPNv4)
  - [5.2. IPv6 VPN over SRv6 Core](#L3BGPVPNv6)
  - [5.3. Global IPv4 over SRv6 Core](#L3BGPINTv4)
  - [5.4. Global IPv6 over SRv6 Core](#L3BGPINTv6)
- [6. BGP-Based Ethernet VPN (EVPN) over SRv6](#EVPNBGP)
  - [6.1. Ethernet Auto-Discovery Route over SRv6 Core](#RT1)
    - [6.1.1. Ethernet A-D per ES Route](#PERES)
    - [6.1.2. Ethernet A-D per EVI Route](#PEREVI)
  - [6.2. MAC/IP Advertisement Route over SRv6 Core](#RT2)
    - [6.2.1. MAC/IP Advertisement Route with MAC Only](#section-6.2.1)
    - [6.2.2. MAC/IP Advertisement Route with MAC+IP](#section-6.2.2)
  - [6.3. Inclusive Multicast Ethernet Tag Route over SRv6 Core](#RT3)
  - [6.4. Ethernet Segment Route over SRv6 Core](#RT4)
  - [6.5. IP Prefix Route over SRv6 Core](#RT5)
  - [6.6. EVPN Multicast Routes (Route Types 6, 7, and 8) over SRv6 Core](#RT678)
- [7. Error Handling](#ERROR)
- [8. IANA Considerations](#IANA)
  - [8.1. BGP Prefix-SID TLV Types Registry](#section-8.1)
  - [8.2. SRv6 Service Sub-TLV Types Registry](#section-8.2)
  - [8.3. SRv6 Service Data Sub-Sub-TLV Types Registry](#section-8.3)
  - [8.4. BGP SRv6 Service SID Flags Registry](#section-8.4)
  - [8.5. SAFI Values Registry](#section-8.5)
- [9. Security Considerations](#SEC)
  - [9.1. Considerations Related to BGP Sessions](#SECSESS)
  - [9.2. Considerations Related to BGP Services](#SECSVC)
  - [9.3. Considerations Related to SR over IPv6 Data Plane](#SECSRV6)
...
SRv6-based BGP services refer to the Layer 3 (L3) and Layer 2 (L2) overlay
      ...
      Private Networks (VPNs)" [RFC4364](#RFC4364) and
      "BGP MPLS-Based Ethernet VPN" [RFC7432](#RFC7432).

SRv6 SID refers to an SRv6 Segment Identifier, as defined in [RFC8402](#RFC8402).
...
SRv6 Service SID refers to an SRv6 SID associated with one of the
      ...
      L3VPN service, as defined in [RFC8986](#RFC8986). This document describes how existing
...
To provide SRv6 service with best-effort connectivity, the egress PE
      ...
      plain IPv6 forwarding [RFC8200](#RFC8200).
...
To provide SRv6 service in conjunction with an underlay Service Level Agreement (SLA) from the
      ...
      route with a Color Extended Community [RFC9012](#RFC9012) for steering flows
      for those routes, as specified in [SEGMENT-ROUTING-POLICY](#I-D.ietf-spring-segment-routing-policy). The ingress PE
      ...
      (SRH) [RFC8754](#RFC8754). The underlay nodes whose SRv6
      SIDs are part of the SRH segment list **MUST** support the SRv6 data
      plane.
...
This document extends the use of the BGP Prefix-SID attribute [RFC8669](#RFC8669) to carry SRv6 SIDs and their associated information
      with the BGP address families that are listed further in this
      section.

The SRv6 Service TLVs are defined as two new TLVs of the BGP
...
**SRv6 L3 Service TLV:**
  This TLV encodes Service SID information for
            ...
            service route, as defined in [RFC4364](#RFC4364), [RFC4659](#RFC4659), [RFC8950](#RFC8950), and [RFC9136](#RFC9136). Some SRv6 Endpoint Behaviors that may be
...
**SRv6 L2 Service TLV:**
  This TLV encodes Service SID information for
            ...
            Route Types for Layer 2 services, as defined in [RFC7432](#RFC7432). Some SRv6
            Endpoint Behaviors that may be encoded are, but not limited to,
            End.DX2, End.DX2V, End.DT2U, and End.DT2M.
...
When an egress PE is enabled for BGP Services over the SRv6 data plane,
      ...
      [RFC4760](#RFC4760), [RFC4659](#RFC4659), [RFC8950](#RFC8950), [RFC7432](#RFC7432), [RFC4364](#RFC4364), and 
        [RFC9136](#RFC9136), where applicable, as described in Sections [5](#L3BGP) and [6](#EVPNBGP).

The support for BGP Multicast VPN (MVPN) Services [RFC6513](#RFC6513) with SRv6 is outside the scope of this document.
...
```
**TLV Type (1 octet):**
  This field is assigned a value from IANA's
            "BGP Prefix-SID TLV Types" subregistry. It is set to 5 for the SRv6 L3
            Service TLV. It is set to 6 for the SRv6 L2 Service TLV.

**TLV Length (2 octets):**
  This field specifies the total length, in octets, of
            the TLV Value.
...
- If the BGP next hop is unchanged during the advertisement, the SRv6
          ...
          Sub-Sub-TLV, **SHOULD** be propagated further. In addition, all Reserved
          fields in the TLV, Sub-TLV, or Sub-Sub-TLV **MUST** be propagated
          unchanged.
- If the BGP next hop is changed, the TLVs, Sub-TLVs, and Sub-Sub-TLVs
          **SHOULD** be updated with the locally allocated SRv6 SID information.
          Any received Sub-TLVs and Sub-Sub-TLVs that are unrecognized **MUST** be
          removed.
...
**SRv6 Service Sub-TLV Type (1 octet):**
  This field identifies the type of SRv6
...
**SRv6 Service Sub-TLV Value (variable):**
  This field contains data specific to
            ...
            Service Data Sub-Sub-TLVs whose format is described in [Section 3.2](#SID-SERVICE-DATA-TLV) below.


<a name="SRv6-SID-INFO"></a>
...
```
**SRv6 Service Sub-TLV Type (1 octet):**
  This field is set to 1 to
              represent the SRv6 SID Information Sub-TLV.
...
**RESERVED1 (1 octet):**
  This field **MUST** be set to 0 by the sender and ignored
              by the receiver.
...
**SRv6 Service SID Flags (1 octet):**
  This field encodes SRv6 Service SID
              Flags -- none are currently defined. It **MUST** be set to 0 by the
              sender and any unknown flags **MUST** be ignored by the receiver.

**SRv6 Endpoint Behavior (2 octets):**
  This field encodes the SRv6 Endpoint
              ...
              introduced by [RFC8986](#RFC8986). The opaque SRv6 Endpoint
              Behavior (i.e., value 0xFFFF) **MAY** be used when the advertising
...
The choice of SRv6 Endpoint Behavior of the SRv6 SID is entirely up
        to the originator of the advertisement. While Sections [5](#L3BGP) and [6](#EVPNBGP) list the
	SRv6 Endpoint Behaviors that are
        ...
        unrecognized SRv6 Endpoint Behavior **MUST NOT** be considered invalid by the
        receiver, except for behaviors that involve the use of arguments (refer
        to [Section 3.2.1](#SRv6-SID-STRUCTURE) for details on argument
        validation). An implementation **MAY** log a rate-limited warning when it
        receives an unexpected behavior.

When multiple SRv6 SID Information Sub-TLVs are present, the
        ingress PE **SHOULD** use the SRv6 SID from the first instance of the
        Sub-TLV. An implementation **MAY** provide a local policy to override this
        selection.

<a name="SID-SERVICE-DATA-TLV"></a>
## SRv6 Service Data Sub-Sub-TLVs
...
**SRv6 Service Data Sub-Sub-TLV Type (1 octet):**
  This field identifies the
              ...
              "SRv6 Service Data Sub-Sub-TLV Types" subregistry.
...
SRv6 Service Data Sub-Sub-TLV Type 1 is assigned for the SRv6 SID
          ...
          defined in [RFC8986](#RFC8986). The terms Locator Block and
          Locator Node correspond to the B and N parts, respectively, of the
          SRv6 Locator that is defined in [RFC8986](#RFC8986). It is
	  carried as Sub-Sub-TLV in the SRv6 SID Information Sub-TLV.
...
Transposition Offset indicates the bit position, and Transposition
          ...
          bits that have been shifted out **MUST** be set to 0 in the SID
          value.

A Transposition Length of 0 indicates nothing is transposed and
          that the entire SRv6 SID value is encoded in the SID Information
          Sub-TLV. In this case, the Transposition Offset **MUST** be set to
          0.

The size of the MPLS Label field limits the bits transposed from
          the SRv6 SID value into it. For example, the size of the MPLS Label field is 20 bits in
          [RFC4364](#RFC4364) and [RFC8277](#RFC8277), and the size is 24 bits in [RFC7432](#RFC7432).

As defined in [RFC8986](#RFC8986), the sum of the Locator
          Block Length (LBL), Locator Node Length (LNL), Function Length (FL),
          and Argument Length (AL) fields **MUST** be less than or equal to 128
...
As an example, consider that the sum of the Locator Block and the
          ...
          the transposition length is set to 20.

BGP speakers that do not support this specification may
          ...
          supporting this specification **MUST** provide a mechanism to control
...
Arguments may be generally applicable for SIDs of only specific
          ...
          **MUST** be set to 0 for SIDs where the Argument is not
          ...
          hence **MUST** ignore SRv6 SIDs with arguments (indicated by a non-zero
          AL) with unknown SRv6 Endpoint Behaviors. For SIDs
          corresponding to an SRv6 Endpoint Behavior that is known, a receiver **MUST**
          validate that the consistency of the AL with the
          specific SRv6 Endpoint Behavior definition.
...
For certain types of BGP Services, like L3VPN where a per-VRF SID
      ...
      have its own unique SID, thereby resulting in inefficient packing.
...
To achieve efficient packing, this document allows either 1) the
      ...
      Scheme is **RECOMMENDED** for the specific service encodings that allow it,
      as described further in Sections [5](#L3BGP) and [6](#EVPNBGP).

As an example, for the EVPN VPWS service prefix described further in
      [Section 6.1.2](#PEREVI), the Function part of the SRv6 SID is encoded in
      ...
      are being taken out of the SID and put into the label field.

In yet another example, for the EVPN Ethernet Auto-Discovery (A-D) per Ethernet
      Segment (ES) route described further in [Section 6.1.1](#PERES), only the
      Argument of the SID needs to be signaled. This Argument part of the SRv6
      SID **MAY** be transposed in the Ethernet Segment Identifier (ESI) Label
...
BGP egress nodes (egress PEs) advertise a set of reachable prefixes.
      Standard BGP update propagation schemes [RFC4271](#RFC4271), which
      may make use of route reflectors [RFC4456](#RFC4456), are used to
...
Egress PEs that support SRv6-based L3 services advertise overlay
      ...
      ingress PE receiving this route **MUST** perform IPv6 encapsulation and
      insert an SRH [RFC8754](#RFC8754) when required; second, it
...
Thus, the Service SID signaled only has local significance at the
      ...
      defined in [RFC8986](#RFC8986).

The SRv6 Service SID **SHOULD** be routable (refer to [RFC8986](#RFC8986)) within the Autonomous System (AS) of the egress PE and serves the dual
...
When steering for SRv6 services is based on shortest path forwarding
      (e.g., best effort or IGP Flexible Algorithm [IGP-FLEX-ALGO](#I-D.ietf-lsr-flex-algo)) to the egress PE, the ingress PE
      encapsulates the IPv4 or IPv6 customer packet in an outer IPv6 header
      (using H.Encaps or H.Encaps.Red flavors specified in [RFC8986](#RFC8986)), where the destination address is the SRv6 Service
      ...
      resolvability is evaluated as per [RFC4271](#RFC4271). If the SRv6
...
For service over SRv6 core, the egress PE sets the BGP next hop to one of
      its IPv6 addresses. Such an address **MAY** be covered by the SRv6 Locator
...
When the BGP route received at an ingress PE is colored with a
      Color Extended Community and a valid SRv6 Policy is available, the
      steering for service flows is performed as described in [SEGMENT-ROUTING-POLICY](#I-D.ietf-spring-segment-routing-policy). When the
      ...
      the egress PE) in the SR Policy segment list, it **MAY** exclude that last
      ...
      S3> would be <S1, S2, S3-Service-SID>.

<a name="L3BGPVPNv4"></a>
## IPv4 VPN over SRv6 Core

The MP_REACH_NLRI over SRv6 core is encoded according to IPv4 VPN
   unicast over IPv6 core defined in [RFC8950](#RFC8950).

The label field of IPv4-VPN NLRI is encoded as specified in [RFC8277](#RFC8277) with the 20-bit Label Value set to the whole or a
        portion of the Function part of the SRv6 SID when the Transposition
        Scheme of encoding ([Section 4](#SIDENCODE)) is used; otherwise,
...
The MP_REACH_NLRI over SRv6 core is encoded according to IPv6 VPN
        over IPv6 core, as defined in [RFC4659](#RFC4659).

The label field of the IPv6-VPN NLRI is encoded as specified in [RFC8277](#RFC8277) with the 20-bit Label Value set to the whole or a
        portion of the Function part of the SRv6 SID when the Transposition
        Scheme of encoding ([Section 4](#SIDENCODE)) is used; otherwise,
        it is set to Implicit NULL. When using the Transposition Scheme, the
        Transposition Length **MUST** be less than or equal to 20 and less than or
...
The SRv6 Service SID is encoded as part of the SRv6 L3 Service TLV. The
        SRv6 Endpoint Behavior **SHOULD** be one of these: End.DX6, End.DT6, or
        End.DT46.
...
The MP_REACH_NLRI over SRv6 core is encoded according to IPv4 over
        IPv6 core, as defined in [RFC8950](#RFC8950).
...
[RFC7432](#RFC7432) provides an extendable method of building an
      EVPN overlay. It primarily focuses on MPLS-based EVPNs,
      and [RFC8365](#RFC8365) extends to IP-based EVPN overlays. [RFC7432](#RFC7432) defines Route Types 1, 2, and 3, which carry prefixes
      ...
      [RFC9136](#RFC9136). Route Types 6, 7, and 8 are defined in [RFC9251](#RFC9251).
...
To support SRv6-based EVPN overlays, one or more SRv6 Service SIDs
      ...
      overlay, and the BGP ingress device receiving this route **MUST** perform
      IPv6 encapsulation and insert an SRH [RFC8754](#RFC8754) when
      required; second, it indicates the value of the Service SID(s) to be
      used in the encapsulation.

The SRv6 Service SID **SHOULD** be routable (refer to [RFC8986](#RFC8986)) within the AS of the egress PE and serves the dual
...
When steering for SRv6 services is based on shortest path forwarding
      ...
      header (using H.Encaps.L2 or H.Encaps.L2.Red flavors specified in [RFC8986](#RFC8986)) where the destination address is the SRv6 Service
      SID associated with the related BGP route update. Therefore, the ingress
      PE **MUST** perform a resolvability check for the SRv6 Service SID before
      considering the received prefix for the BGP best path computation. The
      resolvability is evaluated as per [RFC4271](#RFC4271). If the SRv6
...
For service over SRv6 core, the egress PE sets the BGP next hop to one
      of its IPv6 addresses. Such an address **MAY** be covered by
      the SRv6 Locator
      from which the SRv6 Service SID is allocated. The BGP next hop is used for
      tracking the reachability of the egress PE based on existing BGP
      procedures.
...
Ethernet A-D routes are Route Type 1, as defined in
        [RFC7432](#RFC7432), and may be used to achieve
	...
	cross-connect, mainly to advertise point-to-point service IDs.
...
The 24-bit ESI Label field of the ESI Label extended community
          ...
          Scheme of encoding ([Section 4](#SIDENCODE));
	  otherwise, it is set to Implicit NULL in the higher-order 20 bits (i.e., as 0x000030). In either case, the value is set in the 24 bits. When
          using the Transposition Scheme, the Transposition Length **MUST** be
...
A Service SID enclosed in an SRv6 L2 Service TLV within the BGP
          ...
          Argument for applicable End.DT2M behavior [RFC8986](#RFC8986).
          When the local-bias approach [RFC8365](#RFC8365) is used, the
          Service SID **MAY** be of value 0.

<a name="PEREVI"></a>
### Ethernet A-D per EVI Route

Ethernet A-D per EVPN Instance (EVI) route NLRI encoding over SRv6 core is
          similar to what is described in [RFC7432](#RFC7432) and [RFC8214](#RFC8214)
          with the following change:

**MPLS Label:**
  The 24-bit field carries the whole or a portion of
                the Function part of the SRv6 SID when the Transposition Scheme
                of encoding ([Section 4](#SIDENCODE)) is used;
  	      otherwise,  it is set to Implicit NULL in the higher-order 20 bits (i.e., as 0x000030). In either case, the value is set in the 24 bits. When using the Transposition Scheme, the
                Transposition Length **MUST** be less than or equal to 24 and less
...
EVPN Route Type 2 is used to advertise unicast traffic Media Access Control (MAC)
...
```
NLRI encoding over SRv6 core is similar to what is described in [RFC7432](#RFC7432) with the following changes:

**MPLS Label1:**
  This is associated with the SRv6 L2 Service TLV. This
              24-bit field carries the whole or a portion of the Function part
              of the SRv6 SID when the Transposition Scheme of encoding ([Section 4](#SIDENCODE)) is used; otherwise, it is set to Implicit NULL in the higher-order 20 bits (i.e., as 0x000030). In either case, the value is set in the 24 bits. When using the
              Transposition Scheme, the Transposition Length **MUST** be less than
              or equal to 24 and less than or equal to the FL.

**MPLS Label2:**
  This is associated with the SRv6 L3 Service TLV. This
              24-bit field carries the whole or a portion of the Function part
              of the SRv6 SID when the Transposition Scheme of encoding ([Section 4](#SIDENCODE)) is used; otherwise, it is set to Implicit NULL in the higher-order 20 bits (i.e., as 0x000030). In either case, the value is set in the 24 bits. When using the
              Transposition Scheme, the Transposition Length **MUST** be less than
              or equal to 24 and less than or equal to the FL.
...
Service SIDs enclosed in the SRv6 L2 Service TLV and optionally in the SRv6
...
**MPLS Label1:**
  This is associated with the SRv6 L2 Service TLV. This
                24-bit field carries the whole or a portion of the Function part
                of the SRv6 SID when the Transposition Scheme of encoding ([Section 4](#SIDENCODE)) is used; otherwise, it is set to Implicit NULL in the higher-order 20 bits (i.e., as 0x000030). In either case, the value is set in the 24 bits. When
                using the Transposition Scheme, the Transposition Length **MUST** be
                less than or equal to 24 and less than or equal to the FL.
...
An L2 Service SID enclosed in an SRv6 L2 Service TLV within the
          ...
          within the BGP Prefix-SID attribute **MAY** also be advertised along
          with the route. The SRv6 Endpoint Behavior **SHOULD** be one of these:
...
EVPN Route Type 3 is used to advertise multicast traffic
...
```
NLRI encoding over SRv6 core is similar to what is described in [RFC7432](#RFC7432).

The P-Multicast Service Interface (PMSI) Tunnel Attribute [RFC6514](#RFC6514) is used to identify
...
```
**Flag:**
  This field has a value of 0, as defined per [RFC7432](#RFC7432).

**Tunnel Type:**
  This field is defined per [RFC6514](#RFC6514).

**MPLS label:**
  This 24-bit field carries the whole or a portion of
              the Function part of the SRv6 SID when ingress replication is used
              and the Transposition Scheme of encoding ([Section 4](#SIDENCODE)) is used; otherwise, it is set as defined
              in [RFC6514](#RFC6514). When using the Transposition Scheme,
...
A Service SID enclosed in an SRv6 L2 Service TLV within the
        BGP Prefix-SID attribute is advertised along with the route. The SRv6
        Endpoint Behavior **SHOULD** be End.DT2M.

- When ESI-based filtering is used for multihoming or Ethernet Tree (E-Tree)
            procedures, the ESI Filtering Argument (the Arg.FE2 notation
            introduced in [RFC8986](#RFC8986)) of the Service SID carried
            along with EVPN Route Type 1 **SHOULD** be merged with the
            ...
            mechanisms for multihoming are described in [RFC7432](#RFC7432). Details of filtering mechanisms for
            Leaf-originated BUM traffic in EVPN E-Tree services are provided
            in [RFC8317](#RFC8317).
- When "local-bias" is used as the multihoming
            split-horizon method, the ESI Filtering Argument **SHOULD NOT** be
...
As a reminder, an Ethernet Segment route (i.e., EVPN Route Type 4)
...
SRv6 Service TLVs within the BGP Prefix-SID attribute are not
        ...
        changed -- it remains as described in [RFC7432](#RFC7432).
...
```
NLRI encoding over SRv6 core is similar to what is described in [RFC9136](#RFC9136)
        with the following change:

**MPLS Label:**
  This 24-bit field carries the whole or a portion of
              the Function part of the SRv6 SID when the Transposition Scheme of
              encoding ([Section 4](#SIDENCODE)) is used;
  	    otherwise, it is set to Implicit NULL in the higher-order 20 bits (i.e., as 0x000030). In either case, the value is set in the 24 bits.
              When using the Transposition Scheme, the Transposition Length **MUST**
...
If multiple instances of the SRv6 L3 Service TLV are encountered, all but
      the first instance **MUST** be ignored.

If multiple instances of the SRv6 L2 Service TLV are encountered, all but
      the first instance **MUST** be ignored.
...
The SRv6 overlay service requires the Service SID for forwarding. The
      treat-as-withdraw action [RFC7606](#RFC7606) **MUST** be performed when
      at least one malformed SRv6 Service TLV is present in the BGP Prefix-SID
      attribute.

The SRv6 SID value in the SRv6 SID Information Sub-TLV is invalid when the SID
      ...
      the Sub-Sub-TLV, as specified in [Section 3.2.1](#SRv6-SID-STRUCTURE), is
      not met. The transposition offset and length **MUST** be 0 when the
      Sub-Sub-TLV is advertised along with routes where the Transposition Scheme
      is not applicable (e.g., for global IPv6 service [RFC2545](#RFC2545) where there is no label field). The path having any such
...
This document introduces two new TLV Types of the BGP Prefix-SID
        ...
        Prefix-SID TLV Types" subregistry as follows:

**Table: BGP Prefix-SID TLV Types Subregistry**
...
Value 4 previously corresponded to the SRv6-VPN SID TLV, which
        ...
        by the SRv6 L3 Service and SRv6 L2 Service TLVs.
...
IANA has created and now maintains a new subregistry called
        ...
        Parameters" registry. The registration procedures, per [RFC8126](#RFC8126), for this subregistry are according to [Table 2](#IANASRV6SVCTYPESAP).

**Table: SRv6 Service Sub-TLV Types Subregistry Registration Procedures**
...
IANA has populated this subregistry as follows. Note that the SRv6 SID Information Sub-TLV
	is defined in this document:

**Table: SRv6 Service Sub-TLV Types Subregistry Initial Contents**
...
IANA has created and now maintains a new subregistry called
        ...
        subregistry are according to [Table 4](#IANASRV6DATASSTYPESAP).

**Table: SRv6 Service Data Sub-Sub-TLV Types Subregistry Registration Procedures**
...
**Table: SRv6 Service Data Sub-Sub-TLV Types Subregistry Initial Contents**
...
IANA has created and now maintains a new subregistry called "BGP
...
Techniques related to authentication of BGP sessions for securing
        messages between BGP peers, as discussed in the BGP specification [RFC4271](#RFC4271) and in the security analysis for BGP [RFC4272](#RFC4272), apply. The discussion of the use of the TCP
        Authentication Option to protect BGP sessions is found in [RFC5925](#RFC5925), while [RFC6952](#RFC6952) includes an
        analysis of BGP keying and authentication issues. This document does
        not introduce any additional BGP session security considerations.
...
This document does not introduce new services or BGP NLRI types but
        ...
        security considerations for the respective BGP services, such as [BGP IPv4 over IPv6 NH](#RFC8950), [BGP IPv6 L3VPN](#RFC4659), [BGP
        IPv6](#RFC2545), [BGP EVPN](#RFC7432), and [IP EVPN](#RFC9136), apply as discussed in their respective
        documents.
	[RFC8669](#RFC8669) discusses mechanisms to prevent
...
As a reminder, several of the BGP services (i.e., the AFI/SAFI used
        ...
        mechanism and later extended for others, e.g., EVPN MPLS [RFC7432](#RFC7432) was extended for Virtual eXtensible Local Area Network (VXLAN) encapsulation and Network Virtualization Using Generic Routing Encapsulation (NVGRE) [RFC8365](#RFC8365). [RFC9012](#RFC9012) enables the use of
        ...
        discussed in [RFC8986](#RFC8986)) to external peers (or into the
        Internet) also apply in the case of SRv6.

Specific to SRv6, a misconfiguration or error in the BGP
...
The security considerations of the SR architecture are
        covered by [RFC8402](#RFC8402). More detailed security
        considerations, specifically of SRv6 and SRH, are covered by [RFC8754](#RFC8754) as they relate to SR Attacks (Section [RFC8754](#RFC8754)), Service
        Theft (Section [RFC8754](#RFC8754)), and Topology Disclosure (Section [RFC8754](#RFC8754)).


	As such, an
        operator deploying SRv6 **MUST** follow the considerations described in
        [RFC8754](#RFC8754) to implement the infrastructure
        Access Control Lists (ACLs) and the recommendations described in [BCP 38](#RFC2827) and [BCP 84](#RFC3704).

The SRv6 deployment and SID allocation guidelines, as described in
        [RFC8986](#RFC8986), simplify the deployment of the ACL filters
	...
	Code (HMAC) TLV [RFC8754](#RFC8754), as described in [RFC8986](#RFC8986), for validation.

The SRv6 Endpoint Behaviors implementing the services signaled
   in this document are defined in [RFC8986](#RFC8986); hence, the security
...
<a name="RFC7432"></a> **[RFC7432]** A. Sajassi, R. Aggarwal, N. Bitar, A. Isaac, J. Uttaro, J. Drake, W. Henderickx, "BGP MPLS-Based Ethernet VPN", RFC 7432, DOI 10.17487/RFC7432, February 2015. <https://www.rfc-editor.org/info/rfc7432> [Local MD](rfc7432.md)
...
<a name="RFC8214"></a> **[RFC8214]** S. Boutros, A. Sajassi, S. Salam, J. Drake, J. Rabadan, "Virtual Private Wire Service Support in Ethernet VPN", RFC 8214, DOI 10.17487/RFC8214, August 2017. <https://www.rfc-editor.org/info/rfc8214> [Local MD](rfc8214.md)
...
<a name="RFC8317"></a> **[RFC8317]** A. Sajassi, S. Salam, J. Drake, J. Uttaro, S. Boutros, J. Rabadan, "Ethernet-Tree (E-Tree) Support in Ethernet VPN (EVPN) and Provider Backbone Bridging EVPN (PBB-EVPN)", RFC 8317, DOI 10.17487/RFC8317, January 2018. <https://www.rfc-editor.org/info/rfc8317> [Local MD](rfc8317.md)
...
<a name="RFC8950"></a> **[RFC8950]** S. Litkowski, S. Agrawal, K. Ananthamurthy, K. Patel, "Advertising IPv4 Network Layer Reachability Information (NLRI) with an IPv6 Next Hop", RFC 8950, DOI 10.17487/RFC8950, November 2020. <https://www.rfc-editor.org/info/rfc8950> [Local MD](rfc8950.md)
...
<a name="RFC9136"></a> **[RFC9136]** J. Rabadan, W. Henderickx, J. Drake, W. Lin, A. Sajassi, "IP Prefix Advertisement in Ethernet VPN (EVPN)", RFC 9136, DOI 10.17487/RFC9136, October 2021. <https://www.rfc-editor.org/info/rfc9136> [Local MD](rfc9136.md)

<a name="RFC9251"></a> **[RFC9251]** A. Sajassi, S. Thoria, M. Mishra, K. Patel, J. Drake, W. Lin, "Internet Group Management Protocol (IGMP) and Multicast Listener Discovery (MLD) Proxies for Ethernet VPN (EVPN)", RFC 9251, DOI 10.17487/RFC9251, June 2022. <https://www.rfc-editor.org/info/rfc9251> [Local MD](rfc9251.md)
...
<a name="RFC2827"></a> **[RFC2827]** P. Ferguson, D. Senie, "Network Ingress Filtering: Defeating Denial of Service Attacks which employ IP Source Address Spoofing", BCP 38, RFC 2827, DOI 10.17487/RFC2827, May 2000. <https://www.rfc-editor.org/info/rfc2827> [Local MD](rfc2827.md)

<a name="RFC3704"></a> **[RFC3704]** F. Baker, P. Savola, "Ingress Filtering for Multihomed Networks", BCP 84, RFC 3704, DOI 10.17487/RFC3704, March 2004. <https://www.rfc-editor.org/info/rfc3704> [Local MD](rfc3704.md)
...
<a name="RFC6513"></a> **[RFC6513]** E. Rosen, R. Aggarwal, "Multicast in MPLS/BGP IP VPNs", RFC 6513, DOI 10.17487/RFC6513, February 2012. <https://www.rfc-editor.org/info/rfc6513> [Local MD](rfc6513.md)
...
<a name="I-D.ietf-spring-segment-routing-policy"></a> **[SEGMENT-ROUTING-POLICY]** C. Filsfils, K. Talaulikar, D. Voyer, A. Bogdanov, P. Mattes, "Segment Routing Policy Architecture", Internet-Draft draft-ietf-spring-segment-routing-policy-22, March 2022. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-spring-segment-routing-policy-22>

<a name="ACK"></a>
# Acknowledgements

The authors of this document would like to thank Stephane       Litkowski, Rishabh Parekh, Xiejingrong,
      Rajesh M., Mustapha Aissaoui,
      Alexander Vainshtein, Eduard Metz,
      Shraddha Hegde, Eduard Vasilenko,
      Ron Bonica, and Joel Halpern
      for their comments and review of this document. The
      authors would also like to thank Document Shepherd Matthew Bocci for his review and AD Martin Vigoureux for his review that
...
- Address: 940 Stewart Dr., Sunnyvale, CA, 94085, United States of America
- Email: robert@raszuk.net
