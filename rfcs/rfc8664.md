...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-pce-segment-routing-16
...
**Keywords:** SR, Traffic-Engineering, PCE

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-pce-segment-routing-16)
- [alternate](https://dx.doi.org/10.17487/rfc8664)
...
- [3. Overview of PCEP Operation in SR Networks](#Operation-Overview)
- [4. Object Formats](#object-formats)
  - [4.1. The OPEN Object](#open-object-fmt)
    - [4.1.1. The Path Setup Type Capability TLV](#pst-cap-tlv)
    - [4.1.2. The SR PCE Capability Sub-TLV](#cap-negotiation)
  - [4.2. The RP/SRP Object](#rp-object-fmt)
  - [4.3. ERO](#SR-ERO)
    - [4.3.1. SR-ERO Subobject](#SR-ERO-SUB)
    - [4.3.2. NAI Associated with SID](#SR-ERO-NODAL-32)
  - [4.4. RRO](#SR-RRO)
  - [4.5. METRIC Object](#SR-METRIC)
- [5. Procedures](#procedures)
  - [5.1. Exchanging the SR PCE Capability](#SR-CAP-PROCESS)
  - [5.2. ERO Processing](#SR-ERO-PROCESS)
    - [5.2.1. SR-ERO Validation](#SR-ERO-VALIDATION)
    - [5.2.2. Interpreting the SR-ERO](#SR-ERO-INTERPRET)
  - [5.3. RRO Processing](#SR-RRO-PROCESS)
- [6. Management Considerations](#Management)
  - [6.1. Controlling the Path Setup Type](#control)
  - [6.2. Migrating a Network to Use PCEP Segment-Routed Paths](#migrating)
  - [6.3. Verification of Network Operation](#verification)
  - [6.4. Relationship to Existing Management Models](#models)
- [7. Security Considerations](#Security)
- [8. IANA Considerations](#IANA)
  - [8.1. PCEP ERO and RRO Subobjects](#PCEP-Object-Codepoints)
  - [8.2. New NAI Type Registry](#NAI-Type-Registry)
  - [8.3. New SR-ERO Flag Registry](#IANA-SR-ERO-FLAG)
  - [8.4. PCEP-Error Object](#IANA-Error-Object)
  - [8.5. PCEP TLV Type Indicators](#IANA-TLV-Type-Indicators)
  - [8.6. PATH-SETUP-TYPE-CAPABILITY Sub-TLV Type Indicators](#IANA-subTLV-Type-Indicators)
  - [8.7. New Path Setup Type](#IANA-PATH-SETUP-TYPE)
  - [8.8. New Metric Type](#IANA-METRIC-TYPE)
  - [8.9. SR PCE Capability Flags](#IANA-SR-PCE-CAP-FLAG)
- [9. References](#section-9)
  - [9.1. Normative References](#section-9.1)
  - [9.2. Informative References](#section-9.2)
- [Appendix A. Compatibility with Early Implementations](#Early)
...
it. Both node segments and adjacency segments can be used for SR.

[RFC8402](#RFC8402) describes the SR architecture.  The
corresponding IS-IS and OSPF extensions are specified in [RFC8667](#RFC8667) and [RFC8665](#RFC8665), respectively.

The SR architecture can be implemented using either an MPLS
forwarding plane [RFC8660](#RFC8660) or an IPv6 forwarding plane
[IPv6-SRH](#I-D.ietf-6man-segment-routing-header).  The MPLS forwarding plane can be applied
...
Segment Identifier, respectively.
...
optimization criteria. [RFC8231](#RFC8231) specifies extensions
to PCEP that allow a stateful PCE to compute and recommend network
paths in compliance with [RFC4657](#RFC4657).  It also defines objects
...
A mechanism to dynamically initiate LSPs on a PCC based on the requests from a stateful PCE or a controller using stateful PCE is specified in [RFC8281](#RFC8281). This mechanism is useful in Software-Defined Networking (SDN) applications, such as on-demand engineering or bandwidth calendaring [RFC8413](#RFC8413).

It is possible to use a stateful PCE for computing one or more SR-TE paths, taking into account various constraints and objective functions. Once a path is chosen, the stateful PCE can initiate an SR-TE path on a PCC using the PCEP extensions specified in [RFC8281](#RFC8281) and the SR-specific PCEP extensions specified in this document. Additionally, using procedures described in this document, a PCC can request an SR path from either a stateful or a stateless PCE.

This specification relies on the procedures specified in [RFC8408](#RFC8408) to exchange the Segment Routing capability and to specify that the path setup type of an LSP is Segment Routing.  This specification also updates [RFC8408](#RFC8408) to clarify the use of sub-TLVs in the PATH-SETUP-TYPE-CAPABILITY TLV.  See [Section 4.1.1](#pst-cap-tlv) for details.

This specification provides a mechanism for a network controller (acting as a PCE) to instantiate candidate paths for an SR Policy onto a head-end node (acting as a PCC) using PCEP.  For more information on the SR Policy Architecture, see [SR-POLICY](#I-D.ietf-spring-segment-routing-policy).
...
**MSD:**
  Base MPLS Imposition Maximum SID Depth, as defined in [RFC8491](#RFC8491)
...
In an SR network, the ingress node of an SR path prepends an SR header to all outgoing packets.  The SR header consists of a list of SIDs (or MPLS labels in the context of this document).    The header has all
...
The PCC converts these into an MPLS label stack and next hop, as described in [Section 5.2.2](#SR-ERO-INTERPRET).

This document defines a new ERO subobject denoted by "SR-ERO subobject" that is capable of carrying a SID as well as the identity of the node/adjacency represented by the SID. SR-capable PCEP speakers should be able to generate and/or process such an ERO subobject. 
An ERO containing SR-ERO subobjects can be included in the PCEP Path Computation Reply (PCRep) message defined in [RFC5440](#RFC5440), the Path Computation LSP Initiate Request (PCInitiate) message defined in [RFC8281](#RFC8281), and the Path Computation Update Request (PCUpd) and Path Computation State Report (PCRpt) messages for LSPs defined in [RFC8231](#RFC8231).
...
delegated the LSP to it [RFC8231](#RFC8231).  A PCC can update an
...
path is set up and the old path is torn down, per [RFC8231](#RFC8231).
...
A PCC **MAY** include a Record Route Object (RRO) containing the recorded LSP in PCReq and PCRpt messages as specified in [RFC5440](#RFC5440) and [RFC8231](#RFC8231), respectively. This document defines a new RRO subobject for SR networks. The methods used by a PCC to record the SR-TE LSP are outside the scope of this document.
...
- Defines a new path setup type to be used in the PATH-SETUP-TYPE and PATH-SETUP-TYPE-CAPABILITY TLVs [RFC8408](#RFC8408).
...
according to [RFC5440](#RFC5440), [RFC8231](#RFC8231), [RFC8281](#RFC8281), and any other applicable PCEP specifications.

<a name="object-formats"></a>
# Object Formats

<a name="open-object-fmt"></a>
## The OPEN Object

<a name="pst-cap-tlv"></a>
### The Path Setup Type Capability TLV

[RFC8408](#RFC8408) defines the PATH-SETUP-TYPE-CAPABILITY TLV
...
This specification updates [RFC8408](#RFC8408) as follows.  It
creates a new registry that defines the valid type indicators of the
sub-TLVs of the PATH-SETUP-TYPE-CAPABILITY TLV (see [Section 8.6](#IANA-subTLV-Type-Indicators)).  A PCEP speaker **MUST NOT**
...
recognized by the speaker, then the speaker **MUST** ignore the
sub-TLV.

<a name="cap-negotiation"></a>
...
**Maximum SID Depth (MSD):**
  specifies the maximum number of SIDs (MPLS label stack depth in the context of this document) that a PCC is capable of imposing on a packet.  [Section 5.1](#SR-CAP-PROCESS) explains the relationship between this field and the X-Flag.


<a name="rp-object-fmt"></a>
## The RP/SRP Object

To set up an SR-TE LSP using SR, the Request Parameter (RP) or Stateful PCE Request Parameter (SRP) object **MUST** include the PATH-SETUP-TYPE TLV, specified in [RFC8408](#RFC8408), with the PST set to 1 (and path setup using SR-TE).
...
**The L-Flag:**
  Indicates whether the subobject represents a loose hop in the LSP [RFC3209](#RFC3209). If this flag is set to zero, a PCC **MUST NOT** overwrite the SID value present in the SR-ERO subobject. Otherwise, a PCC **MAY** expand or replace one or more SID values in the received SR-ERO based on its local policy.
...
**NAI Type (NT):**
  Indicates the type and format of the NAI contained in
  	    the object body, if any is present. If the F bit is set to
  	    zero (see below), then the NT field has no meaning and
  	    **MUST** be ignored by the receiver.  This
  	    document describes the following NT values:
...
**Flags:**
  ...
  **M:**
    If this bit is set to 1, the SID value represents an MPLS label stack entry as specified in [RFC3032](#RFC3032). Otherwise, the SID value is an administratively configured value that represents an index into an MPLS label space (either SRGB or SRLB) per [RFC8402](#RFC8402).
...
**SID:**
  ...
  - A 4-octet index defining the offset into an MPLS label space per [RFC8402](#RFC8402) or
  - A 4-octet MPLS label stack entry, where the 20 most significant bits encode the label value per [RFC3032](#RFC3032).
**NAI:**
...
A PCC reports an SR-TE LSP to a PCE by sending a PCRpt message, per [RFC8231](#RFC8231).  The RRO on this message represents the SID list that was applied by the PCC, that is, the actual path taken by the LSP.  The procedures of [RFC8231](#RFC8231) with respect to the RRO apply equally to this specification without change.
...
A PCC **MAY** request that PCE optimizes an individual path computation request to minimize the SID depth of the computed path by using the METRIC object defined in [RFC5440](#RFC5440).  This document defines a new type for the METRIC object to be used for this purpose, as follows:

- T = 11:
              Maximum SID Depth of the requested path.
...
If a PCC sets the N-Flag to 1, then the PCE **MAY** send an SR-ERO subobject containing an NAI and no SID (see [Section 5.2](#SR-ERO-PROCESS)).  Otherwise, the PCE **MUST NOT** send an SR-ERO subobject containing an NAI and no SID.
...
<a name="SR-ERO-PROCESS"></a>
## ERO Processing

<a name="SR-ERO-VALIDATION"></a>
### SR-ERO Validation

If a PCC does not support the SR PCE Capability and thus cannot recognize the SR-ERO or SR-RRO subobjects, it will respond according to the rules for a malformed object per [RFC5440](#RFC5440).
...
If a PCC does not recognize or support the value in the NT field,
	  it **MUST** consider the entire ERO invalid and
...
<a name="SR-ERO-INTERPRET"></a>
### Interpreting the SR-ERO
...
- If the subobjects contain NAIs only, the PCC first converts
          ...
          with it, the PCC behaves as specified in [Section 5.2.2.1](#SR-ERO-INTERPRET-ERROR).
...
<a name="SR-ERO-INTERPRET-ERROR"></a>
...
This document adds a new path setup type to PCEP to allow LSPs
to be set up using Segment Routing techniques.  This path setup
type may be used with PCEP alongside other path setup types,
such as RSVP-TE, or it may be used exclusively.

<a name="control"></a>
## Controlling the Path Setup Type

The following factors control which path setup type is used for
a given LSP.

- The available path setup types are constrained to those that
...
- When a PCE initiates an LSP, it proposes which path setup type
   ...
   network nodes on the path and on its local policy.  The PCC **MAY** choose
   to accept the proposed path setup type or to reject the PCInitiate
   request, based on its local policy.
...
- Implementations **MUST** allow the operator to enable and disable
...
- PCE implementations **MAY** allow the operator to configure a
   ...
   the absence of a specified path setup type.
- PCC implementations **MUST** allow the operator to specify that a path
   ...
   path setup type.
- PCC implementations **MAY** allow the operator to configure a preference
   for the PCC to nominate Segment Routing or RSVP-TE as the path
   setup type if none is specified for an LSP.
- PCC implementations **SHOULD** allow the operator to configure a PCC to
   refuse to set up an LSP using an undesired path setup type.

<a name="migrating"></a>
## Migrating a Network to Use PCEP Segment-Routed Paths
...
will still be present after the session comes back up.

An implementation **SHOULD** allow the operator to manually trigger a PCEP
session to be reset.

An implementation **MAY** automatically reset a PCEP session when
...
unnecessary network traffic.  Therefore, such implementations **SHOULD** allow
the operator to override this behavior and wait instead for a manual reset.

Once Segment Routing is enabled on a PCEP session, it can be used as the
path setup type for future LSPs.
...
- An implementation **SHOULD** allow the operator to view whether the
   ...
   If the PCEP speaker is a PCC, then the implementation **SHOULD** also allow
...
- An implementation **SHOULD** allow the operator to view
   ...
   is a PCC, then the implementation **SHOULD** also allow the operator to view
   the values of the L-Flag and N-Flag and MSD fields that the peer sent.
- An implementation **SHOULD** allow the operator to view
   whether the Segment Routing PST is enabled on the PCEP session.
- If one PCEP speaker advertises the Segment Routing PST capability, but the other
   does not, then the implementation **SHOULD** create a log to inform the
   operator of the capability mismatch.
- An implementation **SHOULD** allow the operator to view the PST that was
   proposed, or requested, for an LSP and the PST that was actually used.
- If a PCEP speaker decides to use a different PST to the one that was
   ...
   The log **SHOULD** give the reason for this choice (local policy,
   equipment capability, etc.).
- If a PCEP speaker rejects a Segment Routing path, then it **SHOULD** create a log
...
The PCEP YANG module is defined in [PCE-PCEP-YANG](#I-D.ietf-pce-pcep-yang).  In the future, this
...
The PCEP MIB [RFC7420](#RFC7420) could also be updated to include this
information.

<a name="Security"></a>
# Security Considerations

The security considerations described in [RFC5440](#RFC5440), [RFC8231](#RFC8231), [RFC8281](#RFC8281), and [RFC8408](#RFC8408) are
applicable to this specification.  No additional security measures are required.

Note that this specification enables a network controller to instantiate a
   ...
   mechanisms of [RFC5440](#RFC5440), [RFC8231](#RFC8231), and [RFC8281](#RFC8281) are not
...
Note that this specification adds the MSD field to the Open message (see [Section 4.1.2](#cap-negotiation)),
   which discloses how many MPLS labels the sender can push onto packets that
   it forwards into the network. If the security mechanisms of [RFC8231](#RFC8231) and [RFC8281](#RFC8281)
...
<a name="PCEP-Object-Codepoints"></a>
## PCEP ERO and RRO Subobjects

This document defines a new subobject type for the PCEP ERO
...
<a name="NAI-Type-Registry"></a>
## New NAI Type Registry
...
Review [RFC8126](#RFC8126). The new registry contains the
following values:
...
<a name="IANA-SR-ERO-FLAG"></a>
## New SR-ERO Flag Registry
...
<a name="IANA-TLV-Type-Indicators"></a>
## PCEP TLV Type Indicators

IANA has allocated the following codepoint in the "PCEP TLV Type Indicators" registry.  Note that this TLV type indicator is deprecated but retained in the registry to ensure compatibility with early implementations of this specification.  See [Appendix A](#Early) for details.

| Value | Meaning | Reference |
| --- | --- | --- |
| 26 | SR-PCE-CAPABILITY (deprecated) | This document |

<a name="IANA-subTLV-Type-Indicators"></a>
...
IANA has created a new sub-registry, named
        ...
        assigned by Standards Action [RFC8126](#RFC8126).  The
...
<a name="IANA-PATH-SETUP-TYPE"></a>
## New Path Setup Type

A sub-registry within the "Path Computation Element Protocol (PCEP) Numbers" registry called "PCEP Path Setup Types" was created in [RFC8408](#RFC8408). IANA has allocated a new codepoint within this registry, as follows:

| Value | Description | Reference |
| --- | --- | --- |
| 1 | Traffic-engineering path is set up using Segment Routing. | This document |

<a name="IANA-METRIC-TYPE"></a>
## New Metric Type
...
<a name="IANA-SR-PCE-CAP-FLAG"></a>
## SR PCE Capability Flags
...
<a name="I-D.ietf-6man-segment-routing-header"></a> **[IPv6-SRH]** C Filsfils, D Dukes, S Previdi, J Leddy, S Matsushima, D Voyer, "IPv6 Segment Routing Header (SRH)", Internet-Draft draft-ietf-6man-segment-routing-header-26, October 2019. Work in Progress <https://tools.ietf.org/html/draft-ietf-6man-segment-routing-header-26>

<a name="I-D.ietf-idr-bgp-ls-segment-routing-msd"></a> **[MSD-BGP]** J Tantsura, U Chunduri, K Talaulikar, G Mirsky, N Triantafillis, "Signaling MSD (Maximum SID Depth) using Border Gateway Protocol Link-State", Internet-Draft draft-ietf-idr-bgp-ls-segment-routing-msd-09, October 2019. Work in Progress <https://tools.ietf.org/html/draft-ietf-idr-bgp-ls-segment-routing-msd-09>
...
<a name="RFC8413"></a> **[RFC8413]** Y. Zhuang, Q. Wu, H. Chen, A. Farrel, "Framework for Scheduled Use of Resources", RFC 8413, DOI 10.17487/RFC8413, July 2018. <https://www.rfc-editor.org/info/rfc8413> [Local MD](rfc8413.md)
...
<a name="I-D.ietf-spring-segment-routing-policy"></a> **[SR-POLICY]** C Filsfils, S Sivabalan, D Voyer, A Bogdanov, P Mattes, "Segment Routing Policy Architecture", Internet-Draft draft-ietf-spring-segment-routing-policy-05, November 2019. Work in Progress <https://tools.ietf.org/html/draft-ietf-spring-segment-routing-policy-05>
...
- Organization: Cisco Systems, Inc.
- Address: Pegasus Parc, De kleetlaan 6a, Diegem, Brabant 1831, Belgium
- Email: cfilsfil@cisco.com

**Jeff Tantsura**

- Organization: Apstra, Inc.
- Address: 333 Middlefield Rd #200, Menlo Park, CA, 94025, United States of America
- Email: jefftant.ietf@gmail.com

**Wim Henderickx**

- Organization: Nokia
- Address: Copernicuslaan 50, Antwerp 2018, CA, 95134, Belgium
- Email: wim.henderickx@nokia.com

**Jon Hardwick**

- Organization: Metaswitch Networks
- Address: 100 Church Street, Enfield, Middlesex, United Kingdom
- Email: jonathan.hardwick@metaswitch.com
