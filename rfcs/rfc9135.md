...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-bess-evpn-inter-subnet-forwarding-15
...
**Keywords:** IRB, inter-subnet-forwarding, symmetric, asymmetric, mobility

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-bess-evpn-inter-subnet-forwarding-15)
- [alternate](https://dx.doi.org/10.17487/rfc9135)
...
- [2. Terminology](#terms)
  - [2.1. Requirements Language](#sect-1.1)
- [3. EVPN PE Model for IRB Operation](#sect-3)
- [4. Symmetric and Asymmetric IRB](#sect-4)
  - [4.1. IRB Interface and Its MAC and IP Addresses](#sect-4.1)
  - [4.2. Operational Considerations](#sect-4.2)
- [5. Symmetric IRB Procedures](#sect-5)
  - [5.1. Control Plane - Advertising PE](#sect-5.1)
  - [5.2. Control Plane - Receiving PE](#sect-5.2)
  - [5.3. Subnet Route Advertisement](#sect-5.3)
  - [5.4. Data Plane - Ingress PE](#sect-5.4)
  - [5.5. Data Plane - Egress PE](#sect-5.5)
- [6. Asymmetric IRB Procedures](#sect-6)
  - [6.1. Control Plane - Advertising PE](#sect-6.1)
  - [6.2. Control Plane - Receiving PE](#sect-6.2)
  - [6.3. Data Plane - Ingress PE](#sect-6.3)
  - [6.4. Data Plane - Egress PE](#sect-6.4)
- [7. Mobility Procedure](#sect-7)
  - [7.1. Initiating a Gratuitous ARP upon a Move](#sect-7.1)
  - [7.2. Sending Data Traffic without an ARP Request](#sect-7.2)
  - [7.3. Silent Host](#sect-7.3)
- [8. BGP Encoding](#sect-8)
  - [8.1. EVPN Router's MAC Extended Community](#sect-8.1)
- [9. Operational Models for Symmetric Inter-Subnet Forwarding](#sect-9)
  - [9.1. IRB Forwarding on NVEs for Tenant Systems](#sect-9.1)
    - [9.1.1. Control Plane Operation](#sect-9.1.1)
    - [9.1.2. Data Plane Operation](#sect-9.1.2)
  - [9.2. IRB Forwarding on NVEs for Subnets behind Tenant Systems](#sect-9.2)
    - [9.2.1. Control Plane Operation](#sect-9.2.1)
    - [9.2.2. Data Plane Operation](#sect-9.2.2)
- [10. Security Considerations](#sect-11)
- [11. IANA Considerations](#sect-12)
- [12. References](#section-12)
  - [12.1. Normative References](#section-12.1)
  - [12.2. Informative References](#section-12.2)
- [Acknowledgements](#sect-10)
- [Authors' Addresses](#authors-addresses)

<a name="intro"></a>
# Introduction

EVPN [RFC7432](#RFC7432) provides an extensible and flexible multihoming VPN
...
**BD:**
  Broadcast Domain. As per [RFC7432](#RFC7432), an EVI consists of a single BD or multiple
     BDs.  In the case of VLAN-bundle and VLAN-based service
     models (see [RFC7432](#RFC7432)), a BD is equivalent to an EVI.  In the
...
**BD Route Target:**
  Refers to the broadcast-domain-assigned Route Target [RFC4364](#RFC4364).  In the case of a VLAN-aware bundle
...
**Ethernet NVO Tunnel:**
  Refers to Network Virtualization Overlay tunnels
     with an Ethernet payload, as specified for VXLAN in [RFC7348](#RFC7348) and for
     NVGRE in [RFC7637](#RFC7637).
...
**IP NVO Tunnel:**
  Refers to Network Virtualization Overlay tunnels
     ...
     in [VXLAN-GPE](#I-D.ietf-nvo3-vxlan-gpe).
...
**MAC-VRF:**
  A Virtual Routing and Forwarding table for
     MAC addresses on an NVE/PE, as per [RFC7432](#RFC7432).  A MAC-VRF is
...
**RT-5:**
  EVPN Route Type 5, i.e., IP Prefix route, as defined in [RFC9136](#RFC9136).
...
**VNI:**
  Virtual Network Identifier.  As in [RFC8365](#RFC8365), the term is used
...
This document also assumes familiarity with the terminology of [RFC7365](#RFC7365), [RFC7432](#RFC7432), and [RFC8365](#RFC8365).
...
```
A tenant needing IRB services on a PE requires an IP-VRF table along with one or more MAC-VRF tables.  An IP-VRF, as defined in [RFC4364](#RFC4364), is the
   instantiation of an IP-VPN instance in a PE.  A MAC-VRF, as defined in
   [RFC7432](#RFC7432), is the instantiation of an EVI in a PE.  A
   ...
   EVPN PE are configured in VLAN-based mode (i.e., [RFC7432](#RFC7432)),
   ...
   an EVPN PE are configured in VLAN-aware bundle mode (i.e., [RFC7432](#RFC7432)), then there are several bridge tables per MAC-VRF (per EVI) --
...
IP-VRF is identified by its corresponding Route Target and Route
   ...
   Target and VLAN ID in order to identify the corresponding bridge table.
...
```
In symmetric IRB, as shown in [Figure 2](#fig-2), the inter-subnet forwarding
...
In asymmetric IRB, the lookup operation is asymmetric and the ingress
   ...
   performs just a single MAC lookup as depicted in [Figure 3](#fig-3) below.
...
```
In asymmetric IRB, as shown in [Figure 3](#fig-3), the inter-subnet forwarding between
...
2. Each PE for a given tenant subnet uses the same anycast default
       ...
       specified in [RFC7432](#RFC7432), which is carried in the EVPN MAC/IP
...
Although both of these options are applicable to both symmetric and
   asymmetric IRB, [option 1](#opt1) is recommended because of the ease of
...
If  [option 1](#opt1) is used, an implementation **MAY** choose to auto-derive the
...
For [option 1](#opt1) -- irrespective of whether only the anycast MAC address or
   ...
   sender hardware address in the payload).  For example, in [Figure 4](#fig-4),
...
As an example that illustrates the previous statement, suppose PE1
   ...
   [Figure 4](#fig-4).  Since both PEs are attached to the bridge table of the
...
Refer to [EVPN](#I-D.ietf-bess-evpn-modes-interop) for more information
...
When a PE (e.g., PE1 in [Figure 4](#fig-4) above) learns the MAC and IP address of
...
- The Route Distinguisher (RD), Ethernet Segment Identifier, Ethernet
      ...
      Address, and MPLS Label1 fields **MUST** be set per [RFC7432](#RFC7432) and
      [RFC8365](#RFC8365).
...
Just as in [RFC7432](#RFC7432), the RD, Ethernet Tag ID, MAC Address Length,
...
This route is advertised with one or more Encapsulation Extended
   Communities [RFC9012](#RFC9012), one for each encapsulation type supported by
   the advertising PE.  If one or more encapsulation types require an
   Ethernet frame, a single EVPN Router's MAC Extended Community ([Section 8.1](#sect-8.1)) is also advertised.  This extended community specifies the MAC
...
When a PE (e.g., PE2 in [Figure 4](#fig-4) above) receives this EVPN MAC/IP
...
When an Ethernet frame is received by an ingress PE (e.g., PE1 in
   [Figure 4](#fig-4) above), the PE uses the AC ID (e.g., VLAN ID) to identify
...
When the tenant's MPLS or NVO encapsulated packet is received over an
   ...
   MAC-VRF instead of an IP-VRF, then the procedures in [Section 6.4](#sect-6.4) for
...
The destination MAC address lookup in the MAC-VRF/BT
   ...
   [RFC7432](#RFC7432) for MPLS and [RFC8365](#RFC8365) for VXLAN encapsulations.
...
Note that the forwarding behavior on the egress PE is the same as the EVPN intra-subnet forwarding described in [RFC7432](#RFC7432) for MPLS and
   [RFC8365](#RFC8365) for NVO networks.  In other words, all the packet
   processing associated with the inter-subnet forwarding semantics is
   confined to the ingress PE for asymmetric IRB mode.

It should also be noted that [RFC7432](#RFC7432) provides a different level of
...
When a TS moves from one NVE (aka source NVE) to another NVE (aka
   ...
   all participating NVEs be updated.  [RFC7432](#RFC7432) describes the MAC
   ...
   for a multihomed TS can be extended per [RFC7432](#RFC7432).
...
Depending on the expected TS's behavior, an NVE needs to handle at least
   the [first](#way1) option and should be able to handle the [second](#way2) and [third](#way3) options.
...
a many-to-one relationship is outside the scope of this document, and it is
   covered in [EXTENDED-MOBILITY](#I-D.ietf-bess-evpn-irb-extended-mobility).  The
...
Since this NVE has previously learned the same MAC and IP addresses
   ...
   it initiates MAC Mobility procedures per [RFC7432](#RFC7432) by advertising an
   EVPN MAC/IP Advertisement route with both the MAC and IP addresses
   filled in (per Sections [5.1](#sect-5.1) and [6.1](#sect-6.1)) along with the MAC Mobility extended
   community, with the sequence number incremented by one.  The target
   NVE also exercises the MAC duplication detection procedure in [RFC7432](#RFC7432).
...
All other remote NVE devices, upon receiving the MAC/IP Advertisement route
   ...
   source NVE, these remote PEs perform the cleanups for their BGP tables.
...
- The target NVE, upon learning this MAC address in the data plane,
      ...
      procedures per [RFC7432](#RFC7432) by advertising an EVPN MAC/IP
...
If an EVPN-IRB NVE is configured not to advertise MAC-only routes,
   ...
   response, it follows the procedure outlined in [Section 7.1](#sect-7.1).  In the
...
Since VXLAN and NVGRE encapsulations require an inner Ethernet header
   ...
   using the new EVPN Router's MAC Extended Community ([Section 8.1](#sect-8.1)).

[Figure 6](#fig-6) below illustrates this scenario, where a given tenant (e.g., an
...
- RD and Ethernet Segment Identifier (ESI) per [RFC7432](#RFC7432)
- Ethernet Tag = 0 (assuming VLAN-based service)
- MAC Address Length = 48
- MAC Address = Mi (where i = 1, 2, 3, 4, or 5) in [Figure 6](#fig-6), above
- IP Address Length = 32 or 128
- IP Address = IPi (where i = 1, 2, 3, 4, or 5) in [Figure 6](#fig-6), above
...
Each NVE advertises an EVPN RT-2 route with two Route Targets (one
   ...
   [RFC9012](#RFC9012), and the second BGP Extended Community includes
   the MAC address of the NVE (e.g., MACx for NVE1 or MACy for NVE2) as
   defined in [Section 8.1](#sect-8.1).  The EVPN Router's MAC Extended Community **MUST** be added
...
If the receiving NVE receives an EVPN RT-2 with only label1 and only
   ...
   approach [RFC7606](#RFC7606) and **SHOULD** log an error message.
...
In this symmetric IRB scenario, inter-subnet traffic between NVEs
   ...
   from TS2 to TS4 will be encapsulated by NVE1 using NVE2's IP-VRF VNI/MPLS label, as long as TS4's host IP is present in NVE1's IP-VRF.
...
The main difference between this solution and the previous one is the
   ...
   defined in [RFC9136](#RFC9136).  These subnet
...
[Figure 7](#fig-7) illustrates this scenario where a given tenant (e.g., an IP-VPN
...
```
Note that in [Figure 7](#fig-7), above, SN1 and SN2 are configured on NVE1,
...
Each NVE advertises a route type 5 (EVPN RT-5, IP Prefix route
   defined in [RFC9136](#RFC9136)) for each of its
...
Each NVE also advertises an EVPN RT-2 (MAC/IP Advertisement route)
   ...
   for each of its TSs exactly as described in [Section 9.1.1](#sect-9.1.1).
...
When receiving the EVPN RT-2 advertisement, the receiving NVE imports the
   ...
   per [Section 9.1.1](#sect-9.1.1).  When both routes exist, recursive route
   ...
   as the inner MAC for VXLAN encapsulation.
...
- Upon receiving the packet, the ingress NVE1 uses the VLAN tag to
      ...
      the frame to its IRB interface as in [Section 9.1.1](#sect-9.1.1).
- The Ethernet header of the packet is stripped, and the packet is
      fed to the IP-VRF, where an IP lookup is performed on the
      destination address.
...
The security considerations for Layer 2 forwarding in this document
   follow those of [RFC7432](#RFC7432) for MPLS encapsulation and those
   of [RFC8365](#RFC8365) for VXLAN or NVGRE encapsulations.  This section
...
This document describes a set of procedures for inter-subnet
   ...
   document follows that of [RFC4365](#RFC4365), with the exception of the
   application of routing protocols between CEs and PEs.  Contrary to
   [RFC4364](#RFC4364), this document does not describe route distribution
...
The VPN scheme described in this document does not provide the
   quartet of security properties mentioned in [RFC4365](#RFC4365)
...
<a name="I-D.ietf-bess-evpn-modes-interop"></a> **[EVPN]** L Krattiger, A Sajassi, S Thoria, J Rabadan, J Drake, "EVPN Interoperability Modes", Internet-Draft draft-ietf-bess-evpn-modes-interop-00, May 2021. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-bess-evpn-modes-interop-00>

<a name="I-D.ietf-bess-evpn-irb-extended-mobility"></a> **[EXTENDED-MOBILITY]** N Malhotra, A Sajassi, A Pattekar, J Rabadan, A Lingala, J Drake, "Extended Mobility Procedures for EVPN-IRB", Internet-Draft draft-ietf-bess-evpn-irb-extended-mobility-07, October 2021. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-bess-evpn-irb-extended-mobility-07>

<a name="RFC4365"></a> **[RFC4365]** E. Rosen, "Applicability Statement for BGP/MPLS IP Virtual Private Networks (VPNs)", RFC 4365, DOI 10.17487/RFC4365, February 2006. <https://www.rfc-editor.org/info/rfc4365> [Local MD](rfc4365.md)
...
<a name="I-D.ietf-nvo3-vxlan-gpe"></a> **[VXLAN-GPE]** F Maino, L Kreeger, U Elzur, "Generic Protocol Extension for VXLAN (VXLAN-GPE)", Internet-Draft draft-ietf-nvo3-vxlan-gpe-12, September 2021. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-nvo3-vxlan-gpe-12>
...
The authors would like to thank Sami Boutros, Jeffrey Zhang,
   ...
   valuable comments.  The authors would also like to thank Linda Dunbar, Florin Balus, Yakov Rekhter, Wim Henderickx, Lucy Yong, and
