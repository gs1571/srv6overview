...
*(SR Prefix-SID Extensions for BGP)*
...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-idr-bgp-prefix-sid-27
...
**Keywords:** SR, MPLS, BGP, Prefix-SID, Label-Index, SRGB

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-idr-bgp-prefix-sid-27)
- [alternate](https://dx.doi.org/10.17487/rfc8669)
...
- [2. MPLS BGP Prefix-SID](#MPLSPREFIXSID)
- [3. BGP Prefix-SID Attribute](#PREFIXSIDATTR)
  - [3.1. Label-Index TLV](#LABELINDEX)
  - [3.2. Originator SRGB TLV](#ORIGINSRGBTLV)
- [4. Receiving BGP Prefix-SID Attribute](#section-4)
  - [4.1. MPLS Data Plane: Labeled Unicast](#RECMPLSLABEL)
- [5. Advertising BGP Prefix-SID Attribute](#section-5)
  - [5.1. MPLS Data Plane: Labeled Unicast](#ADVMPLSLABEL)
- [6. Error Handling of BGP Prefix-SID Attribute](#ERRORHANDLING)
- [7. IANA Considerations](#IANA)
- [8. Manageability Considerations](#MANAGE)
...
A segment is identified through a Segment Identifier (SID). 
      ...
      node of the SR domain prepends an SR header containing SIDs to an incoming packet.

As described in [RFC8402](#RFC8402),
      when SR is applied to the MPLS data plane ([RFC8660](#RFC8660)), the SID consists of a
      label.

[RFC8402](#RFC8402) also 
      ...
      IPv6 addresses [IPv6-SRH](#I-D.ietf-6man-segment-routing-header).
...
A BGP Prefix Segment is a BGP prefix with a Prefix-SID attached.
      A BGP Prefix-SID is always a global SID ([RFC8402](#RFC8402)) within the SR domain
...
This document describes the BGP extensions to signal the BGP
      ...
      originate, receive, and handle error conditions for the attribute.
...
The BGP Prefix-SID attribute defined in this document can be attached
       ...
       Unicast ([RFC4760](#RFC4760) [RFC8277](#RFC8277)).
...
<a name="MPLSPREFIXSID"></a>
# MPLS BGP Prefix-SID

The BGP Prefix-SID is realized on the MPLS data plane ([RFC8660](#RFC8660)) in the following
      way:
...
- According to [RFC8402](#RFC8402),
            each BGP speaker is configured with a label block called the
            Segment Routing Global Block (SRGB). While [RFC8402](#RFC8402) recommends using the
            ...
            different SRGBs are documented in [RFC8402](#RFC8402) and [RFC8670](#RFC8670).
- If traffic engineering within the SR domain is required, each
            ...
            through [BGPLS-SR-EPE](#I-D.ietf-idr-bgpls-segment-routing-epe).
...
- This document assumes that Border Gateway Protocol - Link State
	    (BGP-LS) is the preferred method for a
            ...
            information through [RFC7752](#RFC7752), [BGPLS-SR-EPE](#I-D.ietf-idr-bgpls-segment-routing-epe), and [BGPLS-SR-EXT](#I-D.ietf-idr-bgp-ls-segment-routing-ext). However, as an
...
The Label-Index TLV **MUST** be present in the BGP Prefix-SID attribute
        attached to IPv4/IPv6 Labeled Unicast prefixes ([RFC8277](#RFC8277)). It **MUST** be ignored when received for other 
...
The Originator SRGB TLV contains the SRGB of the node originating
        ...
        when different SRGBs are used in the fabric, for example, [RFC8670](#RFC8670).

Examples of how the receiving routers concatenate the
          ranges and build their neighbor's Segment Routing Global Block (SRGB)
          are included in [RFC8660](#RFC8660).
...
The Originator SRGB TLV may only appear in a BGP Prefix-SID attribute
        ...
        ignored if it is not specified in a manner consistent with [Section 6](#ERRORHANDLING).
...
If a BGP speaker receives a node's SRGB as an attribute of the BGP-LS
         ...
         NLRI **SHOULD** be preferred, and an error should be logged.

# Receiving BGP Prefix-SID Attribute

A BGP speaker receiving a BGP Prefix-SID attribute from an External BGP (EBGP)
...
<a name="RECMPLSLABEL"></a>
## MPLS Data Plane: Labeled Unicast

A BGP session supporting the Multiprotocol BGP Labeled IPv4 or IPv6 Unicast ([RFC8277](#RFC8277)) AFI/SAFI is required.

When the  BGP Prefix-SID attribute is attached to a BGP Labeled IPv4 or IPv6
          Unicast [RFC8277](#RFC8277) AFI/SAFI, it **MUST** contain the Label-Index TLV
...
The mechanisms through which a given label-index value is assigned
        to a given prefix are outside the scope of this document.
...
If multiple different prefixes are received with the same label
           ...
           their BGP Prefix-SID attribute considered to be "conflicting".

If multiple valid paths for the same prefix are received from
          multiple BGP speakers or, in the case of [RFC7911](#RFC7911), 
          ...
          a notable exception being when [RFC5004](#RFC5004) 
          is being used to dampen route changes.
...
When a BGP speaker receives a path from a neighbor with an
        ...
        as the label for the prefix in its local MPLS data plane.
...
When a BGP speaker receives a path from a neighbor with an
        ...
        Unicast ([RFC8277](#RFC8277)) operation.

In the case of an "invalid" BGP Prefix-SID attribute, a BGP speaker **MUST**
        follow the error-handling rules specified in [Section 6](#ERRORHANDLING). 
        ...
        not due to a label-index transition.
...
When a BGP Prefix-SID attribute changes and transitions from
           ...
           corresponding to these newly "acceptable" BGP Prefix-SID attributes.

The outgoing label is always programmed as per classic
        Multiprotocol BGP IPv4/IPv6 Labeled Unicast ([RFC8277](#RFC8277))
        ...
        [RFC3032](#RFC3032) from a neighbor **MUST**
...
The BGP Prefix-SID attribute **MAY** be attached to BGP IPv4/IPv6 Labeled Unicast prefixes
      [RFC8277](#RFC8277). In order to prevent distribution of the BGP
...
A BGP speaker that originates a prefix attaches the BGP Prefix-SID
        ...
        Multiprotocol BGP IPv4/IPv6 Labeled Unicast ([RFC8277](#RFC8277)). The value of the label index in the Label-Index
        TLV is determined by configuration.

A BGP speaker that originates a BGP Prefix-SID attribute **MAY** optionally
...
In all cases, the Label field of the advertised NLRI ([RFC8277](#RFC8277) [RFC4364](#RFC4364)) **MUST** be set to the
...
When a BGP speaker receives a BGP UPDATE message containing a
      ...
      IPv4/IPv6 Labeled Unicast prefix ([RFC8277](#RFC8277)), it **MUST**
      ...
      action specified in [RFC7606](#RFC7606). When discarding an 
      attribute, a BGP speaker **SHOULD** log an error for further analysis.

As per [RFC7606](#RFC7606), if the BGP
...
For future extensibility, unknown TLVs **MUST** be ignored and 
         propagated unmodified.

<a name="IANA"></a>
# IANA Considerations

This document defines a BGP path attribute known as the BGP
...
Under the "Border Gateway Protocol (BGP) Parameters" registry, the new registry titled "BGP
...
**Table: BGP Prefix-SID TLV Types**
...
The value 2 previously corresponded to the IPv6 SID TLV, which was specified 
         ...
         [RFC8402](#RFC8402) has been deferred to 
         future specifications.
...
IANA has also created the "BGP Prefix-SID Label-Index TLV Flags"
         ...
         empty. The registration policy for flag bits is Expert Review [RFC8126](#RFC8126),
         consistent with the "BGP Prefix-SID TLV Types" registry.

Finally, IANA has created the "BGP Prefix-SID Originator SRGB TLV Flags"
         registry under the  "Border Gateway Protocol (BGP) Parameters" registry,
         with a reference to this document. Initially, this 16-bit flags registry is
         empty. The registration policy for flag bits is Expert Review [RFC8126](#RFC8126)
         consistent with the BGP Prefix-SID TLV Types registry.
...
This document defines a BGP attribute to address use
...
The deployment model described in [RFC8670](#RFC8670) assumes multiple
...
This document introduces a BGP attribute (BGP Prefix-SID), which
      inherits the security considerations expressed in: [RFC4271](#RFC4271), [RFC8277](#RFC8277), and
      [RFC8402](#RFC8402).

When advertised using BGPsec as described in [RFC8205](#RFC8205),
     the BGP Prefix-SID attribute doesn't impose any unique
     security considerations. It should be noted that the BGP Prefix-SID
     attribute is not protected by the BGPsec signatures.

It should be noted that,
      as described in [Section 8](#MANAGE), this document refers
...
To prevent a Denial-of-Service (DoS) or Distributed-Denial-of-Service 
      ...
      duplicate error log messages **SHOULD** be deployed.
...
Since BGP-LS is the preferred method for advertising SRGB information,
         ...
         the same node's BGP-LS Node NLRI.
...
<a name="I-D.ietf-idr-bgpls-segment-routing-epe"></a> **[BGPLS-SR-EPE]** S Previdi, K Talaulikar, C Filsfils, K Patel, S Ray, J Dong, "BGP-LS extensions for Segment Routing BGP Egress Peer Engineering", Internet-Draft draft-ietf-idr-bgpls-segment-routing-epe-19, May 2019. Work in Progress <https://tools.ietf.org/html/draft-ietf-idr-bgpls-segment-routing-epe-19>

<a name="I-D.ietf-idr-bgp-ls-segment-routing-ext"></a> **[BGPLS-SR-EXT]** S Previdi, K Talaulikar, C Filsfils, H Gredler, M Chen, "BGP Link-State extensions for Segment Routing", Internet-Draft draft-ietf-idr-bgp-ls-segment-routing-ext-16, June 2019. Work in Progress <https://tools.ietf.org/html/draft-ietf-idr-bgp-ls-segment-routing-ext-16>
...
<a name="RFC5004"></a> **[RFC5004]** E. Chen, S. Sangli, "Avoid BGP Best Path Transitions from One External to Another", RFC 5004, DOI 10.17487/RFC5004, September 2007. <https://www.rfc-editor.org/info/rfc5004> [Local MD](rfc5004.md)
...
<a name="RFC8670"></a> **[RFC8670]** C Filsfils, S Previdi, G Dawra, E Aries, P Lapukhov, "BGP Prefix Segment in Large-Scale Data Centers", RFC 8670, DOI 10.17487/RFC8670, December 2019. <https://www.rfc-editor.org/info/rfc8670> [Local MD](rfc8670.md)
...
The authors would like to thank Krishna Deevi, 
         Juan Alcaide, Howard Yang, and Jakob Heitz for discussions 
         on conflicting BGP Prefix-SID label indices and BGP add paths.

The authors would like to thank Peter Yee, Tony Przygienda,
