...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-nvo3-geneve-16
...
**Keywords:** overlay, tunnel, extensible, variable, metadata, options, endpoint, transit

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-nvo3-geneve-16)
- [alternate](https://dx.doi.org/10.17487/rfc8926)
...
- [1. Introduction](#sec-1)
  - [1.1. Requirements Language](#sec-1.1)
  - [1.2. Terminology](#sec-1.2)
- [2. Design Requirements](#sec-2)
  - [2.1. Control Plane Independence](#sec-2.1)
  - [2.2. Data Plane Extensibility](#sec-2.2)
    - [2.2.1. Efficient Implementation](#sec-2.2.1)
  - [2.3. Use of Standard IP Fabrics](#sec-2.3)
- [3. Geneve Encapsulation Details](#sec-3)
  - [3.1. Geneve Packet Format over IPv4](#sec-3.1)
  - [3.2. Geneve Packet Format over IPv6](#sec-3.2)
  - [3.3. UDP Header](#sec-3.3)
  - [3.4. Tunnel Header Fields](#sec-3.4)
  - [3.5. Tunnel Options](#sec-3.5)
    - [3.5.1. Options Processing](#sec-3.5.1)
- [4. Implementation and Deployment Considerations](#sec-4)
  - [4.1. Applicability Statement](#sec-4.1)
  - [4.2. Congestion-Control Functionality](#sec-4.2)
  - [4.3. UDP Checksum](#sec-4.3)
    - [4.3.1. Zero UDP Checksum Handling with IPv6](#sec-4.3.1)
  - [4.4. Encapsulation of Geneve in IP](#sec-4.4)
    - [4.4.1. IP Fragmentation](#sec-4.4.1)
    - [4.4.2. DSCP, ECN, and TTL](#sec-4.4.2)
    - [4.4.3. Broadcast and Multicast](#sec-4.4.3)
    - [4.4.4. Unidirectional Tunnels](#sec-4.4.4)
  - [4.5. Constraints on Protocol Features](#sec-4.5)
    - [4.5.1. Constraints on Options](#sec-4.5.1)
  - [4.6. NIC Offloads](#sec-4.6)
  - [4.7. Inner VLAN Handling](#sec-4.7)
- [5. Transition Considerations](#sec-5)
- [6. Security Considerations](#sec-6)
  - [6.1. Data Confidentiality](#sec-6.1)
    - [6.1.1. Inter-Data Center Traffic](#sec-6.1.1)
  - [6.2. Data Integrity](#sec-6.2)
  - [6.3. Authentication of NVE Peers](#sec-6.3)
  - [6.4. Options Interpretation by Transit Devices](#sec-6.4)
  - [6.5. Multicast/Broadcast](#sec-6.5)
  - [6.6. Control Plane Communications](#sec-6.6)
...
Networking has long featured a variety of tunneling, tagging, and
   ...
   VLANs [IEEE.802.1Q_2018](#IEEE.802.1Q_2018) and MPLS [RFC3031](#RFC3031) through the more recent
   VXLAN (Virtual eXtensible Local Area Network)  [RFC7348](#RFC7348)
   and NVGRE (Network Virtualization
   Using Generic Routing Encapsulation) [RFC7637](#RFC7637) -- often
...
Work such as "VL2: A Scalable and Flexible Data Center Network" [VL2](#VL2) and "NVO3 Data Plane Requirements" [NVO3-DATAPLANE](#I-D.ietf-nvo3-dataplane-requirements) 
...
The Network
   Virtualization over Layer 3 (NVO3) Framework [RFC7365](#RFC7365) defines many of the concepts commonly
...
Geneve is designed to support network virtualization use cases for data center environments.  In these situations,
   ...
   over IP networks are described in the NVO3 Framework [RFC7365](#RFC7365).
   [Figure 1](#ref-sample-geneve-deployment) shows an example of a
...
Although some protocols for network virtualization have included a
   ...
   notably, VXLAN [RFC7348](#RFC7348) prescribed a multicast-learning-based control plane), these specifications have largely been treated
...
The Geneve packet format consists of a compact tunnel header
   ...
   unit of the indicated type, such as an Ethernet frame. Sections [3.1](#sec-3.1)
   and [3.2](#sec-3.2) illustrate the Geneve packet format transported (for
...
```
<a name="sec-3.2"></a>
## Geneve Packet Format over IPv6
...
The use of an encapsulating UDP [RFC0768](#RFC0768) header follows the
...
**Source Port:**
  A source port selected by the originating tunnel endpoint.  This source port **SHOULD** be the same for all packets
        ...
        using the IPv6 flow label for tunnel use cases, see [RFC6438](#RFC6438).
...
**UDP Checksum:**
  In order to protect the Geneve header, options, and payload from
  	...
  	specified in [RFC0768](#RFC0768) and [RFC1122](#RFC1122) when 
  	...
  	be generated by default as specified in [RFC0768](#RFC0768) 
  	and [RFC8200](#RFC8200) when Geneve
  ...
  Under certain conditions, the UDP checksum **MAY** be set to zero on transmit
  	for packets encapsulated in both IPv4 and IPv6 [RFC8200](#RFC8200).
  	See [Section 4.3](#sec-4.3) for additional
...
**C (1 bit):**
  Critical options present.  One or more options has the critical bit set (see [Section 3.5](#sec-3.5)).  If this bit is set, then
...
**Protocol Type (16 bits):**
  The type of protocol data unit appearing after the Geneve header.  This follows the Ethertype
        [ETYPES](#ETYPES) convention, with Ethernet itself being represented by the
        value 0x6558.
...
**Option Class (16 bits):**
  Namespace for the 'Type' field.  IANA has created a "Geneve Option Class" registry to
        ...
        allocation by IETF Review and for Experimental Use (see [Section 7](#sec-7)).
...
Architecturally, options are intended to be self descriptive and independent. 
   ...
   described in [Section 4.5.1](#sec-4.5.1).
...
Options may be processed by NIC hardware utilizing offloads (e.g., LSO and LRO) 
   as described in [Section 4.6](#sec-4.6). Careful consideration should be 
   given to how the offload capabilities outlined in [Section 4.6](#sec-4.6) 
...
As a UDP-based protocol, Geneve adheres 
	to the UDP usage guidelines as specified in [RFC8085](#RFC8085). 
...
Geneve is intended to be deployed in a data center network environment 
	...
	operators that fits with the definition of controlled environments
       	in [RFC8085](#RFC8085). A network in a controlled environment can be
...
For the purpose of this document, a traffic-managed controlled environment
	...
	of a TMCE is outlined in [RFC8086](#RFC8086). Significant portions of the text 
	in [Section 4.1](#sec-4.1) through [Section 4.3](#sec-4.3) are based 
	on [RFC8086](#RFC8086) as applicable to Geneve.

It is the responsibility of the operator to ensure that the guidelines/requirements
	in this section are followed as applicable to their Geneve deployment(s).
...
The outer UDP checksum **SHOULD** be used with Geneve when transported
   ...
   avoid misdelivery of the payload to different tenant systems).  The UDP checksum provides a statistical guarantee 
	...
	(see [RFC8085](#RFC8085)). In deployments where such a risk exists, 
	an operator **SHOULD** use additional data integrity
	mechanisms such as those offered 
	by IPsec (see [Section 6.2](#sec-6.2)).
...
An operator **MAY** choose to disable UDP checksums
	...
	the conditions (a, b, or c) in [Section 4.3.1](#sec-4.3.1) is met.
...
By default, UDP checksums **MUST** be used when Geneve is transported over IPv6.
	...
	additional requirements in [Section 4.3.1](#sec-4.3.1) are met.

<a name="sec-4.3.1"></a>
### Zero UDP Checksum Handling with IPv6
...
When Geneve is used over IPv6, the UDP checksum is used to protect IPv6 headers, 
	...
	[Section 4.1](#sec-4.1) if one of the following conditions is met.

1. It is known that packet corruption is exceptionally
	...
	network) and the operator is willing to risk undetected packet
	    corruption.
2. It is judged through observational measurements (perhaps through historic
...
3. The Geneve payload is carrying applications that are tolerant of misdelivered 
...
1. Use of UDP checksum over IPv6 **MUST** be the default 
...
2. If Geneve is used with zero UDP checksum over IPv6, then such
	...
	in [RFC6936](#RFC6936) and requirement 1 as specified in [RFC6936](#RFC6936) since it is relevant to Geneve.
...
4. The Geneve tunnel endpoint that encapsulates the tunnel **MAY** use different
...
5. Measures **SHOULD** be taken to prevent Geneve traffic over IPv6 with zero UDP 
...
The above requirements do not change the requirements
	specified in either [RFC8200](#RFC8200) or
	[RFC6936](#RFC6936).
...
It is **RECOMMENDED** that Path MTU Discovery (see [RFC1191](#RFC1191) and [RFC8201](#RFC8201)) be used to prevent or minimize fragmentation.
   ...
   needed [RFC0792](#RFC0792) or Packet Too Big [RFC4443](#RFC4443) messages to the tenant system(s).
   ...
   similar overlay encapsulation services like Pseudowire Emulation
	  Edge-to-Edge (PWE3) are provided in [RFC3985](#RFC3985).
...
Note that some implementations may not be capable of supporting
   ...
   outlined in [INTAREA-TUNNELS](#I-D.ietf-intarea-tunnels).
...
[RFC2983](#RFC2983) provides guidance for mapping DSCP between inner and outer
...
[RFC6040](#RFC6040) describes the mechanism for exposing ECN capabilities on IP
...
Though either the Uniform or Pipe models could be used for handling TTL (or Hop Limit in case of IPv6) when tunneling IP packets, the Pipe model is more consistent with network virtualization.
   [RFC2003](#RFC2003) provides guidance on handling TTL between inner IP header and outer IP tunnels;
   this model is similar to the Pipe model and is **RECOMMENDED** for 
   use with Geneve for network virtualization applications.
...
When physical multicast is in
   ...
   mark the unknown option as critical.  The requirements outlined in [Section 3.4](#sec-3.4) 
   apply for critical options.

In addition, [RFC8293](#RFC8293) provides examples of various mechanisms that can 
   be used for multicast handling in network virtualization overlay networks.
...
<a name="sec-4.5"></a>
## Constraints on Protocol Features
...
While Geneve options are flexible, a control plane may restrict
   ...
   implementation in software or hardware to handle (see [NVO3-ENCAP](#I-D.ietf-nvo3-encap)).
...
When crossing an untrusted link, such as the general Internet, VPN technologies such as IPsec
   [RFC4301](#RFC4301) should be used to provide authentication and/or encryption of
   the IP packets formed as part of Geneve encapsulation (see [Section 6.1.1](#sec-6.1.1)).

Geneve does not otherwise affect the security of the encapsulated
   packets. As per the guidelines of BCP 72 [RFC3552](#RFC3552), the following sections 
   ...
   environment, determine the risks that are applicable to their specific environment, and use appropriate mitigation approaches as applicable.
...
If an operator determines data confidentiality is necessary in their environment 
	...
	encryption mechanisms, such as IPsec, DTLS, etc.
...
The principles described in [Section 4](#sec-4) regarding controlled environments still apply to 
	the geographically separated data center usage outlined in this section.
...
A compromised network node or a transit device within a data center may launch an active
	...
	although non-cryptographic mechanisms may not protect the Geneve portion of the packet from tampering.
...
A rogue network device or a compromised NVE in a data center environment might be able to
	...
	may be used in certain trusted environments to ensure Geneve packets originated
       	from the intended NVE peer.

<a name="sec-6.4"></a>
## Options Interpretation by Transit Devices

Options, if present in the packet, are generated and terminated by tunnel endpoints. As indicated
	in [Section 2.2.1](#sec-2.2.1), transit devices may interpret the options. However, 
	...
	**MUST** ensure that transit devices are trusted and not compromised. The definition of 
	a mechanism to ensure this trust is beyond the scope of this document.
...
In typical data center networks where IP multicasting is not supported in the underlay 
	...
	extensions [RFC5374](#RFC5374), to protect multicast traffic among Geneve NVE groups.

<a name="sec-6.6"></a>
## Control Plane Communications

A Network Virtualization Authority (NVA) as outlined in [RFC8014](#RFC8014) may
...
IANA has allocated UDP port 6081 in the "Service Name and Transport Protocol 
	Port Number Registry" [IANA-SN](#IANA-SN) as the well-known destination port
       	for Geneve:
...
In addition, IANA has created a new subregistry titled "Geneve Option Class"
   ...
   a new "Network Virtualization Overlay (NVO3)" heading in the IANA protocol registries [IANA-PR](#IANA-PR). 
   ...
   The registration rules for the new registry are (as defined by [RFC8126](#RFC8126)):

**Table: Geneve Option Class Registry Ranges**
...
<a name="RFC0768"></a> **[RFC0768]** J. Postel, "User Datagram Protocol", STD 6, RFC 768, DOI 10.17487/RFC0768, August 1980. <https://www.rfc-editor.org/info/rfc768> [Local MD](rfc0768.md)

<a name="RFC0792"></a> **[RFC0792]** J. Postel, "Internet Control Message Protocol", STD 5, RFC 792, DOI 10.17487/RFC0792, September 1981. <https://www.rfc-editor.org/info/rfc792> [Local MD](rfc0792.md)
...
<a name="RFC1191"></a> **[RFC1191]** J.C. Mogul, S.E. Deering, "Path MTU discovery", RFC 1191, DOI 10.17487/RFC1191, November 1990. <https://www.rfc-editor.org/info/rfc1191> [Local MD](rfc1191.md)

<a name="RFC2003"></a> **[RFC2003]** C. Perkins, "IP Encapsulation within IP", RFC 2003, DOI 10.17487/RFC2003, October 1996. <https://www.rfc-editor.org/info/rfc2003> [Local MD](rfc2003.md)
...
<a name="RFC6040"></a> **[RFC6040]** B. Briscoe, "Tunnelling of Explicit Congestion Notification", RFC 6040, DOI 10.17487/RFC6040, November 2010. <https://www.rfc-editor.org/info/rfc6040> [Local MD](rfc6040.md)

<a name="RFC6936"></a> **[RFC6936]** G. Fairhurst, M. Westerlund, "Applicability Statement for the Use of IPv6 UDP Datagrams with Zero Checksums", RFC 6936, DOI 10.17487/RFC6936, April 2013. <https://www.rfc-editor.org/info/rfc6936> [Local MD](rfc6936.md)

<a name="RFC7365"></a> **[RFC7365]** M. Lasserre, F. Balus, T. Morin, N. Bitar, Y. Rekhter, "Framework for Data Center (DC) Network Virtualization", RFC 7365, DOI 10.17487/RFC7365, October 2014. <https://www.rfc-editor.org/info/rfc7365> [Local MD](rfc7365.md)

<a name="RFC8085"></a> **[RFC8085]** L. Eggert, G. Fairhurst, G. Shepherd, "UDP Usage Guidelines", BCP 145, RFC 8085, DOI 10.17487/RFC8085, March 2017. <https://www.rfc-editor.org/info/rfc8085> [Local MD](rfc8085.md)
...
<a name="RFC8201"></a> **[RFC8201]** J. McCann, S. Deering, J. Mogul, R. Hinden, "Path MTU Discovery for IP version 6", STD 87, RFC 8201, DOI 10.17487/RFC8201, July 2017. <https://www.rfc-editor.org/info/rfc8201> [Local MD](rfc8201.md)

## Informative References

<a name="ETYPES"></a> **[ETYPES]** "IEEE 802 Numbers", <https://www.iana.org/assignments/ieee-802-numbers>

<a name="IANA-PR"></a> **[IANA-PR]** "Protocol Registries", <https://www.iana.org/protocols>

<a name="IANA-SN"></a> **[IANA-SN]** "Service Name and Transport Protocol Port Number Registry", <https://www.iana.org/assignments/service-names-port-numbers>

<a name="IEEE.802.1Q_2018"></a> **[IEEE.802.1Q_2018]** "IEEE Standard for Local and Metropolitan Area Networks--Bridges and Bridged Networks", July 2018. <http://ieeexplore.ieee.org/servlet/opac?punumber=8403925>
...
<a name="I-D.ietf-nvo3-dataplane-requirements"></a> **[NVO3-DATAPLANE]** N Bitar, M Lasserre, F Balus, T Morin, L Jin, B Khasnabish, "NVO3 Data Plane Requirements", Internet-Draft draft-ietf-nvo3-dataplane-requirements-03, April 2014. Work in Progress <https://tools.ietf.org/html/draft-ietf-nvo3-dataplane-requirements-03>

<a name="I-D.ietf-nvo3-encap"></a> **[NVO3-ENCAP]** S Boutros, "NVO3 Encapsulation Considerations", Internet-Draft draft-ietf-nvo3-encap-05, February 2020. Work in Progress <https://tools.ietf.org/html/draft-ietf-nvo3-encap-05>

<a name="RFC2983"></a> **[RFC2983]** D. Black, "Differentiated Services and Tunnels", RFC 2983, DOI 10.17487/RFC2983, October 2000. <https://www.rfc-editor.org/info/rfc2983> [Local MD](rfc2983.md)
...
<a name="RFC3552"></a> **[RFC3552]** E. Rescorla, B. Korver, "Guidelines for Writing RFC Text on Security Considerations", BCP 72, RFC 3552, DOI 10.17487/RFC3552, July 2003. <https://www.rfc-editor.org/info/rfc3552> [Local MD](rfc3552.md)

<a name="RFC3985"></a> **[RFC3985]** S. Bryant, P. Pate, "Pseudo Wire Emulation Edge-to-Edge (PWE3) Architecture", RFC 3985, DOI 10.17487/RFC3985, March 2005. <https://www.rfc-editor.org/info/rfc3985> [Local MD](rfc3985.md)
...
<a name="RFC8014"></a> **[RFC8014]** D. Black, J. Hudson, L. Kreeger, M. Lasserre, T. Narten, "An Architecture for Data-Center Network Virtualization over Layer 3 (NVO3)", RFC 8014, DOI 10.17487/RFC8014, December 2016. <https://www.rfc-editor.org/info/rfc8014> [Local MD](rfc8014.md)

<a name="RFC8086"></a> **[RFC8086]** L. Yong, E. Crabbe, X. Xu, T. Herbert, "GRE-in-UDP Encapsulation", RFC 8086, DOI 10.17487/RFC8086, March 2017. <https://www.rfc-editor.org/info/rfc8086> [Local MD](rfc8086.md)

<a name="RFC8293"></a> **[RFC8293]** A. Ghanwani, L. Dunbar, M. McBride, V. Bannai, R. Krishnan, "A Framework for Multicast in Network Virtualization over Layer 3", RFC 8293, DOI 10.17487/RFC8293, January 2018. <https://www.rfc-editor.org/info/rfc8293> [Local MD](rfc8293.md)

<a name="VL2"></a> **[VL2]** Greenberg, A., et al., "VL2: A Scalable and Flexible Data Center Network", August 2009. ACM SIGCOMM Computer Communication Review <https://dl.acm.org/doi/10.1145/1594977.1592576>

<a name="sec-9"></a>
# Acknowledgements

The authors wish to acknowledge Puneet Agarwal,
	David Black, Sami Boutros,
	  Scott Bradner, 
	Martín Casado, Alissa Cooper,
	  Roman Danyliw, Bruce Davie,
	    Anoop Ghanwani, Benjamin        Kaduk, Suresh Krishnan, Mirja Kühlewind, Barry Leiba,
	      Daniel Migault, Greg   Mirksy, Tal Mizrahi, 
	Kathleen Moriarty, Magnus    Nyström, Adam Roach, Sabrina    Tanamal, Dave Thaler, Éric Vyncke, 
	Magnus Westerlund, and many other members of the NVO3 Working Group for their reviews, comments, and suggestions.

The authors would like to thank Sam Aldrin,
	Alia Atlas, Matthew Bocci,
	  Benson Schliesser, and Martin Vigoureux 
...
- Address: 1 Microsoft Way, Redmond, WA, 98052, United States of America
- Email: pankajg@microsoft.com

**Chris Wright**

- Organization: Red Hat Inc.
- Address: 1801 Varsity Drive, Raleigh, NC, 27606, United States of America
...
- Address: 2200 Mission College Blvd., Santa Clara, CA, 95054, United States of America
- Email: ilango.s.ganga@intel.com

**T. Sridhar**

- Organization: VMware, Inc.
- Address: 3401 Hillview Ave., Palo Alto, CA, 94304, United States of America
- Email: tsridhar@utexas.edu
