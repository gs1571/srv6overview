...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-6man-hbh-processing-20
...
- [prev](https://datatracker.ietf.org/doc/draft-ietf-6man-hbh-processing-20)
- [alternate](https://dx.doi.org/10.17487/rfc9673)
...
Information about the current status of this document, any
            ...
            [https://www.rfc-editor.org/info/rfc9673](https://www.rfc-editor.org/info/rfc9673).
...
- [5. Hop-by-Hop Header Processing Procedures](#HBH)
  - [5.1. Processing the Extension Header Carrying Hop-by-Hop Options](#FAST)
    - [5.1.1. Configuration Enabling Hop-by-Hop Header Processing](#CONFIG)
  - [5.2. Hop-by-Hop Options Processing](#ONE)
    - [5.2.1. Router Alert Option](#RTRALERT)
    - [5.2.2. Configuration of Hop-by-Hop Options Processing](#CONFIG2)
- [6. Defining New Hop-by-Hop Options](#NEW)
  - [6.1. Example of Robust Usage](#HOW)
- [7. IANA Considerations](#IANA)
- [8. Security Considerations](#Security)
- [9. Normative References](#section-9)
...
This document specifies procedures for processing IPv6 Hop-by-Hop
  ...
  Protocol Specification [RFC8200](#RFC8200) to make processing of the IPv6 Hop-by-Hop Options header
  practical with the goal of making IPv6 Hop-by-Hop options useful to
  deploy and use at IPv6 routers and hosts.

An IPv6 packet includes Hop-by-Hop
  options by including a Hop-by-Hop
  Options header. The current list of defined Hop-by-Hop options can be found at [IANA-HBH](#IANA-HBH).
  ...
  that ought to be processed.

This document updates [RFC8200](#RFC8200).
...
This document uses the following loosely defined
   terms:
...
NOTE: [RFC6192](#RFC6192) is an example of how designs can
   separate Control Plane and Forwarding Plane
   functions. The separation between hardware and software processing
   described in [RFC6398](#RFC6398) does not apply to all router
   ...
   special processing for Hop-by-Hop options.
...
In early versions of the IPv6 protocol specification
    ...
    not be practical in current high speed routers, as observed in [RFC7045](#RFC7045): "it is to be expected that high-performance routers will
...
- Packets could include multiple Hop-by-Hop options. Too many
        ...
        that might need to be processed. Measurements [Cus23a](#Cus23a) show
...
[RFC6564](#RFC6564) specifies a uniform format for new IPv6 Extension
    Headers, and this update was incorporated into
    [RFC8200](#RFC8200) (note that
    [RFC8200](#RFC8200) obsoleted [RFC2460](#RFC2460)).

When the IPv6 protocol specification was updated and published in
    July 2017 as
    [RFC8200](#RFC8200), the procedures relating to Hop-by-Hop options
    were specified (paragraphs 5 and 6 of [RFC8200](#RFC8200)) as follows:
...
The changes meant that an implementation complied with the IPv6 protocol
    ...
    which Hop-by-Hop options they will process.

The text regarding the processing of Hop-by-Hop options in [RFC8200](#RFC8200) was not intended to change the processing of these
    options. It documented how they were being used in
    the Internet at the time RFC 8200 was published (see [RFC8200](#RFC8200)).  This was a constraint on
...
- Routers can be configured
	...
	router [RFC9098](#RFC9098) [RFC9288](#RFC9288).
...
- Including larger or multiple Hop-by-Hop options in a
        ...
        drop [RFC7872](#RFC7872). A larger Extension Header
...
- Any option that can be used to force packets into the
	...
	exploited as a DoS
        ...
        Router Alert Option [RFC2711](#RFC2711), which
	intentionally forwards packets to the Control Plane
        as discussed in [RFC6398](#RFC6398).
...
[RFC6398](#RFC6398) includes a summary of processing the IP Router Alert Option:
...
This is an example of the need to limit the resources that can
        ...
        where support for a function has not been configured.

There has been research that has discussed
    the general problem with dropping packets containing IPv6 Extension
    Headers, including the Hop-by-Hop Options header.
    For example, [Hendriks](#Hendriks) states that "Dropping all
    ...
    discussed in this section are further discussed in [HBH](#I-D.ietf-v6ops-hbh).

"Transmission and Processing of IPv6 Extension Headers" [RFC7045](#RFC7045) clarifies how intermediate nodes should process
    Extension Headers.  This document is generally consistent with
    [RFC7045](#RFC7045) and addresses an issue that was raised for
    discussion when [RFC2460](#RFC2460) was updated and replaced by
    [RFC8200](#RFC8200). This document updates [RFC8200](#RFC8200) as described in the next section and consequently
    clarifies the description in [RFC7045](#RFC7045),
    using the language of BCP 14 [RFC2119](#RFC2119) [RFC8174](#RFC8174).

This document defines a set of procedures for the Hop-by-Hop
    ...
    while others will only be processed within a limited domain [RFC8799](#RFC8799) (e.g., where a specific service is made available
...
This section describes several changes to [RFC8200](#RFC8200). [Section 5.1](#FAST) describes the processing of
    the Hop-by-Hop options Extension Header, and [Section 5.2](#ONE) describes
    the processing of individual Hop-by-Hop options.
    These sections update the text in paragraph 6 of
   [RFC8200](#RFC8200) and, as noted in [Section 5.2](#ONE), modify	 		
   [RFC8200](#RFC8200).
...
When a packet includes one or more Extension Headers, the Next Header
        ...
        It does not identify the transport protocol.

The Extension Header used to carry Hop-by-Hop options
       is defined in [RFC8200](#RFC8200) and is identified
       by a Next Header value of 0 in the IPv6
        header.  [RFC8200](#RFC8200) requires this Hop-by-Hop
        Options header to appear immediately after the IPv6 header.  [RFC8200](#RFC8200) also requires that a Hop-by-Hop Options header
        only appear at most once in a packet.

The Hop-by-Hop Options header as defined in
        [RFC8200](#RFC8200) can contain
        one or more Hop-by-Hop options.

Routers that process the Hop-by-Hop Options header **SHOULD** do
	...
	specification  (see [RFC9288](#RFC9288)).

Even if a router does not process the Hop-by-Hop Options
...
It is expected that the Hop-by-Hop Options header will be
...
<a name="CONFIG"></a>
### Configuration Enabling Hop-by-Hop Header Processing

[RFC8200](#RFC8200) allows a router to control its processing
...
A Source creating packets with a Hop-by-Hop Options header **SHOULD** use
           ...
           packets without the option(s) being processed (see [Section 6.1](#HOW)).
...
A router configuration needs to avoid vulnerabilities that
        ...
        also do not adversely impact the aggregate forwarding rate.

If a router is unable to process a specific Hop-by-Hop option
...
This document modifies this behavior for the "01", "10", and
	...
	specified for an unrecognized Option Type when the 
        ...
        node **MAY** send an ICMP Parameter Problem, Code 2 [RFC4443](#RFC4443),
	message to the packet's Source Address, pointing to the unrecognized Option Type.
...
When an ICMP Parameter Problem, Code 2, message is delivered to the
	...
	indicates that at least one node on the
        path has failed to recognize the option [RFC4443](#RFC4443).
	...
	and they are not always forwarded to the Source.
        ...
        forwards a packet without processing the list of all options.

<a name="RTRALERT"></a>
### Router Alert Option

The purpose of the Router Alert Option [RFC2711](#RFC2711) is to tell
...
The Router Alert Option includes a two-octet Value field that
	...
	current specified values can 
        be found in the "IPv6 Router Alert Option Values" IANA registry [IANA-RA](#IANA-RA).
...
DISCUSSION
 ...
 This processing causes concerns, which are discussed in [Section 4](#BACKGROUND).
 ...
 with [RFC6398](#RFC6398).

As noted in [RFC6398](#RFC6398), 
...
A node that is configured to process a Router Alert Option
        **MUST** protect itself from an infrastructure attack
        ...
        rate limiting of processing, or other methods [RFC6398](#RFC6398).

As specified in [RFC2711](#RFC2711), the top two bits of the Option Type
	...
	**SHOULD** verify that the Router Alert Option contains a
        ...
        [RFC6398](#RFC6398).  Otherwise, the router implementation **SHOULD** forward
        this packet subject to all normal policies and forwarding rules.

<a name="CONFIG2"></a>
### Configuration of Hop-by-Hop Options Processing

A router can be configured to process a specific Option.  The set
...
A possible approach to implementing this is to maintain a lookup
      ...
      option as described in [Section 5.1](#FAST) of this document.

The actions of the lookup table should be configurable by the operator of
    the router.

<a name="NEW"></a>
# Defining New Hop-by-Hop Options

This section updates [RFC8200](#RFC8200).

Any future new IPv6 Hop-by-Hop options should be designed to
...
If a new Hop-by-Hop option does not meet
    ...
    This is consistent with [RFC6564](#RFC6564).

The general issue of robust operation of packets with new
    Hop-by-Hop options is described in [Section 6.1](#HOW).

<a name="HOW"></a>
## Example of Robust Usage

Recent measurement
        surveys (e.g., [Cus23a](#Cus23a))
        ...
        to provide support for any required Hop-by-Hop options.

The primary motivation of this document is to make it
...
Methods can be developed that do not rely upon all routers
        to implement a specific Hop-by-Hop option (e.g., [RFC9268](#RFC9268))
...
For example, an application can be designed to first send
        ...
        carry application data that needs to be reliably delivered.

Since the set of nodes forming a path can change with time,
        ...
        is explained in [TAPS-ARCH](#I-D.ietf-taps-arch),
        and A/B protocol feature testing is described in [Tram17](#Tram17).

<a name="IANA"></a>
# IANA Considerations

This document updates the processing of
        Hop-by-Hop options.
        IANA has added this document as an
        additional reference for the "Destination Options and Hop-by-Hop Options" registry
        in the "Internet Protocol Version 6 (IPv6) Parameters" registry group [IANA-HBH](#IANA-HBH).

<a name="Security"></a>
# Security Considerations

Security issues caused by including IPv6 Hop-by-Hop options are well known and have
    been documented in several places, including
    [RFC6398](#RFC6398),
    [RFC6192](#RFC6192), [RFC7045](#RFC7045), and
    [RFC9098](#RFC9098).
    The main issue, as noted in [Section 4](#BACKGROUND), is that
    ...
    cause the router to fail or perform suboptimally.
...
Some IPv6 nodes implement features that access more of the protocol
    information than a typical IPv6 router (e.g., [RFC9098](#RFC9098)).
    ...
    on the requirements concerning Hop-by-Hop options.

Finally, this document notes that Internet protocol processing needs to be robust for
    ...
    fail gracefully when a malformed or malicious Hop-by-Hop option is encountered.
...
- A router configuration needs to avoid vulnerabilities that
            ...
            **SHOULD** behave in the same way specified for an unrecognized Option Type when the
            action bits are set to "00", as specified in [Section 5.2](#ONE).
- This document adds criteria for the Router Alert Option ([Section 5.2.1](#RTRALERT))
...
The intent of this document is to highlight that these changes significantly reduce the
    ...
    and enable Hop-by-Hop options to be safely used in the Internet.
...
Helpful comments were received from Brian     Carpenter, Ron Bonica, Ole     Troan, Mike Heard, Tom     Herbert, Cheng Li, Éric     Vyncke, Greg Mirsky, Xiao     Min, Fernando Gont, Darren     Dukes, Peng Shuping, Dave     Thaler, Ana Custura, Tim     Winters, Jingrong Xie, Lorenzo     Colitti, Toerless Eckert, Suresh Krishnan, Mikael Abrahamsson,
    Adrian Farrel, Jie Dong,
    Jen Linkova, Erik Kline, and
...
- Address: 100 Oracle Parkway, Suite 800, Redwood City, CA, 94065, United States of America
- Email: bob.hinden@gmail.com

**Godred Fairhurst**

- Organization: University of Aberdeen
- Address: School of Engineering, Aberdeen, AB24 3UE, United Kingdom
- Email: gorry@erg.abdn.ac.uk
