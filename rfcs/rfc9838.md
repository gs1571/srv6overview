...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-ipsecme-g-ikev2-23
...
**Keywords:** multicast, security, IPsec, GDOI, MSEC

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-ipsecme-g-ikev2-23)
- [alternate](https://dx.doi.org/10.17487/rfc9838)
...
Information about the current status of this document, any
            ...
            [https://www.rfc-editor.org/info/rfc9838](https://www.rfc-editor.org/info/rfc9838).
...
- [1. Introduction and Overview](#section-1)
  - [1.1. Requirements Notation](#section-1.1)
  - [1.2. Terminology](#terms)
- [2. G-IKEv2 Protocol](#section-2)
  - [2.1. G-IKEv2 Integration into the IKEv2 Protocol](#section-2.1)
    - [2.1.1. G-IKEv2 Transport and Port](#section-2.1.1)
  - [2.2. G-IKEv2 Payloads](#section-2.2)
  - [2.3. G-IKEv2 Member Registration and Secure Channel Establishment](#registration)
    - [2.3.1. GSA_AUTH Exchange](#gsa_auth)
    - [2.3.2. GSA_REGISTRATION Exchange](#gsa_registration)
    - [2.3.3. GM Registration Operations](#section-2.3.3)
    - [2.3.4. GCKS Registration Operations](#section-2.3.4)
  - [2.4. Group Maintenance Channel](#section-2.4)
    - [2.4.1. GSA_REKEY](#gsa_rekey)
    - [2.4.2. GSA_INBAND_REKEY Exchange](#gsa_inband_rekey)
    - [2.4.3. Deletion of SAs](#deletion)
  - [2.5. Counter-Based Modes of Operation](#counter-modes)
    - [2.5.1. Allocation of Sender-ID](#sid_alloc)
    - [2.5.2. GM Usage of Sender-ID](#sid-usage)
  - [2.6. Replay Protection for Multicast Data-Security SAs](#seqnum)
  - [2.7. Encryption Transforms with Implicit IV](#implicit-iv)
- [3. Group Key Management and Access Control](#key_management)
  - [3.1. Key Wrap Keys](#kwk)
    - [3.1.1. Default Key Wrap Key](#sk_w)
  - [3.2. GCKS Key Management Semantics](#key_gcks_semantics)
    - [3.2.1. Forward Access Control Requirements](#section-3.2.1)
  - [3.3. GM Key Management Semantics](#keys_gm_semantics)
  - [3.4. SA Keys](#group_sa_keys)
- [4. Header and Payload Formats](#header_payload)
  - [4.1. G-IKEv2 Header](#header)
  - [4.2. Group Identification Payload](#idg_payload)
  - [4.3. Security Association - GM Supported Transforms Payload](#sag_payload)
  - [4.4. Group Security Association Payload](#gsa_payload)
    - [4.4.1. Group Policies](#group_policy)
    - [4.4.2. Group Security Association Policy Substructure](#gsa_policy)
    - [4.4.3. Group-Wide Policy Substructure](#gw_policy)
  - [4.5. Key Download Payload](#kd_payload)
    - [4.5.1. Key Bags](#key_bag)
    - [4.5.2. Group Key Bag Substructure](#group_key_bag)
    - [4.5.3. Member Key Bag Substructure](#member_key_bag)
    - [4.5.4. Key Wrapping](#wrapped_key)
  - [4.6. Delete Payload](#delete)
  - [4.7. Notify Payload](#notify)
    - [4.7.1. INVALID_GROUP_ID Notification](#inv_gr_id)
    - [4.7.2. AUTHORIZATION_FAILED Notification](#autz_failed)
    - [4.7.3. REGISTRATION_FAILED Notification](#reg_failed)
    - [4.7.4. GROUP_SENDER Notification](#sender)
  - [4.8. Authentication Payload](#section-4.8)
- [5. Using G-IKEv2 Attributes](#restrictions)
- [6. Interaction with IKEv2 and ESP Extensions](#ike_ext)
  - [6.1. Implicit IV for Counter-Based Ciphers in ESP](#section-6.1)
  - [6.2. Mixing Preshared Keys in IKEv2 for Post-Quantum Security](#section-6.2)
  - [6.3. Aggregation and Fragmentation Mode for ESP](#section-6.3)
- [7. GDOI Protocol Extensions](#gdoi_ext)
- [8. Security Considerations](#section-8)
  - [8.1. GSA Registration and Secure Channel](#section-8.1)
  - [8.2. GSA Maintenance Channel](#section-8.2)
    - [8.2.1. Authentication/Authorization](#section-8.2.1)
    - [8.2.2. Confidentiality](#section-8.2.2)
    - [8.2.3. On-Path Attack Protection](#section-8.2.3)
    - [8.2.4. Replay/Reflection Attack Protection](#section-8.2.4)
- [9. IANA Considerations](#IANA)
  - [9.1. New Registries](#section-9.1)
    - [9.1.1. Guidance for Designated Experts](#section-9.1.1)
  - [9.2. Changes in the Existing IKEv2 Registries](#section-9.2)
- [10. References](#section-10)
  - [10.1. Normative References](#section-10.1)
  - [10.2. Informative References](#section-10.2)
- [Appendix A. Use of LKH in G-IKEv2](#lkh_key_management)
  - [A.1. Notation](#lkh_notation)
  - [A.2. Group Creation](#section-appendix.a.2)
  - [A.3. Simple Group SA Rekey](#section-appendix.a.3)
  - [A.4. Group Member Exclusion](#section-appendix.a.4)
...
This document presents an extension to IKEv2
      [RFC7296](#RFC7296) called G-IKEv2, which accommodates group key management. A group key management protocol provides IPsec keys and policy to a
      set of IPsec devices that are authorized to communicate using a Group
      Security Association (GSA) defined in Multicast Group Security Architecture [RFC3740](#RFC3740).
      The data communications within the group (e.g., IP multicast packets)
      are protected by a key pushed to the Group Members (GMs) by the Group Controller/Key Server (GCKS).

G-IKEv2 conforms to "The Multicast Group Security
      Architecture" [RFC3740](#RFC3740), "Multicast Extensions to the
      Security Architecture for the Internet Protocol" [RFC5374](#RFC5374),
      and "Multicast Security (MSEC) Group Key Management Architecture" [RFC4046](#RFC4046).
      G-IKEv2 replaces "The Group Domain of Interpretation" [RFC6407](#RFC6407), which defines a
      similar group key management protocol using IKEv1 [RFC2409](#RFC2409) (since deprecated by IKEv2). When G-IKEv2 is
...
G-IKEv2 defines two ways to perform registration. When a GM first
      contacts a GCKS, it uses the GSA_AUTH exchange ([Section 2.3.1](#gsa_auth)) to register to a group. This exchange happens after
      ...
      exchange type is the GSA_REGISTRATION exchange ([Section 2.3.2](#gsa_registration)), which can be used by the GM within the already-established
...
Refreshing the group keys can be performed either in a unicast mode via the
      GSA_INBAND_REKEY exchange ([Section 2.4.2](#gsa_inband_rekey)) performed over a specific IKE SA between a GM and a GCKS 
      or in a multicast mode with the GSA_REKEY pseudo-exchange ([Section 2.4.1](#gsa_rekey)) when new keys are being distributed to all GMs.
...
Large and small groups may use different sets of these mechanisms.
      ...
      use the GSA_REKEY message for efficiency. This is shown in [Figure 1](#large-groups), where multicast communications are indicated with a double line.
...
```
A combination of these approaches is also possible. For example,  
...
It is assumed that readers are familiar with the IPsec
        architecture [RFC4301](#RFC4301) and its extension for multicast
        [RFC5374](#RFC5374). This document defines an extension to the
        IKEv2 protocol [RFC7296](#RFC7296) and skips many of its
        details. The notation and conventions from [RFC7296](#RFC7296)
        are used for describing G-IKEv2 payloads and exchanges.

The following key terms are used throughout this document (mostly
        borrowed from [RFC3740](#RFC3740), [RFC5374](#RFC5374), and [RFC6407](#RFC6407)).

**Group:**
  A set of IPsec devices that communicate to each other using multicast.

**Group Member (GM):**
  An IPsec device that belongs to a group.  A GM is
...
**Data-Security SA:**
  A multicast SA between each multicast sender and the group's receivers.
            ...
            This specification relies on Encapsulating Security Payload (ESP) and Authentication Header (AH) as protocols for Data-Security SAs.
...
**Group Security Association (GSA):**
  A collection of Data-Security SAs and Rekey SAs
            necessary for a GM to receive key updates.
            A GSA describes the working policy for a group.  Refer to the MSEC Group Key Management Architecture [RFC4046](#RFC4046)
...
**Key Encryption Key (KEK):**
  The symmetric key (or a set of keys) used in a Rekey SA to protect its messages. The set of keys may include keys 
...
**Logical Key Hierarchy (LKH):**
  A group management method defined in [RFC2627](#RFC2627).


# G-IKEv2 Protocol

G-IKEv2 is an extension to the IKEv2 protocol [RFC7296](#RFC7296) that provides group authorization, 
        secure policy, and keys download from the GCKS to GMs.

## G-IKEv2 Integration into the IKEv2 Protocol

G-IKEv2 is compatible with most IKEv2 extensions defined so far (see [Section 6](#ike_ext) for details). 
        In particular, it is assumed that, if necessary, the IKE_INTERMEDIATE exchanges [RFC9242](#RFC9242) may be utilized 
        ...
        special handling when used with G-IKEv2.

### G-IKEv2 Transport and Port

As an IKEv2 extension, G-IKEv2 **SHOULD** use the IKEv2 ports (500, 4500). 
G-IKEv2 **MAY** use TCP transport for the IKE SA used for registration (which is unicast),
   as defined in TCP Encapsulation of IKEv2 and IPsec [RFC9329](#RFC9329).
          G-IKEv2 **MAY** also use UDP port 848, the same as Group Domain of Interpretation (GDOI) [RFC6407](#RFC6407), because they serve a similar function. 
          The version number in the IKE header distinguishes the G-IKEv2
          protocol from the GDOI protocol [RFC6407](#RFC6407).

[RFC7296](#RFC7296) describes how IKEv2 supports paths with NATs.
          ...
          this section regarding the IKE SA **MUST** be honored.
          The behavior of GMs and GCKS **MUST NOT** depend on the
...
**Table: Payloads Used in G-IKEv2**

| Notation | Payload | Defined in |
| --- | --- | --- |
| AUTH | Authentication | [RFC7296](#RFC7296) |
| CERT | Certificate | [RFC7296](#RFC7296) |
| CERTREQ | Certificate Request | [RFC7296](#RFC7296) |
| D | Delete | [RFC7296](#RFC7296) |
| GSA | Group Security Association | [Section 4.4](#gsa_payload) |
| HDR | IKE header (not a payload) | [RFC7296](#RFC7296) |
| IDg | Group Identification | [Section 4.2](#idg_payload) |
| IDi | Identification - Initiator | [RFC7296](#RFC7296) |
| IDr | Identification - Responder | [RFC7296](#RFC7296) |
| KD | Key Download | [Section 4.5](#kd_payload) |
| KE | Key Exchange | [RFC7296](#RFC7296) |
| Ni, Nr | Nonce | [RFC7296](#RFC7296) |
| N | Notify | [RFC7296](#RFC7296) |
| SA | Security Association | [RFC7296](#RFC7296) |
| SAg | Security Association - GM Supported Transforms | [Section 4.3](#sag_payload) |
| SK | Encrypted and Authenticated (also known as Encrypted) | [RFC7296](#RFC7296) |

Payloads defined as part of other IKEv2 extensions **MAY** also be included in 
...
**Group Identification (IDg):**
  The GM requests the GCKS for membership
            into the group by sending its IDg payload.

**Security Association - GM Supported
          Transforms (SAg):**
  The GM optionally sends supported transforms so
            that GCKS may select a policy appropriate for all members of the
            group (which is not negotiated, unlike SA parameters in IKEv2).

**Group Security Association (GSA):**
  The GCKS sends the
            group policy to the GM using this payload.

**Key Download (KD):**
  The GCKS sends the keys and the
            security parameters to the GMs using this payload.


The details of the contents of each payload are described in [Section 4](#header_payload).

<a name="registration"></a>
## G-IKEv2 Member Registration and Secure Channel Establishment

Initial registration is combined with establishing a secure connection between the 
        ...
        based on the IKEv2 Intermediate Exchange [RFC9242](#RFC9242) 
        are used. Each exchange consists of request/response pairs. The first exchange, called
        IKE_SA_INIT, is defined in IKEv2 [RFC7296](#RFC7296). 
...
This
        ...
        See [Section 4.5.4](#wrapped_key) for details.

The second exchange, called GSA_AUTH, is similar to the IKEv2 IKE_AUTH exchange [RFC7296](#RFC7296).
...
The established secure channel between the GM and the GCKS is in fact IKE SA (as defined in 
        [RFC7296](#RFC7296)) and is referred to as such throughout this document. 
        However, it is **NOT RECOMMENDED** to use this IKE SA for the purpose of creating
...
The GSA_AUTH exchange is used to authenticate the previous exchanges and
...
```
A GM initiates a GSA_AUTH request to join a group indicated 
          ...
          **MUST** include a Notify payload status type of GROUP_SENDER,
...
```
The GCKS responds with IDr, optional CERT, and AUTH payloads 
...
Possible errors should be handled in accordance with [RFC7296](#RFC7296).
          ...
          etc. In these cases (see [Section 4.7](#notify)), the GSA_AUTH
          ...
          Other types of error notifications can be INVALID_GROUP_ID, AUTHORIZATION_FAILED, or REGISTRATION_FAILED.
...
```
If the GSA_AUTH exchange is completed successfully but 
          the GM finds that the policy sent by the GCKS is
          unacceptable, the member **SHOULD** inform the GCKS about this by initiating the GSA_REGISTRATION exchange
          with the IDg payload and the NO_PROPOSAL_CHOSEN notification 
          (see [Figure 8](#gsa_registration_gm_error)).

<a name="gsa_registration"></a>
### GSA_REGISTRATION Exchange

Once the IKE SA between the GM and the GCKS is established,
          ...
          and processed as defined in [Section 2.3.1](#gsa_auth).
...
```
As with GSA_AUTH exchange, the GCKS can reject the
          registration request when the IDg is invalid or authorization fails,
          ...
          as described in [Section 2.3.1](#gsa_auth).
...
```
This exchange can also be used if the GM finds that the
          policy sent by the GCKS is unacceptable or wants to leave 
          the group for some reason. The GM **SHOULD**
          notify the GCKS by sending IDg and the Notify type
          NO_PROPOSAL_CHOSEN or REGISTRATION_FAILED as shown below. 
          In this case, the GCKS **MUST** remove the GM from the group denoted in IDg.
...
```
### GM Registration Operations

A GM requesting registration contacts the
...
Next, the GM sends the GSA_AUTH request message with the IKEv2 payloads
          ...
          join. A GM intending to emit data traffic **MUST** send a
          ...
          Sender-ID values in case the Data-Security SA supports a counter-mode cipher. [Section 2.5.1](#sid_alloc) includes guidance
          on requesting Sender-ID values.

A GM may be limited in the Transforms IDs that it is
          ...
          (AH [RFC4302](#RFC4302) and/or ESP [RFC4303](#RFC4303)) SAs 
          ...
          protocol (AH, ESP, GIKE_UPDATE) and are defined in [Table 2](#allowed_transforms).
          Other Transform Types **SHOULD NOT** be included as they will be ignored by the GCKS.
...
Generally, a single Proposal for each protocol (GIKE_UPDATE,
          ...
          particular, the restriction from  [RFC7296](#RFC7296) that Authenticated Encryption with Associated Data (AEAD) and non-AEAD
...
The Proposal Num field in the Proposal substructure is treated specially in the SAg payload:
...
Although the SAg payload is optional, it is **RECOMMENDED** that the GM include
          this payload into the GSA_AUTH request to allow the GCKS to select an appropriate policy.

A GM **MAY** also indicate the support for IPcomp by including one or more the IPCOMP_SUPPORTED
...
Upon receiving the GSA_AUTH response, the GM parses the
...
This policy describes zero or more Data-Security SAs (TEK), zero or one Rekey SA (KEK), and zero or one GW policy (although at least one TEK or KEK policy **MUST** be
   Present).
          ...
          with a NO_PROPOSAL_CHOSEN Notify payload (see [Section 2.3.2](#gsa_registration)). 
...
If the GM is not a sender for a received Data-Security SA, 
          then it **MUST** install this SA only in the inbound direction.
          If the GM is a sender for a received Data-Security SA, 
          and it is not going to receive back the data it sends, 
          then it **MUST** install this SA only in the outgoing direction.
...
If the first Message ID the GM should expect to receive is non-zero,
          ...
          considered stale and **MUST** be ignored. This could happen if two GSA_AUTH
...
Once a GM successfully registers to the group, it **MUST** replace
...
Once a GM has received the GIKE_UPDATE policy during a registration,
          the IKE SA **MAY** be closed.  By convention, the GCKS
          closes the IKE SA; the GM **SHOULD NOT** close it.  The
          GCKS **MAY** choose to keep the IKE SA open for inband
          ...
          IKEv2 mechanism described in [RFC7296](#RFC7296).  If for some reason the
          IKE SA is closed and no GIKE_UPDATE policy is received during the
          registration process, the GM **MUST** consider itself
          excluded from the group.  To continue participating in the group,
          the GM needs to re-register.

### GCKS Registration Operations

A G-IKEv2 GCKS listens for incoming requests from GMs. When the GCKS receives an IKE_SA_INIT request, it selects
          an IKE proposal and generates a nonce and Diffie-Hellman (DH) to include in the
          IKE_SA_INIT response.

Upon receiving the GSA_AUTH request, the GCKS authenticates the
...
If the GCKS policy includes a
          ...
          TEK and keys are included in the KD TEK. The GW policy **MAY** also be
          included to provide the Activation
   Time Delay (ATD) and/or Deactivation Time Delay (DTD) ([Section 4.4.3.1.1](#gwp_attr_atd_dtd)) 
          ...
          of operation, the GCKS responds with one or more Sender-ID values (see [Section 2.5](#counter-modes)).

Multicast Extensions to the Security Architecture [RFC5374](#RFC5374) defines two modes of operation for multicast 
...
If the GCKS receives a GSA_REGISTRATION exchange with a request
...
If a GM includes an SAg in its GSA_AUTH or
          GSA_REGISTRATION request, the GCKS may evaluate it according to an
          implementation-specific policy.

- The GCKS could evaluate the list of Transforms and compare it
              ...
              then the GCKS **SHOULD** return a NO_PROPOSAL_CHOSEN notification.
              Alternatively, the GCKS can change the group policy as defined below.
...
- The GCKS could store the list of Transforms and adjust the
...
Depending on its policy, the GCKS may have no further need for the
          ...
          or Notify (e.g., NO_PROPOSAL_CHOSEN) and the GCKS is not intended to use the SA, then the GCKS **SHOULD** close the IKE SA to save resources after a short period of time.
...
**GSA_REKEY:**
  The GSA_REKEY is a pseudo-exchange, consisting
            ...
            policy changes from the GCKS via the GSA_REKEY messages.

**GSA_INBAND_REKEY:**
  The GSA_INBAND_REKEY is a
            ...
            is useful when G-IKEv2 is used with a small group of cooperating devices.


Depending on its policy, the GCKS **MAY** combine these two methods.
...
The GCKS initiates the G-IKEv2 rekey by sending a protected
          ...
          the windowing mechanism described in [RFC7296](#RFC7296) **MUST NOT**
          ...
          the Key Download payload (defined in [Section 4.5.3.3](#mkd_attr_gm_sid)) **MUST NOT** be part of the
...
```
HDR is defined in [Section 4.1](#header). While GSA_REKEY reuses the IKEv2 header,
          ...
          in the GSA payload (see [Section 4.4.2](#gsa_policy)).
          ...
          GSA_REKEY message is sent to the GMs.

The GSA payload contains the current policy for rekey and Data-Security SAs.

          The GSA may contain a new Rekey SA and/or a new Data-Security SA(s) 
          ([Section 4.4](#gsa_payload)).

The KD payload contains the keys for the policy included in the
...
A Delete payload **MAY** be included to instruct the GM to delete
          existing SAs. See [Section 4.6](#delete) for more detail.
...
The AUTH payload **MUST** be included to authenticate the GSA_REKEY
          ...
          and **MUST NOT** be included if authentication is implicit.
          ...
          For this reason, using implicit authentication for GSA_REKEY is **NOT RECOMMENDED** 
          unless source origin authentication is not required (for example, in a small group of
          highly trusted GMs). See more about authentication methods in [Section 4.4.2.1.1](#auth_method).

During GM registration, the GCKS
	  ...
	  be the same
          ...
          is used, then AUTH_KEY **MUST NOT** be sent to GMs.

<a name="gsa_rekey_auth"></a>
#### GSA_REKEY Message Authentication

The content of the AUTH payload generally depends on the authentication method from the Group Controller Authentication Method (GCAUTH) transform 
            ([Section 4.4.2.1.1](#auth_method)). 
This specification defines the use of only one authentication method, Digital Signature, and the AUTH payload contains a digital signature calculated over the content of the not-yet-encrypted GSA_REKEY message.
...
Chunk P consists of the not-yet-encrypted content of the Encrypted payload, excluding 
            the Initialization Vector, the Padding, the Pad Length, and the Integrity Checksum Data fields (see [RFC7296](#RFC7296) for the description 
            of the Encrypted payload). In other words, chunk P is the inner payloads of the Encrypted payload in plaintext form. 
            [Figure 10](#auth_data) illustrates the layout of chunks P and A in the GSA_REKEY message.

Before the calculation of the AUTH payload, the inner payloads
...
For the purpose of the AUTH payload calculation, the Length field in the IKE header and the Payload Length 
            ...
            [Figure 10](#auth_data)) is set to the sum of the lengths of A and P, and the Payload Length
            field in the Encrypted Payload header (denoted as AdjustedPldLen in 
            [Figure 10](#auth_data)) is set to the length of P plus the size of the Payload header (four octets).
...
```
The authentication data is calculated using the authentication algorithm from the Group Controller Authentication Method transform 
            ([Section 4.4.2.1.1](#auth_method)) and the current authentication key provided in the AUTH_KEY attribute ([Section 4.5.3.2](#mkd_attr_auth_key)). 
...
IKEv2 fragmentation [RFC7383](#RFC7383) can be used to
...
- The GCKS must always use IKE fragmentation based on a
                ...
                messages and waiting for response. [RFC7383](#RFC7383) contains recommendations on selecting the
                fragmentation threshold.
- The Path MTU (PMTU) mechanism, defined in [RFC7383](#RFC7383),
                cannot be used due to lack of GSA_REKEY response messages.

The calculation of authentication data **MUST** be applied to whole messages only before possible IKE Fragmentation. 
...
#### GSA_REKEY GCKS Operations

The GCKS builds the rekey message with a Message ID value that
            is one greater than the value included in the previous rekey message. 
            The first message sent over a new Rekey SA **MUST** use a Message ID of 0. 
            The GSA, KD, and N payloads follow with the
            same characteristics as in the GSA Registration exchange. 
            The AUTH payload (if present) is created as defined in [Section 2.4.1.1](#gsa_rekey_auth).
...
Because GSA_REKEY messages are not acknowledged and could be
            ...
            policy. The (encrypted) retransmitted messages **MUST** be bitwise identical and should be
...
GCKS may also include one or several GSA_NEXT_SPI
            ...
            from this situation. See [Section 4.4.2.2.3](#gsa_attr_next_spi) for more detail.

#### GSA_REKEY GM Operations

When a GM receives the rekey message from the GCKS, it
            ...
            the parsing done in the registration exchange.

Replay protection is achieved by a GM rejecting a
...
If the GSA payload includes a Data-Security SA using cipher in 
            ...
            it **MUST NOT** install the Data-Security SAs. It **MUST** initiate a re-registration
            to the GCKS in order to obtain a Sender-ID value (along with
            the current group policy).

Once a new Rekey SA is installed as a result of a GSA_REKEY
            message, the current Rekey SA (over which the message was received)
            **MUST** be silently deleted after waiting the DEACTIVATION_TIME_DELAY interval 
            ...
            (identified by the Protocol and SPI fields in the Delete payload) **MUST** be silently deleted 
            after waiting the DEACTIVATION_TIME_DELAY interval regardless of its expiration time.

If a Data-Security SA is not rekeyed yet and is 
            about to expire (a "soft lifetime" expiration is described
            in [RFC4301](#RFC4301)), the GM **SHOULD**
...
```
Because this is a normal IKEv2 exchange, the HDR is treated as defined
          in IKEv2 [RFC7296](#RFC7296).

#### GSA_INBAND_REKEY GCKS Operations

The GSA, KD, and N payloads are built in the same manner as in
            a registration exchange.

#### GSA_INBAND_REKEY GM Operations

The GM processes the GSA, KD, and N payloads in the same manner
            as if they were received in a registration exchange.

<a name="deletion"></a>
### Deletion of SAs

There are occasions when the GCKS may want to signal to GMs to delete policy when the application sending data traffic has ended or if group
          policy has changed. Deletion of SAs is accomplished by sending
          the Delete Payload described in [RFC7296](#RFC7296) 
          as part of the GSA_REKEY pseudo-exchange as shown below.
...
```
If GCKS has a unicast SA with a GM, then it can use the GSA_INBAND_REKEY
...
```
There may be circumstances where the GCKS may want to start over 
          ...
          with an SPI of zero and a Protocol ID of GIKE_UPDATE.

If a GM receives a Delete payload with zero SPI and a Protocol ID
          ...
          In this situation, the GM **MUST** re-register if it wants to continue 
          participating in this group. The registration is performed as described
          in [Section 2.3](#registration). It is **RECOMMENDED** that a GM waits some randomly chosen time
          ...
          to re-register. However, after excluding a GM from the group, the GCKS **MAY** 
          immediately delete the unicast SA with this GM (if any) if the credentials of this GM are revoked.

<a name="counter-modes"></a>
## Counter-Based Modes of Operation

Several counter-based modes of operation have been specified
        for ESP (e.g., AES-CTR [RFC3686](#RFC3686), AES-GCM [RFC4106](#RFC4106), AES-CCM [RFC4309](#RFC4309),
        ChaCha20-Poly1305 [RFC7634](#RFC7634), and
        AES-GMAC [RFC4543](#RFC4543)) and AH (e.g., AES-GMAC [RFC4543](#RFC4543)). These counter-based modes require that no
...
- The GCKS uses the method described in [RFC6054](#RFC6054), 
        which assigns each sender a portion of the IV space by provisioning each 
        sender with one or more unique Sender-ID values.

<a name="sid_alloc"></a>
### Allocation of Sender-ID
...
During registration, the group sender can choose to request one
          ...
          sender. A group sender **MUST** request as many Sender-ID values matching the number
...
When the group policy includes a counter-based mode of operation,
...
1. A GCKS maintains a Sender-ID counter, which records the Sender-IDs that
...
2. Each time a Sender-ID is allocated, the current value of the
          ...
          is then incremented in preparation for the next allocation.
3. When the GCKS specifies a counter-based mode of operation in
          ...
          requested by the GM.
4. A GCKS allocates new Sender-ID values for each registration operation 
          ...
          will use a unique set of counter-based mode nonces.
5. When the Sender-ID counter maintained by the GCKS reaches its final
          Sender-ID value, no more Sender-ID values can be distributed. Before
          distributing any new Sender-ID values, the GCKS **MUST** 
          exclude all GMs from the group as described
          in [Section 2.4.3](#deletion). This will result in the GMs 
          ...
          the new Data-Security SAs.

<a name="sid-usage"></a>
### GM Usage of Sender-ID

A GM applies the Sender-ID to Data-Security SAs as follows:

- The most significant bits of the IV indicated in the GWP_SENDER_ID_BITS attribute ([Section 4.4.3.1.2](#gwp_attr_sid_bits)) are
          taken to be the Sender-ID field of the IV.
- The Sender-ID is placed in the least significant bits of the Sender-ID
...
<a name="seqnum"></a>
## Replay Protection for Multicast Data-Security SAs

IPsec provides anti-replay service as part of its security
        services.  With multicast extensions for IPsec, replay protection is not
        always possible to achieve (see [RFC3740](#RFC3740)). In particular, if there
        ...
        (see [RFC4303](#RFC4303) and [RFC4302](#RFC4302)), thus making it impossible
        ...
        [Section 4.4.2.1.3](#antireplay)). The GCKS **MAY** create
...
IPsec architecture assumes that whether 
...
The GCKS **MUST** include the Sequence Numbers transform in the GSA payload for every Data-Security SA.
        See [Section 4.4.2.1.3](#antireplay) for more details.

When a Data-Security SA has a single sender, the GCKS **MUST** 
        be configured to rekey the SA frequently enough so that the 32-bit sequence numbers do not wrap.

<a name="implicit-iv"></a>
## Encryption Transforms with Implicit IV

The "Transform Type 1 - Encryption Algorithm Transform IDs" IANA registry [IKEV2-IANA](#IKEV2-IANA) defines several transforms
        with implicit IV. These transforms rely on ESP Sequence Numbers for constructing IV 
        (see [RFC8750](#RFC8750) for details).
        It requires anti-replay service to be enabled for an ESP SA using these encryption transforms.
        Unless the properties of sequence numbers for a multicast ESP SA include their uniqueness (see [Section 2.6](#seqnum)), 
        encryption transforms that rely on Sequence Numbers for IV construction **MUST NOT** be used.
        In any case, such transforms **MUST NOT** be used for any G-IKEv2 SA (both unicast and multicast).

<a name="key_management"></a>
# Group Key Management and Access Control
...
Through the G-IKEv2 rekey, G-IKEv2 supports algorithms such as
      ...
      Secrecy (PFS) property as defined in [RFC7296](#RFC7296).

Group management algorithms providing forward and backward access
   control other than LKH have also been proposed,
   for example, OFT [OFT](#OFT) and Subset Difference [NNL](#NNL). These algorithms could be
      used with G-IKEv2 but are not specified as a part of this
      document.

This specification assumes that all group keys, that are 
...
Every GM always knows at least one KWK -- the KWK that is associated with the IKE SA
        or multicast Rekey SA over which wrapped keys are sent.
        In this document, it is called default KWK and is denoted as "GSK_w".

For the purpose of forward access control, the GCKS may provide each GM 
...
Each KWK is associated with a key wrap algorithm specified in the Key Wrap Algorithm transform.
        The size of these KWKs is determined by the key wrap algorithm used,
        but it **SHOULD NOT** be less than the size of the key for the Encryption Algorithm transform
        for the Rekey SA and for all Data-Security SAs in the group (taking the Key Length attribute into consideration if it is present).

<a name="sk_w"></a>
### Default Key Wrap Key

The default KWK (GSK_w) is only used in the context of a single IKE SA.
...
For the multicast Rekey SA, the GSK_w is provided along with
          other SA keys as defined in [Section 3.4](#group_sa_keys).

<a name="key_gcks_semantics"></a>
## GCKS Key Management Semantics

The Wrapped Key Download method allows the GCKS to employ various key management methods.

**A simple key management method:**
  The GCKS always sends
            group SA keys encrypted with the GSK_w.

**An LKH key management method:**
  The GCKS provides each
...
### Forward Access Control Requirements
...
If forward access control is a desired property of the group,
          a new TEK policy and the associated keys **MUST NOT** be included 
          ...
          be visible to the members who were denied access.

If forward access control policy for the group includes
...
If other methods of using LKH or other group management
          ...
          policy is maintained within a single G-IKEv2 rekey message.

<a name="keys_gm_semantics"></a>
## GM Key Management Semantics

This specification defines GM Key Management semantics
        ...
        The GM behavior is the same for all of these policies.

All keys in G-IKEv2 are transmitted in encrypted form as specified 
        in [Section 4.5.4](#wrapped_key). This format includes a 32-bit Key ID
...
When a GM receives a message from the GCKS installing the new Data-Security or Rekey SA,
...
For a multicast Rekey SA, multiple SA_KEY attributes may be present 
        ...
        are present, then all of them **MUST** contain the same keying material encrypted using different KWKs.
        ...
        or were received in previous messages.

We will use the term "Key Path" to describe an ordered sequence of keys 
...
In the simplest case, the SA_KEY attribute is encrypted
...
- If the new Key Path is empty, then no actions are needed. This may happen 
          if no WRAP_KEY attributes from the received message were used.
- If the new Key Path is non-empty and it ends with the default KWK (GSK_w), then the whole new 
...
- In all other cases, the new Key Path will end at some intermediate key from the GM's current Working Key Path. 
...
[Appendix A](#lkh_key_management) contains an example of how this algorithm works in case of LKH key management method.

<a name="group_sa_keys"></a>
## SA Keys

The keys that are used for Data-Security SAs or a Rekey SA (called
...
For a Data-Security SA, the keys are taken in accordance to the
        third bullet from [RFC7296](#RFC7296). In particular, for the ESP and AH SAs, the encryption
        key (if any) **MUST** be taken from the leftmost bits of
        the keying material and the integrity key (if any) **MUST**
...
The G-IKEv2 is an IKEv2 extension and thus inherits its wire format 
      ...
      Group Identification (IDg) ([Section 4.2](#idg_payload)), Security Association - GM Supported Transforms (SAg) ([Section 4.3](#sag_payload)),
      Group Security Association (GSA) ([Section 4.4](#gsa_payload)), and Key Download (KD) ([Section 4.5](#kd_payload)). 
      The G-IKEv2 header ([Section 4.1](#header)), IDg payload, and SAg payload reuse the IKEv2 format for the IKEv2 header, IDi/IDr payloads, and SA payload, respectively. New exchange types GSA_AUTH, GSA_REGISTRATION, GSA_REKEY, and GSA_INBAND_REKEY are
      also added.

This section describes new payloads and the differences in the processing 
      of existing IKEv2 payloads.

<a name="header"></a>
## G-IKEv2 Header

G-IKEv2 uses the same IKE header format as specified in [RFC7296](#RFC7296). The Major Version is
...
<a name="idg_payload"></a>
## Group Identification Payload

The Group Identification (IDg) payload allows the GM to indicate which group it
        wants to join. The payload is constructed by using the IKEv2
        Identification Payload ([RFC7296](#RFC7296)). 
        ID type ID_KEY_ID **MUST** be supported. ID types ID_IPV4_ADDR, ID_FQDN, ID_RFC822_ADDR, and ID_IPV6_ADDR **SHOULD** be supported. ID types ID_DER_ASN1_DN and ID_DER_ASN1_GN 
        are not expected to be used. The Payload Type for the IDg payload is fifty (50).

<a name="sag_payload"></a>
## Security Association - GM Supported Transforms Payload
...
**Next Payload, C, RESERVED, and Payload Length
          fields:**
  Comprise the IKEv2 generic payload header and are
            defined in [RFC7296](#RFC7296).

**Group Policies (variable):**
  A set of group policies for
            the group.
...
A GW policy allows for the distribution of group-wide policy,
          such as instructions for when to activate and deactivate SAs.

Policies are distributed in substructures to the GSA payload.
          The format of the substructures is defined in [Section 4.4.2](#gsa_policy)
          (for group SA policy) and in [Section 4.4.3](#gw_policy) (for GW policy).
...
The group SA policy substructure contains parameters for a single
          ...
          GCKS **MUST NOT** distribute both ESP and AH policies for
          the same set of Traffic Selectors.
...
**Protocol (1 octet):**
  Identifies the security protocol
              for this group SA.  The values are defined in the "IKEv2 Security
              Protocol Identifiers" registry in [IKEV2-IANA](#IKEV2-IANA). The valid
              values for this field are 6 (GIKE_UPDATE) for Rekey SA
              and 2 (AH) or 3 (ESP) for Data-Security SAs.

**SPI Size (1 octet):**
  Size of the SPI
              for the SA.  SPI size depends on the SA protocol. It is 16 octets for GIKE_UPDATE and 4 octets for AH and ESP.
...
**SPI (variable):**
  Security Parameter Index for the group SA.
...
**Source & Destination Traffic Selectors (variable):**
  ...
  For the Rekey SA (with the GIKE_UPDATE protocol), the
              destination traffic selectors **MUST** define a single
              ...
              traffic selector **SHOULD** define a
              ...
              traffic selector **MAY** define a wildcard IP address
  ...
  If the Data-Security SAs are created in tunnel mode, then it
              **MUST** be tunnel mode with address preservation (see
              Multicast Extensions to the Security Architecture [RFC5374](#RFC5374).  UDP encapsulation of ESP packets [RFC3948](#RFC3948) cannot be specified in G-IKEv2 and thus is
...
**Group SA Transforms (variable):**
  A list of Transform
              ...
              format is defined in IKEv2 ([RFC7296](#RFC7296)).  The "Last Substruc" field
              in each Transform Substructure is set to 3 except for the last
              Transform Substructure, where it is set to 0. [Section 4.4.2.1](#gsa_transforms) describes using IKEv2 transforms in the
              group SA policy substructure.

**Group SA Attributes (variable):**
  Contains policy attributes
              ...
              depending on the protocol and the group policy. [Section 4.4.2.2](#gsa_attr) defines attributes used in the group SA policy
              substructure.


<a name="gsa_transforms"></a>
#### Group SA Transforms

Group SA policy is defined by the means of transforms in the group SA policy substructure.
            For this purpose, the transforms defined in [RFC7296](#RFC7296)
            are used. In addition, new Transform Types are defined for use in G-IKEv2:
            Group Controller Authentication Method (GCAUTH) 
            and Key Wrap Algorithm (KWA); see [Section 9](#IANA).
...
**(*):**
  If the AEAD encryption algorithm is used, then INTEG
                transform either **MUST NOT** be specified or
                **MUST** contain value NONE; otherwise, it
                **MUST** be specified and **MUST** contain
                a value other than NONE.

**(**):**
  May only appear at the time of a GM
                registration (in the GSA_AUTH and GSA_REGISTRATION
                exchanges).


<a name="auth_method"></a>
##### Group Controller Authentication Method Transform

The Group Controller Authentication Method (GCAUTH) transform is used to convey information on how the GCKS 
              will authenticate the GSA_REKEY messages.

This document creates a new IKEv2 IANA registry for Transform IDs of this Transform Type, which has been initially populated as described in [Section 9](#IANA).
              In particular, the following entries have been added:

**Table: Group Controller Authentication Method Transform IDs**
...
**Digital Signature**
  ...
  In the case of the Digital Signature Transform ID, the GCKS
                  ...
                  **MUST** match the content of the Signature
                  ...
                  Signature Transform ID and **MUST NOT** be used
                  with other Transform IDs.


More authentication methods may be defined in the future.

The authentication method **MUST NOT** change as a result of rekey operations.
              This means that the Group Controller Authentication Method transform **MUST NOT** appear in the rekey
...
<a name="wrapping_alg"></a>
##### Key Wrap Algorithm Transform

The Key Wrap Algorithm (KWA) transform is used to convey information about an algorithm
              that is used for key wrapping in G-IKEv2. See [Section 4.5.4](#wrapped_key) for details.

This document creates a new IKEv2 IANA registry for the key wrap algorithms, 
              which has been initially populated as described in [Section 9](#IANA).
              In particular, the following entries have been added:

**Table: Key Wrap Algorithm Transform IDs**
...
**KW_5649_128, KW_5649_192, KW_5649_256:**
  The key wrap
                  algorithm defined in [RFC5649](#RFC5649) with a 128-bit,
                  192-bit, and 256-bit key, respectively.  This key wrap algorithm
                  is designed for use with AES block cipher.

**KW_ARX:**
  The ARX-KW-8-2-4-GX key wrap algorithm
                  defined in [ARX-KW](#ARX-KW). This key wrap algorithm is
...
<a name="antireplay"></a>
##### Sequence Numbers Transform

The Sequence Numbers (SNs) Transform Type is defined in [RFC9827](#RFC9827).
              ...
              that correspond to non-ESN and ESN cases from AH [RFC4302](#RFC4302) and ESP [RFC4303](#RFC4303) specifications.

Transform ID "32-bit Sequential Numbers" **SHOULD** be used by the GCKS for 
              single-sender multicast Data-Security SAs utilizing protocols ESP or AH.

Since both AH [RFC4302](#RFC4302) and ESP [RFC4303](#RFC4303) are defined in such a way that high-order 32 bits of extended sequence numbers are never transmitted, it makes using ESN in multicast Data-Security SAs
              ...
              of ESN for each sender in the group. The algorithm for doing this described in AH [RFC4302](#RFC4302)
              and ESP [RFC4303](#RFC4303) is resource-consuming and is only suitable when a receiver is able to guess
              ...
              **MUST NOT** be used for multicast Data-Security SAs utilizing protocols ESP or AH.

This document defines a new Transform ID 
              for this Transform Type: "32-bit Unspecified Numbers" (2). This Transform ID defines the following properties:

- Sequence numbers are
32 bits in size and are transmitted in the Sequence Number field of AH and ESP
packets.
- The value of sequence numbers is not guaranteed to be unique for the
...
**MAY** also use this Transform ID for single-sender multicast
...
Group SA attributes are generally used to provide GMs with additional parameters
            ...
            existence of other group SAs, or on other conditions.

This document creates a new IKEv2 IANA registry for the types
            of group SA attributes, which has been initially populated as described in [Section 9](#IANA).
            In particular, the following attributes have been added:

**Table: Group SA Attributes**
...
The attributes follow the format defined in IKEv2 ([RFC7296](#RFC7296)). The
...
<a name="gsa_attr_key_lifetime"></a>
##### GSA_KEY_LIFETIME Attribute
...
The GSA_KEY_LIFETIME attribute (1) specifies the maximum time for
              ...
              When the lifetime expires, the group SA and all associated keys **MUST** be deleted. 
              The GCKS may delete the SA at any time before the end of the validity period.

A single attribute of this type **MUST** be included into any group SA policy substructure
              if multicast rekey is employed by the GCKS. This attribute **SHOULD NOT** be used if inband rekey 
              (via the GSA_INBAND_REKEY exchange) is employed by the GCKS for the GM.

<a name="gsa_attr_initial_message_id"></a>
##### GSA_INITIAL_MESSAGE_ID Attribute

The GSA_INITIAL_MESSAGE_ID attribute (2) defines the initial Message ID
...
<a name="gsa_attr_next_spi"></a>
##### GSA_NEXT_SPI Attribute

The optional GSA_NEXT_SPI attribute (3) contains the SPI that the GCKS reserved  
              ...
              resides in (see [Section 4.4.2](#gsa_policy)), and the attribute data contains the
...
Note that this method of detecting lost rekey messages can only be used 
...
Group-specific policy that does not belong to any SA policy can be distributed to
          all GMs using the GW policy substructure.
...
**Protocol (1 octet):**
  **MUST** be zero. This
              value is reserved (see [Section 9](#IANA)) and is never used for
...
**GW Policy Attributes (variable):**
  Contains policy
              ...
              optional depending on the group policy.
...
This document creates a new IKEv2 IANA registry for the types
              of group-wide policy attributes, which has been initially populated as described in [Section 9](#IANA).
              In particular, the following attributes have been added:

**Table: GW Policy Attributes**
...
The attributes follow the format defined in the IKEv2 ([RFC7296](#RFC7296)). The
            "Format" column defines what attribute format is allowed:
            Type/Length/Value (TLV) or Type/Value (TV).  The "Multi-Valued"
            column defines whether multiple instances of the attribute can
            appear.

<a name="gwp_attr_atd_dtd"></a>
##### GWP_ATD and GWP_DTD Attributes

[RFC5374](#RFC5374) specifies a key rollover method that
              requires two values be provided to GMs: Activation
              Time Delay (ATD) and Deactivation Time Delay (DTD).

The GWP_ATD attribute (1) allows a GCKS to set the
...
The GWP_DTD attribute (2) allows the GCKS to set the
...
The GWP_ATD and GWP_DTD attributes contain a 16-bit unsigned integer in  
...
<a name="gwp_attr_sid_bits"></a>
##### GWP_SENDER_ID_BITS Attribute

The GWP_SENDER_ID_BITS attribute (3) declares how many bits
              ...
              to Protect Group Traffic [RFC6054](#RFC6054) and as specified
              in [Section 2.5.2](#sid-usage). Guidance for a GCKS choosing the
              value is provided in [RFC6054](#RFC6054). This value is
              applied to each Sender-ID value distributed in the KD
              payload.

The GCKS **MUST** include this attribute if there are more than one senders
...
**Next Payload, C, RESERVED, and Payload Length fields:**
  Comprise the IKEv2 generic payload header and are defined in [RFC7296](#RFC7296).
...
For this reason, two types of key bags are defined: Group Key Bag and
   ...
   For a Group Key Bag, the Protocol along with the SPI (see [Figure 18](#group_key_bag_format)) 
   identify the SA that is associated with the keys in this bag.

<a name="group_key_bag"></a>
...
**SPI Size (1 octet):**
  Size of the
              ...
              protocol. It is 16 octets for GIKE_UPDATE and 4 octets for AH and ESP.

**Length (2 octets, unsigned integer):**
  Length of this substructure including the header.

**SPI (variable):**
  Security Parameter Index for the corresponding SA.
              The size of this field is determined by the SPI Size field.
              In the case of GIKE_UPDATE, the SPI is the IKEv2 header SPI pair where the
              first 8 octets become the "IKE SA Initiator's SPI" field in the
              G-IKEv2 rekey message IKEv2 HDR, and the second 8 octets become
...
This document creates a new IKEv2 IANA registry for the types
          of Group Key Bag attributes, which has been initially populated as described in [Section 9](#IANA).
          In particular, the following attributes have been added:

**Table: Group Key Bag Attributes**

| Value | Group Key Bag Attributes | Format | Multi-Valued | Used in Protocol |
| --- | --- | --- | --- | --- |
| 0 | Reserved |  |  |  |
| 1 | SA_KEY | TLV | YES*NO | GIKE_UPDATEAH, ESP |
...
<a name="gkd_attr_group_key"></a>
#### SA_KEY Attribute

The SA_KEY attribute (1) contains a keying material for the corresponding SA. 
            The content of the attribute is formatted according to 
            [Section 4.5.4](#wrapped_key) with a precondition that the Key ID field **MUST** always be zero.
            The size of the keying material **MUST** be equal to the total size of the keys needed to be taken
            from this keying material (see [Section 3.4](#group_sa_keys)) for the corresponding SA.

If the key bag is for a Data-Security SA (AH or ESP protocols), 
            then exactly one SA_KEY attribute **MUST** be present with both
            Key ID and KWK ID fields set to zero.

If the key bag is for a Rekey SA (GIKE_UPDATE protocol), 
then exactly one SA_KEY attribute **MUST** be present in the GSA_AUTH, GSA_REGISTRATION, and GSA_INBAND_REKEY exchanges.
            In the GSA_REKEY pseudo-exchange, at least one SA_KEY attribute **MUST** be present, 
            and more attributes **MAY** be present (depending on the key management method employed by the GCKS).

<a name="member_key_bag"></a>
### Member Key Bag Substructure

The Member Key Bag substructure contains keys and other
...
This document creates a new IKEv2 IANA registry for the types
          of Member Key Bag attributes, which has been initially populated as described in [Section 9](#IANA).
          In particular, the following attributes have been added:

**Table: Member Key Bag Attributes**
...
<a name="mkd_attr_kwk"></a>
#### WRAP_KEY Attribute

The WRAP_KEY attribute (1) contains a key that is used 
...
The content of the attribute has a format defined in [Section 4.5.4](#wrapped_key)
            ...
            The size of the attribute data **MUST** be equal to the key size for this key wrap algorithm.

Multiple instances of the WRAP_KEY attributes **MAY** be present in the key bag.

<a name="mkd_attr_auth_key"></a>
#### AUTH_KEY Attribute

The AUTH_KEY attribute (2) contains the key that is used to authenticate 
...
- If digital signatures are used for the GSA_REKEY message
                ...
                public key **MUST** be represented as DER-encoded
                ASN.1 object SubjectPublicKeyInfo, defined in [RFC5280](#RFC5280). The algorithm field inside the
                SubjectPublicKeyInfo object **MUST** match the
                ...
                **MUST** include the RSASSA-PSS-params structure.
- In case of implicit authentication, the AUTH_KEY Attribute
	      is not used and **MUST** be absent (see [Section 2.4.1](#gsa_rekey)).

Multiple instances of the AUTH_KEY attributes **MUST NOT** be sent.

<a name="mkd_attr_gm_sid"></a>
#### GM_SENDER_ID Attribute

The GM_SENDER_ID attribute (3) is used to download one or more Sender-ID 
            values for the exclusive use of a GM.  One or more of these attributes **MUST** be
...
If the GMs have requested multiple Sender-ID values in the GROUP_SENDER notification, then the GCKS **SHOULD**
            provide it with the requested number of Sender-IDs by sending multiple instances of the GM_SENDER_ID
            attribute. The GCKS **MAY** send fewer values than requested by the GM (e.g., if it is running out of Sender-IDs),
...
<a name="wrapped_key"></a>
### Key Wrapping

Symmetric keys in G-IKEv2 are never sent in clear inside G-IKEv2 messages.
          ...
          encryption algorithms in practice. For example, AES Key Wrap with Padding Algorithm [RFC5649](#RFC5649) defines key wrapping using AES,
          and Key Wrapping Constructions using SipHash and ChaCha [ARX-KW](#ARX-KW) define key wrapping using Chacha20.
...
In G-IKEv2, the key wrap algorithm **MUST** be negotiated in the
 ...
 the key wrap algorithm in the group SA policy for the Rekey SA inside the GSA payload. Note that key wrap algorithms 
          for these cases **MAY** be different. For the unicast SA, the key wrap algorithm is negotiated
          ...
          If an SAg payload is included in the GSA_AUTH request, then it **MUST** indicate which key wrap algorithms are supported by the GM.
          In all these cases, the key wrap algorithm is specified in a Key Wrap Algorithm transform (see [Section 4.4.2.1.2](#wrapping_alg)).

The format of the wrapped key is shown in [Figure 20](#key_format).
...
**Key ID (4 octets):**
  ID of the encrypted key. The value
              zero means that the encrypted key contains SA keys (in the form of
              keying material; see [Section 3.4](#group_sa_keys)). Otherwise,
              it contains some intermediate key.

**KWK ID (4 octets):**
  ID of the key that was used to
              ...
              intermediate key was used.

**Encrypted Key (variable):**
  The encrypted key bits. These
...
Delete payload is used in G-IKEv2 when the GCKS wants to 
        ...
        See [Section 2.4.3](#deletion) for detail.

<a name="notify"></a>
## Notify Payload

G-IKEv2 uses the same Notify payload as specified in [RFC7296](#RFC7296).

There are additional Notify message types introduced by G-IKEv2 to
  communicate error conditions and status (see [Section 9](#IANA)).

<a name="inv_gr_id"></a>
### INVALID_GROUP_ID Notification

INVALID_GROUP_ID (45) is a new error type notification that indicates that the IDg payload sent during the registration process denotes an invalid group.
...
<a name="autz_failed"></a>
### AUTHORIZATION_FAILED Notification

AUTHORIZATION_FAILED (46) is a new error type notification that is sent in 
...
<a name="reg_failed"></a>
### REGISTRATION_FAILED Notification

REGISTRATION_FAILED (49) is a new error type notification that is sent 
...
GROUP_SENDER (16429) is a new status type notification that is sent in 
          ...
          how many Sender-ID values the GM desires. The count **MUST** be 4 octets long
          and contain the big-endian representation of the number of 
          requested Sender-IDs. The Protocol ID and SPI Size fields in the Notify payload **MUST** be zero.

## Authentication Payload

G-IKEv2 uses the same Authentication payload as specified in [RFC7296](#RFC7296) to
        ...
        as described in [Section 2.4.1.1](#gsa_rekey_auth).

<a name="restrictions"></a>
# Using G-IKEv2 Attributes

G-IKEv2 defines a number of attributes that are used to convey information
      ...
      For convenience, these restrictions are summarized in [Table 9](#mcast_attr) (for
      multicast rekey operations) and [Table 10](#inband_attr) (for inband rekey operations) below.
...
**Table: Attributes in G-IKEv2 Exchanges with Multicast Rekey Operations**
...
**(1):**
  The GWP_SENDER_ID_BITS attribute **MUST** be present
          ...
          registration request.  Otherwise, it **MUST NOT** be
          present. At least one GM_SENDER_ID attribute **MUST** be
          present in the former case (and more **MAY** be present if
          the GM requested more Sender-IDs), and it **MUST NOT** be
...
**(3):**
  The WRAP_KEY attribute **MUST** be present if the
...
**(4):**
  The AUTH_KEY attribute **MUST** be present in the
          ...
          and **MUST NOT** be present if implicit authentication is
          employed. The AUTH_KEY attribute **MUST** be present in the
...
**Table: Attributes in G-IKEv2 Exchanges with Inband Rekey Operations**
...
**(1):**
  The GWP_SENDER_ID_BITS attribute **MUST** be present if
...
<a name="ike_ext"></a>
# Interaction with IKEv2 and ESP Extensions

A number of IKEv2 and ESP extensions are defined that can be used to extend
      protocol functionality. G-IKEv2 is compatible with most of them.
      In particular, EAP authentication defined in [RFC7296](#RFC7296) can be used 
      to establish registration IKE SA, as well as EAP-only authentication [RFC5998](#RFC5998) and 
      secure password authentication [RFC6467](#RFC6467). 
      G-IKEv2 is compatible with and can use IKEv2 Redirect Mechanism [RFC5685](#RFC5685) and 
      IKEv2 Session Resumption [RFC5723](#RFC5723). 
      G-IKEv2 is also compatible with Multiple Key Exchanges in the IKEv2 
      framework, as defined in [RFC9370](#RFC9370).
...
## Implicit IV for Counter-Based Ciphers in ESP

Using implicit IV for counter-based encryption modes in ESP is defined in [RFC8750](#RFC8750).
        ...
        in multicast SAs (see [Section 4.4.2.1.3](#antireplay)).

## Mixing Preshared Keys in IKEv2 for Post-Quantum Security

G-IKEv2 can take advantage of the protection provided by
        Post-quantum Preshared Keys (PPKs) for IKEv2 [RFC8784](#RFC8784). However, the use of 
        ...
        See [RFC8784](#RFC8784) for details.

While the group keys are protected with PPK and thus are immune to QC, GCKS implementations that care about other data sent over initial IKE SA 
        **MUST** rely on IKEv2 extensions that protect even initial IKE SA against QC
        (like [RFC9867](#RFC9867)).

## Aggregation and Fragmentation Mode for ESP

Aggregation and fragmentation mode for ESP is defined in [RFC9347](#RFC9347). This mode allows IP packets to 
...
It is possible to use the aggregation and fragmentation mode without congestion control for a single-sender multicast ESP SA created in tunnel mode.
...
<a name="gdoi_ext"></a>
# GDOI Protocol Extensions

Few extensions were defined for the GDOI protocol [RFC6407](#RFC6407), like 
        GDOI Support for IEC 62351 Security Services [RFC8052](#RFC8052) or the GDOI GROUPKEY-PUSH Acknowledgement Message [RFC8263](#RFC8263). 
        It is expected that these extensions will be redefined for G-IKEv2 in separate documents, if needed.

# Security Considerations

When an entity joins the group and becomes a GM, it has to  
      trust that the GCKS only authorized entities that are admitted to the group and 
      has to trust that other GMs will not leak the information shared within the group.

## GSA Registration and Secure Channel

G-IKEv2 registration procedure uses IKEv2 initial exchanges,
 inheriting all the security considerations documented in [RFC7296](#RFC7296), including authentication, confidentiality, on-path attack
 ...
 also takes advantage of those protections.
        ...
        whether they have the IKEv2 credentials.

## GSA Maintenance Channel
...
The authentication key is distributed during the GM registration 
...
Confidentiality is provided by distributing a confidentiality key
          as part of the GSA member registration exchange.

### On-Path Attack Protection
...
The strict role separation between the GCKS and the GMs and, as a consequence, 
...
Per this document, new registries have been created for G-IKEv2 under the "Internet Key Exchange Version 2 (IKEv2) Parameters" registry group [IKEV2-IANA](#IKEV2-IANA). The terms
        Reserved, Expert Review, and Private Use are as defined
        in [RFC8126](#RFC8126).

1. IANA has created the "Transform Type 13 - Key Wrap Algorithm Transform IDs" registry.
The registration policy for this registry is Expert Review [RFC8126](#RFC8126). The initial values of the registry are as follows:
            Value
                  Key Wrap Algorithm
              0
                  Reserved
...
2. IANA has created the "Transform Type
            14 - Group Controller Authentication Method Transform
            IDs" registry. The registration policy for this registry is Expert Review [RFC8126](#RFC8126).  The initial values of the registry are as follows:
            Value
              ...
              0
                ...
                1024-65535
                  Reserved for Private Use
3. IANA has created the "Group SA Attributes" registry.  The registration policy for this registry is Expert Review [RFC8126](#RFC8126). The initial values of the registry are as follows:
            Value
                  Group SA Attributes
                  Format
                  Multi-Valued
              ...
              0
                ...
                16384-32767
                  Reserved for Private Use
4. IANA has created the "Group-Wide Policy Attributes" registry. The registration policy for this registry is Expert Review [RFC8126](#RFC8126). The initial values of the registry are as follows:
            Value
                  GW Policy Attributes
                  Format
                  Multi-Valued
              0
                ...
                16384-32767
                  Reserved for Private Use
5. IANA has created the "Group Key Bag Attributes" registry. The registration policy for this registry is Expert Review [RFC8126](#RFC8126). The initial values of the registry are as follows:
            Value
                  Group Key Bag Attributes
                  Format
                  Multi-Valued
              ...
              0
                ...
                16384-32767
                  Reserved for Private Use
6. IANA has created the "Member Key Bag Attributes" registry. The registration policy for this registry is Expert Review [RFC8126](#RFC8126). The initial values of the registry are as follows:
            Value
                  Member Key Bag Attributes
                  Format
                  Multi-Valued
...
In all cases of Expert Review described in this section,
           the designated expert (DE) is expected to ascertain the existence of suitable
           documentation (a specification) as described in [RFC8126](#RFC8126) and 
...
7. In the "IKEv2 Notify Message Error Types" registry, IANA has made the following changes:
  ...
  - Registered "REGISTRATION_FAILED".
  - Updated the references for "INVALID_GROUP_ID" and "AUTHORIZATION_FAILED" to point to this document.
8. An earlier draft of this document [G-IKEV2](#I-D.yeung-g-ikev2) registered the Notify
...
<a name="RFC4302"></a> **[RFC4302]** S. Kent, "IP Authentication Header", RFC 4302, DOI 10.17487/RFC4302, December 2005. <https://www.rfc-editor.org/info/rfc4302> [Local MD](rfc4302.md)

<a name="RFC4303"></a> **[RFC4303]** S. Kent, "IP Encapsulating Security Payload (ESP)", RFC 4303, DOI 10.17487/RFC4303, December 2005. <https://www.rfc-editor.org/info/rfc4303> [Local MD](rfc4303.md)
...
<a name="RFC6054"></a> **[RFC6054]** D. McGrew, B. Weis, "Using Counter Modes with Encapsulating Security Payload (ESP) and Authentication Header (AH) to Protect Group Traffic", RFC 6054, DOI 10.17487/RFC6054, November 2010. <https://www.rfc-editor.org/info/rfc6054> [Local MD](rfc6054.md)
...
<a name="RFC7427"></a> **[RFC7427]** T. Kivinen, J. Snyder, "Signature Authentication in the Internet Key Exchange Version 2 (IKEv2)", RFC 7427, DOI 10.17487/RFC7427, January 2015. <https://www.rfc-editor.org/info/rfc7427> [Local MD](rfc7427.md)
...
<a name="RFC9827"></a> **[RFC9827]** V. Smyslov, "Renaming the Extended Sequence Numbers (ESN) Transform Type in the Internet Key Exchange Protocol Version 2 (IKEv2)", RFC 9827, DOI 10.17487/RFC9827, November 2025. <https://www.rfc-editor.org/info/rfc9827> [Local MD](rfc9827.md)

## Informative References

<a name="ARX-KW"></a> **[ARX-KW]** S. Shinichi, "ARX-KW, a family of key wrapping constructions using SipHash and ChaCha", January 2020. Cryptology ePrint Archive, Paper 2020/059 <https://eprint.iacr.org/2020/059.pdf>

<a name="I-D.yeung-g-ikev2"></a> **[G-IKEV2]** S. Rowles, A. Yeung, P. Tran, Y. Nir, "Group Key Management using IKEv2", Internet-Draft draft-yeung-g-ikev2-07, November 2013. Work in Progress <https://datatracker.ietf.org/doc/html/draft-yeung-g-ikev2-07>

<a name="IKEV2-IANA"></a> **[IKEV2-IANA]** "Internet Key Exchange Version 2 (IKEv2) Parameters", <http://www.iana.org/assignments/ikev2-parameters>

<a name="NNL"></a> **[NNL]** D. Naor, M. Naor, J. Lotspiech, "Revocation and Tracing Schemes for Stateless Receivers", DOI 10.1007/3-540-44647-8_3, 2001. Advances in Cryptology - CRYPTO 2001, Lecture Notes in Computer Science, vol. 2139, pp. 41-62 <http://www.wisdom.weizmann.ac.il/~naor/PAPERS/2nl.pdf>

<a name="OFT"></a> **[OFT]** D. McGrew, A. Sherman, "Key Establishment in Large Dynamic Groups Using One-Way Function Trees", DOI 10.1109/TSE.2003.1199073, May 1998. IEEE Transactions on Software Engineering, vol. 29, no. 5, pp. 444-458 <https://pdfs.semanticscholar.org/d24c/7b41f7bcc2b6690e1b4d80eaf8c3e1cc5ee5.pdf>
...
<a name="RFC2627"></a> **[RFC2627]** D. Wallner, E. Harder, R. Agee, "Key Management for Multicast: Issues and Architectures", RFC 2627, DOI 10.17487/RFC2627, June 1999. <https://www.rfc-editor.org/info/rfc2627> [Local MD](rfc2627.md)

<a name="RFC3686"></a> **[RFC3686]** R. Housley, "Using Advanced Encryption Standard (AES) Counter Mode With IPsec Encapsulating Security Payload (ESP)", RFC 3686, DOI 10.17487/RFC3686, January 2004. <https://www.rfc-editor.org/info/rfc3686> [Local MD](rfc3686.md)

<a name="RFC3740"></a> **[RFC3740]** T. Hardjono, B. Weis, "The Multicast Group Security Architecture", RFC 3740, DOI 10.17487/RFC3740, March 2004. <https://www.rfc-editor.org/info/rfc3740> [Local MD](rfc3740.md)

<a name="RFC3948"></a> **[RFC3948]** A. Huttunen, B. Swander, V. Volpe, L. DiBurro, M. Stenberg, "UDP Encapsulation of IPsec ESP Packets", RFC 3948, DOI 10.17487/RFC3948, January 2005. <https://www.rfc-editor.org/info/rfc3948> [Local MD](rfc3948.md)

<a name="RFC4046"></a> **[RFC4046]** M. Baugher, R. Canetti, L. Dondeti, F. Lindholm, "Multicast Security (MSEC) Group Key Management Architecture", RFC 4046, DOI 10.17487/RFC4046, April 2005. <https://www.rfc-editor.org/info/rfc4046> [Local MD](rfc4046.md)

<a name="RFC4106"></a> **[RFC4106]** J. Viega, D. McGrew, "The Use of Galois/Counter Mode (GCM) in IPsec Encapsulating Security Payload (ESP)", RFC 4106, DOI 10.17487/RFC4106, June 2005. <https://www.rfc-editor.org/info/rfc4106> [Local MD](rfc4106.md)

<a name="RFC4309"></a> **[RFC4309]** R. Housley, "Using Advanced Encryption Standard (AES) CCM Mode with IPsec Encapsulating Security Payload (ESP)", RFC 4309, DOI 10.17487/RFC4309, December 2005. <https://www.rfc-editor.org/info/rfc4309> [Local MD](rfc4309.md)

<a name="RFC4543"></a> **[RFC4543]** D. McGrew, J. Viega, "The Use of Galois Message Authentication Code (GMAC) in IPsec ESP and AH", RFC 4543, DOI 10.17487/RFC4543, May 2006. <https://www.rfc-editor.org/info/rfc4543> [Local MD](rfc4543.md)

<a name="RFC5374"></a> **[RFC5374]** B. Weis, G. Gross, D. Ignjatic, "Multicast Extensions to the Security Architecture for the Internet Protocol", RFC 5374, DOI 10.17487/RFC5374, November 2008. <https://www.rfc-editor.org/info/rfc5374> [Local MD](rfc5374.md)

<a name="RFC5649"></a> **[RFC5649]** R. Housley, M. Dworkin, "Advanced Encryption Standard (AES) Key Wrap with Padding Algorithm", RFC 5649, DOI 10.17487/RFC5649, September 2009. <https://www.rfc-editor.org/info/rfc5649> [Local MD](rfc5649.md)

<a name="RFC5685"></a> **[RFC5685]** V. Devarapalli, K. Weniger, "Redirect Mechanism for the Internet Key Exchange Protocol Version 2 (IKEv2)", RFC 5685, DOI 10.17487/RFC5685, November 2009. <https://www.rfc-editor.org/info/rfc5685> [Local MD](rfc5685.md)

<a name="RFC5723"></a> **[RFC5723]** Y. Sheffer, H. Tschofenig, "Internet Key Exchange Protocol Version 2 (IKEv2) Session Resumption", RFC 5723, DOI 10.17487/RFC5723, January 2010. <https://www.rfc-editor.org/info/rfc5723> [Local MD](rfc5723.md)

<a name="RFC5998"></a> **[RFC5998]** P. Eronen, H. Tschofenig, Y. Sheffer, "An Extension for EAP-Only Authentication in IKEv2", RFC 5998, DOI 10.17487/RFC5998, September 2010. <https://www.rfc-editor.org/info/rfc5998> [Local MD](rfc5998.md)
...
<a name="RFC6467"></a> **[RFC6467]** T. Kivinen, "Secure Password Framework for Internet Key Exchange Version 2 (IKEv2)", RFC 6467, DOI 10.17487/RFC6467, December 2011. <https://www.rfc-editor.org/info/rfc6467> [Local MD](rfc6467.md)

<a name="RFC7383"></a> **[RFC7383]** V. Smyslov, "Internet Key Exchange Protocol Version 2 (IKEv2) Message Fragmentation", RFC 7383, DOI 10.17487/RFC7383, November 2014. <https://www.rfc-editor.org/info/rfc7383> [Local MD](rfc7383.md)

<a name="RFC7634"></a> **[RFC7634]** Y. Nir, "ChaCha20, Poly1305, and Their Use in the Internet Key Exchange Protocol (IKE) and IPsec", RFC 7634, DOI 10.17487/RFC7634, August 2015. <https://www.rfc-editor.org/info/rfc7634> [Local MD](rfc7634.md)

<a name="RFC8052"></a> **[RFC8052]** B. Weis, M. Seewald, H. Falk, "Group Domain of Interpretation (GDOI) Protocol Support for IEC 62351 Security Services", RFC 8052, DOI 10.17487/RFC8052, June 2017. <https://www.rfc-editor.org/info/rfc8052> [Local MD](rfc8052.md)

<a name="RFC8263"></a> **[RFC8263]** B. Weis, U. Mangla, T. Karl, N. Maheshwari, "Group Domain of Interpretation (GDOI) GROUPKEY-PUSH Acknowledgement Message", RFC 8263, DOI 10.17487/RFC8263, November 2017. <https://www.rfc-editor.org/info/rfc8263> [Local MD](rfc8263.md)

<a name="RFC8750"></a> **[RFC8750]** D. Migault, T. Guggemos, Y. Nir, "Implicit Initialization Vector (IV) for Counter-Based Ciphers in Encapsulating Security Payload (ESP)", RFC 8750, DOI 10.17487/RFC8750, March 2020. <https://www.rfc-editor.org/info/rfc8750> [Local MD](rfc8750.md)

<a name="RFC8784"></a> **[RFC8784]** S. Fluhrer, P. Kampanakis, D. McGrew, V. Smyslov, "Mixing Preshared Keys in the Internet Key Exchange Protocol Version 2 (IKEv2) for Post-quantum Security", RFC 8784, DOI 10.17487/RFC8784, June 2020. <https://www.rfc-editor.org/info/rfc8784> [Local MD](rfc8784.md)

<a name="RFC9242"></a> **[RFC9242]** V. Smyslov, "Intermediate Exchange in the Internet Key Exchange Protocol Version 2 (IKEv2)", RFC 9242, DOI 10.17487/RFC9242, May 2022. <https://www.rfc-editor.org/info/rfc9242> [Local MD](rfc9242.md)

<a name="RFC9329"></a> **[RFC9329]** T. Pauly, V. Smyslov, "TCP Encapsulation of Internet Key Exchange Protocol (IKE) and IPsec Packets", RFC 9329, DOI 10.17487/RFC9329, November 2022. <https://www.rfc-editor.org/info/rfc9329> [Local MD](rfc9329.md)

<a name="RFC9347"></a> **[RFC9347]** C. Hopps, "Aggregation and Fragmentation Mode for Encapsulating Security Payload (ESP) and Its Use for IP Traffic Flow Security (IP-TFS)", RFC 9347, DOI 10.17487/RFC9347, January 2023. <https://www.rfc-editor.org/info/rfc9347> [Local MD](rfc9347.md)

<a name="RFC9370"></a> **[RFC9370]** CJ. Tjhai, M. Tomlinson, G. Bartlett, S. Fluhrer, D. Van Geest, O. Garcia-Morchon, V. Smyslov, "Multiple Key Exchanges in the Internet Key Exchange Protocol Version 2 (IKEv2)", RFC 9370, DOI 10.17487/RFC9370, May 2023. <https://www.rfc-editor.org/info/rfc9370> [Local MD](rfc9370.md)

<a name="RFC9867"></a> **[RFC9867]** V. Smyslov, "Mixing Preshared Keys in the IKE_INTERMEDIATE and CREATE_CHILD_SA Exchanges of the Internet Key Exchange Protocol Version 2 (IKEv2) for Post-Quantum Security", RFC 9867, DOI 10.17487/RFC9867, November 2025. <https://www.rfc-editor.org/info/rfc9867> [Local MD](rfc9867.md)

<a name="lkh_key_management"></a>
# Use of LKH in G-IKEv2

[RFC2627](#RFC2627) describes the LKH
      architecture and how a GCKS uses LKH to exclude GMs. This
      section clarifies how the LKH architecture is used with G-IKEv2.

<a name="lkh_notation"></a>
## Notation

In this section, we will use the notation X{Y}, 
...
The content of the KD payload will be shown as a sequence
...
We will also use the notation X->Y->Z
...
When a GCKS forms a group, it creates a key tree as shown in
        [Figure 22](#initial-tree). The key tree contains logical keys (which are represented as
...
```
When GM A joins the group, the GCKS provides it
...
```
## Simple Group SA Rekey

If the GCKS performs a simple SA rekey without changing group membership,
...
If the GCKS has reason to believe that a GM should be excluded,
...
In the example below, the GCKS excludes GM F. For this purpose,
        it changes the key tree as follows, replacing key 2 with key 15 and
        key 5 with key 16. It also generates a new SA key for a new SA3.
...
- GMs A, B, C, and D will be able to decrypt the SA_KEY attribute 1{K_sa3} by using
...
- GMs G and H will construct new Key Path 15->6 and will be able to decrypt
...
- GM E will construct a new Key Path 16->15->11 and will be able to decrypt
          ...
          with the new Key Path (thus replacing key 2 with key 15 and key 5 with key 16).
- GM F won't be able to construct any Key Path leading to any key it possesses,
...
The authors thank Lakshminath Dondeti and
      Jing Xiang for first exploring the use of IKEv2
      for group key management and providing the basis behind the
      protocol. Mike Sullenberger and Amjad Inamdar were instrumental in helping resolve many
      issues in several draft versions of the document.

The authors are grateful to Tero Kivinen,
      Daniel Migault, Gorry       Fairhurst, Robert Sparks, Russ Housley, and Paul Wouters for
