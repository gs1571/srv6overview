...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-ospf-segment-routing-extensions-27
...
- [prev](https://datatracker.ietf.org/doc/draft-ietf-ospf-segment-routing-extensions-27)
- [alternate](https://dx.doi.org/10.17487/rfc8665)
...
- [2. Segment Routing Identifiers](#section-2)
  - [2.1. SID/Label Sub-TLV](#SIDLABEL)
- [3. Segment Routing Capabilities](#SRCAP)
  - [3.1. SR-Algorithm TLV](#SRALGO)
  - [3.2. SID/Label Range TLV](#SIDRANGE)
  - [3.3. SR Local Block TLV](#SRLB)
  - [3.4. SRMS Preference TLV](#SRMS-Pref)
- [4. OSPF Extended Prefix Range TLV](#PFXRANGE)
- [5. Prefix-SID Sub-TLV](#PREFIXSID)
- [6. Adjacency Segment Identifier (Adj-SID)](#ADJSID)
  - [6.1. Adj-SID Sub-TLV](#ADJSIDSUBTLV)
  - [6.2. LAN Adj-SID Sub-TLV](#LANADJSIDSUBTLV)
- [7. Elements of Procedure](#section-7)
  - [7.1. Intra-area Segment Routing in OSPFv2](#section-7.1)
  - [7.2. Inter-area Segment Routing in OSPFv2](#section-7.2)
  - [7.3. Segment Routing for External Prefixes](#section-7.3)
  - [7.4. Advertisement of Adj-SID](#section-7.4)
    - [7.4.1. Advertisement of Adj-SID on Point-to-Point Links](#section-7.4.1)
    - [7.4.2. Adjacency SID on Broadcast or NBMA Interfaces](#section-7.4.2)
- [8. IANA Considerations](#IANA)
  - [8.1. OSPF Router Information (RI) TLVs Registry](#RITLVREG)
  - [8.2. OSPFv2 Extended Prefix Opaque LSA TLVs Registry](#EPLTLVREG)
  - [8.3. OSPFv2 Extended Prefix TLV Sub-TLVs Registry](#EPLSTLVREG)
  - [8.4. OSPFv2 Extended Link TLV Sub-TLVs Registry](#ELLSTLVREG)
  - [8.5. IGP Algorithm Types Registry](#IGPALGOREG)
- [9. TLV/Sub-TLV Error Handling](#Error-handling)
...
Segment Routing (SR) allows a flexible definition of end-to-end
      ...
      interoperate in the presence of LSPs established with RSVP or LDP.

There are additional segment types, e.g., Binding Segment Identifier (SID) defined in [RFC8402](#RFC8402).
...
Extended Prefix/Link Opaque Link State Advertisements (LSAs) defined in [RFC7684](#RFC7684) are used for 
...
Segment Routing requires some additional router capabilities to be advertised 
      to other routers in the area.

These SR capabilities are advertised in the Router Information Opaque LSA
      (defined in [RFC7770](#RFC7770)). The TLVs defined below are applicable to 
...
The SR-Algorithm TLV is optional. It **SHOULD** only be advertised once
        ...
        by the node, such a node is considered as not being Segment Routing capable.
...
- Type:
            ...
            values are defined by this document:
                0:
                  Shortest Path First (SPF) algorithm based on link
...
When multiple SR-Algorithm TLVs are received from a given router,
        ...
        **MUST** be used. If the SR-Algorithm TLV appears in multiple Router
        ...
        Instance ID **MUST** be used and subsequent instances of the SR-Algorithm
        TLV **MUST** be ignored.
...
Prefix-SIDs **MAY** be advertised in the form of an index as described in 
        [Section 5](#PREFIXSID). Such an index defines the offset in the SID/Label space 
...
The SID/Label Range TLV is a top-level TLV of the Router Information 
        Opaque LSA (defined in [RFC7770](#RFC7770)).

The SID/Label Range TLV **MAY** appear multiple times and has the following
        format:
...
- Type:
              ...
              3-octet SID/label range size (i.e., the number of SIDs or labels 
               in the range including the first SID/label). It **MUST** be greater than 0.
              Reserved:
              **SHOULD** be set to 0 on transmission and **MUST** be ignored on reception

Initially, the only supported sub-TLV is the SID/Label Sub-TLV as defined 
        in [Section 2.1](#SIDLABEL). The SID/Label Sub-TLV **MUST** be included 
...
Only a single SID/Label Sub-TLV **MAY** be advertised in the SID/Label Range TLV. If more 
        than one SID/Label Sub-TLV is present, the SID/Label Range TLV **MUST** be ignored.

Multiple occurrences of the SID/Label Range TLV **MAY** be
        advertised in order to advertise multiple ranges. In such a case:

- The originating router **MUST** encode each range into a different SID/Label 
            Range TLV.
- The originating router decides the order in which the set of SID/Label 
...
- The receiving router **MUST** adhere to the order in which the ranges are 
            advertised when calculating a SID/Label from a SID index.
...
The RI LSA can be advertised at any of the defined flooding scopes
        ...
        SID/Label Range TLV advertisement, area-scoped flooding is **REQUIRED**.
...
The SRLB TLV is a top-level TLV of the Router Information 
        Opaque LSA (defined in [RFC7770](#RFC7770)).
...
Initially, the only supported sub-TLV is the SID/Label Sub-TLV as defined 
        ...
        the first SID/Label in the advertised range.

Only a single SID/Label Sub-TLV **MAY** be advertised in the SRLB TLV. If more 
        than one SID/Label Sub-TLV is present, the SRLB TLV **MUST** be ignored.

The originating router **MUST NOT** advertise overlapping ranges.

Each time a SID from the SRLB is allocated, it **SHOULD** also be reported to all
		...
		collisions between allocation instructions.

Within the context of OSPF, the reporting of Local SIDs is done through 
		OSPF sub-TLVs, such as the Adjacency SID ([Section 6](#ADJSID)). However, 
		the reporting of allocated Local SIDs can also be done through other means 
		and protocols, which are outside the scope of this document.

A router advertising the SRLB TLV **MAY** also have other label ranges, outside 
		...
		a local label that is not part of the SRLB.

The RI LSA can be advertised at any of the defined flooding scopes
        (link, area, or autonomous system (AS)). For the purpose of  
         SRLB TLV advertisement, area-scoped flooding is **REQUIRED**.

<a name="SRMS-Pref"></a>
## SRMS Preference TLV

The Segment Routing Mapping Server Preference TLV (SRMS Preference TLV) is used to 
      advertise a preference associated with the node that acts as an SR Mapping Server.
      The role of an SRMS is described in [RFC8661](#RFC8661). 
      SRMS preference is defined in 
      [RFC8661](#RFC8661).

The SRMS Preference TLV is a top-level TLV of the 
      Router Information Opaque LSA (defined in [RFC7770](#RFC7770)).

The SRMS Preference TLV **MAY** only be advertised
         once in the Router Information Opaque LSA and has the following format:
...
When multiple SRMS Preference TLVs are received from a given router, the receiver
    **MUST** use the first occurrence of the TLV in the Router
    ...
    **MUST** be used and subsequent instances of the SRMS Preference TLV **MUST** be ignored.

The RI LSA can be advertised at any of the defined flooding scopes (link, area, 
	...
	area, area-scoped flooding **MAY** be used.

<a name="PFXRANGE"></a>
# OSPF Extended Prefix Range TLV

In some cases, it is useful to advertise attributes for a range of
      prefixes.  The SR Mapping Server, which is described in [RFC8661](#RFC8661), is an example where we need a
...
The OSPF Extended Prefix Range TLV, which is a top-level TLV of the
      Extended Prefix LSA described in [RFC7684](#RFC7684) is defined for this purpose.

Multiple OSPF Extended Prefix Range TLVs **MAY** be
      ...
      **MUST** have the same flooding scope. The OSPF Extended
      Prefix Range TLV has the following format:
...
- Type:
        ...
        TLV allows for future extension.
            Range Size:
            Represents the number of prefixes that are covered by the
        advertisement. The Range Size **MUST NOT** exceed the
        number of prefixes that could be satisfied by the Prefix Length
        without including the IPv4 multicast address range (224.0.0.0/3).
            Flags:
...
The Prefix-SID Sub-TLV is a sub-TLV of the OSPF Extended Prefix TLV described 
      in [RFC7684](#RFC7684) and the OSPF Extended Prefix Range
      TLV described in [Section 4](#PFXRANGE). It **MAY** appear more than once in the 
      parent TLV and has the following format:
...
- Type:
   ...
   +--+--+--+--+--+--+--+--+
            ...
            Prefix-SID originator **MUST** replace the Prefix-SID
        ...
        **MUST** be ignored on reception
            MT-ID:
            Multi-Topology ID (as defined in [RFC4915](#RFC4915))
            Algorithm:
            Single octet identifying the algorithm the Prefix-SID is
        associated with as defined in [Section 3.1](#SRALGO)
        ...
        SR-Algorithm TLV ([Section 3.1](#SRALGO))
        **MUST** ignore the Prefix-SID Sub-TLV.
            ...
            any SID Advertisement received with an invalid setting for V- and L-Flags **MUST** be ignored.
...
The NP-Flag (No-PHP) **MUST** be set and the E-Flag
      **MUST** be clear for Prefix-SIDs 
...
The NP-Flag (No-PHP) **MUST** be set and the E-Flag
      **MUST** be clear for Prefix-SIDs 
        ...
        attached to the Autonomous System Boundary Router (ASBR).
...
- Any upstream neighbor of the Prefix-SID originator **MUST**
...
then the Prefix field in the Extended Prefix Range TLV would be set to
        ...
        the Index value in the Prefix-SID Sub-TLV would be set to 51.
...
Adj-SID is an optional sub-TLV of the Extended Link TLV defined in 
        [RFC7684](#RFC7684). It **MAY** appear multiple times
...
- Type:
   ...
   +-+-+-+-+-+-+-+-+
		...
		Reroute or MPLS-FRR (MPLS-Fast Reroute)
                as described in [RFC8402](#RFC8402).
                      V-Flag:
            ...
            weight is defined in [RFC8402](#RFC8402).
              SID/Index/Label:
              As described in [Section 5](#PREFIXSID)
...
The LAN Adjacency SID is an optional sub-TLV of the Extended Link TLV defined in 
        [RFC7684](#RFC7684). It **MAY** appear multiple
        times in the Extended Link TLV. It is used to advertise a SID/Label for an adjacency 
        to a non-DR (Designated Router) router on a broadcast, Non-Broadcast Multi-Access (NBMA), or hybrid [RFC6845](#RFC6845) network.
...
- Type:
              ...
              Flags:
              Same as in [Section 6.1](#ADJSIDSUBTLV)
              Reserved:
              **SHOULD** be set to 0 on transmission and **MUST** be ignored on reception
              MT-ID:
              Multi-Topology ID (as defined in [RFC4915](#RFC4915))
...
An OSPFv2 router that supports Segment Routing **MAY** advertise Prefix-
        ...
        a loopback IP address as described in [Section 5](#PREFIXSID)).
...
An SR Mapping Server **MUST** use the OSPF Extended Prefix Range TLV when advertising SIDs
...
Area-scoped OSPF Extended Prefix Range TLVs are propagated between areas. Similar 
...
When propagating an OSPF Extended Prefix Range TLV between areas, ABRs **MUST** set the
        IA-Flag. This is used to prevent redundant flooding of the OSPF Extended 
        Prefix Range TLV between areas as described in [Section 4](#PFXRANGE).
...
When an OSPF ABR advertises a Type-3 Summary LSA from an intra-area
        ...
        Prefix Opaque LSA as described in [RFC7684](#RFC7684). 
...
- The ABR will look at its best path to the prefix in the source
            ...
            path to that prefix.
- The ABR will then determine if this router advertised a
            Prefix-SID for the prefix and use it when advertising the
            Prefix-SID to other connected areas.
...
Type-5 LSAs are flooded domain wide. When an ASBR, which supports
        ...
        OSPF Extended Prefix Opaque LSAs as described in [RFC7684](#RFC7684).  The flooding scope of the
...
When a Not-So-Stubby Area (NSSA) [RFC3101](#RFC3101) ABR translates Type-7 LSAs into Type-5 
...
The Adjacency Segment Routing Identifier (Adj-SID) is advertised
        using the Adj-SID Sub-TLV as described in [Section 6](#ADJSID).

### Advertisement of Adj-SID on Point-to-Point Links

An Adj-SID **MAY** be advertised for any adjacency on a point-to-point (P2P) link that is
          ...
          **MAY** be removed from the area. If the adjacency transitions to a
          state lower than 2-Way, then the Adj-SID Advertisement **MUST** be withdrawn from the
          area.

### Adjacency SID on Broadcast or NBMA Interfaces

Broadcast, NBMA, or hybrid [RFC6845](#RFC6845) networks in OSPF are 
          ...
          state with each other and are directly reachable.
...
When Segment Routing is used, each router on the broadcast, NBMA, or hybrid
          ...
          the Adj-SID Sub-TLV as described in [Section 6.1](#ADJSIDSUBTLV).

SR-capable routers **MAY** also advertise a LAN Adjacency SID for other neighbors
          (e.g., Backup Designated Router, DR-OTHER, etc.) on the broadcast, NBMA, or hybrid network using the 
          LAN Adj-SID Sub-TLV as described in [Section 6.2](#LANADJSIDSUBTLV).

<a name="IANA"></a>
# IANA Considerations

This specification updates several existing OSPF
      registries and creates a new IGP registry.
...
**Table: OSPF Router Information (RI) TLVs**
...
**Table: OSPFv2 Extended Prefix Opaque LSA TLVs**
...
**Table: OSPFv2 Extended Prefix TLV Sub-TLVs**
...
**Table: OSPFv2 Extended Link TLV Sub-TLVs**
...
IANA has set up a subregistry called "IGP Algorithm Type" under the
       ...
       ([RFC8126](#RFC8126) and [RFC7120](#RFC7120)).
...
**Table: IGP Algorithm Types**

| Value | Description | Reference |
| --- | --- | --- |
| 0 | Shortest Path First (SPF) algorithm based on link metric. This is 
                ...
                its local policy. | This document |
...
ignored. An error **SHOULD** be logged subject to rate limiting.

<a name="Security"></a>
# Security Considerations

With the OSPFv2 Segment Routing extensions defined herein,
      OSPFv2 will now program the MPLS data plane [RFC3031](#RFC3031) in addition to the IP
      data plane. Previously, LDP [RFC5036](#RFC5036) or another label distribution
      mechanism was required to advertise MPLS labels and program the MPLS data plane.
...
Existing security extensions as described in [RFC2328](#RFC2328) and
       [RFC7684](#RFC7684) apply to these Segment Routing extensions. While 
       ...
       in [RFC7474](#RFC7474) **SHOULD** be used.

Implementations **MUST** assure that malformed TLVs and sub-TLVs defined in this document 
       are detected and do not provide a vulnerability for attackers to crash the OSPFv2 
       router or routing process. Reception of malformed TLVs or sub-TLVs **SHOULD** be counted 
       ...
       from overloading the OSPF control plane.
...
<a name="RFC6845"></a> **[RFC6845]** N. Sheth, L. Wang, J. Zhang, "OSPF Hybrid Broadcast and Point-to-Multipoint Interface Type", RFC 6845, DOI 10.17487/RFC6845, January 2013. <https://www.rfc-editor.org/info/rfc6845> [Local MD](rfc6845.md)
...
<a name="RFC8660"></a> **[RFC8660]** A Bashandy, C Filsfils, S Previdi, B Decraene, S Litkowski, R Shakir, "Segment Routing with MPLS Data Plane", RFC 8660, DOI 10.17487/RFC8660, December 2019. <https://www.rfc-editor.org/info/rfc8660> [Local MD](rfc8660.md)

<a name="RFC8661"></a> **[RFC8661]** A Bashandy, C Filsfils, S Previdi, B Decraene, S Litkowski, "Segment Routing Interworking with LDP", RFC 8661, DOI 10.17487/RFC8661, December 2019. <https://www.rfc-editor.org/info/rfc8661> [Local MD](rfc8661.md)
...
Thanks to Acee Lindem for the detailed review of the document, corrections, 
...
The following people gave a substantial contribution to the content
      ...
      Stephane Litkowski, Igor Milojevic, and Saku Ytti.
...
- Address: Apollo Business Center, Bratislava, 821 09, Slovakia
- Email: ppsenak@cisco.com

**Stefano Previdi**

- Organization: Cisco Systems, Inc.
- Address: Via Del Serafico, 200, Rome, 00142, Italy
- Email: stefano@previdi.net
...
- Organization: Nokia
- Address: Copernicuslaan 50, Antwerp, 2018, Belgium
- Email: wim.henderickx@nokia.com
