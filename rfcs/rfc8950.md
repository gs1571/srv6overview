...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-bess-rfc5549revision-06
...
**Keywords:** bgp, mvpn, vpnv4, vpnv6

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-bess-rfc5549revision-06)
- [alternate](https://dx.doi.org/10.17487/rfc8950)
...
- [1. Introduction](#intro)
  - [1.1. Requirements Language](#requirements)
- [2. Changes Compared to RFC 5549](#diff)
- [3. Extension of AFI/SAFI Definitions for the IPv4 Address Family](#extension)
- [4. Use of BGP Capability Advertisement](#bgp-cap)
- [5. Operations](#operations)
- [6. Usage Examples](#examples)
  - [6.1. IPv4 over IPv6 Core](#example-ipv4oipv6)
  - [6.2. IPv4 VPN Unicast over IPv6 Core](#example-vpnv4unoipv6)
  - [6.3. IPv4 VPN Multicast over IPv6 Core](#example-vpnv4multoipv6)
...
Multiprotocol BGP (MP-BGP) [RFC4760](#RFC4760) specifies that the set of
   ...
   [RFC6074](#RFC6074)) to perform Layer 2 Virtual
   ...
   <1/132> (defined in [RFC4684](#RFC4684)) to advertise Route Target
...
Furthermore, a number of these existing AFIs/SAFIs allow the next hop
   ...
   For example, [RFC4684](#RFC4684) allows the next-hop
...
There are situations such as those described in [RFC4925](#RFC4925) and [RFC5565](#RFC5565) where carriers (or large
...
The AFI/SAFI definitions for the IPv6 address family assume
   ...
   Specifically, as per [RFC2545](#RFC2545) and [RFC8277](#RFC8277), when the <AFI/SAFI> is
   <2/1>, <2/2>, or <2/4>, the next-hop address is assumed
   to be of an IPv6
   type.  As per [RFC4659](#RFC4659), when the <AFI/SAFI> is <2/128>, the next-hop
   address is assumed to be of a VPN-IPv6 type.

However, [RFC4798](#RFC4798) and [RFC4659](#RFC4659)
   specify how an IPv4 address can be
   encoded inside the next-hop IPv6 address field when IPv6 NLRI needs
   to be advertised with an IPv4 next hop.  [RFC4798](#RFC4798) defines how the
   IPv4-mapped IPv6 address format specified in the IPv6 addressing
   architecture ([RFC4291](#RFC4291)) can be
   used for that purpose when the <AFI/SAFI> is <2/1>,
   <2/2>, or <2/4>.  [RFC4659](#RFC4659) defines how the IPv4-mapped IPv6 address format
...
Similarly, the AFI/SAFI definitions for the advertisement of IPv4
   ...
   IPv4 address family type.  Specifically, as per [RFC4760](#RFC4760) and
   [RFC8277](#RFC8277), when the <AFI/SAFI> is <1/1>, <1/2>, or <1/4>, the next-hop address is assumed to be of an IPv4 type.  As per [RFC4364](#RFC4364), when
   the <AFI/SAFI> is <1/128>, the next-hop address is assumed to
   be of a VPN-IPv4 type.  As per [RFC6513](#RFC6513) and [RFC6514](#RFC6514), when
...
This document obsoletes [RFC5549](#RFC5549).
...
This document introduces two significant changes compared to [RFC5549](#RFC5549):

- In [RFC5549](#RFC5549), when AFI/SAFI <1/128>
	...
	(see Sections [3](#extension) and [6.2](#example-vpnv4unoipv6)). This change
	addresses Erratum ID 5253 ([Err5253](#Err5253)).
	As all known and deployed implementations are interoperable today and use the new proposed encoding, the change does not break existing interoperability.
- This document allows AFI/SAFI <1/129> (IPv4 multicast) to use an
	...
	<1/128> (see Sections [3](#extension) and [6.3](#example-vpnv4multoipv6)).
...
Specifically, this document allows advertising the MP_REACH_NLRI attribute [RFC4760](#RFC4760) with this content:
...
- Next Hop Address = IPv6 address of a next hop (potentially followed
      ...
      be constructed as per [RFC2545](#RFC2545).
- NLRI = NLRI as per the AFI/SAFI definition

It also allows advertising the MP_REACH_NLRI attribute [RFC4760](#RFC4760) with this content:

- AFI = 1
- SAFI = 128 or 129
- Length of Next Hop Address = 24 or 48
- Next Hop Address = VPN-IPv6 address of a next hop with an 8-octet RD set to zero (potentially followed
      by the link-local VPN-IPv6 address of the next hop with an 8-octet RD set to zero).
- NLRI = NLRI as per the AFI/SAFI definition
...
Note that this method of using the Length of Next Hop Address
   ...
   definition) is the same as that used in [RFC4684](#RFC4684) and [RFC6074](#RFC6074).

<a name="bgp-cap"></a>
# Use of BGP Capability Advertisement

[RFC5492](#RFC5492) defines a mechanism to allow two BGP speakers to discover
   ...
   capability that can be advertised using [RFC5492](#RFC5492), referred to as the "Extended Next Hop Encoding capability".  This
   ...
   Address field, as specified in [Section 3](#extension).
...
A BGP speaker that wishes to advertise an IPv6 next hop for IPv4 NLRI
   ...
   the Capability Advertisement procedures defined in [RFC5492](#RFC5492) with the
...
- The Capability Value field has the following format:
	    ...
	    that the NLRI of <NLRI AFI / NLRI SAFI> may be advertised with a next-hop address belonging to the network-layer protocol of Nexthop
            AFI.
            the AFI and SAFI values are defined in the "Address
	    Family Numbers"
         and "Subsequent Address Family Identifier (SAFI) Parameters" registries
         (see [IANA-AFI](#IANA-AFI) and [IANA-SAFI](#IANA-SAFI), respectively).
  - each triple <NLRI AFI, NLRI SAFI, Nexthop AFI> indicates
	    that the NLRI of <NLRI AFI / NLRI SAFI> may be advertised with a next-hop address belonging to the network-layer protocol of Nexthop
            AFI.
  - the AFI and SAFI values are defined in the "Address
...
This document does not specify the use of the Extended Next Hop Encoding capability with any other combinations of <NLRI AFI,
   ...
   IPv6 next hops (e.g., AFI/SAFI = <1/132> as defined in [RFC4684](#RFC4684)).
   ...
   [RFC4798](#RFC4798) and AFI/SAFI = <2/128> with
...
A BGP speaker **MUST** only advertise the IPv4 or VPN-IPv4
...
<a name="example-ipv4oipv6"></a>
## IPv4 over IPv6 Core

The extensions defined in this document may be used as discussed in
   [RFC5565](#RFC5565) for the interconnection of IPv4 islands over an IPv6
   backbone.  In this application, Address Family Border Routers (AFBRs;
   as defined in [RFC4925](#RFC4925)) advertise IPv4 NLRI in the MP_REACH_NLRI
...
<a name="example-vpnv4multoipv6"></a>
...
This document does not define any new code points from those
      included in [RFC5549](#RFC5549).

[RFC5549](#RFC5549) added "Extended
      Next Hop Encoding" to the "Capability Codes" registry ([IANA-CAP-CODE](#IANA-CAP-CODE)), which was created by [RFC5492](#RFC5492).  
...
However, as [RFC4272](#RFC4272) discusses, BGP is vulnerable to traffic diversion attacks.
...
Although not expected to be the typical case, the IPv6 address used
   ...
   defined in [RFC4291](#RFC4291)).  Configuration of the security mechanisms
...
<a name="RFC5492"></a> **[RFC5492]** J. Scudder, R. Chandra, "Capabilities Advertisement with BGP-4", RFC 5492, DOI 10.17487/RFC5492, February 2009. <https://www.rfc-editor.org/info/rfc5492> [Local MD](rfc5492.md)
...
<a name="Err5253"></a> **[Err5253]** "Erratum ID 5253", RFC 5549 <https://www.rfc-editor.org/errata/eid5253>

<a name="IANA-AFI"></a> **[IANA-AFI]** "Address Family Numbers", <https://www.iana.org/assignments/address-family-numbers/>

<a name="IANA-CAP-CODE"></a> **[IANA-CAP-CODE]** "Capability Codes", <https://www.iana.org/assignments/capability-codes/>
...
<a name="RFC4798"></a> **[RFC4798]** J. De Clercq, D. Ooms, S. Prevost, F. Le Faucheur, "Connecting IPv6 Islands over IPv4 MPLS Using IPv6 Provider Edge Routers (6PE)", RFC 4798, DOI 10.17487/RFC4798, February 2007. <https://www.rfc-editor.org/info/rfc4798> [Local MD](rfc4798.md)

<a name="RFC4925"></a> **[RFC4925]** X. Li, S. Dawkins, D. Ward, A. Durand, "Softwire Problem Statement", RFC 4925, DOI 10.17487/RFC4925, July 2007. <https://www.rfc-editor.org/info/rfc4925> [Local MD](rfc4925.md)

<a name="RFC5549"></a> **[RFC5549]** F. Le Faucheur, E. Rosen, "Advertising IPv4 Network Layer Reachability Information with an IPv6 Next Hop", RFC 5549, DOI 10.17487/RFC5549, May 2009. <https://www.rfc-editor.org/info/rfc5549> [Local MD](rfc5549.md)
...
<a name="RFC6074"></a> **[RFC6074]** E. Rosen, B. Davie, V. Radoaca, W. Luo, "Provisioning, Auto-Discovery, and Signaling in Layer 2 Virtual Private Networks (L2VPNs)", RFC 6074, DOI 10.17487/RFC6074, January 2011. <https://www.rfc-editor.org/info/rfc6074> [Local MD](rfc6074.md)
...
The authors would like to thank Francois Le Faucheur and Eric Rosen for their work on [RFC5549](#RFC5549).

The authors would like to thank Yakov Rekhter, Pranav Mehta, and John    Scudder for their contributions to the approach defined in [RFC5549](#RFC5549).
