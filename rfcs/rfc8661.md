...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-spring-segment-routing-ldp-interop-15
...
- [prev](https://datatracker.ietf.org/doc/draft-ietf-spring-segment-routing-ldp-interop-15)
- [alternate](https://dx.doi.org/10.17487/rfc8661)
...
Information about the current status of this document, any
            ...
            [https://www.rfc-editor.org/info/rfc8661](https://www.rfc-editor.org/info/rfc8661).
...
- [2. SR-LDP Ships-in-the-Night Coexistence](#sec-2)
  - [2.1. MPLS2MPLS, MPLS2IP, and IP2MPLS Coexistence](#sec-2.1)
- [3. SR and LDP Interworking](#sec-4)
  - [3.1. LDP to SR](#sec-4.1)
    - [3.1.1. LDP to SR Behavior](#sec-4.1.1)
  - [3.2. SR to LDP](#sec-4.2)
    - [3.2.1. Segment Routing Mapping Server (SRMS)](#sec-4.2.1)
    - [3.2.2. SR to LDP Behavior](#sec-4.2.2)
    - [3.2.3. Interoperability of Multiple SRMSes and Prefix-SID Advertisements](#sec-4.2.3)
- [4. SR-LDP Interworking Use Cases](#sec-5)
  - [4.1. SR Protection of LDP-Based Traffic](#sec-5.1)
  - [4.2. Eliminating Targeted LDP Sessions](#sec-5.2)
  - [4.3. Guaranteed FRR Coverage](#sec-5.3)
  - [4.4. Inter-AS Option C, Carrier's Carrier](#sec-5.4)
- [5. IANA Considerations](#sec-6)
- [6. Manageability Considerations](#sec-7)
  - [6.1. SR and LDP Coexistence](#sec-7.1)
  - [6.2. Data-Plane Verification](#sec-7.3)
- [7. Security Considerations](#sec-8)
- [8. References](#section-8)
  - [8.1. Normative References](#section-8.1)
  - [8.2. Informative References](#section-8.2)
- [Appendix A. Migration from LDP to SR](#Appendix-A)
...
Segment Routing, as described in [RFC8402](#RFC8402), can be used on top of the
      MPLS data plane without any modification as described in [RFC8660](#RFC8660).

Segment Routing control plane can coexist with current label
      distribution protocols such as LDP [RFC5036](#RFC5036).
...
[Section 2](#sec-2) describes the coexistence of SR with
      other MPLS control-plane protocols. [Section 3](#sec-4) documents
      the interworking between SR and LDP in the case of nonhomogeneous
      deployment. [Section 4](#sec-5) describes how a partial SR
      ...
      MPLS use cases. [Appendix A](#Appendix-A) documents a method to
      migrate from LDP to SR-based MPLS tunneling.
...
"MPLS Control-Plane Client (MCC)" refers to any control-plane
      ...
      [RFC5036](#RFC5036), RSVP-TE [RFC3209](#RFC3209), BGP [RFC8277](#RFC8277), etc., are examples of MCCs.
...
Segment Routing makes use of the Segment Routing Global Block (SRGB,
      as defined in [RFC8402](#RFC8402)) for the
...
Note that this static label-allocation capability of the label
      ...
      range is reserved by the label manager, and it is the MPLS-TP [RFC5960](#RFC5960) Network Management System (acting as an MCC) that ensures the unique
      allocation of any label within the allocated range and the creation of
      the related MPLS forwarding entry.
...
The EVEN VPN service is supported by PE2 and PE4 while the ODD VPN
      ...
      ODD service via LDP and the EVEN service via SR.

This can be achieved in the following manner:

- The operator configures PE1, PE2, PE3, and PE4 with respective loopbacks
192.0.2.201/32, 192.0.2.202/32, 192.0.2.203/32, and 192.0.2.204/32. These PEs
advertised their VPN routes with next hop set on their respective loopback
address.
- The operator configures A, B, C with respective loopbacks 192.0.2.1/32,
192.0.2.2/32, 192.0.2.3/32.
- The operator configures PE2, A, B, C, and PE4 with SRGB [100, 300].
- The operator attaches the respective Node Segment Identifiers (Node SIDs,
as defined in [RFC8402](#RFC8402)), 202, 101, 102, 103,
...
From an LDP viewpoint, PE1 received an LDP label binding (1037) for a
      ...
      and C received implicit NULL LDP binding from its next-hop PE3.
...
From an SR viewpoint, PE2 maps the IGP route 192.0.2.204/32 onto
      Node SID 204; Node A swaps 204 with 204 and forwards to B; B swaps 204
...
- The ODD service is tunneled from PE1 to PE3 through a continuous LDP LSP
traversing A, B, and C.
- The EVEN service is tunneled from PE2 to PE4 through a continuous SR node
segment traversing A, B, and C.

<a name="sec-2.1"></a>
## MPLS2MPLS, MPLS2IP, and IP2MPLS Coexistence

MPLS2MPLS refers to the forwarding behavior where a router receives
...
These two entries can coexist because their incoming label is
...
The same applies for the MPLS2IP forwarding entries. MPLS2IP is the
        ...
        IPv4/IPv6. For IP2MPLS coexistence, refer to [Section 6.1](#sec-7.1).
...
- PE1, PE2, P5, and P6 are SR capable. PE1, PE2, P5, and P6 are configured
with SRGB (100, 200) and with node segments 101, 102, 105, and 106, respectively.
- A service flow must be tunneled from PE1 to PE3 over a continuous MPLS
...
The end-to-end MPLS tunnel is built by stitching an LDP LSP from PE3 to P6
        and the related node segment from P6 to PE1.

<a name="sec-4.1.1"></a>
### LDP to SR Behavior

It has to be noted that no additional signaling or state is
...
An SR node having LDP neighbors **MUST** create LDP bindings for each
          ...
          distribution control modes as defined in [RFC5036](#RFC5036).
...
This section defines the Segment Routing Mapping Server (SRMS). The
        ...
        specific and defined in [RFC8665](#RFC8665), [RFC8666](#RFC8666), and [RFC8667](#RFC8667).
...
The SRMS is a control-plane-only function that may be located
        ...
        exist in a routing domain to advertise Prefix-SIDs on behalf of non‑SR
        ...
        from inconsistent advertisements are addressed by [RFC8660](#RFC8660).

The example depicted in [Figure 2](#ref-sr-and-ldp-interworking) assumes that the operator
        configures P5 to act as a Segment Routing Mapping Server and
...
PE1 has a service route whose next hop is PE3. PE1 has a node
        ...
        label and the top label is 103.

P5 swaps 103 for 103 and forwards to P6.

P6's next hop for the IGP route "PE3" is not SR capable (P7 does
        not advertise the SR capability). However, P6 has an LDP label binding
        from that next hop for the same FEC (e.g., LDP label 1037). Hence, P6
        swaps 103 for 1037 and forwards to P7.

P7 swaps this label with the LDP label received from P8 and
        forwards to P8.
...
The end-to-end MPLS tunnel is built by stitching an SR node segment from
        PE1 to P6 and an LDP LSP from P6 to PE3.
...
SR-mapping advertisement for a given prefix provides no information
        ...
        mechanisms ([RFC8665](#RFC8665),
        [RFC8666](#RFC8666), and [RFC8667](#RFC8667)), **MAY** be
...
The purpose of SRMS functionality is to support the
          ...
          advertisement. Examples of explicit Prefix-SID Advertisement are the
          Prefix-SID sub-TLVs defined in [RFC8665](#RFC8665), [RFC8666](#RFC8666), and [RFC8667](#RFC8667).

The SRMS functionality allows assigning of Prefix-SIDs to
...
The SRMS functionality consists of two functional blocks: the
          Mapping Server (MS) and Mapping Client (MC).

An MS is a node that advertises an SR mappings. Advertisements
          sent by an MS define the assignment of a Prefix-SID to a prefix
...
An MC is a node that receives and uses the MS mapping
          ...
          SR-mapping advertisement from a Mapping Server and also has received
          a Prefix-SID Advertisement for that same prefix in a prefix
          reachability advertisement, then the MC **MUST** prefer the SID
          ...
          **MUST** be ignored for that prefix. Hence, assigning a Prefix-SID to a
          prefix using the SRMS functionality does not preclude assigning the
          same or different Prefix-SID(s) to the same prefix using explicit
          Prefix-SID Advertisement such as the aforementioned Prefix-SID
          sub-TLVs.
...
For example, consider an IPv4 prefix advertisement received by an
          ...
          prefix. Refer to [RFC8667](#RFC8667) for details
          about the Prefix-SID sub-TLV and SID/Label Binding TLV.
...
An SR node having LDP-only neighbors **MUST** stitch the incoming SR label
          (whose SID is advertised by the SRMS) to the outgoing LDP label.

It has to be noted that the SR to LDP behavior does not propagate
          the status of the LDP FEC that was signaled by LDP when configured
          in ordered mode.

It has to be noted that in the case of SR to LDP, the label
          binding is equivalent to the independent LDP Label Distribution
          Control Mode [RFC5036](#RFC5036) where a label is bound to a
          FEC independently from the received binding for the same FEC.

<a name="sec-4.2.3"></a>
### Interoperability of Multiple SRMSes and Prefix-SID Advertisements
...
The content of SRMS advertisement (i.e., mappings) are a matter
...
If a node acts as an SRMS, it **MAY** advertise a preference to be
          ...
          protocol-specific documents, e.g., [RFC8665](#RFC8665), [RFC8666](#RFC8666), and [RFC8667](#RFC8667). The
          preference value is an unsigned 8-bit integer with the following
          properties:

| 0 | Reserved value indicating advertisements from that
...
- For any prefix for which it did not receive a Prefix-SID
              ...
              specifications [RFC8665](#RFC8665),
              [RFC8666](#RFC8666), and
              [RFC8667](#RFC8667).
- If there is an incoming label collision as specified in [RFC8660](#RFC8660), apply the
              steps specified in [RFC8660](#RFC8660) to resolve the
              collision.

When the SRMS advertises mappings, an implementation should
          ...
          SRMS and the ones advertised by LDP.

<a name="sec-5"></a>
# SR-LDP Interworking Use Cases
...
In [Figure 3](#ref-sr-ldp-interworking-example), let us assume:
...
- X, Y, and Z are not SR capable, e.g., as part of a
          staged migration from LDP to SR, the operator deploys SR first in
          a subpart of the network and then everywhere.
...
- To protect the link BA along the shortest-path of the important flow XY,
B requires a Remote Loop-Free Alternate (RLFA; see [RFC7490](#RFC7490)) repair tunnel to D and, therefore, a targeted LDP session
...
running in the network and, therefore, simplify network operations.
- There is no LFA/RLFA solution to protect the link BE along the shortest
path of the important flow XZ. The operator wants a guaranteed link-based FRR solution.

The operator can meet these objectives by deploying SR only on A,
        B, C, D, E, F, and G:

- The operator configures A, B, C, D, E, F, and G with SRGB [100, 200] and
with node segments 101, 102, 103, 104, 105, 106, and 107, respectively.
- The operator configures D as an SR Mapping Server with the following
policy mapping: (X, 201), (Y, 202), and (Z, 203).
- Each SR node automatically advertises a local adjacency segment for its
...
The RLFA paths are dynamically precomputed as defined in [RFC7490](#RFC7490). Typically, implementations allow to enable an RLFA
        ...
        scope of this document and not relevant to the aspects of SR-LDP
        interwork explained in this document.
...
- Incoming label: local LDP label bound by B for FEC Y
            Outgoing label: LDP label bound by A for FEC Y
              ...
              Outgoing next hop: A
              Backup next hop: repair tunnel: node segment to D {104} with outgoing
 next hop: C
  - Outgoing label: LDP label bound by A for FEC Y
  - Backup outgoing label: SR node segment for Y {202}
  - Outgoing next hop: A
...
It has to be noted that D is selected as a Remote Loop-Free Alternate
        (RLFA) as defined in [RFC7490](#RFC7490).
...
In steady state, X sends its Y-destined traffic to B with a top
        ...
        the LDP label and forwards to Y.

Upon failure of the link BA, B swaps the incoming top label with
        ...
        capable, and therefore, Node A swaps the incoming node segment 202 to the
...
As mentioned in [Section 4.1](#sec-5.1), in the example
        topology described in [Figure 3](#ref-sr-ldp-interworking-example), there is no RLFA-based solution for
        ...
        [RFC7490](#RFC7490) for details). However:
...
- G is reachable from B via the combination of a
        ...
        FG {9001}.
- Note that {106, 107} would have equally worked.
...
In steady state, X sends its Z-destined traffic to B with a top
        ...
        the LDP label and forwards to Z.

Upon failure of the link BE, B swaps the incoming top label with
        ...
        segment 203 for the LDP label announced by its next hop (in this case,
        implicit NULL).
...
- FRR coverage has been achieved without any signaling for
            ...
            session between B and G.

<a name="sec-5.4"></a>
## Inter-AS Option C, Carrier's Carrier

In inter-AS Option C [RFC4364](#RFC4364), two interconnected
...
```
In Inter-AS Option C, B2 advertises to B1 a labeled BGP route [RFC8277](#RFC8277) for PE2, and B1 reflects it to its
        internal peers, e.g., PE1. PE1 learns from a service route reflector a
        service route whose next hop is PE2. PE1 resolves that service route
        on the labeled BGP route to PE2. That labeled BGP route to PE2 is
        itself resolved on the AS1 IGP route to B1.

If AS1 operates SR, then the tunnel from PE1 to B1 is provided by
        the node segment from PE1 to B1.

PE1 sends a service packet with three labels: the top one is the
...
When Label switch paths (LSPs) are defined by stitching LDP LSPs
        ...
        path. These mechanisms are described in [RFC8287](#RFC8287).

<a name="sec-8"></a>
# Security Considerations

This document does not introduce any change to the MPLS data plane
      [RFC3031](#RFC3031) and therefore no additional security of the
      MPLS data plane is required.

This document introduces another form of label binding advertisements. The
security associated with these advertisements is part of the security applied
to routing protocols such as IS-IS [RFC5304](#RFC5304)
and OSPF [RFC5709](#RFC5709), which both optionally make
use of cryptographic authentication mechanisms. This form of advertisement is
more centralized, on behalf of the node advertising the IP reachability, which
presents a different risk profile. This document also specifies a mechanism by
which the ill effects of advertising conflicting label bindings can be
mitigated. In particular, advertisements from the node advertising the IP
reachability is more preferred than the centralized one. This document
...
forwarding. Therefore, this document recommends the operator implement strict
configuration/programmability control, the monitoring of the advertised SIDs,
the preference of an IP reachability SID Advertisement over a centralized SID
...
<a name="RFC8660"></a> **[RFC8660]** A Bashandy, C Filsfils, S Previdi, B Decraene, S Litkowski, R Shakir, "Segment Routing with MPLS Data Plane", December 2019. <https://www.rfc-editor.org/info/rfc8660> [Local MD](rfc8660.md)
...
<a name="RFC5709"></a> **[RFC5709]** M. Bhatia, V. Manral, M. Fanto, R. White, M. Barnes, T. Li, R. Atkinson, "OSPFv2 HMAC-SHA Cryptographic Authentication", RFC 5709, DOI 10.17487/RFC5709, October 2009. <https://www.rfc-editor.org/info/rfc5709> [Local MD](rfc5709.md)

<a name="RFC5960"></a> **[RFC5960]** D. Frost, S. Bryant, M. Bocci, "MPLS Transport Profile Data Plane Architecture", RFC 5960, DOI 10.17487/RFC5960, August 2010. <https://www.rfc-editor.org/info/rfc5960> [Local MD](rfc5960.md)

<a name="RFC7490"></a> **[RFC7490]** S. Bryant, C. Filsfils, S. Previdi, M. Shand, N. So, "Remote Loop-Free Alternate (LFA) Fast Reroute (FRR)", RFC 7490, DOI 10.17487/RFC7490, April 2015. <https://www.rfc-editor.org/info/rfc7490> [Local MD](rfc7490.md)
...
<a name="RFC8665"></a> **[RFC8665]** P Psenak, S Previdi, C Filsfils, H Gredler, R Shakir, W Henderickx, J Tantsura, "OSPF Extensions for Segment Routing", December 2019. <https://doi.org/10.17487/RFC8665> [Local MD](rfc8665.md)

<a name="RFC8666"></a> **[RFC8666]** P Psenak, S Previdi, "OSPFv3 Extensions for Segment Routing", December 2019. <https://doi.org/10.17487/RFC8666> [Local MD](rfc8666.md)

<a name="RFC8667"></a> **[RFC8667]** S Previdi, L Ginsberg, C Filsfils, A Bashandy, H Gredler, B Decraene, "IS-IS Extensions for Segment Routing", December 2019. <https://doi.org/10.17487/RFC8667> [Local MD](rfc8667.md)
...
At time T1, all the routers are upgraded to SR. They are configured
      ...
      106, and 107 (attached to their service-recursing loopback).
