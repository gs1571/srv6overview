...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-spring-nsh-sr-15
...
**Keywords:** NSH, SR, MPLS-SR, SRv6, SFC

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-spring-nsh-sr-15)
- [alternate](https://dx.doi.org/10.17487/rfc9491)
...
Information about the current status of this document, any
            ...
            [https://www.rfc-editor.org/info/rfc9491](https://www.rfc-editor.org/info/rfc9491).
...
- [1. Introduction](#section-1)
  - [1.1. SFC Overview and Rationale](#section-1.1)
  - [1.2. Requirements Language](#section-1.2)
- [2. SFC within Segment Routing Networks](#section-2)
- [3. NSH-Based SFC with SR-MPLS or the SRv6 Transport Tunnel](#section-3)
- [4. SR-Based SFC with the Integrated NSH Service Plane](#section-4)
- [5. Packet Processing for SR-Based SFC](#sec-5)
  - [5.1. SR-Based SFC (SR-MPLS) Packet Processing](#section-5.1)
  - [5.2. SR-Based SFC (SRv6) Packet Processing](#section-5.2)
- [6. Encapsulation](#Encapsulation)
  - [6.1. NSH Using SR-MPLS Transport](#MPLS-SR)
  - [6.2. NSH Using SRv6 Transport](#SRv6)
- [7. Security Considerations](#Security)
- [8. Backwards Compatibility](#section-8)
- [9. Caching Considerations](#section-9)
- [10. MTU Considerations](#section-10)
- [11. IANA Considerations](#IANA)
  - [11.1. Protocol Number for the NSH](#NSHPROTO)
  - [11.2. SRv6 Endpoint Behavior for the NSH](#NSHEPB)
...
The dynamic enforcement of a service-derived and adequate
...
In order to solve those problems and to decouple the service's
        ...
        simplified service delivery, SFC techniques have been introduced [RFC7665](#RFC7665).
...
SFC techniques are meant to rationalize the service delivery logic
        ...
        service functions in the right sequence.  [RFC7498](#RFC7498) provides an overview of the overall SFC problem
        space, and [RFC7665](#RFC7665) specifies an SFC
...
Many approaches can be considered for encoding the information
        ...
        encapsulation scheme: NSH [RFC8300](#RFC8300). This design is pragmatic, as it does not require
...
[RFC8300](#RFC8300) specifies how to
...
As described in [RFC8402](#RFC8402), SR
      ...
      source routing, SR also finds its application in supporting SFC [SERVICE-PROGRAMMING](#I-D.ietf-spring-sr-service-programming).

The two SR data plane encapsulations, namely [SR-MPLS](#RFC8660) and [Segment Routing over IPv6 (SRv6)](#RFC8754), can encode an
      SF as a segment so that a service function chain can be specified as a segment
      list. Nevertheless, and as discussed in [RFC7498](#RFC7498), traffic steering is only a subset of the issues that
...
Referring to [Figure 1](#figure_1), packets of
      ...
      first SFF hop for this service function chain).
...
```
Referring now to [Figure 2](#figure_2), DC1-GW1
...
```
When the packet arrives at DC2, as shown in [Figure 3](#figure_3), the SR encapsulation is removed, and DC2-GW1
...
In this scenario, we assume that the SFs are NSH-aware; therefore,
...
When a packet has to be forwarded to an SF attached to an SFF, the
      ...
      the SF. In the case of SR-MPLS, this will be a Prefix-SID [RFC8402](#RFC8402). In the case of SRv6, the behavior
      described within this document is assigned the name END.NSH, and [Section 11.2](#NSHEPB) describes the allocation of the code point by IANA. The
...
[Figure 4](#figure_4) illustrates an example of
      this scenario.
...
When an SFF receives a packet destined to S and S is a local
...
This section describes the End.NSH behavior and NSH processing
        logic for SRv6. The pseudocode is shown below.

When N receives a packet destined to S and S is a local End.NSH
        SID, the processing is the same as that specified by [RFC8754](#RFC8754), up through
        line S15.
...
When carrying an NSH within an SR-MPLS transport, the full
        encapsulation headers are as illustrated in [Figure 5](#figure_5).
...
If the NEXT operation is to be used, then at the end of the
        ...
        that the NSH follows the SR-MPLS label stack as described by [RFC8596](#RFC8596).
...
If the CONTINUE operation is to be used, this is the equivalent of
        ...
        Prefix-SID Sub-TLV [RFC8667](#RFC8667) [RFC8665](#RFC8665). It is **RECOMMENDED**
...
When carrying a NSH within an SRv6 transport, the full encapsulation
        is as illustrated in [Figure 6](#figure_6).
...
Encapsulation of the NSH following SRv6 is indicated by the IP protocol
        number for the NSH in the Next Header of the SRH.

<a name="Security"></a>
# Security Considerations

Generic SFC-related security considerations are discussed in [RFC7665](#RFC7665).

NSH-specific security considerations are discussed in [RFC8300](#RFC8300).

Generic security considerations related to segment routing are
      discussed in [RFC8754](#RFC8754) and
      [RFC8663](#RFC8663).

# Backwards Compatibility

For SRv6/IPv6, if a processing node does not recognize the NSH, it should
      follow the procedures described in [RFC8200](#RFC8200). For SR-MPLS, if a processing node does
      not recognize the NSH, it should follow the procedures laid out in [RFC3031](#RFC3031).
...
Aligned with [RFC8300](#RFC8300)
      and [RFC8754](#RFC8754), it is
...
**Table: SRv6 Endpoint Behaviors Subregistry**
...
<a name="RFC8663"></a> **[RFC8663]** X. Xu, S. Bryant, A. Farrel, S. Hassan, W. Henderickx, Z. Li, "MPLS Segment Routing over IP", RFC 8663, DOI 10.17487/RFC8663, December 2019. <https://www.rfc-editor.org/info/rfc8663> [Local MD](rfc8663.md)
...
<a name="RFC7498"></a> **[RFC7498]** P. Quinn, T. Nadeau, "Problem Statement for Service Function Chaining", RFC 7498, DOI 10.17487/RFC7498, April 2015. <https://www.rfc-editor.org/info/rfc7498> [Local MD](rfc7498.md)
...
<a name="RFC8596"></a> **[RFC8596]** A. Malis, S. Bryant, J. Halpern, W. Henderickx, "MPLS Transport Encapsulation for the Service Function Chaining (SFC) Network Service Header (NSH)", RFC 8596, DOI 10.17487/RFC8596, June 2019. <https://www.rfc-editor.org/info/rfc8596> [Local MD](rfc8596.md)

<a name="I-D.ietf-spring-sr-service-programming"></a> **[SERVICE-PROGRAMMING]** F. Clad, X. Xu, C. Filsfils, D. Bernier, C. Li, B. Decraene, S. Ma, C. Yadlapalli, W. Henderickx, S. Salsano, "Service Programming with Segment Routing", Internet-Draft draft-ietf-spring-sr-service-programming-08, August 2023. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-spring-sr-service-programming-08>
...
- Organization: Futurewei Technologies
- Address: 2330 Central Expressway, Santa Clara, CA, United States of America
- Email: james.n.guichard@futurewei.com
