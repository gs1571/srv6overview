...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-pce-binding-label-sid-16
...
**Keywords:** PCEP, BSID, Binding, Binding Label

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-pce-binding-label-sid-16)
- [alternate](https://dx.doi.org/10.17487/rfc9604)
...
- [1. Introduction](#Introduction)
  - [1.1. Motivation and Example](#Motivation)
  - [1.2. Summary of the Extension](#Summary)
- [2. Requirements Language](#section-2)
- [3. Terminology](#section-3)
- [4. Path Binding TLV](#TE-PATH-BINDING-TLV)
  - [4.1. SRv6 Endpoint Behavior and SID Structure](#Behavior-Structure)
- [5. Operation](#Operation)
- [6. Binding SID in SR-ERO](#SR-ERO)
- [7. Binding SID in SRv6-ERO](#SRv6-ERO)
- [8. PCE Allocation of Binding Label/SID](#PCECC)
- [9. Security Considerations](#Security)
- [10. Manageability Considerations](#section-10)
  - [10.1. Control of Function and Policy](#section-10.1)
  - [10.2. Information and Data Models](#section-10.2)
  - [10.3. Liveness Detection and Monitoring](#section-10.3)
  - [10.4. Verify Correct Operations](#section-10.4)
  - [10.5. Requirements on Other Protocols](#section-10.5)
  - [10.6. Impact on Network Operations](#section-10.6)
- [11. IANA Considerations](#IANA)
  - [11.1. PCEP TLV Type Indicators](#TLV)
    - [11.1.1. TE-PATH-BINDING TLV](#IANA-TLV)
  - [11.2. LSP Object](#LSP)
  - [11.3. PCEP Error Type and Value](#Error-Type)
...
As per [RFC8402](#RFC8402), SR allows a
      head-end node to steer a packet flow along a given path via an SR
      Policy.  As per [RFC9256](#RFC9256), an SR Policy
...
As described in [RFC8402](#RFC8402), a Binding
      ...
      a global (SR Global Block (SRGB)) SID. As per [RFC9256](#RFC9256), a BSID can also be associated with
...
[RFC5440](#RFC5440) describes the PCEP for communication between
      a Path Computation Client (PCC) and a PCE or between a pair of PCEs as
      per [RFC4655](#RFC4655). [RFC8231](#RFC8231) specifies
      ...
      LSPs delegated to it. [RFC8281](#RFC8281) specifies a mechanism
...
[RFC8664](#RFC8664) provides a mechanism for a PCE (acting as a
      ...
      information on the SR Policy Architecture, see [RFC9256](#RFC9256).
...
A binding label/SID has local significance to the ingress node of
        ...
        [Figure 1](#figure1) with a multi-domain PCE. In the IP/MPLS WAN,
...
As another way to use the extension specified in this document, to
        support the PCE-based central controller [RFC8283](#RFC8283)
        ...
        inform the PCC. See [Section 8](#PCECC) for details.
...
<a name="TE-PATH-BINDING-TLV"></a>
# Path Binding TLV

The new optional TLV called "TE-PATH-BINDING TLV" (the format is
      shown in [Figure 2](#BINDING-LABEL-TLV-FORMAT)) is defined to carry
      the binding label/SID for a TE path. This TLV is associated with the LSP
      object specified in [RFC8231](#RFC8231). This TLV can also be
      carried in the PCEP-ERROR object [RFC5440](#RFC5440) in case of
...
The TE-PATH-BINDING TLV is a generic TLV such that it is able to carry
      ...
      according to the rules specified in [RFC5440](#RFC5440). The value
...
- Binding Type (BT): A one-octet field that identifies the type of
      ...
      values:
        ...
        7 (the padding is not included in the length, as per [RFC5440](#RFC5440)), and the first
        20 bits are used to encode the MPLS label value.
            BT = 1: The binding value is a 32-bit MPLS Label Stack Entry as
        per [RFC3032](#RFC3032) with Label, Traffic
        Class (TC) [RFC5462](#RFC5462), S, and TTL
        ...
        Length **MUST** be set to 20.
            BT = 3: The binding value is a 24-octet field, defined in [Section 4.1](#Behavior-Structure), that contains the SRv6
        SID as well as its Behavior and Structure. The Length
        **MUST** be set to 28.
          [Section 11.1.1](#IANA-TLV) defines the IANA registry used to maintain
...
- Flags: 1 octet of flags. The following flag is defined in the new
      "TE-PATH-BINDING TLV Flag field" registry as described in [Section 11.1.1](#IANA-TLV):
          Flags
            0 1 2 3 4 5 6 7
+-+-+-+-+-+-+-+-+
|R|             |
...
- Binding Value: A variable-length field, padded with trailing zeros to
      ...
      entry as per [RFC3032](#RFC3032). When the BT is 2, the 128 bits
      represent the SRv6 SID. When the BT is 3, the binding value also
      contains the SRv6 Endpoint Behavior and SID Structure, defined in [Section 4.1](#Behavior-Structure). In this document, the TE-PATH-BINDING TLV
      is considered to be empty if no binding value is present. Note that the
      length of the TLV would be 4 in such a case.

<a name="Behavior-Structure"></a>
...
This section specifies the format of the binding value in the
        ...
        [RFC8986](#RFC8986). The format is shown in [Figure 4](#SID-BEHAVIOR-AND-STRUCTURE).
...
- Endpoint Behavior: 2 octets. The Endpoint Behavior code point
          ...
          Behaviors" registry [RFC8986](#RFC8986). When the field is set with the value 0, the
          Endpoint Behavior is considered unknown.
- [RFC8986](#RFC8986) defines an SRv6 SID
...
The SRv6 SID Structure could be used by the PCE for ease of
        ...
        the operator as described in [RFC8986](#RFC8986).
        ...
        rules at SR domain boundaries as described in [RFC8754](#RFC8754). The details of these potential applications are
...
The binding value is usually allocated by the PCC and reported to a
      PCE via a PCRpt message (see [Section 8](#PCECC) where PCE performs the
      allocation). If a PCE does not recognize the TE-PATH-BINDING TLV, it
      would ignore the TLV in accordance with [RFC5440](#RFC5440). If a
...
If a PCE recognizes an invalid binding value (e.g., label value from
      ...
      ("Bad label value") as specified in [RFC8664](#RFC8664).
...
If a PCC receives a TE-PATH-BINDING TLV in any message other than
      ...
      the reason "Reception of a malformed PCEP message" (according to [RFC5440](#RFC5440)). Similarly, if a PCE receives a TE-PATH-BINDING TLV
      ...
      of a malformed PCEP message" (according to [RFC5440](#RFC5440)).
...
In PCEP messages, LSP route information is carried in the Explicit
      Route Object (ERO), which consists of a sequence of subobjects. [RFC8664](#RFC8664) defines the "SR-ERO subobject"
      ...
      included and NT **MUST** be set to zero. [RFC8664](#RFC8664) specifies bit
      settings and error handling in the case when NT=0.

<a name="SRv6-ERO"></a>
# Binding SID in SRv6-ERO

[RFC9603](#RFC9603) defines the "SRv6-ERO
      subobject" for an SRv6 SID. Similarly to SR-ERO ([Section 6](#SR-ERO)), the NAI **MUST NOT** be included and
      the NT **MUST** be set to zero. [RFC8664](#RFC8664) specifies bit settings and error
      handling in the case when NT=0.

<a name="PCECC"></a>
# PCE Allocation of Binding Label/SID

[Section 5](#Operation) already includes the scenario where a PCE
      ...
      described in [RFC8283](#RFC8283). Note that the act of requesting a
      specific binding value ([Section 5](#Operation)) is different from
      the act of allocating a binding label/SID as described in this
      section.

[RFC8283](#RFC8283) introduces the architecture for PCE as a
      central controller as an extension of the architecture described in
      [RFC4655](#RFC4655) and assumes the continued use of PCEP as the
      protocol used between PCE and PCC. [RFC9050](#RFC9050) specifies
      ...
      well (see [PCE-ID-SPACE](#I-D.ietf-pce-controlled-id-space)).

When PCE as a Central Controller (PCECC) operations are supported as per [RFC9050](#RFC9050),
      the binding label/SID **MAY** also be allocated by the PCE itself. Both
      peers need to exchange the PCECC capability as described in [RFC9050](#RFC9050) before the PCE can allocate the binding label/SID on
      its own.

A new P flag in the LSP object [RFC8231](#RFC8231) is introduced
      ...
      segments [PCEP-SR](#I-D.ietf-pce-sr-path-segment)) in the future.
      ...
      P (PCE-allocation): If the bit is set to 1, it indicates
        ...
        described in [Section 5](#Operation).
...
- If one or both speakers (PCE and PCC) have not indicated support
          ...
          [RFC9050](#RFC9050) and a PCEP peer receives P=1 in the LSP
          object, they **MUST**:
...
- A legacy PCEP speaker that does not recognize the P flag in the
          LSP object would ignore it in accordance with [RFC8231](#RFC8231).

It is assumed that the label range to be used by a PCE is known and
      set on both PCEP peers. The exact mechanism is out of the scope of [RFC9050](#RFC9050) and this document. Note that the specific BSID could
...
Note that the P flag in the LSP object **SHOULD NOT** be set to 1
      ...
      (such as one defined in [PCEP-SR](#I-D.ietf-pce-sr-path-segment))
...
The security considerations described in [RFC5440](#RFC5440),
      [RFC8231](#RFC8231), [RFC8281](#RFC8281), [RFC8664](#RFC8664), and [RFC9050](#RFC9050) are applicable to this
      specification. No additional security measure is required.

As described in [RFC8402](#RFC8402) and [RFC8664](#RFC8664), SR intrinsically involves an entity (whether
...
Thus, as per [RFC8231](#RFC8231), it is
      **RECOMMENDED** that these PCEP extensions only be activated
      ...
      (TLS) [RFC8253](#RFC8253), as per the
      recommendations and best current practices in RFC 9325 [BCP195](#BCP195)
      (unless explicitly set aside in
      [RFC8253](#RFC8253)).

# Manageability Considerations

All manageability requirements and considerations listed in [RFC5440](#RFC5440), [RFC8231](#RFC8231), and [RFC8664](#RFC8664) apply to PCEP protocol extensions defined in this
...
The PCEP YANG module [PCEP-YANG](#I-D.ietf-pce-pcep-yang) will
...
The mechanisms defined in this document do not imply any new
        ...
        already listed in [RFC5440](#RFC5440).
...
The mechanisms defined in this document do not imply any new
        ...
        [RFC8231](#RFC8231), and [RFC8664](#RFC8664).
...
The mechanisms defined in [RFC5440](#RFC5440), [RFC8231](#RFC8231), and [RFC8664](#RFC8664) also apply to the
...
IANA has created the "TE-PATH-BINDING TLV BT Field" registry 
          ...
          below. New values are assigned by Standards Action [RFC8126](#RFC8126).
...
IANA has created a new "TE-PATH-BINDING
          ...
          New values are to be assigned by Standards Action [RFC8126](#RFC8126). Each bit should be tracked with
...
<a name="I-D.ietf-pce-controlled-id-space"></a> **[PCE-ID-SPACE]** C. Li, H. Shi, A. Wang, W. Cheng, C. Zhou, "Path Computation Element Communication Protocol (PCEP) extension to advertise the PCE Controlled Identifier Space", Internet-Draft draft-ietf-pce-controlled-id-space-00, June 2024. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-pce-controlled-id-space-00>

<a name="I-D.ietf-pce-sr-path-segment"></a> **[PCEP-SR]** C. Li, M. Chen, W. Cheng, R. Gandhi, Q. Xiong, "Path Computation Element Communication Protocol (PCEP) Extension for Path Segment in Segment Routing (SR)", Internet-Draft draft-ietf-pce-sr-path-segment-09, February 2024. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-pce-sr-path-segment-09>
...
We would like to thank Milos Fabian, Mrinmoy Das, Andrew Stone, Tom Petch, Aijun Wang, Olivier Dugeon, and Adrian Farrel for
      their valuable comments.

Thanks to Julien Meuric for shepherding. Thanks
      to John Scudder for the AD review.

Thanks to Theresa Enghardt for the GENART review.

Thanks to Martin Vigoureux, Benjamin Kaduk, Éric Vyncke, Lars Eggert, Murray Kucherawy, and
      Erik Kline for the IESG reviews.
...
- Organization: RtBrick India
- Address: N-17L, Floor-1, 18th Cross Rd, HSR Layout Sector-3, Bangalore, Karnataka, 560102, India
...
- Organization: Cisco Systems, Inc.
- Address: De Kleetlaan 6a, Brabant, 1831, Belgium
- Email: cfilsfil@cisco.com
