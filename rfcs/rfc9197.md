...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-ippm-ioam-data-17
...
**Keywords:** inband, Telemetry, Tracing,

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-ippm-ioam-data-17)
- [alternate](https://dx.doi.org/10.17487/rfc9197)
...
Information about the current status of this document, any
            ...
            [https://www.rfc-editor.org/info/rfc9197](https://www.rfc-editor.org/info/rfc9197).
...
- [3. Scope, Applicability, and Assumptions](#IOAM_scope)
- [4. IOAM Data-Fields, Types, and Nodes](#IOAM_option_format)
  - [4.1. IOAM Data-Fields and Option-Types](#section-4.1)
  - [4.2. IOAM-Domains and Types of IOAM Nodes](#section-4.2)
  - [4.3. IOAM-Namespaces](#ioam_namespaces)
  - [4.4. IOAM Trace Option-Types](#IOAM_tracing_option)
    - [4.4.1. Pre-allocated and Incremental Trace Option-Types](#TraceOptionDef)
    - [4.4.2. IOAM Node Data Fields and Associated Formats](#trace-node-data-element)
      - [4.4.2.1. Hop_Lim and node_id Short](#Hop_Lim)
      - [4.4.2.2. ingress_if_id and egress_if_id Short](#section-4.4.2.2)
      - [4.4.2.3. Timestamp Seconds](#section-4.4.2.3)
      - [4.4.2.4. Timestamp Fraction](#section-4.4.2.4)
      - [4.4.2.5. Transit Delay](#section-4.4.2.5)
      - [4.4.2.6. Namespace-Specific Data](#section-4.4.2.6)
      - [4.4.2.7. Queue Depth](#section-4.4.2.7)
      - [4.4.2.8. Checksum Complement](#section-4.4.2.8)
      - [4.4.2.9. Hop_Lim and node_id Wide](#section-4.4.2.9)
      - [4.4.2.10. ingress_if_id and egress_if_id Wide](#section-4.4.2.10)
      - [4.4.2.11. Namespace-Specific Data Wide](#section-4.4.2.11)
      - [4.4.2.12. Buffer Occupancy](#section-4.4.2.12)
      - [4.4.2.13. Opaque State Snapshot](#section-4.4.2.13)
    - [4.4.3. Examples of IOAM Node Data](#trace-type-node-data)
  - [4.5. IOAM Proof of Transit Option-Type](#IOAM_proof_of_transit_option)
    - [4.5.1. IOAM Proof of Transit Type 0](#section-4.5.1)
  - [4.6. IOAM Edge-to-Edge Option-Type](#IOAM_edge_to_edge_opt)
- [5. Timestamp Formats](#TimestampSec)
  - [5.1. PTP Truncated Timestamp Format](#PTPFromatSec)
  - [5.2. NTP 64-Bit Timestamp Format](#NTPFormatSec)
  - [5.3. POSIX-Based Timestamp Format](#POSIXFormatSec)
- [6. IOAM Data Export](#export)
- [7. IANA Considerations](#IANA)
  - [7.1. IOAM Option-Type Registry](#IOAM-type-registry)
  - [7.2. IOAM Trace-Type Registry](#ioam-trace-type-registry)
  - [7.3. IOAM Trace-Flags Registry](#Flags-Registry-Sec)
  - [7.4. IOAM POT-Type Registry](#section-7.4)
  - [7.5. IOAM POT-Flags Registry](#pot-flags-sec)
  - [7.6. IOAM E2E-Type Registry](#section-7.6)
  - [7.7. IOAM Namespace-ID Registry](#section-7.7)
- [8. Management and Deployment Considerations](#section-8)
...
This document defines data fields for In situ Operations,
      ...
      the classification given in [RFC7799](#RFC7799), IOAM could be portrayed as Hybrid Type
...
The term "in situ OAM" was originally motivated by the use
...
The key words "**MUST**", "**MUST NOT**", "**REQUIRED**", "**SHALL**", "**SHALL NOT**", 
      ...
      [RFC2119](#RFC2119) [RFC8174](#RFC8174) 
...
**NSH:**
  Network Service Header [RFC8300](#RFC8300)
...
**Short format:**
  refers to
            an IOAM-Data-Field that comprises 4 octets
...
**VXLAN-GPE:**
  Virtual eXtensible Local Area Network,
            Generic Protocol Extension [NVO3-VXLAN-GPE](#I-D.ietf-nvo3-vxlan-gpe)

**Wide format:**
  refers to
            an IOAM-Data-Field that comprises 8 octets
...
IOAM assumes a set of constraints as well as 
      ...
      [IPPM-IOAM-DEPLOYMENT](#I-D.ietf-ippm-ioam-deployment) for IOAM
...
**Domain (or scope) of in situ OAM deployment:**
  IOAM is focused on "limited domains", as defined in [RFC8799](#RFC8799).
...
**Traffic sets that IOAM is applied to:**
  IOAM can be deployed on all or
  	...
  	traffic IOAM is applied to could be beneficial in some deployments.

**Encapsulation independence:**
  The definition of IOAM-Data-Fields is
...
**IOAM implementation:**
  The definition of the IOAM-Data-Fields takes the
  	specifics of devices with hardware data planes and software data planes
  	into account.


<a name="IOAM_option_format"></a>
# IOAM Data-Fields, Types, and Nodes
...
This section details IOAM-related nomenclature and describes data
      ...
      the different types of IOAM nodes.

## IOAM Data-Fields and Option-Types
...
To accommodate the different uses of IOAM, IOAM-Data-Fields fall
        ...
        IOAM-Option-Types (see [Section 7.1](#IOAM-type-registry) for
        details). Corresponding to these IOAM-Option-Types, different
        IOAM-Data-Fields are defined.
...
Future IOAM-Option-Types can be allocated by IANA, as described in 
		 [Section 7.1](#IOAM-type-registry).

## IOAM-Domains and Types of IOAM Nodes

[Section 3](#IOAM_scope) already mentioned that IOAM is expected to be deployed in a limited domain [RFC8799](#RFC8799). 
        ...
        can have different roles in different IOAM-Namespaces.
...
An IOAM encapsulating node incorporates one or more
        IOAM-Option-Types (from the list of IOAM-Types, see [Section 7.1](#IOAM-type-registry)) into packets that IOAM is enabled for.
...
The role of an IOAM encapsulating, IOAM transit, or
        ...
        which is added in a future revision.
...
IOAM-Namespaces allow for a namespace-specific definition and
        ...
        interface (e.g., in case of tunnels). Please refer to [Section 4.3](#ioam_namespaces) for details on IOAM-Namespaces.

<a name="ioam_namespaces"></a>
## IOAM-Namespaces
...
IOAM-Namespaces add further context to IOAM-Option-Types and
	...
	The significance of IOAM-Data-Fields is always within a
        ...
        field always needs to be carried along with the IOAM data-fields themselves.
...
An IOAM-Namespace is identified by a 16-bit namespace identifier
		...
		IOAM-Option-Types defined in this document and **MUST** be included 
...
The IANA-assigned range is intended to allow future
		...
		packet. The Default-Namespace-ID **MUST** be supported by all nodes 
...
- whether one or more IOAM-Option-Types need to be processed by a device.
...
- which IOAM-Option-Type needs to be processed/updated in case
            ...
            deployment.
- whether one or more IOAM-Option-Types have to be removed from the packet,
...
- IOAM-Namespaces provide additional context for
          ...
          Namespace-ID will always be unique.
- Similarly, IOAM-Namespaces can be used to define how certain
            ...
            interpreted within the context of an IOAM-Namespace.
...
To optimize hardware and software implementations, IOAM tracing is
		...
		and support one or both of the following options.
...
IOAM encapsulating nodes and IOAM decapsulating nodes that support
        tracing **MUST** support both Trace Option-Types. For IOAM transit nodes, it
        is sufficient to support one of the Trace Option-Types.  
        In the event that both options are utilized in a deployment 
        at the same time, the Incremental Trace-Option **MUST** be placed
...
- Generic data, i.e., format-free information where syntax and semantics
            ...
            fill level at the time the packet was processed, or even a battery-charge level.
- Information to detect whether IOAM trace data was added at
            every hop or whether certain hops in the domain weren't IOAM
            transit nodes.

It should be noted that the semantics of some of the node data 
		  ...
		  to allow IOAM nodes with various different architectures.

<a name="TraceOptionDef"></a>
### Pre-allocated and Incremental Trace Option-Types
...
The IOAM Pre-allocated Trace-Option and the IOAM Incremental
          ...
          decapsulating node) **MUST NOT** modify any of the fields in the fixed-size "trace option header", other than
...
**Namespace-ID:**
  16-bit identifier of an
                ...
                the "Default-Namespace-ID" (see [Section 4.3](#ioam_namespaces)) 
  			  and **MUST** be known to all the nodes
...
**NodeLen:**
  5-bit unsigned integer. This field
  ...
  If IOAM Trace-Type Bit 22 is not
                ...
                Snapshot" field) in 4-octet units.
  ...
  For
                ...
                and 2 of them are wide, then NodeLen would be 5.

  An IOAM encapsulating node **MUST** set NodeLen.

  A node receiving an IOAM Pre-allocated
                or Incremental Trace-Option relies on the NodeLen value.

**Flags:**
  4-bit field. Flags are
                allocated by IANA, as specified in [Section 7.3](#Flags-Registry-Sec). This document allocates a single
                flag as follows:

  **Bit 0:**
    "Overflow" (O-bit) (most significant
                      ...
                      data, the network element **MUST NOT** add any fields and
    		  **MUST** set
...
**RemainingLen:**
  7-bit unsigned integer. This field specifies the data
              ...
              overflowed. The sender **MUST** assign the
              initial value of the RemainingLen field. The sender
              **MAY** calculate the value of the RemainingLen
              ...
              **MUST** decrease RemainingLen by the amount of
  	    ...
  	    Specifically, the recording
              ...
              then the node **MUST NOT** add any fields.
...
**IOAM Trace-Type:**
  ...
  The IOAM Trace-Type value is a bit field. The
                following bits are defined in this document, with details on
                each bit described in [Section 4.4.2](#trace-node-data-element). The order of packing the
                data fields in each node data element follows the bit order of
                the IOAM Trace-Type field as follows:

  **Bit 0**
    Most significant bit. When set, 
  ...
  **Bit 2**
    When set, indicates the presence of
                      timestamp seconds in the node data.

  **Bit 3**
    When set, indicates the presence of
                      timestamp fraction in the node data.

  **Bit 4**
    When set,  indicates the presence of transit
                      delay in the node data.
  ...
  **Bit 6**
    When set, indicates the presence of queue
                      depth in the node data.

  **Bit 7**
    When set, indicates the presence of the
                      Checksum Complement node data.

  **Bit 8**
    When set, indicates the presence of Hop_Lim
                      and node_id in wide format in the node data.
  ...
  **Bit 11**
    When set, indicates the presence of buffer
                      occupancy in the node data.

  **Bits 12-21**
    Undefined. These values are available 
    		  for future assignment in the IOAM Trace-Type Registry 
    		  ([Section 7.2](#ioam-trace-type-registry)). Every
    		  future node data field corresponding to one of these bits
    		  **MUST** be 4 octets long. An IOAM encapsulating node
    		  **MUST** set the 
  ...
  **Bit 22**
    When set, indicates the presence of the
                      variable-length Opaque State Snapshot field.

  **Bit 23**
    Reserved; **MUST** be set to zero upon
                      transmission and be ignored upon receipt. This bit is
                      reserved to allow for future extensions of the 
                      IOAM Trace-Type bit field.


  [Section 4.4.2](#trace-node-data-element)
                describes the IOAM-Data-Types and their formats. Within an
                IOAM-Domain, possible combinations of these bits making the
                IOAM Trace-Type can be restricted by configuration knobs.

**Reserved:**
  8 bits. An IOAM encapsulating node **MUST**
              set the value to zero upon transmission. IOAM transit nodes
  	    **MUST** ignore the received value.

**Node data List [n]:**
  Variable-length field. This is
...
<a name="trace-node-data-element"></a>
### IOAM Node Data Fields and Associated Formats

All the IOAM-Data-Fields **MUST** be aligned by 4 octets. If a
...
Some IOAM-Data-Fields defined below, such as interface
...
The "Hop_Lim and node_id short" field is a 4-octet field
...
**Hop_Lim:**
  1-octet unsigned integer. It is set to
                  ...
                  0xff when the lower level does not have a field equivalent to TTL / Hop Limit.
...
**node_id:**
  3-octet unsigned integer. A node
                  ...
                  this document. See
                  [IPPM-IOAM-DEPLOYMENT](#I-D.ietf-ippm-ioam-deployment) for 
                  a discussion of deployment-related aspects of the node_id.


#### ingress_if_id and egress_if_id Short

The "ingress_if_id and egress_if_id" field is a 4-octet field
            that is defined as follows:
...
Note that due to the fact that IOAM uses its own
            ...
            a virtual or logical interface, or even a queue.

#### Timestamp Seconds

The "timestamp seconds" field is a 4-octet unsigned integer
            ...
            possible formats, based on either the Precision Time Protocol (PTP) (see e.g., [RFC8877](#RFC8877)),
            NTP [RFC5905](#RFC5905), or POSIX [POSIX](#POSIX). The
            three timestamp formats are specified in [Section 5](#TimestampSec). In all three cases, the timestamp seconds
            field contains the 32 most significant bits of the timestamp
            format that is specified in [Section 5](#TimestampSec). If a
...
The "timestamp fraction" field is a 4-octet unsigned integer
            field. Fraction specifies the fractional portion of the number of
            seconds since the NTP epoch [RFC8877](#RFC8877). The field specifies the time at
            which the packet was received by the node. This field has three
            possible formats, based on either PTP (see e.g., [RFC8877](#RFC8877)),
            NTP [RFC5905](#RFC5905), or POSIX [POSIX](#POSIX). The
            three timestamp formats are specified in [Section 5](#TimestampSec). In all three cases, the timestamp
            fraction field contains the 32 least significant bits of the
            timestamp format that is specified in [Section 5](#TimestampSec). If a node is not capable of populating
...
The "transit delay" field is a 4-octet unsigned integer in the
            range 0 to 231-1. It is the time in nanoseconds the packet spent
            in the transit node. This can serve as an indication of the
            queuing delay at the node. If the transit delay exceeds 231-1
...
The "namespace-specific data" field is a 4-octet field that
...
The "queue depth" field is a 4-octet unsigned integer field.
...
The "Checksum Complement" field is a 4-octet node data that
	    ...
	    encapsulation protocols. When the Checksum Complement is
            ...
            node data **MUST** carry out one of the following two alternatives
	    in order to maintain the correctness of the UDP Checksum value:

1. recompute the UDP Checksum field or
2. use the Checksum Complement to make a checksum-neutral
...
IOAM decapsulating nodes **MUST** recompute the UDP Checksum
            field, since they do not know whether previous hops modified the
            UDP Checksum field or the Checksum Complement field.

Checksum Complement fields are used in a similar manner in
	    [RFC7820](#RFC7820) and [RFC7821](#RFC7821).
...
The "Hop_Lim and node_id wide" field is an 8-octet field
            defined as follows:
...
**Hop_Lim:**
  1-octet unsigned integer. See [Section 4.4.2.1](#Hop_Lim)
  	      for the definition of the field.

**node_id:**
  7-octet unsigned integer. It is a node
                  identifier field to uniquely identify a node within the
                  IOAM-Namespace and associated IOAM-Domain. The procedure to
                  allocate, manage, and map the node_ids is beyond the scope of
                  this document.
...
The "ingress_if_id and egress_if_id wide" field is an 8-octet
...
The "namespace-specific data wide" field is an 8-octet field
...
The "buffer occupancy" field is a 4-octet unsigned integer
            ...
            implementations to use standard units, such as bytes.
...
**Length:**
  1-octet unsigned integer. It is the
                  length in multiples of 4 octets of the Opaque data field that
                  follows Schema ID.

**Schema ID:**
  3-octet unsigned integer identifying
                  the schema of Opaque data.

**Opaque data:**
  Variable-length field. This field
                  is interpreted as specified by the schema identified by the
                  Schema ID.


When this field is part of the data field, but a node
            populating the field has no opaque state data to report, the
            Length **MUST** be set to 0 and the Schema ID **MUST**
	    be set to 0xFFFFFF to mean no schema.

<a name="trace-type-node-data"></a>
### Examples of IOAM Node Data

The format used for the entries in a packet's "node data list" 
...
**0xD40000:**
  If the IOAM Trace-Type is 0xD40000 (0b110101000000000000000000), then
  	    the format of node data is:

**0xC00000:**
  If the IOAM Trace-Type is 0xC00000 (0b110000000000000000000000), then
  	    the format is:

**0x900000:**
  If the IOAM Trace-Type is 0x900000 (0b100100000000000000000000), then
  	    the format is:

**0x840000:**
  If the IOAM Trace-Type is 0x840000 (0b100001000000000000000000), then
  	    the format is:

**0x940000:**
  If the IOAM Trace-Type is 0x940000 (0b100101000000000000000000), then
  	    the format is:

**0x308002:**
  If the IOAM Trace-Type is 0x308002 (0b001100001000000000000010), then
  	    the format is:


<a name="IOAM_proof_of_transit_option"></a>
## IOAM Proof of Transit Option-Type

The IOAM Proof of Transit Option-Type is used to support
        path or service function chain [RFC7665](#RFC7665) verification use cases, i.e., prove that
...
**Cumulative:**
  information that is handed from node to node and
...
**Namespace-ID:**
  16-bit identifier of an
              ...
              "Default-Namespace-ID" (see [Section 4.3](#ioam_namespaces)) 
  			and **MUST** be known to all the nodes implementing
              IOAM. For any other Namespace-ID value that does not match any
              Namespace-ID the node is configured to operate on, the node **MUST NOT** change the contents of the IOAM-Data-Fields.

**IOAM POT-Type:**
  8-bit identifier of a particular POT
              variant that specifies the POT data that is included. This
              document defines IOAM POT-Type 0:
  ...
  **0:**
  ...
  If a node receives an IOAM POT-Type value that it does not
              understand, the node **MUST NOT** change, add to, or remove 
              the contents of the IOAM-Data-Fields.

**IOAM POT flags:**
  8 bits. This document does not define any flags. Bits 0-7 are 
            available for assignment (see [Section 7.5](#pot-flags-sec)). 
            Bits that have not been assigned **MUST** be set to zero upon 
            transmission and be ignored upon receipt.

**POT Option data:**
  Variable-length field. The type of
              which is determined by the IOAM POT-Type.


### IOAM Proof of Transit Type 0
...
**Namespace-ID:**
  16-bit identifier of an
                IOAM-Namespace (see [Section 4.3](#ioam_namespaces) 
                above).

**IOAM POT-Type:**
  8-bit identifier of a particular
                POT variant that specifies the POT data that is included 
                (see [Section 4.5](#IOAM_proof_of_transit_option) 
                above). For this case here, IOAM POT-Type is set to the value 0.

**Bit 0-7:**
  Undefined (see [Section 4.5](#IOAM_proof_of_transit_option) 
                above).
...
**Cumulative:**
  64-bit Cumulative that is updated at
                specific nodes by processing per packet PktID field and
                configured parameters.


<a name="IOAM_edge_to_edge_opt"></a>
## IOAM Edge-to-Edge Option-Type

The IOAM Edge-to-Edge Option-Type carries data that is added by
...
The IOAM Edge-to-Edge Option-Type IOAM-Data-Fields **MUST**
...
**IOAM-E2E-Type:**
  16-bit identifier that specifies
              ...
              IOAM E2E-Type field as follows:
  ...
  **Bit 1**
    When set, it indicates the presence of a 32-bit
  ...
  **Bit 2**
    When set, it indicates the presence of timestamp
                    ...
                    PTP (see e.g., [RFC8877](#RFC8877)), NTP [RFC5905](#RFC5905),
                    or POSIX [POSIX](#POSIX). The three timestamp
    		formats
                    are specified in [Section 5](#TimestampSec). In all
    		three
                    cases, the timestamp seconds field contains the 32 most
                    significant bits of the timestamp format that is specified in
                    [Section 5](#TimestampSec). If a node is not capable of
  ...
  **Bit 3**
    When set, it indicates the presence of timestamp
                    ...
                    formats, based on either PTP (see e.g., [RFC8877](#RFC8877)), NTP
                    [RFC5905](#RFC5905), or POSIX [POSIX](#POSIX). The
                    three timestamp formats are specified in [Section 5](#TimestampSec). In all three cases, the timestamp
                    fraction field contains the 32 least significant bits of the
                    timestamp format that is specified in [Section 5](#TimestampSec). If a node is not capable of
                    populating this field, it assigns the value 0xFFFFFFFF. Note
  ...
  **Bit 4-15**
    Undefined. An IOAM encapsulating node
                  **MUST** set the value of these bits to zero upon transmission
    	      and ignore them upon receipt.


**E2E Option data:**
  Variable-length field. The type of
              which is determined by the IOAM E2E-Type.


<a name="TimestampSec"></a>
# Timestamp Formats

The IOAM-Data-Fields include a timestamp field that is represented
...
<a name="PTPFromatSec"></a>
## PTP Truncated Timestamp Format

The Precision Time Protocol (PTP) uses
        ...
        timestamp. The PTP truncated format is specified in [RFC8877](#RFC8877), and the details are
...
**Timestamp field format:**
  ...
  **Nanoseconds:**
    ...
    **Units:**
      nanoseconds. The value of this field is in the range 0
                    to (109)-1.



**Epoch:**
  PTP epoch. For details, see e.g., [RFC8877](#RFC8877).
...
**Timestamp field format:**
  ...
  **Fraction:**
    ...
    **Units:**
      the unit is 2(-32) seconds, which is roughly equal to
                    233 picoseconds.



**Epoch:**
  NTP epoch. For details, see [RFC5905](#RFC5905).
...
**Synchronization Aspects:**
  Nodes that use this timestamp format will typically be
          synchronized to UTC using NTP [RFC5905](#RFC5905). Thus, the
...
<a name="POSIXFormatSec"></a>
## POSIX-Based Timestamp Format

This timestamp format is based on the POSIX time format [POSIX](#POSIX). The detailed specification of the timestamp format
        used in this document is presented below.
...
**Timestamp field format:**
  ...
  **Microseconds:**
    ...
    **Units:**
      the unit is microseconds. The value of this field is
      		in the range 0 to (106)-1.



**Epoch:**
  POSIX epoch. For details, see [POSIX](#POSIX),
  	    Appendix A.4.16.
...
**Synchronization Aspects:**
  It is assumed that nodes that use this timestamp format run the
              ...
              [RFC5905](#RFC5905). Thus, the timestamp
  	    **MAY** be derived from
              the NTP-synchronized clock, allowing the timestamp to be measured
              with respect to the clock of an NTP server.
...
A way to perform raw data export of IOAM data 
      using IPFIX is discussed in [IPPM-IOAM-RAWEXPORT](#I-D.spiegel-ippm-ioam-rawexport).

<a name="IANA"></a>
# IANA Considerations

IANA has defined a registry group named
	  "In Situ OAM (IOAM)".
...
This registry defines 128 code points for the IOAM
        ...
        explained in [Section 4](#IOAM_option_format). The following code points are defined in
        this document:
...
Code points 4-127 are available for assignment via the "IETF Review" process,
        as per [RFC8126](#RFC8126).
...
The evaluation of a new registration request **MUST** also include
        ...
        field in the newly defined header of the new Option-Type.

<a name="ioam-trace-type-registry"></a>
## IOAM Trace-Type Registry

This registry defines code points for each bit in the 24-bit
        IOAM Trace-Type field for the Pre-allocated Trace Option-Type and Incremental
        Trace Option-Type defined in [Section 4.4](#IOAM_tracing_option). Bits 0-11 are defined in this document in
        [Paragraph 5](#IOAMTraceType) of [Section 4.4.1](#TraceOptionDef):
...
**Bit 5:**
  namespace-specific data in short format
...
**Bit 10:**
  namespace-specific data in wide format
...
Bits 12-21 are available for assignment
        via the "IETF Review" process, as per [RFC8126](#RFC8126).

New registration requests **MUST** use the following template:

**Bit:**
  desired bit to be allocated in the 24-bit
               IOAM Trace Option-Type field for the Pre-allocated Trace Option-Type
                and Incremental Trace Option-Type
...
<a name="Flags-Registry-Sec"></a>
## IOAM Trace-Flags Registry

This registry defines code points for each bit in the 4-bit flags
        for the Pre-allocated Trace-Option and Incremental Trace-Option defined in
	[Section 4.4](#IOAM_tracing_option). The
	meaning of
        Bit 0 (the most significant bit) for trace flags is defined in this
        document in [Paragraph 3](#TraceFlags) of [Section 4.4.1](#TraceOptionDef):

**Bit 0:**
  "Overflow" (O-bit)


Bits 1-3 are available for assignment via the "IETF Review"
        process, as per [RFC8126](#RFC8126).

New registration requests **MUST** use the following template:

**Bit:**
  desired bit to be allocated 
...
## IOAM POT-Type Registry

This registry defines 256 code points to define the IOAM POT-Type for the
        IOAM Proof of Transit Option ([Section 4.5](#IOAM_proof_of_transit_option)). The code point value 0 is
...
**Reference:**
  reference to the document that defines the new POT-Type


<a name="pot-flags-sec"></a>
## IOAM POT-Flags Registry

This registry defines code points for each bit in the 8-bit flags
        for the IOAM POT Option-Type defined in [Section 4.5](#IOAM_proof_of_transit_option).

Bits 0-7 are available for assignment via the
        "IETF Review" process, as per [RFC8126](#RFC8126).

New registration requests **MUST** use the following template:

**Bit:**
  desired bit to be allocated 
            in the 8-bit flags field of the IOAM POT Option-Type
...
This registry defines code points for each bit in the 16-bit
        IOAM E2E-Type field for the IOAM E2E Option ([Section 4.6](#IOAM_edge_to_edge_opt)). Bits 0-3 are defined
        in this document:
...
Bits 4-15 are available for assignment via the
        "IETF Review" process, as per [RFC8126](#RFC8126).
...
**Bit:**
  desired bit to be allocated 
            in the 16-bit IOAM E2E-Type field
...
IANA has set up the "IOAM Namespace-ID" registry that
        ...
        [Section 4.3](#ioam_namespaces) of this
        document. Registry entries for the values 0x8000 to 0xFFFF are
        to be assigned via the "Expert Review" policy, as per [RFC8126](#RFC8126).

Upon receiving a new allocation request, a designated 
	expert will perform the following:

- Review whether the request is complete, i.e., the 
          registration template has been filled in. The expert
          will send incomplete requests back to the requester.
...
It is intended that any allocation will be accompanied 
...
**Reference:**
  reference to the document that
            defines the new Namespace-ID
...
**Status of the registration:**
  Status can be either 
            ...
            the status is changed to "permanent".
...
This document defines the structure and use of IOAM-Data-Fields. 
      ...
      refer to [IPPM-IOAM-DEPLOYMENT](#I-D.ietf-ippm-ioam-deployment), which
      outlines a framework for IOAM deployment and provides best current practices.

<a name="Security"></a>
# Security Considerations

As discussed in [RFC7276](#RFC7276), a successful attack on
...
The Proof of Transit Option-Type (see [Section 4.5](#IOAM_proof_of_transit_option)) is used for verifying the path
...
From a confidentiality perspective, although IOAM options are not expected to
      ...
      if buffers and queues are overflowing.
...
Since IOAM options can include timestamps, if network devices use
      synchronization protocols, then any attack on the time protocol [RFC7384](#RFC7384) can compromise the integrity of the
      timestamp-related data fields.

At the management plane, attacks can be set up by misconfiguring
...
IETF protocols require features to ensure their security. While IOAM-Data-Fields
      ...
      IOAM-Data-Fields carried in an encapsulating protocol **MUST** provide
      ...
      the encapsulating protocol, or it could incorporate the mechanisms specified in [IPPM-IOAM-DATA-INTEGRITY](#I-D.ietf-ippm-ioam-data-integrity).
...
Notably, IOAM is expected to be deployed in limited domains, 
...
This document does not define the data contents of custom fields, 
      ...
      that need to be described where those formats are defined.
...
The security considerations of a system that deploys IOAM, much like
      ...
      in order to avoid external threats.
...
IOAM deployment considerations, including approaches to mitigate the above
      ...
      [IPPM-IOAM-DEPLOYMENT](#I-D.ietf-ippm-ioam-deployment).
...
<a name="POSIX"></a> **[POSIX]** "IEEE/Open Group 1003.1-2017 - IEEE Standard for Information Technology--Portable Operating System Interface (POSIX(TM)) Base Specifications, Issue 7", IEEE Std 1003.1-2017, January 2018. <https://standards.ieee.org/ieee/1003.1/7101/>
...
<a name="I-D.ietf-ippm-ioam-data-integrity"></a> **[IPPM-IOAM-DATA-INTEGRITY]** Frank Brockners, Shwetha Bhandari, Tal Mizrahi, Justin Iurman, "Integrity of In-situ OAM Data Fields", Internet-Draft draft-ietf-ippm-ioam-data-integrity-01, March 2022. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-ippm-ioam-data-integrity-01>

<a name="I-D.ietf-ippm-ioam-deployment"></a> **[IPPM-IOAM-DEPLOYMENT]** Frank Brockners, Shwetha Bhandari, Daniel Bernier, Tal Mizrahi, "In-situ OAM Deployment", Internet-Draft draft-ietf-ippm-ioam-deployment-01, April 2022. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-ippm-ioam-deployment-01>

<a name="I-D.spiegel-ippm-ioam-rawexport"></a> **[IPPM-IOAM-RAWEXPORT]** Mickey Spiegel, Frank Brockners, Shwetha Bhandari, Ramesh Sivakolundu, "In-situ OAM raw data export with IPFIX", Internet-Draft draft-spiegel-ippm-ioam-rawexport-06, February 2022. Work in Progress <https://datatracker.ietf.org/doc/html/draft-spiegel-ippm-ioam-rawexport-06>

<a name="I-D.kitamura-ipv6-record-route"></a> **[IPV6-RECORD-ROUTE]** H. Kitamura, "Record Route for IPv6 (RR6) Hop-by-Hop Option Extension", Internet-Draft draft-kitamura-ipv6-record-route-00, November 2000. Work in Progress <https://datatracker.ietf.org/doc/html/draft-kitamura-ipv6-record-route-00>

<a name="I-D.ietf-nvo3-vxlan-gpe"></a> **[NVO3-VXLAN-GPE]** Fabio Maino, Larry Kreeger, Uri Elzur, "Generic Protocol Extension for VXLAN (VXLAN-GPE)", Internet-Draft draft-ietf-nvo3-vxlan-gpe-12, September 2021. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-nvo3-vxlan-gpe-12>

<a name="RFC7276"></a> **[RFC7276]** T. Mizrahi, N. Sprecher, E. Bellagamba, Y. Weingarten, "An Overview of Operations, Administration, and Maintenance (OAM) Tools", RFC 7276, DOI 10.17487/RFC7276, June 2014. <https://www.rfc-editor.org/info/rfc7276> [Local MD](rfc7276.md)

<a name="RFC7384"></a> **[RFC7384]** T. Mizrahi, "Security Requirements of Time Protocols in Packet Switched Networks", RFC 7384, DOI 10.17487/RFC7384, October 2014. <https://www.rfc-editor.org/info/rfc7384> [Local MD](rfc7384.md)

<a name="RFC7665"></a> **[RFC7665]** J. Halpern, C. Pignataro, "Service Function Chaining (SFC) Architecture", RFC 7665, DOI 10.17487/RFC7665, October 2015. <https://www.rfc-editor.org/info/rfc7665> [Local MD](rfc7665.md)

<a name="RFC7799"></a> **[RFC7799]** A. Morton, "Active and Passive Metrics and Methods (with Hybrid Types In-Between)", RFC 7799, DOI 10.17487/RFC7799, May 2016. <https://www.rfc-editor.org/info/rfc7799> [Local MD](rfc7799.md)

<a name="RFC7820"></a> **[RFC7820]** T. Mizrahi, "UDP Checksum Complement in the One-Way Active Measurement Protocol (OWAMP) and Two-Way Active Measurement Protocol (TWAMP)", RFC 7820, DOI 10.17487/RFC7820, March 2016. <https://www.rfc-editor.org/info/rfc7820> [Local MD](rfc7820.md)

<a name="RFC7821"></a> **[RFC7821]** T. Mizrahi, "UDP Checksum Complement in the Network Time Protocol (NTP)", RFC 7821, DOI 10.17487/RFC7821, March 2016. <https://www.rfc-editor.org/info/rfc7821> [Local MD](rfc7821.md)

<a name="RFC8300"></a> **[RFC8300]** P. Quinn, U. Elzur, C. Pignataro, "Network Service Header (NSH)", RFC 8300, DOI 10.17487/RFC8300, January 2018. <https://www.rfc-editor.org/info/rfc8300> [Local MD](rfc8300.md)

<a name="RFC8799"></a> **[RFC8799]** B. Carpenter, B. Liu, "Limited Domains and Internet Protocols", RFC 8799, DOI 10.17487/RFC8799, July 2020. <https://www.rfc-editor.org/info/rfc8799> [Local MD](rfc8799.md)

<a name="RFC8877"></a> **[RFC8877]** T. Mizrahi, J. Fabini, A. Morton, "Guidelines for Defining Packet Timestamps", RFC 8877, DOI 10.17487/RFC8877, September 2020. <https://www.rfc-editor.org/info/rfc8877> [Local MD](rfc8877.md)

<a name="RFC8926"></a> **[RFC8926]** J. Gross, I. Ganga, T. Sridhar, "Geneve: Generic Network Virtualization Encapsulation", RFC 8926, DOI 10.17487/RFC8926, November 2020. <https://www.rfc-editor.org/info/rfc8926> [Local MD](rfc8926.md)

# Acknowledgements

The authors would like to thank Éric Vyncke, Nalini Elkins, Srihari       Raghavan, Ranganathan T S, Karthik Babu       Harichandra Babu, Akshaya       Nadahalli, LJ Wobker, Erik Nordmark,
      Vengada Prasad Govindan, Andrew       Yourtchenko, Aviv Kfir, Tianran Zhou,
      Zhenbin (Robin), and Greg Mirsky for the
      comments and advice.

This document leverages and builds on top of several concepts
      described in [IPV6-RECORD-ROUTE](#I-D.kitamura-ipv6-record-route). The
      authors would like to acknowledge the work done by the author Hiroshi Kitamura and people involved in writing it.
...
The authors would like to gracefully acknowledge useful review and
      ...
      Carlos J. Bernardos, Haoyu Song,
      Mickey Spiegel, Roman Danyliw, 
      Benjamin Kaduk, Murray S. Kucherawy,
      Ian Swett, Martin Duke, 
      Francesca Palombini, Lars Eggert,
      Alvaro Retana, Erik Kline,
      Robert Wilton, Zaheduzzaman Sarker,
      Dan Romascanu, and Barak Gafni.
...
- Address: 7200-11 Kit Creek Road, NC, 27709, United States of America
- Email: cpignata@cisco.com

**Mickey Spiegel**

- Organization: Barefoot Networks, an Intel company
- Address: 101 Innovation Drive, San Jose, CA, 95134-1941, United States of America
- Email: mickey.spiegel@intel.com

**Barak Gafni**

- Organization: Nvidia
- Address: 350 Oakmead Parkway, Sunnyvale, CA, 94085, United States of America
- Email: gbarak@nvidia.com

**Jennifer Lemon**

- Organization: Broadcom
- Address: 270 Innovation Drive, San Jose, CA, 95134, United States of America
...
- Address: 25 Bank Street, London, E14 5JP, United Kingdom
...
- Organization: Cisco Systems, Inc.
- Address: Hansaallee 249, Duesseldorf, 40549, Germany
- Email: fbrockne@cisco.com

**Shwetha Bhandari**

- Organization: Thoughtspot
- Address: 24th Main Rd, Bangalore, Karnataka, 560 102, India
- Email: shwetha.bhandari@thoughtspot.com

**Tal Mizrahi**

- Organization: Huawei
- Address: 8-2 Matam, Haifa, 3190501, Israel
- Email: tal.mizrahi.phd@gmail.com
