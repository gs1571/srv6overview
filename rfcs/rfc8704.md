...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-opsec-urpf-improvements-04
...
**Workgroup:** OPSEC Working Group

**Keywords:** BGP, source address spoofing, source address validation (SAV), Reverse Path Forwarding (RPF), unicast RPF (uRPF), DDoS mitigation, BCP 38, BCP 84

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-opsec-urpf-improvements-04)
- [alternate](https://dx.doi.org/10.17487/rfc8704)
...
- [2. Review of Existing Source Address Validation Techniques](#review)
  - [2.1. SAV Using Access Control List](#acl)
  - [2.2. SAV Using Strict Unicast Reverse Path Forwarding](#surpf)
  - [2.3. SAV Using Feasible-Path Unicast Reverse Path Forwarding](#fp-urpf)
  - [2.4. SAV Using Loose Unicast Reverse Path Forwarding](#lurpf)
  - [2.5. SAV Using VRF Table](#vrf)
- [3. SAV Using Enhanced Feasible-Path uRPF](#newtech)
  - [3.1. Description of the Method](#descrip)
    - [3.1.1. Algorithm A: Enhanced Feasible-Path uRPF](#algA)
  - [3.2. Operational Recommendations](#recomm)
  - [3.3. A Challenging Scenario](#challenge)
  - [3.4. Algorithm B: Enhanced Feasible-Path uRPF with Additional Flexibility across Customer Cone](#algB)
  - [3.5. Augmenting RPF Lists with ROA and IRR Data](#augment)
  - [3.6. Implementation and Operations Considerations](#impl)
    - [3.6.1. Impact on FIB Memory Size Requirement](#rpfsize)
    - [3.6.2. Coping with BGP's Transient Behavior](#hyst)
  - [3.7. Summary of Recommendations](#summ_recomm)
    - [3.7.1. Applicability of the EFP-uRPF Method with Algorithm A](#discuss)
- [4. Security Considerations](#seccon)
...
Source address validation (SAV) refers to the detection and mitigation of source address (SA) spoofing [RFC2827](#RFC2827). This document identifies a need for and proposes improvement of the unicast Reverse Path Forwarding (uRPF) techniques [RFC3704](#RFC3704) for SAV. Strict uRPF is inflexible about directionality (see [RFC3704](#RFC3704) for definitions), the loose uRPF is oblivious to directionality, and the current feasible-path uRPF attempts to strike a balance between the two [RFC3704](#RFC3704). However, as shown in this document, the existing feasible-path uRPF still has shortcomings. Even with the feasible-path uRPF, ISPs are often apprehensive that they may be dropping customers' data packets with legitimate source addresses.
...
origin AS were received on different interfaces (at border routers), then incoming data packets with source addresses in any of those prefixes should be accepted on any of those interfaces (presented in [Section 3](#newtech)). For some challenging ISP-customer scenarios (see [Section 3.3](#challenge)), this document also describes a more relaxed version of the enhanced feasible-path uRPF technique (presented in [Section 3.4](#algB)). Implementation and operations considerations are discussed in [Section 3.6](#impl).

Throughout this document, the routes under consideration are assumed to have been vetted based on prefix filtering [RFC7454](#RFC7454) and possibly origin validation [RFC6811](#RFC6811).
...
AS A's customer cone is A plus all the ASes that can be reached from A following only P2C links [Luckie](#Luckie).
...
attacks with spoofed addresses [RFC2827](#RFC2827) [RFC3704](#RFC3704).  SAV is performed in network edge devices, such as
border routers, Cable Modem Termination Systems (CMTS) [RFC4036](#RFC4036), and Packet Data Network Gateways (PDN-GWs) in mobile
networks [Firmin](#Firmin).  Ingress Access Control List (ACL) and uRPF are techniques employed for implementing SAV [RFC2827](#RFC2827) [RFC3704](#RFC3704) [ISOC](#ISOC).
...
routing of data packets. Asymmetric routing occurs (see [Figure 1](#fig1)) when a customer AS announces one prefix (P1) to one
...
As an
example, in the multihoming scenario (see Scenario 2 in [Figure 2](#fig2)), if the customer AS announces routes for both prefixes
...
illustrated in [Figure 2](#fig2)), it is possible that
...
in the FIB for the source address. Loose uRPF sacrifices directionality.  It only drops packets if the source address is unreachable in the current FIB (e.g., IANA special-purpose prefixes [SPAR-v4](#SPAR-v4)[SPAR-v6](#SPAR-v6), unallocated, allocated but currently not routed).

<a name="vrf"></a>
## SAV Using VRF Table

The Virtual Routing and Forwarding (VRF) technology [RFC4364](#RFC4364) [Juniper](#Juniper) allows a router
...
avoided) on lateral peer interfaces. That will help to avoid compromising directionality for lateral peer interfaces (which is inevitable with loose uRPF; see [Section 2.4](#lurpf)).

Looking back at Scenarios 1 and 2 (Figures [1](#fig1) and [2](#fig2)), the EFP-uRPF method works better than the other uRPF
methods. Scenario 3 ([Figure 3](#fig3)) further
...
The underlying algorithm in the solution method described above ([Section 3.1](#descrip)) can be specified as follows (to be implemented in a transit AS):
...
customer interfaces, the type of limitation identified in [Figure 4](#fig4) ([Section 3.3](#challenge)) is overcome
...
So, applying the EFP-uRPF method with Algorithm B is recommended on customer interfaces for the challenging scenarios, such as those described in [Section 3.3](#challenge).
...
A or B) can be augmented with data from Route Origin Authorization (ROA) [RFC6482](#RFC6482), as well as Internet Routing Registry
(IRR) data. Special care should be exercised when using IRR data because it is
not always accurate or trusted.  In the EFP-uRPF method with Algorithm A (see [Section 3.1.1](#algA)), if a ROA includes prefix Pi and ASj, then augment
...
belongs in Set A (see Step #3 [Section 3.4](#algB)) and if a ROA includes prefix Pi and ASj,
...
contents. In the algorithms (Sections [3.1.1](#algA) and [3.4](#algB)) described here, the RPF lists are constructed by applying a set of rules to all received BGP routes (not just those selected as best path and installed in the FIB). The concept of uRPF querying an RPF list (instead of the FIB) is similar to uRPF querying a VRF table (see [Section 2.5](#vrf)).

The techniques described in this document require that there should be additional memory (i.e., ternary content-addressable memory (TCAM)) available to store the RPF lists in line cards. For an ISP's AS, the RPF list size for each line card will roughly equal the total number of originated prefixes from ASes in its customer cone (assuming Algorithm B in [Section 3.4](#algB) is used). (Note: EFP-uRPF with Algorithm A (see [Section 3.1.1](#algA)) requires much less memory than EFP-uRPF with Algorithm B.)

The following table shows the measured customer cone sizes in number of prefixes originated (from all ASes in the customer cone) for various types of ISPs [Sriram-RIPE63](#Sriram-RIPE63):

**Table: Customer Cone Sizes (# Prefixes) for Various Types of ISPs**

| Type of ISP | Measured Customer Cone Size in # Prefixes (in turn this is an
...
3. For all other scenarios:
            The EFP-uRPF method with Algorithm B (see [Section 3.4](#algB)) **SHOULD** be applied on customer interfaces.
              The loose uRPF method **SHOULD** be applied on lateral peer and transit provider interfaces.
  - The EFP-uRPF method with Algorithm B (see [Section 3.4](#algB)) **SHOULD** be applied on customer interfaces.
  - The loose uRPF method **SHOULD** be applied on lateral peer and transit provider interfaces.

It is also recommended that prefixes from registered ROAs and IRR route objects that include ASes in an ISP's customer cone **SHOULD** be used to augment the pertaining RPF lists (see [Section 3.5](#augment) for details).

<a name="discuss"></a>
### Applicability of the EFP-uRPF Method with Algorithm A

The EFP-uRPF method with Algorithm A is not mentioned in the above set of recommendations. It is an alternative to EFP-uRPF with Algorithm B and can be used in limited circumstances. The EFP-uRPF method with Algorithm A is expected to work fine if an ISP deploying it has only multihomed stub customers. It is trivially equivalent to strict uRPF if an ISP deploys it for a single-homed stub customer. More generally, it is also expected to work fine when there is absence of limitations, such as those described in [Section 3.3](#challenge). However, caution is required for use of EFP-uRPF with Algorithm A because even if the limitations are not expected at the time of deployment, the vulnerability to change in conditions exists. It may be difficult for an ISP to know or track the extent of use of NO_EXPORT (see [Section 3.3](#challenge)) on routes within its customer cone. If an ISP decides to use EFP-uRPF with Algorithm A, it should make its direct customers aware of the operational recommendations in [Section 3.2](#recomm). This means that the ISP notifies direct customers that at least one prefix originated by each AS in the direct customer's customer cone must propagate to the ISP.
...
The security considerations in BCP 38 [RFC2827](#RFC2827) and RFC 3704 [RFC3704](#RFC3704) apply
for this document as well. In addition, if considering using the EFP-uRPF method with Algorithm A, an ISP or AS operator should be aware of the applicability considerations and potential vulnerabilities discussed in [Section 3.7.1](#discuss).

In augmenting RPF lists with ROA (and possibly reliable IRR) information (see
[Section 3.5](#augment)), a trade-off is made in favor of
...
<a name="CAIDA"></a> **[CAIDA]** "Information for AS 174 (COGENT-174)", October 2019. <https://spoofer.caida.org/as.php?asn=174>

<a name="Cisco1"></a> **[Cisco1]** "Internet Routing Table Growth Causes %ROUTING-FIB-4-RSRC_LOW Message on Trident-Based Line Cards", January 2014. <https://www.cisco.com/c/en/us/support/docs/routers/asr-9000-series-aggregation-services-routers/116999-problem-line-card-00.html>

<a name="Cisco2"></a> **[Cisco2]** "Cisco Nexus 7000 Series NX-OS Unicast Routing Configuration Guide, Release 5.x (Chapter 15: 'Managing the Unicast RIB and FIB')", March 2018. <https://www.cisco.com/c/en/us/td/docs/switches/datacenter/sw/5_x/nx-os/unicast/configuration/guide/l3_cli_nxos/l3_NewChange.html>

<a name="Firmin"></a> **[Firmin]** F Firmin, "The Evolved Packet Core", <https://www.3gpp.org/technologies/keywords-acronyms/100-the-evolved-packet-core>

<a name="ISOC"></a> **[ISOC]** "Addressing the challenge of IP spoofing", September 2015. <https://www.internetsociety.org/resources/doc/2015/addressing-the-challenge-of-ip-spoofing/>

<a name="Juniper"></a> **[Juniper]** "Creating Unique VPN Routes Using VRF Tables", May 2019. <https://www.juniper.net/documentation/en_US/junos/topics/topic-map/l3-vpns-routes-vrf-tables.html#id-understanding-virtual-routing-and-forwarding-tables>

<a name="Luckie"></a> **[Luckie]** M. Luckie, B. Huffaker, A. Dhamdhere, V. Giotsas, kc claffy, "AS Relationships, customer cones, and validation", October 2013. In Proceedings of the 2013 Internet Measurement Conference <https://dl.acm.org/doi/10.1145/2504730.2504735>

<a name="RFC4036"></a> **[RFC4036]** W. Sawyer, "Management Information Base for Data Over Cable Service Interface Specification (DOCSIS) Cable Modem Termination Systems for Subscriber Management", RFC 4036, DOI 10.17487/RFC4036, April 2005. <https://www.rfc-editor.org/info/rfc4036> [Local MD](rfc4036.md)
...
<a name="RFC6482"></a> **[RFC6482]** M. Lepinski, S. Kent, D. Kong, "A Profile for Route Origin Authorizations (ROAs)", RFC 6482, DOI 10.17487/RFC6482, February 2012. <https://www.rfc-editor.org/info/rfc6482> [Local MD](rfc6482.md)
...
<a name="RFC7454"></a> **[RFC7454]** J. Durand, I. Pepelnjak, G. Doering, "BGP Operations and Security", BCP 194, RFC 7454, DOI 10.17487/RFC7454, February 2015. <https://www.rfc-editor.org/info/rfc7454> [Local MD](rfc7454.md)

<a name="SPAR-v4"></a> **[SPAR-v4]** "IANA IPv4 Special-Purpose Address Registry", <https://www.iana.org/assignments/iana-ipv4-special-registry/>

<a name="SPAR-v6"></a> **[SPAR-v6]** "IANA IPv6 Special-Purpose Address Registry", <https://www.iana.org/assignments/iana-ipv6-special-registry/>

<a name="Sriram-RIPE63"></a> **[Sriram-RIPE63]** K Sriram, R Bush, "Estimating CPU Cost of BGPSEC on a Router", March 2012. Presented at RIPE 63 and at the SIDR WG meeting at
	    IETF 83 <http://www.ietf.org/proceedings/83/slides/slides-83-sidr-7.pdf>

<a name="Sriram-URPF"></a> **[Sriram-URPF]** K Sriram, D Montgomery, J Haas, "Enhanced Feasible-Path Unicast Reverse Path Filtering", March 2018. Presented at the OPSEC WG meeting at IETF 101 <https://datatracker.ietf.org/meeting/101/materials/slides-101-opsec-draft-sriram-opsec-urpf-improvements-00>
...
Oliver Borchert,
Mehmet Adalier, and
Joel Jaeggli for comments and suggestions. The comments and suggestions received from the IESG reviewers are also much appreciated.
...
- Organization: USA National Institute of Standards and Technology
- Address: 100 Bureau Drive, Gaithersburg, MD, 20899, United States of America
- Email: ksriram@nist.gov
