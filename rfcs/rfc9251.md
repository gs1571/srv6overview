...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-bess-evpn-igmp-mld-proxy-21
...
- [prev](https://datatracker.ietf.org/doc/draft-ietf-bess-evpn-igmp-mld-proxy-21)
- [alternate](https://dx.doi.org/10.17487/rfc9251)
...
- [2. Specification of Requirements](#section-2)
- [3. Terminology](#section-3)
- [4. IGMP/MLD Proxy](#section-4)
  - [4.1. Proxy Reporting](#section-4.1)
    - [4.1.1. IGMP/MLD Membership Report Advertisement in BGP](#section-4.1.1)
    - [4.1.2. IGMP/MLD Leave Group Advertisement in BGP](#section-4.1.2)
  - [4.2. Proxy Querier](#section-4.2)
- [5. Operation](#section-5)
  - [5.1. PE with Only Attached Hosts for a Given Subnet](#section-5.1)
  - [5.2. PE with a Mix of Attached Hosts and a Multicast Source](#section-5.2)
  - [5.3. PE with a Mix of Attached Hosts, a Multicast Source, and a Router](#section-5.3)
- [6. All-Active Multihoming](#section-6)
  - [6.1. Local IGMP/MLD Membership Report Synchronization](#local-igmp-mld)
  - [6.2. Local IGMP/MLD Leave Group Synchronization](#section-6.2)
    - [6.2.1. Remote Leave Group Synchronization](#section-6.2.1)
    - [6.2.2. Common Leave Group Synchronization](#section-6.2.2)
  - [6.3. Mass Withdraw of the Multicast Membership Report Synch Route in Case of Failure](#section-6.3)
- [7. Single-Active Multihoming](#section-7)
- [8. Selective Multicast Procedures for IR Tunnels](#section-8)
- [9. BGP Encoding](#bgp-encoding)
  - [9.1. Selective Multicast Ethernet Tag Route](#SMET)
    - [9.1.1. Constructing the Selective Multicast Ethernet Tag Route](#section-9.1.1)
    - [9.1.2. Reconstructing IGMP/MLD Membership Reports from the Selective Multicast Route](#section-9.1.2)
    - [9.1.3. Default Selective Multicast Route](#section-9.1.3)
  - [9.2. Multicast Membership Report Synch Route](#section-9.2)
    - [9.2.1. Constructing the Multicast Membership Report Synch Route](#section-9.2.1)
    - [9.2.2. Reconstructing IGMP/MLD Membership Reports from a Multicast Membership
	  Report Synch Route](#section-9.2.2)
  - [9.3. Multicast Leave Synch Route](#section-9.3)
    - [9.3.1. Constructing the Multicast Leave Synch Route](#section-9.3.1)
    - [9.3.2. Reconstructing IGMP/MLD Leave from a Multicast Leave Synch Route](#section-9.3.2)
  - [9.4. Multicast Flags Extended Community](#section-9.4)
  - [9.5. EVI-RT Extended Community](#evi-rt)
  - [9.6. Rewriting of RT ECs and EVI-RT ECs by ASBRs](#section-9.6)
  - [9.7. BGP Error Handling](#section-9.7)
- [10. IGMP Version 1 Membership Report](#section-10)
- [11. Security Considerations](#section-11)
- [12. IANA Considerations](#section-12)
  - [12.1. EVPN Extended Community Sub-Types Registration](#section-12.1)
  - [12.2. EVPN Route Types Registration](#section-12.2)
  - [12.3. Multicast Flags Extended Community Registry](#multicast_flags_extended_community)
...
In data center (DC) applications, a point of delivery (POD) can consist of a
...
1. Reduce flooding of IGMP/MLD messages: Just like the ARP/Neighbor Discovery (ND)
	suppression
...
2. Distributed anycast multicast proxy: It is desirable for the EVPN
	...
	subnet.
3. Selective multicast: This describes forwarding multicast traffic over the EVPN
	...
	Point-to-Multipoint (P2MP) tunnels can be found in [EVPN-BUM](#I-D.ietf-bess-evpn-bum-procedure-updates).
...
The key words "**MUST**", "**MUST NOT**",
      ...
      BCP 14 [RFC2119](#RFC2119) [RFC8174](#RFC8174) when, and only when,  they appear in all capitals, as shown here.
...
**BD:**
  Broadcast Domain. As per [RFC7432](#RFC7432), an EVPN instance
...
This document also assumes familiarity with the terminology of
      [RFC7432](#RFC7432), [RFC3376](#RFC3376), and [RFC2236](#RFC2236).
      ...
      BGP update is stated in [Section 9](#bgp-encoding).
...
When IGMP is used between hosts and their first hop EVPN
	...
	exchange and register IGMP multicast groups; see [Section 9](#bgp-encoding).

### IGMP/MLD Membership Report Advertisement in BGP

When a PE wants to advertise an IGMP Membership Report using
	  the BGP EVPN route, it follows the proceeding rules (BGP encoding
	  is stated in [Section 9](#bgp-encoding)). The first four
...
2. When the first hop PE receives an IGMPv3 Membership Report for (S,G) on a given
	    ...
	    the future.
3. When the first hop PE receives an IGMP version-X Membership Report first for
...
4. When the first hop PE receives an IGMP version-X Membership Report first for
...
3. Upon receiving an EVPN SMET route(s) and before generating the
	    ...
	    host, it will not forward multicast data to it. Per [RFC4541](#RFC4541) , when an
...
### IGMP/MLD Leave Group Advertisement in BGP
...
1. When a PE receives an EVPN SMET route for a given (*,G), it
	    ...
	    "treat-as-withdraw" procedure per [RFC7606](#RFC7606).
...
1. to enable the collection of EVPN PEs providing Layer 2 Virtual Private Network
	  (L2VPN) service to
	  act as a distributed multicast router with an anycast IP address for all
	  attached hosts in that subnet
2. to enable suppression of IGMP Membership Reports and Membership Queries over
	  MPLS/IP core

# Operation

Consider the EVPN network in [Figure 1](#EVPN), where there is an EVPN
...
```
## PE with Only Attached Hosts for a Given Subnet

When PE1 receives an IGMPv2 Membership Report from H1, it does not forward 
...
## PE with a Mix of Attached Hosts and a Multicast Source
...
## PE with a Mix of Attached Hosts, a Multicast Source, and a Router
...
Because the Link Aggregation Group (LAG) flow hashing algorithm used by the CE is unknown at
...
<a name="local-igmp-mld"></a>
## Local IGMP/MLD Membership Report Synchronization
...
## Local IGMP/MLD Leave Group Synchronization
...
1. It computes the Maximum Response Time, which is the duration of the
	  ...
	  Member Query Interval (described in [RFC2236](#RFC2236)), plus a
...
4. It advertises an IGMP Leave Synch route for that (ES,BD).
...
### Remote Leave Group Synchronization
...
### Common Leave Group Synchronization
...
## Mass Withdraw of the Multicast Membership Report Synch Route in Case of Failure

A PE that has received an IGMP Membership Request would have synced the IGMP
	Membership Report by the procedure defined in [Section 6.1](#local-igmp-mld). If a PE with the local Membership Report
...
2. It sends (x,G) traffic to the set of PEs supporting IGMP or MLD Proxies
...
A SMET route-type-specific EVPN NLRI
...
The Route Distinguisher (RD) **SHOULD** be a Type 1 RD [RFC4364](#RFC4364).  The
...
Reserved bits **MUST** be set to 0. They can be defined
	  by other documents in the future.

IGMP is used to receive group membership information from hosts
...
### Reconstructing IGMP/MLD Membership Reports from the Selective Multicast Route

This section describes the procedures used to reconstruct IGMP/MLD Membership
...
### Default Selective Multicast Route
...
```
Consider the EVPN network in [Figure 2](#EVPN-domain), where there is an EVPN
	   ...
	   wildcard (*,*) routes, the procedure from [RFC6625](#RFC6625)
            **MUST** be used.

## Multicast Membership Report Synch Route

This EVPN route type is used to coordinate the IGMP Membership Report (x,G)
...
### Constructing the Multicast Membership Report Synch Route

This section describes the procedures used to construct the IGMP Membership Report
	  ...
	  corresponding to its multihomed ESs.

An IGMP Membership Report Synch route **MUST** carry exactly one
	  ES-Import Route
	  ...
	  was received.  See [Section 9.5](#evi-rt) for details on how to
	  encode and construct the EVI-RT EC.

The RD **SHOULD** be Type 1 [RFC4364](#RFC4364).  The
	  value field comprises an IP address of the PE (typically, the
	  loopback address), followed by a number unique to the PE.
...
The Multicast Group Length **MUST** be set to the length of the
	  Multicast Group
...
### Reconstructing IGMP/MLD Membership Reports from a Multicast Membership
	  Report Synch Route
...
Reserved bits in the flag **MUST** be set to 0. They can be
	defined by other documents in the future.

### Constructing the Multicast Leave Synch Route
...
This section describes the procedures used to construct the IGMP
	  ...
	  EVIs corresponding to its multihomed Ethernet segments.
...
An IGMP Leave Synch route **MUST** carry exactly one ES-Import Route
	  ...
	  Leave was received.  See [Section 9.5](#evi-rt) for details on how to form the
	  EVI-RT EC.
...
The ESI **MUST** be set to the 10-octet 
	  value defined for the ES.
...
The Originator Router Length is the length of the Originator Router Address
	  in bits.

The Originator Router Address is the IP address of the router
	  originating the prefix.

The Reserved field is not part of the route key. The originator **MUST** set
...
### Reconstructing IGMP/MLD Leave from a Multicast Leave Synch Route

This section describes the procedures used to reconstruct IGMP/MLD Leave from
...
The Multicast Flags Extended Community is a new EVPN Extended 
	...
	communities with a Type Value of 0x06.  IANA has assigned 0x09 to Multicast Flags Extended Community in the "EVPN Extended Community Sub-Types" subregistry.
...
A PE that supports IGMP and/or the MLD Proxy on a given BD
	**MUST** attach this extended community to the IMET route it
	...
	Support flags to 1. Note that a PE compliant with [RFC7432](#RFC7432) 
...
The low-order (least significant) 2 bits are defined as the "IGMP
	Proxy Support" and "MLD Proxy Support" bits (see [Section 12.3](#multicast_flags_extended_community). The absence of this
...
Since an IGMP Membership Report Synch or IGMP Leave Synch route does not need
...
This document specifies four new ECs that
...
Suppose a PE receives a particular IGMP Membership Report Synch or IGMP Leave
	...
	procedure per [RFC7606](#RFC7606).

Note that an EVI-RT EC is not a Route Target extended community,
	is not visible to the RT Constrain mechanism [RFC4684](#RFC4684),
...
The value of "n" is 0x0A, 0x0B, 0x0C, or 0x0D, corresponding
	to EVI-RT types 0, 1, 2, or 3, respectively.

## Rewriting of RT ECs and EVI-RT ECs by ASBRs
...
If a received BGP update contains Flags not in accordance with the IGMP/MLD
	version-X expectation, 
	the PE **MUST** apply the "treat-as-withdraw" procedure per [RFC7606](#RFC7606).

If a received BGP update is malformed such that BGP route keys cannot be extracted,
	then the BGP update **MUST** be considered invalid. The receiving PE
	**MUST** apply the "session reset" procedure per [RFC7606](#RFC7606).
...
This document describes a means to efficiently operate IGMP and MLD on a subnet
      ...
      described in [RFC7432](#RFC7432), and specific multicast
      considerations are outlined in
      [RFC6513](#RFC6513) and [RFC6514](#RFC6514).
      The EVPN and associated IGMP Proxy provides a single 
      broadcast domain so the same security considerations of IGMPv2 [RFC2236](#RFC2236), IGMPv3
      [RFC3376](#RFC3376), MLD [RFC2710](#RFC2710),
      or MLDv2 [RFC3810](#RFC3810) apply.

# IANA Considerations

## EVPN Extended Community Sub-Types Registration
...
**Table: EVPN Extended Community Sub-Types Subregistry Allocated Codepoints**
...
## EVPN Route Types Registration

IANA has allocated the following EVPN route types in the "EVPN
...
<a name="multicast_flags_extended_community"></a>
## Multicast Flags Extended Community Registry

IANA has created and now maintains a new subregistry called "Multicast Flags Extended Community" under the "Border Gateway Protocol (BGP) Extended Communities" registry. The registration procedure is First Come First Served [RFC8126](#RFC8126). For the 16-bit Flags field, the bits are numbered 0-15, from high order to low order. The registry was initialized as follows:

**Table: Multicast Flags Extended Community**
...
<a name="RFC2236"></a> **[RFC2236]** W. Fenner, "Internet Group Management Protocol, Version 2", RFC 2236, DOI 10.17487/RFC2236, November 1997. <https://www.rfc-editor.org/info/rfc2236> [Local MD](rfc2236.md)

<a name="RFC2710"></a> **[RFC2710]** S. Deering, W. Fenner, B. Haberman, "Multicast Listener Discovery (MLD) for IPv6", RFC 2710, DOI 10.17487/RFC2710, October 1999. <https://www.rfc-editor.org/info/rfc2710> [Local MD](rfc2710.md)

<a name="RFC3376"></a> **[RFC3376]** B. Cain, S. Deering, I. Kouvelas, B. Fenner, A. Thyagarajan, "Internet Group Management Protocol, Version 3", RFC 3376, DOI 10.17487/RFC3376, October 2002. <https://www.rfc-editor.org/info/rfc3376> [Local MD](rfc3376.md)
...
<a name="RFC6625"></a> **[RFC6625]** E. Rosen, Y. Rekhter, W. Hendrickx, R. Qiu, "Wildcards in Multicast VPN Auto-Discovery Routes", RFC 6625, DOI 10.17487/RFC6625, May 2012. <https://www.rfc-editor.org/info/rfc6625> [Local MD](rfc6625.md)
...
<a name="I-D.ietf-bess-evpn-bum-procedure-updates"></a> **[EVPN-BUM]** Zhaohui Zhang, Wen Lin, Jorge Rabadan, Keyur Patel, Ali Sajassi, "Updates on EVPN BUM Procedures", Internet-Draft draft-ietf-bess-evpn-bum-procedure-updates-14, November 2021. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-bess-evpn-bum-procedure-updates-14>
...
The authors would like to thank Stephane Litkowski, Jorge Rabadan,
      Anoop Ghanwani, Jeffrey Haas, Krishna Muddenahally Ananthamurthy, and Swadesh Agrawal
