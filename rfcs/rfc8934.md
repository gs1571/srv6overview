...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-pce-stateful-pce-lsp-scheduling-27
...
- [prev](https://datatracker.ietf.org/doc/draft-ietf-pce-stateful-pce-lsp-scheduling-27)
- [alternate](https://dx.doi.org/10.17487/rfc8934)
- [alternate](urn:issn:2070-1721)

## Abstract

This document defines a set of extensions to the stateful
...
Information about the current status of this document, any
            ...
            [https://www.rfc-editor.org/info/rfc8934](https://www.rfc-editor.org/info/rfc8934).
...
- [3. Motivation and Objectives](#section-3)
- [4. Procedures and Mechanisms](#section-4)
  - [4.1. LSP Scheduling Overview](#LSPSchedulingOverview)
  - [4.2. Support of LSP Scheduling](#section-4.2)
    - [4.2.1. LSP Scheduling](#section-4.2.1)
    - [4.2.2. Periodical LSP Scheduling](#section-4.2.2)
  - [4.3. Scheduled LSP Creation](#section-4.3)
  - [4.4. Scheduled LSP Modifications](#lsp-modify)
  - [4.5. Scheduled LSP Activation and Deletion](#section-4.5)
- [5. PCEP Objects and TLVs](#section-5)
  - [5.1. Stateful PCE Capability TLV](#section-5.1)
  - [5.2. LSP Object](#section-5.2)
    - [5.2.1. SCHED-LSP-ATTRIBUTE TLV](#SCHED-LSP-ATTRIBUTE)
    - [5.2.2. SCHED-PD-LSP-ATTRIBUTE TLV](#SCHED-PD-LSP-ATTRIBUTE)
- [6. The PCEP Messages](#pcep-msgs)
  - [6.1. The PCRpt Message](#section-6.1)
  - [6.2. The PCUpd Message](#section-6.2)
  - [6.3. The PCInitiate Message](#section-6.3)
  - [6.4. The PCReq message](#section-6.4)
  - [6.5. The PCRep Message](#section-6.5)
  - [6.6. The PCErr Message](#section-6.6)
- [7. Security Considerations](#section-7)
- [8. Manageability Consideration](#section-8)
...
- [9. IANA Considerations](#section-9)
  - [9.1. PCEP TLV Type Indicators](#section-9.1)
    - [9.1.1. SCHED-PD-LSP-ATTRIBUTE TLV Opt Field](#section-9.1.1)
    - [9.1.2. Schedule TLVs Flag Field](#section-9.1.2)
  - [9.2. STATEFUL-PCE-CAPABILITY TLV Flag Field](#section-9.2)
  - [9.3. PCEP-ERROR Object Error Types and Values](#section-9.3)
...
The PCE Communication Protocol (PCEP) defined in
      [RFC5440](#RFC5440) is used between a Path
...
[RFC8231](#RFC8231) describes a set of
...
Traditionally, the usage and allocation of network resources,
      ...
      for transporting any service. [RFC8413](#RFC8413)
      then
...
The scheduled reservation of TE resources allows network operators to
   ...
   service.
      The requirement of
      scheduled LSP provisioning is mentioned in [RFC8231](#RFC8231)
      and [RFC7399](#RFC7399). 
      Also, for
      deterministic networks [RFC8655](#RFC8655), 
      ...
      segment routing [RFC8402](#RFC8402)
...
- Active Stateful PCE [RFC8051](#RFC8051)
- Delegation [RFC8051](#RFC8051)
- PCE-initiated LSP [RFC8281](#RFC8281)
- PCC [RFC5440](#RFC5440)
- PCE [RFC5440](#RFC5440)
- TE LSP [RFC5440](#RFC5440)
- TED (Traffic Engineering Database) [RFC5440](#RFC5440)
- LSP-DB (LSP State Database) [RFC8051](#RFC8051)
...
**Scheduled TE LSP (or Scheduled LSP for short):**
  An LSP with scheduling
              ...
              LSP availability for the required time and duration.

**Scheduled LSP-DB (SLSP-DB):**
  A database of scheduled LSPs.

**Scheduled TED:**
  Traffic engineering database with the
...
**Duration:**
  This value indicates the length of time that
...
A stateful PCE [RFC8231](#RFC8231) can support
      better efficiency 
      by using LSP scheduling
      described in the use case of [RFC8051](#RFC8051). 
...
Note that existing configuration tools can be used for LSP
      scheduling, but as highlighted in [RFC8231](#RFC8231)  
      as well as discussions in
      [RFC8413](#RFC8413), doing this as a part of PCEP
      in a
...
<a name="LSPSchedulingOverview"></a>
## LSP Scheduling Overview

LSP scheduling allows PCEs and PCCs to provide scheduled LSP
...
For stateful PCE supporting LSP scheduling, there are two types of
        ...
        [RFC8231](#RFC8231), while the other is the
...
Furthermore, a scheduled TED can be generated from the scheduled
	...
	scheduled
        TED **MUST** be maintained by all PCEs within the network
        environment.

In the case of implementing PCC-initiated scheduled LSPs, when
        delegating a scheduled LSP, a PCC **MUST** include that
        LSP's scheduling parameters (see [Section 5.2.1](#SCHED-LSP-ATTRIBUTE)), including the start time and duration, using a
        ...
        PCE **MUST** check whether the parameters are valid. If
        they are valid, it **SHALL** check the scheduled TED for
        ...
        the active stateful PCE techniques [RFC8231](#RFC8231).

Note that the active stateful PCE can update to the PCC with the
...
via a possible PCEP extension (see [PCE-STATE-SYNC](#I-D.litkowski-pce-state-sync)). The technique used to synchronize an
...
The scheduled LSP can also be initiated by a PCE itself.  In the
        case of implementing a PCE-initiated scheduled LSP, the stateful PCE
        **SHALL** check the network resource availability for the
        ...
        case, the SCHED-LSP-ATTRIBUTE TLV (see [Section 5.2.1](#SCHED-LSP-ATTRIBUTE)) **MUST**
        be included in the message, whereas for the latter, the
        SCHED-LSP-ATTRIBUTE TLV **SHOULD NOT** be included.  Either
        way, the synchronization to other PCEs **MUST** be done
	...
	**MUST** 
	  ...
	  **MUST**
	    initiate the removal of the LSP
          based on the flag set in the SCHED-LSP-ATTRIBUTE TLV.

## Support of LSP Scheduling
...
When an LSP is configured with arbitrary scheduling period [Ta,
...
### Periodical LSP Scheduling

In addition to LSP scheduling at an arbitrary time period, there
          is also periodical LSP scheduling.

Periodical LSP scheduling means an LSP has multiple time
	  intervals
...
When an LSP is configured with a scheduling interval such as
...
#### Elastic Time LSP Scheduling

In addition to the basic LSP scheduling at an arbitrary time
	    ...
	    and Q
            is called the elastic range upper bound.

For a simple time interval such as [Ta, Tb] with an elastic
	    ...
	    earlier/left or 
            some time such as up to Q seconds later/right.

When an LSP is configured with elastic time interval "[Ta, Tb]
...
If a user wants to keep the same repeat cycle between any two
      ...
      [Ta+X, Tb+X], [Ta+C+X, Tb+C+X], ..., [Ta+nC+X,
            Tb+nC+X], where -P <= X <= Q.

#### Grace Periods

Besides the stated time scheduling, a user may want to have
...
When an LSP is configured with a simple time interval such as
...
up to carry traffic in best effort.

## Scheduled LSP Creation

In order to realize PCC-initiated scheduled LSPs in a centralized
        network environment, a PCC **MUST** separate the setup of
	an LSP into two
...
For a PCE-initiated scheduled LSP, the stateful PCE
        **MUST** automatically compute a path for the scheduled LSP
        ...
        to the PCC to create a normal LSP (as per [RFC8281](#RFC8281)).

For both PCC-initiated and PCE-initiated Scheduled LSPs:

- The stateful PCE **MUST** update its local scheduled
	  LSP-DB
            and scheduled TED with the scheduled LSP.
- Upon receiving the PCUpd message or PCInitiate message for the
	  scheduled
            ...
            SCHED-LSP-ATTRIBUTE TLV (see [Section 5.2.1](#SCHED-LSP-ATTRIBUTE)) or
            SCHED-PD-LSP-ATTRIBUTE TLV (see [Section 5.2.2](#SCHED-PD-LSP-ATTRIBUTE))
            in the message and 
            does not trigger signaling for the LSP
            setup on LSRs immediately.
- The stateful PCE **MUST** update the scheduled LSP
            parameters on any network events using the PCUpd message to the
	    PCC. These
            changes are also synchronized to other PCEs.
- When it is time for the LSP to be set up
            (i.e., at the start time), based on the value of the C flag for
	    the
            scheduled TLV,
            either the PCC **MUST** trigger the LSP to be signaled
	    or the delegated 
            PCE **MUST** send a PCUpd message to the head-end LSR
	        providing the updated path to be signaled 
            (with the A flag set to indicate LSP activation).

<a name="lsp-modify"></a>
## Scheduled LSP Modifications
...
In the PCC-initiated case, the PCC **MUST** send the PCE
        ...
        [Section 5.2.1](#SCHED-LSP-ATTRIBUTE)) or
        SCHED-PD-LSP-ATTRIBUTE TLV (see [Section 5.2.2](#SCHED-PD-LSP-ATTRIBUTE)) carried in the LSP object.  The PCE
        **SHOULD** take the updated resources and schedule into
...
be impacted, and this **MUST** be conveyed by the use of an empty
Explicit Route Object (ERO) in the PCEP messages.

In the PCE-initiated case, the stateful PCE would recompute the
	path based on 
         ...
         message, it **SHOULD** update the path and other
	 scheduling and resource 
         information by sending a PCUpd message.

## Scheduled LSP Activation and Deletion
...
In the PCC-initiated case, when it is time for the LSP to be set up
        ...
        scheduled TLV, either the PCC **MUST** trigger the LSP to
        be signaled, or the delegated PCE **MUST** send a PCUpd
        ...
        **MUST** report the status of the active LSP as per the
        procedures in [RFC8231](#RFC8231), and at this
        time, the LSP **MUST** be considered part of the LSP-DB.
        The A flag **MUST** be set in the scheduled TLV to indicate
        that the LSP is active now. After the scheduled duration expires,
        based on the C flag, the PCC **MUST** trigger the LSP
        deletion on itself, or the delegated PCE **MUST** send a
...
In the PCE-initiated case, based on the local policy, if the
	scheduled LSP is 
	     ...
	     PCC-initiated case,
	          ...
	          **MUST** send the PCInitiate message 
             to the PCC at the start time to create a normal LSP without the
	     scheduled TLV 
             and remove the LSP after the duration expires, as per [RFC8281](#RFC8281).

# PCEP Objects and TLVs
...
A PCC and a PCE indicate their ability to support LSP scheduling
        ...
        multiple PCEs, the PCEs **SHOULD** also establish a PCEP
        ...
        is defined in [RFC8231](#RFC8231) and updated in
        [RFC8281](#RFC8281) and [RFC8232](#RFC8232).  In this document, we define a new flag bit B
        ...
        indicate the support of LSP periodical scheduling.
...
**B (LSP-SCHEDULING-CAPABILITY) - 1 bit (Bit Position 22):**
  If set to 1
                ...
                PCE is capable of LSP scheduling. The B bit **MUST**
  	      be set by both
...
**PD (PD-LSP-CAPABILITY) - 1 bit (Bit Position - 21):**
  If set to 1 by a
  	      ...
  	      LSP
                ...
                the periodical LSP scheduling capability **MUST** be
  	  ignored.
...
The LSP object is defined in [RFC8231](#RFC8231). 
          ...
          scheduling. The LSP object for a scheduled LSP **MUST NOT** include
          these two TLVs. Only one scheduling, either normal or periodical,
          is allowed for a scheduled LSP.
...
The presence of the SCHED-LSP-ATTRIBUTE TLV in the LSP object
          ...
          periodical. The SCHED-LSP-ATTRIBUTE TLV **MUST** be
	    present in the LSP object for each normal-scheduled LSP carried in
	      the PCEP messages. The
          SCHED-PD-LSP-ATTRIBUTE TLV **MUST** be used in the LSP
	  object for each periodic-scheduled LSP carried in the PCEP
	  messages.

Only one SCHED-LSP-ATTRIBUTE TLV **SHOULD** be present
	in the LSP object. 
...
<a name="SCHED-LSP-ATTRIBUTE"></a>
### SCHED-LSP-ATTRIBUTE TLV

The SCHED-LSP-ATTRIBUTE TLV **MAY** be included as an
	  optional TLV
            within the LSP object for LSP scheduling for the requesting
            traffic service.

This TLV **MUST NOT** be included unless both PCEP
	  peers have set the B
            ...
            didn't set the B bit to one, the peer **MUST** generate
...
The format of the SCHED-LSP-ATTRIBUTE TLV is shown in [Figure 1](#sched-lsp-attr-tlv).
...
```
The type of the TLV is 49, and the TLV has a fixed length of
            16 octets.
...
**Flags (8 bits):**
  ...
  **R (1 bit):**
    Set to 1 to indicate that the Start-Time is a relative time,
    		which is the number of seconds from the
                    current time. 
                    The PCEs and PCCs **MUST** synchronize their clocks 
                    when relative time is used. 
                    It is **RECOMMENDED** that the Network Time
    		Protocol 
                    [RFC5905](#RFC5905) 
  ...
  **A (1 bit):**
    Set to 1 to indicate that the 
                    scheduled LSP has been activated.
...
**Start-Time (32 bits):**
  This value, in seconds,
                  ...
                  the corresponding LSP **MUST** be set up and
  		activated. 
                  Note that the transmission delay **SHOULD** be
  		considered when R=1 
                  and the value of Start-Time is small.

**Duration (32 bits):**
  This value, in seconds, indicates the duration that the LSP
                carries a traffic flow and the corresponding LSP
                **MUST** be up to carry traffic. At the expiry of
                this duration, the LSP **MUST** be torn down and
                deleted.  A value of 0 **MUST NOT** be used in Duration
                since it does not make any sense.  The value of Duration
                **SHOULD** be greater than a constant
                MINIMUM-DURATION seconds, where MINIMUM-DURATION is 5.


Start-Time indicates a time at or before which the scheduled LSP
          **MUST** be set up. When the R bit is set to 0, the value
...
In addition, the SCHED-LSP-ATTRIBUTE TLV contains the G flag set
          ...
          period or elastic range, but it **MUST NOT** provide both
          for an LSP.

**GrB (Grace-Before, 16 bits):**
  The grace period time
                  length, in seconds, before the start time.
...
**Elastic-Lower-Bound (16 bits):**
  The maximum amount of
  	    time,
                  in seconds, that the time interval can shift lower/left.
...
<a name="SCHED-PD-LSP-ATTRIBUTE"></a>
### SCHED-PD-LSP-ATTRIBUTE TLV

The periodical LSP is a special case of LSP scheduling. The
...
This TLV **MUST NOT** be included unless both PCEP
	  peers have set the B
	        ...
	        bits are zero), the peer **MUST** generate
            a PCEP Error (PCErr) with a PCEP-ERROR object
                having Error-Type = 19 (Invalid Operation) and 
...
The format of the SCHED-PD-LSP-ATTRIBUTE TLV is shown in [Figure 2](#sched-pd-lsp-attr-tlv).
...
**NR (12 bits):**
  The number of repeats.
                  During each repetition, LSP carries traffic.
...
The Path Computation State Report (PCRpt) message is a PCEP message
	sent by a PCC
      to a PCE to report the status of one or more LSPs, as per [RFC8231](#RFC8231).  Each LSP State
      ...
      case of a scheduled LSP, a scheduled TLV **MUST** be carried
      in the LSP
...
The Path Computation Update Request (PCUpd) message is a PCEP
	message sent by a
      PCE to a PCC to update LSP parameters on one or more LSPs, as per [RFC8231](#RFC8231). Each
...
The LSP Initiate Request (PCInitiate) message is a PCEP message
	sent
...
The Path Computation Request (PCReq) message is a PCEP message sent 
	    ...
	    message to request the 
	        ...
	        **MAY** use the
            ...
            (refer to [Section 4.4](#lsp-modify)).

## The PCRep Message

The Path Computation Reply (PCRep) message is a PCEP message sent
	by a PCE to a PCC in reply to a path
	computation request [RFC5440](#RFC5440), and it
	may 
            contain the LSP object [RFC8231](#RFC8231)
	        to identify the LSP for which the path 
      ...
      path could be found. In the case of a scheduled LSP, a scheduled TLV 
            **MUST** be carried in the LSP object in PCRep message
	    to indicate the 
...
The PCEP Error (PCErr) message is a PCEP message, as described in
        [RFC5440](#RFC5440), for error reporting. This
        ...
        and error values of [RFC5440](#RFC5440) and
        [RFC8231](#RFC8231) wherever appropriate.

The PCEP extensions for scheduling **MUST NOT** be used
        ...
        of LSP object with the scheduled TLV, it **MUST** generate
   ...
   **SHOULD** ignore the TLV. As per [RFC5440](#RFC5440), 
   a legacy PCEP implementation that does not understand this specification
   would consider a scheduled TLV unknown and ignore it.
...
If the PCC
   ...
   LSP-ERROR-CODE TLV ([RFC8231](#RFC8231)) 
   with LSP Error-value = 4 (Unacceptable parameters) in the LSP object 
   (with the scheduled TLV) in the PCRpt message to the PCE.

The scheduled TLV **MUST** be included in the LSP object
	for the scheduled LSPs. 
	If the TLV is missing, the receiving PCEP speaker **MUST**
	send a PCErr message with Error-Type = 6 (Mandatory Object missing)
	and

	Error-value = 16 (Scheduled TLV missing).

# Security Considerations

This document defines the LSP-SCHEDULING-CAPABILITY TLV and 
      SCHED-LSP-ATTRIBUTE TLV; the security considerations
      discussed in [RFC5440](#RFC5440), [RFC8231](#RFC8231), and [RFC8281](#RFC8281) continue to apply. In some deployments,
   ...
   Thus, such deployments **SHOULD** employ suitable PCEP security
   mechanisms 
   like TCP Authentication Option (TCP-AO), which is discussed in [RFC5925](#RFC5925) and
   [RFC8253](#RFC8253). Note that
   [RFC8253](#RFC8253) is considered a security
...
The LSP scheduling feature **MUST** be controlled per
	tunnel by the
        active stateful PCE. The values for parameters like start time and
        duration **SHOULD** be configurable by customer
	applications and based on
        ...
        respectively. 
        One day has 86,400 seconds. One year has 31,536,000 seconds.

When configuring the parameters for time, a user
	**SHOULD** 
        consider leap years and leap seconds.
        If a scheduled LSP has a time interval containing a leap year,
        the duration of the LSP is 366 days plus the rest of the interval.
...
An implementation **SHOULD** allow the operator to view
   ...
   module [PCE-PCEP-YANG](#I-D.ietf-pce-pcep-yang) could be
	extended.
...
Mechanisms defined in this document do not imply any new
	...
	listed in
        ...
        An implementation **SHOULD** allow a user to view
        information, including the status of a scheduled LSP, through a
        Command Line Interface (CLI) tool.  In addition, it **SHOULD**
...
**Table: Additions to PCEP TLV Type Indicators Subregistry**
...
IANA has created and will maintain a new
...
New values are assigned by Standards Action [RFC8126](#RFC8126).

**Table: New SCHED-PD-LSP-ATTRIBUTE TLV Opt Field Subregistry**
...
IANA has created a new subregistry named "Schedule TLVs Flag
          Field" within the "Path Computation Element Protocol (PCEP) Numbers"
          registry.  New values are assigned by Standards Action [RFC8126](#RFC8126).  Each bit should be tracked
...
**Table: New Schedule TLVs Flag Field Subregistry**
...
This document defines new bits in the Flags field in the
        ...
        made the following allocations in this subregistry.

**Table: Additions to STATEFUL-PCE-CAPABILITY TLV Flag Field Subregistry**
...
**Table: Additions to PCEP-ERROR Object Error Types and Values
  Subregistry**

| Error-Type | Meaning | Error-value |
| --- | --- | --- |
| 6 | Mandatory Object missing | 16:  Scheduled TLV missing |
| 19 | Invalid Operation | 15: Attempted LSP scheduling while the scheduling capability was not
      advertised |
...
<a name="I-D.ietf-pce-pcep-yang"></a> **[PCE-PCEP-YANG]** Dhruv Dhody, Jonathan Hardwick, Vishnu Pavan Beeram, Jeff Tantsura, "A YANG Data Model for Path Computation Element Communications Protocol (PCEP)", Internet-Draft draft-ietf-pce-pcep-yang-15, October 2020. Work in Progress <https://tools.ietf.org/html/draft-ietf-pce-pcep-yang-15>

<a name="I-D.litkowski-pce-state-sync"></a> **[PCE-STATE-SYNC]** Stephane Litkowski, Siva Sivabalan, Cheng Li, Haomian Zheng, "Inter Stateful Path Computation Element (PCE) Communication Procedures.", Internet-Draft draft-litkowski-pce-state-sync-08, July 2020. Work in Progress <https://tools.ietf.org/html/draft-litkowski-pce-state-sync-08>
...
<a name="RFC8655"></a> **[RFC8655]** N. Finn, P. Thubert, B. Varga, J. Farkas, "Deterministic Networking Architecture", RFC 8655, DOI 10.17487/RFC8655, October 2019. <https://www.rfc-editor.org/info/rfc8655> [Local MD](rfc8655.md)

# Acknowledgments

The authors of this document would also like to thank Rafal Szarecki, Adrian Farrel, and
      Cyril Margaria for the review and comments.
...
- Address: No.32 Xuanwumen West Street, Xicheng District, Beijing, 100053, China
...
- Organization: Huawei
- Address: 101 Software Avenue, Yuhua District, Nanjing, Jiangsu, 210012, China
...
- Organization: Ericsson
- Address: Via A. Negrone 1/A, Genova - Sestri Ponente, Italy
- Email: daniele.ceccarelli@ericsson.com
