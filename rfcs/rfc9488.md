...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-pce-local-protection-enforcement-11
...
**Keywords:** segment routing, adjacency sid, sla, fast reroute, ti-lfa

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-pce-local-protection-enforcement-11)
- [alternate](https://dx.doi.org/10.17487/rfc9488)
- [alternate](urn:issn:2070-1721)

## Abstract

This document updates RFC 5440 to clarify usage of the Local Protection Desired bit signaled in the Path Computation Element Communication Protocol (PCEP).
                This document also introduces a new flag for signaling protection enforcement in PCEP.
...
Information about the current status of this document, any
            ...
            [https://www.rfc-editor.org/info/rfc9488](https://www.rfc-editor.org/info/rfc9488).
...
- [4. Motivation](#motivation)
  - [4.1. Implementation Differences](#implementation-differences)
  - [4.2. SLA Enforcement](#sla-enforcement)
- [5. Protection Enforcement Flag (E Flag)](#protection-enforcement-flag--e-flag-)
  - [5.1. Backwards Compatibility](#compatibility)
...
The Path Computation Element Communication Protocol (PCEP) [RFC5440](#RFC5440) enables the communication between a Path Computation Client (PCC) and a PCE or between two PCEs based on the PCE architecture [RFC4655](#RFC4655).

PCEP [RFC5440](#RFC5440) utilizes flags,
      values, and concepts previously defined in RSVP-TE Extensions [RFC3209](#RFC3209) and Fast Reroute Extensions to
      RSVP-TE [RFC4090](#RFC4090).  One such concept in
      PCEP is the Local Protection Desired (L) flag in the LSP
      Attributes (LSPA) object in [RFC5440](#RFC5440), which
      was originally defined in the Session Attribute object in [RFC3209](#RFC3209). In RSVP, this flag signals to
...
PCEP Extensions for Segment Routing [RFC8664](#RFC8664) extends support in PCEP for Segment Routing paths. The
      ...
      [RFC9085](#RFC9085) and take the protection into
...
This document updates [RFC5440](#RFC5440) by further describing the behavior of the Local Protection Desired (L) flag and extends on it with the introduction of the Protection Enforcement (E) flag.
...
**LSPA:**
  LSP Attributes object [RFC5440](#RFC5440)
...
As defined in [RFC5440](#RFC5440), the mechanism to signal protection enforcement in PCEP is the previously mentioned L flag defined in the LSPA object.
                    ...
                    For RSVP-signaled paths, local protection is not within control of the PCE. However, [RFC5440](#RFC5440) does state that "[w]hen set, this means that the computed path must include links protected with Fast Reroute as defined in [RFC4090](#RFC4090)."
                    Implementations that use PCEP [RFC5440](#RFC5440) have interpreted the L flag as either PROTECTION MANDATORY or PROTECTION PREFERRED, leading to operational differences.

<a name="sla-enforcement"></a>
## SLA Enforcement
...
The L flag is a boolean bit and thus unable to distinguish between the different
            ...
            PREFERRED, and UNPROTECTED PREFERRED.
                    ...
                    local protection on others. When enforcement is used, the resulting shortest path calculation is impacted.

For example, PROTECTION MANDATORY is for use cases in which an operator may need the LSP to follow a path that has local protection provided along the full path, ensuring that
                    traffic will be fast rerouted at the point if there is a failure anywhere along the path.
...
When used with Segment Routing, an adjacency may have both a protected SID and an unprotected SID.
...
<a name="protection-enforcement-flag--e-flag-"></a>
# Protection Enforcement Flag (E Flag)

[RFC5440](#RFC5440) describes the encoding of the Local Protection Desired (L) flag.
                The Protection Enforcement (E) flag, which extends the L flag, is specified below.

**Table: Codespace of the Flag Field (LSPA Object)**
...
The following shows the format of the LSPA object as defined in [RFC5440](#RFC5440) with the addition of the E flag defined in this document:
...
**Flags (8 bits):**
  **L (Local Protection Desired):**
    This flag is defined in [RFC5440](#RFC5440)
  ...
  **E (Protection Enforcement):**
    This flag controls the strictness
...
When both the L flag and E flag are set to 0, then the PCE **SHOULD**
                ...
                constraint but **MAY** consider the protection eligibility as an UNPROTECTED
...
If a PCE is unable to infer the protection status of a resource, the PCE **MAY** use local policy to define protected status assumptions.
                ...
                When computing a Segment Routing path, it is **RECOMMENDED** that a PCE assume a Node SID is protected. It is also **RECOMMENDED** that a PCE assume an Adjacency SID is protected if the backup flag advertised with the Adjacency SID is set.
...
For a PCC or PCE that does not yet support this document, the E flag is ignored and set to 0 in PCRpt and/or PCUpd messages as per [RFC5440](#RFC5440) for PCC-initiated LSPs or as per [RFC8281](#RFC8281) for PCE-initiated LSPs. It is important to note that [RFC8231](#RFC8231) and [RFC8281](#RFC8281) permit the LSPA object [RFC5440](#RFC5440) to be included in PCUpd messages for PCC-initiated and PCE-initiated LSPs.
...
For a PCC that does support this document, the E flag **MAY** be set to 1 depending on local configuration.
                    If communicating with a PCE that does not yet support this document, the PCE follows the behavior specified in [RFC5440](#RFC5440) and ignores the E flag.
                    Thus, a computed path might not respect the enforcement constraint.
...
For PCE-initiated LSPs, the PCC **MAY** process the E flag value received from the PCE in a PCUpd message. The PCE **SHOULD** ignore the E flag value received from the PCC in a PCRpt message as it may be communicating with a PCC
                that does not support this document.
...
Securing the PCEP session using Transport Layer Security (TLS) [RFC8253](#RFC8253), as per the recommendations and best current practices in [RFC9325](#RFC9325), is **RECOMMENDED**.
...
**Table: Addition to LSPA Object Flag Field Registry**
...
<a name="RFC4090"></a> **[RFC4090]** P. Pan, G. Swallow, A. Atlas, "Fast Reroute Extensions to RSVP-TE for LSP Tunnels", RFC 4090, DOI 10.17487/RFC4090, May 2005. <https://www.rfc-editor.org/info/rfc4090> [Local MD](rfc4090.md)
...
- Organization: Nokia
- Address: 600 March Road, Kanata, Ontario, K2K 2T6, Canada
- Email: andrew.stone@nokia.com
...
- Address: Pribinova 10, Bratislava, 811 09, Slovakia
- Email: ssidor@cisco.com

**Siva Sivabalan**

- Organization: Ciena Corporation
- Address: 385 Terry Fox Drive, Kanata, Ontario, K2K 0L1, Canada
- Email: ssivabal@ciena.com
