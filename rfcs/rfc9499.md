...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-dnsop-rfc8499bis-10
...
- [prev](https://datatracker.ietf.org/doc/draft-ietf-dnsop-rfc8499bis-10)
- [alternate](https://dx.doi.org/10.17487/rfc9499)
...
- [2. Names](#names)
- [3. DNS Response Codes](#dns-response-codes)
- [4. DNS Transactions](#dns-transactions)
- [5. Resource Records](#rrs)
- [6. DNS Servers and Clients](#dns-servers-and-clients)
- [7. Zones](#zones)
- [8. Wildcards](#wildcards)
- [9. Registration Model](#reg-model)
- [10. General DNSSEC](#dnssec-general)
- [11. DNSSEC States](#dnssec-states)
- [12. Security Considerations](#securitycons)
- [13. IANA Considerations](#ianacons)
- [14. References](#section-14)
  - [14.1. Normative References](#section-14.1)
  - [14.2. Informative References](#section-14.2)
- [Appendix A. Definitions Updated by This Document](#updates-list)
- [Appendix B. Definitions First Defined in This Document](#new-def)
- [Acknowledgements](#acknowledgements)
- [Index](#section-appendix.d)
...
whose messages in both directions have the same format.  ([Section 2](#names) gives a definition of "global DNS", which is often
what people mean when they say "the DNS".)  The protocol and message
format are defined in [RFC1034](#RFC1034) and [RFC1035](#RFC1035). These RFCs defined some terms, and later documents
defined others. Some of the terms from [RFC1034](#RFC1034) and
[RFC1035](#RFC1035) have somewhat different meanings now than
they did in 1987.
...
For example, the WHATWG defines "domain" at [https://url.spec.whatwg.org/](https://url.spec.whatwg.org/).
The Root Server System Advisory Committee (RSSAC) has a good
lexicon [RSSAC026](#RSSAC026).
...
See [Appendix A](#updates-list) for a list of the definitions
...
This document is a small revision to [RFC8499](#RFC8499); that document was
a substantial revision to [RFC7719](#RFC7719).
...
(as mentioned below) implementing this architecture. [Section 2](#names) defines
...
**Domain name:**
  ...
  Note that this is a definition independent of the DNS RFCs ([RFC1034](#RFC1034) and [RFC1035](#RFC1035)), and the definition here
  also applies to systems other than the DNS. [RFC1034](#RFC1034) defines the "domain
  ...
  "names that match the syntax in [RFC1035](#RFC1035)", but possibly with additional
...
**Global DNS:**
  ...
  follows. Most of the rules here come from [RFC1034](#RFC1034) and [RFC1035](#RFC1035), although the term "global DNS" has not been defined before now.
  ...
  **Composition of names:**
    ...
    calculation.
    (Multicast DNS [RFC6762](#RFC6762) allows names up to 255 bytes plus a terminating zero byte
  ...
  **Format of names:**
    ...
    **Wire format:**
      The basic wire format for names in the
                  ...
                  length octet. [RFC1035](#RFC1035) also
    ...
    **Presentation format:**
      The presentation format for names
                  ...
                  instead of "example.tld". [RFC1035](#RFC1035) defines a method for showing octets that do not
                  display in ASCII.
  ...
  **Administration of names:**
    ...
    definition of "delegation" in [Section 7](#zones)).  Policies for administration of
  ...
  **Types of data that can be associated with names:**
    ...
    unique data structures defined in many different RFCs and in the IANA registry at [IANA_Resource_Registry](#IANA_Resource_Registry).

  **Types of metadata for names:**
    Any name that is published in the DNS appears as a set
    of resource records (see the definition of "RRset" in [Section 5](#rrs)).  Some names
    ...
    the definition of "empty non-terminals" in [Section 7](#zones)).

  **Protocol for getting data from a name:**
    The protocol described in [RFC1035](#RFC1035).
...
**Private DNS:**
  Names that use the protocol described in [RFC1035](#RFC1035) but do not rely on
  the global DNS root zone or names that are otherwise not generally available on the
  Internet but are using the protocol described in [RFC1035](#RFC1035).  A system can
  use both the global DNS and one or more private DNS systems; for example, see "Split DNS"
  in [Section 6](#dns-servers-and-clients).
...
**Multicast DNS (mDNS):**
  ...
  names."  (Quoted from [RFC6762](#RFC6762), Abstract)
...
**Locally served DNS zone:**
  ...
  [RFC6303](#RFC6303) defines subdomains of IN-ADDR.ARPA
  ...
  [RFC6303](#RFC6303) and [RFC7793](#RFC7793), or implicit, such as those defined by local DNS administration and not known to the
  resolution client.
...
**Fully Qualified Domain Name (FQDN):**
  ...
  This term first appeared in [RFC819](#RFC0819). In this document, names
...
**Host name:**
  ...
  widely used but are not defined in [RFC1034](#RFC1034), [RFC1035](#RFC1035),
  [RFC1123](#RFC1123), or [RFC2181](#RFC2181). The
  DNS was originally deployed into the Host Tables environment as
  outlined in [RFC952](#RFC0952), and it is likely that the term followed
  ...
  the rules in [RFC1034](#RFC1034), which is also called the "preferred name
...
**Top-Level Domain (TLD):**
  ...
  delegation-centric zones (defined in [Section 7](#zones)), and there are significant policy issues
...
**Internationalized Domain Name (IDN):**
  ...
  time of this writing, normally called "IDNA2008", is defined in [RFC5890](#RFC5890), [RFC5891](#RFC5891), [RFC5892](#RFC5892), [RFC5893](#RFC5893), and [RFC5894](#RFC5894).  These documents define many IDN-specific terms
  such as "LDH label", "A-label", and "U-label".  [RFC6365](#RFC6365) defines more terms that relate to
  internationalization (some of which relate to IDNs); [RFC6055](#RFC6055) has a much more extensive discussion of IDNs,
...
**Alias:**
  ...
  DNAME resource record (DNAME records are defined in [RFC6672](#RFC6672)). See also "canonical name".

**Canonical name:**
  A CNAME resource record 
  "identifies its owner name as an
  alias, and specifies the corresponding canonical name in the RDATA
  section of the RR." (Quoted from [RFC1034](#RFC1034))
...
**CNAME:**
  ...
  (Quoted from [RFC2181](#RFC2181). The quoted
  text has been changed from "label" to "owner".)


<a name="dns-response-codes"></a>
# DNS Response Codes

Some of the response codes (RCODEs) that are defined in [RFC1035](#RFC1035) have acquired their own
shorthand names.  All of the RCODEs are listed at
[IANA_Resource_Registry](#IANA_Resource_Registry), although
that list uses mixed-case capitalization, while most documents use all caps.
Some of the common names for values defined in [RFC1035](#RFC1035) are described in this section.
...
**NOERROR:**
  This RCODE appears as "No error condition" in  [RFC1035](#RFC1035).

**FORMERR:**
  This RCODE appears as "Format error - The name server was unable to interpret the query" in [RFC1035](#RFC1035).

**SERVFAIL:**
  This RCODE appears as "Server failure - The name server was unable to process this query due to a problem with the name
  server" in [RFC1035](#RFC1035).

**NXDOMAIN:**
  This RCODE appears as "Name Error [...] this code signifies that the domain name
  referenced in the query does not exist." in [RFC1035](#RFC1035).
  [RFC2308](#RFC2308) established NXDOMAIN as a synonym for Name Error.

**NOTIMP:**
  This RCODE appears as "Not Implemented - The name server does not support the requested kind of query" in [RFC1035](#RFC1035).
...
**NODATA:**
  ...
  response has to be inferred from the answer." (Quoted from [RFC2308](#RFC2308))
  ...
  contain an SOA record, or there will be no NS records there." (Quoted from [RFC2308](#RFC2308))
  Note that referrals have a similar format to NODATA replies; [RFC2308](#RFC2308)
  ...
  that NXRRSET is a specific error code defined in [RFC2136](#RFC2136).
...
**Negative response:**
  ...
  Sections [RFC2308](#RFC2308) and [RFC2308](#RFC2308) of [RFC2308](#RFC2308) describe the types of negative responses in detail.


<a name="dns-transactions"></a>
# DNS Transactions

The header of a DNS message is its first 12 octets. Many of the fields and flags in
the diagrams in Sections [RFC1035](#RFC1035) through [RFC1035](#RFC1035) of [RFC1035](#RFC1035) are referred to by their names
in each diagram.
...
**Class:**
  A class "identifies a protocol family or instance of a protocol".  (Quoted from [RFC1034](#RFC1034))
  "The DNS tags all data with a class as well as the type, so that we can allow parallel use
  of different formats for data of type address."  (Quoted from [RFC1034](#RFC1034))
...
**QNAME:**
  ...
  class (QCLASS) and asks for RRs which match."  (Quoted from [RFC1034](#RFC1034))
  Strictly speaking, the definition comes from [RFC1035](#RFC1035), where the QNAME is defined in respect of the Question
  section. 
  This definition appears to be applied consistently, as the discussion
  of inverse queries in [RFC1035](#RFC1035) refers to the "owner name of
  ...
  are deprecated in [RFC3425](#RFC3425); thus, relevant
  definitions do not appear in this document.)

  However, [RFC2308](#RFC2308) has an alternate definition that
  ...
  specified in the data field of the CNAME record." (Quoted from [RFC1034](#RFC1034))  This is made explicit in the
  resolution algorithm outlined in [RFC1034](#RFC1034), which says to "change QNAME to the canonical name
  ...
  **QNAME (final):**
  ...
  Note that, because the definition in [RFC2308](#RFC2308) is
  actually for a different concept than what was in [RFC1034](#RFC1034), it would have been better if [RFC2308](#RFC2308) had used a different name for that concept. In
...
**Referrals:**
  ...
  server is authoritative for that answer (see [RFC1035](#RFC1035)).
...
**RR:**
  An acronym for resource record.  (See [RFC1034](#RFC1034).)

**RRset:**
  A set of resource records "with the same label, class and type, but with different
  data" (according to [RFC2181](#RFC2181)).  Also written as "RRSet" in some documents. As a clarification,
  "same label" in this definition means "same owner name".
  In addition, [RFC2181](#RFC2181) states that "the TTLs of all RRs in an RRSet must be the same".

  Note that RRSIG resource records do not match this definition.
  	  [RFC4035](#RFC4035) says:
...
**Master file:**
  ...
  zone, though it can be used to list a cache's contents."  (Quoted from [RFC1035](#RFC1035))
...
**Presentation format:**
  ...
  [RFC1034](#RFC1034) or [RFC1035](#RFC1035). The term "presentation format"
  first appears in [RFC4034](#RFC4034).

**EDNS:**
  The extension mechanisms for DNS, defined in [RFC6891](#RFC6891). Sometimes called "EDNS0" or "EDNS(0)"
...
**OPT:**
  ...
  transaction. (Definition paraphrased from [RFC6891](#RFC6891).) It is used by EDNS.

**Owner:**
  "The domain name where the RR is found."  (Quoted from [RFC1034](#RFC1034))  Often appears in the term "owner name".
...
**SOA field names:**
  ...
  Those fields are defined in [RFC1035](#RFC1035).
  The names (in the order they appear in the SOA RDATA) are MNAME, RNAME, SERIAL, REFRESH, RETRY,
  EXPIRE, and MINIMUM.
  Note that the meaning of the MINIMUM field is updated in [RFC2308](#RFC2308); the new definition
...
**TTL:**
  ...
  bit set to zero."  (Quoted from [RFC2181](#RFC2181))
  Note that [RFC1035](#RFC1035)
  erroneously stated that this is a signed integer; that was fixed by [RFC2181](#RFC2181).
  ...
  [RFC1035](#RFC1035)) [RFC1035](#RFC1035) states "the time interval (in seconds) that the resource
  record may be cached before it should be discarded".  Despite being defined for a resource record, the TTL of every
  resource record in an RRset is required to be the same ([RFC2181](#RFC2181)).
  ...
  has a very short TTL) even though this is against the advice in [RFC1035](#RFC1035).
...
**Address records:**
  ...
  [RFC2181](#RFC2181) informally defines these as "(A, AAAA, etc)".
  Note that new types of address records could be defined in the future.


<a name="dns-servers-and-clients"></a>
...
queries and sends responses using the DNS protocol defined in [RFC1035](#RFC1035) and its successors.
...
[RSSAC026](#RSSAC026).  That document defines terms such as "root
...
**Resolver:**
  ...
  servers in response to client requests."  (Quoted from [RFC1034](#RFC1034))  A resolver performs
  ...
  A related term is "resolve", which is not formally defined in [RFC1034](#RFC1034)
  or [RFC1035](#RFC1035). An imputed definition might be "asking a question that
...
**Stub resolver:**
  ...
  fully defined in [RFC1034](#RFC1034).
  They are fully defined in [RFC1123](#RFC1123).

**Iterative mode:**
  A resolution mode of a server that receives DNS
  queries and responds with a referral to another server. [RFC1034](#RFC1034)
...
**Recursive mode:**
  ...
  the original queries. [RFC1034](#RFC1034) describes this as "the
  first server pursues the query for the client at another server".
  [RFC1034](#RFC1034) says: "in [recursive]
...
**Recursive resolver:**
  ...
  [RFC4697](#RFC4697) tried to differentiate between a
  recursive resolver and an iterative resolver.

**Recursive query:**
  A query with the Recursion Desired (RD) bit set to 1 in the header. (See [RFC1035](#RFC1035).) If recursive service is available and is requested by the RD bit in the query,
  the server uses its resolver to answer the query. (See [RFC1034](#RFC1034).)
...
**Iterative resolution:**
  ...
  to another server and lets the client pursue the query there. (See [RFC1034](#RFC1034).)

  In iterative resolution, the client repeatedly makes non-recursive queries and follows referrals
  and/or aliases. The iterative resolution algorithm is described in [RFC1034](#RFC1034).

**Full resolver:**
  This term is used in [RFC1035](#RFC1035), but it is not defined there. RFC
  1123 defines a "full-service resolver" that may or may not be what was intended
  by "full resolver" in [RFC1035](#RFC1035).
...
**Full-service resolver:**
  [RFC1123](#RFC1123) defines this term
...
**Priming:**
  ...
  some or all of those root servers." (Quoted from [RFC8109](#RFC8109))
...
**Root hints:**
  ...
  For many pieces of software, this list comes built into the software."  (Quoted from [IANA_RootFiles](#IANA_RootFiles))
  This file is often used in priming.

**Negative caching:**
  "The storage of knowledge that something does not exist, cannot
  or does not give an answer." (Quoted from [RFC2308](#RFC2308))

**Authoritative server:**
  "A server that knows the content of a DNS zone from local knowledge, and thus can answer
  queries about that zone without needing to query other servers." (Quoted from [RFC2182](#RFC2182))
...
**Authoritative-only server:**
  ...
  queries from iterative resolvers looking for information in zones it serves."  (Quoted from [RFC4697](#RFC4697))
...
**Zone transfer:**
  ...
  (See [Section 7](#zones) for a description of zones.)
  There are two common standard ways to do zone transfers:
  the AXFR ("Authoritative Transfer") mechanism to copy the full zone (described in [RFC5936](#RFC5936), and 
  the IXFR ("Incremental Transfer") mechanism to copy only parts of the zone that have changed (described in [RFC1995](#RFC1995)).
...
**Secondary server:**
  ...
  zone." (Quoted from [RFC1996](#RFC1996))
  Secondary servers are also discussed in [RFC1034](#RFC1034).
  [RFC2182](#RFC2182) describes secondary servers in
  more detail.  Although early DNS RFCs such as [RFC1996](#RFC1996) referred to this as a "slave", the
...
**Primary server:**
  ...
  for one or more [secondary] servers." (Quoted from [RFC1996](#RFC1996))  Or, more
  specifically, [RFC2136](#RFC2136) calls it "an authoritative server configured to be the source of AXFR or IXFR data
  for one or more [secondary] servers".
  Primary servers are also discussed in [RFC1034](#RFC1034).
  Although early DNS RFCs such as [RFC1996](#RFC1996) referred to this as a "master", the current
  common usage has shifted to "primary".

**Primary master:**
  "The primary master is named in the zone's SOA MNAME field and
  optionally by an NS RR." (Quoted from [RFC1996](#RFC1996))
  [RFC2136](#RFC2136) defines "primary master" as
  ...
  The idea of a primary master is only used in [RFC1996](#RFC1996) and [RFC2136](#RFC2136).
...
**Stealth server:**
  ...
  the zone." (Quoted from [RFC1996](#RFC1996))
...
**Forwarding:**
  ...
  [RFC5625](#RFC5625) does not give a specific definition for forwarding, but
  ...
  that term has not yet been defined (even in [RFC5625](#RFC5625)).

**Forwarder:**
  [RFC2308](#RFC2308) describes a forwarder as "a
  nameserver used to resolve queries instead of directly using the
  authoritative nameserver chain".  [RFC2308](#RFC2308) further says "The
  ...
  [RFC2308](#RFC2308) is silent on whether a forwarder is iterative-only or
...
**Open resolver:**
  ...
  Open resolvers are discussed in [RFC5358](#RFC5358).
...
**Split DNS:**
  ...
  [RFC2775](#RFC2775) gives a related definition that is too specific to be generally useful.
...
**Anycast:**
  ...
  (Quoted from [RFC4786](#RFC4786))
  See [RFC4786](#RFC4786) for more detail on Anycast and other terms that are
  specific to its use.
...
**Instance:**
  ...
  instance'."  (Quoted from [RSSAC026](#RSSAC026))

**Privacy-enabling DNS server:**
  "A DNS server that implements
  DNS over TLS [RFC7858](#RFC7858) and may optionally implement DNS over DTLS
  [RFC8094](#RFC8094)."  (Quoted from [RFC8310](#RFC8310))
  Other types of DNS servers might also be considered privacy-enabling, such as those
  running DNS-over-HTTPS [RFC8484](#RFC8484) or DNS-over-QUIC [RFC9250](#RFC9250).

**DNS-over-TLS (DoT):**
  DNS over TLS as defined in [RFC7858](#RFC7858) and its successors.

**DNS-over-HTTPS (DoH):**
  DNS over HTTPS as defined in [RFC8484](#RFC8484) and its successors.

**DNS-over-QUIC (DoQ):**
  DNS over QUIC as defined in [RFC9250](#RFC9250) and its successors. 
  [RFC9250](#RFC9250) specifically defines DoQ as general-purpose transport
  for DNS that can be used in stub to recursive, recursive to authoritative, and
  zone transfer scenarios.

**Classic DNS:**
  DNS over UDP or DNS over TCP as defined in [RFC1035](#RFC1035) and its successors.
...
**XFR-over-TLS (XoT):**
  DNS zone transfer over TLS, as specified in [RFC9103](#RFC9103).
...
**Zone:**
  ...
  redundant service for the data in a zone."  (Quoted from [RFC1034](#RFC1034))

**Child:**
  "The entity on record that has the delegation of the domain from the
  Parent." (Quoted from [RFC7344](#RFC7344))

**Parent:**
  "The domain in which the Child is registered."  (Quoted from [RFC7344](#RFC7344))  Earlier,
  "parent name server" was defined in [RFC0882](#RFC0882) as "the name server that has authority over the place
  in the domain name space that will hold the new domain".  (Note
  that [RFC0882](#RFC0882) was obsoleted by [RFC1034](#RFC1034) and [RFC1035](#RFC1035).) 
  [RFC819](#RFC0819) also has some description of
  the relationship between parents and children.

**Origin:**
  There are two different uses for this term:

  1. "The domain name that
...
of the domain at the zone's origin."  (Quoted from [RFC2181](#RFC2181))  These days, this sense of
"origin" and "apex" (defined below) are often used
interchangeably.
  2. The domain name within which a given relative domain name
appears in zone files.  Generally seen in the context of "$ORIGIN", which is a
control entry defined in [RFC1035](#RFC1035), as part of the master
...
**Apex:**
  ...
  [RFC4033](#RFC4033) defines it as "the name at the child's side of a zone cut".
  ...
  [RFC1034](#RFC1034) uses the term "top node of the zone" as a synonym of "apex", but that term is not widely used.
...
**Zone cut:**
  ...
  'child' zone (below the cut) from a 'parent' zone (above the cut)."  (Quoted from [RFC2181](#RFC2181); note that this is barely an ostensive
  definition.)
  [RFC1034](#RFC1034) uses "cuts" instead of "zone cut".
...
**Authoritative data:**
  ...
  [RFC4033](#RFC4033), defines "Authoritative RRset", which is related
...
**Lame delegation:**
  ...
  not set up as a primary or secondary for the zone)." (Quoted from [RFC1912](#RFC1912))
  ...
  even run name servers."  (Quoted from [RFC1713](#RFC1713))
...
**Glue records:**
  ...
  the address we wish to learn."  (Quoted from [RFC1034](#RFC1034))
  ...
  that might appear."  (Quoted from [RFC2181](#RFC2181))
  Although glue is sometimes used today
  with this wider definition in mind, the context surrounding the definition in [RFC2181](#RFC2181) 
...
**Delegation-centric zone:**
  ...
  The term is used in [RFC4956](#RFC4956) and [RFC5155](#RFC5155), but it is not defined in either document.
...
**Occluded name:**
  ...
  'occluded'."  (Quoted from [RFC5936](#RFC5936))
...
**Fast flux DNS:**
  ...
  (Quoted from [RFC6561](#RFC6561), with a typo corrected)
...
**Service name:**
  ...
  records." (Quoted from [RFC6335](#RFC6335))
...
**Wildcard:**
  [RFC1034](#RFC1034) defined "wildcard", but in a way that turned out to be
  confusing to implementers.
  For an extended discussion of wildcards, including clearer definitions, see [RFC4592](#RFC4592).
  ...
  (Quoted from [RFC1034](#RFC1034))

**Asterisk label:**
  "The first octet is the normal label type and length for a 1-octet-long
  label, and the second octet is the ASCII representation [RFC20](#RFC0020)
  for the '*' character.
  A descriptive name of a label equaling that value is an 'asterisk
  label'." (Quoted from [RFC4592](#RFC4592))
...
**Wildcard domain name:**
  ...
  (Quoted from [RFC4592](#RFC4592))  The second octet in this label is the ASCII representation for the "*" character.

**Closest encloser:**
  "The longest existing ancestor of a name."
  (Quoted from [RFC5155](#RFC5155))
...
**Closest provable encloser:**
  ...
  (Quoted from [RFC5155](#RFC5155))
  See [Section 10](#dnssec-general) for more on "opt-out".
...
**EPP:**
  ...
  of registration information between registries and registrars. EPP is defined in [RFC5730](#RFC5730).

**WHOIS:**
  A protocol specified in [RFC3912](#RFC3912), often used for querying registry databases.
...
**RDAP:**
  ...
  [RFC7480](#RFC7480), [RFC7481](#RFC7481), [RFC7485](#RFC7485), [RFC9082](#RFC9082), [RFC9083](#RFC9083), and
  [RFC9224](#RFC9224).
...
**Public suffix:**
  "A domain that is controlled by a public registry." (Quoted from [RFC6265](#RFC6265)) A common definition for this term is a domain under which subdomains can be registered by third parties and on which HTTP cookies
  (which are described in detail in [RFC6265](#RFC6265)) should not be set.
  ...
  maintained by Mozilla [https://publicsuffix.org/](https://publicsuffix.org/).
...
**Subordinate and Superordinate:**
  These terms are introduced in [RFC5731](#RFC5731) for use in the registration model, but not defined there.
...
<a name="dnssec-general"></a>
# General DNSSEC

Most DNSSEC terms are defined in [RFC4033](#RFC4033),
[RFC4034](#RFC4034), [RFC4035](#RFC4035), and [RFC5155](#RFC5155). The
...
**DNSSEC-aware and DNSSEC-unaware:**
  ...
  However, [RFC4033](#RFC4033) defines many types of resolvers and
...
**Signed zone:**
  ...
  It should also be noted that, since the publication of [RFC6840](#RFC6840), NSEC records are no
  longer required for signed zones: a signed zone might include NSEC3 records instead.
  [RFC7129](#RFC7129) provides additional background commentary and some context for the NSEC and
  NSEC3 mechanisms used by DNSSEC to provide authenticated denial-of-existence responses.
  NSEC and NSEC3 are described below.

**Online signing:**
  [RFC4470](#RFC4470) defines "on-line signing" (note the hyphen) as
...
**Unsigned zone:**
  [RFC4033](#RFC4033) defines this as "a zone that is not signed". [RFC4035](#RFC4035) defines this as a "zone that does not include these records [properly constructed DNSKEY,
  Resource Record Signature (RRSIG), Next Secure (NSEC), and (optionally) DS records] according to the
  rules in this section..."  There is an important note at the end of [RFC4035](#RFC4035) that defines an
...
**NSEC:**
  ...
  (Quoted from [RFC4033](#RFC4033)) In short, an NSEC record provides authenticated denial of
  existence.
  ...
  of RR types present at the NSEC RR's owner name." (Quoted from [RFC4034](#RFC4034))
...
**NSEC3:**
  ...
  NSEC3 and NSEC3PARAM resource records are defined in [RFC5155](#RFC5155).

  Note that [RFC6840](#RFC6840) says that [RFC5155](#RFC5155) "is now considered part of the DNS Security Document Family
  as described by [RFC4033](#RFC4033)". This means that some of the definitions from earlier RFCs that
...
**Opt-out:**
  ...
  insecure delegations. (Adapted from [RFC7129](#RFC7129))

**Insecure delegation:**
  "A signed name containing a delegation (NS RRset), but lacking a DS RRset,
  signifying a delegation to an unsigned subzone." (Quoted from [RFC4956](#RFC4956))

**Zone enumeration:**
  "The practice of discovering the full content of a zone via successive queries."
  (Quoted from [RFC5155](#RFC5155)) This is also sometimes called "zone walking".
...
DNS RRsets in a response.
  ...
  [RFC4033](#RFC4033), says that a "Validating Security-Aware Stub
  ...
  and third definitions above.  The process of validating an RRSIG resource record is described in [RFC4035](#RFC4035).

  [RFC5155](#RFC5155) refers to validating responses throughout the document in the
  ...
  [RFC4033](#RFC4033), describes the chain linking trust anchor to DNS data as the "authentication chain".  A
  response is considered to be authentic if "all RRsets in the Answer and 
  Authority sections
  of the response [are considered] to be authentic" (Quoted from [RFC4035](#RFC4035))  DNS data or
  responses deemed to be authentic or validated have a security status of "secure" ([RFC4035](#RFC4035); [RFC4033](#RFC4033)).  "Authenticating
  both DNS keys and data is a matter of local policy, which may extend or even override the
  [DNSSEC] protocol extensions..." (Quoted from [RFC4033](#RFC4033))
...
**Validating resolver:**
  A security-aware recursive name server, security-aware resolver, or
    ...
    sometimes ambiguous and needs to be evaluated in context (see [Section 6](#dns-servers-and-clients)), "validating resolver" is a
...
**Zone signing key (ZSK):**
  ...
  require signatures, other than the apex DNSKEY RRset." (Quoted from [RFC6781](#RFC6781))
...
**Combined signing key (CSK):**
  ...
  Scheme." (Quoted from [RFC6781](#RFC6781)) This is sometimes called a "combined
...
**Secure Entry Point (SEP):**
  ...
  (Quoted from [RFC6781](#RFC6781))  Note that the
  ...
  The original definition of SEPs was in [RFC3757](#RFC3757). That definition
  clearly indicated that the SEP was a key, not just a bit in the key. The
  abstract of [RFC3757](#RFC3757) says:
  ...
  That definition of the SEP as a key was made obsolete by [RFC4034](#RFC4034),
  and the definition from [RFC6781](#RFC6781) is consistent with [RFC4034](#RFC4034).
...
**Trust anchor:**
  ...
  trusted means outside the DNS protocol." (Quoted from [RFC4033](#RFC4033))

**DNSSEC Policy (DP):**
  A statement that "sets forth the security requirements and
  standards to be implemented for a DNSSEC-signed zone." (Quoted from [RFC6841](#RFC6841))
...
**DNSSEC Practice Statement (DPS):**
  ...
  controls at a high level." (Quoted from [RFC6841](#RFC6841))
...
[RFC4033](#RFC4033) and [RFC4035](#RFC4035), although the definitions in the two documents differ a bit.  This document makes no effort to reconcile the definitions in the two documents and takes no
position as to whether they need to be reconciled.

[RFC4033](#RFC4033) says:

[RFC4035](#RFC4035) says:

<a name="securitycons"></a>
...
<a name="IANA_RootFiles"></a> **[IANA_RootFiles]** "Root Files", <https://www.iana.org/domains/root/files>

<a name="RFC0882"></a> **[RFC0882]** P. Mockapetris, "Domain names: Concepts and facilities", RFC 882, DOI 10.17487/RFC0882, November 1983. <https://www.rfc-editor.org/info/rfc882> [Local MD](rfc0882.md)

<a name="RFC1034"></a> **[RFC1034]** P. Mockapetris, "Domain names - concepts and facilities", STD 13, RFC 1034, DOI 10.17487/RFC1034, November 1987. <https://www.rfc-editor.org/info/rfc1034> [Local MD](rfc1034.md)

<a name="RFC1035"></a> **[RFC1035]** P. Mockapetris, "Domain names - implementation and specification", STD 13, RFC 1035, DOI 10.17487/RFC1035, November 1987. <https://www.rfc-editor.org/info/rfc1035> [Local MD](rfc1035.md)

<a name="RFC1123"></a> **[RFC1123]** R. Braden, "Requirements for Internet Hosts - Application and Support", STD 3, RFC 1123, DOI 10.17487/RFC1123, October 1989. <https://www.rfc-editor.org/info/rfc1123> [Local MD](rfc1123.md)

<a name="RFC1912"></a> **[RFC1912]** D. Barr, "Common DNS Operational and Configuration Errors", RFC 1912, DOI 10.17487/RFC1912, February 1996. <https://www.rfc-editor.org/info/rfc1912> [Local MD](rfc1912.md)

<a name="RFC1996"></a> **[RFC1996]** P. Vixie, "A Mechanism for Prompt Notification of Zone Changes (DNS NOTIFY)", RFC 1996, DOI 10.17487/RFC1996, August 1996. <https://www.rfc-editor.org/info/rfc1996> [Local MD](rfc1996.md)

<a name="RFC2136"></a> **[RFC2136]** P. Vixie, S. Thomson, Y. Rekhter, J. Bound, "Dynamic Updates in the Domain Name System (DNS UPDATE)", RFC 2136, DOI 10.17487/RFC2136, April 1997. <https://www.rfc-editor.org/info/rfc2136> [Local MD](rfc2136.md)

<a name="RFC2181"></a> **[RFC2181]** R. Elz, R. Bush, "Clarifications to the DNS Specification", RFC 2181, DOI 10.17487/RFC2181, July 1997. <https://www.rfc-editor.org/info/rfc2181> [Local MD](rfc2181.md)

<a name="RFC2182"></a> **[RFC2182]** R. Elz, R. Bush, S. Bradner, M. Patton, "Selection and Operation of Secondary DNS Servers", BCP 16, RFC 2182, DOI 10.17487/RFC2182, July 1997. <https://www.rfc-editor.org/info/rfc2182> [Local MD](rfc2182.md)

<a name="RFC2308"></a> **[RFC2308]** M. Andrews, "Negative Caching of DNS Queries (DNS NCACHE)", RFC 2308, DOI 10.17487/RFC2308, March 1998. <https://www.rfc-editor.org/info/rfc2308> [Local MD](rfc2308.md)

<a name="RFC4033"></a> **[RFC4033]** R. Arends, R. Austein, M. Larson, D. Massey, S. Rose, "DNS Security Introduction and Requirements", RFC 4033, DOI 10.17487/RFC4033, March 2005. <https://www.rfc-editor.org/info/rfc4033> [Local MD](rfc4033.md)

<a name="RFC4034"></a> **[RFC4034]** R. Arends, R. Austein, M. Larson, D. Massey, S. Rose, "Resource Records for the DNS Security Extensions", RFC 4034, DOI 10.17487/RFC4034, March 2005. <https://www.rfc-editor.org/info/rfc4034> [Local MD](rfc4034.md)

<a name="RFC4035"></a> **[RFC4035]** R. Arends, R. Austein, M. Larson, D. Massey, S. Rose, "Protocol Modifications for the DNS Security Extensions", RFC 4035, DOI 10.17487/RFC4035, March 2005. <https://www.rfc-editor.org/info/rfc4035> [Local MD](rfc4035.md)

<a name="RFC4592"></a> **[RFC4592]** E. Lewis, "The Role of Wildcards in the Domain Name System", RFC 4592, DOI 10.17487/RFC4592, July 2006. <https://www.rfc-editor.org/info/rfc4592> [Local MD](rfc4592.md)

<a name="RFC5155"></a> **[RFC5155]** B. Laurie, G. Sisson, R. Arends, D. Blacka, "DNS Security (DNSSEC) Hashed Authenticated Denial of Existence", RFC 5155, DOI 10.17487/RFC5155, March 2008. <https://www.rfc-editor.org/info/rfc5155> [Local MD](rfc5155.md)

<a name="RFC5358"></a> **[RFC5358]** J. Damas, F. Neves, "Preventing Use of Recursive Nameservers in Reflector Attacks", BCP 140, RFC 5358, DOI 10.17487/RFC5358, October 2008. <https://www.rfc-editor.org/info/rfc5358> [Local MD](rfc5358.md)

<a name="RFC5730"></a> **[RFC5730]** S. Hollenbeck, "Extensible Provisioning Protocol (EPP)", STD 69, RFC 5730, DOI 10.17487/RFC5730, August 2009. <https://www.rfc-editor.org/info/rfc5730> [Local MD](rfc5730.md)

<a name="RFC5731"></a> **[RFC5731]** S. Hollenbeck, "Extensible Provisioning Protocol (EPP) Domain Name Mapping", STD 69, RFC 5731, DOI 10.17487/RFC5731, August 2009. <https://www.rfc-editor.org/info/rfc5731> [Local MD](rfc5731.md)

<a name="RFC5855"></a> **[RFC5855]** J. Abley, T. Manderson, "Nameservers for IPv4 and IPv6 Reverse Zones", BCP 155, RFC 5855, DOI 10.17487/RFC5855, May 2010. <https://www.rfc-editor.org/info/rfc5855> [Local MD](rfc5855.md)

<a name="RFC5936"></a> **[RFC5936]** E. Lewis, A. Hoenes, "DNS Zone Transfer Protocol (AXFR)", RFC 5936, DOI 10.17487/RFC5936, June 2010. <https://www.rfc-editor.org/info/rfc5936> [Local MD](rfc5936.md)

<a name="RFC6561"></a> **[RFC6561]** J. Livingood, N. Mody, M. O'Reirdan, "Recommendations for the Remediation of Bots in ISP Networks", RFC 6561, DOI 10.17487/RFC6561, March 2012. <https://www.rfc-editor.org/info/rfc6561> [Local MD](rfc6561.md)

<a name="RFC6781"></a> **[RFC6781]** O. Kolkman, W. Mekking, R. Gieben, "DNSSEC Operational Practices, Version 2", RFC 6781, DOI 10.17487/RFC6781, December 2012. <https://www.rfc-editor.org/info/rfc6781> [Local MD](rfc6781.md)

<a name="RFC6840"></a> **[RFC6840]** S. Weiler, D. Blacka, "Clarifications and Implementation Notes for DNS Security (DNSSEC)", RFC 6840, DOI 10.17487/RFC6840, February 2013. <https://www.rfc-editor.org/info/rfc6840> [Local MD](rfc6840.md)

<a name="RFC6841"></a> **[RFC6841]** F. Ljunggren, AM. Eklund Lowinder, T. Okubo, "A Framework for DNSSEC Policies and DNSSEC Practice Statements", RFC 6841, DOI 10.17487/RFC6841, January 2013. <https://www.rfc-editor.org/info/rfc6841> [Local MD](rfc6841.md)

<a name="RFC6891"></a> **[RFC6891]** J. Damas, M. Graff, P. Vixie, "Extension Mechanisms for DNS (EDNS(0))", STD 75, RFC 6891, DOI 10.17487/RFC6891, April 2013. <https://www.rfc-editor.org/info/rfc6891> [Local MD](rfc6891.md)

<a name="RFC7344"></a> **[RFC7344]** W. Kumari, O. Gudmundsson, G. Barwood, "Automating DNSSEC Delegation Trust Maintenance", RFC 7344, DOI 10.17487/RFC7344, September 2014. <https://www.rfc-editor.org/info/rfc7344> [Local MD](rfc7344.md)

<a name="RFC7719"></a> **[RFC7719]** P. Hoffman, A. Sullivan, K. Fujiwara, "DNS Terminology", RFC 7719, DOI 10.17487/RFC7719, December 2015. <https://www.rfc-editor.org/info/rfc7719> [Local MD](rfc7719.md)

<a name="RFC8310"></a> **[RFC8310]** S. Dickinson, D. Gillmor, T. Reddy, "Usage Profiles for DNS over TLS and DNS over DTLS", RFC 8310, DOI 10.17487/RFC8310, March 2018. <https://www.rfc-editor.org/info/rfc8310> [Local MD](rfc8310.md)

<a name="RFC8499"></a> **[RFC8499]** P. Hoffman, A. Sullivan, K. Fujiwara, "DNS Terminology", BCP 219, RFC 8499, DOI 10.17487/RFC8499, January 2019. <https://www.rfc-editor.org/info/rfc8499> [Local MD](rfc8499.md)

<a name="RFC9250"></a> **[RFC9250]** C. Huitema, S. Dickinson, A. Mankin, "DNS over Dedicated QUIC Connections", RFC 9250, DOI 10.17487/RFC9250, May 2022. <https://www.rfc-editor.org/info/rfc9250> [Local MD](rfc9250.md)

<a name="RFC9471"></a> **[RFC9471]** M. Andrews, S. Huque, P. Wouters, D. Wessels, "DNS Glue Requirements in Referral Responses", RFC 9471, DOI 10.17487/RFC9471, September 2023. <https://www.rfc-editor.org/info/rfc9471> [Local MD](rfc9471.md)

## Informative References

<a name="IANA_Resource_Registry"></a> **[IANA_Resource_Registry]** "Resource Record (RR) TYPEs", <https://www.iana.org/assignments/dns-parameters/>

<a name="RFC0020"></a> **[RFC20]** V.G. Cerf, "ASCII format for network interchange", STD 80, RFC 20, DOI 10.17487/RFC0020, October 1969. <https://www.rfc-editor.org/info/rfc20> [Local MD](rfc0020.md)

<a name="RFC0819"></a> **[RFC819]** Z. Su, J. Postel, "The Domain Naming Convention for Internet User Applications", RFC 819, DOI 10.17487/RFC0819, August 1982. <https://www.rfc-editor.org/info/rfc819> [Local MD](rfc0819.md)

<a name="RFC0952"></a> **[RFC952]** K. Harrenstien, M.K. Stahl, E.J. Feinler, "DoD Internet host table specification", RFC 952, DOI 10.17487/RFC0952, October 1985. <https://www.rfc-editor.org/info/rfc952> [Local MD](rfc0952.md)

<a name="RFC1713"></a> **[RFC1713]** A. Romao, "Tools for DNS debugging", FYI 27, RFC 1713, DOI 10.17487/RFC1713, November 1994. <https://www.rfc-editor.org/info/rfc1713> [Local MD](rfc1713.md)

<a name="RFC1995"></a> **[RFC1995]** M. Ohta, "Incremental Zone Transfer in DNS", RFC 1995, DOI 10.17487/RFC1995, August 1996. <https://www.rfc-editor.org/info/rfc1995> [Local MD](rfc1995.md)

<a name="RFC2775"></a> **[RFC2775]** B. Carpenter, "Internet Transparency", RFC 2775, DOI 10.17487/RFC2775, February 2000. <https://www.rfc-editor.org/info/rfc2775> [Local MD](rfc2775.md)

<a name="RFC3172"></a> **[RFC3172]** G. Huston, "Management Guidelines & Operational Requirements for the Address and Routing Parameter Area Domain ("arpa")", BCP 52, RFC 3172, DOI 10.17487/RFC3172, September 2001. <https://www.rfc-editor.org/info/rfc3172> [Local MD](rfc3172.md)

<a name="RFC3425"></a> **[RFC3425]** D. Lawrence, "Obsoleting IQUERY", RFC 3425, DOI 10.17487/RFC3425, November 2002. <https://www.rfc-editor.org/info/rfc3425> [Local MD](rfc3425.md)

<a name="RFC3493"></a> **[RFC3493]** R. Gilligan, S. Thomson, J. Bound, J. McCann, W. Stevens, "Basic Socket Interface Extensions for IPv6", RFC 3493, DOI 10.17487/RFC3493, February 2003. <https://www.rfc-editor.org/info/rfc3493> [Local MD](rfc3493.md)

<a name="RFC3757"></a> **[RFC3757]** O. Kolkman, J. Schlyter, E. Lewis, "Domain Name System KEY (DNSKEY) Resource Record (RR) Secure Entry Point (SEP) Flag", RFC 3757, DOI 10.17487/RFC3757, April 2004. <https://www.rfc-editor.org/info/rfc3757> [Local MD](rfc3757.md)

<a name="RFC3912"></a> **[RFC3912]** L. Daigle, "WHOIS Protocol Specification", RFC 3912, DOI 10.17487/RFC3912, September 2004. <https://www.rfc-editor.org/info/rfc3912> [Local MD](rfc3912.md)

<a name="RFC4470"></a> **[RFC4470]** S. Weiler, J. Ihren, "Minimally Covering NSEC Records and DNSSEC On-line Signing", RFC 4470, DOI 10.17487/RFC4470, April 2006. <https://www.rfc-editor.org/info/rfc4470> [Local MD](rfc4470.md)

<a name="RFC4641"></a> **[RFC4641]** O. Kolkman, R. Gieben, "DNSSEC Operational Practices", RFC 4641, DOI 10.17487/RFC4641, September 2006. <https://www.rfc-editor.org/info/rfc4641> [Local MD](rfc4641.md)

<a name="RFC4697"></a> **[RFC4697]** M. Larson, P. Barber, "Observed DNS Resolution Misbehavior", BCP 123, RFC 4697, DOI 10.17487/RFC4697, October 2006. <https://www.rfc-editor.org/info/rfc4697> [Local MD](rfc4697.md)
...
<a name="RFC4956"></a> **[RFC4956]** R. Arends, M. Kosters, D. Blacka, "DNS Security (DNSSEC) Opt-In", RFC 4956, DOI 10.17487/RFC4956, July 2007. <https://www.rfc-editor.org/info/rfc4956> [Local MD](rfc4956.md)

<a name="RFC5625"></a> **[RFC5625]** R. Bellis, "DNS Proxy Implementation Guidelines", BCP 152, RFC 5625, DOI 10.17487/RFC5625, August 2009. <https://www.rfc-editor.org/info/rfc5625> [Local MD](rfc5625.md)
...
<a name="RFC5891"></a> **[RFC5891]** J. Klensin, "Internationalized Domain Names in Applications (IDNA): Protocol", RFC 5891, DOI 10.17487/RFC5891, August 2010. <https://www.rfc-editor.org/info/rfc5891> [Local MD](rfc5891.md)

<a name="RFC5892"></a> **[RFC5892]** P. Faltstrom, "The Unicode Code Points and Internationalized Domain Names for Applications (IDNA)", RFC 5892, DOI 10.17487/RFC5892, August 2010. <https://www.rfc-editor.org/info/rfc5892> [Local MD](rfc5892.md)

<a name="RFC5893"></a> **[RFC5893]** H. Alvestrand, C. Karp, "Right-to-Left Scripts for Internationalized Domain Names for Applications (IDNA)", RFC 5893, DOI 10.17487/RFC5893, August 2010. <https://www.rfc-editor.org/info/rfc5893> [Local MD](rfc5893.md)

<a name="RFC5894"></a> **[RFC5894]** J. Klensin, "Internationalized Domain Names for Applications (IDNA): Background, Explanation, and Rationale", RFC 5894, DOI 10.17487/RFC5894, August 2010. <https://www.rfc-editor.org/info/rfc5894> [Local MD](rfc5894.md)

<a name="RFC6055"></a> **[RFC6055]** D. Thaler, J. Klensin, S. Cheshire, "IAB Thoughts on Encodings for Internationalized Domain Names", RFC 6055, DOI 10.17487/RFC6055, February 2011. <https://www.rfc-editor.org/info/rfc6055> [Local MD](rfc6055.md)

<a name="RFC6265"></a> **[RFC6265]** A. Barth, "HTTP State Management Mechanism", RFC 6265, DOI 10.17487/RFC6265, April 2011. <https://www.rfc-editor.org/info/rfc6265> [Local MD](rfc6265.md)

<a name="RFC6303"></a> **[RFC6303]** M. Andrews, "Locally Served DNS Zones", BCP 163, RFC 6303, DOI 10.17487/RFC6303, July 2011. <https://www.rfc-editor.org/info/rfc6303> [Local MD](rfc6303.md)
...
<a name="RFC6365"></a> **[RFC6365]** P. Hoffman, J. Klensin, "Terminology Used in Internationalization in the IETF", BCP 166, RFC 6365, DOI 10.17487/RFC6365, September 2011. <https://www.rfc-editor.org/info/rfc6365> [Local MD](rfc6365.md)

<a name="RFC6672"></a> **[RFC6672]** S. Rose, W. Wijngaards, "DNAME Redirection in the DNS", RFC 6672, DOI 10.17487/RFC6672, June 2012. <https://www.rfc-editor.org/info/rfc6672> [Local MD](rfc6672.md)
...
<a name="RFC7129"></a> **[RFC7129]** R. Gieben, W. Mekking, "Authenticated Denial of Existence in the DNS", RFC 7129, DOI 10.17487/RFC7129, February 2014. <https://www.rfc-editor.org/info/rfc7129> [Local MD](rfc7129.md)

<a name="RFC7480"></a> **[RFC7480]** A. Newton, B. Ellacott, N. Kong, "HTTP Usage in the Registration Data Access Protocol (RDAP)", STD 95, RFC 7480, DOI 10.17487/RFC7480, March 2015. <https://www.rfc-editor.org/info/rfc7480> [Local MD](rfc7480.md)

<a name="RFC7481"></a> **[RFC7481]** S. Hollenbeck, N. Kong, "Security Services for the Registration Data Access Protocol (RDAP)", STD 95, RFC 7481, DOI 10.17487/RFC7481, March 2015. <https://www.rfc-editor.org/info/rfc7481> [Local MD](rfc7481.md)

<a name="RFC9082"></a> **[RFC9082]** S. Hollenbeck, A. Newton, "Registration Data Access Protocol (RDAP) Query Format", STD 95, RFC 9082, DOI 10.17487/RFC9082, June 2021. <https://www.rfc-editor.org/info/rfc9082> [Local MD](rfc9082.md)

<a name="RFC9083"></a> **[RFC9083]** S. Hollenbeck, A. Newton, "JSON Responses for the Registration Data Access Protocol (RDAP)", STD 95, RFC 9083, DOI 10.17487/RFC9083, June 2021. <https://www.rfc-editor.org/info/rfc9083> [Local MD](rfc9083.md)

<a name="RFC9224"></a> **[RFC9224]** M. Blanchet, "Finding the Authoritative Registration Data Access Protocol (RDAP) Service", STD 95, RFC 9224, DOI 10.17487/RFC9224, March 2022. <https://www.rfc-editor.org/info/rfc9224> [Local MD](rfc9224.md)

<a name="RFC7485"></a> **[RFC7485]** L. Zhou, N. Kong, S. Shen, S. Sheng, A. Servin, "Inventory and Analysis of WHOIS Registration Objects", RFC 7485, DOI 10.17487/RFC7485, March 2015. <https://www.rfc-editor.org/info/rfc7485> [Local MD](rfc7485.md)

<a name="RFC7793"></a> **[RFC7793]** M. Andrews, "Adding 100.64.0.0/10 Prefixes to the IPv4 Locally-Served DNS Zones Registry", BCP 163, RFC 7793, DOI 10.17487/RFC7793, May 2016. <https://www.rfc-editor.org/info/rfc7793> [Local MD](rfc7793.md)

<a name="RFC7858"></a> **[RFC7858]** Z. Hu, L. Zhu, J. Heidemann, A. Mankin, D. Wessels, P. Hoffman, "Specification for DNS over Transport Layer Security (TLS)", RFC 7858, DOI 10.17487/RFC7858, May 2016. <https://www.rfc-editor.org/info/rfc7858> [Local MD](rfc7858.md)

<a name="RFC8094"></a> **[RFC8094]** T. Reddy, D. Wing, P. Patil, "DNS over Datagram Transport Layer Security (DTLS)", RFC 8094, DOI 10.17487/RFC8094, February 2017. <https://www.rfc-editor.org/info/rfc8094> [Local MD](rfc8094.md)

<a name="RFC8109"></a> **[RFC8109]** P. Koch, M. Larson, P. Hoffman, "Initializing a DNS Resolver with Priming Queries", BCP 209, RFC 8109, DOI 10.17487/RFC8109, March 2017. <https://www.rfc-editor.org/info/rfc8109> [Local MD](rfc8109.md)

<a name="RFC8484"></a> **[RFC8484]** P. Hoffman, P. McManus, "DNS Queries over HTTPS (DoH)", RFC 8484, DOI 10.17487/RFC8484, October 2018. <https://www.rfc-editor.org/info/rfc8484> [Local MD](rfc8484.md)

<a name="RFC9103"></a> **[RFC9103]** W. Toorop, S. Dickinson, S. Sahib, P. Aras, A. Mankin, "DNS Zone Transfer over TLS", RFC 9103, DOI 10.17487/RFC9103, August 2021. <https://www.rfc-editor.org/info/rfc9103> [Local MD](rfc9103.md)

<a name="RSSAC026"></a> **[RSSAC026]** "RSSAC0226 RSSAC Lexicon", 2017. <https://www.icann.org/en/system/files/files/rssac-026-14mar17-en.pdf>

<a name="updates-list"></a>
# Definitions Updated by This Document

The following definitions from RFCs are updated by this document:

- Forwarder in [RFC2308](#RFC2308)
- QNAME in [RFC2308](#RFC2308)
- Secure Entry Point (SEP) in [RFC3757](#RFC3757);
note, however, that this RFC is already obsolete
(see [RFC4033](#RFC4033), [RFC4034](#RFC4034), [RFC4035](#RFC4035)).
...
- "Alias" in [Section 2](#names)
- "Apex" in [Section 7](#zones)
- "arpa" in [Section 7](#zones)
- "Authoritative DoT (ADot)" in [Section 6](#dns-servers-and-clients)
- "Bailiwick" in [Section 7](#zones)
- "Class independent" in [Section 5](#rrs)
- "Classic DNS" in [Section 6](#dns-servers-and-clients)
- "Delegation-centric zone" in [Section 7](#zones)
- "Delegation" in [Section 7](#zones)
- "DNS operator" in [Section 9](#reg-model)
- "DNSSEC-aware" in [Section 10](#dnssec-general)
- "DNSSEC-unaware" in [Section 10](#dnssec-general)
- "Forwarding" in [Section 6](#dns-servers-and-clients)
- "Full resolver" in [Section 6](#dns-servers-and-clients)
- "Fully Qualified Domain Name" in [Section 2](#names)
- "Global DNS" in [Section 2](#names)
- "Hardware Security Module (HSM)" in [Section 10](#dnssec-general)
- "Host name" in [Section 2](#names)
- "IDN" in [Section 2](#names)
- "In-domain" in [Section 7](#zones)
- "Iterative resolution" in [Section 6](#dns-servers-and-clients)
- "Label" in [Section 2](#names)
- "Locally served DNS zone" in [Section 2](#names)
- "Naming system" in [Section 2](#names)
- "Negative response" in [Section 3](#dns-response-codes)
- "Non-recursive query" in [Section 6](#dns-servers-and-clients)
- "Open resolver" in [Section 6](#dns-servers-and-clients)
- "Passive DNS" in [Section 6](#dns-servers-and-clients)
- "Policy-implementing resolver" in [Section 6](#dns-servers-and-clients)
- "Presentation format" in [Section 5](#rrs)
- "Priming" in [Section 6](#dns-servers-and-clients)
- "Private DNS" in [Section 2](#names)
- "Recursive DoT (RDot)" in [Section 6](#dns-servers-and-clients)
- "Recursive resolver" in [Section 6](#dns-servers-and-clients)
- "Referrals" in [Section 4](#dns-transactions)
- "Registrant" in [Section 9](#reg-model)
- "Registrar" in [Section 9](#reg-model)
- "Registry" in [Section 9](#reg-model)
- "Root zone" in [Section 7](#zones)
- "Secure Entry Point (SEP)" in [Section 10](#dnssec-general)
- "Sibling domain" in [Section 7](#zones)
- "Signing software" in [Section 10](#dnssec-general)
- "Split DNS" in [Section 6](#dns-servers-and-clients)
- "Stub resolver" in [Section 6](#dns-servers-and-clients)
- "Subordinate" in [Section 8](#wildcards)
- "Superordinate" in [Section 8](#wildcards)
- "TLD" in [Section 2](#names)
- "Validating resolver" in [Section 10](#dnssec-general)
- "Validation" in [Section 10](#dnssec-general)
- "View" in [Section 6](#dns-servers-and-clients)
- "Zone transfer" in [Section 6](#dns-servers-and-clients)

<a name="acknowledgements"></a>
# Acknowledgements

[RFC8499](#RFC8499) and its predecessor, [RFC7719](#RFC7719), were co-authored by Andrew Sullivan.
The current document, which is a small update to [RFC8499](#RFC8499), has just two authors.
Andrew's work on the earlier documents is greatly appreciated.

Numerous people made significant contributions to [RFC8499](#RFC8499) and [RFC7719](#RFC7719).
