...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-idr-tunnel-encaps-22
...
- [prev](https://datatracker.ietf.org/doc/draft-ietf-idr-tunnel-encaps-22)
- [alternate](https://dx.doi.org/10.17487/rfc9012)
...
- [1. Introduction](#section-1)
  - [1.1. Brief Summary of RFC 5512](#summary)
  - [1.2. Deficiencies in RFC 5512](#deficiencies)
  - [1.3. Use Case for the Tunnel Encapsulation Attribute](#use-case)
  - [1.4. Brief Summary of Changes from RFC 5512](#section-1.4)
  - [1.5. Update to RFC 5640](#section-1.5)
  - [1.6. Effects of Obsoleting RFC 5566](#section-1.6)
- [2. The Tunnel Encapsulation Attribute](#encaps-attr)
- [3. Tunnel Encapsulation Attribute Sub-TLVs](#section-3)
  - [3.1. The Tunnel Egress Endpoint Sub-TLV (Type Code 6)](#tunnel-egress-endpoint)
    - [3.1.1. Validating the Address Subfield](#address-validation)
  - [3.2. Encapsulation Sub-TLVs for Particular Tunnel Types (Type Code 1)](#section-3.2)
    - [3.2.1. VXLAN (Tunnel Type 8)](#section-3.2.1)
    - [3.2.2. NVGRE (Tunnel Type 9)](#section-3.2.2)
    - [3.2.3. L2TPv3 (Tunnel Type 1)](#section-3.2.3)
    - [3.2.4. GRE (Tunnel Type 2)](#gre)
    - [3.2.5. MPLS-in-GRE (Tunnel Type 11)](#section-3.2.5)
  - [3.3. Outer Encapsulation Sub-TLVs](#section-3.3)
    - [3.3.1. DS Field (Type Code 7)](#section-3.3.1)
    - [3.3.2. UDP Destination Port (Type Code 8)](#section-3.3.2)
  - [3.4. Sub-TLVs for Aiding Tunnel Selection](#section-3.4)
    - [3.4.1. Protocol Type Sub-TLV (Type Code 2)](#protocol-type)
    - [3.4.2. Color Sub-TLV (Type Code 4)](#color)
  - [3.5. Embedded Label Handling Sub-TLV (Type Code 9)](#section-3.5)
  - [3.6. MPLS Label Stack Sub-TLV (Type Code 10)](#label-stack)
  - [3.7. Prefix-SID Sub-TLV (Type Code 11)](#prefix-sid)
- [4. Extended Communities Related to the Tunnel Encapsulation Attribute](#section-4)
  - [4.1. Encapsulation Extended Community](#encapsulation-extcomm)
  - [4.2. Router's MAC Extended Community](#section-4.2)
  - [4.3. Color Extended Community](#color-extcomm)
- [5. Special Considerations for IP-in-IP Tunnels](#ipip-considerations)
- [6. Semantics and Usage of the Tunnel Encapsulation Attribute](#usage)
- [7. Routing Considerations](#section-7)
  - [7.1. Impact on the BGP Decision Process](#section-7.1)
  - [7.2. Looping, Mutual Recursion, Etc.](#section-7.2)
- [8. Recursive Next-Hop Resolution](#recursive-nh-resolution)
- [9. Use of Virtual Network Identifiers and Embedded Labels When Imposing a Tunnel Encapsulation](#use-of-vni)
  - [9.1. Tunnel Types without a Virtual Network Identifier Field](#section-9.1)
  - [9.2. Tunnel Types with a Virtual Network Identifier Field](#section-9.2)
    - [9.2.1. Unlabeled Address Families](#section-9.2.1)
    - [9.2.2. Labeled Address Families](#section-9.2.2)
- [10. Applicability Restrictions](#applicability)
- [11. Scoping](#scoping)
- [12. Operational Considerations](#section-12)
- [13. Validation and Error Handling](#validation-and-error)
- [14. IANA Considerations](#section-14)
  - [14.1. Obsoleting RFC 5512](#section-14.1)
  - [14.2. Obsoleting Code Points Assigned by RFC 5566](#obsoleting-5566-and-5640)
  - [14.3. Border Gateway Protocol (BGP) Tunnel Encapsulation Grouping](#section-14.3)
  - [14.4. BGP Tunnel Encapsulation Attribute Tunnel Types](#section-14.4)
  - [14.5. Subsequent Address Family Identifiers](#section-14.5)
  - [14.6. BGP Tunnel Encapsulation Attribute Sub-TLVs](#section-14.6)
  - [14.7. Flags Field of VXLAN Encapsulation Sub-TLV](#section-14.7)
  - [14.8. Flags Field of NVGRE Encapsulation Sub-TLV](#section-14.8)
  - [14.9. Embedded Label Handling Sub-TLV](#section-14.9)
  - [14.10. Color Extended Community Flags](#color-extcomm-flags)
...
- [Appendix A. Impact on RFC 8365](#impact-on-8365)
...
This document obsoletes [RFC5512](#RFC5512).  The deficiencies of [RFC5512](#RFC5512), and
   a summary of the changes made, are discussed in Sections [1.1](#summary)-[1.3](#use-case).
   The material from [RFC5512](#RFC5512) that is retained has been incorporated
   into this document.  Since [RFC5566](#RFC5566)
   relies on [RFC5512](#RFC5512), it is likewise obsoleted.
...
<a name="summary"></a>
## Brief Summary of RFC 5512

[RFC5512](#RFC5512) defines a BGP path attribute known as the Tunnel
...
[RFC5512](#RFC5512) only allows the Tunnel Encapsulation attribute to be
...
## Deficiencies in RFC 5512

While the ability to specify tunnel information in a BGP UPDATE is
   useful, the procedures of [RFC5512](#RFC5512) have certain limitations:
...
- There is no way to use the Tunnel Encapsulation attribute to
      specify the tunnel egress endpoint address of a given tunnel; [RFC5512](#RFC5512)
...
- If the respective best routes to two different address prefixes
      have the same next hop, [RFC5512](#RFC5512) does not provide a
...
- In the specification of the sub-TLVs in [RFC5512](#RFC5512), each sub-TLV has a
...
<a name="use-case"></a>
## Use Case for the Tunnel Encapsulation Attribute
...
Consider the case of a router R1 forwarding an IP packet P.  Let D be
	    ...
	    R2 may be part of a "BGP-free core", where there are no BGP-distributed routes at all in the core.  Or, as in [RFC5565](#RFC5565), D may be an
...
## Brief Summary of Changes from RFC 5512

This document addresses the deficiencies identified in [Section 1.2](#deficiencies) by:

- Deprecating the Encapsulation SAFI.
- Defining a new "Tunnel Egress Endpoint sub-TLV" ([Section 3.1](#tunnel-egress-endpoint)) that can be
...
One of the sub-TLVs defined in [RFC5512](#RFC5512) is the "Encapsulation sub-TLV".  For a given tunnel, the Encapsulation sub-TLV specifies some
   ...
   [RFC5512](#RFC5512): Virtual eXtensible Local Area Network (VXLAN) [RFC7348](#RFC7348), Network Virtualization Using Generic Routing Encapsulation (NVGRE)
   [RFC7637](#RFC7637), and MPLS in Generic Routing Encapsulation (MPLS-in-GRE) [RFC4023](#RFC4023).  MPLS-in-UDP [RFC7510](#RFC7510) is also
   supported, but an Encapsulation sub-TLV for it is not needed since there are
   no additional parameters to be signaled.

Some of the encapsulations mentioned in the previous paragraph need
   to be further encapsulated inside UDP and/or IP.  [RFC5512](#RFC5512) provides
...
[RFC5512](#RFC5512) defines an Encapsulation Extended
    ...
    Community can be used in a backwards-compatible fashion (see [Section 4.1](#encapsulation-extcomm)). It is
...
This document updates [RFC5640](#RFC5640) by indicating that the Load-Balancing Block
 	sub-TLV **MAY** be included in any Tunnel Encapsulation attribute where
  	load balancing is desired.

## Effects of Obsoleting RFC 5566
...
This specification obsoletes RFC 5566. This has the effect of,
	...
	In the "BGP Tunnel Encapsulation Attribute Tunnel Types" registry [IANA-BGP-TUNNEL-ENCAP](#IANA-BGP-TUNNEL-ENCAP), the following code points have been marked as deprecated:
	...
	In the "BGP Tunnel Encapsulation Attribute Sub-TLVs" registry [IANA-BGP-TUNNEL-ENCAP](#IANA-BGP-TUNNEL-ENCAP),
	"IPsec Tunnel Authenticator" (type code 3) has been marked as deprecated. 
	See [Section 14.2](#obsoleting-5566-and-5640).

<a name="encaps-attr"></a>
...
The Tunnel Encapsulation attribute is an optional transitive BGP path
   ...
   attribute in the "BGP Path Attributes" registry [IANA-BGP-PARAMS](#IANA-BGP-PARAMS).  The attribute is composed of a set of Type-Length-Value
   ...
   is structured as shown in [Figure 1](#ref-tunnel-tlv-value-field).
...
**Tunnel Type (2 octets):**
  Identifies a type of tunnel.  The field
        contains values from the IANA registry
  "BGP Tunnel Encapsulation Attribute Tunnel Types" [IANA-BGP-TUNNEL-ENCAP](#IANA-BGP-TUNNEL-ENCAP).
        See [Section 3.4.1](#protocol-type) for discussion of special treatment of tunnel types with names of the form "X-in-Y".

**Length (2 octets):**
  The total number of octets of the Value field.
...
Each sub-TLV consists of three fields: A 1-octet type, a 1-octet or
   ...
   of value.  A sub-TLV is structured as shown in [Figure 2](#ref-tunnel-encapsulation-sub-tlv-value-field).
...
```
**Sub-TLV Type (1 octet):**
  Each sub-TLV type defines a certain
        property about the Tunnel TLV that contains this sub-TLV.
        The field contains values from the IANA registry 
  "BGP Tunnel Encapsulation Attribute Sub-TLVs" [IANA-BGP-TUNNEL-ENCAP](#IANA-BGP-TUNNEL-ENCAP).

**Sub-TLV Length (1 or 2 octets):**
  The total number of octets of the
...
<a name="tunnel-egress-endpoint"></a>
## The Tunnel Egress Endpoint Sub-TLV (Type Code 6)
...
The Tunnel Egress Endpoint
   ...
   Value field contains three subfields:
...
"Address Family Numbers" registry [IANA-ADDRESS-FAM](#IANA-ADDRESS-FAM).  
...
There is one special case: the Tunnel Egress Endpoint sub-TLV **MAY** have a
	  ...
	  the Length field of Tunnel Egress Endpoint sub-TLV **MUST** contain the
	  value 6 (0x06).
...
When the Tunnel Encapsulation attribute is carried in an UPDATE message
          ...
          paragraph of [Section 6](#usage)), each TLV **MUST** have one, and only one, Tunnel Egress Endpoint sub-TLV. If a TLV does not have a Tunnel Egress Endpoint sub-TLV, that TLV should be treated as if it had a malformed Tunnel
          Egress Endpoint sub-TLV (see below).
...
In the context of this specification, if the Address Family subfield
   ...
   [Section 13](#validation-and-error)).
...
- It can be determined that the IP address in the sub-TLV's Address
      ...
      the route that contains the attribute.  [Section 3.1.1](#address-validation) 
      describes an optional procedure to make this determination.

Error handling is specified in [Section 13](#validation-and-error).
...
### Validating the Address Subfield
...
This section provides a procedure that **MAY** be applied to 
   ...
   it cannot guarantee it. See [Section 15](#security) for discussion of the limitations of this
   ...
   benefit beyond those discussed in [Section 11](#scoping).
...
The Route Origin Autonomous System Number (ASN) of a BGP route that
   ...
   specified in [RFC6811](#RFC6811). Call this value
   Route_AS.
...
In order to determine the Route Origin ASN of the address depicted in
   ...
   discussed in [RFC4271](#RFC4271). The relevant AS
   path to consider is the last one encountered while performing the
   recursive lookup; the procedures of [RFC6811](#RFC6811) are applied to
...
## Encapsulation Sub-TLVs for Particular Tunnel Types (Type Code 1)

This section defines Encapsulation sub-TLVs for the following
   tunnel types: VXLAN [RFC7348](#RFC7348), NVGRE
   [RFC7637](#RFC7637), MPLS-in-GRE [RFC4023](#RFC4023), L2TPv3
   [RFC3931](#RFC3931), and GRE [RFC2784](#RFC2784).

Rules for forming the encapsulation based on the information in a
   given TLV are given in Sections [6](#usage) and [9](#use-of-vni).

Recall that the tunnel type itself is identified by the Tunnel Type
   field in the attribute header ([Section 2](#encaps-attr)); the
...
This document defines an Encapsulation sub-TLV for [VXLAN](#RFC7348) tunnels.
   When the tunnel type is VXLAN, the length of the sub-TLV is 12 octets. The structure of the Value field in the Encapsulation sub-TLV is shown in [Figure 4](#ref-vxlan-encapsulation-sub-tlv).
...
**V:**
  This bit is set to 1 to indicate that a Virtual
        ...
        Please see [Section 9](#use-of-vni).
...
**R:**
  The remaining bits in the 8-bit Flags field are reserved for
        ...
        modification. Any receiving routers **MUST** ignore
        these bits upon receipt.

**VN-ID:**
  If the V bit is set to 1, the VN-ID field contains a 3-octet VN-ID value.  If the V bit is set to 0, the VN-ID field **MUST** be set
...
**Reserved:**
  **MUST** be set to zero on transmission and disregarded
        on receipt.
...
- [Section 9](#use-of-vni) describes how the VNI (VXLAN Network Identifier) field of the VXLAN encapsulation header is set.
...
Note that in order to send an IP packet or an MPLS packet through a
   ...
   [RFC7348](#RFC7348).  The VXLAN Encapsulation sub-TLV may contain information
   (for example, the MAC address) that is used to form this Ethernet header.

### NVGRE (Tunnel Type 9)

This document defines an Encapsulation sub-TLV for [NVGRE](#RFC7637) tunnels.
   When the tunnel type is NVGRE, the length of the sub-TLV is 12 octets. The structure of the Value field in the Encapsulation sub-TLV is shown in [Figure 5](#ref-nvgre-encapsulation-sub-tlv).
...
**V:**
  This bit is set to 1 to indicate that a VN-ID is
        ...
        the VN-ID field is disregarded. Please see [Section 9](#use-of-vni).

**M:**
  This bit is set to 1 to indicate that a MAC Address is
        present in the Encapsulation sub-TLV. If set to 0,
        the MAC Address field is disregarded.
...
**VN-ID:**
  If the V bit is set to 1, the VN-ID field contains a 3-octet VN-ID value, used to set the NVGRE Virtual Subnet Identifier (VSID; see [Section 9](#use-of-vni)).  If the V bit is set to 0, the VN-ID field **MUST** be set
        to zero on transmission and disregarded on receipt.
...
- [Section 9](#use-of-vni) describes how the VSID field of the
      NVGRE encapsulation header is set.

### L2TPv3 (Tunnel Type 1)

When the tunnel type of the TLV is [L2TPv3 over IP](#RFC3931), the length of the sub-TLV is 
   between 4 and 12
   octets, depending on the length of the cookie. 
   The structure of the Value field of the Encapsulation sub-TLV is shown in [Figure 6](#ref-l2tpv3-encapsulation-sub-tlv).
...
When the tunnel type of the TLV is [GRE](#RFC2784), the length of the sub-TLV is 4 octets. The structure of the Value field of the Encapsulation sub-TLV is shown in [Figure 7](#ref-gre-encapsulation-sub-tlv).
...
```
**GRE Key:**
  4-octet field [RFC2890](#RFC2890) that is generated by the
...
When the tunnel type is [MPLS-in-GRE](#RFC4023), the length of the sub-TLV is 4 octets. The structure of the Value field of the Encapsulation sub-TLV is shown in [Figure 8](#ref-mpls-in-gre-encapsulation-sub-tlv).
...
Note that the GRE tunnel type defined in [Section 3.2.4](#gre) can be used
   ...
   also includes a Protocol Type sub-TLV ([Section 3.4.1](#protocol-type)) specifying MPLS
...
Although the MPLS-in-GRE tunnel type is just a special case of the GRE
   ...
   of backwards compatibility with, for example, implementations of [RFC8365](#RFC8365).

## Outer Encapsulation Sub-TLVs
...
Most of the tunnel types that can be specified in the Tunnel
   ...
   [RFC2474](#RFC2474)).  Any one-octet value can be transported; the semantics
   of the DSCP (Differentiated Services Code Point) field is beyond the scope of this document.  The Value field is always a single octet.
...
### UDP Destination Port (Type Code 8)
...
Some of the tunnel types that can be specified in the Tunnel
   ...
   If the reserved value zero is received, the sub-TLV **MUST** be treated 
   as malformed, according to the rules of [Section 13](#validation-and-error).
...
```
## Sub-TLVs for Aiding Tunnel Selection

<a name="protocol-type"></a>
### Protocol Type Sub-TLV (Type Code 2)
...
The Protocol Type sub-TLV **MAY** be included in a given TLV to indicate
   ...
   payload types **MUST NOT** be encapsulated in the relevant tunnel. The Value
   field of the sub-TLV contains a 2-octet value from IANA's
"ETHER TYPES"
   registry [IANA-ETHERTYPES](#IANA-ETHERTYPES). If the reserved value 0xFFFF is 
   received, the sub-TLV **MUST** be treated 
   as malformed according to the rules of [Section 13](#validation-and-error).
...
Note that for tunnel types whose names are of the form "X-in-Y"
   ...
   Protocol Type sub-TLV (see [Section 3.4.1](#protocol-type)) specifying
   ...
   sub-TLV specifying anything other than "X" **MUST** be ignored; this is
   discussed further in [Section 13](#validation-and-error).

<a name="color"></a>
### Color Sub-TLV (Type Code 4)

The Color sub-TLV **MAY** be used as a way to "color" the
   ...
   in [Section 4.3](#color-extcomm).  For the use of this sub-TLV and extended community,
   please see [Section 8](#recursive-nh-resolution).

The format of the Value field is depicted in 
   [Figure 15](#ref-color-extended-community).

If the Length field of a Color sub-TLV has a value other than 8, or the first two
   octets of its Value field are not 0x030b, the sub-TLV **MUST** be
   treated as if it were an unrecognized sub-TLV (see [Section 13](#validation-and-error)).

## Embedded Label Handling Sub-TLV (Type Code 9)
...
Certain BGP address families (corresponding to particular AFI/SAFI
   ...
   to refer to any AFI/SAFI that has embedded labels.
...
The Embedded Label Handling sub-TLV may be included in any TLV of the
   ...
   then the sub-TLV **MUST** be disregarded.  If the sub-TLV is contained in a
   TLV whose tunnel type does not have a virtual network identifier in
   its encapsulation header, the sub-TLV **MUST** be disregarded.  In
   those cases where the sub-TLV is ignored, it **MUST NOT** be
...
**2:**
  The embedded label is not carried in the payload but is either carried
...
If any value other than 1 or 2 is carried, the sub-TLV **MUST** be 
	considered malformed, according to the procedures of [Section 13](#validation-and-error).

Please see [Section 9](#use-of-vni) for the details of how this sub-TLV is used when
...
```
<a name="label-stack"></a>
## MPLS Label Stack Sub-TLV (Type Code 10)

This sub-TLV allows an MPLS label stack [RFC3032](#RFC3032) to be associated
...
**Each label stack entry has the format shown in [Figure 13](#ref-mpls-label-stack-sub-tlv).**
...
```
The fields are as defined in [RFC3032](#RFC3032) and [RFC5462](#RFC5462).
...
If a packet is to be sent through the tunnel identified in a
   ...
   packet before any
   other labels are pushed onto the packet. (See [Section 6](#usage)
...
If the MPLS Label Stack sub-TLV is included in a TLV identifying a
   tunnel type that uses virtual network identifiers (see [Section 9](#use-of-vni)),
   the contents of the MPLS Label Stack sub-TLV **MUST** be pushed onto the
   packet before the procedures of [Section 9](#use-of-vni) are applied.
...
The number of label stack entries in the sub-TLV **MUST** be determined
   ...
   stack, the S bits of all the entries being pushed **MUST** be cleared.  When the label stack entries are pushed onto a packet that does not already have a
...
The Traffic Class (TC) field [RFC3270](#RFC3270)[RFC5129](#RFC5129) of each label stack entry **SHOULD** be set to 0,
...
The TTL (Time to Live) field of each label stack entry **SHOULD** be set
   ...
   router that is pushing the stack onto a packet **MUST** change the value to
...
Although this specification does not supply detailed instructions
   ...
   invalid or unsupported label stack is received, the tunnel **MAY** be treated as not feasible, according to the 
   procedures of [Section 6](#usage).

<a name="prefix-sid"></a>
## Prefix-SID Sub-TLV (Type Code 11)

[RFC8669](#RFC8669) defines a BGP path
...
This document defines a Prefix-SID (Prefix Segment Identifier) sub-TLV.
   ...
   value of the Value field of a BGP Prefix-SID attribute [RFC8669](#RFC8669).

[RFC8669](#RFC8669) only defines behavior when the BGP Prefix-SID
   attribute is attached to routes of type IPv4/IPv6 Labeled Unicast
   [RFC4760](#RFC4760)[RFC8277](#RFC8277), and it only
   defines values of the BGP Prefix-SID attribute for those cases. Therefore, 
   similar limitations exist for the Prefix-SID sub-TLV: it **SHOULD** only
   be included in a BGP UPDATE message for one of the address families
   for which [RFC8669](#RFC8669) has a defined behavior, namely BGP IPv4/IPv6 Labeled Unicast [RFC4760](#RFC4760) [RFC8277](#RFC8277). If included in a BGP UPDATE for
   any other address family, it **MUST** be ignored.
...
The Prefix-SID sub-TLV can occur in a TLV identifying any type of
   ...
   **MUST** be interpreted to be the SRGB used by the tunnel's
...
The corresponding MPLS label is pushed on after the processing of the
   MPLS Label Stack sub-TLV, if present, as specified in [Section 3.6](#label-stack).
   It is pushed on before any other labels (for example, a label embedded in an
   UPDATE's NLRI or a label determined by the procedures of [Section 9](#use-of-vni)) are pushed on the stack.
...
<a name="encapsulation-extcomm"></a>
## Encapsulation Extended Community

The Encapsulation Extended Community is a Transitive Opaque Extended
   Community.

The Encapsulation Extended Community encoding is as shown in [Figure 14](#ref-Encapsulation-extended-community).
...
This extended community may be attached to a route of any
   ...
   semantics are the same as semantics of a Tunnel TLV in a Tunnel Encapsulation attribute,
    for which the following three conditions all hold:

1. It identifies the same tunnel type.
2. It has a Tunnel Egress Endpoint sub-TLV for which one of the following
       two conditions holds:
            Its Address Family subfield contains zero, or
              Its Address subfield contains the address of the Next Hop field of 
           the route to which the Tunnel Encapsulation attribute is attached.
  1. Its Address Family subfield contains zero, or
  2. Its Address subfield contains the address of the Next Hop field of 
           the route to which the Tunnel Encapsulation attribute is attached.
3. It has no other sub-TLVs.

Such a Tunnel TLV is called a "barebones" Tunnel TLV.

The Encapsulation Extended Community was first defined in [RFC5512](#RFC5512).
   ...
   a barebones TLV, it **MUST** be encoded using the corresponding 
   Encapsulation Extended Community.  Notwithstanding, an implementation 
   **MUST** be prepared to process a tunnel received encoded as a barebones 
   TLV.
...
Note that for tunnel types of the form "X-in-Y" (for example, MPLS-in-GRE),
   ...
   type "Y". Packets with other payload types **MUST NOT** be carried through
   such tunnels. See also [Section 2](#encaps-attr).
...
[EVPN-INTER-SUBNET](#I-D.ietf-bess-evpn-inter-subnet-forwarding) defines a router's MAC 
...
<a name="color-extcomm"></a>
## Color Extended Community

The Color Extended Community is a Transitive Opaque Extended
   Community with the encoding shown in [Figure 15](#ref-color-extended-community).
...
The value of the high-order octet of the extended Type field is 0x03, which 
   ...
   configured locally. No flags are defined in this document; this field **MUST** be set to 
   zero by the originator and ignored by the receiver; the value **MUST NOT** be 
  ...
  [Section 8](#recursive-nh-resolution).

<a name="ipip-considerations"></a>
...
- UPDATE U has a Tunnel Encapsulation attribute, containing at least
      ...
      properties:
          The tunnel type is supported (that is, router R knows how to set up tunnels of that type, how to create the encapsulation header for tunnels of that type, etc.).
            The tunnel is of a type that can be used to carry packet P (for example, an MPLS-in-UDP tunnel would not be a feasible tunnel for
		 ...
		 is evaluated as per [RFC4271](#RFC4271).  If the IP address is 
  ...
  2. The tunnel is of a type that can be used to carry packet P (for example, an MPLS-in-UDP tunnel would not be a feasible tunnel for
         carrying an IP packet, unless the IP packet can first be
         encapsulated in a MPLS packet).
  3. The tunnel is specified in a TLV whose Tunnel Egress Endpoint sub-TLV
		 identifies an IP address that is reachable. The reachability condition
		 is evaluated as per [RFC4271](#RFC4271).  If the IP address is 
		 reachable via more than one forwarding table, local policy is used 
		 to determine which table to use.
...
If the Tunnel Encapsulation attribute contains several TLVs (that is, if
   ...
   one or more Color sub-TLVs ([Section 3.4.2](#color)) and/or the Protocol Type sub-TLV ([Section 3.4.1](#protocol-type)), the choice of tunnel may be influenced by these 
   sub-TLVs. Many other factors, for example, minimization of encapsulation-header 
   overhead, could also be used to influence selection.
...
See also [Section 13](#validation-and-error), which provides further 
    specification regarding validation and exception cases.

# Routing Considerations

## Impact on the BGP Decision Process
...
The presence of the Tunnel Encapsulation attribute affects
	      ...
	      includes no tunnel that is feasible, then that route **MUST NOT** be considered resolvable for the purposes of the route resolvability 
	      condition ([RFC4271](#RFC4271)).

## Looping, Mutual Recursion, Etc.
...
Consider a packet destined for address X.  Suppose a BGP UPDATE for
   ...
   can have no good outcome. [RFC4271](#RFC4271) describes an analogous case
...
<a name="recursive-nh-resolution"></a>
...
Then packet P **MUST** be sent through one of the tunnels identified in
   the Tunnel Encapsulation attribute of UPDATE U2.  See [Section 6](#usage) for
   further details.

However, suppose that one of the TLVs in U2's Tunnel Encapsulation
   attribute contains one or more Color sub-TLVs.  In that case, packet P **MUST NOT** be sent through the tunnel contained in that TLV, unless U1 is
...
If the TLV specifying a tunnel contains an MPLS Label Stack sub-TLV,
   ...
   [Section 3.6](#label-stack) are applied before the procedures of this section.

If the TLV specifying a tunnel contains a Prefix-SID sub-TLV, the
   procedures of [Section 3.7](#prefix-sid) are applied before the procedures of this
   section.  If the TLV also contains an MPLS Label Stack sub-TLV, the
   procedures of [Section 3.6](#label-stack) are applied before the procedures of
   [Section 3.7](#prefix-sid).

## Tunnel Types without a Virtual Network Identifier Field
...
## Tunnel Types with a Virtual Network Identifier Field
...
<a name="valid-vni"></a>
#### When a Valid VNI Has Been Signaled
...
- If the TLV contains an Embedded Label 
...
<a name="no-valid-vni"></a>
#### When a Valid VNI Has Not Been Signaled
...
- If the TLV does not contain an Embedded Label Handling sub-TLV, or
      ...
      Network Identifier field of the encapsulation header.
...
In a given UPDATE of a labeled address family, the label embedded in
   ...
   [9.2.2.1](#valid-vni) or [9.2.2.2](#no-valid-vni) cause the embedded label to be
   ...
   and design and is outside the scope of this document.

Note that if the Tunnel Encapsulation attribute is attached to a VPN-
   IP route [RFC4364](#RFC4364), if Inter-AS "option b" (see
   [RFC4364](#RFC4364)) is being used, and if the Tunnel Egress Endpoint sub-TLV contains
...
Other documents may define other ways to signal tunnel information in
   BGP. For example, [RFC6514](#RFC6514) defines the "P-Multicast
   ...
   signaling tunnels. Any document specifying such joint use **MUST** 
...
The Tunnel Encapsulation attribute is defined as a transitive
   ...
   attribute **MUST** be used only within a well-defined scope, for example, within a
...
To prevent the Tunnel Encapsulation attribute from being distributed
   ...
   granularities (for example, per route and/or per attribute TLV) **MAY** be 
   supported.  For each external BGP (EBGP) session, filtering of the attribute on 
   incoming UPDATEs **MUST** be enabled by default.
...
In addition, any BGP speaker that understands the attribute **MUST** be
   ...
   filtering **SHOULD** be possible on a per-BGP-session basis.  For each EBGP
...
- Any BGP speaker that
   understands it **MUST** be able to filter it from incoming BGP UPDATE
   messages.
- It **MUST** be possible to filter the Encapsulation
   Extended Community from outgoing messages.
- In both cases, this
...
The Tunnel Encapsulation attribute is a sequence of TLVs, each of
   ...
   case, the TLV **MUST** be considered to be malformed,  
   and the "Treat-as-withdraw" procedure of 
   [RFC7606](#RFC7606) is applied.
...
If a Tunnel Encapsulation attribute does not have any valid TLVs, or
   ...
   procedure of [RFC7606](#RFC7606) is applied.
...
The following sub-TLVs defined in this document **MUST NOT** occur more
   ...
   each such sub-TLV type **MUST** be disregarded.  However, the
...
If a TLV of a Tunnel Encapsulation attribute contains a sub-TLV that
   ...
   propagated with the attribute, the unrecognized sub-TLV **MUST** remain in
   the attribute.

In general, if a TLV contains a sub-TLV that is malformed,
   the sub-TLV **MUST** be treated as if it were an unrecognized sub-TLV.
   There is one exception to this rule: if a TLV
   contains a malformed Tunnel Egress Endpoint sub-TLV (as defined in
   [Section 3.1](#tunnel-egress-endpoint)), the entire TLV **MUST** be ignored and **MUST** be removed from the Tunnel Encapsulation attribute before the
   route carrying that attribute is distributed.
...
Within a Tunnel Encapsulation attribute that is carried by a BGP
   ...
   paragraph of [Section 6](#usage), a TLV that does not contain exactly one
...
A TLV identifying a particular tunnel type may contain a sub-TLV that
   ...
   However, the sub-TLV **MUST NOT** be considered to be malformed and **MUST NOT** be removed from the TLV before the route carrying the Tunnel
...
- "Border Gateway Protocol (BGP) Extended Communities" registry [IANA-BGP-EXT-COMM](#IANA-BGP-EXT-COMM)
- "Border Gateway Protocol (BGP) Parameters" registry [IANA-BGP-PARAMS](#IANA-BGP-PARAMS)
- "Border Gateway Protocol (BGP) Tunnel Encapsulation" registry [IANA-BGP-TUNNEL-ENCAP](#IANA-BGP-TUNNEL-ENCAP)
- "Subsequent Address Family Identifiers (SAFI) Parameters" registry [IANA-SAFI](#IANA-SAFI)

<a name="obsoleting-5566-and-5640"></a>
## Obsoleting Code Points Assigned by RFC 5566

Since this document obsoletes RFC 5566, the code points 
	assigned by that RFC are similarly obsoleted. Specifically, the 
	following code points have been marked as deprecated.

In the
"BGP Tunnel Encapsulation Attribute Tunnel Types" registry [IANA-BGP-TUNNEL-ENCAP](#IANA-BGP-TUNNEL-ENCAP):
...
And in the "BGP Tunnel Encapsulation Attribute Sub-TLVs" registry [IANA-BGP-TUNNEL-ENCAP](#IANA-BGP-TUNNEL-ENCAP):
...
IANA has created a new registry grouping named
   "Border Gateway Protocol (BGP) Tunnel Encapsulation" [IANA-BGP-TUNNEL-ENCAP](#IANA-BGP-TUNNEL-ENCAP).
...
IANA has relocated the "BGP Tunnel Encapsulation Attribute Tunnel Types" registry to be 
   under the "Border Gateway Protocol (BGP) Tunnel Encapsulation" grouping [IANA-BGP-TUNNEL-ENCAP](#IANA-BGP-TUNNEL-ENCAP).

## Subsequent Address Family Identifiers

IANA has modified the "SAFI Values" registry [IANA-SAFI](#IANA-SAFI) to 
   indicate that the Encapsulation SAFI (value 7) has been
   obsoleted.  This document is listed as the reference for this change.
...
IANA has relocated the "BGP Tunnel Encapsulation Attribute Sub-TLVs" registry to be under 
   the "Border Gateway Protocol (BGP) Tunnel Encapsulation" grouping [IANA-BGP-TUNNEL-ENCAP](#IANA-BGP-TUNNEL-ENCAP).
...
IANA has created a registry named "Flags Field of VXLAN Encapsulation Sub-TLVs" under 
   the "Border Gateway Protocol (BGP) Tunnel Encapsulation" grouping [IANA-BGP-TUNNEL-ENCAP](#IANA-BGP-TUNNEL-ENCAP). The registration policy for
   this registry is "Standards Action". The minimum possible value is 0, and the 
   maximum is 7.

The initial values for this new registry are indicated in [Table 5](#flags-vxlan).
...
IANA has created a registry named "Flags Field of NVGRE Encapsulation Sub-TLVs" under 
   the "Border Gateway Protocol (BGP) Tunnel Encapsulation" grouping [IANA-BGP-TUNNEL-ENCAP](#IANA-BGP-TUNNEL-ENCAP). The registration policy for
   this registry is "Standards Action". The minimum possible value is 0, and the 
   maximum is 7.

The initial values for this new registry are indicated in [Table 6](#flags-nvgre).
...
IANA has created a registry named "Embedded Label Handling Sub-TLVs" under 
   the "Border Gateway Protocol (BGP) Tunnel Encapsulation" grouping [IANA-BGP-TUNNEL-ENCAP](#IANA-BGP-TUNNEL-ENCAP). The registration policy for
   this registry is "Standards Action". The minimum possible value is 0, and the 
   maximum is 255.

The initial values for this new registry are indicated in [Table 7](#embedded-label).
...
<a name="color-extcomm-flags"></a>
## Color Extended Community Flags

IANA has created a registry named "Color Extended Community Flags" under 
   the "Border Gateway Protocol (BGP) Tunnel Encapsulation" grouping [IANA-BGP-TUNNEL-ENCAP](#IANA-BGP-TUNNEL-ENCAP). The registration policy for
   this registry is "Standards Action". The minimum possible value is 0, and the 
   maximum is 15.

This new registry contains columns for "Bit Position", "Description", and
   "Reference". No values have currently been registered.

<a name="security"></a>
# Security Considerations

As [Section 11](#scoping) discusses, it is intended that the
...
As [RFC4272](#RFC4272) discusses, BGP is vulnerable to traffic-diversion attacks. The Tunnel Encapsulation attribute adds a new
...
In order to further mitigate the risk of diversion of traffic from
   its intended destination, [Section 3.1.1](#address-validation)
   ...
   pass the validation procedures of [Section 3.1.1](#address-validation). BGP origin validation [RFC6811](#RFC6811) and BGPsec [RFC8205](#RFC8205) provide means to increase assurance that the origins being validated have not been
   falsified.
...
Many tunnels carry traffic that embeds a destination address that comes
   ...
   [Section 10](#applicability) for further discussion of one such 
   scenario.

RFC 8402 specifies that "SR domain boundary routers **MUST** filter any
   external traffic" ([RFC8402](#RFC8402)). For these
   ...
   discussed in [Section 3.7](#prefix-sid). This implies that the duty
...
<a name="RFC2784"></a> **[RFC2784]** D. Farinacci, T. Li, S. Hanks, D. Meyer, P. Traina, "Generic Routing Encapsulation (GRE)", RFC 2784, DOI 10.17487/RFC2784, March 2000. <https://www.rfc-editor.org/info/rfc2784> [Local MD](rfc2784.md)

<a name="RFC2890"></a> **[RFC2890]** G. Dommety, "Key and Sequence Number Extensions to GRE", RFC 2890, DOI 10.17487/RFC2890, September 2000. <https://www.rfc-editor.org/info/rfc2890> [Local MD](rfc2890.md)
...
<a name="RFC3270"></a> **[RFC3270]** F. Le Faucheur, L. Wu, B. Davie, S. Davari, P. Vaananen, R. Krishnan, P. Cheval, J. Heinanen, "Multi-Protocol Label Switching (MPLS) Support of Differentiated Services", RFC 3270, DOI 10.17487/RFC3270, May 2002. <https://www.rfc-editor.org/info/rfc3270> [Local MD](rfc3270.md)

<a name="RFC3931"></a> **[RFC3931]** J. Lau, M. Townsley, I. Goyret, "Layer Two Tunneling Protocol - Version 3 (L2TPv3)", RFC 3931, DOI 10.17487/RFC3931, March 2005. <https://www.rfc-editor.org/info/rfc3931> [Local MD](rfc3931.md)

<a name="RFC4023"></a> **[RFC4023]** T. Worster, Y. Rekhter, E. Rosen, "Encapsulating MPLS in IP or Generic Routing Encapsulation (GRE)", RFC 4023, DOI 10.17487/RFC4023, March 2005. <https://www.rfc-editor.org/info/rfc4023> [Local MD](rfc4023.md)
...
<a name="RFC5129"></a> **[RFC5129]** B. Davie, B. Briscoe, J. Tay, "Explicit Congestion Marking in MPLS", RFC 5129, DOI 10.17487/RFC5129, January 2008. <https://www.rfc-editor.org/info/rfc5129> [Local MD](rfc5129.md)
...
<a name="RFC7348"></a> **[RFC7348]** M. Mahalingam, D. Dutt, K. Duda, P. Agarwal, L. Kreeger, T. Sridhar, M. Bursell, C. Wright, "Virtual eXtensible Local Area Network (VXLAN): A Framework for Overlaying Virtualized Layer 2 Networks over Layer 3 Networks", RFC 7348, DOI 10.17487/RFC7348, August 2014. <https://www.rfc-editor.org/info/rfc7348> [Local MD](rfc7348.md)
...
<a name="RFC7637"></a> **[RFC7637]** P. Garg, Y. Wang, "NVGRE: Network Virtualization Using Generic Routing Encapsulation", RFC 7637, DOI 10.17487/RFC7637, September 2015. <https://www.rfc-editor.org/info/rfc7637> [Local MD](rfc7637.md)
...
<a name="I-D.ietf-bess-evpn-inter-subnet-forwarding"></a> **[EVPN-INTER-SUBNET]** Ali Sajassi, Samer Salam, Samir Thoria, John E Drake, Jorge Rabadan, "Integrated Routing and Bridging in EVPN", Internet-Draft draft-ietf-bess-evpn-inter-subnet-forwarding-13, February 2021. Work in Progress <https://tools.ietf.org/html/draft-ietf-bess-evpn-inter-subnet-forwarding-13>

<a name="IANA-ADDRESS-FAM"></a> **[IANA-ADDRESS-FAM]** "Address Family Numbers", <https://www.iana.org/assignments/address-family-numbers/>

<a name="IANA-BGP-EXT-COMM"></a> **[IANA-BGP-EXT-COMM]** "Border Gateway Protocol (BGP) Extended Communities", <https://www.iana.org/assignments/bgp-extended-communities/>

<a name="IANA-BGP-PARAMS"></a> **[IANA-BGP-PARAMS]** "Border Gateway Protocol (BGP) Parameters", <https://www.iana.org/assignments/bgp-parameters/>

<a name="IANA-BGP-TUNNEL-ENCAP"></a> **[IANA-BGP-TUNNEL-ENCAP]** "Border Gateway Protocol (BGP) Tunnel Encapsulation", <https://www.iana.org/assignments/bgp-tunnel-encapsulation/>

<a name="IANA-ETHERTYPES"></a> **[IANA-ETHERTYPES]** "IEEE 802 Numbers: ETHER TYPES", <https://www.iana.org/assignments/ieee-802-numbers/>

<a name="IANA-SAFI"></a> **[IANA-SAFI]** "Subsequent Address Family Identifiers (SAFI) Parameters", <https://www.iana.org/assignments/safi-namespace/>
...
<a name="RFC5512"></a> **[RFC5512]** P. Mohapatra, E. Rosen, "The BGP Encapsulation Subsequent Address Family Identifier (SAFI) and the BGP Tunnel Encapsulation Attribute", RFC 5512, DOI 10.17487/RFC5512, April 2009. <https://www.rfc-editor.org/info/rfc5512> [Local MD](rfc5512.md)

<a name="RFC5565"></a> **[RFC5565]** J. Wu, Y. Cui, C. Metz, E. Rosen, "Softwire Mesh Framework", RFC 5565, DOI 10.17487/RFC5565, June 2009. <https://www.rfc-editor.org/info/rfc5565> [Local MD](rfc5565.md)

<a name="RFC5566"></a> **[RFC5566]** L. Berger, R. White, E. Rosen, "BGP IPsec Tunnel Encapsulation Attribute", RFC 5566, DOI 10.17487/RFC5566, June 2009. <https://www.rfc-editor.org/info/rfc5566> [Local MD](rfc5566.md)

<a name="RFC5640"></a> **[RFC5640]** C. Filsfils, P. Mohapatra, C. Pignataro, "Load-Balancing for Mesh Softwires", RFC 5640, DOI 10.17487/RFC5640, August 2009. <https://www.rfc-editor.org/info/rfc5640> [Local MD](rfc5640.md)

<a name="RFC6514"></a> **[RFC6514]** R. Aggarwal, E. Rosen, T. Morin, Y. Rekhter, "BGP Encodings and Procedures for Multicast in MPLS/BGP IP VPNs", RFC 6514, DOI 10.17487/RFC6514, February 2012. <https://www.rfc-editor.org/info/rfc6514> [Local MD](rfc6514.md)

<a name="RFC7510"></a> **[RFC7510]** X. Xu, N. Sheth, L. Yong, R. Callon, D. Black, "Encapsulating MPLS in UDP", RFC 7510, DOI 10.17487/RFC7510, April 2015. <https://www.rfc-editor.org/info/rfc7510> [Local MD](rfc7510.md)
...
<a name="RFC8365"></a> **[RFC8365]** A. Sajassi, J. Drake, N. Bitar, R. Shekhar, J. Uttaro, W. Henderickx, "A Network Virtualization Overlay Solution Using Ethernet VPN (EVPN)", RFC 8365, DOI 10.17487/RFC8365, March 2018. <https://www.rfc-editor.org/info/rfc8365> [Local MD](rfc8365.md)
...
<a name="impact-on-8365"></a>
# Impact on RFC 8365

[RFC8365](#RFC8365) references RFC 5512 for its definition of 
...
[RFC8365](#RFC8365) talks about the use of the Encapsulation Extended
	...
	example, in the second paragraph of Section [RFC8365](#RFC8365)), encapsulations advertised
...
In particular, a review of [RFC8365](#RFC8365) is called for, to 
...
[RFC8365](#RFC8365) also refers to a draft version of this specification in
	the final paragraph of Section [RFC8365](#RFC8365). That paragraph references 
	Section 8.2.2.2 of the draft. In this document, the correct reference
	would be [Section 9.2.2.2](#no-valid-vni). There are no substantive 
	differences between the section in the referenced draft version and that in
	this document.

# Acknowledgments

This document contains text from RFC 5512, authored by Pradosh Mohapatra and Eric Rosen.  The authors of the current document wish to thank 
   them for their contribution.  RFC 5512 itself built upon prior work by Gargi Nalawade, Ruchi Kapoor, Dan Tappan, David Ward, Scott Wainner, Simon Barber, Lili Wang, and Chris Metz, whom the authors also thank for their 
...
The authors wish to thank Lou Berger, Ron Bonica, Martin Djernaes,
   John Drake, Susan Hares, Satoru Matsushima, Thomas Morin, Dhananjaya Rao, Ravi Singh, Harish Sitaraman, Brian Trammell, Xiaohu Xu, and Zhaohui Zhang for their review, comments, and/or helpful discussions. Alvaro Retana provided an
   especially comprehensive review.
...
- Address: 731 Lexington Ave, New York City, NY, 10022, USA
...
- Address: 2077 Gateway Pl, San Jose, CA, 95110, United States of America
- Email: keyur@arrcus.com

**Gunter Van de Velde**

- Organization: Nokia
- Address: Copernicuslaan 50, Antwerpen, 2018, Belgium
- Email: gunter.van_de_velde@nokia.com
