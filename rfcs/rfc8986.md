...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-spring-srv6-network-programming-28
...
**Keywords:** SRv6, Segment Routing, IPv6 Segment Routing

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-spring-srv6-network-programming-28)
- [alternate](https://dx.doi.org/10.17487/rfc8986)
...
- [3. SRv6 SID](#section-3)
  - [3.1. SID Format](#sid_format)
  - [3.2. SID Allocation within an SR Domain](#section-3.2)
  - [3.3. SID Reachability](#section-3.3)
- [4. SR Endpoint Behaviors](#behaviors)
  - [4.1. End: Endpoint](#section-4.1)
    - [4.1.1. Upper-Layer Header](#upper)
  - [4.2. End.X: L3 Cross-Connect](#section-4.2)
  - [4.3. End.T: Specific IPv6 Table Lookup](#section-4.3)
  - [4.4. End.DX6: Decapsulation and IPv6 Cross-Connect](#section-4.4)
  - [4.5. End.DX4: Decapsulation and IPv4 Cross-Connect](#section-4.5)
  - [4.6. End.DT6: Decapsulation and Specific IPv6 Table Lookup](#section-4.6)
  - [4.7. End.DT4: Decapsulation and Specific IPv4 Table Lookup](#section-4.7)
  - [4.8. End.DT46: Decapsulation and Specific IP Table Lookup](#section-4.8)
  - [4.9. End.DX2: Decapsulation and L2 Cross-Connect](#section-4.9)
  - [4.10. End.DX2V: Decapsulation and VLAN L2 Table Lookup](#section-4.10)
  - [4.11. End.DT2U: Decapsulation and Unicast MAC L2 Table Lookup](#section-4.11)
  - [4.12. End.DT2M: Decapsulation and L2 Table Flooding](#section-4.12)
  - [4.13. End.B6.Encaps: Endpoint Bound to an SRv6 Policy with Encapsulation](#section-4.13)
  - [4.14. End.B6.Encaps.Red: End.B6.Encaps with Reduced SRH](#section-4.14)
  - [4.15. End.BM: Endpoint Bound to an SR-MPLS Policy](#section-4.15)
  - [4.16. Flavors](#BehFlavors)
    - [4.16.1. PSP: Penultimate Segment Pop of the SRH](#section-4.16.1)
    - [4.16.2. USP: Ultimate Segment Pop of the SRH](#section-4.16.2)
    - [4.16.3. USD: Ultimate Segment Decapsulation](#section-4.16.3)
- [5. SR Policy Headend Behaviors](#section-5)
  - [5.1. H.Encaps: SR Headend with Encapsulation in an SR Policy](#section-5.1)
  - [5.2. H.Encaps.Red: H.Encaps with Reduced Encapsulation](#section-5.2)
  - [5.3. H.Encaps.L2: H.Encaps Applied to Received L2 Frames](#section-5.3)
  - [5.4. H.Encaps.L2.Red: H.Encaps.Red Applied to Received L2 Frames](#section-5.4)
- [6. Counters](#section-6)
- [7. Flow-Based Hash Computation](#OpsFlowLabel)
- [8. Control Plane](#cp)
  - [8.1. IGP](#igp)
  - [8.2. BGP-LS](#section-8.2)
  - [8.3. BGP IP/VPN/EVPN](#section-8.3)
  - [8.4. Summary](#section-8.4)
- [9. Security Considerations](#section-9)
- [10. IANA Considerations](#section-10)
  - [10.1. Ethernet Next Header Type](#ianaethernet)
  - [10.2. SRv6 Endpoint Behaviors Registry](#iana_registry)
    - [10.2.1. Registration Procedures](#iana_policy)
    - [10.2.2. Initial Registrations](#section-10.2.2)
...
Segment Routing [RFC8402](#RFC8402) leverages
...
[SRV6-NET-PGM-ILLUST](#I-D.filsfils-spring-srv6-net-pgm-illustration) illustrates the concepts defined in this
      document.

Familiarity with the [Segment
      Routing Header](#RFC8754) is expected.

# Terminology

The following terms used within this document are defined in [RFC8402](#RFC8402): Segment Routing (SR), SR Domain, Segment
      ID (SID), SRv6, SRv6 SID, SR Policy, Prefix-SID, and Adj-SID.

The following terms used within this document are defined in [RFC8754](#RFC8754): Segment Routing Header (SRH), SR
      source node, transit node, SR Segment Endpoint Node, Reduced SRH,
      Segments Left, and Last Entry.
...
**Per-CE VPN label:**
  A single label for each attachment circuit that is shared by all routes
  with the same "outgoing attachment circuit" ([RFC4364](#RFC4364))

**Per-VRF VPN label:**
  A single label for the entire VPN Routing and Forwarding (VRF) table that is
  shared by all routes from that VRF ([RFC4364](#RFC4364))
...
**SRv6 SID function:**
  ...
  behavior bound to the SID. It is formally defined in [Section 3.1](#sid_format) of this document.

**SRv6 Endpoint behavior:**
  A packet processing behavior executed at an SRv6 Segment Endpoint
  Node. [Section 4](#behaviors) of this document defines
...
- SRH with SID list <S1, S2, S3>
      ...
      S2, S1; SL) notation is more convenient.
...
[RFC8402](#RFC8402) defines an SRv6 Segment Identifier as an IPv6 address
...
Its processing is defined in [RFC8754](#RFC8754) and reproduced here as a reminder:

[Section 4](#behaviors) of this document defines
      a new set of SRv6 SID behaviors in addition to that defined in [RFC8754](#RFC8754).
...
When the LOC part of the SRv6 SIDs is routable, it leads to the
        node, which instantiates the SID.
...
The term "function" refers to the bit string in the SRv6 SID. The
        ...
        behaviors are defined in [Section 4](#behaviors) of this
        document.

An SRv6 Endpoint behavior may require additional
        information for its processing (e.g., related to the flow or
        service). This information may be encoded in the ARG bits of the
        SID.

In such a case, the semantics and format of the ARG bits are
        defined as part of the SRv6 Endpoint behavior specification.
...
## SID Allocation within an SR Domain
...
As an example, one mobile service provider has commercially
        ...
        infrastructure addresses from the Unique Local Address (ULA) space [RFC4193](#RFC4193). They specifically allocated three
        ...
        prefix from which all SIDs of that router are allocated.
...
A service provider receiving the current minimum allocation of a
...
When an operator instantiates a SID at a node, they specify a SID
        ...
        document (see [Table 6](#endpoint_cp_types)).

The node advertises the SID, B:N:FUNCT, in the control plane (see
        [Section 8](#cp)) together with the SRv6 Endpoint
        Behavior codepoint identifying the behavior of the SID.
...
- A network operator may assign an SRv6 SID block
...
- At Router 3, within the locator 2001:db8:bbbb:3::/64, the
            network operator or the router performs dynamic assignment for:
            Function 0x0100 associated with the behavior End.X
              ...
              Function 0x0101 associated with the behavior End.X
                (Endpoint with L3 cross-connect) between router 3 and its
...
An SRv6 SID is said to be routed if its SID belongs to an IPv6
        prefix advertised via a routing protocol. An SRv6 SID that does not
        fulfill this condition is non-routed.
...
A packet could be steered through a non-routed SID
        ...
        global and local segments, respectively [RFC8402](#RFC8402).
...
| End | Endpoint
              The SRv6 instantiation of a Prefix-SID [RFC8402](#RFC8402) |
| End.X | Endpoint with L3 cross-connect
              The SRv6 instantiation of an Adj-SID [RFC8402](#RFC8402) |
...
The list is not exhaustive. In practice, any behavior can be attached
...
When an SRv6-capable node (N) receives an IPv6 packet whose
      ...
      in [RFC8200](#RFC8200). For SRv6 SIDs
...
[Section 4.16](#BehFlavors) defines flavors of some of these behaviors.

[Section 10.2](#iana_registry) of this document
      defines the IANA registry used to maintain all these behaviors as well
      as future ones defined in other documents.

## End: Endpoint

The Endpoint behavior ("End" for short) is the most basic
        behavior. It is the instantiation of a Prefix-SID [RFC8402](#RFC8402).
...
It is **RECOMMENDED** that an implementation of local
          configuration only allows Upper-Layer header processing of types
          that do not result in the packet being forwarded (e.g., ICMPv6).

## End.X: L3 Cross-Connect

The "Endpoint with L3 cross-connect" behavior ("End.X" for short) is a variant of the End behavior.

It is the SRv6 instantiation of an Adj-SID [RFC8402](#RFC8402), and its main use is for traffic-engineering policies.

Any SID instance of this behavior is associated with a set, J, of
        one or more L3 adjacencies.
...
When the End.X behavior is associated with a BGP Next-Hop, it is
        the SRv6 instantiation of the BGP peering segments [RFC8402](#RFC8402).
...
The End.T behavior is used for multi-table operation in the
...
When processing the Upper-Layer header of a packet matching a FIB entry locally instantiated as an End.T SID, process the packet as per [Section 4.1.1](#upper).
...
One of the applications of the End.DX6 behavior is the L3VPNv6 use case where a FIB lookup in a specific tenant table at the egress Provider Edge (PE) is not required. This is equivalent to the per-CE VPN label in MPLS [RFC4364](#RFC4364).
...
One of the applications of the End.DX4 behavior is the L3VPNv4 use
        ...
        not required. This is equivalent to the per-CE VPN label in MPLS [RFC4364](#RFC4364).

The End.DX4 SID **MUST** be the last segment in an SR
        Policy, and it is associated with one or more L3 IPv4 adjacencies
        J.

When N receives a packet destined to S and S is a local End.DX4
...
One of the applications of the End.DT6 behavior is the
        L3VPNv6 use case where a FIB
        lookup in a specific tenant table at the egress PE is required. This
        is equivalent to the per-VRF VPN label in MPLS [RFC4364](#RFC4364).

Note that an End.DT6 may be defined for the main IPv6 table, in
        which case an End.DT6 supports the equivalent of an IPv6-in-IPv6
        decapsulation (without VPN/tenant implication).

The End.DT6 SID **MUST** be the last segment in an SR
        Policy, and a SID instance is associated with an IPv6 FIB table T.
...
One of the applications of the End.DT4 behavior is the L3VPNv4 use case where a FIB lookup in a specific tenant table at the egress PE is required. This is equivalent to the per-VRF VPN label in MPLS [RFC4364](#RFC4364).
...
One of the applications of the End.DT46 behavior is the L3VPN
        ...
        (for IPv4 and IPv6) in MPLS [RFC4364](#RFC4364).
...
The End.DT46 SID **MUST** be the last segment in an SR
        Policy, and a SID instance is associated with an IPv4 FIB table T4 and
        an IPv6 FIB table T6.

When N receives a packet destined to S and S is a local End.DT46
        SID, N does the following:
...
One of the applications of the End.DX2 behavior is the L2VPN [RFC4664](#RFC4664) / EVPN Virtual Private Wire Service (VPWS) [RFC7432](#RFC7432) [RFC8214](#RFC8214) use case.
...
One of the applications of the End.DX2V behavior is the EVPN
...
One of the applications of the End.DT2U behavior is the EVPN
        Bridging Unicast [RFC7432](#RFC7432). Any SID
...
Two of the applications of the End.DT2M behavior are the EVPN
        ...
        with Ethernet Segment Identifier (ESI) filtering [RFC7432](#RFC7432) and the EVPN Ethernet-Tree
        (E-Tree) [RFC8317](#RFC8317) use cases.

Any SID instance of this behavior is associated with an L2 table
...
One of its applications is to express scalable traffic-engineering policies across multiple domains. It is one of the SRv6 instantiations of a Binding SID [RFC8402](#RFC8402).
...
The SRH Last Entry field is set as defined in [RFC8754](#RFC8754).
...
The End.BM behavior is required to express scalable traffic-engineering policies across multiple domains where some domains support the MPLS instantiation of Segment Routing. This is an SRv6 instantiation of an SR-MPLS Binding SID [RFC8402](#RFC8402).
...
When processing the Upper-Layer header of a packet matching a FIB entry locally instantiated as an End.BM SID, process the packet as per [Section 4.1.1](#upper).
...
SR Segment Endpoint Nodes advertise the SIDs instantiated on
            them via control-plane protocols as described in [Section 8](#cp). Different behavior IDs are allocated for
            flavored and unflavored SIDs (see [Table 6](#endpoint_cp_types)).

An SR Segment Endpoint Node that offers both PSP- and
            non-PSP-flavored behavior advertises them as two different
            SIDs.

The SR Segment Endpoint Node only advertises the PSP flavor if
            the operator enables this capability at the node.

The PSP operation is deterministically controlled by the SR
            source node.

A PSP-flavored SID is used by the SR source node when it needs
...
A penultimate SR Segment Endpoint Node is one that, as part of
...
The PSP operation only takes place at a penultimate SR Segment
            ...
            the pseudocode below since Segments Left would not be zero.
...
The usage of PSP does not increase the MTU of the IPv6 packet
            and hence does not have any impact on the Path MTU (PMTU)
            discovery mechanism.

As a reminder, [RFC8754](#RFC8754) defines the SR Deployment Model
            within the SR Domain [RFC8402](#RFC8402). Within this framework, the Authentication
            Header (AH) is not used to secure the SRH as described in [RFC8754](#RFC8754). Hence, the discussion of applicability of PSP
            along with AH usage is beyond the scope of this document.

In the context of this specification, the End, End.X, and End.T
            behaviors with PSP do not contravene [RFC8200](#RFC8200) because the
...
- As part of the decapsulation process, the
            egress PE is required to parse and remove fewer bytes from the
            packet.
- If a lookup on an upper-layer IP header is required (e.g.,
...
One of the applications of the USD flavor is the case of a
          ...
          TI-LFA Point of Local Repair and forward the inner packet.

# SR Policy Headend Behaviors

This section describes a set of SRv6 Policy Headend [RFC8402](#RFC8402) behaviors.

**Table: SR Policy Headend Behaviors**
...
N steers the transit packets P1 and P2 into an SRv6
...
The received packet is encapsulated unmodified (with the exception of the IPv4 TTL or IPv6 Hop Limit that is decremented as described in [RFC2473](#RFC2473)).

The H.Encaps behavior is valid for any kind of L3 traffic. This behavior is commonly used for L3VPN with IPv4 and IPv6 deployments. It may be also used for TI-LFA [SR-TI-LFA](#I-D.ietf-rtgwg-segment-routing-ti-lfa) at the Point of Local Repair.

The push of the SRH **MAY** be omitted when the SRv6
...
The H.Encaps.L2 behavior encapsulates a received Ethernet [IEEE.802.3_2018](#IEEE.802.3_2018) frame and its attached VLAN header, if present, in an IPv6 packet with an SRH. The Ethernet frame becomes the payload of the new IPv6 packet.
...
The encapsulating node **MUST** remove the preamble (if
        any) and frame check sequence (FCS) from the Ethernet frame upon
        encapsulation, and the decapsulating node **MUST**
...
<a name="OpsFlowLabel"></a>
# Flow-Based Hash Computation
...
The End, End.T, and End.X SIDs express topological behaviors and
        ...
        a controller) during the SR Policy computation.
...
These SIDs provide important topological behaviors for the IGP to
        build Fast Reroute (FRR) solutions based on TI-LFA [SR-TI-LFA](#I-D.ietf-rtgwg-segment-routing-ti-lfa) and
...
BGP-LS provides the functionality for topology discovery that
        ...
        Policies using the SRv6 SIDs.

## BGP IP/VPN/EVPN

The End.DX4, End.DX6, End.DT4, End.DT6, End.DT46, End.DX2,
        End.DX2V, End.DT2U, and End.DT2M SIDs can be signaled in BGP.

In some scenarios, an egress PE advertising a VPN route might wish
        ...
        in [Table 6](#endpoint_cp_types). The details of
        such control-plane signaling mechanisms are out of the scope of this
        document.

## Summary

The following table summarizes which SID behaviors
        may be signaled in which control-plane protocol.

**Table: SRv6 Locally Instantiated SIDs Signaling**
...
**Table: SRv6 Policy Headend Behaviors Signaling**
...
For example, a BGP-LS advertisement of H.Encaps behavior would
        ...
        pushed by N without significant performance degradation.

As a reminder, an SR Policy is always assigned a Binding SID
        [RFC8402](#RFC8402). Binding SIDs are also advertised
        in BGP-LS as shown in [Table 3](#localsid_signaling). Hence, [Table 4](#transit_signaling) only focuses on the generic capabilities related to
        H.Encaps.

# Security Considerations

The security considerations for Segment Routing are discussed in
      [RFC8402](#RFC8402).  [RFC8754](#RFC8754) describes the SR
      Deployment Model and the requirements for securing the SR Domain. The
      security considerations of [RFC8754](#RFC8754) also cover topics
      ...
      addresses allocated for end users and systems (as illustrated in [RFC8754](#RFC8754)),
      ...
      SRv6 Domain.  Additionally, [RFC8754](#RFC8754)
...
This document introduces SRv6 Endpoint and SR Policy Headend
      ...
      [Section 4.1.1](#upper)). As such, this document does not introduce any
...
The SID behaviors specified in this document have the same HMAC TLV
      ...
      field as the SID behavior specified in [RFC8754](#RFC8754).

# IANA Considerations

<a name="ianaethernet"></a>
## Ethernet Next Header Type

IANA has allocated "Ethernet" (value 143) in the "Assigned
        Internet Protocol Numbers" registry (see [https://www.iana.org/assignments/protocol-numbers/](https://www.iana.org/assignments/protocol-numbers/)).  
        Value 143 in the Next Header field of an IPv6 header or any extension
        header indicates that the payload is an Ethernet frame [IEEE.802.3_2018](#IEEE.802.3_2018).

<a name="iana_registry"></a>
## SRv6 Endpoint Behaviors Registry

IANA has created a new top-level registry
        called "Segment Routing" (see [https://www.iana.org/assignments/segment-routing/](https://www.iana.org/assignments/segment-routing/)). This
        registry serves as a top-level registry for all
        Segment Routing subregistries.

Additionally, IANA has created a new subregistry called "SRv6 Endpoint Behaviors"
...
<a name="iana_policy"></a>
### Registration Procedures

The range of the registry is 0-65535 (0x0000-0xFFFF). The table below contains the allocation ranges and 
        registration policies [RFC8126](#RFC8126) for each:
...
<a name="IEEE.802.3_2018"></a> **[IEEE.802.3_2018]** "IEEE Standard for Ethernet", DOI 10.1109/IEEESTD.2018.8457469, August 2018. IEEE 802.3-2018 <https://ieeexplore.ieee.org/document/8457469>
...
<a name="RFC2473"></a> **[RFC2473]** A. Conta, S. Deering, "Generic Packet Tunneling in IPv6 Specification", RFC 2473, DOI 10.17487/RFC2473, December 1998. <https://www.rfc-editor.org/info/rfc2473> [Local MD](rfc2473.md)

<a name="RFC6437"></a> **[RFC6437]** S. Amante, B. Carpenter, S. Jiang, J. Rajahalme, "IPv6 Flow Label Specification", RFC 6437, DOI 10.17487/RFC6437, November 2011. <https://www.rfc-editor.org/info/rfc6437> [Local MD](rfc6437.md)
...
<a name="RFC4664"></a> **[RFC4664]** L. Andersson, E. Rosen, "Framework for Layer 2 Virtual Private Networks (L2VPNs)", RFC 4664, DOI 10.17487/RFC4664, September 2006. <https://www.rfc-editor.org/info/rfc4664> [Local MD](rfc4664.md)
...
<a name="I-D.ietf-rtgwg-segment-routing-ti-lfa"></a> **[SR-TI-LFA]** Stephane Litkowski, Ahmed Bashandy, Clarence Filsfils, Pierre Francois, Bruno Decraene, Daniel Voyer, "Topology Independent Fast Reroute using Segment Routing", Internet-Draft draft-ietf-rtgwg-segment-routing-ti-lfa-06, February 2021. Work in Progress <https://tools.ietf.org/html/draft-ietf-rtgwg-segment-routing-ti-lfa-06>

<a name="I-D.filsfils-spring-srv6-net-pgm-illustration"></a> **[SRV6-NET-PGM-ILLUST]** C Filsfils, P Camarillo, Z Li, S Matsushima, B Decraene, D Steinberg, D Lebrun, R Raszuk, J Leddy, "Illustrations for SRv6 Network Programming", Internet-Draft draft-filsfils-spring-srv6-net-pgm-illustration-03, September 2020. Work in Progress <https://tools.ietf.org/html/draft-filsfils-spring-srv6-net-pgm-illustration-03>
...
- Organization: Lapishills Consulting Limited
- Address: Cyprus
...
- Organization: Barefoot Networks
- Address: United States of America
...
- Organization: Gran Sasso Science Institute
- Address: Italy
