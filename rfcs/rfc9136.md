...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-bess-evpn-prefix-advertisement-11
...
**Keywords:** RT5, RT-5, Type-5, Interface-less, Interface-ful

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-bess-evpn-prefix-advertisement-11)
- [alternate](https://dx.doi.org/10.17487/rfc9136)
...
- [1. Introduction](#sect-1)
  - [1.1. Terminology](#sect-1.1)
- [2. Problem Statement](#sect-2)
  - [2.1. Inter-Subnet Connectivity Requirements in Data Centers](#sect-2.1)
  - [2.2. The Need for the EVPN IP Prefix Route](#sect-2.2)
- [3. The BGP EVPN IP Prefix Route](#sect-3)
  - [3.1. IP Prefix Route Encoding](#sect-3.1)
  - [3.2. Overlay Indexes and Recursive Lookup Resolution](#sect-3.2)
- [4. Overlay Index Use Cases](#sect-4)
  - [4.1. TS IP Address Overlay Index Use Case](#sect-4.1)
  - [4.2. Floating IP Overlay Index Use Case](#sect-4.2)
  - [4.3. Bump-in-the-Wire Use Case](#sect-4.3)
  - [4.4. IP-VRF-to-IP-VRF Model](#sect-4.4)
    - [4.4.1. Interface-less IP-VRF-to-IP-VRF Model](#sect-4.4.1)
    - [4.4.2. Interface-ful IP-VRF-to-IP-VRF with SBD IRB](#sect-4.4.2)
    - [4.4.3. Interface-ful IP-VRF-to-IP-VRF with Unnumbered SBD IRB](#sect-4.4.3)
- [5. Security Considerations](#sect-5)
- [6. IANA Considerations](#sect-6)
- [7. References](#section-7)
  - [7.1. Normative References](#section-7.1)
  - [7.2. Informative References](#section-7.2)
- [Acknowledgments](#sect-8)
- [Contributors](#sect-9)
...
[RFC7365](#RFC7365) provides a framework for Data Center (DC) Network Virtualization
   ...
   multi-tenant DCs. [RFC8365](#RFC8365) discusses the use of EVPN as the technology of
   choice to provide Layer 2 or intra-subnet services in these DCs.  This
   document, along with [RFC9135](#RFC9135), specifies the use of EVPN for Layer 3 or inter-subnet connectivity services.

[RFC9135](#RFC9135) defines some
   ...
   [RFC9135](#RFC9135) describes how Media Access
   ...
   in [RFC9135](#RFC9135) and defines
   how EVPN may be used to advertise IP prefixes. Interoperability between
   EVPN and Layer 3 Virtual Private Network (VPN) [RFC4364](#RFC4364) IP Prefix routes is out of the
   scope of this document.

[Section 2.1](#sect-2.1) describes the
   inter-subnet connectivity requirements in DCs. [Section 2.2](#sect-2.2) explains why a new EVPN route
   type is required for IP prefix advertisements. Sections [3](#sect-3), [4](#sect-4), and [5](#sect-5) will
...
The key words "**MUST**", "**MUST NOT**", "**REQUIRED**", "**SHALL**", "**SHALL NOT**",
   ...
   14 [RFC2119](#RFC2119) [RFC8174](#RFC8174) when, and only when, they appear in all
...
**BD:**
  Broadcast Domain. As per [RFC7432](#RFC7432), an EVI consists
  	of a single BD or multiple BDs. In case of VLAN-bundle and VLAN-based
  	service models (see [RFC7432](#RFC7432)), a BD is equivalent to
...
**BD Route Target:**
  Refers to the broadcast-domain-assigned Route Target [RFC4364](#RFC4364). In case of a VLAN-aware
  	bundle service model, all the BD instances in the MAC-VRF share the
  	same Route Target.

**BT:**
  Bridge Table. The instantiation of a BD in a
  	  MAC-VRF, as per [RFC7432](#RFC7432).
...
**EVI:**
  EVPN Instance spanning the NVE/PE devices that are
  	participating on that EVPN, as per [RFC7432](#RFC7432).

**EVPN:**
  Ethernet VPN, as per [RFC7432](#RFC7432).

**GENEVE:**
  Generic Network Virtualization Encapsulation, as per [RFC8926](#RFC8926).
...
**MAC-VRF:**
  A Virtual Routing and Forwarding table for
  	MAC addresses on an NVE/PE, as per [RFC7432](#RFC7432). A MAC-VRF is also an instantiation of an EVI in an
  	NVE/PE.
...
**SBD:**
  Supplementary Broadcast Domain. A BD that does not
  	...
  	required in IP-VRF-to-IP-VRF use cases (see [Section 4.4](#sect-4.4)).
...
**VNI:**
  Virtual Network Identifier. As in [RFC8365](#RFC8365), the
...
**VTEP:**
  VXLAN Termination End Point, as per [RFC7348](#RFC7348).

**VXLAN:**
  Virtual eXtensible Local Area Network, as per [RFC7348](#RFC7348).


This document also assumes familiarity with the terminology of
	[RFC7365](#RFC7365), [RFC7432](#RFC7432), and [RFC8365](#RFC8365).
...
[RFC7432](#RFC7432) is used as the control plane for an NVO solution in DCs, where NVE devices can be located in hypervisors or
   Top-of-Rack (ToR) switches, as described in [RFC8365](#RFC8365).
...
- The Tenant Systems may be VA entities that
	...
	VA). The Virtual Router Redundancy Protocol (VRRP) [RFC5798](#RFC5798) is one
...
[Figure 1](#fig-1) illustrates some of the examples described above.
...
- TS2 and TS3 are VAs that send/receive traffic
   ...
   traffic to a given subnet. For example, vIP23 in [Figure 1](#fig-1) is a floating IP that
...
For a BD to which an ingress NVE is attached, "Overlay Index" is
   ...
   example, vIP23 ([Figure 1](#fig-1)) is an Overlay Index that any NVE attached
...
All the DC use cases in [Figure 1](#fig-1) require inter-subnet
   forwarding; therefore, the individual host routes and subnets:

1. must be advertised from the NVEs (since VAs and VMs do not
	participate in dynamic routing protocols) and
2. may be associated with an Overlay Index that can be a VA IP address,
	a floating IP address, a MAC address, or an ESI. The Overlay Index is
	further discussed in [Section 3.2](#sect-3.2).

<a name="sect-2.2"></a>
## The Need for the EVPN IP Prefix Route

[RFC7432](#RFC7432) defines a MAC/IP Advertisement route (also
...
One example of such use cases is the "floating IP" example described
   in [Section 2.1](#sect-2.1). In this example, it is
...
1. The clean and clear advertisements of IPv4 or IPv6 prefixes
        in a Network Layer Reachability Information (NLRI) message without
        MAC addresses are allowed.
2. Since the route type is different from the MAC/IP Advertisement
	route, the current procedures described in [RFC7432](#RFC7432) do not need to
	be modified.
3. A flexible implementation is allowed where the prefix can be linked to
...
4. An EVPN implementation not requiring IP prefixes can simply discard
...
The BGP EVPN NLRI as defined in [RFC7432](#RFC7432) is shown below:
...
This document defines an additional route type (RT-5) in the IANA
   "EVPN Route Types" registry [EVPNRouteTypes](#EVPNRouteTypes) to be used for the
...
According to [RFC7606](#RFC7606), a node that doesn't recognize the
   ...
   attached. Regular procedures described in [RFC7432](#RFC7432) would apply in that case for both
...
- The Route Distinguisher (RD) and Ethernet Tag ID **MUST** be used as
     defined in [RFC7432](#RFC7432) and [RFC8365](#RFC8365). In particular, the RD is unique
     per MAC-VRF (or IP-VRF). The MPLS Label field is set to either an
     MPLS label or a VNI, as described in [RFC8365](#RFC8365) for other EVPN route
     types.
- The Ethernet Segment Identifier **MUST** be a non-zero 10-octet
     identifier if the ESI is used as an Overlay Index (see the
     definition of "Overlay Index" in [Section 3.2](#sect-3.2)). It **MUST** be all bytes zero otherwise. The ESI format is described in [RFC7432](#RFC7432).
...
- The GW IP Address field is a 4- or 16-octet field (IPv4 or
     ...
     not used as an Overlay Index. Refer to [Section 3.2](#sect-3.2) for the
     definition and use of the Overlay Index.
- The MPLS Label field is encoded as 3 octets, where the high-order
     20 bits contain the label value, as per [RFC7432](#RFC7432). When sending,
     ...
     **MUST** be "treat as withdraw" [RFC7606](#RFC7606).
...
An IP Prefix route **MAY** be sent along with an EVPN Router's MAC Extended Community
   (defined in [RFC9135](#RFC9135)) to
   carry the MAC address that is used as the Overlay Index. Note that the MAC
   address may be that of a TS.

As described in [Section 3.2](#sect-3.2), certain data combinations in a received
...
- The ESI and GW IP fields may both be zero at the same time.
	...
	**SHOULD** be treat as withdraw [RFC7606](#RFC7606).
...
The indirection provided by the Overlay Index and its recursive
   ...
   example described in [Section 2.2](#sect-2.2)).

[Table 1](#fields_overlay_table) shows the different RT-5 field combinations allowed by this
   ...
   indicated as "None" in [Table 1](#fields_overlay_table). If there is no Overlay Index, the
   receiving NVE/PE will not perform any recursive resolution, and the
   actual next hop is given by the RT-5's BGP next hop.

**Table: RT-5 Fields and Indicated Overlay Index**
...
*****
  MAC with "Zero" value means no EVPN Router's MAC Extended Community is
  	 ...
  	 a MAC address **MUST** be the 6-octet MAC address specified by [IEEE-802.1Q](#IEEE-802.1Q). Examples
...
******
  In this case, the Overlay Index may be the RT-5's MAC address or
  	 ...
  	 [Table 1](#fields_overlay_table) is used in the IP-VRF-to-IP-VRF
  	 implementations described in Sections [4.4.1](#sect-4.4.1) and [4.4.3](#sect-4.4.3). The support of a MAC Overlay Index in this model is
  	 **OPTIONAL**.
...
If the combination of ESI, GW IP, MAC, and Label in the receiving RT-5
   is different than the combinations shown in [Table 1](#fields_overlay_table), the router will
   process the route as per the rules described at the beginning of this
	section ([Section 3.2](#sect-3.2)).

[Table 2](#use_overlay_table) shows the different inter-subnet use cases described in this
   document and the corresponding coding of the Overlay Index in the
   route type 5 (RT-5).

**Table: Use Cases and Overlay Indexes for Recursive Resolution**

| Section | Use Case | Overlay Index in the RT-5 |
| --- | --- | --- |
| [4.1](#sect-4.1) | TS IP address | GW IP |
| [4.2](#sect-4.2) | Floating IP address | GW IP |
| [4.3](#sect-4.3) | "Bump-in-the-wire" | ESI or MAC |
| [4.4](#sect-4.4) | IP-VRF-to-IP-VRF | GW IP, MAC, or None |
...
This
	...
	corresponding IPv6 values.

<a name="sect-4.1"></a>
## TS IP Address Overlay Index Use Case

[Figure 5](#fig-2) illustrates an example of inter-subnet forwarding for
...
1. NVE2 advertises the following BGP routes on behalf of TS2:
      ...
      Encapsulation Extended Community [RFC9012](#RFC9012) with the corresponding tunnel type. The
...
5. When the packet arrives at NVE2:
  ...
  - Encapsulation is stripped off and, based on a MAC lookup
       (assuming MAC forwarding on the egress NVE), the packet is
       forwarded to TS2, where it will be properly routed.
6. Should TS2 move from NVE2 to NVE3, MAC Mobility procedures will be applied
to the MAC route M2/IP2, as defined in [RFC7432](#RFC7432).  Route type 5
...
Sometimes TSs work in active/standby mode where an
   ...
   already introduced in Sections [2.1](#sect-2.1) and [2.2](#sect-2.2), is illustrated in [Figure 6](#fig-3).
...
In this use case, a GW IP is used as an Overlay Index for the same
   reasons as in [Section 4.1](#sect-4.1). However, this GW IP is a floating IP that belongs
...
1. NVE2 advertises the following BGP routes for TS2:
  ...
  - Route type 5 (IP Prefix route) containing: IPL = 24, IP = SN1,
	      ESI = 0, and GW
       IP address = vIP23. The prefix and GW IP are learned by policy.
2. NVE3 advertises the following BGP route for TS3 (it does not advertise an
RT-2 for M3/vIP23):
            Route type 5 (IP Prefix route) containing: IPL = 24, IP = SN1,
	      ESI = 0, and GW
     IP address = vIP23. The prefix and GW IP are learned by policy.
...
3. DGW1 and DGW2 import both received routes based on the Route Target:
  ...
  - SN1/24 is added to the IP-VRF in DGW1 and DGW2 with Overlay Index
     vIP23 pointing at M2 in the local BD-10.
4. When DGW1 receives a packet from the WAN with destination IPx, where IPx
belongs to SN1/24:
...
5. When the packet arrives at NVE2:
  ...
  - Encapsulation is stripped off and, based on a MAC lookup (assuming
       MAC forwarding on the egress NVE), the packet is forwarded to TS2,
       where it will be properly routed.
6. When the redundancy protocol running between TS2 and TS3 appoints TS3 as
the new active TS for SN1, TS3 will now own the floating vIP23 and will signal
this new ownership using a gratuitous ARP REPLY message (explained in [RFC5227](#RFC5227)) or similar. Upon receiving the new owner's notification,
...
[Figure 7](#fig-4) illustrates an example of inter-subnet forwarding for an IP
...
1. an ESI, i.e., ESI23, that can be provisioned on the attachment
	  ports of NVE2 and NVE3, as shown in [Figure 7](#fig-4) or
2. the VA's MAC address, which can be added to NVE2 and NVE3 by
	  policy.
...
The advantage of using an ESI as the Overlay Index as opposed to the VA's MAC
   ...
   reused. For instance, the mass withdrawal mechanism described in [RFC7432](#RFC7432) for fast
   ...
   [Section 4.4.3](#sect-4.4.3).

The model supports VA redundancy in a similar way to the one
   described in [Section 4.2](#sect-4.2) for the floating IP Overlay Index use case,
...
1. Assuming TS2 is the active TS in ESI23, NVE2 advertises the
   following BGP routes:
     ...
     Encapsulation Extended Community as per [RFC8365](#RFC8365).
              Route type 5 (IP Prefix route) containing: IPL = 24, IP = SN1,
	      ESI = ESI23, and GW IP address = 0. The EVPN Router's MAC Extended Community defined in
     [RFC9135](#RFC9135) is added and carries the MAC address (M2)
  ...
  - Route type 5 (IP Prefix route) containing: IPL = 24, IP = SN1,
     ...
     associated with the TS behind which SN1 sits. M2 may be learned by policy; however, the
     MAC in the Extended Community is preferred if sent with the route.
2. NVE3 advertises the following BGP route for TS3 (no AD per EVI route is
advertised):
            Route type 5 (IP Prefix route) containing: IPL = 24, IP = SN1,
...
5. When the packet arrives at NVE2:
     ...
     case), the BD-10 context is identified for a MAC lookup (assuming a MAC-based disposition model [RFC7432](#RFC7432)), or the VNI may directly identify
...
This use case is similar to the scenario described in [RFC9135](#RFC9135); however, the new
   requirement here is the advertisement of IP prefixes as opposed to
   only host routes.

In the examples described in Sections [4.1](#sect-4.1), [4.2](#sect-4.2), and [4.3](#sect-4.3), the BD
...
In order to provide connectivity for [(1)](#step1), MAC/IP Advertisement routes (RT-2) are
   needed so that IRB or TS MACs and IPs can be distributed.
   Connectivity type [(2)](#step2) is accomplished by the exchange of IP Prefix
...
[RFC9135](#RFC9135) defines an asymmetric IRB model and a symmetric
...
[Figure 8](#fig-5) depicts the Interface-less IP-VRF-to-IP-VRF model.
...
1. The NVEs and DGWs must provide connectivity between hosts in SN1,
	...
	to/from the WAN.
2. The IP-VRF instances in the NVE/DGWs are directly connected through
	NVO tunnels, and no IRBs and/or BD instances are instantiated to
	connect the IP-VRFs.
3. The solution must provide Layer 3 connectivity among the IP-VRFs
	for Ethernet NVO tunnels -- for instance, VXLAN or GENEVE.
4. The solution may provide Layer 3 connectivity among the IP-VRFs for
...
- The first one is the BGP Encapsulation Extended Community, as per
	  [RFC9012](#RFC9012), identifying the tunnel type.
- The second one is the EVPN Router's MAC Extended Community, as per
	  [RFC9135](#RFC9135), containing the MAC address associated with the NVE advertising the
...
The model described above is called an "interface-less" model since the
   ...
   those tunnels to be terminated in SBDs instead, as in Sections [4.4.2](#sect-4.4.2) or [4.4.3](#sect-4.4.3).

<a name="sect-4.4.2"></a>
### Interface-ful IP-VRF-to-IP-VRF with SBD IRB

[Figure 9](#fig-6) depicts the Interface-ful IP-VRF-to-IP-VRF with SBD IRB model.
...
1. As in [Section 4.4.1](#sect-4.4.1), the NVEs and DGWs must provide connectivity
	  between hosts in SN1, SN2, and IP10 and in hosts sitting at the other end
	  of the WAN.
2. However, the NVE/DGWs are now connected through Ethernet NVO
	  tunnels terminated in the SBD instance. The IP-VRFs use IRB
	  interfaces for their connectivity to the SBD.
3. Each SBD IRB has an IP and a MAC address, where the IP address
	  must be reachable from other NVEs or DGWs.
4. The SBD is attached to all the NVE/DGWs in the tenant domain BDs.
5. The solution must provide Layer 3 connectivity for Ethernet NVO
              ...
              Route type 5 (IP Prefix route) containing:
                ...
                Route type 2 (MAC/IP Advertisement route for the SBD IRB) containing:
                  ML = 48, M = M1, IPL = 32, IP = IP1, Label = 10.
                    A BGP Encapsulation Extended Community [RFC9012](#RFC9012).
...
[Figure 10](#fig-7) depicts the Interface-ful IP-VRF-to-IP-VRF with unnumbered SBD IRB model. Note that this model is similar to the one described in [Section 4.4.2](#sect-4.4.2), only
...
1. As in Sections [4.4.1](#sect-4.4.1) and [4.4.2](#sect-4.4.2), the NVEs and DGWs must provide
	connectivity between hosts in SN1, SN2, and IP1 and in hosts sitting at the
	other end of the WAN.
2. As in [Section 4.4.2](#sect-4.4.2), the NVE/DGWs are connected through Ethernet
	NVO tunnels terminated in the SBD instance. The IP-VRFs use IRB
	interfaces for their connectivity to the SBD.
3. However, each SBD IRB has a MAC address only and no IP address
...
4. As in [Section 4.4.2](#sect-4.4.2), the SBD is composed of all the NVE/DGW BDs of
	the tenant that need inter-subnet forwarding.
5. As in [Section 4.4.2](#sect-4.4.2), the solution must provide Layer 3 connectivity
...
Each NVE/DGW will advertise an RT-5 for each of its prefixes with the
   same fields as described in [Section 4.4.2](#sect-4.4.2), except:
...
The example is similar to the one in [Section 4.4.2](#sect-4.4.2):

1. NVE1 advertises the following BGP routes:
              Route type 5 (IP Prefix route) containing the same values as
          in the example in [Section 4.4.2](#sect-4.4.2), except:
	  ...
	  for the recursive lookup to an RT-2).
          ...
          as in [Section 4.4.2](#sect-4.4.2), except:
...
The model described above is called an "interface-ful with unnumbered SBD
   IRB" model (as in [Section 4.4.2](#sect-4.4.2)) but without the SBD IRB having an IP address.
...
This document provides a set of procedures to achieve inter-subnet
   ...
   [RFC7432](#RFC7432) apply to the intra-subnet forwarding or communication
   within each of those BDs. In addition, the security considerations in
   [RFC4364](#RFC4364) should also be understood, since this document and
   [RFC4364](#RFC4364) may be used in similar applications.

Contrary to [RFC4364](#RFC4364), this document does not describe PE/CE route
...
IANA has registered value 5 in the "EVPN Route Types" registry [EVPNRouteTypes](#EVPNRouteTypes)
      defined by [RFC7432](#RFC7432) as follows:
...
<a name="EVPNRouteTypes"></a> **[EVPNRouteTypes]** "EVPN Route Types", <https://www.iana.org/assignments/evpn>
...
<a name="IEEE-802.1Q"></a> **[IEEE-802.1Q]** "IEEE Standard for Local and Metropolitan Area Networks -- Bridges and Bridged Networks", July 2018. <https://standards.ieee.org/standard/802_1Q-2018.html>
...
<a name="RFC5227"></a> **[RFC5227]** S. Cheshire, "IPv4 Address Conflict Detection", RFC 5227, DOI 10.17487/RFC5227, July 2008. <https://www.rfc-editor.org/info/rfc5227> [Local MD](rfc5227.md)

<a name="RFC5798"></a> **[RFC5798]** S. Nadas, "Virtual Router Redundancy Protocol (VRRP) Version 3 for IPv4 and IPv6", RFC 5798, DOI 10.17487/RFC5798, March 2010. <https://www.rfc-editor.org/info/rfc5798> [Local MD](rfc5798.md)
...
The authors would like to thank Mukul Katiyar, Jeffrey Zhang, and Alex Nichol for
   their valuable feedback and contributions. Tony Przygienda
   and Thomas Morin also helped improve this document with their feedback. Special thanks to Eric Rosen for his detailed
...
- Organization: Nokia
- Address: 777 E. Middlefield Road, Mountain View, CA, 94043, United States of America
- Email: jorge.rabadan@nokia.com
