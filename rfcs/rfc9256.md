...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-spring-segment-routing-policy-22
...
**Keywords:** Segment Routing, SR Policy, SR-MPLS, SRv6, Traffic Engineering

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-spring-segment-routing-policy-22)
- [alternate](https://dx.doi.org/10.17487/rfc9256)
...
- [1. Introduction](#Introduction)
  - [1.1. Requirements Language](#reqlang)
- [2. SR Policy](#SR-policy)
  - [2.1. Identification of an SR Policy](#SR-policy-identification)
  - [2.2. Candidate Path and Segment List](#SR-policy-candidate-path)
  - [2.3. Protocol-Origin of a Candidate Path](#SR-policy-protocol-origin)
  - [2.4. Originator of a Candidate Path](#SR-policy-candidate-path-originator)
  - [2.5. Discriminator of a Candidate Path](#SR-policy-candidate-path-discriminator)
  - [2.6. Identification of a Candidate Path](#SR-policy-candidate-path-identification)
  - [2.7. Preference of a Candidate Path](#SR-policy-candidate-path-preference)
  - [2.8. Validity of a Candidate Path](#SR-policy-candidate-path-validity)
  - [2.9. Active Candidate Path](#SR-policy-candidate-path-active)
  - [2.10. Validity of an SR Policy](#SR-policy-validity)
  - [2.11. Instantiation of an SR Policy in the Forwarding Plane](#SR-policy-instantiation)
  - [2.12. Priority of an SR Policy](#SR-policy-priority)
  - [2.13. Summary](#SR-policy-summary)
- [3. Segment Routing Database](#SR-database)
- [4. Segment Types](#SID-List)
  - [4.1. Explicit Null](#Explicit-Null)
- [5. Validity of a Candidate Path](#Path-validity)
  - [5.1. Explicit Candidate Path](#Path-validity-explicit)
  - [5.2. Dynamic Candidate Path](#Path-validity-dynamic)
  - [5.3. Composite Candidate Path](#Path-validity-composite)
- [6. Binding SID](#Binding-SID)
  - [6.1. BSID of a Candidate Path](#Binding-SID-candidate-path)
  - [6.2. BSID of an SR Policy](#Binding-SID-sr-policy)
  - [6.3. Forwarding Plane](#Binding-SID-forwarding)
  - [6.4. Non-SR Usage of Binding SID](#BSID-to-tunnel)
- [7. SR Policy State](#Policy-state)
- [8. Steering into an SR Policy](#Steering)
  - [8.1. Validity of an SR Policy](#Steering-validity)
  - [8.2. Drop-upon-Invalid SR Policy](#Steering-drop)
  - [8.3. Incoming Active SID is a BSID](#Steering-Incoming-BSID)
  - [8.4. Per-Destination Steering](#Steering-per-destination)
  - [8.5. Recursion on an On-Demand Dynamic BSID](#Steering-per-dynamic-BSID)
  - [8.6. Per-Flow Steering](#Steering-per-flow)
  - [8.7. Policy-Based Routing](#Steering-policy-based)
  - [8.8. Optional Steering Modes for BGP Destinations](#Steering-optional-bgp)
- [9. Recovering from Network Failures](#protection)
  - [9.1. Leveraging TI-LFA Local Protection of the Constituent IGP Segments](#Local-protection-tilfa)
  - [9.2. Using an SR Policy to Locally Protect a Link](#Local-protection-policy)
  - [9.3. Using a Candidate Path for Path Protection](#cp-path-protection)
- [10. Security Considerations](#Security)
- [11. Manageability Considerations](#MGMT)
- [12. IANA Considerations](#IANA)
  - [12.1. Guidance for Designated Experts](#guidance)
...
Segment Routing (SR) [RFC8402](#RFC8402) allows a node to steer
...
A Segment Routing Policy (SR Policy) [RFC8402](#RFC8402) is an
      ...
      segments associated with that SR Policy written into them. [RFC8660](#RFC8660) describes the representation and processing of this
      ordered list of segments as an MPLS label stack for SR-MPLS, while [RFC8754](#RFC8754) and [RFC8986](#RFC8986) describe the same for
      Segment Routing over IPv6 (SRv6) with the use of the Segment Routing
      Header (SRH).

[RFC8402](#RFC8402) introduces the SR Policy construct and
      provides an overview of how it is leveraged for Segment Routing
      use cases. This document updates [RFC8402](#RFC8402) to specify
...
The general concept of SR Policy provides a framework that enables
      ...
      specific purpose (e.g., for a specific Service Level Agreement (SLA)) from that node.

The Segment Routing architecture [RFC8402](#RFC8402) specifies
...
<a name="SR-policy-identification"></a>
...
An SR Policy **MUST** be identified through the tuple <Headend,
        ...
        Policy **MUST** be identified by the <Color, Endpoint> tuple.

The headend is the node where the policy is
        instantiated/implemented. The headend is specified as an IPv4 or IPv6
        address and **MUST** resolve to a unique node in the SR domain [RFC8402](#RFC8402).

The endpoint indicates the destination of the policy. The endpoint
        is specified as an IPv4 or IPv6 address and **SHOULD** resolve to a unique
        node in the domain. In a specific case (refer to [Section 8.8.1](#Steering-optional-bgp-color-only-steering)), the endpoint
...
The endpoint and the color are used to automate the steering of
        service or transport routes on SR Policies (refer to [Section 8](#Steering)).

An implementation **MAY** allow the assignment of a symbolic name
        comprising printable ASCII [RFC0020](#RFC0020) characters (i.e.,
        ...
        SR Policy name **MAY** also be signaled along with a candidate path of the
        SR Policy (refer to [Section 2.2](#SR-policy-candidate-path)). An SR
...
<a name="SR-policy-candidate-path"></a>
...
An SR Policy is associated with one or more candidate paths. A
        ...
        Communication Protocol (PCEP) [RFC8664](#RFC8664) [PCEP-SR-POLICY-CP](#I-D.ietf-pce-segment-routing-policy-cp) or BGP SR Policy
        [BGP-SR-POLICY](#I-D.ietf-idr-segment-routing-te-policy).
...
An explicit candidate path is expressed as a segment list or a set
        of segment lists.
...
If a candidate path is associated with a set of segment lists, each
        ...
        (refer to [Section 2.11](#SR-policy-instantiation) for details). The
        default weight is 1.
...
- The colors of each of the constituent SR Policies and the
            parent SR Policy **MUST** be different.
- The constituent SR Policies **MUST NOT** use composite candidate
            paths.

Each constituent SR Policy of a composite candidate path is
        associated with weight for load-balancing purposes (refer to [Section 2.11](#SR-policy-instantiation) for details). The default weight is
        1.

[Section 2.13](#SR-policy-summary) illustrates an information
        model for hierarchical relationships between the SR Policy constructs
        described in this section.

<a name="SR-policy-protocol-origin"></a>
...
A headend may be informed about a candidate path for an SR Policy
        ...
        PCEP [RFC8664](#RFC8664) [PCEP-SR-POLICY-CP](#I-D.ietf-pce-segment-routing-policy-cp), or BGP [BGP-SR-POLICY](#I-D.ietf-idr-segment-routing-te-policy).

Protocol-Origin of a candidate path is an 8-bit value associated
...
The headend assigns different Protocol-Origin values to each
        ...
        described in [Section 2.9](#SR-policy-candidate-path-active). The
        table below specifies the **RECOMMENDED** default values of
        Protocol-Origin:

**Table: Protocol-Origin Default Values**
...
Note that the above order is to satisfy the need for having a clear
        ...
        selection is described in [Section 2.9](#SR-policy-candidate-path-active).

<a name="SR-policy-candidate-path-originator"></a>
...
When signaling is via BGP SR Policy, the ASN and node address are
        provided by BGP (refer to [BGP-SR-POLICY](#I-D.ietf-idr-segment-routing-te-policy)) on the headend.

<a name="SR-policy-candidate-path-discriminator"></a>
...
The Discriminator is a 32-bit value associated with a candidate
        ...
        from a specific Protocol-Origin as specified below:
...
- When signaling is via PCEP, the method to uniquely signal an
            ...
            described in [PCEP-SR-POLICY-CP](#I-D.ietf-pce-segment-routing-policy-cp). The default
            value is 0.
- When signaling is via BGP SR Policy, the BGP process receiving
          the route provides the distinguisher (refer to [BGP-SR-POLICY](#I-D.ietf-idr-segment-routing-te-policy)) as the Discriminator. Note that
          ...
          Policy will be seen for a given Discriminator.

Its application in the candidate path selection is described in
        [Section 2.9](#SR-policy-candidate-path-active).

<a name="SR-policy-candidate-path-identification"></a>
...
The identity of a candidate path **MUST** be uniquely established in
...
The tuple <Protocol-Origin, Originator, Discriminator>
        uniquely identifies a candidate path.

Candidate paths **MAY** also be assigned or signaled with a symbolic
        name comprising printable ASCII [RFC0020](#RFC0020) characters
        (i.e., 0x20 to 0x7E) to serve as a user-friendly attribute for
        debugging and troubleshooting purposes. Such symbolic names **MUST NOT**
        be considered as identifiers for a candidate path. The signaling of
        the candidate path name via BGP and PCEP is described in [BGP-SR-POLICY](#I-D.ietf-idr-segment-routing-te-policy) and [PCEP-SR-POLICY-CP](#I-D.ietf-pce-segment-routing-policy-cp), respectively.

<a name="SR-policy-candidate-path-preference"></a>
...
It is **RECOMMENDED** that each candidate path of a given SR Policy has
        a different Preference.

The signaling of the candidate path Preference via BGP and PCEP is
        described in [BGP-SR-POLICY](#I-D.ietf-idr-segment-routing-te-policy) and [PCEP-SR-POLICY-CP](#I-D.ietf-pce-segment-routing-policy-cp),
        respectively.

<a name="SR-policy-candidate-path-validity"></a>
...
A candidate path is usable when it is valid. The **RECOMMENDED**
        ...
        [Section 5](#Path-validity).

<a name="SR-policy-candidate-path-active"></a>
## Active Candidate Path

A candidate path is selected when it is valid and it is determined
...
The rules are framed with multiple protocols and sources in mind
...
- The Discriminator performs the final tie-breaking step to ensure
...
Only the active candidate path **MUST** be used for forwarding traffic
        ...
        described in [Section 9.3](#cp-path-protection).
...
If a set of segment lists is associated with the active path of the
        ...
        according to the relative weight of each segment list.
...
Upon topological change, many policies could be re-computed or
        ...
        is the highest priority. The default value of priority is 128.

An SR Policy may comprise multiple candidate paths received from
        the same or different sources. A candidate path **MAY** be signaled with a
        ...
        signaled priority values.

<a name="SR-policy-summary"></a>
## Summary
...
An SR Policy computation node (e.g., headend or controller) maintains
      ...
      requirement for an implementation to create a new database as such.
...
- IGP information (topology, IGP metrics based on IS-IS [RFC1195](#RFC1195) and OSPF [RFC2328](#RFC2328) [RFC5340](#RFC5340))
- Segment Routing information (such as Segment Routing Global
          Block, Segment Routing Local Block, Prefix-SIDs, Adj-SIDs, BGP
          Peering SID, SRv6 SIDs) [RFC8402](#RFC8402) [RFC8986](#RFC8986)
- TE Link Attributes (such as TE metric, Shared Risk Link Groups,
          attribute-flag, extended admin group) [RFC5305](#RFC5305) [RFC3630](#RFC3630) [RFC5329](#RFC5329)
- Extended TE Link attributes (such as latency, loss) [RFC8570](#RFC8570) [RFC7471](#RFC7471)
- Inter-AS Topology information [RFC9086](#RFC9086)

The attached domain topology may be learned via protocol/mechanisms
      such as IGP, Border Gateway Protocol - Link State (BGP-LS), or NETCONF.

A non-attached (remote) domain topology may be learned via
      protocol/mechanisms such as BGP-LS or NETCONF.
...
In some use cases, the SR-DB may only contain the attached domain
      ...
      domains and in this case, it is multi-domain capable.

The SR-DB may also contain the SR Policies instantiated in the
      network. This can be collected via BGP-LS [BGP-LS-TE-POLICY](#I-D.ietf-idr-te-lsp-distribution) or PCEP
      [RFC8231](#RFC8231) (along with [PCEP-SR-POLICY-CP](#I-D.ietf-pce-segment-routing-policy-cp) and
      [PCEP-BSID-LABEL](#I-D.ietf-pce-binding-label-sid)). This
      information allows to build an end-to-end policy on the basis of
      intermediate SR policies (see [Section 6](#Binding-SID) for further details).

The SR-DB may also contain the Maximum SID Depth (MSD) capability of
      nodes in the topology. This can be collected via IS-IS [RFC8491](#RFC8491), OSPF [RFC8476](#RFC8476), BGP-LS [RFC8814](#RFC8814), or PCEP [RFC8664](#RFC8664).
...
The use of the SR-DB for path computation and for the validation of
      ...
      are covered in [SR-POLICY-CONSID](#I-D.filsfils-spring-sr-policy-considerations).

<a name="SID-List"></a>
# Segment Types

A segment list is an ordered set of segments represented as <S1,
      S2, ... Sn> where S1 is the first segment.

Based on the desired data plane, either the MPLS label stack or the
      SRv6 Segment Routing Header [RFC8754](#RFC8754) is built from the
...
**Type A: SR-MPLS Label:**
  An MPLS label
            ...
            defined in [RFC8402](#RFC8402) or other SR-MPLS specifications)
            can be used. Additionally, special purpose labels like explicit-null
            or in general any MPLS label **MAY** also be used. For example, this type can be
            used to specify a label representation that maps to an optical
            transport path on a packet transport node.

**Type B: SRv6 SID:**
  An IPv6 address
            corresponding to any of the SID behaviors for SRv6 (as defined in
            [RFC8986](#RFC8986) or other SRv6 specifications) can be used.
            Optionally, the SRv6 SID behavior (as defined in [RFC8986](#RFC8986) or other SRv6 specifications) and structure (as
            defined in [RFC8986](#RFC8986)) **MAY** also be provided for the
...
**Type C: IPv4 Prefix with optional SR Algorithm:**
  In
            ...
            segment (as defined in [RFC8402](#RFC8402)). The SR algorithm
            (refer to [RFC8402](#RFC8402)) to be used **MAY**
            also be provided.
...
**Type D: IPv6 Global Prefix with optional SR Algorithm for SR-MPLS:**
  In
            ...
            Prefix SID segment (as defined in [RFC8402](#RFC8402)). The SR
            Algorithm (refer to [RFC8402](#RFC8402)) to be
...
**Type E: IPv4 Prefix with Local Interface ID:**
  This
            ...
            SID (as defined in [RFC8402](#RFC8402)) SR-MPLS label for
            ...
            Interface ID link descriptor follows semantics as specified in [RFC5307](#RFC5307). This type can also be used to indicate
            ...
            port or circuit at the specified node.
...
**Type G: IPv6 Prefix and Interface ID for link endpoints as Local, Remote pair for SR-MPLS:**
  This
            ...
            descriptor follows semantics as specified in [RFC7752](#RFC7752). This type can also be used to indicate
            indirection into a layer 2 interface (i.e., without IP address) like
            a representation of an optical transport path or a layer 2 Ethernet
            port or circuit at the specified node.
...
**Type H: IPv6 Addresses for link endpoints as Local, Remote pair for SR-MPLS:**
  This
            type allows for identification of an Adjacency SID or BGP Peer Adjacency
            SID (as defined in [RFC8402](#RFC8402)) label for links with
            ...
            follow semantics as specified in [RFC7752](#RFC7752).
...
**Type I: IPv6 Global Prefix with optional SR Algorithm for SRv6:**
  The
            ...
            defined in [RFC8402](#RFC8402)), such as a SID associated with
            the End behavior (as defined in [RFC8986](#RFC8986)) of the
            node that is originating the prefix. The SR Algorithm (refer to
            [RFC8402](#RFC8402)), the SRv6 SID behavior
            (as defined in [RFC8986](#RFC8986) or other SRv6
            specifications), and structure (as defined in [RFC8986](#RFC8986)) **MAY** also be provided.
...
**Type J: IPv6 Prefix and Interface ID for link endpoints as Local, Remote pair for SRv6:**
  This
            ...
            Adjacency SID or BGP Peer Adjacency SID (as defined in [RFC8402](#RFC8402)), such as a SID associated with the End.X
            behavior (as defined in [RFC8986](#RFC8986)) associated with
            ...
            semantics as specified in [RFC7752](#RFC7752). The SR Algorithm
            (refer to [RFC8402](#RFC8402)), the SRv6 SID
            behavior (as defined in [RFC8986](#RFC8986) or other SRv6
            specifications), and structure (as defined in [RFC8986](#RFC8986)) **MAY** also be provided.
...
**Type K: IPv6 Addresses for link endpoints as Local, Remote pair for SRv6:**
  This type allows for identification of an SRv6 SID corresponding to
            an Adjacency SID or BGP Peer Adjacency SID (as defined in [RFC8402](#RFC8402)), such as a SID associated with
            the End.X behavior (as defined in [RFC8986](#RFC8986)) associated with link or adjacency with Global
            ...
            semantics as specified in [RFC7752](#RFC7752). The SR Algorithm (refer to [RFC8402](#RFC8402)), the SRv6 SID behavior (as defined in [RFC8986](#RFC8986) or other SRv6 specifications),
            and structure (as defined in [RFC8986](#RFC8986)) **MAY** also be provided.


When the algorithm is not specified for the SID types above which
      optionally allow for it, the headend **SHOULD** use the Strict Shortest Path
      algorithm if available and otherwise, it **SHOULD** use the default Shortest
      Path algorithm.

      The specification of the algorithm enables the use of SIDs specific to
      the IGP Flex Algorithm [IGP-FLEX-ALGO](#I-D.ietf-lsr-flex-algo) in SR Policy.

For SID types C through K, a SID value **MAY** also be optionally
      provided to the headend for verification purposes. [Section 5.1](#Path-validity-explicit) describes the resolution and
      verification of the SIDs and segment lists on the headend.
...
<a name="Explicit-Null"></a>
## Explicit Null

A Type A SID **MAY** be any MPLS label, including special purpose
        labels.

For example, assuming that the desired traffic-engineered path from
...
When steering unlabeled IPv6 BGP destination traffic using an SR
        ...
        Null Label Policy is processed as specified in [BGP-SR-POLICY](#I-D.ietf-idr-segment-routing-te-policy). When
        an "IPv6 Explicit NULL label" is not present as the bottom
        label, the headend **SHOULD** automatically impose one. Refer to [Section 8](#Steering) for more details.

<a name="Path-validity"></a>
# Validity of a Candidate Path

<a name="Path-validity-explicit"></a>
## Explicit Candidate Path

An explicit candidate path is associated with a segment list or a
        set of segment lists.

An explicit candidate path is provisioned by the operator directly
        or via a controller.
...
An explicit candidate path **MAY** consist of a single explicit
        ...
        Policy using its BSID (refer to [Section 8.3](#Steering-Incoming-BSID)).

A segment list of an explicit candidate path **MUST** be declared
        invalid when any of the following is true:

- It is empty.
- Its weight is 0.
- It comprises a mix of SR-MPLS and SRv6 segment types.
- The headend is unable to perform path resolution for the first
            SID into one or more outgoing interface(s) and next-hop(s).
...
- The headend detects a mismatch between the SID value provided
            and the SID value resolved by context provided for SIDs of type
            C through K in its SR-DB.
- The headend is unable to perform SID resolution for any
            non-first SID of type C through K into an MPLS label or an SRv6
            SID.
...
In multi-domain deployments, it is expected that the headend may be
        ...
        A or B **MUST** be used for the SIDs for which the reachability cannot be
        verified. Note that the first SID **MUST** always be reachable regardless
        of its type.

Additionally, a segment list **MAY** be declared invalid when both of
        the conditions below are met :

- Its last segment is not a Prefix SID (including BGP Peer
...
Additionally, an explicit candidate path **MAY** be declared invalid
        ...
        segment types of different SR data planes.

<a name="Path-validity-dynamic"></a>
...
The headend re-computes the solution segment list any time the
        inputs to the problem change (e.g., topology changes).
...
<a name="Path-validity-composite"></a>
## Composite Candidate Path

A composite candidate path is specified as a group of its
        constituent SR Policies.

A composite candidate path is valid when it has at least one valid
        constituent SR Policy.

<a name="Binding-SID"></a>
# Binding SID

The Binding SID (BSID) is fundamental to Segment Routing [RFC8402](#RFC8402). It provides scaling, network opacity, and service
      independence. [SR-POLICY-CONSID](#I-D.filsfils-spring-sr-policy-considerations) illustrates some
      of these benefits. This section describes the association of BSID with
      an SR Policy.

<a name="Binding-SID-candidate-path"></a>
## BSID of a Candidate Path

Each candidate path **MAY** be defined with a BSID.

Candidate paths of the same SR Policy **SHOULD** have the same
        BSID.

Candidate paths of different SR Policies **MUST NOT** have the same
        BSID.

<a name="Binding-SID-sr-policy"></a>
...
In the case of SR-MPLS, SRv6 BSIDs (e.g., with the behavior End.BM
        [RFC8986](#RFC8986)) **MAY** be associated with the SR Policy in
        ...
        [RFC8986](#RFC8986)) **MAY** be associated with the SR Policy.

Optionally, instead of only checking that the BSID of the active
        path is available, a headend **MAY** check that it is available within the
...
When the specified BSID is not available (optionally is not in the
        SRLB), an alert message **MUST** be generated via mechanisms like
        syslog.

In the cases (as described above) where SR Policy does not have a
        BSID available, the SR Policy **MAY** dynamically bind a BSID to
        itself. Dynamically bound BSIDs **SHOULD** use an available SID outside the
        SRLB.
...
<a name="Binding-SID-sr-policy-unspecified-bsid"></a>
### Frequent Use Case : Unspecified BSID

All the candidate paths of the same SR Policy can have an
          unspecified BSID.

In such a case, a BSID **MAY** be dynamically bound to the SR Policy
...
<a name="Binding-SID-policy-same-bsid"></a>
### Frequent Use Case: All Specified to the Same BSID

All the paths of the SR Policy can have the same specified
          BSID.

<a name="Binding-SID-specified-bsid"></a>
### Specified-BSID-only
...
An implementation **MAY** support the configuration of the
          ...
          behavior **MAY** also be signaled on a per-SR-Policy basis to the
          headend.

When this restrictive behavior is enabled, if the candidate path
          ...
          and the candidate path is considered invalid. An alert **MUST** be
          triggered for this error via mechanisms like syslog. Other candidate
          paths **MUST** then be evaluated for becoming the active candidate
          path.

<a name="Binding-SID-forwarding"></a>
## Forwarding Plane
...
An implementation **MAY** choose to associate a Binding SID with any
        ...
        of forming routing protocol adjacencies over them.
...
The details of this kind of usage are beyond the scope of this
        ...
        in [POI-SR](#I-D.anand-spring-poi-sr).

<a name="Policy-state"></a>
# SR Policy State
...
The SR Policy state is maintained on the headend to represent the
      ...
      segment list(s) are active. The SR Policy state **MUST** also reflect the
      ...
      information reported for SR Policies are specified in [SR-POLICY-YANG](#I-D.ietf-spring-sr-policy-yang).

The SR Policy state can be reported by the headend node via BGP-LS
      [BGP-LS-TE-POLICY](#I-D.ietf-idr-te-lsp-distribution) or PCEP [RFC8231](#RFC8231) [PCEP-BSID-LABEL](#I-D.ietf-pce-binding-label-sid).
...
SR Policy state on the headend also includes traffic accounting
      ...
      [SR-TRAFFIC-COUNTERS](#I-D.filsfils-spring-sr-traffic-counters) and [SR-TRAFFIC-ACCOUNTING](#I-D.ali-spring-sr-traffic-accounting), respectively.

Implementations **MAY** support an administrative state to control
      locally provisioned policies via mechanisms like command-line interface (CLI) or NETCONF.
...
- any steering of a service (Pseudowire (PW)), destination
            ...
            destination or the recursing next-hop).

<a name="Steering-drop"></a>
...
- an invalid SR Policy and its BSID is kept in the forwarding
            plane with an action to drop.
...
The Drop-Upon-Invalid behavior has been deployed in use cases
...
<a name="Steering-Incoming-BSID"></a>
## Incoming Active SID is a BSID

Let us assume that headend H has a valid SR Policy P of
        segment list <S1, S2, S3> and BSID B.

In the case of SR-MPLS, when H receives a packet K with label stack
        <B, L2, L3>, H pops B and pushes <S1, S2, S3> and forwards
        the resulting packet according to SID S1.

In the case of SRv6, the processing is similar and follows the SR
        Policy headend behaviors as specified in [RFC8986](#RFC8986).
...
This is another key benefit of the segment routing in general and
...
When Drop-Upon-Invalid (refer to [Section 8.2](#Steering-drop)) is
        not in use, for an invalid SR Policy P, its BSID B is not in the
        forwarding plane and hence, the packet K is dropped by H.

<a name="Steering-per-destination"></a>
...
This section describes how a headend applies steering of flows
        ...
        community [RFC9012](#RFC9012).

In the case of SR-MPLS, let us assume that headend H:

- learns a BGP route R/r via next-hop N, Color Extended community
            C, and VPN label V.
- has a valid SR Policy P to (color = C, endpoint = N) of
            segment list <S1, S2, S3> and BSID B.
- has a BGP policy that matches on the Color Extended community C
            and allows its usage as SLA steering information.

If all these conditions are met, H installs R/r in RIB/FIB with
        next-hop = SR Policy P of BSID B instead of via N.

Indeed, H's local BGP policy and the received BGP route indicate
        ...
        therefore, installs the BGP route on that policy.
...
When H receives a packet K with a destination matching R/r, H
        pushes the label stack <S1, S2, S3, V> and sends the resulting
        packet along the path to S1.

Note that any SID associated with the BGP route is inserted after
        the segment list of the SR Policy (i.e., <S1, S2, S3, V>).

In the case of SRv6, the processing is similar and follows the SR
        Policy headend behaviors as specified in [RFC8986](#RFC8986).

The same behavior applies to any type of service route: any
        AFI/SAFI of BGP [RFC4760](#RFC4760) or the Locator/ID Separation Protocol (LISP) [RFC6830](#RFC6830) for both IPv4/IPv6.
...
In a BGP multi-path scenario, the BGP route **MAY** be resolved over a
        ...
        **MAY** provide options to prefer one type over the other or other forms
        of local policy to determine the paths that are selected.

<a name="Steering-per-destination-colors"></a>
### Multiple Colors
...
- learns a BGP route R/r via next-hop N, Color Extended
              communities C1 and C2.
- has a valid SR Policy P1 to (color = C1, endpoint = N) of
              segment list <S1, S2, S3> and BSID B1.
- has a valid SR Policy P2 to (color = C2, endpoint = N) of
              segment list <S4, S5, S6> and BSID B2.
- has a BGP policy that matches the Color Extended communities
              C1 and C2 and allows their usage as SLA steering information

If all these conditions are met, H installs R/r in RIB/FIB
          with next-hop = SR Policy P2 of BSID=B2 (instead of N) because C2
          > C1.

When the SR Policy with a specific color is not instantiated or
          in the down/inactive state, the SR Policy with the next highest
          numerical value of color is considered.

<a name="Steering-per-dynamic-BSID"></a>
## Recursion on an On-Demand Dynamic BSID

In the previous section, it was assumed that H had a
        pre-established "explicit" SR Policy (color C, endpoint N).
...
In this section, independent of the a priori existence of any
        ...
        dynamic candidate path for the SR Policy (C, N) as soon as:

- the BGP process learns of a route R/r via N and with Color
            Extended community C.
- a local policy at node H authorizes the on-demand SR Policy
            path instantiation and maps the color to a dynamic SR Policy path
            optimization template.

<a name="Steering-per-dynamic-BSID-color"></a>
### Multiple Colors

When a BGP route R/r via N has multiple Color Extended
          ...
          as described in [Section 8.4.1](#Steering-per-destination-colors).

<a name="Steering-per-flow"></a>
## Per-Flow Steering

This section provides an example of how a headend might apply
        per-flow steering in practice.
...
- is configured to instantiate an array of paths to N where the
            ...
            especially when derived from the MPLS TC bits [RFC5462](#RFC5462).
- is configured to match flows in its ingress interfaces (upon
...
- N via recursion on an array A (instead of the immediate
            outgoing link associated with the IGP shortest path to N).
- Entry A(0) set to the immediate outgoing link of the IGP
            shortest path to N.
...
- H forwards K along the shortest path to N (i.e., pushes the
            Prefix-SID of N).
- H pushes <S1, S2, S3> on packet K1 and forwards the
            resulting frame along the shortest path to S1.
- H pushes <S4, S5, S6> on packet K2 and forwards the
            resulting frame along the shortest path to S4.
...
For SRv6, the processing is similar and the segment lists of the
        ...
        using the SR Policy headend behaviors as specified in [RFC8986](#RFC8986).
...
<a name="Steering-policy-based"></a>
...
<a name="Steering-optional-bgp"></a>
## Optional Steering Modes for BGP Destinations

<a name="Steering-optional-bgp-color-only-steering"></a>
### Color-Only BGP Destination Steering

In the previous section, it is seen that the steering on an SR
...
This is the most likely form of BGP destination steering and the
          one recommended for most use cases.

This section defines an alternative steering mechanism based only
          on the Color Extended community.
...
This is the classic case described in this document previously
          and what is recommended in most scenarios.
...
The null endpoint is 0.0.0.0 for IPv4 and :: for IPv6 (all bits
          set to the 0 value).

Please refer to [BGP-SR-POLICY](#I-D.ietf-idr-segment-routing-te-policy) for the updates to
          the BGP Color Extended community for the implementation of these
          mechanisms.

<a name="Steering-optional-bgp-multi-color"></a>
### Multiple Colors and CO flags
...
<a name="Steering-optional-bgp-drop-on-invalid"></a>
### Drop-upon-Invalid

This document defined earlier that when all the following
          conditions are met, H installs R/r in RIB/FIB with next-hop = SR
          Policy P of BSID B instead of via N.

- H learns a BGP route R/r via next-hop N, Color Extended
              community C.
- H has a valid SR Policy P to (color = C, endpoint = N) of
              segment list <S1, S2, S3> and BSID B.
- H has a BGP policy that matches the Color Extended community
              C and allows its usage as SLA steering information.

This behavior is extended by noting that the BGP Policy may
          require the BGP steering to always stay on the SR Policy whatever
          its validity.

This is the "drop-upon-invalid" option described in [Section 8.2](#Steering-drop) applied to BGP-based steering.

<a name="protection"></a>
# Recovering from Network Failures

<a name="Local-protection-tilfa"></a>
...
In any topology, Topology-Independent Loop-Free Alternate (TI-LFA)
        [SR-TI-LFA](#I-D.ietf-rtgwg-segment-routing-ti-lfa) provides a
        50 msec local protection technique for IGP SIDs. The backup path is
        computed on a per-IGP-SID basis along the post-convergence path.
...
In a network that has deployed TI-LFA, an SR Policy instantiated
        only with non-protected Adj SIDs does not benefit from any local
        protection.

<a name="Local-protection-policy"></a>
...
```
An SR Policy can be instantiated at node 2 to protect link
        2-to-6. A typical explicit segment list would be <3, 9, 6>.
...
A typical use case occurs for links outside an IGP domain: e.g., 1,
        ...
        to be a fast-reroute backup path for the link 2-to-6.

<a name="cp-path-protection"></a>
...
- A specific candidate path is provisioned as the backup for any
            (active) candidate path.
- The headend picks the next (lower) preference valid candidate
            path as the backup for the active candidate path.
...
The headend **MAY** compute a priori and validate such backup candidate
        ...
        most appropriate alternative for the active candidate path. A fast-reroute mechanism **MAY** then be used to trigger sub-50 msec switchover
        from the active to the backup candidate path in the forwarding plane.
        Mechanisms like Bidirectional Forwarding Detection (BFD) **MAY** be used
        for fast detection of such failures.

<a name="Security"></a>
# Security Considerations

This document specifies in detail the SR Policy construct introduced
      in [RFC8402](#RFC8402) and its instantiation on a router supporting
      SR along with descriptions of mechanisms for the steering of traffic flows
      over it. Therefore, the security considerations of [RFC8402](#RFC8402) apply. The security consideration related to SR-MPLS
      [RFC8660](#RFC8660) and SRv6 [RFC8754](#RFC8754) [RFC8986](#RFC8986) also apply.
...
[Section 8](#Steering) specifies mechanisms for the steering of
      ...
      packets for those routes along undesired paths.

In Sections [2.1](#SR-policy-identification) and [2.6](#SR-policy-candidate-path-identification), the document
      mentions that a symbolic name **MAY** be signaled along with a candidate
...
This document specifies in detail the SR Policy construct introduced
      in [RFC8402](#RFC8402) and its instantiation on a router supporting
      SR along with descriptions of mechanisms for the steering of traffic flows
      over it. Therefore, the manageability considerations of [RFC8402](#RFC8402) apply.

A YANG model for the configuration and operation of SR Policy has
      been defined in [SR-POLICY-YANG](#I-D.ietf-spring-sr-policy-yang).

<a name="IANA"></a>
# IANA Considerations

IANA has created a new subregistry called
      "Segment Types" under the "Segment Routing" registry that was
      created by [RFC8986](#RFC8986). This subregistry maintains the
      alphabetic identifiers for the segment types (as specified in [Section 4](#SID-List))
      ...
      as specified in [RFC8126](#RFC8126).
...
| G | IPv6 Prefix and Interface ID for link endpoints as Local, Remote
        pair for SR-MPLS | RFC 9256 |
| H | IPv6 Addresses for link endpoints as Local, Remote pair for
        SR-MPLS | RFC 9256 |
| I | IPv6 Global Prefix with optional SR Algorithm for SRv6 | RFC 9256 |
| J | IPv6 Prefix and Interface ID for link endpoints as Local, Remote
        pair for SRv6 | RFC 9256 |
| K | IPv6 Addresses for link endpoints as Local, Remote pair for
        SRv6 | RFC 9256 |

<a name="guidance"></a>
## Guidance for Designated Experts

The Designated Expert (DE) is expected to ascertain the existence
        of suitable documentation (a specification) as described in [RFC8126](#RFC8126) and to verify that the document is permanently and
...
<a name="I-D.ietf-idr-te-lsp-distribution"></a> **[BGP-LS-TE-POLICY]** S Previdi, K Talaulikar, J Dong, M Chen, H Gredler, J Tantsura, "Distribution of Traffic Engineering (TE) Policies and State using BGP-LS", Internet-Draft draft-ietf-idr-te-lsp-distribution-17, April 2022. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-idr-te-lsp-distribution-17>

<a name="I-D.ietf-idr-segment-routing-te-policy"></a> **[BGP-SR-POLICY]** S Previdi, C Filsfils, K Talaulikar, P Mattes, D Jain, S Lin, "Advertising Segment Routing Policies in BGP", Internet-Draft draft-ietf-idr-segment-routing-te-policy-18, June 2022. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-idr-segment-routing-te-policy-18>

<a name="I-D.ietf-lsr-flex-algo"></a> **[IGP-FLEX-ALGO]** P Psenak, S Hegde, C Filsfils, K Talaulikar, A Gulko, "IGP Flexible Algorithm", Internet-Draft draft-ietf-lsr-flex-algo-20, May 2022. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-lsr-flex-algo-20>

<a name="I-D.ietf-pce-binding-label-sid"></a> **[PCEP-BSID-LABEL]** S Sivabalan, C Filsfils, J Tantsura, S Previdi, C Li, "Carrying Binding Label/Segment Identifier (SID) in PCE-based Networks.", Internet-Draft draft-ietf-pce-binding-label-sid-15, March 2022. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-pce-binding-label-sid-15>

<a name="I-D.ietf-pce-segment-routing-policy-cp"></a> **[PCEP-SR-POLICY-CP]** Mike Koldychev, Siva Sivabalan, Colby Barth, Shuping Peng, Hooman Bidgoli, "PCEP extension to support Segment Routing Policy Candidate Paths", Internet-Draft draft-ietf-pce-segment-routing-policy-cp-07, April 2022. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-pce-segment-routing-policy-cp-07>

<a name="I-D.anand-spring-poi-sr"></a> **[POI-SR]** Madhukar Anand, Sanjoy Bardhan, Ramesh Subrahmaniam, Jeff Tantsura, Utpal Mukhopadhyaya, Clarence Filsfils, "Packet-Optical Integration in Segment Routing", Internet-Draft draft-anand-spring-poi-sr-08, July 2019. Work in Progress <https://datatracker.ietf.org/doc/html/draft-anand-spring-poi-sr-08>

<a name="RFC0020"></a> **[RFC0020]** V.G. Cerf, "ASCII format for network interchange", STD 80, RFC 20, DOI 10.17487/RFC0020, October 1969. <https://www.rfc-editor.org/info/rfc20> [Local MD](rfc0020.md)

<a name="RFC1195"></a> **[RFC1195]** R.W. Callon, "Use of OSI IS-IS for routing in TCP/IP and dual environments", RFC 1195, DOI 10.17487/RFC1195, December 1990. <https://www.rfc-editor.org/info/rfc1195> [Local MD](rfc1195.md)
...
<a name="RFC5462"></a> **[RFC5462]** L. Andersson, R. Asati, "Multiprotocol Label Switching (MPLS) Label Stack Entry: "EXP" Field Renamed to "Traffic Class" Field", RFC 5462, DOI 10.17487/RFC5462, February 2009. <https://www.rfc-editor.org/info/rfc5462> [Local MD](rfc5462.md)
...
<a name="RFC8476"></a> **[RFC8476]** J. Tantsura, U. Chunduri, S. Aldrin, P. Psenak, "Signaling Maximum SID Depth (MSD) Using OSPF", RFC 8476, DOI 10.17487/RFC8476, December 2018. <https://www.rfc-editor.org/info/rfc8476> [Local MD](rfc8476.md)

<a name="RFC8491"></a> **[RFC8491]** J. Tantsura, U. Chunduri, S. Aldrin, L. Ginsberg, "Signaling Maximum SID Depth (MSD) Using IS-IS", RFC 8491, DOI 10.17487/RFC8491, November 2018. <https://www.rfc-editor.org/info/rfc8491> [Local MD](rfc8491.md)
...
<a name="RFC8814"></a> **[RFC8814]** J. Tantsura, U. Chunduri, K. Talaulikar, G. Mirsky, N. Triantafillis, "Signaling Maximum SID Depth (MSD) Using the Border Gateway Protocol - Link State", RFC 8814, DOI 10.17487/RFC8814, August 2020. <https://www.rfc-editor.org/info/rfc8814> [Local MD](rfc8814.md)
...
<a name="I-D.filsfils-spring-sr-policy-considerations"></a> **[SR-POLICY-CONSID]** C Filsfils, K Talaulikar, P Krol, M Horneffer, P Mattes, "SR Policy Implementation and Deployment Considerations", Internet-Draft draft-filsfils-spring-sr-policy-considerations-09, April 2022. Work in Progress <https://datatracker.ietf.org/doc/html/draft-filsfils-spring-sr-policy-considerations-09>

<a name="I-D.ietf-spring-sr-policy-yang"></a> **[SR-POLICY-YANG]** K Raza, S Sawaya, Z Shunwan, D Voyer, M Durrani, S Matsushima, V Beeram, "YANG Data Model for Segment Routing Policy", Internet-Draft draft-ietf-spring-sr-policy-yang-01, April 2021. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-spring-sr-policy-yang-01>

<a name="I-D.ietf-rtgwg-segment-routing-ti-lfa"></a> **[SR-TI-LFA]** Stephane Litkowski, Ahmed Bashandy, Clarence Filsfils, Pierre Francois, Bruno Decraene, Daniel Voyer, "Topology Independent Fast Reroute using Segment Routing", Internet-Draft draft-ietf-rtgwg-segment-routing-ti-lfa-08, January 2022. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ietf-rtgwg-segment-routing-ti-lfa-08>

<a name="I-D.ali-spring-sr-traffic-accounting"></a> **[SR-TRAFFIC-ACCOUNTING]** Z Ali, C Filsfils, K Talaulikar, S Sivabalan, M Horneffer, R Raszuk, S Litkowski, D Voyer, R Morton, G Dawra, "Traffic Accounting in Segment Routing Networks", Internet-Draft draft-ali-spring-sr-traffic-accounting-07, May 2022. Work in Progress <https://datatracker.ietf.org/doc/html/draft-ali-spring-sr-traffic-accounting-07>

<a name="I-D.filsfils-spring-sr-traffic-counters"></a> **[SR-TRAFFIC-COUNTERS]** C Filsfils, Z Ali, M Horneffer, D Voyer, M Durrani, R Raszuk, "Segment Routing Traffic Accounting Counters", Internet-Draft draft-filsfils-spring-sr-traffic-counters-02, October 2021. Work in Progress <https://datatracker.ietf.org/doc/html/draft-filsfils-spring-sr-traffic-counters-02>

<a name="Acknowledgement"></a>
# Acknowledgement

The authors would like to thank Tarek Saad,
      Dhanendra Jain, Ruediger       Geib, Rob Shakir, Cheng       Li, Dhruv Dhody, Gyan       Mishra, Nandan Saha, Jim       Guichard, Martin Vigoureux, Benjamin Schwartz, David Schinazi,
...
- Address: De kleetlaan 6a, Diegem, 1831, Belgium
- Email: cfilsfil@cisco.com
...
- Address: 671 de la gauchetiere W, Montreal, Quebec, H3B 2M8, Canada
- Email: daniel.voyer@bell.ca
...
- Address: One Microsoft Way, Redmond, WA, 98052-6399, United States of America
- Email: pamattes@microsoft.com
