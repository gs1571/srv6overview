...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-pce-association-group-10
...
**Keywords:** PCE, PCEP, Association, Group

## Related Documents

- [prev](https://datatracker.ietf.org/doc/draft-ietf-pce-association-group-10)
- [alternate](https://dx.doi.org/10.17487/rfc8697)
- [alternate](urn:issn:2070-1721)

## Abstract

This document introduces a generic mechanism to create a grouping of Label Switched Paths (LSPs) in the
...
- [3. Architectural Overview](#Overview)
  - [3.1. Motivations](#Motivation)
  - [3.2. Relationship to the SVEC List](#section-3.2)
  - [3.3. Operational Overview](#Operation-overview)
  - [3.4. Operator-Configured Association Range](#Range)
- [4. Discovery of Supported Association Types](#Discovery)
  - [4.1. ASSOC-Type-List TLV](#section-4.1)
    - [4.1.1. Procedure](#section-4.1.1)
- [5. Operator-Configured Association Range TLV](#Range-tlv)
  - [5.1. Procedure](#pro)
- [6. ASSOCIATION Object](#association)
  - [6.1. Object Definition](#Definition)
    - [6.1.1. Global Association Source TLV](#Global-association-src)
    - [6.1.2. Extended Association ID TLV](#EXT-association)
    - [6.1.3. Association Source](#AssocSrc)
    - [6.1.4. Unique Identification for an Association Group](#id)
  - [6.2. Relationship to the RSVP ASSOCIATION Object](#section-6.2)
  - [6.3. Object Encoding in PCEP Messages](#Object-Encoding)
    - [6.3.1. Stateful PCEP Messages](#section-6.3.1)
    - [6.3.2. Request Message](#section-6.3.2)
    - [6.3.3. Reply Message](#section-6.3.3)
  - [6.4. Processing Rules](#Rules)
- [7. IANA Considerations](#IANA-considerations)
  - [7.1. PCEP Object](#section-7.1)
  - [7.2. PCEP TLV](#section-7.2)
  - [7.3. Association Flags](#section-7.3)
  - [7.4. Association Type](#iana_assoc_type)
  - [7.5. PCEP-Error Object](#section-7.5)
...
- [Appendix A. Example of an Operator-Configured Association Range](#ex)
...
[RFC5440](#RFC5440) describes the Path Computation Element (PCE)
...
[RFC8231](#RFC8231) specifies a set of extensions to PCEP
      to enable stateful control of TE LSPs within and across PCEP sessions in
      compliance with [RFC4657](#RFC4657). It includes mechanisms to
      ...
      whereby LSPs are initiated from the PCE is described in [RFC8281](#RFC8281).

[RFC4872](#RFC4872) defines the RSVP ASSOCIATION object, which
      ...
      state. [RFC6780](#RFC6780) describes how the ASSOCIATION object can
...
This document introduces a generic mechanism to create a grouping of LSPs.
       ...
       on the PCEP peers. Refer to [Section 3.3](#Operation-overview) for
       more details.
...
This document uses the following terms defined in [RFC8051](#RFC8051):
...
This document uses the following terms defined in [RFC8231](#RFC8231):

- LSP State Report (PCRpt)
- LSP Update Request (PCUpd)
- State Timeout Interval

This document uses the following terms defined in [RFC8281](#RFC8281):
...
A stateful PCE provides the ability to update existing LSPs and to
     ...
     required (see [PCE-PROTECTION](#I-D.ietf-pce-stateful-path-protection)). For diverse paths, an
     association between a group of LSPs could be used (see [PCE-DIVERSITY](#I-D.ietf-pce-association-diversity)).  Another use for an LSP grouping would be to apply
...
Note that [RFC5440](#RFC5440) defines a mechanism for the
       ...
       (Request Parameters) object. [RFC6007](#RFC6007) further clarifies
...
state exists. In the case of PCEP session termination, the LSP state cleanup
    **MUST** also take care of associations.
...
Multiple types of associations can exist, each with its own
     ...
     the same Association Type or different Association Types.

<a name="Range"></a>
## Operator-Configured Association Range

Some Association Types are dynamic, some are operator configured,
...
A range of Association IDs for each Association Type (and
       Association Source) is kept for the Operator-configured
       Associations. Dynamic associations **MUST NOT** use the
       Association ID from this range.
...
This range, as set at the PCEP speaker (a PCC or PCE, as an
       ...
       purpose ([Section 5](#Range-tlv)). See [Appendix A](#ex) for an
       example.

The Association ID range for sources other than the PCEP
       speaker (for example, a Network Management System (NMS)) is not
       communicated in PCEP, and the procedure for Operator-configured
       Association Range settings is outside the scope of this document.

<a name="Discovery"></a>
# Discovery of Supported Association Types

This section defines PCEP extensions so as to support
   the capability advertisement of the Association Types supported by a PCEP speaker.
...
A new PCEP ASSOC-Type-List (Association Types list) TLV is defined.  The
   ...
   the list of supported Association Types.

## ASSOC-Type-List TLV

The PCEP ASSOC-Type-List TLV is **OPTIONAL**.  It **MAY** be carried within an OPEN
   object sent by a PCEP speaker in an Open message to a PCEP peer so as to
   indicate the list of supported Association Types.

The PCEP ASSOC-Type-List TLV format is compliant with the PCEP TLV format defined
   in [RFC5440](#RFC5440).  That is, the TLV is composed of 2 octets
   ...
   the total size of the TLV would be 8 octets).
...
**Value:**
  List of 2-byte Association Type code points, identifying
      the Association Types supported by the sender of the Open
      message.

**Assoc-Type (2 bytes):**
  Association Type code point identifier.
    IANA manages the "ASSOCIATION Type Field" code point registry (see [Section 7.4](#iana_assoc_type)).


### Procedure

An ASSOC-Type-List TLV within an OPEN object in an Open
   ...
   message). As specified in [RFC5440](#RFC5440), a PCEP peer that does not recognize the
   ASSOC-Type-List TLV will silently ignore it.
...
Thus, the ASSOC-Type-List TLV **MUST** be included if at least one mandatory
 Association Type needs to be advertised, and the ASSOC-Type-List TLV **MAY** be
 ...
 Association Type is not supported by the PCEP speaker.
...
If the use of the ASSOC-Type-List TLV is triggered by support for a
   ...
   types) to ease the operations of the PCEP peer.

<a name="Range-tlv"></a>
# Operator-Configured Association Range TLV

This section defines a PCEP extension to support
   the advertisement of the Operator-configured Association Range used for an Association Type by the PCEP speaker (as an Association Source).
...
The PCEP OP-CONF-ASSOC-RANGE TLV is **OPTIONAL**.  It **MAY** be carried within
   ...
   defined in [RFC5440](#RFC5440).  That is, the TLV is composed of 2
...
**Value:**
  Includes the following fields, repeated for each
  Association Type:

  **Reserved (2 bytes):**
    **MUST** be set to 0 on transmission and **MUST** be
       ignored on receipt.

  **Assoc-Type (2 bytes):**
    The Association Type ([Section 7.4](#iana_assoc_type)). The Association Types will be defined in future
       documents.

  **Start-Assoc-ID (2 bytes):**
    The "start association" identifier for the
       Operator-configured Association Range for the particular Association
       Type. The values 0 and 0xffff **MUST NOT** be used; on receipt of these
       values in the TLV, the session is rejected, and an error message is sent
       (see [Section 5.1](#pro)).

  **Range (2 bytes):**
    The number of associations marked for the
       Operator-configured Associations. Range **MUST** be greater than 0, and it
       **MUST** be such that (Start-Assoc-ID + Range) does not cross the
       largest Association ID value of 0xffff. If this condition is not satisfied, the session
       is rejected, and an error message is sent (see [Section 5.1](#pro)).
...
As specified in [RFC5440](#RFC5440), a PCEP peer that does not recognize the
...
The Operator-configured Association Range **SHOULD** be included for each
   ...
   TLV **MAY** be skipped, in which case the full range of Association IDs
   ...
   value for its Operator-configured Association Range.
...
The absence of the OP-CONF-ASSOC-RANGE TLV in an OPEN object **MUST** be
   ...
   Source is not a PCEP speaker, the default value for the Operator-configured Association Range is used for the Association Source.

If the Assoc-Type is not recognized or supported by the PCEP speaker, it **MUST** ignore that respective (Start-Assoc-ID + Range). If the Assoc-Type is recognized/supported but Start-Assoc-ID or Range is set incorrectly, the PCEP
   session **MUST** be rejected with Error-Type 1 and Error-value 1 (PCEP
   session establishment failure / Reception of an invalid Open
...
A given Assoc-Type **MAY** appear more than once in the
   ...
   TLV **MUST NOT** send overlapping ranges for an Association Type. If a PCEP
   peer receives overlapping ranges for an Association Type, it **MUST** consider
...
There may be cases where an Operator-configured Association was
   ...
   during session establishment. At this time, the local PCEP speaker **MUST**
   ...
   Association Source, it **MUST** generate an error (as described in [Section 6.4](#Rules)).
...
The ASSOCIATION Object-Type value is 1 for IPv4, and its format is
    shown in [Figure 3](#Association-Object-Fmt1):
...
The ASSOCIATION Object-Type value is 2 for IPv6, and its format is
    shown in [Figure 4](#Association-Object-Fmt2):
...
```
**Reserved (2 bytes):**
  **MUST** be set to 0 and ignored upon
      receipt.
...
**Association Type (2 bytes):**
  The Association Type
    ([Section 7.4](#iana_assoc_type)). The Association Types
...
**Association ID (2 bytes):**
  The identifier of the association
      ...
      scope of the Association Type and Association Source.

**Association Source:**
  Contains a valid IPv4 address (4 bytes)
      ...
      Association ID. See [Section 6.1.3](#AssocSrc) for details.

**Optional TLVs:**
  The optional TLVs follow the PCEP TLV format
      defined in [RFC5440](#RFC5440). This document defines two optional
      TLVs. Other documents can define more TLVs in the future.


<a name="Global-association-src"></a>
...
<a name="EXT-association"></a>
### Extended Association ID TLV

The Extended Association ID TLV is an optional TLV for use in
...
The Association Source field in the ASSOCIATION object is 
   ...
   otherwise, in which case the Association Source is set based on the local
    ...
    NMS, or any other valid IP address that scopes the Association ID for the Association Type.

<a name="id"></a>
### Unique Identification for an Association Group
...
The combination of the mandatory fields Association Type, Association
    ...
    Extended Association ID) are included, then they **MUST** be included in
    ...
    "association parameters". Note that the ASSOCIATION object **MAY** include
...
## Relationship to the RSVP ASSOCIATION Object

The format of the PCEP ASSOCIATION object defined in this document
       is aligned with the RSVP ASSOCIATION object [RFC6780](#RFC6780). Various Association Types related to RSVP
       association are defined in [RFC4872](#RFC4872), [RFC4873](#RFC4873), and [RFC7551](#RFC7551). The PCEP extensions
       that define new Association Types should clarify how the PCEP
       associations would work with RSVP associations and vice versa.

<a name="Object-Encoding"></a>
## Object Encoding in PCEP Messages

Message formats in this document are expressed using Routing BNF
     (RBNF) as used in [RFC5440](#RFC5440) and defined in [RFC5511](#RFC5511).

### Stateful PCEP Messages

The ASSOCIATION object **MAY** be carried in the PCUpd, PCRpt, and
    Path Computation Initiate (PCInitiate) messages.
...
When carried in a PCRpt message, this object is used to report the
    ...
    ([RFC8231](#RFC8231)), as well as whenever the
...
When an LSP is first reported to the PCE, the PCRpt message **MUST**
    ...
    PCRpt message **SHOULD** include only the associations that are being
    modified or removed.

The PCRpt message is defined in [RFC8231](#RFC8231)
...
When an LSP is delegated to a stateful PCE, the stateful PCE can create
    ...
    LSP from one or more association groups by setting the R flag to 1 in the ASSOCIATION object.
...
The PCUpd message is defined in [RFC8231](#RFC8231)
    and updated as shown below:
...
Unless a PCEP speaker wants to delete an association
    from an LSP or make changes to the association, it does not need to
    include the ASSOCIATION object in future stateful messages.

A PCE initiating a new LSP can also include the association groups that this LSP belongs to. This is done by including the ASSOCIATION object in a
    PCInitiate message. The PCInitiate message **MUST** include all the association groups that it belongs to.
    The PCInitiate message is defined in [RFC8281](#RFC8281)
    and updated as shown below:
...
When carried in a PCReq message, the ASSOCIATION object is used to
    ...
    object within a PCReq message **MUST** be set to 0 while sending and ignored
    on receipt.

The PCReq message is defined in [RFC5440](#RFC5440) and updated in
   [RFC8231](#RFC8231). It is further updated below for
   association groups:
...
Note that the LSP object **MAY** be present for the passive stateful
   PCE mode.
...
The PCRep message is defined in [RFC5440](#RFC5440) and updated in
   [RFC8231](#RFC8231). It is further updated below for association groups:
...
Association groups can be operator configured on the necessary PCEP
    ...
    part of the association. As described in [Section 6.1.4](#id), the
...
If a PCEP speaker does not recognize the ASSOCIATION object in the
    ...
    Object" as described in [RFC5440](#RFC5440). In the case of a PCReq
    message, the PCE would react based on the P flag as per [RFC5440](#RFC5440). If a PCEP speaker understands the ASSOCIATION object
    but does not support the Association Type, it **MUST** return a PCErr message
    ...
    consider this object malformed and process it as a malformed message [RFC5440](#RFC5440). On receiving a PCEP message with an ASSOCIATION
    ...
    association, it **MUST** return a PCErr message with Error-Type 26
     "Association Error" and Error-value 3 "Too many
    association groups". These numbers **MAY** be set by the operator or chosen
    based on a local policy.

If a PCE peer is unwilling or unable to process the ASSOCIATION object
    in the stateful message, it **MUST** return a PCErr message with the
    Error-Type "Not supported object" and follow the relevant procedures
    described in [RFC5440](#RFC5440). In the case of a PCReq message, the
    PCE would react based on the P flag as per [RFC5440](#RFC5440). On
    receiving a PCEP message with an ASSOCIATION object, if a PCEP speaker
    could not add the LSP to the association group for any reason, it **MUST**
    return a PCErr message with Error-Type 26 
    "Association Error" and Error-value 7 "Cannot join the association
    group".

If a PCEP speaker receives an ASSOCIATION object for an Operator-configured Association 
      and the Association ID is not in the Operator-configured Association Range
      for the Association Type and Association Source, it **MUST** return a PCErr message with Error-Type 26 
...
If a PCEP speaker receives an ASSOCIATION object in a PCReq message
      ...
      was not created dynamically, or was not learned from a PCEP peer), it **MUST** return a
...
If the association information (related to the association group as a
    ...
    information, it **MUST** return a PCErr message with Error-Type 26 "Association Error" and Error-value 5
...
If a PCEP speaker receives an ASSOCIATION object with the R bit set for
    ...
    is not known, it **MUST** return a PCErr message with Error-Type 26
     "Association Error" and Error-value 4
    "Association unknown".

The dynamic associations are cleared along with the LSP state
    information as per [RFC8231](#RFC8231). When a PCEP session is
    ...
    is considered empty and thus deleted.
...
Upon LSP delegation revocation, the PCC **MAY** clear the association
    ...
    perform this action in a make-before-break fashion (same as [RFC8231](#RFC8231)).
...
IANA has corrected the capitalization in the meaning for value 31 in the above registry
   to "Extended Association ID"; it was previously listed as "Extended
   Association Id".

IANA has made a new assignment in
   the existing "PCEP TLV Type Indicators" registry as follows:

**Table: ASSOC-Type-List PCEP TLV Type Indicator**
...
Per this document, IANA has created a subregistry of the
    ...
    assigned by Standards Action [RFC8126](#RFC8126).  Each bit is
...
**Table: New ASSOCIATION Flag Field**

| Bit | Description | Reference |
| --- | --- | --- |
| 15 | R (Removal) | RFC 8697 |

<a name="iana_assoc_type"></a>
## Association Type
...
Per this document, IANA has created a subregistry of the
        ...
        assigned by Standards Action [RFC8126](#RFC8126).  Each
...
**Table: New ASSOCIATION Type Field**
...
**Table: PCEP-ERROR Types and Names**
...
The security considerations described in [RFC8231](#RFC8231) and [RFC5440](#RFC5440) apply to the
   ...
   message as described in [Section 6.4](#Rules). An attacker could impact
   ...
   Transport Layer Security (TLS) [RFC8253](#RFC8253), as per the
   recommendations and best current practices in [RFC7525](#RFC7525), is
   **RECOMMENDED**.
...
Much of the information carried in the ASSOCIATION object as per this
  ...
  hide any sensitive business relationships.

# Manageability Considerations

All manageability requirements and considerations listed in [RFC5440](#RFC5440) and [RFC8231](#RFC8231) apply to PCEP protocol
  extensions defined in this document. In addition, requirements and
  considerations listed in this section apply.

## Control of Function and Policy

A PCE or PCC implementation **MUST** allow Operator-configured Associations and
  **SHOULD** allow the setting of the Operator-configured Association Range ([Section 3.4](#Range)) as described in this document.

## Information and Data Models

The PCEP YANG module is defined in [PCEP-YANG](#PCEP-YANG). In the
   future, this YANG module should be extended or augmented to provide
   the following additional information related to association groups.

An implementation **SHOULD** allow the operator to view the associations
  configured or created dynamically. Future implementations **SHOULD** allow
...
It might also be useful to find out how many associations for each
  Association Type currently exist and to know how many free Association IDs are available for a particular Association Type and source.

## Liveness Detection and Monitoring

Mechanisms defined in this document do not imply any new liveness detection and monitoring requirements in addition to those already listed in [RFC5440](#RFC5440).

## Verifying Correct Operation

Mechanisms defined in this document do not imply any new operation verification requirements in addition to those already listed in [RFC5440](#RFC5440) 
  and [RFC8231](#RFC8231).
...
Mechanisms defined in [RFC5440](#RFC5440) 
  and 
  [RFC8231](#RFC8231) also apply to PCEP extensions defined in this document.
...
<a name="RFC6780"></a> **[RFC6780]** L. Berger, F. Le Faucheur, A. Narayanan, "RSVP ASSOCIATION Object Extensions", RFC 6780, DOI 10.17487/RFC6780, October 2012. <https://www.rfc-editor.org/info/rfc6780> [Local MD](rfc6780.md)
...
<a name="RFC6007"></a> **[RFC6007]** I. Nishioka, D. King, "Use of the Synchronization VECtor (SVEC) List for Synchronized Dependent Path Computations", RFC 6007, DOI 10.17487/RFC6007, September 2010. <https://www.rfc-editor.org/info/rfc6007> [Local MD](rfc6007.md)
...
<a name="PCEP-YANG"></a> **[PCEP-YANG]** D Dhody, J Hardwick, V Beeram, J Tantsura, "A YANG Data Model for Path Computation Element Communications Protocol (PCEP)", Internet-Draft draft-ietf-pce-pcep-yang-13, October 2019. Work in Progress <https://tools.ietf.org/html/draft-ietf-pce-pcep-yang-13>

<a name="I-D.ietf-pce-stateful-path-protection"></a> **[PCE-PROTECTION]** H Ananthakrishnan, S Sivabalan, C Barth, I Minei, M Negi, "PCEP Extensions for Associating Working and Protection LSPs with Stateful PCE", Internet-Draft draft-ietf-pce-stateful-path-protection-11, September 2019. Work in Progress <https://tools.ietf.org/html/draft-ietf-pce-stateful-path-protection-11>

<a name="I-D.ietf-pce-association-diversity"></a> **[PCE-DIVERSITY]** S Litkowski, S Sivabalan, C Barth, M Negi, "Path Computation Element Communication Protocol (PCEP) Extension for LSP Diversity Constraint Signaling", Internet-Draft draft-ietf-pce-association-diversity-14, January 2020. Work in Progress <https://tools.ietf.org/html/draft-ietf-pce-association-diversity-14>

<a name="ex"></a>
# Example of an Operator-Configured Association Range
...
Consider an Association Type T1 (which allows both dynamic and
    ...
    <0x1000, 0xffff> is considered.

<a name="Acknowledgments"></a>
# Acknowledgments

We would like to thank Yuji Kamite and Joshua George for
     their contributions to this document. Thanks also to Venugopal Reddy,
     Cyril Margaria, Rakesh      Gandhi, Mike Koldychev, and Adrian Farrel for their useful comments.

We would like to thank Julien Meuric for shepherding this
   document and providing comments with text suggestions.

Thanks to Stig Venaas for the RTGDIR review comments.

Thanks to Alvaro Retana, Mirja Kühlewind, Martin Vigoureux,
      Barry Leiba, Eric Vyncke,
      Suresh Krishnan, and Benjamin Kaduk for the IESG comments.
...
- Address: Bantian, Longgang District, Shenzhen, 518129, China
...
- Address: 170 West Tasman Dr., San Jose, CA, 95134, United States of America
- Email: msiva@cisco.com
...
- Organization: Huawei
- Address: Divyashree Techno Park, Whitefield, Bangalore, KA, 560066, India
- Email: dhruv.ietf@gmail.com

**Yosuke Tanaka**

- Organization: NTT Communications Corporation
- Address: Granpark Tower 3-4-1 Shibaura, Minato-ku, Tokyo, 108-8118, Japan
- Email: yosuke.tanaka@ntt.com
