...
**Consensus:** true
**IPR:** trust200902
**Doc Name:** draft-ietf-ospf-te-link-attr-reuse-16
...
- [prev](https://datatracker.ietf.org/doc/draft-ietf-ospf-te-link-attr-reuse-16)
- [alternate](https://dx.doi.org/10.17487/rfc8920)
...
- [3. Existing Advertisement of Link Attributes](#LEG_ADV)
- [4. Advertisement of Link Attributes](#section-4)
  - [4.1. OSPFv2 Extended Link Opaque LSA and OSPFv3 E-Router-LSA](#section-4.1)
- [5. Advertisement of Application-Specific Values](#ADVAPPVAL)
- [6. Reused TE Link Attributes](#REUSED_ATTR)
  - [6.1. Shared Risk Link Group (SRLG)](#SRLG)
  - [6.2. Extended Metrics](#section-6.2)
  - [6.3. Administrative Group](#section-6.3)
  - [6.4. Traffic Engineering Metric](#section-6.4)
- [7. Maximum Link Bandwidth](#SPECIALMAXBANDW)
- [8. Considerations for Extended TE Metrics](#EXT_METRICS)
- [9. Local Interface IPv6 Address Sub-TLV](#LOCALIPV6ADDR)
- [10. Remote Interface IPv6 Address Sub-TLV](#REMOTEIPV6ADDR)
- [11. Attribute Advertisements and Enablement](#section-11)
- [12. Deployment Considerations](#section-12)
  - [12.1. Use of Legacy RSVP-TE LSA Advertisements](#LEGACY_OSPF)
  - [12.2. Interoperability, Backwards Compatibility, and Migration Concerns](#IBCMC)
    - [12.2.1. Multiple Applications: Common Attributes with  RSVP-TE](#MACARSVP)
    - [12.2.2. Multiple Applications: Some Attributes Not Shared with RSVP-TE](#MAALLNS)
    - [12.2.3. Interoperability with Legacy Routers](#LEGACY)
    - [12.2.4. Use of Application-Specific Advertisements for RSVP-TE](#APPRSVP)
- [13. Security Considerations](#section-13)
- [14. IANA Considerations](#IANA)
  - [14.1. OSPFv2](#OSPFV2IANA)
  - [14.2. OSPFv3](#OSPFV3IANA)
- [15. References](#section-15)
  - [15.1. Normative References](#section-15.1)
  - [15.2. Informative References](#section-15.2)
...
Advertisement of link attributes by the OSPFv2 [RFC2328](#RFC2328) and OSPFv3 [RFC5340](#RFC5340) protocols in support of traffic engineering (TE) was
       introduced by [RFC3630](#RFC3630) and [RFC5329](#RFC5329), respectively. It has been extended
       by [RFC4203](#RFC4203), [RFC7308](#RFC7308), and [RFC7471](#RFC7471).  Use
       ...
       referred to as RSVP-TE [RFC3209](#RFC3209).
...
In recent years, new applications have been introduced that have use
       ...
       Such applications include Segment Routing (SR) Policy [SEGMENT-ROUTING](#I-D.ietf-spring-segment-routing-policy) and
       Loop-Free Alternates (LFAs) [RFC5286](#RFC5286). This has introduced ambiguity in that if a
...
The key words "**MUST**", "**MUST NOT**", "**REQUIRED**", "**SHALL**", "**SHALL NOT**",
    ...
    BCP 14 [RFC2119](#RFC2119) [RFC8174](#RFC8174) when, and only when, they appear in all
...
There are existing advertisements used in support of RSVP-TE.  These
       ...
       Advertisement (LSA) [RFC3630](#RFC3630) and
       OSPFv3 Intra-Area-TE-LSA [RFC5329](#RFC5329). Additional RSVP-TE link attributes have been
       defined by [RFC4203](#RFC4203), [RFC7308](#RFC7308), and [RFC7471](#RFC7471).

Extended Link Opaque LSAs as defined in [RFC7684](#RFC7684) for OSPFv2 and  
  E-Router-LSAs [RFC8362](#RFC8362) for OSPFv3 are used to advertise link
  attributes that are used by applications other than RSVP-TE or GMPLS [RFC4203](#RFC4203). 
...
The following are the advantages of Extended Link Opaque LSAs as defined in [RFC7684](#RFC7684) 
   for OSPFv2 and E-Router-LSAs [RFC8362](#RFC8362) for OSPFv3 with respect 
   to the advertisement of link attributes originally defined for RSVP-TE when used in packet 
   networks and in GMPLS:

1. Advertisement of the link attributes does not make the link part of the RSVP-TE topology. 
      It avoids any conflicts and is fully compatible with [RFC3630](#RFC3630) and
      [RFC5329](#RFC5329).
2. The OSPFv2 TE Opaque LSA and OSPFv3 Intra-Area-TE-LSA remain
	   truly opaque to OSPFv2 and OSPFv3 as originally defined in [RFC3630](#RFC3630) and [RFC5329](#RFC5329), respectively. Their contents are not inspected
	   by OSPF, which instead acts as a pure transport.
3. There is a clear distinction between link attributes used by RSVP-TE and 
	link attributes used by other OSPFv2 or OSPFv3 applications.
4. All link attributes that are used by other applications are advertised in the Extended Link Opaque LSA in OSPFv2 [RFC7684](#RFC7684) or the OSPFv3 
	E-Router-LSA [RFC8362](#RFC8362) in OSPFv3.
...
The Extended Link Opaque LSA [RFC7684](#RFC7684) and E-Router-LSA 
   [RFC8362](#RFC8362) are used to advertise any link attributes used 
   for non-RSVP-TE applications in OSPFv2 or OSPFv3, respectively, including those that have 
   been originally defined for RSVP-TE applications (see [Section 6](#REUSED_ATTR)).

TE link attributes used for RSVP-TE/GMPLS continue to use the OSPFv2 TE Opaque LSA 
   [RFC3630](#RFC3630) and OSPFv3 Intra-Area-TE-LSA [RFC5329](#RFC5329).
...
The format of the link attribute TLVs that have been defined for
	 ...
	 registry [RFC7684](#RFC7684) and from the
	 "OSPFv3 Extended-LSA Sub-TLVs" registry [RFC8362](#RFC8362), as specified in [Section 14](#IANA).
...
To allow advertisement of the application-specific values of the link attribute, a new 
 ...
 of the OSPFv2 Extended Link TLV [RFC7684](#RFC7684) and OSPFv3 Router-Link TLV 
 [RFC8362](#RFC8362).
...
**Standard Application Identifier Bit Mask:**
  Optional
            ...
            registry, which is defined in [RFC8919](#RFC8919).  Current assignments are repeated here for
...
- Shared Risk Link Group [RFC4203](#RFC4203)
- Unidirectional Link Delay [RFC7471](#RFC7471)
- Min/Max Unidirectional Link Delay [RFC7471](#RFC7471)
- Unidirectional Delay Variation [RFC7471](#RFC7471)
- Unidirectional Link Loss [RFC7471](#RFC7471)
- Unidirectional Residual Bandwidth [RFC7471](#RFC7471)
- Unidirectional Available Bandwidth [RFC7471](#RFC7471)
- Unidirectional Utilized Bandwidth [RFC7471](#RFC7471)
- Administrative Group [RFC3630](#RFC3630)
- Extended Administrative Group [RFC7308](#RFC7308)
- TE Metric [RFC3630](#RFC3630)

<a name="REUSED_ATTR"></a>
# Reused TE Link Attributes

This section defines the use case and indicates the codepoints ([Section 14](#IANA)) from the "OSPFv2 Extended Link TLV
...
The SRLG of a link can be used in OSPF-calculated IPFRR (IP Fast Reroute)
   [RFC5714](#RFC5714) to compute a backup path 
...
To advertise the SRLG of the link in the OSPFv2 Extended Link TLV, the same format
   for the sub-TLV defined in [RFC4203](#RFC4203) is used with TLV 
   type 11. Similarly, for OSPFv3 to advertise the SRLG in the OSPFv3 Router-Link
   TLV, TLV type 12 is used.

## Extended Metrics

[RFC3630](#RFC3630) defines several link bandwidth types. [RFC7471](#RFC7471) 
...
To advertise extended link metrics in the OSPFv2 Extended Link TLV, the same format
    for the sub-TLVs defined in [RFC7471](#RFC7471) is used with the following 
    TLV types:
...
To advertise extended link metrics in the Router-Link TLV inside
	the OSPFv3 E-Router-LSA, the same format for the sub-TLVs defined in [RFC7471](#RFC7471) is used with the following 
    TLV types:
...
[RFC3630](#RFC3630) and [RFC7308](#RFC7308) define the Administrative Group and
...
To advertise the Administrative Group and Extended Administrative Group in the OSPFv2
    Extended Link TLV, the same format for the sub-TLVs defined in [RFC3630](#RFC3630)
    and [RFC7308](#RFC7308) is used with the following TLV types:
...
To advertise the Administrative Group and Extended Administrative Group in the OSPFv3
    Router-Link TLV, the same format for the sub-TLVs defined in [RFC3630](#RFC3630)
    and [RFC7308](#RFC7308) is used with the following TLV types:
...
[RFC3630](#RFC3630) defines the Traffic Engineering Metric.

To advertise the Traffic Engineering Metric in the OSPFv2 Extended Link TLV, 
  the same format for the sub-TLV defined in [RFC3630](#RFC3630)
...
Instead, it **MAY** be 
  ...
  LSA in OSPFv2 [RFC7684](#RFC7684) or as a sub-TLV of
...
To advertise the maximum link bandwidth in the OSPFv2 Extended Link TLV, the same
  format for the sub-TLV defined in [RFC3630](#RFC3630) is used with 
  TLV type 23.
...
<a name="EXT_METRICS"></a>
# Considerations for Extended TE Metrics

[RFC7471](#RFC7471) defines a number of dynamic performance metrics associated
...
The Local Interface IPv6 Address sub-TLV is an application-independent attribute of the
  link that is defined in [RFC5329](#RFC5329). Because it is an application-independent attribute, it **MUST NOT** be advertised in the ASLA sub-TLV.  Instead, it **MAY** be 
  advertised as a sub-TLV of the Router-Link TLV inside the OSPFv3 E-Router-LSA [RFC8362](#RFC8362).

To advertise the Local Interface IPv6 Address sub-TLV in the OSPFv3 Router-Link TLV, 
  the same format for the sub-TLV defined in [RFC5329](#RFC5329) is used with  
  TLV type 24.

<a name="REMOTEIPV6ADDR"></a>
...
In the case of RSVP-TE, the advertisement of application-specific
   ...
   the OSPFv2 TE Opaque LSA  [RFC3630](#RFC3630) and OSPFv3 Intra-Area-TE-LSA 
   [RFC5329](#RFC5329).
...
This document allows the advertisement of application-specific link
   ...
   Identifier Bit Mask are not present (see [Section 5](#ADVAPPVAL)).  
...
<a name="LEGACY_OSPF"></a>
## Use of Legacy RSVP-TE LSA Advertisements

Bit identifiers for standard applications are defined in [Section 5](#ADVAPPVAL).
...
Under the conditions defined above, implementations that support the
   ...
   the associated issues can be found in [Section 12.2](#IBCMC).
...
Existing deployments of RSVP-TE, SR Policy, and/or LFA utilize the
        legacy advertisements listed in [Section 3](#LEG_ADV). Routers that do not
...
2. Enable the use of the application-specific advertisements on
	   all routers.
...
Link attributes that are not allowed to be advertised in the ASLA sub-TLV, 
          ...
          OSPFv2 TE Opaque LSA [RFC3630](#RFC3630) and OSPFv3 Intra-Area-TE-LSA 
          [RFC5329](#RFC5329) and **MUST NOT** be advertised in the ASLA sub-TLV.

# Security Considerations

Existing security extensions as described in [RFC2328](#RFC2328), 
       [RFC5340](#RFC5340), and [RFC8362](#RFC8362) apply to extensions 
       ...
       mechanisms such as those specified in [RFC5709](#RFC5709), 
       [RFC7474](#RFC7474), [RFC4552](#RFC4552), or 
       [RFC7166](#RFC7166) **SHOULD** be
      used.
...
This document defines a new way to advertise link attributes.
      ...
      for example, [RFC3630](#RFC3630).  As the
...
The "OSPFv2 Extended Link TLV Sub-TLVs" registry [RFC7684](#RFC7684) defines sub-TLVs at any level of
...
The "OSPFv3 Extended-LSA Sub-TLVs" registry [RFC8362](#RFC8362) defines sub-TLVs at any level of nesting for OSPFv3
...
<a name="RFC5714"></a> **[RFC5714]** M. Shand, S. Bryant, "IP Fast Reroute Framework", RFC 5714, DOI 10.17487/RFC5714, January 2010. <https://www.rfc-editor.org/info/rfc5714> [Local MD](rfc5714.md)
...
- Address: 1194 N. Mathilda Ave, Sunnyvale, California, 94089, United States of America
- Email: jdrake@juniper.net
